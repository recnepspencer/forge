...
| **forge-core** | Shared error/policy language | `KernelError`, `GeometrySource` trait, `PolicyResult<T>`, `PolicyKind`, `PolicyQuery`, `ToleranceDecision`, `DecisionLog`, `OperationResult` | No business logic, no arena, no geometry math |
...
| **forge-view** | Representation layer (SDF/mesh) | — (scaffold) | — |

## "I Need To…" Decision Table

| I need to… | Put it in | Import from |
|------------|-----------|-------------|
| Compare floats for a geometry decision | `forge-geom` | `forge_math::sign::CertifiedTriSign` |
| Make a topology decision based on geometry | `forge-topo` | Call a `forge-geom` function, use its result |
| Add a tolerance threshold | `forge-kernel::core::context::ToleranceConfig` | Kernel passes it down |
...
| Build a halfedge mesh from BSP | `forge-kernel::mesh_builder` | `forge_kernel::mesh_builder::{build_halfedge_mesh, make_cube}` |
...
| Mark a signal node dirty | `forge-signal::eval` | `forge_signal::eval::mark_dirty` |
| Evaluate a signal graph | `forge-signal::eval` | `forge_signal::eval::evaluate` |

## Key API Paths

```
forge_math::sign::TriSign              — Neg / Zero / Pos
forge_math::sign::CertifiedTriSign     — Compile-time proof of exact evaluation
...
forge_geom::plane::Plane               — Analytic plane (normal + offset)
forge_geom::PlaneSet                   — Vec<Plane> implementing GeometrySource
forge_geom::ray::compute_ray_plane_intersection  — Ray-plane hit point
forge_geom::ray::dominant_projection_axes        — Best 2D projection for a face
...
forge_signal::eval::{mark_dirty, evaluate}  — Push/pull evaluation engine
...
forge_kernel::mesh_builder::{build_halfedge_mesh, make_cube}  — BSP→mesh
forge_kernel::boolean::{BooleanInput, BooleanOp, BooleanResult, execute_boolean}

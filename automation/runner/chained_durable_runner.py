...
def main() -> int:
    ...
    from runner.facade.runtime_state import refresh_projection_for_run
    ...
    while True:
        ...
        projection = refresh_projection_for_run(args.current_run_id)
        ...
        if projection["completed_at"] is not None:
            print(f"starting next run: {args.next_run_id} from {next_config}", flush=True)
            return start_run(
                next_config,
                args.next_run_id,
                args.loop,
        ...
        print(f"waiting: {args.current_run_id} is at {projection['current']}", flush=True)

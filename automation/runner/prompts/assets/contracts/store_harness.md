...
For S.4.5, treat the work as building a usable physical database test operating
system. Keep the golden-path authoring API ergonomic, and put strict
Proof/Foundational/canonical/evidence machinery underneath Store-owned
authority. Do not implement future S.5-S.12 product behavior; use readiness
shape probes, typed extension slots, and explicit non-claim evidence where the
spec requires them.
...
current phase status, prompt kind, and open findings to correct it so the next
prompt resumes from the real state.

...
- Fix the full visible blocking finding family, not just the first file that
  failed.
...
- Distinguish clearly between:
  - a public ordinary-lane blocker
  - an immediate trusted-owner blocker
  - trusted-internal hardening residue
- Do not keep widening the repair if the remaining issue is only
  trusted-internal hardening residue and the public ordinary lane is already
...
- If review named multiple independent leaks in the same cutover family, close
  the blocking ones in this turn unless one of them is genuinely blocked by a
  deeper seam you are already replacing.
- If the remaining leak is not reachable from the admitted ordinary caller path
  and does not leave an immediate trusted-owner bypass, record it as follow-up
  hardening in chat rather than reopening the same cutover endlessly.
...
Completion rule for this turn:

- the repair is complete when the phase's admitted public lane is singular,
  the displaced ordinary bypasses are removed or mechanically denied, and the
  hostile proof exercises that real lane
- do not keep the phase open solely to eliminate every conceivable
  trusted-internal reconstruction path unless the spec explicitly makes that a

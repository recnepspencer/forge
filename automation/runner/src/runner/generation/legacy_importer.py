...
from runner.authority.run_identity import RuntimePaths, acquire_active_run_lock
from runner.phase_programs.transition_rules import first_turn
...
def import_legacy_run(
    ...
    with acquire_active_run_lock(paths):
        ...
        if not imported_current_is_compatible(projection, config, legacy.get("current")):
            raise ValueError(
                f"import projected current {projection['current']!r} but legacy current was {legacy.get('current')!r}"
            )
        return active_run_id


def import_phase_events(paths: RuntimePaths, phase: dict[str, Any], current: dict[str, Any] | None, append_runtime_event) -> None:
    ...
    if current_turn == "implement":
        return
    append_runtime_event(paths, "implementation_completed", phase_id=phase_id, turn="implement", payload=phase_payload(notes))
    if current_turn == "review":
        return
    if status == "regressed" and qa_status == "failed":
        append_runtime_event(paths, "review_failed", phase_id=phase_id, turn="review", payload=phase_payload(notes))
    ...
    if current_turn in {"test_repair_plan", "test_repair_implement"}:
        append_runtime_event(paths, "test_review_failed", phase_id=phase_id, turn="test_review", payload=phase_payload(notes))
        if current_turn == "test_repair_plan":
            return
        append_runtime_event(paths, "test_repair_plan_posted", phase_id=phase_id, turn="test_repair_plan", payload=phase_payload(notes))
        return
    append_runtime_event(paths, "test_review_passed", phase_id=phase_id, turn="test_review", payload=phase_payload(notes))
    ...
    append_runtime_event(paths, "code_quality_review_passed", phase_id=phase_id, turn="code_quality_review", payload=phase_payload(notes))
...
def imported_current_is_compatible(
    ...
    config: dict[str, Any],
    legacy_current: dict[str, Any] | None,
) -> bool:
    ...
    if legacy_phase["status"] != "complete" or legacy_phase["qa_status"] != "passed":
        return False
    return projection["current"] == first_unfinished_phase(projection, config)
...
def first_unfinished_phase(projection: dict[str, Any], config: dict[str, Any]) -> dict[str, Any] | None:
    for phase in projection["phases"]:
        if phase["status"] != "complete" or phase["qa_status"] != "passed":
            return {"phase": phase["id"], "turn": first_turn(config, phase)}
    return None

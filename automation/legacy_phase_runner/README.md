# Legacy JSON Phase Runner

This runner now uses an append-only event log as authority.

- Static milestone config lives in `automation/legacy_phase_runner/config/`
- Authoritative runtime events live in `automation/legacy_phase_runner/runtime/events/`
- Operator-readable derived projections live in `automation/legacy_phase_runner/runtime/projections/`
- Optional raw Codex JSON stream logs live in `automation/legacy_phase_runner/runtime/logs/`
...
```powershell
python automation/legacy_phase_runner/runner.py validate `
  automation/legacy_phase_runner/config/automation-runner-milestone-1.json
```

Start a new run:

```powershell
python automation/legacy_phase_runner/runner.py start `
  automation/legacy_phase_runner/config/automation-runner-milestone-1.json `
  --run-id automation-runner-m1-legacy `
  --loop `
  --sleep-seconds 30
```
...
```powershell
python automation/legacy_phase_runner/runner.py resume automation-runner-m1-legacy `
  --loop `
  --sleep-seconds 30
```
...
```powershell
python automation/legacy_phase_runner/runner.py status automation-runner-m1-legacy
```

Stop a run without deleting authority:

```powershell
python automation/legacy_phase_runner/runner.py stop automation-runner-m1-legacy `
...
```powershell
python automation/legacy_phase_runner/runner.py import-legacy `
  old-state.json `
  automation/legacy_phase_runner/config/automation-runner-milestone-1.json `
  --run-id automation-runner-m1-legacy-import
```

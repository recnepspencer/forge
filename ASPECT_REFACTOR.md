...
Rules and reference for the `forge-relational` aspect-native refactor.
...
- `crates/forge-foundational/docs/aspect-contracts-values-and-authoritative-state/README.md`
...
plan is written from real `forge-foundational` meaning instead of remembered or
invented relational semantics.

## Core Rules

- Native aspect meaning is `forge-foundational` meaning.
...
`forge-foundational` is not an optional helper crate. It is the **only**
authority for aspect meaning inside migrated `forge-relational` code.
...
| --- | --- | --- |
| Declare aspect law | `forge_foundational::aspects()` → contract builders | Relational aspect key enums, `StructuredDataContract*`, payload schema declarations |
...
| Ingress from JSON/payload bytes | `forge_foundational::compatibility()` lowering | `structured_field_observation`, document rescans, loose object merge |
| Deny/ admit transitions | `forge_proof::TransitionOutcome` on foundational APIs | Invented `Result` errors that hide denial categories |
...
Import from `forge_foundational::facade` or the documented front doors. Do not
...
- wire `forge_foundational` into the live choke point now
...
2. Read this file.
3. Immediately read `crates/forge-foundational/docs/aspect-contracts-values-and-authoritative-state/README.md`.
...
Reference layout under `crates/forge-relational/src`. Use to choose the next

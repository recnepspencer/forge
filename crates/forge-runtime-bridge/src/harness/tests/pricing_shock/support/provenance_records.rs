...
pub(in crate::harness::tests::pricing_shock) fn snapshot_with_corrupted_provenance_field(
    ...
    for record in snapshot.records() {
        if record.correlation_id() == &target_correlation_id {
            records.push(SnapshotReadRecord::for_request(
                &target_request,
                forge_foundational::facade::AspectValue::String(replacement_content.clone().into()),
            ));
            replaced = true;
        } else {
...
pub(in crate::harness::tests::pricing_shock) fn pricing_aspect_value_text(
    ...
    match value {
        forge_foundational::facade::AspectValue::String(value) => match value {
            forge_foundational::facade::InternedString::Raw(text) => text.clone(),
            forge_foundational::facade::InternedString::Symbol(symbol) => {
                format!("symbol:{}", symbol.0)
            }
        },
        forge_foundational::facade::AspectValue::Int64(value) => value.to_string(),

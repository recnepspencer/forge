...
#[derive(Clone, Copy)]
struct ExpectedLaneEvidence {
    outcome: Option<BridgeSubscriptionCertificationComparisonOutcome>,
    primary_failure_boundary: Option<BridgeSubscriptionCertificationFailureBoundary>,
}

fn expected_evidence_for_lane(
    lane_kind: BridgeSubscriptionReferenceWorkloadLaneKind,
    ...
    Some(match lane_kind {
        ...
        Lane::IncompatibleSharingRejection => ExpectedLaneEvidence {
            outcome: Some(Outcome::RejectedAtExpectedBoundary),
            primary_failure_boundary: Some(Boundary::IllegalSharingReuse),
        },
        Lane::StaleCheckpointRejection => ExpectedLaneEvidence {
            outcome: Some(Outcome::RejectedAtExpectedBoundary),
            primary_failure_boundary: Some(Boundary::CheckpointIncompatibility),
        },

...
impl BridgeExistingTruthBindingBundle {
    pub fn direct_entity(
        ...
        let resolved_entity_identity: Arc<str> = resolved_entity_identity.into();
        ...
        let binding_digest = aggregate_digest(
            ...
            [
                ...
                format!("resolved:{}", resolved_entity_identity.as_ref()),
                format!("collection:{target_collection_label}"),
            ],
        );
        Self {
            family: BridgeExistingTruthBindingFamily::DirectEntityIdentity,

...
mod tests {
    ...
    use crate::traverse::{VertexRingIterator, FaceEdgeIterator};
    ...
    fn bowtie_vertex_traversal() {
        ...
        let ring: Vec<_> = VertexRingIterator::new(draft.arena(), v_center).unwrap()
            .map(|r| r.unwrap()).collect();
    ...
    fn commutative_dag_fuzzer() {
        ...
        let edges: Vec<_> = FaceEdgeIterator::new(seed_state.arena(), mvf.face).unwrap()
            .map(|r| r.unwrap()).collect();
    ...
    fn mef_handles_ambiguous_vertex_selection() {
        ...
        let f1_edges_count = FaceEdgeIterator::new(draft.arena(), mvf.face).unwrap().count();
        let f2_edges_count = FaceEdgeIterator::new(draft.arena(), mef.new_face).unwrap().count();
        assert_eq!(f1_edges_count, 3, "Original face must be a triangle after diagonal split");
        assert_eq!(f2_edges_count, 3, "New face must be a triangle after diagonal split");
    ...
    fn ship_of_theseus_rewire() {
        ...
        let edges_before: Vec<_> = FaceEdgeIterator::new(state_before.arena(), original_face).unwrap()
            .map(|r| r.unwrap()).collect();
        ...
        let edges: Vec<_> = FaceEdgeIterator::new(state_before.arena(), original_face).unwrap()
            .map(|r| r.unwrap()).collect();

...
use crate::traverse::{FaceEdgeIterator, VertexRingIterator, face_edge_count, edge_faces};
...
fn build_tetrahedron_via_euler_operators() {
    ...
    let se3_face = {
        let edges = FaceEdgeIterator::new(draft.arena(), mef1.new_face).unwrap();
        let mut found = None;
        for eid_res in edges {
...
fn kv15_validation_catches_broken_twins() {
    ...
    draft.arena_mut().get_half_edge_mut(mvf.half_edge).unwrap().set_twin(
...
fn traversal_vertex_ring_for_seed() {
    ...
    let mvf = apply_op(&mut draft, MakeVertexFace).unwrap().into_value();
    let ring: Vec<_> = VertexRingIterator::new(draft.arena(), mvf.vertex).unwrap()
...
fn kv16_identical_sequence_produces_identical_lineage() {
    let run_sequence = || {
        ...
        let vertex_hash = draft.arena().get_vertex(mvf.vertex).unwrap()
            .lineage().unwrap().get_ancestry_hash();

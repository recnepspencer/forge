...
fn brutal_bio_mesh_high_valence_pole_and_megaloop() {
    ...
    for _ in 0..15_000 {
        let se = apply_op(&mut draft, SplitEdge { edge: current_edge, parameter: 0.5 })
            .unwrap().into_value();
...
fn brutal_aerospace_sliver_churn_generational_integrity() {
    ...
    for i in 0..500 {
        ...
        let se = logged_op(&format!("SE[{i}]"), apply_op(&mut draft, SplitEdge {
            edge: boundary_edge, parameter: 0.1,
        })).unwrap();

        let face_id = draft.arena().get_half_edge(boundary_edge).unwrap().face();
        let mef = logged_op(&format!("MEF[{i}]"), apply_op(&mut draft, MakeEdgeFace {
            vertex_a: mvf.vertex,
            vertex_b: se.new_vertex,
            face: face_id,
        })).unwrap();
        ...
        logged_op(&format!("JF[{i}]"),  apply_op(&mut draft, JoinFaces { edge: mef.half_edge_ab })).unwrap();
        logged_op(&format!("KEV[{i}]"), apply_op(&mut draft, KillEdgeVertex { edge: se.he_am })).unwrap();

        assert!(draft.arena().get_face(doomed_face).is_err(),   "stale face must be rejected");
        assert!(draft.arena().get_vertex(doomed_vertex).is_err(), "stale vertex must be rejected");
    }

    let final_state = draft.commit().expect("churn caused silent topology corruption");
...
fn brutal_dag_determinism_path_independence() {
    let path_a_hash = {
        ...
        let mef1 = apply_op(&mut draft, MakeEdgeFace { vertex_a: mvf.vertex, vertex_b: se1.new_vertex, face: mvf.face }).unwrap().into_value();

        let _se_top    = apply_op(&mut draft, SplitEdge { edge: mef1.half_edge_ab, parameter: 0.5 }).unwrap().into_value();
        let _se_bottom = apply_op(&mut draft, SplitEdge { edge: mef1.half_edge_ba, parameter: 0.5 }).unwrap().into_value();
    ...
    assert_eq!(path_a_hash, path_b_hash, "operation order must not alter the structural hash");
}

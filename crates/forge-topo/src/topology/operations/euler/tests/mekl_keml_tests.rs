...
fn keml_splits_loop() {
    ...
    let keml = apply_op(&mut draft, KillEdgeMakeLoop { edge: mekl.he_ab })
        .unwrap()
        .into_value();
...
fn keml_euler_delta() {
    ...
    apply_op(&mut draft, KillEdgeMakeLoop { edge: mekl.he_ab })
        .unwrap()
        .into_value();
...
fn mekl_keml_roundtrip() {
    ...
    let _keml = apply_op(&mut draft, KillEdgeMakeLoop { edge: mekl.he_ab })
        .unwrap()
        .into_value();
...
fn mekl_keml_on_multi_hole_face() {
    ...
    let inner_loop2 = arena.insert_loop(crate::arena::LoopData::new(he67, face), None);
    arena
        .get_face_mut(face)
        .unwrap()

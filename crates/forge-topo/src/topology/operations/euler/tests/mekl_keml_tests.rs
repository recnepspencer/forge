...
use crate::euler::make_edge_kill_loop::MakeEdgeKillLoop;
use crate::euler::kill_edge_make_loop::KillEdgeMakeLoop;
...
fn mekl_absorbs_inner_loop() {
    ...
    let mekl = apply_op(&mut draft, MakeEdgeKillLoop {
        he_a: outer_he,
        he_b: inner_he,
    }).unwrap().into_value();
    ...
    let loop_size = FaceEdgeIterator::new(draft.arena(), face)
        .unwrap()
        .count();
    assert_eq!(loop_size, 8,
        "outer(3) + inner(3) + 2 bridge halfedges = 8");
}
...
fn mekl_euler_delta() {
    ...
    apply_op(&mut draft, MakeEdgeKillLoop {
        he_a: outer_he,
        he_b: inner_he,
    }).unwrap().into_value();

    assert_eq!(draft.arena().vertex_count(), vtx_before, "V delta must be 0");
    assert_eq!(draft.arena().half_edge_count(), he_before + 2, "HE delta must be +2");
    assert_eq!(draft.arena().edge_count(), edge_before + 1, "E delta must be +1");
    assert_eq!(draft.arena().loop_count(), loop_before - 1, "L delta must be -1");
    let _ = face;
}
...
fn mekl_rejects_same_loop() {
    ...
    let (face, outer_he, _inner_he, _inner_loop, _verts) =
        build_face_with_hole(&mut draft);

    let outer_he_next = draft.arena().get_half_edge(outer_he).unwrap().next();

    let result = apply_op(&mut draft, MakeEdgeKillLoop {
        he_a: outer_he,
        he_b: outer_he_next,
    });
...
fn keml_splits_loop() {
    ...
    let keml = apply_op(&mut draft, KillEdgeMakeLoop {
        edge: mekl.he_ab,
    }).unwrap().into_value();
    ...
    loop {
        ...
        if current == new_loop_start { break; }
    }
    assert_eq!(count, 3, "restored inner loop must have 3 halfedges (triangle)");
}
...
fn keml_euler_delta() {
    ...
    apply_op(&mut draft, KillEdgeMakeLoop {
        edge: mekl.he_ab,
    }).unwrap().into_value();
...
fn mekl_keml_roundtrip() {
    ...
    let _keml = apply_op(&mut draft, KillEdgeMakeLoop {
        edge: mekl.he_ab,
    }).unwrap().into_value();

    assert_eq!(draft.arena().vertex_count(), v_before, "V must be preserved");
    assert_eq!(draft.arena().half_edge_count(), he_before, "HE must be preserved");
    ...
    assert_eq!(
        draft.arena().get_face(face).unwrap().inner_loop_count(), inner_before,
...
fn mekl_keml_on_multi_hole_face() {
    ...
    let (face, outer_he, inner_he1, _inner_loop1, _verts1) =
        build_face_with_hole(&mut draft);
    ...
    let v6 = arena.insert_vertex(crate::arena::VertexData::new(placeholder_he), None);
    ...
    let inner_loop2 = arena.insert_loop(
        crate::arena::LoopData::new(he67, face), None);
    arena.get_face_mut(face).unwrap().add_inner_loop(inner_loop2);

    assert_eq!(draft.arena().get_face(face).unwrap().inner_loop_count(), 2,

...
use crate::euler::kill_face_vertex::KillFaceVertex;
...
fn kfv_destroys_face_vertex() {
    ...
    apply_op(&mut draft, KillFaceVertex { face: mfv.face })
        .unwrap()
        .into_value();
...
fn kfv_rejects_non_isolated_face() {
    ...
    apply_op(
        &mut draft,
        crate::euler::split_edge::SplitEdge {
            edge: mvf.half_edge,
            parameter: 0.5,
        },
    )
    ...
    let result = apply_op(&mut draft, KillFaceVertex { face: mvf.face });
    assert!(
        result.is_err(),

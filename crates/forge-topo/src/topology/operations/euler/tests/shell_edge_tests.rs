...
fn mvf_shell_and_edge_are_live_and_wired() {
    ...
    assert!(draft.arena().get_shell(shell_id).is_ok(), "face.shell must be a live ShellId");

    let edge_id = draft.arena().get_half_edge(out.half_edge).unwrap().edge();
    assert!(draft.arena().get_edge(edge_id).is_ok(), "halfedge.edge must be a live EdgeId");

    let rep = draft.arena().get_edge(edge_id).unwrap().half_edge();
    assert_eq!(rep, out.half_edge, "EdgeData.half_edge must round-trip to the seed halfedge");
}
...
fn mef_inherits_shell_does_not_create_new_one() {
    ...
    assert_eq!(draft.arena().shell_count(), 1, "MEF must not create a new Shell");

    let shell_a = draft.arena().get_face(mvf.face).unwrap().shell();
    let shell_b = draft.arena().get_face(mef.new_face).unwrap().shell();
    assert_eq!(shell_a, original_shell, "original face must still reference the original Shell");
    assert_eq!(shell_b, original_shell, "new MEF face must inherit the same Shell");
}
...
fn twin_halfedges_share_edge_id() {
    ...
    assert_eq!(ab_data.edge(), ba_data.edge(), "twin halfedges must share the same EdgeId");
...
fn mef_then_join_faces_is_exact_inverse() {
    ...
    let mef_edge_id = draft.arena().get_half_edge(mef.half_edge_ab).unwrap().edge();
    assert_eq!(draft.arena().edge_count(), edges_before + 1, "MEF must add 1 Edge");

    apply_op(&mut draft, JoinFaces { edge: mef.half_edge_ab }).unwrap().into_value();

    assert_eq!(draft.arena().edge_count(),  edges_before,  "JoinFaces must remove 1 Edge (net zero)");
    assert_eq!(draft.arena().shell_count(), shells_before, "Shell count must be unchanged");
    assert!(draft.arena().get_edge(mef_edge_id).is_err(),  "MEF EdgeId must be dead after JoinFaces");
}
...
fn split_edge_then_kev_is_exact_inverse() {
    ...
    assert_eq!(draft.arena().edge_count(), edges_before + 1, "SplitEdge must add 1 Edge");

    apply_op(&mut draft, KillEdgeVertex { edge: se2.he_am }).unwrap().into_value();

    assert_eq!(draft.arena().edge_count(), edges_before, "KEV must remove 1 Edge (net zero)");
    assert!(draft.arena().get_edge(split_edge_id).is_err(), "SplitEdge EdgeId must be dead after KEV");
}

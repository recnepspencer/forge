...
fn kv17_split_edge_children_carry_parent_ancestry() {
    ...
    let child_vertex_lineage = draft.arena().get_vertex(se.new_vertex).unwrap().lineage().unwrap();
    let child_he_lineage     = draft.arena().get_half_edge(se.he_mb).unwrap().lineage().unwrap();

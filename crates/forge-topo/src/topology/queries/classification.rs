...
use super::traverse::{
    ...
    is_boundary_edge,
    radial_valence,
    vertex_neighborhood_orbits,
    FaceAllEdgesIterator,
    VertexRingIterator,
};

/// Return all distinct faces adjacent to `face` across shared geometric edges.
pub fn face_adjacent_faces(arena: &TopologyArena, face: FaceId) -> Result<Vec<FaceId>, KernelError> {
    arena.get_face(face)?;
...
pub fn is_outer_loop(arena: &TopologyArena, face: FaceId, loop_id: LoopId) -> Result<bool, KernelError> {
    Ok(arena.get_face(face)?.outer_loop() == loop_id)
}

/// True when `loop_id` is one of the face inner loops.
pub fn is_inner_loop(arena: &TopologyArena, face: FaceId, loop_id: LoopId) -> Result<bool, KernelError> {
    if is_outer_loop(arena, face, loop_id)? {
        return Ok(false);
    }

    let loops = face_loops(arena, face)?;
    Ok(loops.into_iter().skip(1).any(|candidate| candidate == loop_id))
}
...
mod tests {
    ...
    fn seed_edge_and_vertex_classification_is_boundary_and_manifold_vertex() {
        ...
        assert_eq!(vertex_faces(state.arena(), mvf.vertex).unwrap(), vec![mvf.face]);
    }
    ...
    fn face_adjacency_detects_split_faces() {
        ...
        assert_eq!(face_adjacent_faces(state.arena(), mvf.face).unwrap(), vec![mef.new_face]);
        assert_eq!(face_adjacent_faces(state.arena(), mef.new_face).unwrap(), vec![mvf.face]);
    }

    ...
    for (vid, _) in arena.iter_vertices() {
        ...
        for (he_id, he_data) in arena.iter_half_edges() {
            if he_data.origin() == vid {
                ...
                let out_idx = disks.iter().position(|d| d.contains(&he_id)).unwrap_or(usize::MAX);
                ...
                let in_idx = disks.iter().position(|d| d.contains(&incoming_twin_candidate)).unwrap_or(usize::MAX);

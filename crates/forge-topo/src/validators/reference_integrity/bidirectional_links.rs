    ...
    for (vertex_id, vertex_data) in arena.iter_vertices() {
        ...
        let he_data = arena.get_half_edge(rep_he).map_err(|_| {
            vf(
                "bidirectional_links",
                format!(
                    "Vertex {} primary-disk HE {} is deleted",
                    vertex_id.index(),
                    rep_he.index()
                ),
            )
    ...
    for (edge_id, edge_data) in arena.iter_edges() {
        ...
        let he_data = arena.get_half_edge(rep_he).map_err(|_| {
            vf(
                "bidirectional_links",
                format!(
                    "Edge {} representative HE {} is deleted",
                    edge_id.index(),
                    rep_he.index()
                ),
            )
    ...
    for (loop_id, loop_data) in arena.iter_loops() {
        ...
        let he_data = arena.get_half_edge(rep_he).map_err(|_| {
            vf(
                "bidirectional_links",
                format!(
                    "Loop {} representative HE {} is deleted",
                    loop_id.index(),
                    rep_he.index()
                ),
            )
    ...
    for (shell_id, shell_data) in arena.iter_shells() {
        ...
        let face_data = arena.get_face(rep_face).map_err(|_| {
            vf(
                "bidirectional_links",
                format!(
                    "Shell {} representative face {} is deleted",
                    shell_id.index(),
                    rep_face.index()
                ),
            )

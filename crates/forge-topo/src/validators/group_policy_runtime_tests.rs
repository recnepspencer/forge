...
use super::*;
use crate::b_rep::{TopologyArena, ShellData, ShellKind, ShellOrientation};
...
fn draft_auto_derives_policy_from_shell_metadata() {
    ...
    assert!(
        !policy.should_run(InvariantGroup::ShellClosure, ValidationCheckpoint::PostCommit),
        "Sheet topology must skip ShellClosure even at PostCommit",
    );

    // Sheet topology: PointerCoherence should still run
    assert!(
        policy.should_run(InvariantGroup::PointerCoherence, ValidationCheckpoint::PerOp),
...
fn verify_catches_stale_solid_kind_on_open_topology() {
    ...
    draft.arena_mut().get_shell_mut(shell_id).unwrap()
...
fn verify_catches_hidden_hole_on_solid() {
    ...
    assert!(result.is_err(), "Must panic when any face in a Solid shell has boundary edges");
}
...
fn verify_catches_watertight_sheet_as_missed_opportunity() {
    ...
    let lump = draft.arena_mut().insert_lump(crate::b_rep::LumpData::new(body));
    let region = draft.arena_mut().insert_region(crate::b_rep::RegionData::new(lump));
    ...
    assert!(result.is_err(), "Must panic when a Sheet has NO boundary edges (accidental solid)");
}
...
fn verify_catches_face_in_wire_shell() {
    ...
    assert!(result.is_err(), "Must panic when a Wire shell contains faces");
}

        ...
        paste::paste! {
            impl TopologyArena {
                ...
                pub fn [<insert_ $m>](&mut self, data: $data) -> $id {
                    let (index, gen) = Self::insert_slot(&mut self.connectivity.$slots, &mut self.connectivity.$free_head, data);
                    self.connectivity.$count += 1;
                    <$id>::new(index, gen)
                }
                ...
                pub fn [<remove_ $m>](&mut self, id: $id) -> Result<$data, KernelError> {
                    ...
                    slot.generation += 1;
                    slot.next_free = self.connectivity.$free_head;
                    self.connectivity.$free_head = Some(id.index());
                    self.connectivity.$count -= 1;
                    Ok(data)
                }
            }
...
impl TopologyArena {
    ...
    pub fn insert_shell(&mut self, data: ShellData) -> ShellId {
        let (index, gen) = Self::insert_slot(
            &mut self.connectivity.shell_slots,
            &mut self.connectivity.free_shell_head,
            data,
        );
        self.connectivity.active_shell_count += 1;
        self.grow_shell_sidecars(self.connectivity.shell_slots.len());
    ...
    pub fn remove_shell(&mut self, id: ShellId) -> Result<ShellData, KernelError> {
        ...
        let data = slot.data.take().ok_or_else(|| {
            cold_err_deleted("Shell", id.index(), id.generation(), slot.generation)
        })?;
        slot.generation += 1;
        slot.next_free = self.connectivity.free_shell_head;
        self.connectivity.free_shell_head = Some(id.index());
        self.connectivity.active_shell_count -= 1;
...
impl TopologyArena {
    ...
    pub fn insert_loop(&mut self, data: LoopData) -> LoopId {
        let (index, gen) = Self::insert_slot(
            &mut self.connectivity.loop_slots,
            &mut self.connectivity.free_loop_head,
            data,
        );
        self.connectivity.active_loop_count += 1;
    ...
    pub fn remove_loop(&mut self, id: LoopId) -> Result<LoopData, KernelError> {
        ...
        let data = slot.data.take().ok_or_else(|| {
            cold_err_deleted("Loop", id.index(), id.generation(), slot.generation)
        })?;
        slot.generation += 1;
        slot.next_free = self.connectivity.free_loop_head;
        self.connectivity.free_loop_head = Some(id.index());
        self.connectivity.active_loop_count -= 1;
        Ok(data)
    }
}
...
impl TopologyArena {
    ...
    pub fn insert_vertex(&mut self, data: VertexData) -> VertexId {
        let (index, gen) = Self::insert_slot(
            &mut self.connectivity.vertex_slots,
            &mut self.connectivity.free_vertex_head,
            data,
        );
        self.connectivity.active_vertex_count += 1;
        self.grow_vertex_sidecars(self.connectivity.vertex_slots.len());
    ...
    pub fn remove_vertex(&mut self, id: VertexId) -> Result<VertexData, KernelError> {
        ...
        let data = slot.data.take().ok_or_else(|| {
            cold_err_deleted("Vertex", id.index(), id.generation(), slot.generation)
        })?;
        slot.generation += 1;
        slot.next_free = self.connectivity.free_vertex_head;
        self.connectivity.free_vertex_head = Some(id.index());
        self.connectivity.active_vertex_count -= 1;
        self.clear_vertex_sidecar(id.index() as usize);
        self.metadata.nmt_extra_disks.remove(&id);
        Ok(data)
    }
}
...
impl TopologyArena {
    ...
    pub fn insert_edge(&mut self, data: EdgeData) -> EdgeId {
        let (index, gen) = Self::insert_slot(
            &mut self.connectivity.edge_slots,
            &mut self.connectivity.free_edge_head,
            data,
        );
        self.connectivity.active_edge_count += 1;
        self.grow_edge_sidecars(self.connectivity.edge_slots.len());
    ...
    pub fn remove_edge(&mut self, id: EdgeId) -> Result<EdgeData, KernelError> {
        ...
        let data = slot.data.take().ok_or_else(|| {
            cold_err_deleted("Edge", id.index(), id.generation(), slot.generation)
        })?;
        slot.generation += 1;
        slot.next_free = self.connectivity.free_edge_head;
        self.connectivity.free_edge_head = Some(id.index());
        self.connectivity.active_edge_count -= 1;
...
impl TopologyArena {
    ...
    pub fn insert_face(&mut self, data: FaceData) -> FaceId {
        ...
        let (index, gen) = Self::insert_slot(
            &mut self.connectivity.face_slots,
            &mut self.connectivity.free_face_head,
            data,
        );
        self.connectivity.active_face_count += 1;
    ...
    pub fn remove_face(&mut self, id: FaceId) -> Result<FaceData, KernelError> {
        ...
        let data = slot.data.take().ok_or_else(|| {
            cold_err_deleted("Face", id.index(), id.generation(), slot.generation)
        })?;
        slot.generation += 1;
        slot.next_free = self.connectivity.free_face_head;
        self.connectivity.free_face_head = Some(id.index());
        self.connectivity.active_face_count -= 1;
...
impl TopologyArena {
    ...
    pub fn insert_half_edge(&mut self, data: HalfEdgeData) -> HalfEdgeId {
        ...
        let (index, gen) = Self::insert_slot(
            &mut self.connectivity.half_edge_slots,
            &mut self.connectivity.free_half_edge_head,
            data,
        );
        self.connectivity.active_half_edge_count += 1;
        let id = HalfEdgeId::new(index, gen);
        self.index_add_halfedge(id, face, origin);
        self.grow_halfedge_sidecars(self.connectivity.half_edge_slots.len());
    ...
    pub fn remove_half_edge(&mut self, id: HalfEdgeId) -> Result<HalfEdgeData, KernelError> {
        ...
        let data = slot.data.take().ok_or_else(|| {
            cold_err_deleted("HalfEdge", id.index(), id.generation(), slot.generation)
        })?;
        slot.generation += 1;
        slot.next_free = self.connectivity.free_half_edge_head;
        self.connectivity.free_half_edge_head = Some(id.index());
        self.connectivity.active_half_edge_count -= 1;
    ...
    pub fn populate_half_edge(&mut self, id: HalfEdgeId, data: HalfEdgeData) {
        let face = data.face();
        let origin = data.origin();
        Self::populate_slot(&mut self.connectivity.half_edge_slots, id.index(), data);
        self.index_add_halfedge(id, face, origin);
        self.clear_halfedge_sidecar(id.index() as usize);
    }
    ...
    pub fn populate_face(&mut self, id: FaceId, data: FaceData) {
        let shell = data.shell();
        Self::populate_slot(&mut self.connectivity.face_slots, id.index(), data);
        self.index_add_face(id, shell);
    }
    ...
    pub fn populate_loop(&mut self, id: LoopId, data: LoopData) {
        Self::populate_slot(&mut self.connectivity.loop_slots, id.index(), data);
    }
    ...
    pub fn populate_vertex(&mut self, id: VertexId, data: VertexData) {
        Self::populate_slot(&mut self.connectivity.vertex_slots, id.index(), data);
        self.clear_vertex_sidecar(id.index() as usize);
    }
    ...
    pub fn populate_edge(&mut self, id: EdgeId, data: EdgeData) {
        Self::populate_slot(&mut self.connectivity.edge_slots, id.index(), data);
        self.clear_edge_sidecar(id.index() as usize);
    }
}

...
fn projected_manifold_edges_rejects_valence_three_edge() {
    ...
    let has_valence_three = (0..projection.edge_count()).any(|edge_index| {
        projection.radial_valence(ProjectedEdgeId::new(edge_index as u32)) == 3
    });
...
fn build_antiparallel_valence_fixture(draft: &mut SpecDraft, valence: usize) {
    ...
    for index in 0..valence {
        ...
        let half_edge_shared = draft
            .create_node(SpecNodeKind::HalfEdge, None, &format!("shared-half-edge-{index}"))
            .unwrap();
        let half_edge_return = draft
            .create_node(SpecNodeKind::HalfEdge, None, &format!("return-half-edge-{index}"))
            .unwrap();
        ...
        draft
            ...
            .unwrap();
        draft
            .add_relation(RelationKind::LoopEntryHalfEdge, loop_id, half_edge_shared, 0, &format!("loop-entry-{index}"))
            .unwrap();

        for (kind, source, target, role) in [
            (RelationKind::HalfEdgeNext, half_edge_shared, half_edge_return, format!("shared-next-{index}")),
            (RelationKind::HalfEdgeNext, half_edge_return, half_edge_shared, format!("return-next-{index}")),
            (RelationKind::HalfEdgeRadialNext, half_edge_return, half_edge_return, format!("return-radial-{index}")),
            (RelationKind::HalfEdgeUsesEdge, half_edge_shared, shared_edge, format!("shared-edge-rel-{index}")),
            (RelationKind::HalfEdgeUsesEdge, half_edge_return, return_edge, format!("return-edge-rel-{index}")),
            (RelationKind::HalfEdgeOriginVertex, half_edge_shared, origin, format!("shared-origin-{index}")),
            (RelationKind::HalfEdgeOriginVertex, half_edge_return, endpoint, format!("return-origin-{index}")),
            (RelationKind::HalfEdgeBoundsFace, half_edge_shared, face, format!("shared-face-{index}")),
            (RelationKind::HalfEdgeBoundsFace, half_edge_return, face, format!("return-face-{index}")),
        ] {

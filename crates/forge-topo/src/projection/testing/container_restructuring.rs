...
fn build_split_shell_state() -> SpecState {
    ...
    let extra_face = draft.execute(MakeFaceVertexMutation { shell: seed.value.shell }).unwrap();
    draft.execute(SplitShellMutation { shell: seed.value.shell, faces_to_move: vec![extra_face.value.face] }).unwrap();
    draft.commit().unwrap()
}
...
fn build_rehome_lump_state() -> SpecState {
    ...
    draft.execute(RehomeLumpMutation { lump: b.value.lump, target_body: a.value.body }).unwrap();
    draft.commit().unwrap()
}
...
fn build_extract_lump_state() -> SpecState {
    ...
    let solid = draft.execute(MakeSolidMutation).unwrap();
    let extra = draft.execute(MakeLumpRegionMutation { body: solid.value.body }).unwrap();
    draft.execute(ExtractLumpMutation { lump: extra.value.lump }).unwrap();
    draft.commit().unwrap()
}
...
fn build_split_lump_state() -> SpecState {
    ...
    draft.add_relation(RelationKind::LumpOwnsRegion, solid.value.lump, region, 0, "extra-region").unwrap();
    draft.execute(SplitLumpMutation { lump: solid.value.lump, regions_to_move: vec![region] }).unwrap();
    draft.commit().unwrap()
}
...
fn build_merge_lumps_state() -> SpecState {
    ...
    let solid = draft.execute(MakeSolidMutation).unwrap();
    let extra = draft.execute(MakeLumpRegionMutation { body: solid.value.body }).unwrap();
    draft.execute(MergeLumpsMutation { target: solid.value.lump, source: extra.value.lump }).unwrap();
    draft.commit().unwrap()
}
...
fn build_split_body_state() -> SpecState {
    ...
    let solid = draft.execute(MakeSolidMutation).unwrap();
    let extra = draft.execute(MakeLumpRegionMutation { body: solid.value.body }).unwrap();
    draft.execute(SplitBodyMutation { body: solid.value.body, lumps_to_move: vec![extra.value.lump] }).unwrap();
    draft.commit().unwrap()
}
...
fn build_legacy_rehome_shell_state() -> TopologyState {
    ...
    let extra = draft.execute(crate::operations::lifecycle::lump::MakeLumpRegion { body: solid.body }).unwrap().into_value();
    let shell = draft.execute(MakeEmptyShell { region: solid.region, kind: ShellKind::Sheet }).unwrap().into_value();
    draft.execute(RehomeShell { shell: shell.shell, target_region: extra.region }).unwrap();
    draft.commit().unwrap()
}
...
fn build_legacy_split_shell_state() -> TopologyState {
    ...
    let seed = draft
        ...
        .unwrap()
        .into_value();
    let extra_face = draft.execute(MakeFaceVertex { shell: seed.shell }).unwrap().into_value();
    draft.execute(SplitShell { shell: seed.shell, faces_to_move: vec![extra_face.face] }).unwrap();
    draft.commit().unwrap()
}
...
fn insert_shell_face_seed(
    ...
    draft
        ...
        .unwrap()
        .set_half_edge(half_edge);
    if draft.arena().get_shell(shell).unwrap().representative_face().is_dangling() {
        draft
            .arena_mut()
            .get_shell_mut(shell)
            .unwrap()
...
fn build_legacy_rehome_lump_state() -> TopologyState {
    ...
    draft.execute(RehomeLump { lump: b.lump, target_body: a.body }).unwrap();
    draft.commit().unwrap()
}
...
fn build_legacy_split_lump_state() -> TopologyState {
    ...
    draft.arena_mut().get_lump_mut(solid.lump).unwrap().add_region(region);
    draft.execute(SplitLump { lump: solid.lump, regions_to_move: vec![region] }).unwrap();
    draft.commit().unwrap()
}
...
fn build_legacy_merge_lumps_state() -> TopologyState {
    ...
    let solid = draft.execute(MakeSolid).unwrap().into_value();
    let extra = draft.execute(crate::operations::lifecycle::lump::MakeLumpRegion { body: solid.body }).unwrap().into_value();
    draft.execute(MergeLumps { target: solid.lump, source: extra.lump }).unwrap();
    draft.commit().unwrap()
}
...
fn build_legacy_split_body_state() -> TopologyState {
    ...
    let solid = draft.execute(MakeSolid).unwrap().into_value();
    let extra = draft.execute(crate::operations::lifecycle::lump::MakeLumpRegion { body: solid.body }).unwrap().into_value();
    draft.execute(SplitBody { body: solid.body, lumps_to_move: vec![extra.lump] }).unwrap();
    draft.commit().unwrap()
}

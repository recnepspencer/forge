...
mod tests {
    ...
    fn commutative_dag_fuzzer() {
        ...
        draft_a.execute(SplitEdge { edge: e1 }).unwrap();
        draft_a.execute(SplitEdge { edge: e2 }).unwrap();
        let state_a = draft_a.commit().unwrap();

        // Draft B: split e2 first, then e1
        let mut draft_b = seed_state.into_mutation();
        draft_b.execute(SplitEdge { edge: e2 }).unwrap();
        draft_b.execute(SplitEdge { edge: e1 }).unwrap();
    ...
    fn self_intersecting_spaghetti_split() {
        ...
        for _ in 0..iterations {
            let edge_to_kill = draft
                .arena()
                .get_vertex(seed_vertex)
                .unwrap()
                .primary_disk();
            draft
                .execute(KillEdgeVertex { edge: edge_to_kill })
                .unwrap();
        }
    ...
    fn ship_of_theseus_rewire() {
        ...
        for e in edges {
            draft_mod.execute(SplitEdge { edge: e }).unwrap();
        }

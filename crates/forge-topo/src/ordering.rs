...
/// Compute a spatial hash from a 3D position using the standard grid.
pub fn compute_entity_spatial_hash(position: &[f64; 3]) -> u64 {
    forge_math::linalg::compute_spatial_hash(position, forge_geom::GRID_SCALE)
}

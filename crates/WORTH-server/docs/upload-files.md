...
- `WorthServerCompatibilityFacade::prepare_upload(...)`
- `WorthServerCompatibilityFacade::begin_binary_ingress(...)`
- `WorthServerCompatibilityFacade::upload(...)`
- `WorthServerCompatibilityFacade::interrupt_binary_ingress(...)`
...
`worth-server` refuses to pretend those are the same thing automatically.
...
```rust
use worth_server::{
    WorthServerMultipartUpload, WorthServerUploadManifest, WorthServerUploadPart,
};
use serde_json::json;

let upload = WorthServerMultipartUpload::new(

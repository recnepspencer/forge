...
pub(crate) struct WorthServerIngressMetricSnapshot {
...
pub struct WorthServerIngressPerformanceReceipt {
    inner: FoundationalCounterBackedPerformanceReceipt<FoundationalAuthoritativePerformanceClaim>,
}

impl WorthServerIngressPerformanceReceipt {
    pub(crate) fn build(
        metrics: WorthServerIngressMetricSnapshot,
        contract_name: &'static str,
    ) -> Result<Self, WorthServerIngressPerformanceReceiptError> {
        let claim = performance()
            ...
            .finish()
            .map_err(WorthServerIngressPerformanceReceiptError::PerformanceClaim)?;
        let bundle = performance_bundle(claim)
            ...
            .finish()
            .map_err(WorthServerIngressPerformanceReceiptError::PerformanceBundle)?;
        let receipt = counter_backed_performance_receipt(bundle)
            ...
            .finish()
            .map_err(WorthServerIngressPerformanceReceiptError::CounterReceipt)?;
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum WorthServerIngressPerformanceReceiptError {

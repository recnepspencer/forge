...
use crate::{WorthServerResponseEnvelope, WorthServerResponseReceipt};

use super::classification::WorthServerOperatorEvidenceClass;

pub(crate) fn build_attachment_bundle(
    class: &WorthServerOperatorEvidenceClass,
    ...
    WorthServerOperatorEvidenceAttachmentError,
> {
    let locator = BoundaryArtifactLocator::new(
        BoundaryArtifactId::new(boundary_artifact_id(&[
            "worth-server.operator-evidence".to_string(),
...
) -> Result<BuiltSupportAttachment, WorthServerOperatorEvidenceAttachmentError> {
    match class {
        WorthServerOperatorEvidenceClass::QueryReadSucceeded
        | WorthServerOperatorEvidenceClass::QueryMutationSucceeded
        | WorthServerOperatorEvidenceClass::DownstreamDeliverySucceeded => {
            let executed_receipt = receipt.executed().ok_or(
                WorthServerOperatorEvidenceAttachmentError::SuccessRequiresExecutedReceipt,
            )?;
                ...
                .attested_by(support_publication_receipt)
            {
                ...
                TransitionOutcome::Denied(denial) => {
                    Err(WorthServerOperatorEvidenceAttachmentError::SupportConstruction(denial))
                }
                outcome => panic!("unexpected support publication outcome: {outcome:?}"),
            }
        }
        WorthServerOperatorEvidenceClass::RequestContextDenied(_)
        | WorthServerOperatorEvidenceClass::MiddlewareDenied(_)
        | WorthServerOperatorEvidenceClass::QueryHandoffDenied(_) => {
            match FoundationalBoundaryEvidenceSupportFrontDoor
...
fn response_receipt(response: &WorthServerResponseEnvelope) -> &WorthServerResponseReceipt {
...
fn support_receipt_boundary(canonical_digest: &str) -> FoundationalBoundaryEvidenceReceiptBoundary {
    let commit_id = FoundationalCommitId::new(BoundaryHandle::new(boundary_artifact_id(&[
        "worth-server.operator-evidence.support.commit".to_string(),
        canonical_digest.to_string(),
    ])));
    let parent_basis =
        FoundationalCommitParentBasis::new(EquivalenceBasisId::new(boundary_artifact_id(&[
            "worth-server.operator-evidence.support.parent".to_string(),
            canonical_digest.to_string(),
        ])));
...
pub enum WorthServerOperatorEvidenceAttachmentError {

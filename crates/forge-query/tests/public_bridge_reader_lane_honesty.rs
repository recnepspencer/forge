use forge_query::{
    ForgeQueryPublicBridgeReaderLanePosture, ForgeQueryPublicBridgeReaderLaneSabotageOutcome,
};
...
fn assert_projection_reads_are_real(
    reads: &[forge_query::ForgeQueryPublicBridgeProjectionConsumptionEvidence],
) {
    let titles = reads
        .iter()

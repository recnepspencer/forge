...
use forge_query::facade::consumer_kit::{
    ...
    query_consumer_residue_audit, query_test_backend_residue_audit, ForgeQueryConsumerResidueClass,
    ForgeQueryConsumerResidueDetection, ForgeQueryConsumerResidueReport,
};
use forge_query::ForgeQueryEvidenceScope;
...
fn registry_rows_are_complete_unique_and_point_to_consumer_kit_lanes() {
    ...
    for case in HOSTILE_CLASS_CASES {
        ...
        if case.class.is_proof_folklore_for_test() {
            assert_eq!(row.detection(), ForgeQueryConsumerResidueDetection::Ast);
        }
    }
    let test_backend_classes = forge_query_test_backend_residue_classes()
        .into_iter()
        .collect::<std::collections::BTreeSet<_>>();
    assert!(
        test_backend_classes.contains(&ForgeQueryConsumerResidueClass::RuntimeBridgeHandAssembly)
    );
    assert!(!test_backend_classes.contains(&ForgeQueryConsumerResidueClass::LocalQueryReport));
}
...
fn consumer_residue_audit_rejects_invalid_inventory_inputs() {
    ...
    assert_eq!(
        empty_name.kind(),
        forge_query::facade::consumer_kit::ForgeQueryBoundaryAuditErrorKind::EmptyCrateName
    );
    ...
    assert_eq!(
        empty_roots.kind(),
        forge_query::facade::consumer_kit::ForgeQueryBoundaryAuditErrorKind::MissingRequiredRoot
    );

    let missing_root = query_consumer_residue_audit("downstream")
        .required_root(std::env::temp_dir().join("forge-query-missing-consumer-residue-root"))
        .evaluate()
...
fn test_backend_residue_audit_remains_a_filtered_compatibility_lane() {
    ...
    assert_eq!(
        ...
        ForgeQueryConsumerResidueClass::RuntimeBridgeHandAssembly.as_str()
    );
    assert_eq!(
        report.report_identity().scope(),
    ...
    assert!(report.finding_identities().iter().all(|identity| {
        identity.scope() == ForgeQueryEvidenceScope::ConsumerTestBackendResidueFinding
    }));
}
...
) -> &forge_query::facade::consumer_kit::ForgeQueryConsumerResidueFinding {
    let matches = report
        .findings()
        .iter()
...
fn residue_workspace(label: &str, source: &str) -> PathBuf {
    ...
    let root = std::env::temp_dir()
        .join("forge-query-consumer-residue-audit")
...
impl ProofFolkloreClassForTest for ForgeQueryConsumerResidueClass {
    fn is_proof_folklore_for_test(self) -> bool {
        matches!(
            self,

...
) -> Result<ForgeQueryGraphObligationResidueRow, &'static str> {
    match surface {
...
fn query_capability_blockers_are_explicit_query_gap_rows() {
    for family in [
        ...
        let manifest =
            ForgeQueryGraphObligationResidueManifest::capped([phase_two_query_gap_row(family)])
                .unwrap();
        let row = &manifest.rows()[0];
        assert_eq!(row.decision(), "query-gap");
        assert_eq!(row.owner(), "forge-query");

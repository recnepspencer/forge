...
impl ForgeQueryDeclarationInput<GeometryDomain> for TrimSegmentAtIntersection {
    ...
    fn canonical_declaration_entries(&self) -> Vec<ForgeQueryDeclarationCanonicalEntry> {
        vec![
            ForgeQueryDeclarationCanonicalEntry::text("segment_ref", "segment:outer-wall-a"),
            ForgeQueryDeclarationCanonicalEntry::text("intersection_ref", "intersection:trim-42"),
        ]
    }
}
...
fn main() {
    ...
    let handle = query
        ...
        .admit()
        .unwrap();
    let progressed = match handle.declare_review_and_progress(TrimSegmentAtIntersection) {
        Ok(progressed) => progressed,
        Err(_) => return,
    };

    let _ = handle.orchestrate_routes_from_progressed_transcript(progressed);
}

...
impl ForgeQueryDomainOperatingContext<GeometryDomain> for GeometryOperatingContext {
    ...
    fn required_config_sections(&self) -> &'static [ForgeQueryConfigSectionFamily] {
        &[ForgeQueryConfigSectionFamily::Query, ForgeQueryConfigSectionFamily::RuntimeBridge]
    }
...
fn main() {
    ...
    let evidence = handle
        .describe_foundational(
            forge_query::facade::ForgeQueryDeclarationFoundationalEvidenceInput::admitted_progression(
                progressed.clone(),
            ),
        )
        .unwrap();
    let route_plan = handle
        .plan_routes(
            forge_query::facade::ForgeQueryDeclarationRoutePlanInput::admitted(
                progressed,
                evidence,
            ),
        )
    ...
    let _ = handle.route_bridge_continuation(route_plan);
}

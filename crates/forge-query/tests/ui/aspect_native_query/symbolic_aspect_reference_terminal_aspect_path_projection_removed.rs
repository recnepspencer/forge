...
fn main() {
    let reference = ForgeQuerySymbolicAspectReference::same_batch_entity_identity(
        ForgeQueryAspectTouch::field_path(AspectKey::new("owner").unwrap(), CanonicalFieldPath::single(FieldKey::new("id").unwrap())),
        ForgeQuerySymbolicTargetReference::new("draft-task").unwrap(),
    );
    let _ = reference.terminal_aspect_path_projection();
}

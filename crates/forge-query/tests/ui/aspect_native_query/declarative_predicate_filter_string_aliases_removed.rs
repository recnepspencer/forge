...
fn main() {
    let equality = DeclarativeEqualityFilter::new(
        AspectFieldKey::new("identity", "id").unwrap(),
    ...
    let integer = DeclarativeIntegerComparisonFilter::greater_than(
        AspectFieldKey::new("metrics", "rank").unwrap(),
        1,
    );
    ...
    let membership = DeclarativeSetMembershipFilter::new(
        AspectFieldKey::new("status", "value").unwrap(),
    ...
    let presence =
        DeclarativePresenceFilter::is_present(AspectFieldKey::new("owner", "id").unwrap());

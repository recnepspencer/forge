...
fn assert_requirement_kinds_are_documented(requirement_kinds: &BTreeSet<&str>) {
    assert!(requirement_kinds
        .contains(ForgeQueryGraphReadAccessRequirementKind::DirectionalAdjacency.as_str()));
    assert!(requirement_kinds
        .contains(ForgeQueryGraphReadAccessRequirementKind::TraversalWorkset.as_str()));

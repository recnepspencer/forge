use forge_query::facade::runtime::{
    ForgeQueryAdmittedGraphReadRelationDirection, ForgeQueryGraphReadAccessShape,
    ForgeQueryGraphReadFanoutPosture, ForgeQueryGraphReadOrderingPosture,
...
fn graph_read_access_shape_changes_for_relation_direction_and_depth() {
    ...
    assert_shape_dimensions(
        ...
        ExpectedGraphReadShape {
            root: ForgeQueryGraphReadRootPosture::Anchored,
            scope: ForgeQueryReadScopeClass::AnchoredExpansion,
            operators: &[ForgeQueryGraphReadTraversalOperator::BoundedAncestor],
            directions: &[ForgeQueryAdmittedGraphReadRelationDirection::Ancestor],
            max_depth: 2,
            fanout: ForgeQueryGraphReadFanoutPosture::SingleRelation,
            predicate: ForgeQueryGraphReadPredicateFamily::None,
            ordering: ForgeQueryGraphReadOrderingPosture::Unordered,
            result: ForgeQueryGraphReadResultPressure::CollectionNarrow,
        },
    );
    assert_eq!(
...
fn graph_read_access_shape_classifies_predicate_families_and_mixed_predicates() {
    ...
    assert_eq!(
        ...
        &ForgeQueryGraphReadPredicateFamily::Range
    );
    assert_eq!(
        text.predicate_family(),
        &ForgeQueryGraphReadPredicateFamily::Text
    );
    assert_eq!(
        membership.predicate_family(),
        &ForgeQueryGraphReadPredicateFamily::Membership
    );
    assert_eq!(
        presence.predicate_family(),
        &ForgeQueryGraphReadPredicateFamily::Presence
    );
    assert_eq!(
        mixed.predicate_family(),
        &ForgeQueryGraphReadPredicateFamily::Mixed
    );
}
...
struct ExpectedGraphReadShape<'a> {
    root: ForgeQueryGraphReadRootPosture,
    scope: ForgeQueryReadScopeClass,
    operators: &'a [ForgeQueryGraphReadTraversalOperator],
    directions: &'a [ForgeQueryAdmittedGraphReadRelationDirection],
    max_depth: usize,
    fanout: ForgeQueryGraphReadFanoutPosture,
    predicate: ForgeQueryGraphReadPredicateFamily,
    ordering: ForgeQueryGraphReadOrderingPosture,
    result: ForgeQueryGraphReadResultPressure,
}

fn assert_shape_dimensions(
    shape: &ForgeQueryGraphReadAccessShape,
...
fn access_shape<'a>(
    workspace: &'a forge_query::facade::runtime::ForgeQueryWorkspace,
    family: &'a forge_query::facade::runtime::ForgeQueryReadFamily,

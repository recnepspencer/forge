use forge_query::facade::consumer_kit::{
    graph_read_bypass_adoption, graph_read_bypass_audit, ForgeQueryBoundaryAuditSourceSet,
    ForgeQueryGraphReadBypassAdoptionErrorKind, ForgeQueryGraphReadBypassClass,
...
"#;
    let report = graph_read_bypass_audit("facade-consumer")
        .required_sources(
            ForgeQueryBoundaryAuditSourceSet::new("facade-consumer").source_file(
                "topology.walk",
                "src/topology.rs",
                source,
            ),
    ...
    assert!(
        report.finding_count_for_class(ForgeQueryGraphReadBypassClass::ManualRelationRowLoop) > 0
    );
    assert!(report.finding_count_for_class(ForgeQueryGraphReadBypassClass::AdHocAdjacencyMap) > 0);
    assert!(report
        .findings()
        .iter()
        .all(|finding| finding.authority_violation().is_graph_read_access_bypass()));

    let residue = ForgeQueryGraphReadBypassResidueManifest::capped([
        ...
        ForgeQueryGraphReadBypassResidueRow::explicit(
            ...
            "worth-topo Phase 16 adoption",
            "Milestone 9.10 Phase 15",
            report.finding_count_for_class(ForgeQueryGraphReadBypassClass::AdHocAdjacencyMap),
            report.finding_count_for_class(ForgeQueryGraphReadBypassClass::AdHocAdjacencyMap),
    ...
    let incomplete_residue = ForgeQueryGraphReadBypassResidueManifest::capped([
        ForgeQueryGraphReadBypassResidueRow::explicit(
            ForgeQueryGraphReadBypassClass::ManualRelationRowLoop,
            "worth-topo Phase 16 adoption",
            "Milestone 9.10 Phase 15",
    ...
    assert_eq!(
        coverage_error.kind(),
        &ForgeQueryGraphReadBypassResidueErrorKind::ResidueCoverageShortfall
    );

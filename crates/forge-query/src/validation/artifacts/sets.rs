...
impl ValidatedPredicateSet {
    ...
    pub(crate) fn digest_parts(&self) -> impl Iterator<Item = String> + '_ {
        self.entries
            .iter()
            .map(ValidatedPredicateEntry::digest_part)
    }

    pub(crate) fn from_entries(mut entries: Vec<ValidatedPredicateEntry>) -> Self {
        entries.sort();
        entries.dedup();
        Self { entries }
    }
...
impl ValidatedOrderingSet {
    ...
    pub(crate) fn from_entries(mut entries: Vec<ValidatedOrderingEntry>) -> Self {
        entries.sort();
        entries.dedup();
        Self { entries }
    }

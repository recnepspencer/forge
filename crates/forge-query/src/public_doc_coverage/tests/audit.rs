...
fn audit_flags_dropped_real_surface_coverage_as_undocumented() {
    ...
    let audit = ForgeQueryPublicDocCoverageAudit::from_inventory(&coverage);
...
fn audit_flags_readme_and_journey_gaps() {
    ...
    let readme_audit = ForgeQueryPublicDocCoverageAudit::from_inventory(&without_readme);
    assert_eq!(
    ...
    let journey_audit = ForgeQueryPublicDocCoverageAudit::from_inventory(&without_journey);
    assert_eq!(
    ...
    let mismatch_audit = ForgeQueryPublicDocCoverageAudit::from_inventory(&mismatched_coverage);
    assert_eq!(

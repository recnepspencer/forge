...
use super::ForgeQueryPublicGoldenTranscriptKind;
use crate::orchestration_inventory::ForgeQueryOrchestrationSurfaceInventory;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryPublicDocCoverageAudit {
...
impl ForgeQueryPublicDocCoverageAudit {
    pub fn current() -> Self {
        Self::from_inventory(&ForgeQueryPublicDocCoverageInventory::current())
    }

    pub fn from_inventory(inventory: &ForgeQueryPublicDocCoverageInventory) -> Self {
        let live_surface_names = ForgeQueryOrchestrationSurfaceInventory::current()
            .rows()
            .iter()
        ...
        let used_surface_goldens = inventory
            ...
            .filter(|golden| golden.kind() == ForgeQueryPublicGoldenTranscriptKind::SurfaceCoverage)
            .map(|golden| golden.label().to_string())
            .collect::<BTreeSet<_>>();
        let readme = read_workspace_text("crates/forge-query/docs/domain-capabilities/README.md")
            .unwrap_or_default();
        ...
        let mut orphan_golden_rows =
            super::goldens::forge_query_public_doc_coverage_golden_transcripts()
                .iter()
                .filter(|golden| {
                    golden.kind() == ForgeQueryPublicGoldenTranscriptKind::SurfaceCoverage
...
fn doc_row_exists(row: &ForgeQueryPublicDocCoverageRow) -> bool {
...
fn golden_exists(row: &ForgeQueryPublicDocCoverageRow) -> bool {

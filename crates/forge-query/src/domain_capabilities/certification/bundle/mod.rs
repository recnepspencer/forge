...
use super::output_manifest::forge_query_domain_capability_certification_output_manifest;
use super::reports::{
...
pub struct ForgeQueryDomainCapabilityCertificationOutput {
    name: &'static str,
    digest: String,
}

impl ForgeQueryDomainCapabilityCertificationOutput {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryDomainCapabilityCertificationBundle {
    output_manifest: Vec<&'static str>,
    representative_report: ForgeQueryDomainCapabilityRepresentativeReport,
    slope_report: ForgeQueryDomainCapabilitySlopeReport,
    outputs: Vec<ForgeQueryDomainCapabilityCertificationOutput>,
    certification_bundle_digest: String,
}

impl ForgeQueryDomainCapabilityCertificationBundle {
    fn new(
        representative_report: ForgeQueryDomainCapabilityRepresentativeReport,
        slope_report: ForgeQueryDomainCapabilitySlopeReport,
    ) -> Self {
        let output_manifest =
            forge_query_domain_capability_certification_output_manifest().to_vec();
        ...
        let outputs = output_specs
            .iter()
            .map(|output| {
                ForgeQueryDomainCapabilityCertificationOutput::new(
                    output.name(),
    ...
    pub fn representative_report(&self) -> &ForgeQueryDomainCapabilityRepresentativeReport {
        &self.representative_report
    }

    pub fn slope_report(&self) -> &ForgeQueryDomainCapabilitySlopeReport {
        &self.slope_report
    }
    ...
    pub fn outputs(&self) -> &[ForgeQueryDomainCapabilityCertificationOutput] {
        &self.outputs
    }
    ...
    pub fn output_digest(&self, key: &str) -> Option<&str> {
        self.outputs
            ...
            .map(ForgeQueryDomainCapabilityCertificationOutput::digest)
    }
...
pub fn certify_domain_capabilities() -> ForgeQueryDomainCapabilityCertificationBundle {
    let representative_report = forge_query_domain_capability_representative_report();
    let slope_report = forge_query_domain_capability_slope_report(&representative_report);

    ForgeQueryDomainCapabilityCertificationBundle::new(representative_report, slope_report)
}

fn validate_output_manifest(
    output_manifest: &[&'static str],
    outputs: &[self::outputs::ForgeQueryDomainCapabilityCertificationOutputSpec],
) {

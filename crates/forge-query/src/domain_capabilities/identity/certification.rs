...
pub(crate) fn compose_compile_fail_boundary_digest(
    boundaries: &[ForgeQueryDomainCapabilityCompileFailBoundary],
) -> String {
...
pub(crate) fn compose_target_dx_row_digest(label: &str, dx_focus: &str) -> String {
    seal(
        domain_capability_certification_scope_encoder("domain_capability_target_dx_row_v1")
            .field_shape(ForgeQueryEvidenceTag::new("label"), label)
            .field_shape(ForgeQueryEvidenceTag::new("dx_focus"), dx_focus),
    )
}

pub(crate) fn compose_golden_transcript_digest(
    transcripts: &[ForgeQueryDomainCapabilityGoldenTranscript],
) -> String {
    compose_certification_sequence_digest(
...
pub(crate) fn compose_certified_surface_row_digest(
    ...
    seal(
        domain_capability_certification_scope_encoder("domain_capability_certified_surface_row_v1")
            ...
            .field_shape(ForgeQueryEvidenceTag::new("ordinary_lane"), ordinary_lane)
            .field_shape(
                ForgeQueryEvidenceTag::new("inspectable_lane"),
                inspectable_lane,
            )
            .field_shape(ForgeQueryEvidenceTag::new("proof_lane"), proof_lane)
            .field_shape(ForgeQueryEvidenceTag::new("raw_lane"), raw_lane)
            .field_shape(
                ForgeQueryEvidenceTag::new("implementation_path"),
                implementation_path,
            ),
    )
}
...
pub(crate) fn compose_scaled_category_digest(
    categories: &[ForgeQueryDomainCapabilityCategory],
) -> String {
    compose_certification_sequence_digest(
...
pub(crate) fn compose_slope_scale_entry_digest(
    ...
    seal(
        domain_capability_certification_scope_encoder("domain_capability_slope_scale_v1")
            .field_shape(ForgeQueryEvidenceTag::new("label"), label)
            .field_usize(ForgeQueryEvidenceTag::new("scale"), scale)
            .field_usize(ForgeQueryEvidenceTag::new("width"), width)

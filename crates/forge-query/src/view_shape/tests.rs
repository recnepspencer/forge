use crate::authoring::{AspectFieldSelector, AuthoredResultShapeField, GuidedAuthoringPath, RootEntityKey};
use crate::view_shape::{
    admit_view_shape, plan_admitted_view_shape, validate_canonical_bundle_for_admitted_view_shape,
    ViewShapeDescriptor, ViewShapeFailureClass, ViewShapeInvalidationPosture, ViewShapePatchPosture,
};
...
fn table_denies_detail_queries() {
    ...
    assert_eq!(error.failure_class(), &ViewShapeFailureClass::IncompatibleCanonicalFamily);
}

#[test]
fn inspector_denies_collection_queries() {
    let error =
        admit_view_shape(&direct_collection(), ViewShapeDescriptor::inspector_detail_observed())
            .unwrap_err();
    assert_eq!(error.failure_class(), &ViewShapeFailureClass::IncompatibleCanonicalFamily);
}
...
fn focused_inspector_requires_focus_contract() {
    let error =
        crate::view_shape::admit_view_shape(&direct_detail(), ViewShapeDescriptor::inspector_detail_focused_missing_for_test())
            .unwrap_err();
    assert_eq!(error.failure_class(), &ViewShapeFailureClass::FocusAspectRequired);
}

#[test]
fn kanban_grouped_requires_grouping_contract() {
    let error =
        crate::view_shape::admit_view_shape(&direct_collection(), ViewShapeDescriptor::kanban_grouped_missing_for_test())
            .unwrap_err();
    assert_eq!(error.failure_class(), &ViewShapeFailureClass::GroupingAspectRequired);
}
...
fn observed_and_focused_inspector_produce_distinct_plan_metadata() {
    ...
    let focused = plan_admitted_view_shape(
        validate_canonical_bundle_for_admitted_view_shape(
            &canonical,
            detail_schema_view(),
            admit_view_shape(&canonical, ViewShapeDescriptor::inspector_detail_focused("profile"))
                .unwrap(),
        )
        .unwrap(),
        basis_intent(),
    ...
    assert_ne!(observed.view_plan_digest().as_str(), focused.view_plan_digest().as_str());
    assert_eq!(
...
fn kanban_grouped_is_admitted_with_explicit_grouping_aspect() {
    ...
    assert_eq!(planned.family(), crate::view_shape::ViewShapeFamily::KanbanGrouped);
    assert_eq!(planned.delivery_metadata().grouping_aspect(), Some("status"));
    ...
    assert_eq!(grouped_evidence.identity_binding().field_key(), "identity.id");
    assert_eq!(grouped_evidence.grouping_binding().field_key(), "status.lane");

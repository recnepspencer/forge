...
pub struct ForgeQueryPlatformEntryParityLane {
    kind: &'static str,
    surface: Option<&'static str>,
}

impl ForgeQueryPlatformEntryParityLane {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ForgeQueryPlatformEntryParityAssertionClass {
    Equality,
}

impl ForgeQueryPlatformEntryParityAssertionClass {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ForgeQueryPlatformEntryParityRow {
    label: &'static str,
    control_lane: ForgeQueryPlatformEntryParityLane,
    parity_lane: ForgeQueryPlatformEntryParityLane,
    assertion_class: ForgeQueryPlatformEntryParityAssertionClass,
    proof_path: &'static str,
    proof_name: &'static str,
}

impl ForgeQueryPlatformEntryParityRow {
    const fn new(
        label: &'static str,
        control_lane: ForgeQueryPlatformEntryParityLane,
        parity_lane: ForgeQueryPlatformEntryParityLane,
        ...
        Self {
            ...
            control_lane,
            parity_lane,
            assertion_class: ForgeQueryPlatformEntryParityAssertionClass::Equality,
            proof_path,
            proof_name,
        }
    }
    ...
    pub fn control_lane(&self) -> ForgeQueryPlatformEntryParityLane {
        self.control_lane
    }

    pub fn parity_lane(&self) -> ForgeQueryPlatformEntryParityLane {
        self.parity_lane
    }

    pub fn assertion_class(&self) -> ForgeQueryPlatformEntryParityAssertionClass {
...
pub struct ForgeQueryPlatformEntryParityManifest {
    rows: Vec<ForgeQueryPlatformEntryParityRow>,
    parity_digest: String,
}

impl ForgeQueryPlatformEntryParityManifest {
    fn new(rows: Vec<ForgeQueryPlatformEntryParityRow>) -> Self {
    ...
    pub fn rows(&self) -> &[ForgeQueryPlatformEntryParityRow] {
        &self.rows
    }
    ...
    pub fn row_for_label(&self, label: &str) -> Option<&ForgeQueryPlatformEntryParityRow> {
        self.rows.iter().find(|row| row.label() == label)
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryPlatformEntryParityAudit {
    parity_digest: String,
...
impl ForgeQueryPlatformEntryParityAudit {
    pub fn current() -> Self {
        Self::from_manifest(&forge_query_platform_entry_parity_manifest())
    }

    pub fn from_manifest(manifest: &ForgeQueryPlatformEntryParityManifest) -> Self {
...
pub fn forge_query_platform_entry_parity_manifest() -> ForgeQueryPlatformEntryParityManifest {
    ForgeQueryPlatformEntryParityManifest::new(vec![
        ForgeQueryPlatformEntryParityRow::new(
            "geometry_preview_helper_matches_generic_signal_path",
            ForgeQueryPlatformEntryParityLane::new(
                "helper",
        ...
        ForgeQueryPlatformEntryParityRow::new(
            ...
            ForgeQueryPlatformEntryParityLane::new("generic_grouped_authoring", None),
        ...
        ForgeQueryPlatformEntryParityRow::new(
            "recovery_brief_matches_between_checked_and_proof_grouped_wrong_world",
            ForgeQueryPlatformEntryParityLane::new("checked_recovery", None),
            ForgeQueryPlatformEntryParityLane::new("proof_recovery", None),
...
mod tests {
    ...
    fn parity_manifest_rows_are_present_and_backed_by_real_tests() {
        let manifest = forge_query_platform_entry_parity_manifest();

        assert_eq!(manifest.rows().len(), 5);
        assert!(!manifest.parity_digest().is_empty());
        assert!(ForgeQueryPlatformEntryParityAudit::current()
            .missing_equivalence_rows()
            .is_empty());
        assert!(ForgeQueryPlatformEntryParityAudit::current()
            .unknown_surfaces()

...
use crate::runtime::{
    ...
    ForgeQueryConcurrentSubmissionIntake, ForgeQueryConcurrentSubmissionRecord,
};
...
pub(in crate::runtime::tests) fn execute_phase_sixteen_concurrent_hostile_matrix(
) -> ForgeQueryConcurrentHostileMatrixArtifact {
    ...
    ForgeQueryConcurrentHostileMatrixArtifact::certify(
        primary.topology,
...
) -> Vec<ForgeQueryConcurrentHostileMatrixSabotage> {
    [
        ForgeQueryConcurrentHostileMatrixSabotageKind::CommittedReadHotPathLock,
        ForgeQueryConcurrentHostileMatrixSabotageKind::SharedReadMintRowClone,
        ForgeQueryConcurrentHostileMatrixSabotageKind::ReaderDerivedEvaluation,
        ForgeQueryConcurrentHostileMatrixSabotageKind::OrphanedSnapshotGeneration,
        ForgeQueryConcurrentHostileMatrixSabotageKind::UnretiredReadPin,
        ForgeQueryConcurrentHostileMatrixSabotageKind::JournalGap,
        ForgeQueryConcurrentHostileMatrixSabotageKind::ReplayResidue,
        ForgeQueryConcurrentHostileMatrixSabotageKind::DeliveryResidue,
    ]
    .into_iter()
    .map(|kind| ForgeQueryConcurrentHostileMatrixSabotage::perturb(kind, artifact))
    .collect()
}
...
struct ConcurrentMatrixState {
    ...
    preview_closeout_digests: Vec<String>,
    branch_basis_digests: Vec<String>,
    receipts: Vec<ForgeQueryWriteReceipt>,
    delivery_residue_count: usize,
    last_artifact: Option<ForgeQueryPublishedDerivedArtifactHandle>,
}
...
impl ConcurrentMatrixState {
    ...
    fn execute_concurrent_reader_and_submitter_matrix(
        ...
        let intake = ForgeQueryConcurrentSubmissionIntake::new();
    ...
    fn submit_concurrent_record(&mut self, record: ForgeQueryConcurrentSubmissionRecord) {
        assert!(
    ...
    fn submit_command(&mut self, command: ForgeQueryWriteCommand) {
        let receipt = self
            .workspace
            .submissions()
    ...
    fn finish(mut self) -> ConcurrentMatrixRun {
        ...
        let counters = ForgeQueryConcurrentHostileMatrixCounterSnapshot::from_runtime(
            &self.workspace.runtime,
            self.delivery_residue_count,
        );
        let topology = ForgeQueryConcurrentHostileMatrixTopology::new(

...
use crate::runtime::{
    ForgeQueryReadBuiltInOperator, ForgeQueryReadDenialKind, ForgeQueryReadExecutionEngine,
...
fn compose_read_executes_local_traversal_detail_with_attached_receipt() {
    ...
    assert!(result
        .receipt()
        .operator_families()
        .contains(&ForgeQueryReadOperatorFamily::Projection));
    assert!(result
        .receipt()
        .operator_families()
...
fn compose_read_reports_typed_invalid_root_denial() {
    ...
    match error {
        ForgeQueryRuntimeError::ReadCompositionDenied(denial) => {
            assert_eq!(denial.kind(), &ForgeQueryReadDenialKind::InvalidRoot);

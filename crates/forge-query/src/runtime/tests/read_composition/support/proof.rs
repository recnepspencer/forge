...
use crate::runtime::{
    ...
    ForgeQueryReadDenialKind, ForgeQueryReadGraphFamily, ForgeQueryReadRelationshipProofPosture,
...
pub(crate) fn assert_scope_shape_denial(
    ...
    assert_eq!(denial.kind(), &ForgeQueryReadDenialKind::ScopeShapeDenied);
    let mismatch = denial
...
pub(crate) fn assert_built_in_operator_denial(
    denial: &ForgeQueryReadDenial,
    expected_operator: ForgeQueryReadBuiltInOperator,
    expected_reason: ForgeQueryReadBuiltInOperatorDenialReason,
) {
    assert_eq!(
        denial.kind(),
        &ForgeQueryReadDenialKind::BuiltInOperatorDenied
    );
...
pub(crate) fn assert_collection_receipt(
    ...
    expected_scope: ForgeQueryReadScopeClass,
) {
    ...
    assert!(receipt
        .operator_families()
        .contains(&crate::runtime::ForgeQueryReadOperatorFamily::Ordering));
...
pub(crate) fn assert_relationship_proof_not_required(result: &ForgeQueryReadResult) {
    let receipt = result.receipt();
    assert_eq!(

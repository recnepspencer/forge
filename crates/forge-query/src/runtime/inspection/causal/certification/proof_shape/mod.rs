...
pub struct CausalInspectionProofShapeCertification {
    ...
    forged_authority_witness_rejected: bool,
...
impl CausalInspectionProofShapeCertification {
    pub fn from_runtime_path(
        ...
        let inspected_artifact_digest = changed_artifact.artifact_digest().to_string();
    ...
    #[cfg(test)]
    pub(in crate::runtime) fn forged_for_tests(
        changed_artifact: &QueryCausalInspectionArtifact,
        representative_matrix: &CausalInspectionRepresentativeMatrix,
        boundary_audit: &CausalInspectionBoundaryAudit,
    ) -> Self {
        let mut proof =
            Self::from_runtime_path(changed_artifact, representative_matrix, boundary_audit);
        proof.forged_authority_witness_rejected = false;
        proof.proof_shape_digest = digest::forged_test_proof_shape_digest(&proof);
        proof
    }
    ...
    pub fn forged_authority_witness_rejected(&self) -> bool {
        self.forged_authority_witness_rejected
    }

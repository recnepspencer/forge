...
use super::ForgeQueryParsedAspectTarget;
use forge_foundational::facade::{AspectKey, CanonicalFieldPath};

#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct ForgeQueryAspectTouch {
...
impl ForgeQueryAspectTouch {
    ...
    pub(crate) fn from_parsed_target(target: ForgeQueryParsedAspectTarget) -> Self {
        Self { target }
    }
    ...
    pub(crate) fn from_native_parts(
        ...
        Self {
            target: ForgeQueryParsedAspectTarget::from_native_parts(aspect_key, field_path),
        }
    }

    pub(crate) fn into_parsed_target(self) -> ForgeQueryParsedAspectTarget {
        self.target
    }

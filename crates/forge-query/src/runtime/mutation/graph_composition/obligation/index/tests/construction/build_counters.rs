use crate::runtime::{
    ForgeQueryGraphObligationIndex, ForgeQueryGraphObligationOperatingWorldSelector,
};
...
fn index_build_counters_are_stable_and_shape_sensitive() {
    ...
    let first =
        ForgeQueryGraphObligationIndex::from_catalog(&catalog(vec![left.clone(), right.clone()]));
    let reordered = ForgeQueryGraphObligationIndex::from_catalog(&catalog(vec![right, left]));
    let smaller =
        ForgeQueryGraphObligationIndex::from_catalog(&catalog(vec![schema_registration(
            "schema",

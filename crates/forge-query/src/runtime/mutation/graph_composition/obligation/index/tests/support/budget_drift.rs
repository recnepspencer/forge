use crate::runtime::{
    ...
    ForgeQueryGraphObligationExecutionScope, ForgeQueryGraphObligationIndex,
...
fn changing_registration_budget_changes_index_and_selection_identity() {
    let world_selector = ForgeQueryGraphObligationOperatingWorldSelector::any_committed_authority();
    ...
    let default_index =
        ForgeQueryGraphObligationIndex::from_catalog(&catalog(vec![default_registration]));
    let budgeted_index =
        ForgeQueryGraphObligationIndex::from_catalog(&catalog(vec![budgeted_registration]));

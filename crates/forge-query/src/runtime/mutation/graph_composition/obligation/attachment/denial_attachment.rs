...
use crate::runtime::{
    ...
    ForgeQueryGraphObligationDispatchContextKind, ForgeQueryGraphObligationExecutionStatus,
    ForgeQueryGraphObligationKind,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryGraphObligationDenialAttachmentProjection {
    envelope_digest: String,
    execution_point: ForgeQueryGraphObligationDispatchContextKind,
    touch_descriptor_digest: String,
    operating_world_digest: String,
    dispatch_digest: String,
    denial_projection_digest: String,
    rows: Vec<ForgeQueryGraphObligationDenialAttachmentProjectionRow>,
    projection_digest: ForgeQueryEvidenceIdentity,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryGraphObligationDenialAttachmentProjectionRow {
...
impl ForgeQueryGraphObligationDenialAttachmentProjection {
    pub(crate) fn from_dispatch_projection_and_denial(
        dispatch: &ForgeQueryAuthoritativeMutationObligationDispatchProjection,
        denial: &ForgeQueryGraphObligationDenialProjection,
    ) -> Option<Self> {
        ...
        let rows = denial
            ...
            .iter()
            .filter_map(|row| {
                ForgeQueryGraphObligationDenialAttachmentProjectionRow::from_denial_row(
                    row,
                    dispatch,
                    &envelope_digest,
                    execution_point,
        ...
        let row_identities = rows
            .iter()
            .map(ForgeQueryGraphObligationDenialAttachmentProjectionRow::row_evidence_identity)
            .collect::<Vec<_>>();
        let denial_projection_digest = denial.projection_digest().to_string();
        let projection_digest = forge_query_evidence_identity(
        ...
        .field_evidence_identity_sequence(ForgeQueryEvidenceTag::new("row"), row_identities)
        .seal();
        Some(Self {
            envelope_digest,
            execution_point,
    ...
    pub fn execution_point(&self) -> ForgeQueryGraphObligationDispatchContextKind {
    ...
    pub fn rows(&self) -> &[ForgeQueryGraphObligationDenialAttachmentProjectionRow] {
        &self.rows
    }
    ...
    pub(crate) fn projection_evidence_identity(&self) -> &ForgeQueryEvidenceIdentity {
        &self.projection_digest
    }
}

impl ForgeQueryGraphObligationDenialAttachmentProjectionRow {
    fn from_denial_row(
        row: &ForgeQueryGraphObligationDenialProjectionRow,
        dispatch: &ForgeQueryAuthoritativeMutationObligationDispatchProjection,
        envelope_digest: &str,
        execution_point: ForgeQueryGraphObligationDispatchContextKind,
        ...
        .field_value(
            ForgeQueryEvidenceTag::new("dispatch_plan"),
    ...
    fn row_evidence_identity(&self) -> &ForgeQueryEvidenceIdentity {
        &self.row_digest
    }
}

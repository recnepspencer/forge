use super::{
    ...
    ForgeQueryGraphObligationKind, FORGE_QUERY_GRAPH_OBLIGATION_DISPATCH_ENVELOPE_SCHEME,
};
...
fn multi_obligation_touch_records_every_fired_rule() {
    ...
    assert_eq!(
        envelope.kind_count(ForgeQueryGraphObligationKind::BlockingInvariant),
        1
    );
    assert_eq!(
    ...
    assert_eq!(
        envelope
            .rows()
            .iter()
            .map(ForgeQueryGraphObligationDispatchPlan::plan_digest)
...
fn context_digest_participates_in_envelope_identity() {
    ...
    let envelope_a = ForgeQueryGraphObligationDispatchEnvelope::builder(world_a)
        .record(blocking_plan())
        .seal()
        .expect("world-a envelope");
    let envelope_b = ForgeQueryGraphObligationDispatchEnvelope::builder(world_b)
        .record(blocking_plan())
        .seal()
...
fn touch_digest_participates_in_envelope_identity() {
    ...
    let envelope_a = ForgeQueryGraphObligationDispatchEnvelope::builder(touch_a)
        .record(blocking_plan())
        .seal()
        .expect("touch-a envelope");
    let envelope_b = ForgeQueryGraphObligationDispatchEnvelope::builder(touch_b)
        .record(blocking_plan())
        .seal()

...
use crate::runtime::{
    ForgeQueryReadDenial, ForgeQueryReadDenialKind, ForgeQueryReadRelationshipProofDenial,
};
...
pub(super) fn admit_read_relationship_proof(
    ...
    built_in_operators: &[crate::runtime::ForgeQueryReadBuiltInOperator],
) -> Result<Option<RelationshipProofAdmission>, ForgeQueryReadDenial> {
...
fn synthetic_runtime_read_context(
    canonical: &CanonicalQueryArtifact,
    schema_basis: &SchemaBasisDigest,
) -> Result<crate::policy_basis::AdmittedPolicyTenantContext, ForgeQueryReadDenial> {
...
) -> Result<RelationshipProofDescriptorSet, ForgeQueryReadDenial> {
...
fn policy_tenant_admission_denial(error: PolicyTenantAdmissionError) -> ForgeQueryReadDenial {
    ForgeQueryReadDenial::new_relationship_proof_admission_denied(
        ForgeQueryReadRelationshipProofDenial::for_policy_failure(error.failure_class()),
        format!("{error:?}"),
    )
}

fn relationship_proof_admission_denial(error: RelationshipProofError) -> ForgeQueryReadDenial {
    ForgeQueryReadDenial::new_relationship_proof_admission_denied(
        ForgeQueryReadRelationshipProofDenial::for_relationship_proof_failure(
            error.failure_class(),
        ),
        format!("{error:?}"),
    )
...
mod tests {
    ...
    use crate::runtime::{ForgeQueryReadDenialKind, ForgeQueryReadRelationshipProofDenialStage};
    ...
    fn policy_tenant_failures_map_to_structured_relationship_proof_denial() {
        ...
        assert_eq!(
            denial.kind(),
            &ForgeQueryReadDenialKind::RelationshipProofAdmissionDenied
        );
        ...
        assert_eq!(
            structured.stage(),
            &ForgeQueryReadRelationshipProofDenialStage::SyntheticRuntimeContext
        );
        assert_eq!(
    ...
    fn relationship_proof_failures_map_to_structured_read_denial() {
        ...
        assert_eq!(
            structured.stage(),
            &ForgeQueryReadRelationshipProofDenialStage::DescriptorAdmission
        );

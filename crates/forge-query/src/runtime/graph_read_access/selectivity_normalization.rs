...
pub(crate) fn normalize_boolean_selectivity_for_access_shape(
    ...
    let branches = expression
        ...
        .map(|branch| {
            let rows = branch
                ...
                .collect::<Vec<_>>();
            ForgeQueryBooleanSelectivityBranch::from_expression_branch(
                ForgeQueryBooleanSelectivityBranchKind::from(branch.branch_kind()),
    ...
    ForgeQueryBooleanSelectivityShape::new(
        access_shape,
        boolean_topology,
        anchor_posture,
...
) -> ForgeQueryBooleanSelectivityAdmissionPosture {
    if rows.iter().any(|row| {
        row.selectivity_class() == &ForgeQueryPredicateSelectivityClass::TraversalPredicate
    }) {
        ForgeQueryBooleanSelectivityAdmissionPosture::RequiresAccessCapabilityRegistration
    } else {
...
fn anchor_posture(
    rows: &[ForgeQueryBooleanPredicateSelectivityRow],
) -> ForgeQueryPredicateAnchorPosture {
    let has_exact = rows
        .iter()
    ...
    match (
        ...
        (true, _, _, _) => ForgeQueryPredicateAnchorPosture::NoPredicateAnchor,
        (false, true, _, true) | (false, false, true, true) => {
            ForgeQueryPredicateAnchorPosture::MixedAnchorAndBroad
        }
        (false, true, _, false) => ForgeQueryPredicateAnchorPosture::AnchoredByExactPredicate,
        (false, false, true, false) => {
            ForgeQueryPredicateAnchorPosture::AnchoredByMembershipPredicate
        }
        (false, false, false, true) => ForgeQueryPredicateAnchorPosture::BroadOnly,
        (false, false, false, false) => ForgeQueryPredicateAnchorPosture::NoPredicateAnchor,
    }
}
...
fn traversal_predicate_ordering_posture(
    ...
    match (rows.len(), pre_traversal_count) {
        (0, _) => ForgeQueryTraversalPredicateOrderingPosture::NoPredicate,
        (total, eligible) if total == eligible => {
            ForgeQueryTraversalPredicateOrderingPosture::PreTraversalEligible
        }
        (_, 0) => ForgeQueryTraversalPredicateOrderingPosture::PostTraversalFilterRequired,
        _ => ForgeQueryTraversalPredicateOrderingPosture::Mixed,
    }
}

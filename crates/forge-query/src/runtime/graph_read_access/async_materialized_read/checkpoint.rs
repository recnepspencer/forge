...
pub struct ForgeQueryGraphReadCheckpointInterval {
    digest: String,
    frontier_pages: usize,
}

impl ForgeQueryGraphReadCheckpointInterval {
    pub fn frontier_pages(frontier_pages: usize) -> Self {
        let normalized_frontier_pages = frontier_pages.max(1);
        let digest = hash_parts(&[
            "forge_query_graph_read_checkpoint_interval_v1".to_string(),
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryGraphReadMaterializationCheckpoint {
    digest: String,
...
impl ForgeQueryGraphReadMaterializationCheckpoint {
    pub fn digest(&self) -> &str {
        &self.digest
    }
    ...
    pub(crate) fn from_progress(
        ...
        let digest = hash_parts(&[
            "forge_query_graph_read_materialization_checkpoint_v1".to_string(),

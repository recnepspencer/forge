use super::ForgeQueryGraphReadMaterializationReceipt;
use crate::identity::hash_parts;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryGraphReadMaterializedRowProof {
    digest: String,
    materialization_digest: String,
    row_ordinal: usize,
}

impl ForgeQueryGraphReadMaterializedRowProof {
    ...
    fn from_materialization(materialization_digest: &str, row_ordinal: usize) -> Self {
        let digest = hash_parts(&[
            "forge_query_graph_read_materialized_row_proof_v1".to_string(),
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryGraphReadMaterializedArtifact {
    digest: String,
    receipt: ForgeQueryGraphReadMaterializationReceipt,
    row_proofs: Vec<ForgeQueryGraphReadMaterializedRowProof>,
}

impl ForgeQueryGraphReadMaterializedArtifact {
    pub fn digest(&self) -> &str {
        &self.digest
    }

    pub fn receipt(&self) -> &ForgeQueryGraphReadMaterializationReceipt {
        &self.receipt
    }
    ...
    pub fn row_proofs(&self) -> &[ForgeQueryGraphReadMaterializedRowProof] {
        &self.row_proofs
    }
    ...
    pub(crate) fn from_receipt(receipt: ForgeQueryGraphReadMaterializationReceipt) -> Self {
        ...
        let digest = hash_parts(&[
            "forge_query_graph_read_materialized_artifact_v1".to_string(),
...
fn materialized_row_proofs_for_receipt(
    receipt: &ForgeQueryGraphReadMaterializationReceipt,
) -> Vec<ForgeQueryGraphReadMaterializedRowProof> {
    (0..receipt.emitted_rows())
        .map(|row_ordinal| {
            ForgeQueryGraphReadMaterializedRowProof::from_materialization(
...
fn materialized_row_proof_set_digest(
    row_proofs: &[ForgeQueryGraphReadMaterializedRowProof],
) -> String {
    hash_parts(
        &row_proofs
            .iter()

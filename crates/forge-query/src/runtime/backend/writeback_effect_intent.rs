...
use crate::runtime::{ForgeQueryBackendAdmissibleMutation, ForgeQueryMutationFamily};
...
pub(crate) struct ForgeQueryBridgeWritebackEffectIntent {
    intent: BridgeWritebackEffectIntent,
}

impl ForgeQueryBridgeWritebackEffectIntent {
...
fn authoritative_patch_from_admitted_mutation(
    mutation: &ForgeQueryBackendAdmissibleMutation,
) -> Result<AuthoritativeRecordAspectPatch, ForgeQueryWorkspaceError> {
    ...
    if matches!(mutation.mutation_family(), ForgeQueryMutationFamily::Delete) {
    ...
    let mut patch = match AuthoritativeRecordAspectPatch::whole_aspect(whole_sets, whole_clears) {
        ...
        TransitionOutcome::Denied(denial) => {
            return Err(ForgeQueryWorkspaceError::new(format!("{denial:?}")));
        }
...
fn validated_whole_aspect(
    ...
    value: AspectValue,
) -> Result<forge_foundational::facade::ContractValidatedAspectArtifact, ForgeQueryWorkspaceError> {
    ...
    match validate_aspect_value(&contract, value.into()) {
        ...
        TransitionOutcome::Denied(denial) => {
            Err(ForgeQueryWorkspaceError::new(format!("{denial:?}")))
        }
        TransitionOutcome::Deferred(never)
        | TransitionOutcome::Stale(never)
        | TransitionOutcome::RebindRequired(never)
...
fn field_patch(
    ...
    let mask = AspectMask::<MutationMask>::new(
        draft
            ...
            .map(|field| {
                forge_foundational::facade::CanonicalFieldPath::new([field])
...
fn synthetic_struct_contract(
    aspect_key: &AspectKey,
    draft: &FieldPatchDraft,
) -> Result<AspectContract, ForgeQueryWorkspaceError> {
    ...
    let shape = StructAspectShape::new(declarations)
        .ok_or_else(|| ForgeQueryWorkspaceError::new("duplicate field patch declaration"))?;
...
fn field_declaration(
    field: FieldKey,
    value_type: forge_foundational::facade::ScalarAspectType,
    requirement: FieldRequirement,
) -> Result<FieldDeclaration, ForgeQueryWorkspaceError> {
    ...
    .ok_or_else(|| ForgeQueryWorkspaceError::new("invalid synthetic field declaration"))
}

fn single_field_key(
    field_path: &forge_foundational::facade::CanonicalFieldPath,
) -> Result<FieldKey, ForgeQueryWorkspaceError> {
...
fn writeback_intent_error(error: BridgeWritebackEffectIntentError) -> ForgeQueryWorkspaceError {
    ForgeQueryWorkspaceError::new(format!("{error:?}"))
}

...
use super::{
    ForgeQueryDerivedArtifactBinding, ForgeQueryDerivedMaterializationResult,
...
impl ForgeQueryRetainedScalarFieldFact {
    pub fn field_path(&self) -> &ForgeQueryRetainedFieldPath {
        &self.field_path
    }
...
#[derive(Clone, Debug, PartialEq)]
struct ForgeQueryRetainedScalarRow {
    fields: BTreeMap<ForgeQueryRetainedFieldPath, AspectValue>,
}

impl ForgeQueryRetainedScalarRow {
    fn admit_paths_from_materialization(
        ...
        field_paths: Vec<ForgeQueryRetainedFieldPath>,
        ...
        let fields = field_paths
            ...
            .collect::<Result<BTreeMap<_, _>, ForgeQueryRuntimeError>>()?;

        Ok(Self { fields })
    }

    fn into_facts(self) -> Vec<ForgeQueryRetainedScalarFieldFact> {
        self.fields
            .into_iter()
            .map(|(field_path, value)| ForgeQueryRetainedScalarFieldFact { field_path, value })
            .collect()
    }
}
...
impl ForgeQueryRetainedScalarFactSet {
    ...
    pub fn target(&self) -> &ForgeQueryDerivedMaterializationTarget {
        &self.target
    }
...
impl ForgeQueryDerivedArtifactBinding {
    ...
    where
        I: IntoIterator<Item = ForgeQueryRetainedFieldPath>,
    {
        self.consume_scalar_field_paths_for_target(
            &ForgeQueryDerivedMaterializationTarget::from(view),
...
fn consume_scalar_field_paths_from_materialization(
    ...
    target: ForgeQueryDerivedMaterializationTarget,
    materialization: &ForgeQueryDerivedMaterializationResult,
    mut field_paths: Vec<ForgeQueryRetainedFieldPath>,
) -> Result<ForgeQueryRetainedScalarFactSet, ForgeQueryRuntimeError> {
    field_paths.sort();
    field_paths.dedup();
    let facts = ForgeQueryRetainedScalarRow::admit_paths_from_materialization(
        &target,
        materialization,
        field_paths,
    )?

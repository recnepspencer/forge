...
use crate::runtime::{
    ...
    ForgeQueryGraphReadAccessAdmissionPosture, ForgeQueryGraphReadAccessPlanConsumption,
    ForgeQueryGraphReadStreamingReceipt,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryGraphReadAccessReceiptSummary {
    digest: String,
...
impl ForgeQueryGraphReadAccessReceiptSummary {
    ...
    pub fn has_admitted_access_plan(&self) -> bool {
        !self.admitted_plan_digest.is_empty()
            && self.admission_posture != ForgeQueryGraphReadAccessAdmissionPosture::Denied
    }

    pub(in crate::runtime) fn from_execution_parts(
        read_graph_digest: &str,
        plan: &ForgeQueryAdmittedGraphReadAccessPlan,
        plan_consumption: &ForgeQueryGraphReadAccessPlanConsumption,
        ephemeral_receipt: Option<&ForgeQueryEphemeralGraphIndexReceipt>,
        streaming_receipt: Option<&ForgeQueryGraphReadStreamingReceipt>,
    ) -> Self {
        ...
        let digest = hash_parts(&[
            "forge_query_graph_read_access_receipt_summary_v1".to_string(),

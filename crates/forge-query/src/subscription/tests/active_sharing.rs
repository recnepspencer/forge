...
fn attachment_budget(policy: DeliveryBackpressurePolicy) -> SubscriptionConsumerAttachmentBudget {
...
fn equivalent_consumers_share_one_lane_but_keep_distinct_attachment_state() {
    ...
    let duplicate = admit_active_subscription_lane(activation, active_budget()).unwrap();
    ...
    let first_attachment = attach_subscription_consumer(
        ...
        attachment_budget(DeliveryBackpressurePolicy::RetainWithinWindow),
    )
    .unwrap();
...
fn acknowledgement_requires_receipt_for_same_attachment_and_advancing_sequence() {
    ...
    .unwrap();
    let receipt = emitted_receipt(&mut runtime, &attachment, "employee.name");
...
fn acknowledgement_receipt_from_another_consumer_is_denied() {
    ...
    .unwrap();
    let wrong_receipt = emitted_receipt(&mut runtime, &second_attachment, "employee.name");
...
fn slow_consumer_gap_policy_changes_only_that_consumer_delivery_digest() {
    ...
    let slow = attach_subscription_consumer(
        ...
        &second_handle,
        SubscriptionConsumerAttachmentRequest::admitted("consumer-b", "cursor-b"),
        attachment_budget(DeliveryBackpressurePolicy::DropWithGapNotice),
    )
    .unwrap();
    ...
    assert_ne!(
        ...
        slow.performance_receipt().performance_receipt_digest()
    );
...
fn shared_consumers_emit_independent_fanout_delivery_evidence() {
    ...
    .unwrap();
    let first_receipt = emitted_receipt(&mut runtime, &first_attachment, "employee.name");
    let first_fanout_delivery = runtime.counters().fanout_delivery_count();
    let second_receipt = emitted_receipt(&mut runtime, &second_attachment, "employee.name");

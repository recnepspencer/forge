...
fn linear_scan_debt_is_explicit_when_lookup_posture_allows_it() {
    ...
    let admission = admit_active_subscription_lane(
        activation,
        active_budget().with_lookup_class(ActiveLaneLookupClass::LinearScanDebtExplicit),
    )
    .unwrap();
...
fn linear_scan_lookup_class_denies_before_lane_exists() {
    ...
    let error = admit_active_subscription_lane(
        activation,
        active_budget().with_lookup_class(ActiveLaneLookupClass::LinearScanDenied),
    )
    .unwrap_err();
...
fn meaning_mismatch_denies_explicit_shared_join() {
    ...
    let first = admit_active_subscription_lane(activation, active_budget()).unwrap();
    let mismatch = admit_active_subscription_lane(mismatched_activation, active_budget()).unwrap();
    let mut runtime = ActiveSubscriptionRuntime::new();
...
fn durable_checkpoint_overclaim_denies_before_lane_exists() {
    ...
    let error = admit_active_subscription_lane(
        activation,
        active_budget().with_durable_checkpoint_request(),
    )
    .unwrap_err();
...
fn lifecycle_heap_allocation_debt_is_explicit_and_digest_bound() {
    let activation = activation_for(LiveQueryFamily::Detail, None);
    let normal = admit_active_subscription_lane(activation.clone(), active_budget()).unwrap();

...
pub enum ForgeQueryConcurrentHostileMatrixPosture {
    Open,
    Partial,
    Closed,
}

impl ForgeQueryConcurrentHostileMatrixPosture {
...
pub(super) fn classify_concurrent_hostile_matrix_posture(
...
) -> ForgeQueryConcurrentHostileMatrixPosture {
    if topology_satisfied
        ...
        && sabotage_sensitive
    {
        return ForgeQueryConcurrentHostileMatrixPosture::Closed;
    }
    if counter_residue_count > 0 || !topology_satisfied {
        return ForgeQueryConcurrentHostileMatrixPosture::Open;
    }

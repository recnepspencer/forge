...
use super::inventory::forge_query_journal_identity_inventory;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryJournalIdentityScanFailure {
    path: String,
    pattern: &'static str,
}

impl ForgeQueryJournalIdentityScanFailure {
...
pub fn scan_journal_identity_forbidden_patterns(
    workspace_root: &Path,
) -> Vec<ForgeQueryJournalIdentityScanFailure> {
    ...
    forge_query_journal_identity_inventory()
        .iter()
    ...
    forge_query_journal_identity_inventory()
        ...
        .flat_map(|row| {
            row.required_patterns()
                ...
                .map(|pattern| ForgeQueryJournalIdentityScanFailure::new(row.path(), *pattern))
                .collect::<Vec<_>>()
        })
        .collect()
}
...
fn scan_patterns(
    ...
    let Some(contents) = read_relative_path(workspace_root, relative_path) else {
        return vec![ForgeQueryJournalIdentityScanFailure::new(
            relative_path,
    ...
    patterns
        ...
        .map(|pattern| ForgeQueryJournalIdentityScanFailure::new(relative_path, *pattern))
        .collect()
}

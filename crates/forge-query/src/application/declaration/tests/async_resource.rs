...
fn async_source_family_changes_mutate_declaration_identity_explicitly() {
    ...
    let host = handle
        .declare(AsyncResourceReadDeclaration::new(
            "edge:42",
            vec![resource_request(
                ForgeQueryAsyncSourceFamily::HostResource,
                ForgeQueryAsyncLoadingPosture::Blocking,
                ForgeQueryAsyncFailurePosture::FailClosed,
                &[("edge", "edge:42")],
            )],
...
fn async_clause_accessors_retain_normalized_request_meaning() {
    ...
    match clause {
        crate::application::ForgeQueryAsyncDeclarationClause::ResourceRequest {
            source_family,
            loading_posture,
            failure_posture,
            request_identity,
        } => {
            assert_eq!(*source_family, ForgeQueryAsyncSourceFamily::BridgeResource);
            assert_eq!(
                *loading_posture,
                ForgeQueryAsyncLoadingPosture::BackgroundRefresh
            );
        ...
        crate::application::ForgeQueryAsyncDeclarationClause::CompletionRequest { .. } => {
...
fn async_and_temporal_clauses_compose_into_canonical_identity() {
    ...
    let temporal_async = handle
        .declare(
            ...
            .with_temporal(vec![ForgeQueryTemporalDeclarationClause::stale_after(
...
fn async_clauses_fail_closed_without_family_opt_in() {
    ...
    match handle.declare_checked(AsyncUnsupportedSplitEdgeDeclaration::new(
        ...
        crate::application::ForgeQueryDeclaredFamilyChecked::AsyncUnsupported(denial) => {
            assert_eq!(
...
fn async_declaration_families_can_fail_closed_as_deferred_debt() {
    ...
    match handle.declare_checked(DeferredAsyncResourceReadDeclaration::new(
        ...
        crate::application::ForgeQueryDeclaredFamilyChecked::AsyncDeferred(denial) => {
            assert_eq!(
                denial.async_support(),

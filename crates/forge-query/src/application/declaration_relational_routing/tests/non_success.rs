...
fn non_relational_route_plans_fail_closed_before_binding() {
    match checked_from_progressed(
        ...
        ForgeQueryDeclarationRelationalRoutingChecked::Denied(denial) => {
            assert_eq!(
                denial.cause(),
                crate::application::ForgeQueryDeclarationRelationalRoutingDenialCause::EnvelopeNotCoveredForRelationalRouting
            );
        }
...
fn explicit_routing_rejects_envelopes_from_the_wrong_handle() {
    ...
    match alternate.route_relational_truth_checked(
        ...
        ForgeQueryDeclarationRelationalRoutingChecked::Denied(denial) => {
            assert_eq!(
                denial.cause(),
                crate::application::ForgeQueryDeclarationRelationalRoutingDenialCause::RelationalEnvelopeMismatch
            );
        }

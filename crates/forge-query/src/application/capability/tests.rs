...
fn facade_support_matrix_stays_in_sync_with_capability_admission() {
    ...
    assert!(report.section_postures().iter().any(|posture| {
        posture.section() == ForgeQueryConfigSectionFamily::Signal
            && posture.owner() == crate::application::ForgeQuerySubsystemOwner::Signal
            && posture.enabled()
    }));
...
fn config_section_resolution_remains_owned_by_subsystem() {
    ...
    assert_eq!(
        ...
        crate::application::ForgeQuerySubsystemOwner::Relational
    );
...
fn invalid_store_config_rejects_before_facade_construction() {
    let error = ForgeQueryApplicationFacade::new(
        ForgeQueryConfig::runtime_backed_default()
            ...
            .with_store(crate::application::ForgeQueryStoreConfig::enabled()),
    )

...
fn denied_route_receipts_preserve_typed_phase_nine_cause() {
    ...
    match handle.receipt_routes_checked(ForgeQueryDeclarationReceiptInput::route_checked(
...
fn public_denial_lanes_diverge_for_distinct_denial_topology() {
    ...
    let required_intent = match handle.receipt_routes_checked(
        ForgeQueryDeclarationReceiptInput::route_checked(route_checked_from_input(
            &handle,
            ReceiptInput::<RequiredIntentReceiptFamily>::new("edge:42"),
        )),
    ) {
        ForgeQueryDeclarationReceiptChecked::Denied(denial) => denial,
        _ => panic!("required intent receipt should deny"),
    };

    let forbidden_intent = match handle.receipt_routes_checked(
        ForgeQueryDeclarationReceiptInput::route_checked(route_checked_with_intent(
            &handle,
            ReceiptInput::<ForbiddenIntentReceiptFamily>::new("edge:42"),
            crate::application::ForgeQueryDeclarationRouteIntent::RelationalOnly,
        )),
    ) {
        ForgeQueryDeclarationReceiptChecked::Denied(denial) => denial,
    ...
    assert_eq!(
        ...
        Some(ForgeQueryDeclarationRoutePlanDenialCause::IntentForbidden)
    );
    assert_ne!(

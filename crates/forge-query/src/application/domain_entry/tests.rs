use super::{ForgeQueryDomainEntryChecked, ForgeQueryDomainEntryMarker};
...
impl ForgeQueryDomainEntryMarker for LocalSpatialDomain {
...
impl ForgeQueryDomainEntryMarker for LocalTopologyDomain {
    fn domain_key(&self) -> &'static str {
        "test.topology"
    }
...
impl ForgeQueryDomainEntryMarker for LocalDeferredArtifactDomain {
    fn domain_key(&self) -> &'static str {
...
fn admitted_checked_entry_matches_ordinary_entry_support_posture() {
    ...
    match checked {
        ForgeQueryDomainEntryChecked::Admitted(admitted) => {
    ...
    match checked {
        ForgeQueryDomainEntryChecked::Unsupported(unsupported) => {
            ...
            assert!(unsupported
                ...
                .iter()
                .any(|posture| {
                    posture.section() == ForgeQueryConfigSectionFamily::Query && !posture.enabled()
                }));
        }

...
fn proof_shape_audit_names_every_required_rejection_lane() {
    ...
    for violation in [
        ...
        BasisLifecycleProofShapeViolation::OperationLaneForgery,
        BasisLifecycleProofShapeViolation::ForgedLowerRuntimeAuthority,
    ] {
        let row = audit
            .row_for(violation)
...
fn proof_shape_audit_rows_require_prior_artifacts_not_digest_substitutes() {
    ...
    for row in audit.rows() {
        ...
        assert!(
            row.rejected_artifact().contains("forged")

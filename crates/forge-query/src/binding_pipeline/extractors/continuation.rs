use crate::application::{
    aspect_coverage_from_publication, ForgeQueryAdmittedConfiguredDomainHandle,
    ForgeQueryDeclarationFamilyMarker, ForgeQueryDeclarationInput, ForgeQueryDomainEntryMarker,
    ForgeQueryDomainOperatingContext,
};
use crate::binding_pipeline::{
    bind_continuation_from_target_on_handle, ForgeQueryBindingCandidateRecord,
...
>(
    ...
    for candidate in candidates {
        ...
        records.push(ForgeQueryBindingCandidateRecord::new(
            label,
            source_kind,
            specificity,
            Some(&binding_target),
    ...
    let request = match bridge_request {
        Some(bridge_request) => {
            ForgeQueryResolveContinuationFromTargetRequest::new(envelope, contract)
                .with_bridge_request(bridge_request)
                .with_partial_denial_if(!partial_narrowing)
                .with_exact_fit_only_if(!allow_superset)
        }
        None => ForgeQueryResolveContinuationFromTargetRequest::new(envelope, contract)
            .with_partial_denial_if(!partial_narrowing)
            .with_exact_fit_only_if(!allow_superset),
    };
...
trait ContinuationRequestFlags<D: ForgeQueryDomainEntryMarker, I: ForgeQueryDeclarationInput<D>> {
    fn with_partial_denial_if(self, deny: bool) -> Self;
    fn with_exact_fit_only_if(self, exact: bool) -> Self;
}

impl<D: ForgeQueryDomainEntryMarker, I: ForgeQueryDeclarationInput<D>>
    ContinuationRequestFlags<D, I> for ForgeQueryResolveContinuationFromTargetRequest<D, I>
{
    fn with_partial_denial_if(self, deny: bool) -> Self {
        if deny {

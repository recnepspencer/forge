...
use crate::binding_pipeline::{
    ...
    ForgeQueryEnvelopeResolverSubject, ForgeQueryResolveEnvelopeFromTargetRequest,
};
...
) -> ForgeQueryBindingTranscript<crate::application::ForgeQueryDeclarationEnvelopeInput<D, I>> {
    let (source, contract, allow_superset, partial_narrowing, route_intent) = request.into_parts();
    match source {
        ForgeQueryEnvelopeResolverSubject::Progression(progressed) => {
            ...
            let receipt_checked = forge_query_checked_declaration_receipt(
            ...
            ForgeQueryBindingTranscript::new(
                ...
                ForgeQueryBindingOutcome::Bound(
        ...
        ForgeQueryEnvelopeResolverSubject::Receipt(receipt) => {

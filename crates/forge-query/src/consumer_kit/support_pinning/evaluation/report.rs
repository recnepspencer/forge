use super::finding::ForgeQuerySupportPinFinding;
use crate::consumer_kit::support_pinning::error::ForgeQuerySupportPinningError;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQuerySupportPinReport {
    ...
    findings: Vec<ForgeQuerySupportPinFinding>,
    report_digest: String,
}

impl ForgeQuerySupportPinReport {
    pub(crate) fn new(
        consumer_name: String,
        contract_digest: String,
        observed_schema_identity: String,
    ...
    pub fn assert_satisfied(&self) -> Result<(), ForgeQuerySupportPinningError> {
        if self.blocking_finding_count == 0 {
            ...
            let blocking_findings = self
                ...
                .cloned()
                .collect::<Vec<_>>();
            Err(ForgeQuerySupportPinningError::with_blocking_findings(
    ...
    pub fn findings(&self) -> &[ForgeQuerySupportPinFinding] {
        &self.findings
    }

...
fn observed_row_drift_is_localized_but_nonblocking() {
    ...
    let observer = support_pinning_contract("write-observer")
        .against_snapshot(&basis)
        .unwrap()
        .observe_family(ForgeQueryRuntimeFacadeFamily::Write)
        .unwrap()
        .seal()
        .unwrap();
...
fn assert_write_finding(
    report: &crate::consumer_kit::support_pinning::ForgeQuerySupportPinReport,
    ...
    let finding = report
        ...
        .iter()
        .find(|finding| {
            finding.kind() == kind && finding.family() == Some(ForgeQueryRuntimeFacadeFamily::Write)
        })

use crate::{
    ForgeQueryBoundaryAuditError, ForgeQueryBoundaryAuditErrorKind, ForgeQueryBoundaryAuditSource,
...
use super::report::{ForgeQueryGraphReadBypassCounters, ForgeQueryGraphReadBypassReport};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryGraphReadBypassAudit {
    consumer_name: String,
    source_sets: Vec<ForgeQueryBoundaryAuditSourceSet>,
    source_inventory_identities: Vec<ForgeQueryEvidenceIdentity>,
}

pub fn graph_read_bypass_audit(consumer_name: impl Into<String>) -> ForgeQueryGraphReadBypassAudit {
    ForgeQueryGraphReadBypassAudit {
...
impl ForgeQueryGraphReadBypassAudit {
    pub fn required_sources(mut self, sources: ForgeQueryBoundaryAuditSourceSet) -> Self {
    ...
    pub fn evaluate(self) -> Result<ForgeQueryGraphReadBypassReport, ForgeQueryBoundaryAuditError> {
        ...
        for source_set in &self.source_sets {
            ...
            for source in source_set.sources() {
                    ...
                    detect_graph_read_bypass_candidates(source.label(), source.path(), &masked)
                {
                    ...
                    let finding = ForgeQueryGraphReadBypassFinding::sealed(
                        candidate.row.class(),
        ...
        let finding_identities = findings
            ...
            .collect::<Vec<_>>();
        let counters = ForgeQueryGraphReadBypassCounters::new(
        ...
        Ok(ForgeQueryGraphReadBypassReport::sealed(
            self.consumer_name,
            audited_source_labels,
            self.source_inventory_identities,
            findings,
...
fn validate_request(
    audit: &ForgeQueryGraphReadBypassAudit,
...
fn validate_source_set_for_graph_read_bypass(
    source_set: &ForgeQueryBoundaryAuditSourceSet,
) -> Result<(), ForgeQueryBoundaryAuditError> {
...
fn sort_findings(findings: &mut [ForgeQueryGraphReadBypassFinding]) {

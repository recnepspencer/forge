use crate::facade::consumer_kit::{
    graph_read_bypass_audit, ForgeQueryBoundaryAuditSourceSet, ForgeQueryGraphReadBypassClass,
...
"#;
    let report = graph_read_bypass_audit("seeded")
        .required_sources(ForgeQueryBoundaryAuditSourceSet::new("seeded").source("lib", source))
        .evaluate()
...
"#;
    let report = graph_read_bypass_audit("multiline")
        .required_sources(ForgeQueryBoundaryAuditSourceSet::new("multiline").source("lib", source))
        .evaluate()
...
"####;
    let report = graph_read_bypass_audit("clean")
        .required_sources(ForgeQueryBoundaryAuditSourceSet::new("clean").source("lib", source))
        .evaluate()
...
fn multi_source_counters_explain_audit_breadth() {
    let report = graph_read_bypass_audit("counter-proof")
        .required_sources(
            ForgeQueryBoundaryAuditSourceSet::new("counter-proof")
                .source_file(
                    "dirty",
                    "src/dirty.rs",
...
fn residue_rows_require_metadata_and_reject_growth() {
    let row = ForgeQueryGraphReadBypassResidueRow::explicit(
        ForgeQueryGraphReadBypassClass::ManualFrontierScan,
        "worth-topo adoption",
    ...
    let missing_owner = ForgeQueryGraphReadBypassResidueRow::explicit(
        ForgeQueryGraphReadBypassClass::ManualFrontierScan,
        "",
        "Milestone 9.10 Phase 15",
        1,
    ...
    assert_eq!(
        missing_owner.kind(),
        &ForgeQueryGraphReadBypassResidueErrorKind::MissingRequiredField
    );
    assert_eq!(missing_owner.field_name(), Some("owner"));

    let cap_error = ForgeQueryGraphReadBypassResidueRow::explicit(
        ForgeQueryGraphReadBypassClass::ManualFrontierScan,
        "worth-topo adoption",
        "Milestone 9.10 Phase 15",
        2,
    ...
    assert_eq!(
        cap_error.kind(),
        &ForgeQueryGraphReadBypassResidueErrorKind::CountExceedsCap
    );

    let previous = ForgeQueryGraphReadBypassResidueManifest::capped([row.clone()])
        .expect("previous residue should admit");
    let duplicate_error =
        ForgeQueryGraphReadBypassResidueManifest::capped([row.clone(), row.clone()])
            .expect_err("duplicate residue classes should reject");
    assert_eq!(
        duplicate_error.kind(),
        &ForgeQueryGraphReadBypassResidueErrorKind::DuplicateClass
    );
    let grown = ForgeQueryGraphReadBypassResidueManifest::capped([
    ...
    let growth_error =
        ForgeQueryGraphReadBypassResidueManifest::certify_candidate_against_previous(
            &previous, &grown,
        )
        .expect_err("growing residue class must fail certification");
    assert_eq!(
        growth_error.kind(),
        &ForgeQueryGraphReadBypassResidueErrorKind::ResidueGrowth
    );

    let changed_contract = ForgeQueryGraphReadBypassResidueManifest::capped([
        ForgeQueryGraphReadBypassResidueRow::explicit(
            ForgeQueryGraphReadBypassClass::ManualFrontierScan,
            "different owner",
            row.introduced_in(),
    ...
    assert_eq!(
        contract_error.kind(),
        &ForgeQueryGraphReadBypassResidueErrorKind::ResidueContractChanged
    );
}

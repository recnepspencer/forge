...
pub(super) fn collect_rs_files(
    crate_name: &str,
    root: &Path,
    files: &mut Vec<ForgeQueryBoundaryAuditSourceInventoryFile>,
...
fn discovered_inventory_file(
    crate_name: &str,
    path: &Path,
) -> Result<ForgeQueryBoundaryAuditSourceInventoryFile, ForgeQueryBoundaryAuditError> {
    ...
    Ok(ForgeQueryBoundaryAuditSourceInventoryFile::discovered(

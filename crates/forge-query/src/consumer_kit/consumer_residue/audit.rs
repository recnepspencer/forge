...
use super::report::{ForgeQueryConsumerResidueReport, ForgeQueryConsumerResidueReportCounters};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryConsumerResidueAudit {
    consumer_name: String,
    required_roots: Vec<PathBuf>,
    allowed_query_owned_roots: Vec<PathBuf>,
    class_filter: Option<Vec<ForgeQueryConsumerResidueClass>>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeQueryConsumerResidueQueryOwnedRootAuthority {
    _private: (),
}

pub fn query_consumer_residue_audit(
    consumer_name: impl Into<String>,
) -> ForgeQueryConsumerResidueAudit {
    ForgeQueryConsumerResidueAudit {
...
pub(crate) fn query_owned_consumer_residue_root_authority(
    ...
    ForgeQueryConsumerResidueQueryOwnedRootAuthority { _private: () }
}

impl ForgeQueryConsumerResidueAudit {
    ...
    pub fn required_query_owned_implementation_root(
        ...
        _authority: &ForgeQueryConsumerResidueQueryOwnedRootAuthority,
    ) -> Self {
        let root = root.into();
    ...
    pub(crate) fn with_class_filter(
        mut self,
        classes: impl IntoIterator<Item = ForgeQueryConsumerResidueClass>,
    ) -> Self {
        self.class_filter = Some(classes.into_iter().collect());
        self
    }

    pub fn evaluate(self) -> Result<ForgeQueryConsumerResidueReport, ForgeQueryBoundaryAuditError> {
...
struct ForgeQueryConsumerResidueScan {
...
fn validate_consumer_residue_audit_request(
    audit: &ForgeQueryConsumerResidueAudit,
...
fn scan_consumer_residue_required_roots(
    audit: &ForgeQueryConsumerResidueAudit,
) -> Result<ForgeQueryConsumerResidueScan, ForgeQueryBoundaryAuditError> {
    let mut audited_roots = Vec::new();
    let mut audited_source_paths = Vec::new();
    let mut findings = Vec::new();
    let mut counters = ForgeQueryConsumerResidueReportCounters::default();
    ...
    Ok(ForgeQueryConsumerResidueScan {
        audited_roots,
...
fn assert_consumer_residue_root_exists(root: &Path) -> Result<(), ForgeQueryBoundaryAuditError> {
    if root.exists() {
        return Ok(());
    }
    Err(ForgeQueryBoundaryAuditError::new(
        ForgeQueryBoundaryAuditErrorKind::MissingRequiredRoot,
        format!(
...
fn sort_consumer_residue_findings(findings: &mut [ForgeQueryConsumerResidueFinding]) {
...
fn seal_consumer_residue_report(
    consumer_name: String,
    scan: ForgeQueryConsumerResidueScan,
    ...
    ForgeQueryConsumerResidueReport::sealed(
        consumer_name,
        scan.audited_roots,
        scan.findings,
        finding_identities,
...
fn seal_consumer_residue_source_inventory(
    ...
    ForgeQueryConsumerResidueSourceInventory::sealed(
        audited_source_paths,
        skipped_non_rust_file_count,
...
fn scan_root(
    ...
    findings: &mut Vec<ForgeQueryConsumerResidueFinding>,
    counters: &mut ForgeQueryConsumerResidueReportCounters,
) -> Result<(), ForgeQueryBoundaryAuditError> {

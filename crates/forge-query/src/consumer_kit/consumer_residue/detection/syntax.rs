...
use super::super::registry::ForgeQueryConsumerResidueClass;
...
pub(crate) struct ForgeQueryConsumerResidueSyntaxClassification {
...
pub(crate) fn classify_consumer_residue_syntax(
...
) -> Result<ForgeQueryConsumerResidueSyntaxClassification, ForgeQueryBoundaryAuditError> {
    ...
    Ok(ForgeQueryConsumerResidueSyntaxClassification {
...
struct ConsumerResidueVisitor<'a> {
    ...
    source_path: &'a str,
    is_query_owned: bool,
    class_filter: Option<&'a [ForgeQueryConsumerResidueClass]>,
...
impl<'a> ConsumerResidueVisitor<'a> {
    ...
    fn record(
        ...
        self.findings
            .push(ForgeQueryConsumerResidueFinding::discovered(
...
impl<'ast> Visit<'ast> for ConsumerResidueVisitor<'_> {
    ...
    fn visit_type_path(&mut self, node: &'ast syn::TypePath) {
        self.visit_counted();
        if path_contains_ident(&node.path, "ForgeQuerySupportSnapshotRow") {
            self.record(

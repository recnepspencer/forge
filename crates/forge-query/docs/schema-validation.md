...
Schema validation is Forge Query's legality gate for canonical query bundles.
...
Validation is not a cosmetic schema check. It is the point where Forge Query
...
```rust
use forge_query::validation::validate_canonical_bundle;
...
```rust
use forge_query::validation::{validate_canonical_bundle, QueryValidationError};
...
- Use [Query Expressions And Result Shapes](./query-expressions-and-result-shapes.md)
...
- Validation happens before [Historical Basis, Diff, And Comparison Queries](./historical-diff-and-basis.md)
...
- [Query Expressions And Result Shapes](./query-expressions-and-result-shapes.md)

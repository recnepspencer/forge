...
consume `forge-query` correctly. Use it when a crate needs evidence reports,
...
- you need a test workspace that uses ordinary `ForgeQueryWorkspace` behavior
...
use forge_query::facade::consumer_kit::{
    ...
    support_pinning_contract, ForgeQueryBoundaryAuditSourceSet,
    ForgeQueryPinnedSupportStatus, ForgeQueryPinnedTeachingPosture,
    ForgeQueryRuntimeFacadeFamily, ForgeQueryTestBackendSchema,
    in_memory_test_runtime,
};
...
let sources = ForgeQueryBoundaryAuditSourceSet::new("workflow-editor")
    .source_file(
        "read-tests",
        "tests/read_path.rs",
        r#"
        fn read_path(workspace: &mut ForgeQueryWorkspace) {
...
- `ForgeQueryBoundaryAuditReport::findings()`
- `ForgeQueryBoundaryAuditReport::report_identity()`
...
```rust
let closure = forge_query::facade::ForgeQueryApplicationFacade::runtime_backed_default()
    .support_report()
...
- The boundary audit is syntax-based. Associated-path detection checks registry
  public-symbol suffixes such as `ForgeQueryWorkspace::write`. Method-call

...
- avoid building restart/recovery flows on debt labeled `Deferred` or `BlockedOnForgeStore`
...
1. **Capability families** (`ForgeQueryCapabilityFamily` in application support) — coarse lanes like `DurableArtifacts` marked deferred at the registry level.
2. **Runtime public support** (`ForgeQueryRuntimePublicSupportMatrix` / `runtime/support/profile.rs`) — `StoreBackedExecution`, `DurableArtifacts`, and related rows.
3. **Lane-specific profiles** — saved-query (`runtime_backed_saved_query_support_profile`), query-context, subscription matrix rows.

**Runtime-backed** means admission, execution, and evidence are coherent inside the current workspace/runtime instance. **Store-backed** means reload, cross-process continuity, or forge-store durability—largely **not** verified on the public path today.

## Main Entry Points

| Area | Symbols / modules |
|------|-------------------|
| Capability registry | `application/support/registry.rs` — `ForgeQueryCapabilityFamily` |
...
| Facade workspace | `ForgeQueryRuntime`, `ForgeQueryWorkspace` — same runtime instance for compose/execute/live |
...
1. Open or build a `ForgeQueryRuntime` / workspace (runtime-backed instance).
...
```text
Host app
  └── ForgeQueryRuntime (process-local)
        ├── plans / receipts / live graph
        ├── inspection evidence (retained)
        └── [deferred] forge-store reload / durable cursors
```
...
- API names containing “saved”, “context”, or “subscription” do **not** guarantee forge-store durability.
...
- Building “write to DB then reload query context from store” on `BlockedOnForgeStore` rows.
...
| `ForgeQueryCapabilityFamily::DurableArtifacts` | **Deferred** |

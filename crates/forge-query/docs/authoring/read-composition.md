...
The canonical artifact underneath that flow is `ForgeQueryReadGraph`.
...
closure wrapper by hand. The kernel now exposes `ForgeQueryReadFamily` as a
...
1. `compose_read(...)` opens a `ForgeQueryReadBuilder`
...
4. that request canonicalizes into one `ForgeQueryReadGraph`
...
8. the runtime returns `ForgeQueryReadResult { payload, receipt }`
...
1. the runtime builds the same admitted `ForgeQueryReadGraph`
2. it exposes that graph through `ForgeQueryReadInvariantPackContext`
...
- a `ForgeQueryReadDomainInvariantSummary`
...
```rust
use forge_query::facade::{
    AspectFieldSelector, AuthoredResultShapeField, ForgeQueryReadInvariantPackViolation,
...
assert!(matches!(
    family.admission(),
    forge_query::facade::ForgeQueryReadFamilyAdmission::DomainInvariantAdmitted(_)
));
```
...
assert!(manager_detail
    .receipt()
    .operator_families()
    .contains(&forge_query::facade::ForgeQueryReadOperatorFamily::Predicate));
```
...
`ForgeQueryReadDenial`, which is then surfaced through
...
`ForgeQueryRuntimeError::ReadCompositionDomainInvariantDenied(...)` instead.

...
- read `ForgeQueryMutationSurfaceReport` / closeout support without implying store-backed durable mutation logs
...
- `ForgeQueryWorkspace::public_authoritative_mutation_evidence_support()`
- `ForgeQueryWorkspace::public_authoritative_mutation_evidence_closeout()`
- `ForgeQueryWorkspace::public_mutation_surface_report()`
- `runtime/surface/mutation_evidence/` — `ForgeQueryMutationProvenanceEvidence`, batch aggregates
...
- `ForgeQueryMutationProvenanceEvidence` exposes digest accessors (`contract_digest`, `causality_digest`, `authoritative_artifact_digest`, etc.)—stable for tests and agents.

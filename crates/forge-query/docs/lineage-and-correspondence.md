...
Lineage and correspondence queries let Forge Query ask identity questions that
...
Forge Query keeps these concerns explicit because a record can correspond
...
```rust
use forge_query::correspondence::{
...
```rust
use forge_query::identity_evolution::{
...
- Use [Schema Validation](./schema-validation.md) and the canonical query stack

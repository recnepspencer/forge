...
shape, authority posture, relationship-proof posture, and work budget.
...
Diagnostics then preserve how the system moved from selection to activation or
why it denied before getting there.

## How It Executes

1. `select_query_subscription_family(...)` classifies the query and admitted
   view shape.
2. Selection verifies required widths, bridge families, allocation budgets, and
   relationship-proof posture.
3. If selection succeeds, the runtime lowers the declaration and continues
   through bridge and admission stages.
4. Diagnostic traces record the stage-by-stage outcomes.
5. `report_query_subscription_support(...)` can emit a support matrix and
...
```rust
let error = select_query_subscription_family(masked_detail_query, work_budget)
    .expect_err("detail query cannot select a table-family subscription");

assert_eq!(error.failure_class().as_str(), "view_mismatch");
```
...
- View family from [Scopes, Templates, Saved Queries, And View Shapes](./scopes-templates-saved-queries-and-view-shapes.md)
  is a major input to selection.
- Authority and basis posture connect directly to
  [Aspects And Authority Lanes](./aspects-and-authority-lanes.md).
- Runtime-facing consumers eventually observe the selected family through
  [Live Views](./live-views.md), [Computed](./computed.md), and

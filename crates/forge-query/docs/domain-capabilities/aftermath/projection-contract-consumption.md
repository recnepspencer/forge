...
- `forge_query_domain(...).for_admitted_intent_plan(...).establishes_projection_contract(...).because(...).materialize()`
- `forge_query_domain(...).for_admitted_intent_plan(...).declares_projection_residue(...).because(...).materialize()`

Supporting request type:

- `ForgeQueryProjectionContractRequest`
...
2. build a `ForgeQueryProjectionContractRequest`
...
```rust
let contract = forge_query_domain("worth.spatial")

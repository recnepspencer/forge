...
- Pair this with [Read Composition](./read-composition.md) when you need typed
  consumed facts from a `ForgeQueryReadResult` rather than only its payload.
- Pair it with [Reads, Observation, and Materialization](./reads-observe-materialize.md)
  when you need to decide whether rows are enough or whether a caller really
  needs typed fact consumption.
- Pair it with [Writes And Intents](./writes-and-intents.md) when a write
...
- Pair it with [Support Matrix And Admission](./support-matrix-and-admission.md)
...
- [Reads, Observation, and Materialization](./reads-observe-materialize.md)

...
- consuming staged work through `workspace.next_effect_intent(...)` remains tied
  to intent-family support posture
...
6. If the effect staged pending write intent and the runtime admits that path,
   `workspace.next_effect_intent(...)` consumes one pending unit through the
   intent authority path.
...
```rust
use forge_query::facade::{ForgeQueryEffectHandle, ForgeQueryLiveView};
...
- Use [Live Views](./live-views.md) when the trigger should be current truth.
- Use [Computed](./computed.md) when the trigger should be derived runtime
  state.
...
- Pending write-intent staging is available, but authoritative execution still
  depends on admitted intent support.

...
fn family_action_key(
    ...
    match action {
        ...
        forge_proof::FamilyLifecycleAction::Rewrite { target, .. } => (1, None, Some(*target.value())),
        forge_proof::FamilyLifecycleAction::Supersede { target, .. } => (2, None, Some(*target.value())),
        forge_proof::FamilyLifecycleAction::Create { symbol, .. } => (3, Some(*symbol.value()), None),
    }
}
...
fn compose_ready_flow(
    ...
    let joined = compose_ready(
        forge_proof::TransitionOutcome::<_, &'static str>::success(left),
        || forge_proof::TransitionOutcome::success(right),
    );

...
`forge-proof` is Forge's compile-time proof-bearing progression substrate.
...
Those boundaries matter. `forge-proof` owns progression law. `forge-foundational` owns shared descriptive and boundary vocabulary.
...
`forge-proof` gives you typed carriers whose shape says what is true right now.
...
This is deliberate. `forge-proof` does not collapse denial, deferment, stale inputs, rebind-required inputs, and hard failure into one undifferentiated error channel.
...
This is how `forge-proof` prevents hidden "deserialize and hope" flows.
...
The point is not to create a runtime engine inside `forge-proof`. The point is to keep execution-admitted law honest at compile time.
...
## Relationship To forge-foundational
...
- `forge-proof` says what is legal and what has been proven
- `forge-foundational` says how to describe and package that at boundaries

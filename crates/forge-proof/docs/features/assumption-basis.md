...
Assumption-basis types make the basis of a stronger form explicit. Instead of carrying "some context" implicitly, `forge-proof` wraps that dependency in `AssumptionBasis<B>` or `FreshnessScopedBasis<F, B>` so the type tells you what the form currently depends on.
...
```rust
use forge_proof::AssumptionBasis;
...
- `forge-proof` owns basis carriage and weakening law, not basis semantics for every domain

...
fn graph_read_access_docs_share_runtime_vocabulary() {
    let posture_labels = WorthQueryGraphReadAccessAdmissionPosture::ALL
        .iter()
        .map(WorthQueryGraphReadAccessAdmissionPosture::as_str)
        .collect::<Vec<_>>();
    let denial_labels = WorthQueryGraphReadAccessDenialKind::ALL
        .iter()
        .map(WorthQueryGraphReadAccessDenialKind::as_str)
        .collect::<Vec<_>>();
    let requirement_labels = WorthQueryGraphReadAccessRequirementKind::all()
        .iter()
        .map(WorthQueryGraphReadAccessRequirementKind::as_str)
        .collect::<Vec<_>>();
    let owner_labels = WorthQueryGraphReadRequiredCapabilityOwner::ALL
        .iter()
        .map(WorthQueryGraphReadRequiredCapabilityOwner::as_str)
        .collect::<Vec<_>>();

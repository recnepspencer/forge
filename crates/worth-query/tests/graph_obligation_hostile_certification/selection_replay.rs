use worth_query::facade::runtime::WorthQueryGraphObligationIndex;
...
fn selection_replay_is_pure_for_equivalent_touch_world_and_index() {
    let first_index = WorthQueryGraphObligationIndex::from_catalog(&registration_catalog());
    let second_index = WorthQueryGraphObligationIndex::from_catalog(&registration_catalog());
...
fn false_fire_touch_does_not_select_unrelated_graph_obligations() {
    let index = WorthQueryGraphObligationIndex::from_catalog(&registration_catalog());

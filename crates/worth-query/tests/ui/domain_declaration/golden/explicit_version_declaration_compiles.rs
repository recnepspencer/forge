...
fn main() {
    ...
    let version = WorthQueryDeclarationCanonicalizationVersion::explicit(
        CanonicalizationRuleVersion::new("worth.query.declaration.v1").unwrap(),
    );
    let declaration = handle.declare_with_version(SplitEdgeDeclaration, version).unwrap();
    let _ = declaration.version();
}

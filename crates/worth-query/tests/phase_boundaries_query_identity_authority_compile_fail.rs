...
fn query_identity_authority_substitution_boundaries_hold() {
    ...
    for target in
        worth_query::facade::identity_authority::worth_query_identity_phase_one_compile_fail_targets(
        )
    {
        t.compile_fail(target.path());
    }
    for target in worth_query::facade::identity_authority::worth_query_identity_phase_one_subscription_phase_seven_reentry_targets(
    ) {
        t.compile_fail(target.path());
    }
}

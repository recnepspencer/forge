...
impl WorthQueryDerivedViewMaintainer for PublicHostileMaintainer {
    fn maintain(
        &mut self,
        ...
        WorthQueryDerivedPatch::whole_refresh_materialized(
            view.name(),
            WorthQueryCommitIdentity::from_relational_commit_id((next + 1) as u64),
            [touch("title.value")],
...
struct PublicBridgeHostileSubmissionRead {
    ...
    title_read: WorthQueryPublicBridgeProjectionConsumptionEvidence,
...
fn record_public_bridge_branch_churn(workspace: &mut WorthQueryWorkspace) -> (String, String) {
...
fn discard_public_bridge_preview_churn(workspace: &mut WorthQueryWorkspace) -> String {
...
) -> WorthQueryPublicBridgeReaderLaneCertification {
    let reader_lane = WorthQueryPublicBridgeReaderLaneCertification::certify(
        projection_reads,
...
fn insert_task_command(id: &str, title: &str) -> WorthQueryWriteCommand {

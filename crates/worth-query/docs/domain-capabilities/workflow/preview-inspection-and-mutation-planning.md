...
- `worth_query_domain(...).for_intent(...).inspects_query_preview(...).because(...).materialize()`
...
- `WorthQueryIntentPreviewMutationPlan::workflow_declaration()`
- `WorthQueryIntentPreviewMutationPlan::into_workflow_declaration()`
...
let artifact = worth_query_domain("worth.spatial")
    ...
    .inspects_query_preview(
        ...
        worth_query::facade::runtime::BridgePreviewSessionIdentity::new("preview-session:42"),
    )
...
```rust
let plan = worth_query_domain("worth.spatial")
    .for_intent(&declaration)
    .plans_preview_mutation(
        "topology.preview_mutation",
        worth_query::facade::runtime::BridgePreviewSessionIdentity::new("preview-session:77"),
    )

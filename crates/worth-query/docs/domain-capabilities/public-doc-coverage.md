...
- `WorthQueryPublicDocCoverageInventory::rows()`
- `WorthQueryPublicDocCoverageInventory::row_for_public_name(...)`
- `WorthQueryPublicDocCoverageAudit::current()`
- `WorthQueryPublicDocCoverageAudit::from_inventory(...)`
- `WorthQueryPublicDocCoverageRow`
- `WorthQueryPublicDocReference`
...
`WorthQueryPublicDocCoverageInventory::current()` starts from
...
`WorthQueryPublicDocCoverageAudit::current()` then checks that published
...
```rust
for golden in worth_query_public_doc_coverage_golden_transcripts() {
    let _ = golden.label();
    let _ = golden.path();

...
- `worth_query_domain(...).for_intent(...).advises(...).because(...).materialize()`
- `worth_query_domain(...).for_intent(...).violates_invariant(...).because(...).materialize()`
- `worth_query_domain(...).for_admitted_intent_plan(...).advises(...).because(...).materialize()`
- `worth_query_domain(...).for_admitted_intent_plan(...).violates(...).because(...).materialize()`
...
You are not constructing a raw `WorthQueryIntentAdmissionDecision` yourself.
...
```rust
let decision = worth_query_domain("worth.spatial")
...
let digest = match decision {
    WorthQueryIntentAdmissionDecision::Advisory(advisory) => advisory.decision_digest(),
    WorthQueryIntentAdmissionDecision::Admitted(plan) => plan.decision_digest(),
    WorthQueryIntentAdmissionDecision::Violation(violation) => violation.decision_digest(),
};
```

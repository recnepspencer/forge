...
- `worth_query_domain(...).for_intent(&declaration)`
- `worth_query_domain(...).for_admitted_intent_plan(&plan)`
...
```rust
let declaration_artifact = worth_query_domain("worth.spatial")
...
```rust
let runtime_decision = worth_query_domain("worth.spatial")

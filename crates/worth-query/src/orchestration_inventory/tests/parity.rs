use crate::application::WorthQueryDeclarationEntryOrchestrationVerbInventory;
use crate::orchestration_inventory::{
    WorthQueryOrchestrationInventoryAudit, WorthQueryOrchestrationSurfaceFamily,
...
fn declaration_entry_inventory_projects_from_canonical_orchestration_inventory() {
    let declaration_inventory = WorthQueryDeclarationEntryOrchestrationVerbInventory::current();
    let projected = WorthQueryOrchestrationSurfaceInventory::current()
        .rows()
        .iter()
        .filter(|row| {
            matches!(
            ...
            ) && row.visibility() != WorthQueryOrchestrationSurfaceVisibility::OrdinaryOutcome
        })

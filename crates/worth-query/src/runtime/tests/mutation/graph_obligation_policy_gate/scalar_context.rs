...
fn collaborative_and_restricted_policy_rules_produce_different_gate_verdicts() {
    ...
    assert_eq!(
        ...
        Some(WorthQueryGraphMutationPolicyGateVerdict::Allow)
    );
...
fn narrowed_policy_basis_advises_without_losing_basis_identity() {
    ...
    assert_eq!(
        ...
        Some(WorthQueryGraphMutationPolicyGateVerdict::Advise)
    );
    assert_eq!(
        receipt
            .obligation_dispatch()
...
fn assert_task_live_row_count(runtime: WorthQueryRuntime, workspace: &str, expected: usize) {

...
fn equivalent_touch_fronts_preserve_same_obligation_authority_projection() {
    ...
    assert_eq!(
        ...
        Some(WorthQueryGraphObligationDispatchContextKind::AuthoritativeCommandBatch)
    );
    assert_eq!(
...
fn runtime_with_selection_foundation_collection_obligation(collection: &str) -> WorthQueryRuntime {
...
fn dispatch_authority_signature(
    projection: &WorthQueryAuthoritativeMutationObligationDispatchProjection,
) -> Vec<(
    String,
    String,
    String,

...
pub enum WorthQueryBridgeBackedVerificationSupportStatus {
    Admitted,
    Denied,
}

impl WorthQueryBridgeBackedVerificationSupportStatus {
...
pub struct WorthQueryBridgeBackedVerificationSupportRow {
    operation_family: String,
    target_binding_family: String,
    current_posture_status: WorthQueryBridgeBackedVerificationSupportStatus,
...
impl WorthQueryBridgeBackedVerificationSupportRow {
    pub(crate) fn new(
    ...
    pub fn current_posture_status(&self) -> WorthQueryBridgeBackedVerificationSupportStatus {
...
) -> Vec<WorthQueryBridgeBackedVerificationSupportRow> {
    support_profile
        .bridge_backed_verification_support_rows()
        .iter()
        .map(|row| {
            let current_posture_status = match support_profile.posture() {
                WorthQueryRuntimeBackendPosture::Scaffold if row.scaffold_profile_supported() => {
                ...
                | WorthQueryRuntimeBackendPosture::Primary => {
                    WorthQueryBridgeBackedVerificationSupportStatus::Denied
                }
            };
            let denial_class = match current_posture_status {
                WorthQueryBridgeBackedVerificationSupportStatus::Admitted => None,
                WorthQueryBridgeBackedVerificationSupportStatus::Denied => {
                    row.denial_class_when_primary_unsupported()
                }
            };
            WorthQueryBridgeBackedVerificationSupportRow::new(

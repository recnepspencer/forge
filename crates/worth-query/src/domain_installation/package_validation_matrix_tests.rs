...
fn full_package(reversed_axes: u8) -> WorthQueryDomainPackage<MatrixDomain> {
    ...
    if reversed_axes & 0b00001 != 0 {
        invariants.reverse();
    }
    if reversed_axes & 0b00010 != 0 {
        obligations.reverse();
    }
    if reversed_axes & 0b00100 != 0 {
        operations.reverse();
    }
    if reversed_axes & 0b01000 != 0 {
        families.reverse();
    }
    if reversed_axes & 0b10000 != 0 {
...
fn every_package_semantic_family_is_order_independent() {
    ...
    for reversed_axes in 1..32 {
        let permuted = full_package(reversed_axes).validate().unwrap();
        assert_eq!(
            permuted.identity(),
            &canonical_identity,
            "canonical package identity drifted for independent ordering mask {reversed_axes:05b}"
        );
        ...
        assert_eq!(
            ...
            "compiled installation products drifted for independent ordering mask {reversed_axes:05b}"
        );
    }
}

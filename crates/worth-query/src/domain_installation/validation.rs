...
fn canonicalize_and_validate_obligations(
    registrations: &mut Vec<WorthQueryGraphObligationRegistration>,
) -> Result<(), WorthQueryDomainPackageValidationDenial> {
    let catalog =
    ...
    if catalog.registration_count() != registrations.len() {
        return Err(WorthQueryDomainPackageValidationDenial::new(
            WorthQueryDomainPackageValidationDenialKind::DuplicateGraphObligation,
            "duplicate graph obligation registration",
        ));
    }
    registrations.sort_by_key(|registration| registration.registration_digest().to_string());
    Ok(())
}

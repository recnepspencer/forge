...
impl WorthQueryDomainInstallationRegistry {
    ...
    fn from_artifacts_at_generation(
        ...
        let execution_index =
            WorthQueryInstalledDomainExecutionIndex::build(&artifacts, runtime_authority);
    ...
    pub(crate) fn rebuild_execution_index_report(
        ...
        let rebuilt = self.rebuild_execution_index();
    ...
    pub(crate) fn rebuild_execution_index(&self) -> WorthQueryInstalledDomainExecutionIndex {
        let artifacts = self
            .records
            .iter()
            .map(|record| record.artifact.clone())
            .collect::<Vec<_>>();
        WorthQueryInstalledDomainExecutionIndex::build(&artifacts, self.runtime_authority)
    }
    ...
    pub(crate) fn destroy_and_rebuild_execution_index(
        ...
        let artifacts = self
            ...
            .map(|record| record.artifact.clone())
            .collect::<Vec<_>>();
        let empty = WorthQueryInstalledDomainExecutionIndex::build(&[], self.runtime_authority);

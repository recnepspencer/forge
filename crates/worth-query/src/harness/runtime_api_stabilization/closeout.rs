...
use crate::runtime::{
    WorthQueryHandleContract, WorthQueryRuntimeFamilySupportStatus,
    WorthQueryRuntimePublicApiContract, WorthQueryRuntimePublicApiNamingContract,
...
impl RuntimeApiStabilizationCloseout {
    pub(super) fn from_matrix(matrix: &RuntimeApiStabilizationCertificationMatrix) -> Self {
        let support_contract = WorthQueryRuntimePublicApiContract::from_support_profile(
            &WorthQueryRuntimeSupportProfile::scaffold_backend_profile(),
        );
        let support_matrix =
            WorthQueryRuntimePublicSupportMatrix::from_public_api_contract(&support_contract);
        let handle_contract = WorthQueryHandleContract::from_public_api_contract(&support_contract);
        let naming_contract = WorthQueryRuntimePublicApiNamingContract::standard();
        ...
        let must_not_assume_yet = vec![
            ...
            "domain-specific geometry/workflow/table semantics live inside worth-query".to_string(),
        ];
        let migration_guidance = vec![
            "enter through WorthQueryRuntime::workspace and keep downstream APIs handle-shaped"
                .to_string(),
        ...
        let required_verification_commands = vec![
            ...
            "cargo test -p worth-query runtime_api_stabilization".to_string(),
            "cargo test -p worth-query runtime_public_support".to_string(),
        ...
        let closeout_self_check_answers = vec![
            ...
            "downstream examples are pressure tests, not worth-query domain semantics".to_string(),
        ];
        let mut parts = vec![

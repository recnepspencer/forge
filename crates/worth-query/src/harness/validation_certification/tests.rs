...
fn schema_aware_rejection_and_projection_legality_adapter_emits_named_matrix() {
    ...
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "legal-detail-query-parity"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "unknown-aspect-projection"));
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "ordering-only-authority-boundary"));
    assert!(matrix
        .rows
        .iter()
    ...
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "redundant-greater-than-normalization"));
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "bounded-range-normalization"));
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "text-contains-predicate-parity"));
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "scalar-membership-predicate-parity"));
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "membership-intersection-normalization"));
    assert!(matrix
        .rows
        .iter()
        .any(|row| row.row_name == "presence-predicate-parity"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "non-orderable-ordering-field"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "predicate-contradiction-rejection"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "empty-range-rejection"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "text-predicate-capability-rejection"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "membership-capability-rejection"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "presence-capability-rejection"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "incompatible-predicate-family"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "invalid-result-shape-binding"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "structured-content-illegality"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "workflow-context-illegality"));
    assert!(matrix
        .rejection_rows
        .iter()
        .any(|row| row.row_name == "forbidden-widening-case"));
}

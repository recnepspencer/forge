use super::error::WorthQueryBoundaryAuditError;
use super::evidence::{
...
use super::source_set::WorthQueryBoundaryAuditSourceSet;
use super::syntax_resolution::{
    classify_boundary_audit_source, hard_prohibition_boundary_audit_call_index,
};

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct WorthQueryHardProhibitionBoundaryAudit;

pub fn hard_prohibition_boundary_audit() -> WorthQueryHardProhibitionBoundaryAudit {
    WorthQueryHardProhibitionBoundaryAudit
}

impl WorthQueryHardProhibitionBoundaryAudit {
    pub fn covering_sources(
        self,
        sources: WorthQueryBoundaryAuditSourceSet,
    ) -> WorthQueryBoundaryAuditEvaluation {
        WorthQueryBoundaryAuditEvaluation { sources }
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthQueryBoundaryAuditEvaluation {
    sources: WorthQueryBoundaryAuditSourceSet,
}

impl WorthQueryBoundaryAuditEvaluation {
    pub fn evaluate(&self) -> Result<WorthQueryBoundaryAuditReport, WorthQueryBoundaryAuditError> {
        ...
        Ok(WorthQueryBoundaryAuditReport::sealed(
    ...
    pub fn assert_clean(&self) -> WorthQueryBoundaryAuditReport {
    ...
    pub fn try_assert_clean(
        &self,
    ) -> Result<WorthQueryBoundaryAuditReport, WorthQueryBoundaryAuditFailure> {
        let report = self
            .evaluate()

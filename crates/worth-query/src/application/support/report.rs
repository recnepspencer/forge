...
impl WorthQuerySupportReport {
    ...
    pub(crate) fn from_validated_config_and_matrix(
        ...
        let query_read_surface_available = support_matrix
            .descriptor(WorthQueryCapabilityFamily::QueryRead)
            .is_some_and(|descriptor| descriptor.status() == WorthQueryCapabilityStatus::Admitted);
        let preview_session_surface_available = support_matrix
            .descriptor(WorthQueryCapabilityFamily::PreviewSession)
            .is_some_and(|descriptor| descriptor.status() == WorthQueryCapabilityStatus::Admitted);
        let hostile_matrix = identity_boundary_hostile_matrix_artifact();
        let identity_boundary_closure = WorthQueryIdentityBoundaryClosure::derived(

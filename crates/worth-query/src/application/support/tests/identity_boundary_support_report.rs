...
use crate::application::{
    identity_boundary_hostile_matrix_artifact, WorthQueryApplicationFacade,
    WorthQueryFolkloreResidueStatus, WorthQueryMilestoneClosureStatus,
...
fn support_report_derives_closed_identity_boundary_from_clean_runtime_backed_surface() {
    ...
    assert_eq!(
        ...
        "WorthQueryRuntimeError::stop_class()"
    );
    assert_eq!(
...
fn support_report_keeps_identity_boundary_partial_when_ordinary_surface_is_disabled() {
    ...
    assert_eq!(closure.status(), WorthQueryMilestoneClosureStatus::Partial);
    assert_eq!(
        closure.evidence_identity().status(),
...
fn support_report_digest_tracks_identity_boundary_publication_and_status() {
    let admitted_report = WorthQueryApplicationFacade::runtime_backed_default().support_report();

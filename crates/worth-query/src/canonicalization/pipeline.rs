use crate::authoring::{
    AuthoredQueryBundleRequest, InternalQueryFamily, InternalResultShapeFamily, RawAuthoredQuery,
...
fn enforce_supported_families(
    #[cfg(test)] query_family: &InternalQueryFamily,
    #[cfg(test)] result_shape_family: &InternalResultShapeFamily,
    #[cfg(not(test))] _query_family: &InternalQueryFamily,
    #[cfg(not(test))] _result_shape_family: &InternalResultShapeFamily,
) -> Result<(), QueryCanonicalizationError> {
    #[cfg(test)]
    {
        if let InternalQueryFamily::Unsupported(family) = query_family {
            return Err(QueryCanonicalizationError::UnsupportedAuthoredQueryFamily { family });
        }

        if let InternalResultShapeFamily::Unsupported(family) = result_shape_family {
            return Err(
                QueryCanonicalizationError::UnsupportedAuthoredResultShapeFamily { family },
            );
        }
    }
...
#[cfg(test)]
pub(crate) fn validate_canonical_ordering_for_test(
    query: &super::artifacts::CanonicalQueryArtifact,
    result_shape: &super::artifacts::CanonicalResultShapeArtifact,
) -> Result<(), QueryCanonicalizationError> {
    validate_canonical_ordering(query, result_shape)
}

#[cfg(test)]
pub(crate) fn validate_digest_basis_consistency_for_test(
    query: &super::artifacts::CanonicalQueryArtifact,
    result_shape: &super::artifacts::CanonicalResultShapeArtifact,
) -> Result<(), QueryCanonicalizationError> {

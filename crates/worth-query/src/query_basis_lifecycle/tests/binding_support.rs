...
) -> worth_runtime_bridge::facade::AdmittedBridgeSubscription {
...
pub fn delivered_continuity(
    runtime: &RuntimeBridge,
) -> worth_runtime_bridge::facade::BridgeDeliveredContinuityResult {
    let route = runtime
        .plan_committed_patch_with_mapping_context(
            worth_runtime_bridge::facade::BridgeRouteRequest::for_commit(test_commit_identity(
                COMMIT_A,
            )),
...
fn snapshot_records(_key: &str, value: &str) -> Vec<SnapshotReadRecord> {
    let read = worth_runtime_bridge::facade::SnapshotReadRequest::for_relational_record(

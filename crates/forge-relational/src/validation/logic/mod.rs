...
use crate::validation::data::{
    ...
    InvariantRule, InvariantViolation,
};
                ...
                match rule {
                    InvariantRule::LiveEntityRequiresKind => {
                        for partition_id in state.partition_ids() {
                            ...
                            let slots =
                                state.touched_entity_slots(partition_id).unwrap_or_else(|| {
                    ...
                    InvariantRule::UniqueEntityPayloadField(field) => {
                            ...
                            touched_visible_entity_ids(state, version_id)
                        {
                            ...
                            let touched_set =
                                touched_entity_ids.iter().copied().collect::<BTreeSet<_>>();
...
fn planned_entity_field_values(
    ...
    for intent in &merged_plan.merged_intents {
        match intent {
            crate::transactions::data::TransactionIntent::CreateEntity(spec) => {
            ...
            crate::transactions::data::TransactionIntent::BulkCreateEntities {
                payloads, ..
            } => {
                saw_entity_change = true;
            ...
            crate::transactions::data::TransactionIntent::UpdateEntity { entity_id, payload } => {
                saw_entity_change = true;
                if let Some(value) = payload
                    .as_json()
                    .and_then(|value| value.get(field))
            ...
            crate::transactions::data::TransactionIntent::DeleteEntity { .. }
            | crate::transactions::data::TransactionIntent::CreateRelation(_)
            | crate::transactions::data::TransactionIntent::BulkCreateRelations { .. }
            | crate::transactions::data::TransactionIntent::DeleteRelation { .. } => {}
        }
    }

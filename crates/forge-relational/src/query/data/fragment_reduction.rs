...
fn reduce_traversal_entities(
    ...
    keyed_entities.sort_by(|left, right| {
        left.0.cmp(&right.0).then_with(|| {
            super::query_entity_record_digest(&left.1)
                .cmp(&super::query_entity_record_digest(&right.1))
        })
    });
    let mut seen = std::collections::BTreeSet::new();
    keyed_entities
...
fn reduce_traversal_relations(
    ...
    keyed_relations.sort_by(|left, right| {
        left.0.cmp(&right.0).then_with(|| {
            super::query_relation_record_digest(&left.1)
                .cmp(&super::query_relation_record_digest(&right.1))
        })
    });
    let mut seen = std::collections::BTreeSet::new();
    keyed_relations

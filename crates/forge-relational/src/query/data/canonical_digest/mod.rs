...
use read_record_terms::{
    encode_entity_read_record, encode_entity_records, encode_relation_read_record,
...
pub(crate) fn query_result_reduction_digest(
    ...
    encode_query_ordering_contract(&mut bytes, ordering);
    encode_entity_records(&mut bytes, entities);
    encode_relation_records(&mut bytes, relations);
    sha256_hex(&bytes)
}

pub(crate) fn query_entity_record_digest(record: &EntityReadRecord) -> String {
    let mut bytes = Vec::new();
...
pub(crate) fn query_relation_record_digest(record: &RelationReadRecord) -> String {
    let mut bytes = Vec::new();
...
fn encode_canonical_query_result(bytes: &mut Vec<u8>, result: &CanonicalQueryResult) {
    ...
    encode_entity_records(bytes, &result.entities);
    encode_relation_records(bytes, &result.relations);

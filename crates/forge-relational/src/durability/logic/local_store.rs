use std::fs;
use std::path::{Path, PathBuf};

use serde::{Deserialize, Serialize};

use crate::durability::data::{
    DurabilityError, DurableCheckpoint, DurableCheckpointId, DurableSegmentId, DurableStore,
    DurableStoreLayout, RecoveryFailureClass,
};
use crate::logic::runtime::RelationalRuntime;

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(super) struct DurableStoreManifestFile {
    pub(super) store: DurableStore,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(super) struct DurableSegmentFile {
    pub(super) entries: Vec<crate::durability::data::DurableCommitEnvelope>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(super) struct DurableCheckpointFile {
    pub(super) checkpoint: DurableCheckpoint,
}

impl RelationalRuntime {
    pub(super) fn ensure_loaded_store(&self) -> Result<DurableStore, DurabilityError> {
        if let Some(store) = &self.durability.store {
            return load_or_initialize_store(store.layout.clone());
        }
        let Some(layout) = self.config.durable_store_layout.clone() else {
            return Err(DurabilityError {
                class: RecoveryFailureClass::DurableIoFailure,
                detail: "persisted durability mode requires a durable store layout".to_string(),
            });
        };
        load_or_initialize_store(layout)
    }

    pub(super) fn load_store_from_disk(&self) -> Result<DurableStore, DurabilityError> {
        let Some(layout) = self.config.durable_store_layout.clone() else {
            return Err(DurabilityError {
                class: RecoveryFailureClass::DurableIoFailure,
                detail: "persisted durability mode requires a durable store layout".to_string(),
            });
        };
        load_or_initialize_store(layout)
    }

    pub(super) fn persist_store_manifest(
        &self,
        store: &DurableStore,
    ) -> Result<(), DurabilityError> {
        ensure_store_dirs(&store.layout)?;
        write_json(
            &manifest_path(&store.layout),
            &DurableStoreManifestFile {
                store: store.clone(),
            },
        )
    }
}

pub(super) fn current_segment_ids(store: Option<&DurableStore>) -> Vec<DurableSegmentId> {
    store
        .map(|store| {
            store
                .segments
                .iter()
                .map(|segment| segment.segment_id)
                .collect()
        })
        .unwrap_or_default()
}

pub(super) fn manifest_path(layout: &DurableStoreLayout) -> PathBuf {
    layout.root_path.join("manifest.json")
}

pub(super) fn segment_file_path(
    layout: &DurableStoreLayout,
    segment_id: DurableSegmentId,
) -> PathBuf {
    layout
        .root_path
        .join("segments")
        .join(format!("segment-{}.json", segment_id.0))
}

pub(super) fn checkpoint_file_path(
    layout: &DurableStoreLayout,
    checkpoint_id: DurableCheckpointId,
) -> PathBuf {
    layout
        .root_path
        .join("checkpoints")
        .join(format!("checkpoint-{}.json", checkpoint_id.0))
}

pub(super) fn ensure_store_dirs(layout: &DurableStoreLayout) -> Result<(), DurabilityError> {
    fs::create_dir_all(layout.root_path.join("segments")).map_err(io_error)?;
    fs::create_dir_all(layout.root_path.join("checkpoints")).map_err(io_error)?;
    Ok(())
}

pub(super) fn load_or_initialize_store(
    layout: DurableStoreLayout,
) -> Result<DurableStore, DurabilityError> {
    ensure_store_dirs(&layout)?;
    let manifest = manifest_path(&layout);
    if manifest.exists() {
        return Ok(read_json::<DurableStoreManifestFile>(&manifest)?.store);
    }
    let store = DurableStore {
        layout: layout.clone(),
        segments: Vec::new(),
        checkpoints: Vec::new(),
    };
    write_json(
        &manifest,
        &DurableStoreManifestFile {
            store: store.clone(),
        },
    )?;
    Ok(store)
}

pub(super) fn read_json<T: for<'de> Deserialize<'de>>(path: &Path) -> Result<T, DurabilityError> {
    let bytes = fs::read(path).map_err(io_error)?;
    serde_json::from_slice(&bytes).map_err(|error| DurabilityError {
        class: RecoveryFailureClass::CorruptCheckpoint,
        detail: format!("failed to deserialize {}: {error}", path.display()),
    })
}

pub(super) fn write_json<T: Serialize>(path: &Path, value: &T) -> Result<(), DurabilityError> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(io_error)?;
    }
    let temp_path = path.with_extension("tmp");
    let bytes = serde_json::to_vec_pretty(value).map_err(|error| DurabilityError {
        class: RecoveryFailureClass::DurableIoFailure,
        detail: format!("failed to serialize {}: {error}", path.display()),
    })?;
    fs::write(&temp_path, bytes).map_err(io_error)?;
    fs::rename(&temp_path, path).map_err(io_error)?;
    Ok(())
}

pub(super) fn io_error(error: std::io::Error) -> DurabilityError {
    DurabilityError {
        class: RecoveryFailureClass::DurableIoFailure,
        detail: error.to_string(),
    }
}

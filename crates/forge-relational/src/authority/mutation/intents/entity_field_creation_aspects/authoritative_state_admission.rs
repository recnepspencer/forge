...
pub(super) fn admit_entity_creation_state(
    validated_artifacts: Vec<ContractValidatedAspectArtifact>,
) -> Result<Option<AuthoritativeRecordAspectState>, EntityAuthoritativeAspectStateDenial> {
    ...
    match admit_authoritative_record_aspect_state(validated_artifacts) {
        ...
        TransitionOutcome::Denied(denial) => {
            Err(EntityAuthoritativeAspectStateDenial::StateAdmissionDenied { denial })
        }
    }
}

use forge_foundational::{MaterializedFoundationalProfileSet, SupportProfiledArtifact};
...
fn support_witness_digest(
    ...
    bytes.extend_from_slice(b"forge.relational.merge_support.witness.v1");
...
fn append_profile_set_bytes(
    bytes: &mut Vec<u8>,
    profile: &forge_foundational::FoundationalProfileSet,
) {
    bytes.push(match profile.diagnostic_richness() {
        forge_foundational::DiagnosticRichnessProfile::OperationalMinimal => 1,
        forge_foundational::DiagnosticRichnessProfile::Standard => 2,
        forge_foundational::DiagnosticRichnessProfile::Forensic => 3,
    });
    bytes.push(match profile.support_posture() {
        forge_foundational::SupportPostureProfile::InternalOnly => 1,
        forge_foundational::SupportPostureProfile::SupportReady => 2,
        forge_foundational::SupportPostureProfile::CertificationReady => 3,
    });
    bytes.push(match profile.compatibility_posture() {
        forge_foundational::CompatibilityPostureProfile::NativeOnly => 1,
        forge_foundational::CompatibilityPostureProfile::CompatibilityLowered => 2,
        forge_foundational::CompatibilityPostureProfile::CompatibilityRequired => 3,
    });
    bytes.push(match profile.admission_readiness() {
        forge_foundational::AdmissionReadinessProfile::CandidateOnly => 1,
        forge_foundational::AdmissionReadinessProfile::Admitted => 2,
        forge_foundational::AdmissionReadinessProfile::ProductionGateReady => 3,
    });
    bytes.push(match profile.retention_delivery() {
        forge_foundational::RetentionDeliveryProfile::Ephemeral => 1,
        forge_foundational::RetentionDeliveryProfile::Retained => 2,
        forge_foundational::RetentionDeliveryProfile::Durable => 3,
    });
    bytes.push(match profile.certification_posture() {
        forge_foundational::CertificationPostureProfile::Uncertified => 1,
        forge_foundational::CertificationPostureProfile::EvidenceBacked => 2,
        forge_foundational::CertificationPostureProfile::ProductionCertified => 3,
    });
}

fn append_narrowing_bytes(
    bytes: &mut Vec<u8>,
    narrowing: Option<forge_foundational::FoundationalProfileNarrowingRecord>,
) {
    match narrowing {
        None => bytes.push(0),
        Some(record) => {
            ...
            bytes.push(match record.kind() {
                forge_foundational::FoundationalProfileNarrowingKind::RichnessReduced => 1,
                forge_foundational::FoundationalProfileNarrowingKind::RetentionNarrowed => 2,
                forge_foundational::FoundationalProfileNarrowingKind::SupportPostureReduced => 3,
                forge_foundational::FoundationalProfileNarrowingKind::CertificationPostureReduced => 4,
                forge_foundational::FoundationalProfileNarrowingKind::CompatibilityRestricted => 5,
            });

...
fn derived_index_contract_success_branch_scoped_build_keeps_storage_fallback() {
    ...
    let main_outcome = super::create_entity_outcome(&mut runtime, "main-a");
    runtime
        .create_branch(
        ...
        .unwrap();
    let feature_outcome = super::create_entity_outcome_on_branch(
        &mut runtime,
        "feature-a",
        BranchId("feature".to_string()),
    );
    ...
    let packet = QueryWorkPacket::bulk(
        ...
        vec![ReadTarget::Entity(
            super::changed_entities(&main_outcome)[0],
        )],
    );
...
fn derived_index_contract_failure_unknown_index_keeps_truth_reads_correct() {
    let mut runtime = super::runtime_with_test_schema();
    let outcome = super::create_entity_outcome(&mut runtime, "main-a");
    let snapshot = runtime.snapshot();
    let packet = QueryWorkPacket::bulk(
        "entities",
        vec![ReadTarget::Entity(super::changed_entities(&outcome)[0])],
    );

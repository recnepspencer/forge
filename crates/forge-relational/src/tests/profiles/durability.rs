...
fn explicit_snapshots_can_skip_cache_protection_and_still_read_until_release() {
    ...
    let read_path = runtime.inspect_snapshot_read_path(&snapshot).unwrap();
    assert!(read_path
        .entries
        .iter()
...
fn visibility_cache_recent_window_is_bounded_and_reports_hits() {
    ...
    let first_read_path = runtime.inspect_version_read_path(first.version_id).unwrap();
    ...
    let evicted_first_read_path = runtime.inspect_version_read_path(first.version_id).unwrap();
...
fn branch_head_protection_can_be_lazy_without_populating_visibility_cache() {
    ...
    let _ = runtime.read_version(base.version_id);

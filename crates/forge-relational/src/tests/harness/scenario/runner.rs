...
use crate::tests::support::{
    ...
    test_schema_registry, update_entity_on_branch,
    InternedString, KindId, RecordPayload, SchemaVersionId,
};
...
pub(crate) fn run_seeded_scenario(config: SeededScenarioConfig) -> SeededScenarioWorld {
    ...
    for step in 0..config.steps {
        ...
        let latest_position = runtime.publication_access().latest_patch().unwrap().position;
...
fn choose_operation(
    ...
    match generator.next_u64() % 15 {
        ...
        3 if config.replacement_pressure && !entities.is_empty() => ScenarioOperation::ReplaceEntity {
            entity_slot: random_slot(generator, entities.len()),
            name: String::new(),
            branch_slot: 0,
            partition: scenario_partition(generator.next_u64()),
        ...
        6 if config.relation_pressure && !relations.is_empty() => ScenarioOperation::DeleteRelation {
        ...
        8 if config.branch_pressure && branches.len() > 1 => ScenarioOperation::MergeBranchIntoMain {
...
fn apply_operation(
    ...
    match operation {
        ...
        ScenarioOperation::CaptureSnapshot => snapshots.push(runtime.visibility_authority().snapshot()),
        ...
        ScenarioOperation::DurableCheckpoint
        | ScenarioOperation::CompactDurableStore => {}
    }
}
...
pub(crate) fn build_property_runtime(mode: RuntimeHarnessMode) -> RelationalRuntime {
    match mode {
        RuntimeHarnessMode::InMemory(profile) => crate::facade::runtime::RelationalRuntimeApi::builder()
            .profile(profile)
        ...
        RuntimeHarnessMode::Persisted => crate::facade::runtime::RelationalRuntimeApi::builder()
            ...
            .durable_store_layout(DurableStoreLayout {
                root_path: crate::tests::support::unique_test_store_path("forge-relational-property"),

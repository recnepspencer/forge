...
fn graph_obligation_docs_share_production_support_and_budget_vocabulary() {
    let expected_kind_labels = WorthQueryGraphObligationKind::ALL
        .into_iter()
        .map(WorthQueryGraphObligationKind::as_str)
        .collect::<Vec<_>>();
    let expected_status_labels = WorthQueryGraphObligationSupportStatus::ALL
        .into_iter()
        .map(WorthQueryGraphObligationSupportStatus::as_str)
        .collect::<Vec<_>>();
    let authority_matrix =
        WorthQueryGraphObligationSupportMatrix::milestone_9_9_authority_surface();
    let expected_lane_labels = WorthQueryGraphObligationSupportLane::MILESTONE_9_9_COVERED
        .into_iter()
        .map(WorthQueryGraphObligationSupportLane::as_str)
        .collect::<Vec<_>>();
...
fn budget_terms_from_matrix(matrix: &WorthQueryGraphObligationSupportMatrix) -> Vec<&'static str> {
    let sample = matrix.rows().first().expect("authority matrix row");
    vec![
        "BudgetExceeded",
        WorthQueryGraphObligationExecutionStatus::BudgetExceeded.as_str(),

...
fn alignment_gap_prefixes_cover_every_public_doc_audit_failure_family() {
    let scenarios = [
        (
            WorthQueryPublicDocCoverageAudit::from_inventory(&inventory_without_public_name(
                "orchestrate_signal_compatibility",
            )),
            "undocumented:orchestrate_signal_compatibility",
        ),
        (
            WorthQueryPublicDocCoverageAudit::from_inventory(&inventory_with_replaced_row(
                row_without_golden(&current_row("prepare_continuation_from_target")),
            )),
            "missing_golden:prepare_continuation_from_target",
        ),
        (
            WorthQueryPublicDocCoverageAudit::from_inventory(&inventory_with_extra_row(
                orphan_row(),
            )),
            "orphan_doc:fake_surface",
        ),
        (
            WorthQueryPublicDocCoverageAudit::from_inventory(&inventory_without_golden_label(
...
fn docs_alignment_stays_exact_for_doc_pointer_and_journey_mismatch_failures() {
    let broken_doc_pointer = WorthQueryPublicDocCoverageAudit::from_inventory(
    ...
    let broken_journey = WorthQueryPublicDocCoverageAudit::from_inventory(
...
fn current_inventory_audit_and_closeout_alignment_share_one_clean_truth() {
    let inventory = WorthQueryPublicDocCoverageInventory::current();

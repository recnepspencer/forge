...
use crate::public_doc_coverage::{
    worth_query_public_doc_coverage_golden_transcripts, WorthQueryPublicDocCoverageInventory,
...
fn golden_manifest_is_referenced_by_live_coverage_rows() {
    let coverage = WorthQueryPublicDocCoverageInventory::current();
    let expected = worth_query_public_doc_coverage_golden_transcripts()
        .iter()
        .filter(|row| row.kind() == WorthQueryPublicGoldenTranscriptKind::SurfaceCoverage)
        .map(|row| row.label())

use super::super::{
    hard_prohibition_boundary_audit, WorthQueryBoundaryAuditCoverageMechanism,
    WorthQueryBoundaryAuditSourceSet, WorthQueryBoundaryAuditSyntaxClass,
};
use crate::WorthQueryProhibitedSeam;

#[test]
fn detects_seeded_method_call_bypass_from_registry() {
    let source = "fn worthd_workspace_write(workspace: &mut WorthQueryWorkspace) {\n    workspace.write(command);\n}\n";
...
fn detects_seeded_associated_path_call_bypass_from_registry() {
    let source = "fn worthd_workspace_batch() {\n    WorthQueryWorkspace::batch(commands);\n}\n";
    let report = hard_prohibition_boundary_audit()
        .covering_sources(
        ...
        .covering_sources(
            WorthQueryBoundaryAuditSourceSet::new("worth-kernel").source(
                ...
                fn unrelated_type_with_same_method_name() {
                    ...
                    worth_query::WorthQueryWorkspace::batch(commands);
                }
            "#,
            ),
        )

use crate::WorthQueryBoundaryAuditSourceSite;

use super::super::registry::{
    registry_row_for_class, WorthQueryGraphReadBypassClass, WorthQueryGraphReadBypassRegistryRow,
};
...
pub(in crate::consumer_kit::graph_read_bypass_audit) struct GraphReadBypassCandidate {
    ...
    pub(in crate::consumer_kit::graph_read_bypass_audit) source_site:
        WorthQueryBoundaryAuditSourceSite,
}
    ...
    for (line_index, line) in masked_source.lines().enumerate() {
        ...
        candidates.extend(detect_line_classes(line, &scan_state).into_iter().map(
            |(class, column)| GraphReadBypassCandidate {
                ...
                source_site: WorthQueryBoundaryAuditSourceSite::new(
                    source_label,
                    source_path,
                    line_index + 1,
                    column + 1,
...
) -> Vec<(WorthQueryGraphReadBypassClass, usize)> {
...
fn push_if(
    classes: &mut Vec<(WorthQueryGraphReadBypassClass, usize)>,
    line: &str,

...
) -> Vec<WorthQueryConsumerResidueFinding> {
    let searchable = mask_comments_and_string_literals(source);
    worth_query_consumer_residue_registry()
        .iter()
        .filter(|row| row.detection() == WorthQueryConsumerResidueDetection::ExactText)
        .filter(|row| class_filter_allows(class_filter, row.class()))
        .flat_map(|row| {
            source_locations_for_pattern(&searchable, row.detection_key()).map(
                move |(line, column)| {
                    WorthQueryConsumerResidueFinding::discovered(

...
fn public_surface_inventory_names_batch_and_hidden_runtime_boundaries() {
    ...
    let writeback = inventory
        .rows()
        .iter()
        .find(|row| row.surface_kind() == EffectPublicSurfaceKind::WritebackCommonPath)
        .expect("writeback common-path row should exist");
    assert_eq!(
        writeback.primary_artifact_kind(),
    ...
    assert!(writeback
        .entrypoint()
        .expect("writeback common-path row should advertise an entrypoint")
        .contains("scope_admitted_effect_plan(...).lower().execute()"));
    assert!(writeback.lower_runtime_visibility_hidden());
    ...
    assert!(diagnostics
        ...
        .contains("transition_rules"));
    assert!(diagnostics
        .entrypoint()
        .expect("diagnostics/envelope surface should have a concrete entrypoint")
        .contains("materialize_diagnostics"));

    let certification = inventory
        .rows()
        .iter()
        .find(|row| row.surface_kind() == EffectPublicSurfaceKind::ProductionCertification)
        .expect("production certification row should exist");
    assert_eq!(
        certification.availability(),
        EffectPublicSurfaceAvailability::Implemented
    );
    assert_eq!(certification.primary_artifact_kind(), None);
    assert_eq!(
        certification.entrypoint(),
        Some("certify_effect_execution_pipeline()")
    );
    assert!(certification.lower_runtime_visibility_hidden());
    assert_eq!(
        inventory
            .rows()
            .iter()
            .filter(|row| row.surface_kind() == EffectPublicSurfaceKind::ProductionCertification)
            .count(),
        1
    );
    assert!(inventory.rows().iter().all(|row| {
        row.entrypoint()
            .map(|entrypoint| {
                !entrypoint.contains("certify_effect_lifecycle_phase4")
                    && !entrypoint.contains("certify_effect_lifecycle_seeded")
            })
            .unwrap_or(true)
    }));
}
...
fn support_matrix_retains_deferred_and_rebind_postures_after_enrichment() {
    ...
    assert!(matrix.rows().iter().any(|row| {
        row.basis_family() == BasisFamily::PreviewDerived
            ...
            && row.authority_owner() == EffectAuthorityOwner::WORTHRelational
    }));
    assert!(matrix.rows().iter().any(|row| {
    ...
    assert!(matrix.rows().iter().any(|row| {
        row.basis_family() == BasisFamily::StoreBacked
            ...
            && row.receipt_artifact_kind() == EffectReceiptArtifactKind::WorthQueryWriteReceipt
    }));
}

...
fn external_commit(label: &str) -> WorthQueryCommitIdentity {
...
fn touches<const N: usize>(paths: [&str; N]) -> [WorthQueryAspectTouch; N] {
...
fn retained_value_aspects(rows: &[WorthQueryRetainedMaterializedRow]) -> Vec<AspectValue> {
    let value_path =
...
fn retained_string_field(row: &WorthQueryRetainedMaterializedRow, field: &str) -> String {
    let field_path =
...
fn retained_u64_field(row: &WorthQueryRetainedMaterializedRow, field: &str) -> u64 {
    let field_path =
...
fn dependency_or_produced_touches(view: &WorthQueryDerivedView) -> Vec<WorthQueryAspectTouch> {
    if view.produced_aspect_touches().is_empty() {

...
pub(super) struct WorthQueryBatchReceiptAggregates {
    pub(super) touched_aspects: Vec<WorthQueryAspectTouch>,
...
pub(super) fn derive_batch_receipt_aggregates(
...
) -> WorthQueryBatchReceiptAggregates {
    ...
    WorthQueryBatchReceiptAggregates {
        ...
        considered_computed_view_count: write_receipts
            .iter()
            .map(WorthQueryWriteReceipt::considered_computed_view_count)
            .sum(),
        considered_effect_count: write_receipts
            .iter()
            .map(WorthQueryWriteReceipt::considered_effect_count)
            .sum(),
        delivered_effect_count: write_receipts
            .iter()
            .map(WorthQueryWriteReceipt::delivered_effect_count)
            .sum(),
        pending_write_intent_count: write_receipts
            .iter()
            .map(WorthQueryWriteReceipt::pending_write_intent_count)
            .sum(),
        suppressed_effect_count: write_receipts
            .iter()
            .map(WorthQueryWriteReceipt::suppressed_effect_count)
            .sum(),
        meaningful_effect_suppression_count: write_receipts
            .iter()
            .map(WorthQueryWriteReceipt::meaningful_effect_suppression_count)
            .sum(),
        effect_expression_failure_count: write_receipts
            .iter()
            .map(WorthQueryWriteReceipt::effect_expression_failure_count)
            .sum(),
        refresh_fallback: write_receipts
            .iter()
            .any(WorthQueryWriteReceipt::refresh_fallback),
    }
}

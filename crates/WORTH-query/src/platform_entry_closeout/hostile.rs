...
pub enum WorthQueryPlatformEntryHostileDivergenceClass {
    DistinctSemanticStop,
    DistinctBoundary,
    DistinctCoverageKind,
}

impl WorthQueryPlatformEntryHostileDivergenceClass {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct WorthQueryPlatformEntryHostileRow {
    label: &'static str,
    anchor_surface: Option<&'static str>,
    divergence_class: WorthQueryPlatformEntryHostileDivergenceClass,
    proof_path: &'static str,
    proof_name: &'static str,
}

impl WorthQueryPlatformEntryHostileRow {
    const fn new(
        label: &'static str,
        anchor_surface: Option<&'static str>,
        divergence_class: WorthQueryPlatformEntryHostileDivergenceClass,
    ...
    pub fn divergence_class(&self) -> WorthQueryPlatformEntryHostileDivergenceClass {
...
pub struct WorthQueryPlatformEntryHostileManifest {
    rows: Vec<WorthQueryPlatformEntryHostileRow>,
    hostile_digest: String,
}

impl WorthQueryPlatformEntryHostileManifest {
    fn new(rows: Vec<WorthQueryPlatformEntryHostileRow>) -> Self {
    ...
    pub fn rows(&self) -> &[WorthQueryPlatformEntryHostileRow] {
        &self.rows
    }
    ...
    pub fn row_for_label(&self, label: &str) -> Option<&WorthQueryPlatformEntryHostileRow> {
        self.rows.iter().find(|row| row.label() == label)
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthQueryPlatformEntryHostileAudit {
    hostile_digest: String,
...
impl WorthQueryPlatformEntryHostileAudit {
    pub fn current() -> Self {
        Self::from_manifest(&worth_query_platform_entry_hostile_manifest())
    }

    pub fn from_manifest(manifest: &WorthQueryPlatformEntryHostileManifest) -> Self {
...
pub fn worth_query_platform_entry_hostile_manifest() -> WorthQueryPlatformEntryHostileManifest {
    use WorthQueryPlatformEntryHostileDivergenceClass::{
        DistinctBoundary as B, DistinctCoverageKind as C, DistinctSemanticStop as S,
    };

    WorthQueryPlatformEntryHostileManifest::new(vec![
        WorthQueryPlatformEntryHostileRow::new(
            "signal_compatible_remains_distinct_from_prepared",
...
mod tests {
    ...
    fn hostile_manifest_rows_are_present_and_backed_by_real_proof_files() {
        let manifest = worth_query_platform_entry_hostile_manifest();

        assert_eq!(manifest.rows().len(), 7);
        assert!(!manifest.hostile_digest().is_empty());
        assert!(WorthQueryPlatformEntryHostileAudit::current()
            .missing_divergence_rows()
            .is_empty());
        assert!(WorthQueryPlatformEntryHostileAudit::current()
            .unknown_surfaces()

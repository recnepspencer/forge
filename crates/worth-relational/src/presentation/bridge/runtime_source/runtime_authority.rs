...
#[derive(Clone)]
pub(crate) enum RelationalBridgeRuntimeAuthority {
    Immutable(Arc<RelationalRuntime>),
    Shared(Arc<Mutex<RelationalRuntime>>),
}

impl RelationalBridgeRuntimeAuthority {
...
impl std::fmt::Debug for RelationalBridgeRuntimeAuthority {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("RelationalBridgeRuntimeAuthority")

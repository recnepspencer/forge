...
fn retained_entity_projection_denies_mixed_branch_and_snapshot_axes() {
    ...
    let committed = create_entity_outcome(&mut runtime, "retained-projection");
...
fn retained_entity_projection_enforces_partition_authority_before_projection() {
    let mut runtime = runtime_with_test_schema();
    let admitted = create_entity_in_partition(&mut runtime, "partition-main", PartitionId::main());
    let foreign =
        create_entity_in_partition(&mut runtime, "partition-foreign", PartitionId::new(7));
    let main_identity = runtime.main_branch_identity();

...
fn persistent_index_carries_explicit_set_and_removal_allocations() {
    ...
    let initial = RelationalPersistentRegionSet::initial(1, BTreeMap::new(), &mut issuer)
        .expect("empty owner index builds");
    let removed = [PartitionId(7), PartitionId(11)]
        .into_iter()
        .collect::<BTreeSet<_>>();
    let replaced =
        RelationalPersistentRegionSet::replace(2, &initial, BTreeMap::new(), removed, &mut issuer)

...
use crate::schema::data::{
    AcyclicityContractDeclaration, AllowedCycleClass, CardinalityContractDeclaration,
    ConnectivityMinimumContractDeclaration, ConnectivityMinimumEnforcement, DirectedTraversalKind,
    EndpointDeletionIntegrityDeclaration, EndpointDeletionIntegrityMode,
    EndpointKindContractDeclaration, MinimumCardinalityEnforcement, PairMinimumSemantics,
    PartitionIsolationContractDeclaration, PartitionIsolationMode, RelationIntegrityPlanRevision,
    SymmetryContractDeclaration, SymmetryMode, UniquenessContractDeclaration, UniquenessScope,
};

pub(super) fn derive_legacy_relation_integrity_plan_revision(
    ...
    let mut digest = LegacyRevisionDigest::new();
    ...
    digest.mix_endpoint_deletion_contracts(endpoint_deletion_integrity_contracts);
...
struct LegacyRevisionDigest(u128);

impl LegacyRevisionDigest {
    ...
    fn mix_cardinality_contracts(&mut self, contracts: &[CardinalityContractDeclaration]) {
        for declaration in contracts {
            ...
            self.mix_string(declaration.contract_id.as_str());
            self.mix_optional(declaration.source_max, u64::MAX);
            self.mix_optional(declaration.target_max, u64::MAX);
            self.mix_optional(declaration.pair_max, u64::MAX);
            self.mix_optional(declaration.source_min, 0);
            self.mix_optional(declaration.target_min, 0);
            self.mix_optional(declaration.pair_min, 0);
    ...
    fn mix_endpoint_deletion_contracts(
        &mut self,
    ...
    fn mix_optional(&mut self, value: Option<u64>, absent: u64) {
        self.mix_bytes(&value.unwrap_or(absent).to_le_bytes());
    }

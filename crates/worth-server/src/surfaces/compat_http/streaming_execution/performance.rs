...
pub(crate) struct WorthServerStreamingMetricSnapshot {
...
pub struct WorthServerStreamingPerformanceReceipt {
    inner: FoundationalCounterBackedPerformanceReceipt<FoundationalAuthoritativePerformanceClaim>,
}

impl WorthServerStreamingPerformanceReceipt {
    pub(crate) fn build(
        metrics: WorthServerStreamingMetricSnapshot,
    ) -> Result<Self, WorthServerStreamingPerformanceReceiptError> {
        let claim = performance()
            ...
            .finish()
            .map_err(WorthServerStreamingPerformanceReceiptError::PerformanceClaim)?;
        let bundle = performance_bundle(claim)
            ...
            .finish()
            .map_err(WorthServerStreamingPerformanceReceiptError::PerformanceBundle)?;
        let receipt = counter_backed_performance_receipt(bundle)
            ...
            .finish()
            .map_err(WorthServerStreamingPerformanceReceiptError::CounterReceipt)?;
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum WorthServerStreamingPerformanceReceiptError {

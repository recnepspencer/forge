...
use super::{
    lifecycle::WorthServerStoredBinaryIngress,
    performance::{
        WorthServerIngressMetricSnapshot, WorthServerIngressPerformanceReceipt, CLEANUP_OPERATIONS,
...
pub enum WorthServerUploadCleanupReason {
    Interrupted,
    Expired,
    OwnershipMismatch,
    Abandoned,
...
pub struct WorthServerUploadCleanupReceipt {
...
impl WorthServerUploadCleanupReceipt {
    pub(crate) fn new(
        stored_ingress: &WorthServerStoredBinaryIngress,
        reason: WorthServerUploadCleanupReason,
    ) -> Self {
        let staged_bytes = stored_ingress.staged_authoritative_bytes();
        let performance_receipt = WorthServerIngressPerformanceReceipt::build(
            WorthServerIngressMetricSnapshot {
        ...
        let canonical_digest = format!(
            "worth-server-upload-cleanup-receipt-v1|reason={reason:?}|session={}|tenant={}|workspace={}|branch={}|truth_drift_free=true|cleanup_ops={}|cleanup_bytes={}",
    ...
    pub fn reason(&self) -> WorthServerUploadCleanupReason {
        self.reason
    }
    ...
    let source_basis =
        FoundationalBoundaryEvidenceSourceBasis::boundary_artifact(BoundaryArtifactLocator::new(
            BoundaryArtifactId::new(boundary_artifact_id(&[
                "worth-server.upload-cleanup".to_string(),

...
use crate::WorthServerCompatibilityCachePolicy;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthServerCacheabilityPolicy {
    surface_kind: String,
...
impl WorthServerCacheabilityPolicy {
    pub(crate) fn from_compatibility_policy(
        surface_kind: &str,
        diagnostics_profile: DiagnosticRichnessProfile,
        metadata_identity: &str,
        branch_digest: &str,
        compatibility_policy: &WorthServerCompatibilityCachePolicy,
        remask_safe_for_shared_caches: bool,
    ) -> Self {
        let provenance = build_provenance(surface_kind, metadata_identity, branch_digest);
        let canonical_digest = format!(
            "worth-server-cacheability-policy-v1|surface={surface_kind}|cache_control={}|vary={}|public={}|intermediary_safe=false|branch_scoped=true|auth_scoped=true|remask_safe={remask_safe_for_shared_caches}|diagnostics={diagnostics_profile:?}|provenance_locality={:?}",
    ...
    pub(crate) fn scoped_private(
        ...
        let provenance = build_provenance(surface_kind, metadata_identity, branch_digest);
        let canonical_digest = format!(
            "worth-server-cacheability-policy-v1|surface={surface_kind}|cache_control={cache_control}|vary={}|public=false|intermediary_safe=false|branch_scoped=true|auth_scoped=true|remask_safe=false|diagnostics={diagnostics_profile:?}|provenance_locality={:?}",
            vary.join(","),
            provenance.locality(),
        );
        Self {
    ...
    let source_basis =
        FoundationalBoundaryEvidenceSourceBasis::boundary_artifact(BoundaryArtifactLocator::new(
            BoundaryArtifactId::new(boundary_artifact_id(&[
                "worth-server.file-identity.cacheability".to_string(),

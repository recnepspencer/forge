...
pub(crate) struct WorthServerBinaryEgressMetricSnapshot {
...
pub struct WorthServerBinaryEgressPerformanceReceipt {
    inner: FoundationalCounterBackedPerformanceReceipt<FoundationalAuthoritativePerformanceClaim>,
}

impl WorthServerBinaryEgressPerformanceReceipt {
    pub(crate) fn build(
        metrics: WorthServerBinaryEgressMetricSnapshot,
    ) -> Result<Self, WorthServerBinaryEgressPerformanceReceiptError> {
        let claim = performance()
            ...
            .finish()
            .map_err(WorthServerBinaryEgressPerformanceReceiptError::PerformanceClaim)?;
        let bundle = performance_bundle(claim)
            ...
            .finish()
            .map_err(WorthServerBinaryEgressPerformanceReceiptError::PerformanceBundle)?;
        let receipt = counter_backed_performance_receipt(bundle)
            ...
            .finish()
            .map_err(WorthServerBinaryEgressPerformanceReceiptError::CounterReceipt)?;
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum WorthServerBinaryEgressPerformanceReceiptError {

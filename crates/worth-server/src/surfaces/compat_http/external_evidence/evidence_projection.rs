...
use crate::{
    WorthServerCompatibilityFileEnvelope, WorthServerOperatorEvidenceFacade,
...
pub(crate) fn build_upload_certification_bundle(
    ...
    file_envelope: &WorthServerCompatibilityFileEnvelope,
    response: &WorthServerResponseEnvelope,
    receipt: &crate::WorthServerIngressPerformanceReceipt,
    ...
    build_binary_bundle(
        ...
        file_envelope,
        response,
        WorthServerBinaryCounterSet::new(receipt.receipt().clone()),
    )
}
...
pub(crate) fn build_download_certification_bundle(
    ...
    file_envelope: &WorthServerCompatibilityFileEnvelope,
    response: &WorthServerResponseEnvelope,
    receipt: &crate::WorthServerBinaryEgressPerformanceReceipt,
) -> WorthServerBinaryCertificationBundle {
    build_binary_bundle(
...
pub(crate) fn build_streaming_export_certification_bundle(
    ...
    file_envelope: &WorthServerCompatibilityFileEnvelope,
    response: &WorthServerResponseEnvelope,
    receipt: &crate::WorthServerStreamingPerformanceReceipt,
) -> WorthServerBinaryCertificationBundle {
    build_binary_bundle(
...
fn build_compatibility_bundle(
    ...
    let evidence = WorthServerExternalEvidenceRecord::project(
        surface_label,
        response.clone(),
        operator_evidence,
    )
    .expect("compatibility operator evidence should materialize");
    let external_counters = WorthServerExternalCounterSet::new(build_external_receipt(
        surface,
        evidence.operator_record(),
    ));
    WorthServerCompatibilityCertificationBundle::new(
...
fn build_binary_bundle(
    ...
    WorthServerBinaryCertificationBundle::new(
        support_posture.canonical_label(),
        policy_truth_digest(file_envelope),
        provenance_truth_digest(file_envelope),
        response,
...
fn build_external_receipt(
    surface: CompatHttpEvidenceSurface,
    operator_record: &crate::WorthServerOperatorEvidenceRecord,
...
fn counter_rows(
    surface: CompatHttpEvidenceSurface,
    classification: &crate::WorthServerOperatorEvidenceClass,
) -> [(&'static str, u64); 11] {
    [
        (
            READ_SUCCESSES,
        ...
        (
            ...
            u64::from(matches!(
                classification,
                crate::WorthServerOperatorEvidenceClass::MiddlewareDenied(_)
            )),
        ),
        (
            ...
            u64::from(matches!(
                classification,
                crate::WorthServerOperatorEvidenceClass::QueryHandoffDenied(_)
            )),
        ),
        (
            DENIALS,
...
fn policy_truth_digest(file_envelope: &WorthServerCompatibilityFileEnvelope) -> String {
    let policy = file_envelope.policy_decision();
    format!(
        "worth-server-file-policy-truth-v1|identity={}|tenant={}|workspace={}|branch={}|operation={}|lane={}|support={}|authorization={}",
...
fn provenance_truth_digest(file_envelope: &WorthServerCompatibilityFileEnvelope) -> String {
    let provenance = file_envelope.transfer_provenance();
    format!(
        "worth-server-file-transfer-truth-v1|identity={}|tenant={}|workspace={}|branch={}|operation={}|disposition={:?}|content_type={}|bytes={}|range_honored={}",

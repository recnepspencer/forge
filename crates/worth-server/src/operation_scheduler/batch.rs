...
use super::{
    ...
    WorthServerSchedulerCancellationDirective, WorthServerSchedulerCertificationSabotage,
    WorthServerSchedulerConflictDenial, WorthServerSchedulerConflictDenialFacts,
    WorthServerSchedulerLane,
};

#[derive(Debug)]
pub struct WorthServerScheduledOperationBatch {
    responses: WorthServerResponseFacade,
    slots: Vec<WorthServerOperationExecutionSlot>,
}

impl WorthServerScheduledOperationBatch {
    pub(crate) fn new(
        responses: WorthServerResponseFacade,
        ...
        let slots = plans
            ...
            .enumerate()
            .map(|(ordinal, plan)| {
                WorthServerOperationExecutionSlot::from_lowered_plan(ordinal, plan)
            })
    ...
    pub fn slots(&self) -> &[WorthServerOperationExecutionSlot] {
        &self.slots
    }

    pub fn execute(self) -> WorthServerExecutedOperationBatch {
    ...
    pub fn execute_with_cancellation(
        ...
        let ordered = self
            .slots
            .iter()
            .any(|slot| slot.scheduler_lane_kind() != &WorthServerSchedulerLane::SharedRead);
        if ordered {
...
fn validate_batch_execution_mode(
    slots: &[WorthServerOperationExecutionSlot],
    ...
    if contains_shared_read && contains_ordered {
        return Err(
            WorthServerSchedulerConflictDenial::unsupported_ordered_operation(
...
fn validate_batch_conflicts(
    ...
    for (left_index, left_slot) in slots.iter().enumerate() {
        if left_slot.scheduler_lane_kind() == &WorthServerSchedulerLane::SharedRead {
            continue;
        }
        ...
        for right_slot in &slots[left_index + 1..] {
            ...
            if left_basis_digest == right_basis_digest {
                return Err(WorthServerSchedulerConflictDenial::conflicting_mutation_plan(
                    format!(
                ...
                .with_conflict_facts(
                    WorthServerSchedulerConflictDenialFacts::conflicting_mutation_plan(
                        left_slot.scheduler_lane(),

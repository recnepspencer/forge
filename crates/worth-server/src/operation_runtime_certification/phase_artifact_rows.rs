use crate::{
    operation_runtime_certification::WorthServerProductEditorReadinessCertification,
    WorthServerOperationFamily, WorthServerRouteInventory,
};
...
pub(super) fn requirement_row(
    ...
    WorthServerProductOperationRuntimeRequirementRow::new(
        artifact_name,
        if ready {
            WorthServerProductOperationRuntimeRequirementStatus::Ready
        } else {
            WorthServerProductOperationRuntimeRequirementStatus::Blocked
        },
        digest,
        detail,
    )

    ...
    pub(crate) fn from_state_snapshot(
        ...
        match snapshot.remask_posture() {
            ...
            Some(remask) if remask.disposition_kind().as_str() == "remasked" => {
                Self::Remasked(WorthServerDirectRemaskArtifact::new(
                    remask.reason_kind().as_str(),
                    remask.support_for_reporting(),
                    remask.basis_for_reporting(),
                    remask.policy_for_reporting(),
                    remask.tenant_truth_for_reporting(),
                    remask.tenant_schema_for_reporting(),
                    remask.relationship_proof_for_reporting(),
                    remask.schema_context_for_reporting(),
                    remask.remask_for_reporting(),
                ))
            }

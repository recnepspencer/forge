...
impl WorthServerDirectLeaseDeclaration {
    pub(crate) fn from_admitted_declaration(
        ...
        let canonical_digest = format!(
            "worth-server-direct-lease-v1|principal:{principal_id}|tenant:{tenant_id}|workspace:{workspace_id}|branch:{branch_label}|bound:{workspace_name}|basis:{resume_basis_digest}|declaration:{declaration_digest}|binding:{declaration_binding_label}|support:{support_digest}"
        );
        Ok(Self {
            principal_id,
            tenant_id,

...
use super::transport_denial::{WorthServerTransportDenial, WorthServerTransportDenialCode};

const MAX_JSON_BODY_BYTES: usize = 1024 * 1024;

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum WorthServerRouteBranchTarget {
    Main,
...
impl WorthServerRouteTransportRequest {
    pub fn new(
    ...
    pub(crate) fn branch_target(&self) -> &WorthServerRouteBranchTarget {
        &self.branch_target
    }
...
pub(crate) fn decode_json_body(
...
) -> Result<Option<serde_json::Value>, WorthServerTransportDenial> {
    ...
    if request.body_bytes.len() > MAX_JSON_BODY_BYTES {
        return Err(WorthServerTransportDenial::new(
            WorthServerTransportDenialCode::OversizedBody,
            format!(
    ...
    let json: serde_json::Value = serde_json::from_slice(&request.body_bytes).map_err(|error| {
        WorthServerTransportDenial::new(
            WorthServerTransportDenialCode::MalformedJson,
...
fn query_payload_json(request: &WorthServerRouteTransportRequest) -> Option<Value> {
...
pub(crate) fn query_value<'a>(
    request: &'a WorthServerRouteTransportRequest,
    name: &str,
) -> Option<&'a str> {
    request
        .query_pairs()

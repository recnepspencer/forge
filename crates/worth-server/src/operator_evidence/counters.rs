...
pub(crate) fn build_counter_receipt(
...
) -> Result<WorthServerOperatorCounterReceipt, WorthServerOperatorEvidenceCounterError> {
    let claim = performance()
        ...
        .finish()
        .map_err(WorthServerOperatorEvidenceCounterError::PerformanceClaim)?;
    ...
    let bundle = performance_bundle(claim)
        ...
        .finish()
        .map_err(WorthServerOperatorEvidenceCounterError::PerformanceBundle)?;
    ...
    let receipt = counter_backed_performance_receipt(bundle)
        ...
        .finish()
        .map_err(WorthServerOperatorEvidenceCounterError::CounterReceipt)?;

    Ok(WorthServerOperatorCounterReceipt::new(receipt))
}
...
fn expected_count(class: &WorthServerOperatorEvidenceClass, counter_name: &str) -> u64 {
    match counter_name {
        REQUEST_CONTEXT_DENIAL_COUNT
            if matches!(
                class,
        ...
        MIDDLEWARE_DENIAL_COUNT
            if matches!(class, WorthServerOperatorEvidenceClass::MiddlewareDenied(_)) =>
        {
            1
        }
        QUERY_HANDOFF_DENIAL_COUNT
        ...
        QUERY_READ_SUCCESS_COUNT
            if matches!(class, WorthServerOperatorEvidenceClass::QueryReadSucceeded) =>
        {
            1
        }
        QUERY_MUTATION_SUCCESS_COUNT
...
pub enum WorthServerOperatorEvidenceCounterError {

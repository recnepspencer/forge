use crate::{
    WorthServerDenialCode, WorthServerQueryHandoffDenialCode, WorthServerRequestContextDenialCode,
    WorthServerResponseEnvelope, WorthServerSuccessKind,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum WorthServerOperatorEvidenceClass {
    RequestContextDenied(WorthServerRequestContextDenialCode),
    MiddlewareDenied(WorthServerDenialCode),
    QueryHandoffDenied(WorthServerQueryHandoffDenialCode),
    QueryReadSucceeded,
    QueryMutationSucceeded,
    DownstreamDeliverySucceeded,
}

impl WorthServerOperatorEvidenceClass {
    pub(crate) fn from_response_envelope(response: &WorthServerResponseEnvelope) -> Self {
        if let Some(success) = response.success() {
            return match success.payload().kind() {
                WorthServerSuccessKind::QueryRead
                | WorthServerSuccessKind::DirectRead
                | WorthServerSuccessKind::DirectState
                | WorthServerSuccessKind::DirectInspection
                | WorthServerSuccessKind::DirectProjection => Self::QueryReadSucceeded,
                WorthServerSuccessKind::DirectMutation | WorthServerSuccessKind::QueryMutation => {
                    Self::QueryMutationSucceeded
                }
                WorthServerSuccessKind::DownstreamDelivery => Self::DownstreamDeliverySucceeded,
            };
        }

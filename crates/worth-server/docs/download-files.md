...
- `WorthServerCompatibilityFacade::download(...)`
- `WorthServerCompatibilityFacade::plan_binary_resume(...)`
- `WorthServerCompatibilityFacade::plan_binary_integrity(...)`
...
2. Build a `WorthServerBinaryDownloadRequest`.
3. Call `download(...)`.
4. If needed, call `plan_binary_resume(session)` and build a follow-up request
   with `WorthServerBinaryResumeRequest`.

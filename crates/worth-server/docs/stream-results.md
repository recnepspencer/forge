...
- `WorthServerCompatibilityFacade::stream(...)`
...
2. Call `stream(...)` with a `WorthServerStreamSelection`.
...
use worth_server::{WorthServerStreamSelection, WorthServerStreamingResponse};

let response = match server.compat_http().stream(
    prepared_stream_execution_input,
    WorthServerStreamSelection::incremental().with_chunk_bytes(16 * 1024),
) {
...
match response {
    WorthServerStreamingResponse::Stream(mut stream) => {
    ...
    WorthServerStreamingResponse::Buffered(export) => {
        consume(export.payload_bytes());
    }
    WorthServerStreamingResponse::BackgroundExport(export) => {

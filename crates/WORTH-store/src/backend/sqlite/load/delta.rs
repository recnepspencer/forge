    ...
    let rows = statement
        .query_map([], |row| {
            Ok(records::BranchSharedBaseRecord {
                branch_id: worth_relational::facade::history::BranchId(row.get::<_, String>(0)?),
                source_branch_id: worth_relational::facade::history::BranchId(
                    row.get::<_, String>(1)?,
                ),
                source_frontier_commit_id: row
                    .get::<_, Option<i64>>(2)?
                    .map(|value| worth_relational::facade::history::CommitId(value as u64)),
    ...
    let rows = statement
        .query_map([], |row| {
            ...
            Ok(records::BranchDeltaLayerRecord {
                ...
                branch_id: worth_relational::facade::history::BranchId(row.get::<_, String>(1)?),
                base_frontier_commit_id: row
                    .get::<_, Option<i64>>(2)?
                    .map(|value| worth_relational::facade::history::CommitId(value as u64)),
                target_frontier_commit_id: worth_relational::facade::history::CommitId(
                    row.get::<_, i64>(3)? as u64,
                ),
                commit_ids: commit_ids
                    .into_iter()
                    .map(worth_relational::facade::history::CommitId)
                    .collect(),

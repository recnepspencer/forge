use crate::{WORTHStore, WORTHStoreBuilder};
...
fn build_store(builder: WORTHStoreBuilder) -> (WORTHStore, u64) {
...
fn execute_tiering_batch(store: &mut WORTHStore, snapshot_id: u64) {
...
fn interleaved_tiering_lane(builder: WORTHStoreBuilder) -> (WORTHStore, u64) {
...
fn recalled_tiering_lane(builder: WORTHStoreBuilder) -> (WORTHStore, u64) {
    let (mut store, snapshot_id) = build_store(builder);
    let derived = store
        .plan_derived_tier_move(
            conservative_policy(),
...
fn foreground_read_interleaving_lane(builder: WORTHStoreBuilder) -> WORTHStore {
...
fn continuation_interleaving_lane(builder: WORTHStoreBuilder) -> WORTHStore {
    let mut runtime = runtime_with_demo_schema();
...
fn interleaving_counter_lane(builder: WORTHStoreBuilder) -> WORTHStore {
...
fn milestone_13_suite() -> CertificationSuite<String, String> {
    let (control_store, control_snapshot_id) = build_store(WORTHStoreBuilder::new().in_memory());
    ...
    let reopened_local_store = WORTHStoreBuilder::new()

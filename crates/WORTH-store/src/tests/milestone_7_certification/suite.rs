...
fn milestone_7_suite() -> CertificationSuite<String, String> {
    let clean_bundle = {
        ...
        let control = WORTHStore::restore_from_authoritative_export(
    ...
    let support_gap_local = {
        let path = unique_test_store_path("worth-store-m7-support-gap-local");
        let mut runtime = runtime_with_demo_schema();
        create_entity(&mut runtime, "alpha");
        let envelope = latest_envelope(&runtime);
    ...
    let support_gap_sqlite = {
        let path = unique_test_sqlite_path("worth-store-m7-support-gap-sqlite");
        let mut runtime = runtime_with_demo_schema();
        create_entity(&mut runtime, "alpha");
        let envelope = latest_envelope(&runtime);
        let mut store = WORTHStoreBuilder::new()
    ...
    let typed_cursor_gap = {
        let path = unique_test_store_path("worth-store-m7-typed-cursor-gap");
        let mut runtime = runtime_with_demo_schema();
        create_entity(&mut runtime, "alpha");
        let envelope = latest_envelope(&runtime);
        let mut store = WORTHStoreBuilder::new()

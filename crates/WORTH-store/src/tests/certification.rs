...
fn milestone_1_suite() -> CertificationSuite<String, String> {
    ...
    let mut in_memory_store = WORTHStoreBuilder::new().in_memory().build().unwrap();
    in_memory_store
    ...
    let file_bundle = {
        let mut file_store = WORTHStoreBuilder::new()
            .local_file(unique_test_store_path("worth-store-certification"))
            .build()
            .unwrap();
    ...
    let sqlite_bundle = {
        let mut sqlite_store = WORTHStoreBuilder::new()
            .sqlite_file(unique_test_sqlite_path("worth-store-certification-sqlite"))
            .build()
            .unwrap();
    ...
    let rebuilt = WORTHStore::restore_from_authoritative_export(
        in_memory_store

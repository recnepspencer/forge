...
fn bulk_frozen_manifest_and_checkpoint_fetch_survive_sqlite_reopen() {
    let path = unique_test_sqlite_path("worth-store-bulk-fetch");
...
fn bulk_witness_with_missing_canonical_commit_fails_reopen_closed() {
    let path = unique_test_store_path("worth-store-bulk-witness-missing-commit");
    let mut store = WORTHStoreBuilder::new()
...
fn bulk_checkpoint_gap_fails_local_file_reopen_closed() {
    let path = unique_test_store_path("worth-store-bulk-checkpoint-gap");
    let mut store = WORTHStoreBuilder::new()
        .local_file(path.clone())
        .build()
        .unwrap();
...
fn bulk_checkpoint_completed_chunk_regression_fails_local_file_reopen_closed() {
    let path = unique_test_store_path("worth-store-bulk-checkpoint-regression");
    let mut store = WORTHStoreBuilder::new()
        .local_file(path.clone())
        .build()
        .unwrap();
...
fn bulk_checkpoint_index_reference_missing_checkpoint_fails_sqlite_reopen_closed() {
    let path = unique_test_sqlite_path("worth-store-bulk-checkpoint-index-gap");
    let mut store = WORTHStoreBuilder::new()
        .sqlite_file(path.clone())
        .build()
        .unwrap();
...
fn bulk_checkpoint_completed_chunk_regression_fails_sqlite_reopen_closed() {
    let path = unique_test_sqlite_path("worth-store-bulk-checkpoint-regression-sqlite");
    let mut store = WORTHStoreBuilder::new()
        .sqlite_file(path.clone())
        .build()
        .unwrap();

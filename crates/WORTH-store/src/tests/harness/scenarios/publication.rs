use crate::{DurableMutationRequest, WORTHStoreBuilder, PublicationWriteOutcome};
...
pub fn durable_publication_reports() -> DurablePublicationScenarioResult {
    let mut local = WORTHStoreBuilder::new()
        .local_file(unique_test_store_path("worth-store-publication-local"))
        .durable_mode(runtime_with_demo_schema())
        .build()
        .unwrap();
    let mut sqlite = WORTHStoreBuilder::new()
        .sqlite_file(unique_test_sqlite_path("worth-store-publication-sqlite"))
        .durable_mode(runtime_with_demo_schema())
        .build()
        .unwrap();

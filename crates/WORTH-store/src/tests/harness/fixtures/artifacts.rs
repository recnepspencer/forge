...
pub fn append_two_mainline_commits(store: &mut WORTHStore) -> (CommitId, CommitId) {
...
pub fn append_three_mainline_commits(store: &mut WORTHStore) -> (CommitId, CommitId, CommitId) {
    let mut runtime = runtime_with_demo_schema();
    let entity_id = create_entity(&mut runtime, "alpha");
    let first = latest_envelope(&runtime);
    let first_id = first.commit.commit_id;

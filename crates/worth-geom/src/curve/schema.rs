...
impl CurveGeom {
    ...
    /// stays in the geometry layer via `worth_math::linalg`.
    pub fn line_from_endpoints(origin: [f64; 3], dest: [f64; 3], min_length: f64) -> Option<Self> {
        let delta = worth_math::linalg::sub(dest, origin);
        let direction = worth_math::linalg::normalize_checked(delta)?;
        let len = worth_math::linalg::norm(delta);

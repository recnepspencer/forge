...
//! DEPENDENCIES: `Plane`, `worth_math::linalg`
...
pub fn dodecahedron(center: [f64; 3], scale: f64) -> Result<Vec<Plane>, MathError> {
    ...
    raw_normals
        .iter()
        .map(|n| {
            let norm = worth_math::linalg::normalize_checked(*n).ok_or(MathError::InvalidInput(
...
pub fn pyramid(
    ...
    for i in 0..sides {
        ...
        let normal = worth_math::linalg::normalize_checked(raw_normal).ok_or(
...
pub fn wedge(center: [f64; 3], dimensions: [f64; 3]) -> Result<Vec<Plane>, MathError> {
    ...
    let slope_normal = worth_math::linalg::normalize_checked([0.0, hz, wx]).ok_or(

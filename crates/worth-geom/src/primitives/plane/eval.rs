...
//!
//! DEPENDENCIES: `worth-math` (Rational, predicates, sign, linalg)

use worth_math::arithmetic::Rational;
use worth_math::linalg::{cross, norm_sq};
...
pub fn intersect_three_planes(
    ...
    let det = worth_math::linalg::det3_rows(n0, n1, n2);
    ...
    let x =
        worth_math::linalg::det3_rows([d0, n0[1], n0[2]], [d1, n1[1], n1[2]], [d2, n2[1], n2[2]])
            * inv_det;
    let y =
        worth_math::linalg::det3_rows([n0[0], d0, n0[2]], [n1[0], d1, n1[2]], [n2[0], d2, n2[2]])
            * inv_det;
    let z =
        worth_math::linalg::det3_rows([n0[0], n0[1], d0], [n1[0], n1[1], d1], [n2[0], n2[1], d2])
            * inv_det;
...
pub fn exact_eq(a: &Plane, b: &Plane) -> bool {
    ...
    let positive_ratio = matches!(reference, Some(worth_math::sign::TriSign::Pos));
    if !positive_ratio {
        return false;
    }
...
pub fn normals_aligned_exact(a: &Plane, b: &Plane) -> bool {
    ...
    dot.sign() == worth_math::sign::TriSign::Pos
}

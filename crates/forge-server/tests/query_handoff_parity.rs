use forge_server::{
    ForgeServerMiddlewareConfig, ForgeServerQueryHandoffInput, ForgeServerQueryHandoffOperation,
...
fn prepare_lowers_equivalent_cross_surface_reads_to_the_same_canonical_handoff_artifact() {
    ...
    assert_eq!(
        forge_native.support_posture(),
        compat_http.support_posture()
    );
    assert_eq!(forge_native.operation(), compat_http.operation());
    assert_eq!(
        forge_native.workspace().name(),

use forge_server::{
    ...
    ForgeServerProductOperationBaseDigest, ForgeServerProductOperationOutcome,
    ForgeServerProductOperationSurfaceDenialCode, ForgeServerProductSessionCreationRequest,
};
...
fn failed_product_operation_is_recorded_for_replay_after_scheduler_admission() {
    ...
    let idempotency_key =
        ForgeServerProductIdempotencyKey::new("failed-apply-attempt-1").expect("valid key");

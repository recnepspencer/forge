...
use forge_server::{
    ...
    ForgeServerMultipartUpload, ForgeServerQueryHandoffDenialCode, ForgeServerUploadExpectation,
    ForgeServerUploadManifest, ForgeServerUploadPart,
};
use serde_json::json;
...
fn compat_http_upload_rejects_malformed_part_graphs_and_unsupported_content_shapes() {
    ...
    let missing_part = compat_upload_denied(
        server.compat_http().upload(compat_upload_execution_input(
            ...
            "files.avatar.upload",
            "boundary-a",
            ForgeServerMultipartUpload::new(manifest_for("task-1")).with_part(
    ...
    let duplicate_part_identity = compat_upload_denied(
        server.compat_http().upload(compat_upload_execution_input(
            ...
            "boundary-duplicate",
            ForgeServerMultipartUpload::new(
                ForgeServerUploadManifest::new(single_insert_body("task-1"))
                    .with_file_part("avatar")
                    .with_file_part("avatar"),
            )
            .with_part(
    ...
    let oversized_part = compat_upload_denied(
        server.compat_http().upload(compat_upload_execution_input(
            ...
            "files.avatar.upload",
            "boundary-b",
            ForgeServerMultipartUpload::new(manifest_for("task-1"))
                .with_part(
                    ForgeServerUploadPart::file("avatar")
                        .with_content_type("image/png")
                        .with_declared_length(128),
    ...
    let wrong_content_type = compat_upload_denied(
        server.compat_http().upload(
            forge_server::ForgeServerCompatibilityUploadExecutionInput::new(
                prepared_request(
                    &server,
    ...
    let denial = prepared_upload_denied(
        server.compat_http().prepare_upload(
            forge_server::ForgeServerCompatibilityUploadExecutionInput::new(
                ...
                "files.avatar.upload",
                ForgeServerMultipartUpload::new(ForgeServerUploadManifest::new(json!(null)))
                    .with_expectation(ForgeServerUploadExpectation::continue_required())
                    .with_part(
                        ForgeServerUploadPart::file("avatar")
                            .with_content_type("image/png")
                            .with_declared_length(128),
...
fn compat_http_upload_expect_continue_denies_unauthorized_requests_before_any_body_progression() {
    ...
    assert_eq!(
        denial.code(),
        forge_server::ForgeServerCompatibilityDenialCode::MiddlewareDenied
    );
    assert!(denial
        .detail()

...
use forge_server::{
    request_context::DiagnosticRichnessProfile, ForgeServerDenialCode,
    ForgeServerForgeNativeSessionDenialCode, ForgeServerPipelineInput, ForgeServerPipelineIntent,
    ForgeServerPreparedQueryHandoffKind, ForgeServerRequestContextDenialCode,
};

use crate::forge_native_runtime::{
    build_server, build_server_with_disabled_forge_native, build_server_with_preview_denial,
    denied_prepared_session, denied_session, expect_preview_access_denial,
    forge_native_session_input_builder, resolve_forge_native_request_context,
    server_with_request_context_default,
};

#[test]
fn forge_native_prepare_session_matches_lower_lane_request_context_and_admission() {
    let server = build_server(true);

    let prepared = match server.forge_native().prepare_session(
    ...
    let lower_lane_context = resolve_forge_native_request_context(&server);
    let lower_lane_admission = match server.middleware().admit(ForgeServerPipelineInput::new(
    ...
    assert_eq!(
        ...
        "forge_native.session"
    );
}

#[test]
fn forge_native_common_session_lane_wraps_prepared_session_without_semantic_drift() {
    let server = build_server(true);
...
#[test]
fn forge_native_prepare_session_denies_when_surface_family_is_absent() {
    let server = build_server(false);

    let denial = denied_prepared_session(
        server.forge_native().prepare_session(
            forge_native_session_input_builder()
                .build()
                .expect("forge-native session input should validate"),
        ),
...
#[test]
fn forge_native_prepare_session_denies_when_surface_family_is_disabled() {
    let server = build_server_with_disabled_forge_native();
...
#[test]
fn forge_native_prepare_session_preserves_request_context_denial_artifact() {
    let server = build_server(true);
...
#[test]
fn forge_native_prepare_session_preserves_middleware_denial_artifact() {
...
#[test]
fn forge_native_prepare_session_honors_requested_diagnostics_profile_for_surface_absence() {
    let server = build_server(false);
...
#[test]
fn forge_native_session_wrapper_preserves_denial_topology_without_reclassification() {
    ...
    let middleware_denial = denied_session(
        build_server_with_preview_denial().forge_native().session(
            forge_native_session_input_builder()
                .with_preview_id("preview-7")
                .build()
                .expect("forge-native session input should validate"),

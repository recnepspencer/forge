use forge_query::facade::{ForgeQueryAspectMutationBuilder, ForgeQueryWriteCommand};
use forge_server::{
    ForgeServer, ForgeServerLoweredOperationPlan, ForgeServerOperationFamily,
    ForgeServerOperationPlannerInput, ForgeServerOperationRequestInput, ForgeServerQueryOperation,
    ForgeServerRequestContextInput, ForgeServerSchedulerCancellationDirective,
    ForgeServerSchedulerCancellationPosture, ForgeServerSchedulerConflictDenialCode,
...
fn conflicting_mutation_plans_localize_scheduler_denial() {
    ...
    assert_eq!(
        denial.code(),
        ForgeServerSchedulerConflictDenialCode::ConflictingMutationPlan
    );
    assert!(denial
        .detail()
...
fn execute_interleaved_batch() -> forge_server::ForgeServerExecutedOperationBatch {
    let server = scheduler_server();
    server
        .operation_scheduler()
        .schedule_batch([
...
fn scheduler_server() -> ForgeServer {
...
fn lower_submission_plan(
    ...
    lower_mutation_plan(
        ...
        None,
        basis_digest,
        ForgeServerQueryOperation::single_mutation("tasks.insert", insert_task(task_identity)),
    )
}
...
fn lower_product_mutation_plan(
    ...
    lower_mutation_plan(
        ...
        ForgeServerQueryOperation::single_mutation("editor.apply", insert_task(task_identity)),
    )
}
...
fn lower_session_coordination_plan(
    ...
    let admission = admit_forge_native_session(
        server,
        resolve_request_context(server, forge_native_request_input()),
        operation_name,
    );
    ...
    server
        ...
        .lower(ForgeServerOperationPlannerInput::new(
            operation_admission,
            forge_server::ForgeServerQueryHandoffOperation::direct_mutation_execution(
...
fn lower_mutation_plan(
    ...
    let operation_request = server
        .operation_requests()
        .admit_from_forge_native_admission(&admission, builder.build())
    ...
    server
        ...
        .lower(ForgeServerOperationPlannerInput::new(
            operation_admission,
            forge_server::ForgeServerQueryHandoffOperation::query_mutation_execution(operation),
        ))
        .expect("mutation plan should lower")
}

fn forge_native_request_input() -> ForgeServerRequestContextInput {
...
fn admit_mutation(
    server: &ForgeServer,
    resolved: forge_server::ForgeServerResolvedRequestContext,
    operation_name: &str,
    ...
    match server
        ...
        .admit(forge_server::ForgeServerPipelineInput::new(
            resolved,
            forge_server::ForgeServerPipelineIntent::query_mutation(operation_name),
        )) {
...
fn admit_forge_native_session(
    ...
    match server
        ...
        .admit(forge_server::ForgeServerPipelineInput::new(
            resolved,
            forge_server::ForgeServerPipelineIntent::forge_native_session(operation_name),
        )) {
        TransitionOutcome::Success(admission) => admission,
        other => panic!("expected admitted forge-native session pipeline result, got {other:?}"),
    }
}

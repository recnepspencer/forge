use forge_server::{
    ForgeServerProductIdempotencyKey, ForgeServerProductOperationInput,
...
fn product_editor_fixture_has_real_pressure_shape() {
    ...
    let render_digest = match render.outcome() {
        ForgeServerProductOperationOutcome::Success(success) => success.result_digest(),
    ...
    let replay_key =
        ForgeServerProductIdempotencyKey::new("phase-thirteen-pressure-replay").expect("key");

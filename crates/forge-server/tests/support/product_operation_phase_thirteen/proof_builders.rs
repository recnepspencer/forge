...
use forge_server::{
    ForgeServerEditorLikeOperationFixture, ForgeServerProductIdempotentReplayCertificationProof,
...
pub async fn complete_editor_like_fixture() -> ForgeServerEditorLikeOperationFixture {
    ...
    let shared_read_batch = session
        ...
        .execute_shared_read_batch(vec![
            forge_server::ForgeServerProductOperationInput::new(
    ...
    let shared_read =
        ForgeServerProductSharedReadCertificationProof::from_batch(&shared_read_batch)
    ...
    let replay_key =
        forge_server::ForgeServerProductIdempotencyKey::new("phase-thirteen-cert-replay")
    ...
    let mutation = ForgeServerProductMutationCertificationProof::new(&applied, &finalized)
        .expect("mutation proof");
    let finalize_denial_reason = match finalize_denied.outcome() {
        forge_server::ForgeServerProductOperationOutcome::Denied(denial) => denial.reason_key(),
        other => panic!("expected finalize denial, got {other:?}"),
    };
    let pressure = ForgeServerProductPressureShapeCertificationProof::new(
        &render,
        &select,
        &actions_before,
        &actions_after,
    ...
    let stale = ForgeServerProductStaleApplyDenialCertificationProof::from_denial(&stale)
        .expect("stale denial proof");
    let replay =
        ForgeServerProductIdempotentReplayCertificationProof::new(&authoritative, &replayed)
            .expect("replay proof");
    let route_parity =
        observed_route_parity(&server, &session, &initial_basis, &after_apply_basis).await;

    ForgeServerEditorLikeOperationFixture::new()
...
pub async fn blocked_editor_like_fixture() -> ForgeServerEditorLikeOperationFixture {
    let backend = StatefulEditorLikeBackend::new();
    let server = build_server(&backend);
    let session = direct_session(&server);
    let initial_basis = backend.basis_digest();
    let shared_read_batch = session
        .product_operations()
        .execute_shared_read_batch(vec![forge_server::ForgeServerProductOperationInput::new(
            "product_editor.render",
            render_payload(),
        )
        .with_basis_digest(&initial_basis)])
    ...
    ForgeServerEditorLikeOperationFixture::new()
        ...
        .with_mutation_certification(
            ForgeServerProductMutationCertificationProof::new(&apply, &finalize)
                .expect("mutation proof"),
        )
        .with_pressure_shape(ForgeServerProductPressureShapeCertificationProof::new(
            &render,
            &select,
            &actions,
            &actions,

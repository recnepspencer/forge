...
use forge_server::{
    ForgeServerBinaryDownloadRequest, ForgeServerCompatHttpRouteFamily,
    ForgeServerTransferByteClass, ForgeServerTransferCleanupReason,
};
use serde_json::json;
...
fn compat_http_phase_twelve_transfer_lifecycle_accounting_stays_narrow_and_reconstructable() {
    ...
    let stream = match stream {
        forge_server::ForgeServerStreamingResponse::Stream(value) => value,
    ...
    assert_cleanup_evidence(
        ...
        ForgeServerTransferCleanupReason::DownloadRetryAdmitted,
    );
    ...
    let ordinary_download = compat_download_success(
        retry_server.compat_http().download(
            forge_server::ForgeServerBinaryDownloadExecutionInput::new(
                ...
                "files.asset.download",
                ForgeServerBinaryDownloadRequest::new(b"ordinary-download".to_vec()),
            ),
        ),
    );
    assert!(
...
fn canonical_upload(identity: &str) -> forge_server::ForgeServerMultipartUpload {
    forge_server::ForgeServerMultipartUpload::new(
        forge_server::ForgeServerUploadManifest::new(json!({
            "command": {
                "family": "insert",
                "collection": "Task",
                "aspects": {
    ...
    .with_part(
        forge_server::ForgeServerUploadPart::file("blob")

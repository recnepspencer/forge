...
- `ForgeServerCompatibilityFacade::read(...)`
- `ForgeServerCompatibilityFacade::state(...)`
- `ForgeServerCompatibilityFacade::inspect(...)`
...
3. A canonical Forge query handoff.
...
1. Build a `ForgeServerCompatibilityRequestInput`.
2. Call `prepare_request(...)` to normalize and admit request context.
3. Wrap the prepared request in `ForgeServerCompatibilityExecutionInput`.
...
```text
authorization, x-forge-branch, x-forge-diagnostics
```
...
use forge_server::{
    ForgeServer, ForgeServerCompatHttpRouteFamily,
    ForgeServerCompatibilityExecutionInput, ForgeServerCompatibilityRequestInput,
};

let server: ForgeServer = build_server();
...
let read = match server.compat_http().read(
    ForgeServerCompatibilityExecutionInput::new(prepared, "users.profile"),
) {
    TransitionOutcome::Success(value) => value,

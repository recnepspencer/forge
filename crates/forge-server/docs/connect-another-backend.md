...
This guide is for a service that wants to call `forge-server` as a downstream
system.
...
A backend should use `forge-server` when it wants a strict execution boundary
...
Another backend should treat `forge-server` as an authority boundary, not as a
thin helper library.

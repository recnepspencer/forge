...
pub enum ForgeServerProductSessionCoordinationTarget {
    SessionCreation,
    ExistingSession { product_session_identity: String },
}

impl ForgeServerProductSessionCoordinationTarget {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ForgeServerOperationAuthorityMetadata {
    SharedReadOnly {
        basis_kind: String,
        basis_digest: String,
    ...
    ProductSessionCoordination {
        target: ForgeServerProductSessionCoordinationTarget,
...
impl ForgeServerOperationAuthorityMetadata {
    pub fn shared_read(
    ...
    pub(crate) fn canonical_digest(&self) -> String {
        match self {
            Self::SharedReadOnly {
                ...
                "forge-server-operation-authority-metadata-v1|kind=shared-read-only|basis_kind={basis_kind}|basis_digest={basis_digest}|label={authority_label}|product_support_posture={product_support_posture}"
            ),
            Self::DeterministicSubmission {
                submission_lane,
                journal_posture,
                ...
                "forge-server-operation-authority-metadata-v1|kind=deterministic-submission|lane={submission_lane}|journal={journal_posture}|base={base_digest_posture}|idempotency={idempotency_posture}"
            ),
            Self::ProductDraftMutation {
                product_session_identity,
                draft_scope,
                ...
                "forge-server-operation-authority-metadata-v1|kind=product-draft-mutation|session={product_session_identity}|draft={draft_scope}|base={base_digest_posture}|idempotency={idempotency_posture}"
            ),
            Self::ProductSessionCoordination {
                target,
                coordination_lane,
            } => format!(
                "forge-server-operation-authority-metadata-v1|kind=product-session-coordination|target={}|lane={coordination_lane}",
            ...
            Self::BinaryStreaming {
                ...
                "forge-server-operation-authority-metadata-v1|kind=binary-streaming|stream={stream_kind}|preflight={preflight_posture}|size={size_posture}|cancellation={cancellation_posture}|partial_failure={partial_failure_posture}"
            ),
            Self::DiagnosticsOnly { diagnostics_lane } => format!(
                "forge-server-operation-authority-metadata-v1|kind=diagnostics-only|lane={diagnostics_lane}"
            ),
            Self::LeaseCoordination {
                lease_target,
                resume_basis_digest,
                ...
                "forge-server-operation-authority-metadata-v1|kind=lease-coordination|target={lease_target}|basis={}|lane={coordination_lane}",
    ...
    pub fn product_session_coordination_target(
        &self,
    ) -> Option<(&ForgeServerProductSessionCoordinationTarget, &str)> {
        match self {
            Self::ProductSessionCoordination {
                target,
                coordination_lane,

use crate::{ForgeServerOperationFamily, ForgeServerOperationRequest};

use super::{ForgeServerOperationAuthorityMetadata, ForgeServerProductSessionCoordinationTarget};

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ForgeServerSharedReadBasisKind {
...
impl ForgeServerSharedReadBasisKind {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ForgeServerProductSupportPosture {
    ProductionAdmitted,
    Unsupported,
    Unknown,
    IncompatibleBasis,
...
impl ForgeServerProductSupportPosture {
    fn as_str(&self) -> &'static str {
        match self {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ForgeServerOperationAuthorityDeclaration {
    SharedRead {
        basis_kind: ForgeServerSharedReadBasisKind,
        product_support_posture: ForgeServerProductSupportPosture,
    },
...
impl ForgeServerOperationAuthorityDeclaration {
    pub fn query_shared_read() -> Self {
        Self::SharedRead {
            basis_kind: ForgeServerSharedReadBasisKind::QuerySharedReadBasis,
            product_support_posture: ForgeServerProductSupportPosture::ProductionAdmitted,
        }
    }

    pub fn product_shared_read(basis_kind: ForgeServerSharedReadBasisKind) -> Self {
    ...
    pub(crate) fn lower(
        ...
        match self {
            ...
            Self::DeterministicSubmission {
                ...
                base_digest_posture,
                idempotency_posture,
            } => Ok(ForgeServerOperationAuthorityMetadata::deterministic_submission(
                submission_lane,
                journal_posture,
            ...
            Self::ProductDraftMutation {
                ...
                base_digest_posture,
                idempotency_posture,
            } => Ok(ForgeServerOperationAuthorityMetadata::product_draft_mutation(
                operation_request
                    .identity()
            ...
            Self::ProductSessionCoordination { coordination_lane } => {
                Ok(ForgeServerOperationAuthorityMetadata::product_session_coordination(
                    match operation_request.identity().product_session_identity() {
                        ...
                        None => ForgeServerProductSessionCoordinationTarget::SessionCreation,
                    },
                    coordination_lane,
                ))
            }
            ...
            } => Ok(ForgeServerOperationAuthorityMetadata::binary_streaming(
                stream_kind,
                preflight_posture,
                size_posture,
                cancellation_posture,
            ...
            Self::LeaseCoordination { coordination_lane } => {
                Ok(ForgeServerOperationAuthorityMetadata::lease_coordination(
                    operation_request.identity().operation_name(),
                    operation_request
                        .identity()
                        .basis_digest()

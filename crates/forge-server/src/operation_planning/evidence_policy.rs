...
pub struct ForgeServerOperationPlanEvidencePolicy {
    diagnostics_profile: DiagnosticRichnessProfile,
    materialization_lane: &'static str,
    evidence_identity: String,
}

impl ForgeServerOperationPlanEvidencePolicy {
    pub(crate) fn from_diagnostics_profile(diagnostics_profile: DiagnosticRichnessProfile) -> Self {
        ...
        let evidence_identity = format!(
            "forge-server-operation-plan-evidence-policy-v1|diagnostics={materialization_lane}"
        );
        Self {
            diagnostics_profile,

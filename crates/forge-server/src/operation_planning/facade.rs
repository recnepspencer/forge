use crate::{
    config::ForgeServerQueryHandoffConfig, ForgeServerOperationConcurrencyClass,
    ...
    ForgeServerPreparedQueryHandoffKind, ForgeServerQueryHandoff, ForgeServerQueryHandoffOperation,
...
use super::{
    ...
    ForgeServerOperationPlanReceipt, ForgeServerOperationPlannerInput,
};

#[derive(Clone, Debug)]
pub struct ForgeServerOperationPlanner {
    query_handoff_config: ForgeServerQueryHandoffConfig,
    operation_registry: Option<ForgeServerOperationRegistry>,
}

impl ForgeServerOperationPlanner {
    ...
    pub fn lower(
        &self,
        input: ForgeServerOperationPlannerInput,
    ) -> Result<ForgeServerLoweredOperationPlan, ForgeServerOperationPlanDenial> {
        ...
        validate_prepared_intent(admission, &operation).map_err(|detail| {
            ForgeServerOperationPlanDenial::new(
        ...
        let readiness = match &self.operation_registry {
            ...
            None => ForgeServerOperationReadinessFacade::default(),
        };
        ...
        let readiness_closure = readiness
            ...
            .map_err(|denial| {
                ForgeServerOperationPlanDenial::from_readiness_denial(denial, diagnostics_profile)
            })?;
        ...
        let strategy = ForgeServerOperationExecutionStrategy::from_authority_kind(
            query_handoff
                .operation_admission()
                .authority_footprint()
                .authority_kind(),
        ...
        let evidence_policy =
            ForgeServerOperationPlanEvidencePolicy::from_diagnostics_profile(diagnostics_profile);
        let counters = ForgeServerOperationPlanCounters::new(
        ...
        let receipt = ForgeServerOperationPlanReceipt::new(
            query_handoff
        ...
        Ok(ForgeServerLoweredOperationPlan::new(
            query_handoff,
            strategy,
            evidence_policy,
            counters,
...
fn build_query_handoff(
    ...
    workspace: forge_query::facade::ForgeQueryWorkspace,
    downstream_delivery_contract: forge_query::facade::ForgeQueryRuntimeDownstreamDeliveryContract,
    readiness_closure: ForgeServerOperationReadinessClosure,
) -> ForgeServerQueryHandoff {
    let canonical_digest = format!(
        "forge-server-query-handoff-v3|tenant:{}|workspace:{}|bound:{}|operation:{}|operation_admission:{}|support:{}|precondition:{}|concurrency:{}|contract:{}",
        operation_admission
            .operation_request()
    ...
    let support_posture = readiness_closure
        ...
        .cloned()
        .unwrap_or_else(
            || crate::ForgeServerQuerySupportPosture::ProductIndependent {
                label: "not-required".to_string(),
            },
        );
    ForgeServerQueryHandoff::new(
        operation_admission,
        operation,
        workspace,
        downstream_delivery_contract,
...
fn validate_prepared_intent(
    ...
    let admitted = match operation {
        ForgeServerQueryHandoffOperation::QueryRead { operation_name } => {
            prepared.kind() == ForgeServerPreparedQueryHandoffKind::QueryRead
                && prepared.operation_name() == operation_name
        }
        ForgeServerQueryHandoffOperation::QueryMutation { operation_name, .. } => {
            prepared.kind() == ForgeServerPreparedQueryHandoffKind::QueryMutation
                && prepared.operation_name() == operation_name
        }
        ...
        ForgeServerQueryHandoffOperation::DownstreamDelivery { .. } => true,
        _ => false,
    };
    if admitted {
        Ok(())
...
fn support_rows_consulted(receipt: &crate::ForgeServerOperationSupportCompositionReceipt) -> usize {
    receipt.query_rows_consulted().len() + receipt.product_rows_consulted().len()
}

fn footprint_breadth(footprint: &crate::ForgeServerOperationAuthorityFootprint) -> usize {
...
fn canonical_plan_identity(
    ...
    format!(
        "forge-server-operation-plan-identity-v1|request={}|footprint={}|authorization={}|support={}|precondition={}|strategy={}|scheduler_lane={}",
        query_handoff
            .operation_admission()
            .operation_request()
            .identity()
...
fn scheduler_lane(operation_admission: &crate::ForgeServerOperationAdmissionPosture) -> String {
    match operation_admission.authority_footprint().authority_kind() {
        crate::ForgeServerOperationAuthorityKind::SharedReadOnly
        | crate::ForgeServerOperationAuthorityKind::DiagnosticsOnly => "shared-read".to_string(),
        crate::ForgeServerOperationAuthorityKind::DeterministicSubmission => operation_admission
            .authority_metadata()
            .submission_lane()
        ...
        crate::ForgeServerOperationAuthorityKind::ProductSessionCoordination => {
            ...
            match target {
                crate::ForgeServerProductSessionCoordinationTarget::SessionCreation => format!(

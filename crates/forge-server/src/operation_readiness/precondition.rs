...
pub enum ForgeServerOperationPreconditionPosture {
    NotRequired { canonical_digest: String },
    CompatibilityMutation(ForgeServerCompatibilityMutationPrecondition),
    ProductBasis(ForgeServerProductBasisPrecondition),
}

impl ForgeServerOperationPreconditionPosture {
    ...
    pub(crate) fn not_required(label: &str) -> Self {
        Self::NotRequired {
            canonical_digest: format!(
                "forge-server-operation-precondition-posture-v1|class=not-required|label={label}"
            ),
        }
    }

    pub fn compatibility_mutation(&self) -> Option<&ForgeServerCompatibilityMutationPrecondition> {
        match self {
    ...
    pub fn product_basis(&self) -> Option<&ForgeServerProductBasisPrecondition> {
        match self {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeServerProductBasisPrecondition {
...
impl ForgeServerProductBasisPrecondition {
    pub(crate) fn evaluate(
        operation_name: &str,
        requested_basis_digest: Option<&str>,
        observed_basis_digest: &str,
    ) -> Result<Self, ForgeServerOperationReadinessDenial> {
        if let Some(expected_basis_digest) = requested_basis_digest {
            if expected_basis_digest != observed_basis_digest {
                ...
                .with_facts(
                    ForgeServerOperationReadinessDenialFacts::default()
                        .with_basis_mismatch(expected_basis_digest, observed_basis_digest),
                ));
            }
        }
        let canonical_digest = format!(
            "forge-server-product-basis-precondition-v1|operation={}|requested_basis={}|observed_basis={}",
            operation_name.trim(),
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeServerCompatibilityMutationPrecondition {
    requested_basis_digest: Option<String>,
...
impl ForgeServerCompatibilityMutationPrecondition {
    pub(crate) fn evaluate(
...
fn canonical_idempotency_binding(
    prepared_request: &ForgeServerCompatibilityPreparedRequest,
    mutation_request_digest: &str,
) -> Result<String, ForgeServerOperationReadinessDenial> {
    let idempotency_key =
...
fn read_single_query_pair(
    prepared_request: &ForgeServerCompatibilityPreparedRequest,
    query_name: &str,
) -> Result<Option<String>, ForgeServerOperationReadinessDenial> {

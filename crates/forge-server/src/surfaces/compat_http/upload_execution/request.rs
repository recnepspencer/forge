...
pub enum ForgeServerUploadExpectation {
    ContinueOptional,
    ContinueRequired,
}

impl ForgeServerUploadExpectation {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ForgeServerUploadTransferMode {
    DeclaredLength,
    UnknownLength,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ForgeServerUploadContentEncoding {
    Identity,
    Gzip,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeServerUploadChunk {
    wire_bytes: Vec<u8>,
}

impl ForgeServerUploadChunk {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeServerUploadPart {
    name: String,
    content_type: String,
    declared_length: u64,
    ...
    wire_chunks: Vec<ForgeServerUploadChunk>,
    declared_integrity_digest: Option<String>,
}

impl ForgeServerUploadPart {
    pub fn file(name: impl Into<String>) -> Self {
        Self {
            name: name.into(),
            content_type: "application/octet-stream".to_string(),
            declared_length: 0,
            transfer_mode: ForgeServerUploadTransferMode::DeclaredLength,
            content_encoding: ForgeServerUploadContentEncoding::Identity,
    ...
    pub fn with_transfer_mode(mut self, transfer_mode: ForgeServerUploadTransferMode) -> Self {
    ...
    pub fn with_wire_chunk(mut self, chunk: ForgeServerUploadChunk) -> Self {
    ...
    pub fn transfer_mode(&self) -> ForgeServerUploadTransferMode {
        self.transfer_mode
    }

    pub fn content_encoding(&self) -> ForgeServerUploadContentEncoding {
    ...
    pub fn wire_chunks(&self) -> &[ForgeServerUploadChunk] {
        &self.wire_chunks
    }
    ...
    pub fn total_wire_len(&self) -> u64 {
        if self.wire_chunks.is_empty() {
            ...
            self.wire_chunks
                .iter()
                .map(ForgeServerUploadChunk::wire_len)
                .sum()
        }
    }
    ...
    pub fn effective_wire_chunks(&self) -> Vec<ForgeServerUploadChunk> {
        if self.wire_chunks.is_empty() {
            vec![ForgeServerUploadChunk::new(
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeServerUploadManifest {
    metadata_body: Value,
    declared_file_parts: Vec<String>,
    declared_integrity_digest: Option<String>,
}

impl ForgeServerUploadManifest {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeServerMultipartUpload {
    manifest: ForgeServerUploadManifest,
    expectation: ForgeServerUploadExpectation,
    parts: Vec<ForgeServerUploadPart>,
    canonical_digest: String,
}

impl ForgeServerMultipartUpload {
    pub fn new(manifest: ForgeServerUploadManifest) -> Self {
        let mut upload = Self {
            manifest,
            expectation: ForgeServerUploadExpectation::continue_optional(),
            parts: Vec::new(),
    ...
    pub fn with_expectation(mut self, expectation: ForgeServerUploadExpectation) -> Self {
        self.expectation = expectation;
        self.refresh_canonical_digest();
        self
    }

    pub fn with_part(mut self, part: ForgeServerUploadPart) -> Self {
        self.parts.push(part);
        self.refresh_canonical_digest();
        self
    }

    pub fn manifest(&self) -> &ForgeServerUploadManifest {
        &self.manifest
    }

    pub fn expectation(&self) -> ForgeServerUploadExpectation {
        self.expectation
    }

    pub fn parts(&self) -> &[ForgeServerUploadPart] {
        &self.parts
    }
    ...
    fn refresh_canonical_digest(&mut self) {
        let mut part_digests = self
            .parts
            .iter()
            .map(ForgeServerUploadPart::canonical_digest)
            .collect::<Vec<_>>();
        part_digests.sort();
        self.canonical_digest = format!(
            "forge-server-multipart-upload-v1|expectation={:?}|manifest={}|parts={}",
            self.expectation,

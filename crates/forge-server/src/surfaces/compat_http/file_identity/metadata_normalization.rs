...
pub struct ForgeServerMetadataNormalizationReceipt {
...
impl ForgeServerMetadataNormalizationReceipt {
...
pub(crate) fn validate_manifest_metadata_normalization(
    ...
    ForgeServerMetadataNormalizationReceipt::from_manifest(
        "validation-only",
        "validation-only",
        metadata_body,
        diagnostics_profile,
...
fn build_receipt(
    ...
    let canonical_digest = format!(
        "forge-server-metadata-normalization-receipt-v1|identity={metadata_identity}|filename={canonical_filename}|source={source_kind}|keys={}|diagnostics={diagnostics_profile:?}|receipt_kind={:?}|provenance_locality={:?}",
    ...
    let source_basis =
        FoundationalBoundaryEvidenceSourceBasis::boundary_artifact(BoundaryArtifactLocator::new(
            BoundaryArtifactId::new(boundary_artifact_id(&[
                "forge-server.file-identity.metadata-normalization".to_string(),
...
fn receipt_boundary(
    ...
    let commit_id = FoundationalCommitId::new(forge_foundational::facade::BoundaryHandle::new(
        boundary_artifact_id(&[
            "forge-server.file-identity.metadata-normalization.commit".to_string(),
            metadata_identity.to_string(),
            canonical_filename.to_string(),
            source_kind.to_string(),
            key_digest.clone(),
    ...
    let parent_basis = FoundationalCommitParentBasis::new(
        forge_foundational::facade::EquivalenceBasisId::new(boundary_artifact_id(&[
            "forge-server.file-identity.metadata-normalization.parent".to_string(),
            metadata_identity.to_string(),
            canonical_filename.to_string(),
            source_kind.to_string(),
            key_digest,

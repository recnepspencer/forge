...
pub enum ForgeServerFileTransferDisposition {
...
impl ForgeServerFileTransferDisposition {
...
pub struct ForgeServerFileTransferProvenance {
...
impl ForgeServerFileTransferProvenance {
    pub(crate) fn new(
        ...
        let canonical_digest = format!(
            "forge-server-file-transfer-provenance-v1|identity={metadata_identity}|tenant={tenant_id}|workspace={workspace_digest}|branch={branch_digest}|operation={operation_name}|disposition={}|content_type={}|bytes={bytes_selected}|range_honored={range_honored}|diagnostics={:?}|locality={:?}|freshness={:?}|source_kind={:?}",
            disposition.as_str(),
    ...
    pub fn disposition(&self) -> ForgeServerFileTransferDisposition {
        self.disposition
    }
    ...
    let source_basis =
        FoundationalBoundaryEvidenceSourceBasis::boundary_artifact(BoundaryArtifactLocator::new(
            BoundaryArtifactId::new(boundary_artifact_id(&[
                "forge-server.file-linkage.transfer".to_string(),

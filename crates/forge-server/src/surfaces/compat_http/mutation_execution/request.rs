...
pub struct ForgeServerCompatibilityMutationRequest {
    commands: Vec<ForgeServerCompatibilityMutationCommand>,
    batch: bool,
    canonical_digest: String,
}

impl ForgeServerCompatibilityMutationRequest {
    pub(crate) fn parse(
        body: Value,
        ...
        match (command, commands) {
            (Some(_), Some(_)) => Err(ForgeServerQueryHandoffDenial::new(
                ForgeServerQueryHandoffDenialCode::CompatibilityMutationRequestInvalid,
                diagnostics_profile,
            ...
            (Some(command), None) => {
                let parsed =
                    ForgeServerCompatibilityMutationCommand::parse(command, diagnostics_profile)?;
            ...
            (None, Some(commands)) => {
                ...
                for command in rows {
                    parsed.push(ForgeServerCompatibilityMutationCommand::parse(
                        command,
                        diagnostics_profile,
                    )?);
                }
            ...
            (None, None) => Err(ForgeServerQueryHandoffDenial::new(
                ForgeServerQueryHandoffDenialCode::CompatibilityMutationRequestInvalid,
                diagnostics_profile,
    ...
    fn new(commands: Vec<ForgeServerCompatibilityMutationCommand>, batch: bool) -> Self {
        let canonical_digest = format!(
            "compat-http-mutation-request-v1|kind:{}|commands:{}",
            if batch { "batch" } else { "single" },
            commands
                .iter()
                .map(ForgeServerCompatibilityMutationCommand::canonical_digest)
                .collect::<Vec<_>>()
                .join("||"),
        );
        Self {
    ...
    pub fn commands(&self) -> &[ForgeServerCompatibilityMutationCommand] {
        &self.commands
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ForgeServerCompatibilityMutationCommand {
    Insert {
        collection: String,
...
impl ForgeServerCompatibilityMutationCommand {
    fn parse(
        value: &Value,
        diagnostics_profile: forge_foundational::facade::DiagnosticRichnessProfile,
    ) -> Result<Self, ForgeServerQueryHandoffDenial> {
        ...
        match family.as_str() {
            ...
            other => Err(ForgeServerQueryHandoffDenial::new(
...
fn aspects_field(
    ...
    field: &str,
    diagnostics_profile: forge_foundational::facade::DiagnosticRichnessProfile,
) -> Result<BTreeMap<String, Value>, ForgeServerQueryHandoffDenial> {
    let value = object.get(field).ok_or_else(|| {
        ForgeServerQueryHandoffDenial::new(
            ForgeServerQueryHandoffDenialCode::CompatibilityMutationRequestInvalid,
            diagnostics_profile,

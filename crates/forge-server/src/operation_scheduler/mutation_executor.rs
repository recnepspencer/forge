...
use crate::{ForgeServerQueryOperation, ForgeServerResponseFacade, ForgeServerResponseInput};

use super::{
    ForgeServerExecutedOperationBatch, ForgeServerOperationExecutionSlot,
    ForgeServerOperationSchedulerCounters, ForgeServerScheduledMutationResult,
    ForgeServerScheduledOperationOutcome, ForgeServerSchedulerCancellationDirective,
    ForgeServerSchedulerCancellationPosture, ForgeServerSchedulerFailurePosture,
    ForgeServerSchedulerLane, ForgeServerSchedulerRuntimeFailure,
};
...
pub(crate) fn execute_slots(
    ...
    outcomes.sort_by_key(ForgeServerScheduledOperationOutcome::ordinal);
    let counters_snapshot = counters.clone();
    for outcome in &mut outcomes {
        outcome.attach_scheduler_counters(counters_snapshot.clone());
    }
    ForgeServerExecutedOperationBatch::new(outcomes, counters)
}
...
fn execute_group(group: OrderedLaneExecution) -> GroupExecution {
    ...
    for mut execution in group.executions {
        if let Some(failed_slot_ordinal) = failed_slot_ordinal {
            ...
            outcomes.push(
                ForgeServerScheduledOperationOutcome::failed_without_counters(
                    execution.slot,
                    ForgeServerSchedulerFailurePosture::OrderedLaneClosed {
        ...
        if is_stale_for_current_basis(&execution.slot, latest_lane_basis_digest.as_deref()) {
            ...
            outcomes.push(
                ForgeServerScheduledOperationOutcome::failed_without_counters(
                    execution.slot,
                    ForgeServerSchedulerFailurePosture::StaleMutationBasis {
        ...
        if execution.cancel_after_admission_before_execution {
            ...
            outcomes.push(
                ForgeServerScheduledOperationOutcome::cancelled_without_counters(
                    execution.slot,
                    ForgeServerSchedulerCancellationPosture::AfterAdmissionBeforeExecution,
                ),
            );
            continue;
        }
        ...
        match execution_result {
            Ok(mutation_result) => {
                if execution.cancel_during_execution {
                    ...
                    outcomes.push(
                        ForgeServerScheduledOperationOutcome::cancelled_without_counters(
                            execution.slot,
                            ForgeServerSchedulerCancellationPosture::DuringExecution,
                        ),
                    );
                    continue;
                }
                ...
                let response_envelope = group.response_facade.shape_with_defaults(
                    ForgeServerResponseInput::query_handoff_success(execution.slot.take_handoff()),
                );
                latest_lane_basis_digest = Some(mutation_result.snapshot_basis_digest());
                outcomes.push(
                    ForgeServerScheduledOperationOutcome::mutation_success_without_counters(
                        execution.slot,
                        mutation_result,
                        response_envelope,
                    ),
            ...
            Err(error) => {
                ...
                outcomes.push(
                    ForgeServerScheduledOperationOutcome::failed_without_counters(
                        execution.slot,
                        ForgeServerSchedulerFailurePosture::IsolatedRuntimeFailure {
                            runtime_failure:
                                ForgeServerSchedulerRuntimeFailure::from_mutation_runtime_error(
                                    error,
                                ),
                        },
                    ),
...
fn execute_slot_operation(
    slot: &mut ForgeServerOperationExecutionSlot,
) -> Result<ForgeServerScheduledMutationResult, ForgeQueryRuntimeError> {
    ...
    match query_operation {
        ForgeServerQueryOperation::SingleMutation { command, .. } => {
            ...
            let inspection = match workspace.inspect(&receipt)? {
                ForgeQueryInspection::WriteReceipt(inspection) => inspection,
                other => panic!("expected write receipt inspection, got {other:?}"),
            };
            Ok(ForgeServerScheduledMutationResult::Single {
                receipt,
                inspection,
            })
        }
        ForgeServerQueryOperation::BatchMutation { commands, .. } => {
            ...
            Ok(ForgeServerScheduledMutationResult::Batch {
                receipt,
                inspection,
            })
        }
...
fn increment_admission_counter(
    counters: &mut ForgeServerOperationSchedulerCounters,
    lane: &ForgeServerSchedulerLane,
) {
    match lane {
        ForgeServerSchedulerLane::DeterministicSubmission { .. } => {
            counters.increment_admitted_submission_slot_count();
        }
        ForgeServerSchedulerLane::ProductDraftMutation { .. }
        | ForgeServerSchedulerLane::ProductSessionCoordination { .. } => {
            counters.increment_admitted_mutation_slot_count();
        }
        ForgeServerSchedulerLane::SharedRead => {}
    }
}
...
fn current_basis_digest(slot: &ForgeServerOperationExecutionSlot) -> String {
    slot.handoff()
        .workspace()
        .snapshot_identity()
...
struct PreparedGroups {
    ...
    early_counters: ForgeServerOperationSchedulerCounters,
    early_outcomes: Vec<ForgeServerScheduledOperationOutcome>,
...
impl PreparedGroups {
    fn new(
        ...
        let mut early_counters = ForgeServerOperationSchedulerCounters::default();
        ...
        for slot in slots {
            if directives.before_admission(slot.ordinal()) {
                ...
                early_outcomes.push(
                    ForgeServerScheduledOperationOutcome::cancelled_without_counters(
                        slot,
                        ForgeServerSchedulerCancellationPosture::BeforeAdmission,
                    ),
                );
                continue;
            }
...
impl CancellationPlan {
    fn new(
        ...
        for directive in directives {
            match directive {
                ForgeServerSchedulerCancellationDirective::BeforeAdmission { slot_ordinal } => {
                ...
                ForgeServerSchedulerCancellationDirective::DuringExecution { slot_ordinal } => {

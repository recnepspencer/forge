use forge_query::facade::{ForgeQueryRuntimeError, ForgeQuerySharedReadContext};

use crate::{
    ForgeServerLoweredOperationPlan, ForgeServerOperationConcurrencyClass,
    ForgeServerOperationExecutionStrategy, ForgeServerOperationPlanProof,
    ForgeServerOperationPreconditionPosture, ForgeServerQueryHandoff,
    ForgeServerQueryHandoffOperation,
};

use super::{ForgeServerSchedulerConflictDenial, ForgeServerSchedulerLane};

#[derive(Debug)]
pub struct ForgeServerOperationExecutionSlot {
    ordinal: usize,
    scheduler_lane: ForgeServerSchedulerLane,
    dependency_group: Option<String>,
    plan_proof: ForgeServerOperationPlanProof,
    workspace_name: String,
    operation_label: String,
    precondition_posture: ForgeServerOperationPreconditionPosture,
    handoff: Option<ForgeServerQueryHandoff>,
}

impl ForgeServerOperationExecutionSlot {
    pub(crate) fn from_lowered_plan(
        ordinal: usize,
        plan: ForgeServerLoweredOperationPlan,
    ) -> Result<Self, ForgeServerSchedulerConflictDenial> {
        let scheduler_lane = ForgeServerSchedulerLane::from_lowered_plan(&plan)?;
    ...
    pub(crate) fn scheduler_lane_kind(&self) -> &ForgeServerSchedulerLane {
        &self.scheduler_lane
    }
    ...
    pub(crate) fn handoff(&self) -> &ForgeServerQueryHandoff {
        self.handoff
            .as_ref()
            .expect("scheduled slot should retain handoff until materialization")
    }

    pub(crate) fn handoff_mut(&mut self) -> &mut ForgeServerQueryHandoff {
        self.handoff
            .as_mut()
            .expect("scheduled slot should retain handoff until materialization")
    }

    pub(crate) fn take_handoff(&mut self) -> ForgeServerQueryHandoff {
        self.handoff
            .take()
...
fn validate_plan_for_scheduler(
    ...
    match scheduler_lane {
        ForgeServerSchedulerLane::SharedRead => validate_shared_read_plan(plan),
...
fn validate_shared_read_plan(
    ...
    if plan.strategy() != ForgeServerOperationExecutionStrategy::SharedReadExecution {
        return Err(ForgeServerSchedulerConflictDenial::non_shared_read_plan(
            format!(
    ...
    match plan.query_handoff().operation() {
        ...
        | ForgeServerQueryHandoffOperation::DirectInspection { .. } => Ok(()),
        unsupported => Err(
            ForgeServerSchedulerConflictDenial::unsupported_shared_read_operation(format!(
...
fn validate_ordered_plan(
    ...
    {
        return Err(ForgeServerSchedulerConflictDenial::unsupported_ordered_operation(
    ...
    match (plan.strategy(), plan.query_handoff().operation(), scheduler_lane) {
        (
            ForgeServerOperationExecutionStrategy::DeterministicSubmission,
        ...
        (
            ForgeServerOperationExecutionStrategy::ProductAdapterExecution,
            ForgeServerQueryHandoffOperation::DirectMutation {
                scheduled_operation: Some(_),
                ..
            }
        ...
        (
            ForgeServerOperationExecutionStrategy::SessionCoordination,
            ForgeServerQueryHandoffOperation::DirectMutation {
                scheduled_operation: Some(_),
                ..
            }
        ...
        (
            ...
            _,
        ) => Err(ForgeServerSchedulerConflictDenial::unsupported_ordered_operation(
            "ordered mutation scheduling requires a scheduled query mutation payload in the lowered plan",
        )),
        (_, unsupported, _) => Err(
            ForgeServerSchedulerConflictDenial::unsupported_ordered_operation(format!(
...
fn dependency_group(
    ...
    if *scheduler_lane != ForgeServerSchedulerLane::SharedRead {
        return None;
    }

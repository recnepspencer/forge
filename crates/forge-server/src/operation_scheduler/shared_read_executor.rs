...
use crate::{
    ForgeServerQueryHandoffOperation, ForgeServerResponseFacade, ForgeServerResponseInput,
};
...
use super::{
    ...
    ForgeServerSchedulerCertificationSabotage, ForgeServerSchedulerFailurePosture,
    ForgeServerSchedulerRuntimeFailure,
};
                ...
                outcomes.push(
                    ForgeServerScheduledOperationOutcome::failed_without_counters(
                        ...
                        ForgeServerSchedulerFailurePosture::IsolatedRuntimeFailure {
                            runtime_failure: ForgeServerSchedulerRuntimeFailure::opaque(
                                error.to_string(),
                            ),
                        },
                    ),
            ...
            Ok(execution_digest) => {
                if execution.cancel_during_execution {
                    ...
                    let response_envelope = group.response_facade.shape_with_defaults(
                        ForgeServerResponseInput::query_handoff_success(
                            execution.slot.take_handoff(),
                        ),
                    );
                    outcomes.push(
                        ForgeServerScheduledOperationOutcome::success_without_counters(
                            execution.slot,
            ...
            Err(detail) => {
                ...
                outcomes.push(
                    ForgeServerScheduledOperationOutcome::failed_without_counters(
                        execution.slot,
                        ForgeServerSchedulerFailurePosture::IsolatedRuntimeFailure {
                            runtime_failure: ForgeServerSchedulerRuntimeFailure::opaque(detail),
                        },
                    ),
                );
            }
...
fn execute_slot_operation(slot: &mut ForgeServerOperationExecutionSlot) -> Result<String, String> {
    let handoff = slot.handoff_mut();
    match handoff.operation().clone() {
        ForgeServerQueryHandoffOperation::QueryRead { operation_name }
        | ForgeServerQueryHandoffOperation::DirectRead { operation_name } => handoff
            .workspace_mut()
            .read_live_by_name(&operation_name)
            .map(|result| result.receipt().result_digest().to_string())
            .map_err(|error| error.to_string()),
        ForgeServerQueryHandoffOperation::DirectState { target_label } => handoff
            .workspace_mut()
            .state_live_by_name(&target_label)
            .map(|state| {
                state
                    .state_digest()
                    .terminal_projection_for_reporting()
            ...
            .map_err(|error| error.to_string()),
        ForgeServerQueryHandoffOperation::DirectInspection { target_label } => handoff
            .workspace_mut()
            .inspect_live_by_name(&target_label)
...
impl CertificationSabotagePlan {
    fn new(sabotage: impl IntoIterator<Item = ForgeServerSchedulerCertificationSabotage>) -> Self {
        let mut plan = Self::default();

...
use super::{
    execution_bridge::ForgeServerRouteExecutionOutcome,
    request_decoding::{ForgeServerRouteBranchTarget, ForgeServerRouteTransportRequest},
    response_projection::{semantic_response, transport_denial_response},
    ForgeServerOperationRouter, ForgeServerRouteAssembly, ForgeServerTransportDenial,
};

pub(crate) fn project_axum_router(
    assembly: &ForgeServerRouteAssembly,
    operation_router: ForgeServerOperationRouter,
) -> Router {
...
async fn semantic_route_handler(
    method: Method,
    uri: Uri,
    State(operation_router): State<ForgeServerOperationRouter>,
    headers: HeaderMap,
...
async fn operational_route_handler(
    ...
    match bridge.execute(ForgeServerRouteTransportRequest::new(
        "operational",
        "operational",
        "operational",
        ForgeServerRouteBranchTarget::Main,
    )) {
        Ok(ForgeServerRouteExecutionOutcome::Operational(outcome)) => {
            semantic_response(ForgeServerRouteExecutionOutcome::Operational(outcome))
        }
...
async fn unknown_route_handler() -> Response {
    transport_denial_response(ForgeServerTransportDenial::new(
        super::ForgeServerTransportDenialCode::UnknownRoute,
...
fn lower_transport_request(
    ...
    body: Vec<u8>,
) -> Result<ForgeServerRouteTransportRequest, ForgeServerTransportDenial> {
    ...
    let branch_target = if let Some(preview_id) = optional_header(headers, "x-preview-id") {
        ForgeServerRouteBranchTarget::Preview {
            preview_id: preview_id.to_string(),
        }
    } else if let Some(branch_id) = optional_header(headers, "x-branch-id") {
        ForgeServerRouteBranchTarget::Branch {
            branch_id: branch_id.to_string(),
        }
    } else {
        ForgeServerRouteBranchTarget::Main
    };
    let mut request = ForgeServerRouteTransportRequest::new(
...
fn required_header<'a>(
    ...
    optional_header(headers, name).ok_or_else(|| {
        ForgeServerTransportDenial::new(
            match name {
                "x-principal-id" => {
                    super::ForgeServerTransportDenialCode::MissingAuthenticatedPrincipalId
                }
                "x-tenant-id" => super::ForgeServerTransportDenialCode::MissingTenantId,
                _ => super::ForgeServerTransportDenialCode::MissingWorkspaceId,
            },

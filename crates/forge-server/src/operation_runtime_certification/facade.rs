use crate::{
    ForgeServerOperationFamily, ForgeServerOperationInventory,
    ForgeServerProductAdapterRegistrationReceipt, ForgeServerQueryDependencyAuditFacade,
    ForgeServerRouteInventory, ForgeServerSurfaceFamily,
};
...
use super::{
    ...
    ForgeServerProductOperationRuntimeRequirementRow, ForgeServerProductOperationRuntimeSupportRow,
};

#[derive(Clone, Debug)]
pub struct ForgeServerProductOperationRuntimeCertificationFacade {
    query_dependency_audit: ForgeServerQueryDependencyAuditFacade,
    operation_inventory: ForgeServerOperationInventory,
    route_inventory: ForgeServerRouteInventory,
    product_adapter_receipts: Vec<ForgeServerProductAdapterRegistrationReceipt>,
}

impl ForgeServerProductOperationRuntimeCertificationFacade {
    pub(crate) fn new(
        query_dependency_audit: ForgeServerQueryDependencyAuditFacade,
        operation_inventory: ForgeServerOperationInventory,
        route_inventory: ForgeServerRouteInventory,
    ...
    pub fn certify_product_editor_readiness(
        ...
        let editor_readiness = ForgeServerProductEditorReadinessCertification::new(fixture);
        let no_product_semantics = ForgeServerNoProductSemanticsCertification::derive(
            &self.operation_inventory,
            &self.route_inventory,
        );
        let requirements = ForgeServerProductOperationRuntimeArtifactRequirements::new(vec![
        ...
        let support_row = ForgeServerProductOperationRuntimeSupportRow::new(requirements);
        ForgeServerProductOperationRuntimeCertification::new(
            support_row,
            editor_readiness,
    ...
    fn query_dependency_requirement_row(&self) -> ForgeServerProductOperationRuntimeRequirementRow {
        ...
        let ready = self
            ...
            .all(|row| {
                row.enabled()
                    && row
                        .exposed_surfaces()
                        .contains(&ForgeServerSurfaceFamily::ForgeNative)
                    && row
                        .exposed_surfaces()
                        .contains(&ForgeServerSurfaceFamily::CompatHttp)
            });
        let digest = self
            .operation_inventory
            .rows()
    ...
    fn request_contract_requirement_row(&self) -> ForgeServerProductOperationRuntimeRequirementRow {
    ...
    fn product_adapter_requirement_row(&self) -> ForgeServerProductOperationRuntimeRequirementRow {
        let ready = self
            .product_adapter_receipts
            .iter()
        ...
        let digest = self
            .product_adapter_receipts
            .iter()
            .map(ForgeServerProductAdapterRegistrationReceipt::canonical_digest)
            .collect::<Vec<_>>()
            .join("|");
        build_requirement_row(
            "product-adapter",
    ...
    fn product_session_requirement_row(&self) -> ForgeServerProductOperationRuntimeRequirementRow {
        let required = [
            ...
            self.route_inventory.rows().iter().any(|row| {
                row.operation_name() == Some(*operation_name)
                    && row.operation_family()
                        == Some(ForgeServerOperationFamily::ProductSessionCoordination)
            })
        });
        let digest = self
            .route_inventory
    ...
    fn route_assembly_requirement_row(&self) -> ForgeServerProductOperationRuntimeRequirementRow {
        let required = [

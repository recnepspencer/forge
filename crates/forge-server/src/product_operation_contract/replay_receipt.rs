...
pub enum ForgeServerProductOperationReplayReceipt {
    Authoritative {
...
impl ForgeServerProductOperationReplayReceipt {
    pub(crate) fn authoritative(idempotency_key: &str, request_digest: &str) -> Self {
        Self::Authoritative {
            idempotency_key: idempotency_key.to_string(),
            request_digest: request_digest.to_string(),
            canonical_digest: format!(
                "forge-server-product-operation-replay-v1|class=authoritative|key={idempotency_key}|request={request_digest}"
            ),
        }
    }
    ...
    pub(crate) fn replayed(
        ...
        Self::Replayed {
            ...
            canonical_digest: format!(
                "forge-server-product-operation-replay-v1|class=replayed|key={idempotency_key}|request={request_digest}|authoritative={authoritative_operation_digest}"
            ),
        }
    }

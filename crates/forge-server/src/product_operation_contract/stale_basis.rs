...
pub struct ForgeServerProductStaleBasisDenial {
    expected_base_digest: String,
    observed_base_digest: String,
    canonical_digest: String,
}

impl ForgeServerProductStaleBasisDenial {
    pub(crate) fn new(
        ...
        let canonical_digest = format!(
            "forge-server-product-stale-basis-denial-v1|expected={expected_base_digest}|observed={observed_base_digest}"
        );
        Self {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ForgeServerProductRebaseRequired {
    denial: ForgeServerProductStaleBasisDenial,
}

impl ForgeServerProductRebaseRequired {
    pub fn new(denial: ForgeServerProductStaleBasisDenial) -> Self {
        Self { denial }
    }

    pub fn denial(&self) -> &ForgeServerProductStaleBasisDenial {
        &self.denial
    }

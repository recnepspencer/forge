...
Freshness and downgrade surfaces let `worth-proof` represent that a form was once strong but is no longer fully current. Instead of hiding that loss of confidence, the crate exposes explicit freshness classes and downgrade helpers.
...
- when you work directly with freshness wrapper types, prefer `use worth_proof::raw::*;`
...
```rust
use worth_proof::{CurrentValidity, FreshnessScopedBasis, RebindRequiredBasis};

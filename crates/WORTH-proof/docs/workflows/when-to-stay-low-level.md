...
This workflow explains when a caller should keep using the raw `worth-proof` substrate directly instead of trying to compress everything into a higher-level helper or wrapper.
...
```rust
use worth_proof::{PreConstructionGate, TransitionOutcome};

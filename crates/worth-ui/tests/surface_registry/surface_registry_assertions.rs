use worth_ui::facade::{
    CapabilityDiagnosticCode, CapabilityRegistrationDiagnostic, FrozenSurfaceCapabilities,
};
    ...
    let actual_codes_and_identities = diagnostics
        ...
        .map(|diagnostic| {
            (
                ...
                diagnostic
                    .identity_text()
                    .expect("diagnostic should identify surface registration"),
            )
        })
        .collect::<Vec<_>>();
...
pub(crate) fn assert_registered_surface_ids(
    surfaces: &FrozenSurfaceCapabilities,
    expected_surface_ids: &[&str],
) {
    let actual_surface_ids = surfaces
        .descriptors()
        .iter()
        .map(|descriptor| descriptor.id().as_str())
        .collect::<Vec<_>>();
    assert_eq!(actual_surface_ids, expected_surface_ids);
}

...
fn expect_string_literal_token(
    module_id: &WorthUiSourceModuleId,
    source_length: usize,
    stream: &mut WorthUiSourceTokenStream,
    message: &str,
...
fn unexpected_token_diagnostic(token: WorthUiSourceToken, message: &str) -> WorthUiParseDiagnostic {
    WorthUiParseDiagnostic::new(
...
fn span_from_bounds(start: &WorthUiSourceSpan, end: &WorthUiSourceSpan) -> WorthUiSourceSpan {
    WorthUiSourceSpan::new(
...
fn token_identifier_text(token: &WorthUiSourceToken) -> String {
    match token.kind() {
...
fn token_string_literal_text(token: &WorthUiSourceToken) -> String {
    match token.kind() {

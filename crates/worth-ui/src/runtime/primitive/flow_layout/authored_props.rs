...
pub(super) fn flow_layout_authored_props(
    ...
    host.inspect_active_authored_surface_props(surface_id)
        .map(|entry| {
            ...
            AuthoredFlowLayoutProp::new(entry.key(), value, entry.source_span())
        })
        .collect()
}

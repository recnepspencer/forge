use super::digest::{flow_layout_denial_digest, hash_text};
use super::schema::{
    WorthUiFlowLayoutPropSchema, WorthUiFlowLayoutValueDenialCode, WorthUiFlowLayoutValueKind,
};

#[derive(Clone, Debug, PartialEq)]
pub struct WorthUiFlowLayoutValueDenialReceipt {
    surface_id: String,
    prop_key: String,
    ...
    denial_code: WorthUiFlowLayoutValueDenialCode,
    source_span: Option<crate::runtime::WorthUiPrimitiveSourceSpan>,
    denial_digest: u64,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthUiFlowLayoutDenialPresentation {
    title: &'static str,
    rows: Vec<WorthUiFlowLayoutDenialPresentationRow>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthUiFlowLayoutDenialPresentationRow {
    label: &'static str,
    value: String,
}

impl WorthUiFlowLayoutValueDenialReceipt {
    pub(super) fn new(
        surface_id: &str,
        ...
        let denial_digest = flow_layout_denial_digest(surface_id, schema, &raw_value);
        Self {
    ...
    pub(super) fn unknown_prop(
        ...
        let schema_id = "worth.primitive.flow.prop.unknown";
        let value_kind = WorthUiFlowLayoutValueKind::Unknown;
        let denial_code = WorthUiFlowLayoutValueDenialCode::UnknownFlowLayoutProp;
        let denial_digest = hash_text(&format!(
            "flow-denial|surface:{surface_id}|schema:{schema_id}|prop:{prop_key}|kind:{:?}|value:{}|code:{:?}",
        ...
        Self {
            ...
            examples: &["flow_kind", "flow_gap", "flow_align"],
            semantic_slice: crate::runtime::WorthUiSemanticSliceId::PrimitiveFlowLayout,
            fact_family: crate::runtime::WorthUiRuntimeFactFamily::PrimitiveFlowLayout,
            denial_code,
            source_span,
            denial_digest,
        }
    ...
    pub fn presentation(&self) -> WorthUiFlowLayoutDenialPresentation {
        let span = self
            .source_span()
            .map(|span| format!("{}..{}", span.start_byte(), span.end_byte()))
            .unwrap_or_else(|| "unavailable".to_owned());
        WorthUiFlowLayoutDenialPresentation::new(
            "Flow layout value rejected",
            vec![
...
impl WorthUiFlowLayoutDenialPresentation {
    fn new(title: &'static str, rows: Vec<(&'static str, String)>) -> Self {
        Self {
            title,
            rows: rows
                .into_iter()
                .map(|(label, value)| WorthUiFlowLayoutDenialPresentationRow { label, value })
                .collect(),
        }
    }
    ...
    pub fn rows(&self) -> &[WorthUiFlowLayoutDenialPresentationRow] {
        &self.rows
    }
}

impl WorthUiFlowLayoutDenialPresentationRow {

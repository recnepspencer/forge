...
use super::super::admission::{
    primitive_content_receipt_digest, WorthUiValidatedPrimitiveContentPropSet,
};
...
impl WorthUiPrimitiveContentAdmissionReceipt {
    pub(crate) fn resolved_receipt(
        ...
        for kind in prop_set.order() {
            if !prop_set.participation().participates_in_layout() {
                break;
            }
            match kind {
                ...
                WorthUiPrimitiveContentItemKind::Image => {
                    if let Some(asset_id) = prop_set.image_asset_id() {
                        if let Some(descriptor) = runtime
                            .active_capability_snapshot()
                            .image_assets()
                            .get(asset_id)
                        {
                            items.push(WorthUiPrimitiveContentItem::image(
                                WorthUiPrimitiveImageAssetReceipt::from_descriptor(descriptor),
                            ));
                        }
                    }
                }

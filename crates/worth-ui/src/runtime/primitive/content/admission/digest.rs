use super::super::receipt::{WorthUiPrimitiveContentItem, WorthUiPrimitiveContentReceipt};
...
pub(super) fn primitive_content_admission_digest(
    ...
    hash_text(&format!(
        "content-admission|surface:{surface_id}|authored:{authored_digest}|kind:{:?}|order:{:?}|text:{}|icon:{:?}|image:{:?}|text_size:{}:{}|icon_size:{}:{}|stroke:{}:{}|spacer:{}:{}|badge:{:?}|divider:{}:{}|a11y:{:?}|role:{}|presence:{}",
        prop_set.kind(),
        prop_set.order(),
        prop_set.text(),
        prop_set.icon_id().map(|id| id.as_str()),
        prop_set.image_asset_id(),
        ...
        prop_set.role().token(),
        prop_set.participation().token()
    ))
}

pub(crate) fn primitive_content_receipt_digest(
    _admission_digest: u64,
    receipt: &WorthUiPrimitiveContentReceipt,
) -> u64 {
    let mut basis = format!(
        "content-receipt|kind:{:?}|presence:{}|role:{}|a11y:{:?}",
        receipt.kind(),
        receipt.participation().token(),
        receipt.role().token(),
...
fn content_item_digest_basis(item: &WorthUiPrimitiveContentItem) -> String {
    match item {
        ...
        WorthUiPrimitiveContentItem::Image(item) => format!(
            "image:{}:{}:{}:{}:{}:{}",
            item.asset_id(),
            item.source_kind(),
            item.source_key(),

use super::digest::{event_geometry_denial_digest, hash_text};
use super::schema::{
    WorthUiEventGeometryPropSchema, WorthUiEventGeometryValueDenialCode,
    WorthUiEventGeometryValueKind,
};

#[derive(Clone, Debug, PartialEq)]
pub struct WorthUiEventGeometryValueDenialReceipt {
    surface_id: String,
    prop_key: String,
    ...
    denial_code: WorthUiEventGeometryValueDenialCode,
    source_span: Option<crate::runtime::WorthUiPrimitiveSourceSpan>,
    denial_digest: u64,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthUiEventGeometryDenialPresentation {
    title: &'static str,
    rows: Vec<WorthUiEventGeometryDenialPresentationRow>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthUiEventGeometryDenialPresentationRow {
    label: &'static str,
    value: String,
}

impl WorthUiEventGeometryValueDenialReceipt {
    pub(super) fn new(
        surface_id: &str,
        ...
        let denial_digest = event_geometry_denial_digest(surface_id, schema, &raw_value);
        Self {
    ...
    pub(super) fn unknown_prop(
        ...
        let schema_id = "worth.primitive.event_geometry.prop.unknown";
        let value_kind = WorthUiEventGeometryValueKind::Unknown;
        let denial_code = WorthUiEventGeometryValueDenialCode::UnknownEventGeometryProp;
        let denial_digest = hash_text(&format!(
            "event-geometry-denial|surface:{surface_id}|schema:{schema_id}|prop:{prop_key}|kind:{:?}|value:{}|code:{:?}",
        ...
        Self {
            ...
            examples: &["event_cursor", "event_hit_area", "event_hit_slop"],
            semantic_slice: crate::runtime::WorthUiSemanticSliceId::PrimitiveEventGeometry,
            fact_family: crate::runtime::WorthUiRuntimeFactFamily::PrimitiveEventGeometry,
            denial_code,
            source_span,
            denial_digest,
        }
    ...
    pub fn presentation(&self) -> WorthUiEventGeometryDenialPresentation {
        let span = self
            .source_span()
            .map(|span| format!("{}..{}", span.start_byte(), span.end_byte()))
            .unwrap_or_else(|| "unavailable".to_owned());
        WorthUiEventGeometryDenialPresentation::new(
            "Event geometry value rejected",
            vec![
...
impl WorthUiEventGeometryDenialPresentation {
    fn new(title: &'static str, rows: Vec<(&'static str, String)>) -> Self {
        Self {
            title,
            rows: rows
                .into_iter()
                .map(|(label, value)| WorthUiEventGeometryDenialPresentationRow { label, value })
                .collect(),
        }
    }
    ...
    pub fn rows(&self) -> &[WorthUiEventGeometryDenialPresentationRow] {
        &self.rows
    }
}

impl WorthUiEventGeometryDenialPresentationRow {

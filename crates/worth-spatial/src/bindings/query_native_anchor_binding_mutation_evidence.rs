...
use crate::bindings::query_native_target_identity::{
    anchor_binding_target_identity_from_envelope, GeometryTargetIdentityFactError,
    GeometryTargetIdentityFactReceipt,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PrimitiveAnchorBindingMutationEvidence {
...
impl PrimitiveAnchorBindingMutationEvidence {
    fn from_bound_envelope(
        projection_payload: &PrimitiveAnchorBindingProjectionPayload,
        target_identity: GeometryTargetIdentityFactReceipt,
        envelope: &ForgeQueryDeclarationEnvelope<
            PrimitiveAnchorBindingQueryDomain,
            PrimitiveAnchorBindingDeclarationEntry,
        >,
    ) -> Result<Self, PrimitiveAnchorBindingMutationEvidenceError> {
...
pub enum PrimitiveAnchorBindingMutationEvidenceError {
...
impl PrimitiveAnchorBindingMutationEvidenceError {
...
pub fn primitive_anchor_binding_mutation_evidence<C>(
...
) -> Result<PrimitiveAnchorBindingMutationEvidence, PrimitiveAnchorBindingMutationEvidenceError>
where
    C: ForgeQueryDomainOperatingContext<PrimitiveAnchorBindingQueryDomain>,
{
    match handle.orchestrate_declaration_entry_outcome(declaration.clone()) {
        ForgeQueryOrdinaryOutcome::Bound(envelope) => declaration
            .projection_payload()
            .map_err(PrimitiveAnchorBindingMutationEvidenceError::DeclarationDenied)
            .and_then(|projection_payload| {
                declaration
                    .target_identity_payload()
                    .map_err(PrimitiveAnchorBindingMutationEvidenceError::DeclarationDenied)
                    .and_then(|target_payload| {
                        anchor_binding_target_identity_from_envelope(target_payload, &envelope)
                            .map_err(PrimitiveAnchorBindingMutationEvidenceError::TargetIdentity)
                            .and_then(|target_identity| {
                                PrimitiveAnchorBindingMutationEvidence::from_bound_envelope(
                                    projection_payload,
                                    target_identity,
                                    &envelope,
                                )
        ...
        | ForgeQueryOrdinaryOutcome::WrongWorld(posture) => {
            Err(PrimitiveAnchorBindingMutationEvidenceError::outcome_not_bound(&posture))
        }
    }
}

...
use subject::{
    ...
    cross_layer_surface_smuggling_denial, false_closure_denial, grazing_basket_stack_subject,
    missing_boundary_denial, missing_projection_denial, missing_retained_checkpoint_denial,
    near_graze_uncertain_denial, respond, storm_smuggling_denial, surface_smuggling_denial,
    whole_stack_broadening_denial,
};
...
fn mb_m6_nmt_4_rejects_cross_layer_checkpoint_projection_and_surface_smuggling() {
    ...
    assert!(surface.human_reason().contains("non-plane"));
...
fn mb_m6_nmt_4_touched_layer_counters_block_whole_stack_laundering() {
    ...
    let projection_response = respond(WorthUserResponseSource::from_grazing_basket_stack_denial(
        &missing_projection,
    ));

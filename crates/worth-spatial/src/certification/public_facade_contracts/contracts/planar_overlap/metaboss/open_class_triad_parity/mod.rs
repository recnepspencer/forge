...
use subject::{
    closed_storm_evidence, cross_class_checkpoint_denial, cross_class_projection_denial,
    denied_upgrade_denial, missing_lane_denial, open_class_triad_closeout_evidence,
...
fn mb_m6_nmt_3_open_class_triad_parity_compares_nine_receipt_backed_lanes() {
    run_with_real_workload_stack(|| {
        ...
        let closeout = open_class_triad_closeout_evidence("mb-m6-nmt-3-closeout");
        let receipt = NmtBossCloseoutReceipt::from_certified_scope_set(
            NmtBossId::OpenClassTriadParity,
            &closeout.certified_scopes,
            &closeout.matrix,
        )
        .expect("open-class triad parity must close out from certified scope evidence");
        assert_eq!(receipt.boss(), NmtBossId::OpenClassTriadParity);
...
fn mb_m6_nmt_3_cross_class_checkpoint_and_projection_forgery_denies() {
    run_with_real_workload_stack(|| {
        ...
        let checkpoint_denial = cross_class_checkpoint_denial(&subject);
        subject
            .receipt
        ...
        let storm_denial = storm_extraction_denial(&subject, &closed_storm_evidence());
        assert!(storm_denial
            .human_reason()
...
fn run_with_real_workload_stack(test: impl FnOnce() + Send + 'static) {
    std::thread::Builder::new()
        .name("mb-m6-nmt-3-real-workload".to_string())
        .stack_size(16 * 1024 * 1024)
        .spawn(test)
        .expect("spawn open-class triad workload stack")
        .join()
        .expect("open-class triad workload stack test");
}

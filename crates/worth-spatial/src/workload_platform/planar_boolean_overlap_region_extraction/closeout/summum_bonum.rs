...
use crate::workload_platform::planar_boolean_overlap_region_extraction::{
    PlanarBooleanOverlapReadinessLoopLedgerBinding, PlanarBooleanOverlapRegionCheckpointParityReceipt,
    PlanarBooleanOverlapRegionDecisionKind, PlanarBooleanOverlapRegionEvidenceReceipt,
...
impl PlanarBooleanOverlapRegionSummumBonumCloseout {
    pub fn certify(
        ...
        verify_binding(input.readiness, input.readiness_consumer, input.readiness_binding)?;
        verify_receipt_chain(input.readiness_binding, input.overlap_ledger_bundle, input.evidence_receipt)?;
        ...
        let subcases = certify_subcases(input.overlap_ledger_bundle, input.replay_parity_receipt, counters)?;
        ...
        Ok(Self {
            closeout_identity: truth_digest_parts(
                ...
                &[
                    ...
                    format!("checkpoint:{}", input.checkpoint_parity_receipt.checkpoint_identity()),
                ],
            ),
            request_identity: input.evidence_receipt.request_identity().to_string(),
            readiness_handoff_identity: input.evidence_receipt.readiness_handoff_identity().to_string(),
            readiness_consumer_identity: input.evidence_receipt.readiness_consumer_identity().to_string(),
            readiness_binding_identity: input.evidence_receipt.readiness_binding_identity().to_string(),
            loop_ledger_receipt_identity: input.evidence_receipt.loop_ledger_receipt_identity().to_string(),
            ...
            overlap_decision_log_identity: input.overlap_ledger_bundle.decision_log().decision_log_identity().to_string(),
            overlap_ordering_basis_identity: ledger.ordering_basis_identity().to_string(),
            overlap_region_identity_map_identity: receipt.overlap_region_identity_map_identity().to_string(),
            persistent_name_propagation_map_identity: receipt.persistent_name_propagation_map_identity().to_string(),
            ...
            checkpoint_identity: input.checkpoint_parity_receipt.checkpoint_identity().to_string(),
            replay_evidence_identity: input.checkpoint_parity_receipt.replay_evidence_identity().to_string(),
            subcases,
            counters,
        })
    }

    pub fn closeout_identity(&self) -> &str { &self.closeout_identity }
    ...
    pub fn counters(&self) -> PlanarBooleanOverlapRegionSummumBonumCloseoutCounters { self.counters }
    pub fn subcases(&self) -> &[PlanarBooleanOverlapRegionSummumBonumSubcaseRow] { &self.subcases }
...
impl PlanarBooleanOverlapRegionSummumBonumCloseoutCounters {
    pub fn readiness_inputs_consumed(self) -> usize { self.readiness_inputs_consumed }
    pub fn overlap_ledger_receipts_consumed(self) -> usize { self.overlap_ledger_receipts_consumed }
    pub fn replay_rows_verified(self) -> usize { self.replay_rows_verified }
    pub fn decision_rows_verified(self) -> usize { self.decision_rows_verified }
    pub fn ledger_rows_verified(self) -> usize { self.ledger_rows_verified }
    pub fn boundary_only_rows_verified(self) -> usize { self.boundary_only_rows_verified }
    pub fn area_rows_verified(self) -> usize { self.area_rows_verified }
    pub fn mixed_boundary_area_rows_verified(self) -> usize { self.mixed_boundary_area_rows_verified }
    pub fn pairwise_rediscovery_attempts(self) -> usize { self.pairwise_rediscovery_attempts }
}
...
fn verify_readiness_consumer(
    ...
    let matches = readiness.selected_route_identity_digest() == consumer.selected_route_identity_digest()
        && readiness.selected_family_identity() == consumer.selected_family_identity()
        && readiness.selected_product_identity_digest() == consumer.selected_product_identity_digest()
        && readiness.selected_witness_identity_digest() == consumer.selected_witness_identity_digest()
    ...
    if matches { Ok(()) } else { Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::ReadinessConsumerMismatch, PHASE_FIFTEEN_FENCE_SUBCASE, "phase-16 closeout requires one matching readiness handoff and readiness consumer")) }
}
...
fn verify_binding(
    ...
    let matches = binding.selected_route_identity_digest() == readiness.selected_route_identity_digest()
        ...
        && binding.selected_product_identity_digest() == readiness.selected_product_identity_digest()
        && binding.selected_product_identity_digest() == consumer.selected_product_identity_digest()
        && binding.selected_witness_identity_digest() == readiness.selected_witness_identity_digest()
        && binding.selected_witness_identity_digest() == consumer.selected_witness_identity_digest()
    ...
    if matches { Ok(()) } else { Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::ReadinessBindingMismatch, "synthetic_readiness_or_mismatched_loop_ledger_is_rejected", "phase-16 closeout requires the carried readiness-to-loop binding to match readiness authority")) }
}
...
fn verify_receipt_chain(
    ...
    if binding.loop_ledger_receipt_identity() != evidence_receipt.loop_ledger_receipt_identity() {
        return Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::LoopLedgerMismatch, "synthetic_readiness_or_mismatched_loop_ledger_is_rejected", "phase-16 closeout requires one readiness-bound loop-ledger receipt identity"));
    }
    if receipt.receipt_identity() != evidence_receipt.overlap_ledger_receipt_identity() {
        return Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::OverlapLedgerMismatch, "synthetic_overlap_ledger_is_rejected", "phase-16 closeout requires one overlap-ledger receipt carried by evidence"));
    }
    if receipt.request_identity() != evidence_receipt.request_identity() {
        return Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::RequestIdentityMismatch, "synthetic_overlap_ledger_is_rejected", "phase-16 closeout requires one request identity across overlap ledger and evidence"));
    }
    Ok(())
}
...
fn verify_replay_and_checkpoint(
    ...
    if replay_parity_receipt.checkpoint_receipt() != checkpoint_parity_receipt {
        return Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::ReplayParityMismatch, SubcaseKind::CheckpointReplayPreservesRegionIdentityAndNames.spec_name(), "phase-16 closeout requires replay parity to carry the same checkpoint receipt"));
    }
    if checkpoint_parity_receipt.replay_evidence_identity() != evidence_receipt.replay_evidence_identity() {
    ...
    for kind in [PlanarBooleanOverlapRegionReplayParityRowKind::OverlapEvidenceReceipt, PlanarBooleanOverlapRegionReplayParityRowKind::RequestIdentity, PlanarBooleanOverlapRegionReplayParityRowKind::ReadinessHandoff, PlanarBooleanOverlapRegionReplayParityRowKind::ReadinessConsumer, PlanarBooleanOverlapRegionReplayParityRowKind::ReadinessBinding, PlanarBooleanOverlapRegionReplayParityRowKind::OverlapDecisionLog, PlanarBooleanOverlapRegionReplayParityRowKind::OverlapLedgerReceipt, PlanarBooleanOverlapRegionReplayParityRowKind::OverlapIdentityMap, PlanarBooleanOverlapRegionReplayParityRowKind::PersistentNamePropagationMap, PlanarBooleanOverlapRegionReplayParityRowKind::SubshapeSignatureMap, PlanarBooleanOverlapRegionReplayParityRowKind::RetainedReplayCheckpoint] {
        if replay_parity_receipt.rows().iter().all(|row| row.kind() != kind) {
            return Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::MissingReplayParityRow, SubcaseKind::CheckpointReplayPreservesRegionIdentityAndNames.spec_name(), format!("phase-16 closeout is missing replay row {kind:?}")));
        }
    }
...
fn verify_canonical_identities(
    ...
    if identities.iter().all(|identity| !identity.is_empty()) { Ok(()) } else { Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::MissingCanonicalIdentity, PHASE_FIFTEEN_FENCE_SUBCASE, "phase-16 closeout requires canonical overlap identity, persistent-name, and subshape-signature authority")) }
}
...
fn build_counters(
    ...
    let boundary_only_rows_verified = bundle.ledger().rows().iter().filter(|row| row.correspondence_only()).count();
    let area_rows_verified = bundle.ledger().rows().iter().filter(|row| row.area_overlap_component_identity().is_some()).count();
    let mixed_boundary_area_rows_verified = usize::from(boundary_only_rows_verified > 0 && area_rows_verified > 0);
    let bundle_counters = bundle.counters();
    let pairwise_rediscovery_attempts = bundle_counters
        .decision_rows_admitted()
        .saturating_sub(bundle_counters.identity_rows_examined() + bundle_counters.ledger_rows_admitted());
    PlanarBooleanOverlapRegionSummumBonumCloseoutCounters {
...
fn certify_subcases(
    ...
    replay_parity_receipt: &PlanarBooleanOverlapRegionReplayParityReceipt,
    counters: PlanarBooleanOverlapRegionSummumBonumCloseoutCounters,
) -> Result<Vec<PlanarBooleanOverlapRegionSummumBonumSubcaseRow>, PlanarBooleanOverlapRegionSummumBonumCloseoutDenial> {
    let ledger_rows = bundle.ledger().rows();
    let boundary_only_rows = ledger_rows.iter().filter(|row| row.correspondence_only()).count();
    let area_rows = ledger_rows.iter().filter(|row| row.area_overlap_component_identity().is_some()).count();
    let all_boundary_only_are_non_area = ledger_rows.iter().filter(|row| row.correspondence_only()).all(|row| row.area_overlap_component_identity().is_none());
    ...
    let has_lineage = ledger_rows.iter().all(|row| !row.lineage_identities().is_empty());
    let has_nested_lineage = ledger_rows.iter().any(|row| row.lineage_identities().len() > 1 || row.canonical_source_loop_identities().len() > 1);
    ...
    {
        return Err(PlanarBooleanOverlapRegionSummumBonumCloseoutDenial::new(Kind::OrderingParityInstability, SubcaseKind::BenignLoopOrderVariationPreservesLedgerDigest.spec_name(), "phase-16 closeout requires a stable ordering basis and ledger digest surface"));
    }
    ...
    Ok(vec![
        PlanarBooleanOverlapRegionSummumBonumSubcaseRow::new(SubcaseKind::BoundaryOnlyCoincidentEdgesDoNotAdmitArea, format!("boundary_only_rows={boundary_only_rows}")),
        PlanarBooleanOverlapRegionSummumBonumSubcaseRow::new(SubcaseKind::OppositeSenseSameAreaOverlapHasStableWinding, format!("stable_regions={}", region_to_winding.len())),
        PlanarBooleanOverlapRegionSummumBonumSubcaseRow::new(SubcaseKind::NestedOverlapIslandsPreserveRegionIdentity, "nested lineage carried by admitted overlap rows"),
        PlanarBooleanOverlapRegionSummumBonumSubcaseRow::new(SubcaseKind::MixedBoundaryAndAreaContactDoesNotCollapse, format!("boundary_only_rows={boundary_only_rows}; area_rows={area_rows}")),
        PlanarBooleanOverlapRegionSummumBonumSubcaseRow::new(SubcaseKind::BenignLoopOrderVariationPreservesLedgerDigest, format!("ordering_basis={}", bundle.ledger().ordering_basis_identity())),
        PlanarBooleanOverlapRegionSummumBonumSubcaseRow::new(SubcaseKind::CheckpointReplayPreservesRegionIdentityAndNames, format!("replay_rows={}", replay_parity_receipt.rows().len())),
        PlanarBooleanOverlapRegionSummumBonumSubcaseRow::new(SubcaseKind::OverlapStormUsesIndexNotPairwiseRediscovery, format!("decision_rows={}; ledger_rows={}", counters.decision_rows_verified, counters.ledger_rows_verified)),
    ])
}

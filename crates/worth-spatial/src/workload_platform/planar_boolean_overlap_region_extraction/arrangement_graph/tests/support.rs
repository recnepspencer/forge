...
pub(crate) fn overlap_request_and_support(
    order: LoopFixtureEntryOrder,
) -> (
    PlanarBooleanOverlapRegionExtractionRequest,
    PlanarBooleanLoopReconstructionParticipationSupport,
) {
    static CANONICAL_SUPPORT: OnceLock<(
        PlanarBooleanOverlapRegionExtractionRequest,
        PlanarBooleanLoopReconstructionParticipationSupport,
    )> = OnceLock::new();
    static REPLAYED_SUPPORT: OnceLock<(
        PlanarBooleanOverlapRegionExtractionRequest,
        PlanarBooleanLoopReconstructionParticipationSupport,
    )> = OnceLock::new();
    let cache = match &order {
        LoopFixtureEntryOrder::Canonical => &CANONICAL_SUPPORT,
        LoopFixtureEntryOrder::Replayed => &REPLAYED_SUPPORT,
    };
    if let Some(cached) = cache.get() {
        return cached.clone();
    }

    let support = build_overlap_request_and_support(order);
    let _ = cache.set(support.clone());
    support
}

fn build_overlap_request_and_support(
    order: LoopFixtureEntryOrder,
) -> (
    PlanarBooleanOverlapRegionExtractionRequest,
    PlanarBooleanLoopReconstructionParticipationSupport,

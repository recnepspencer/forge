...
fn shares_partition_basis(
    left: &PlanarBooleanOverlapIslandCandidateRow,
    right: &PlanarBooleanOverlapIslandCandidateRow,
) -> bool {
    if is_boundary_area_envelope_pair(left, right) {
        return false;
    }
...
fn is_boundary_area_envelope_pair(
    left: &PlanarBooleanOverlapIslandCandidateRow,
    right: &PlanarBooleanOverlapIslandCandidateRow,
) -> bool {
    match (left.kind(), right.kind()) {
        (AreaOverlap, BoundaryContact) => area_envelopes_boundary(left, right),
        (BoundaryContact, AreaOverlap) => area_envelopes_boundary(right, left),
        _ => false,
    }
}

fn area_envelopes_boundary(
    area: &PlanarBooleanOverlapIslandCandidateRow,
    boundary: &PlanarBooleanOverlapIslandCandidateRow,
) -> bool {
    strictly_contains_all(
        area.boundary_component_identities(),
        boundary.boundary_component_identities(),
    ) || strictly_contains_all(
        area.boundary_segment_identities(),
        boundary.boundary_segment_identities(),
    )
}

fn strictly_contains_all(left: &[String], right: &[String]) -> bool {
    !right.is_empty()
        && left.len() > right.len()
        && right
            .iter()
            .all(|identity| left.iter().any(|candidate| candidate == identity))
}

...
use super::runtime_declaration::{
    ...
    LoweredSpatialOperation, LoweredSpatialTargetBindingPosture, RuntimeAnchorSemantic,
...
pub(super) fn admit_requested_intent(
    requested: RequestedLoweringIntent,
) -> Result<AdmittedLoweringIntent, SpatialLoweringDenial> {
    match requested {
        RequestedLoweringIntent::Move(placement, motion) => {
            crate::spatial_intent::lowering::admit_spatial_placement(placement.clone())
                .map(|admitted| AdmittedLoweringIntent::Move(placement, admitted, motion))
        }
        RequestedLoweringIntent::Offset(placement, motion) => {
            crate::spatial_intent::lowering::admit_spatial_placement(placement.clone())
                .map(|admitted| AdmittedLoweringIntent::Offset(placement, admitted, motion))
        }
        RequestedLoweringIntent::Rotate(placement, motion) => {
            crate::spatial_intent::lowering::admit_spatial_placement(placement.clone())
                .map(|admitted| AdmittedLoweringIntent::Rotate(placement, admitted, motion))
        }
        RequestedLoweringIntent::Reorient(placement, motion) => {
            crate::spatial_intent::lowering::admit_spatial_placement(placement.clone())
                .map(|admitted| AdmittedLoweringIntent::Reorient(placement, admitted, motion))
        }
        RequestedLoweringIntent::LiesOn(placement, constraint) => {
            crate::spatial_intent::lowering::admit_spatial_placement(placement.clone())
                .map(|admitted| AdmittedLoweringIntent::LiesOn(placement, admitted, constraint))
        }
        RequestedLoweringIntent::PointsToward(placement, constraint) => {
            crate::spatial_intent::lowering::admit_spatial_placement(placement.clone()).map(
                |admitted| AdmittedLoweringIntent::PointsToward(placement, admitted, constraint),
            )
        }
        RequestedLoweringIntent::AnchorMatch(placement, constraint) => {
            crate::spatial_intent::lowering::admit_spatial_placement(placement.clone()).map(
                |admitted| AdmittedLoweringIntent::AnchorMatch(placement, admitted, constraint),
            )
        }
    }
    .map_err(|_| SpatialLoweringDenial::InvalidExistingPlacement)
}

pub(super) fn lower_admitted_intent<C: SpatialWitnessCatalog>(
    admitted: AdmittedLoweringIntent,
    catalog: &C,
) -> Result<LoweredSpatialIntent, SpatialLoweringDenial> {
    match admitted {
        AdmittedLoweringIntent::Move(placement, _, motion) => {
            let anchor =
                lower_translation_anchor_with_catalog(&placement, motion.spec().anchor(), catalog)?;
            Ok(build(
                runtime(
        ...
        AdmittedLoweringIntent::Rotate(placement, admitted, motion) => match motion.spec().anchor()
        {
            ...
            _ => {
                let pivot = lower_rotation_anchor(&placement, motion.spec().anchor(), catalog)?;
                Ok(build(
                    runtime(
                        LoweredSpatialIntentFamily::Rotate,
        ...
        AdmittedLoweringIntent::AnchorMatch(placement, _, constraint) => {
            let anchor =
                lower_subject_anchor(&placement, constraint.spec().anchor(), catalog)?.into_point();

...
pub fn validate(view: &WorthTopologyView) -> Result<(), WorthTopologyValidationError> {
    validate_wire_half_edge_membership(view)?;
    validate_vertex_presence_for_branching(view)?;
    validate_wire_connectivity(view)?;
    validate_branch_vertices_use_distinct_edges(view)?;
    Ok(())
}
...
fn validate_wire_connectivity(view: &WorthTopologyView) -> Result<(), WorthTopologyValidationError> {

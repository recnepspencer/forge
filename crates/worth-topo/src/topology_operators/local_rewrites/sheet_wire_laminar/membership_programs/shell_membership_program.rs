            ...
            .compose_graph(|graph| {
                ...
                graph.retarget_existing_verified(
                    ...
                    |update| {
                        ...
                        let update = TopologyNativeQueryRowField::TopologySourceIdentity
                            .set_on(
                                TopologyNativeQueryRowField::TopologyKind.set_on(
                                    update,
                                    TopologyRelationKind::RegionOwnsShell.kind_name(),
                        ...
                        if let Some(field) = region_dependency_path
                            .and_then(TopologyNativeQueryRowField::from_query_aspect_path)
                        {
                            field.set_on(update, TopologyRelationKind::RegionOwnsShell.kind_name())
                        } else {
                            update
                        }
                    },
                ...
                for (face_handle, face_relation_id, face_identity) in &face_relation_rows {
                    graph.retarget_existing_verified(
                        ...
                        |verify| {
                            ...
                            {
                                field
                                    .set_on(verify, TopologyRelationKind::ShellOwnsFace.kind_name())
                            } else {
                                verify
                            }
                        },
                        |update| {
                            ...
                            let update = TopologyNativeQueryRowField::TopologyTargetIdentity
                                .set_on(update, face_identity.clone());
                            if let Some(field) = face_dependency_path
                                .and_then(TopologyNativeQueryRowField::from_query_aspect_path)
                            {
                                field

...
use super::boundary_audit::topology_query_hard_prohibition_boundary_audit;
use super::evidence_reports::topology_query_adoption_evidence_report;
use super::runtime_support::evaluate_current_topology_support_pins;

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthTopoQueryConsumerKitAdoptionStatus {
...
impl WorthTopoQueryConsumerKitAdoptionStatus {
...
#[derive(Debug)]
pub enum WorthTopoQueryConsumerKitAdoptionError {
    SupportPinning(ForgeQuerySupportPinningError),
    BoundaryAudit(ForgeQueryBoundaryAuditError),
    EvidenceReport(EvidenceReportError),
}

pub fn current_topology_query_consumer_kit_adoption_status(
) -> Result<WorthTopoQueryConsumerKitAdoptionStatus, WorthTopoQueryConsumerKitAdoptionError> {
    let support_report = evaluate_current_topology_support_pins()
        .map_err(WorthTopoQueryConsumerKitAdoptionError::SupportPinning)?;
    support_report
        .assert_satisfied()
        .map_err(WorthTopoQueryConsumerKitAdoptionError::SupportPinning)?;
    let boundary_report = topology_query_hard_prohibition_boundary_audit()
        .map_err(WorthTopoQueryConsumerKitAdoptionError::BoundaryAudit)?;
    boundary_report.assert_clean();
    let evidence_report =
        topology_query_adoption_evidence_report(&support_report, &boundary_report)
            .map_err(WorthTopoQueryConsumerKitAdoptionError::EvidenceReport)?;

    Ok(WorthTopoQueryConsumerKitAdoptionStatus {
...
mod tests {
    ...
    #[test]
    fn topology_public_status_exposes_adoption_status_without_query_internals() {
        let status = current_topology_query_consumer_kit_adoption_status()
            .expect("topology Query consumer-kit adoption should be current");

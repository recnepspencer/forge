...
pub(super) fn current_topology_support_snapshot() -> ForgeQuerySupportSnapshot {
    snapshot_from_profile(ForgeQueryRuntimeSupportProfile::scaffold_backend_profile())
}

pub(super) fn current_topology_support_pin_contract(
    snapshot: &ForgeQuerySupportSnapshot,
) -> Result<ForgeQuerySupportPinContract, ForgeQuerySupportPinningError> {
    let mut builder = support_pinning_contract("worth-topo.query-adoption.phase-three")
...
pub(super) fn evaluate_current_topology_support_pins(
) -> Result<ForgeQuerySupportPinReport, ForgeQuerySupportPinningError> {
    let snapshot = current_topology_support_snapshot();
    current_topology_support_pin_contract(&snapshot)?.evaluate_snapshot(&snapshot)
}
...
mod tests {
    ...
    #[test]
    fn topology_support_pins_evaluate_against_real_query_snapshot() {
        let report = evaluate_current_topology_support_pins()
            .expect("topology support pins should evaluate through Query");

        assert!(report.satisfied());
        assert_eq!(
            report.consumer_name(),
            "worth-topo.query-adoption.phase-three"
        );
    ...
    #[test]
    fn topology_support_pin_drift_is_localized_to_inspect_requirement() {
        let basis = current_topology_support_snapshot();
        let drifted = snapshot_from_profile(
            ForgeQueryRuntimeSupportProfile::scaffold_backend_profile().with_family_support(
                ForgeQueryRuntimeFamilySupport::deferred(
                    ForgeQueryRuntimeFacadeFamily::Inspect,
                    "worth-topo phase three drift fixture",
                ),
            ),
        );
        let contract = current_topology_support_pin_contract(&basis)
            .expect("topology support pin contract should seal");
        ...
        assert!(report.findings().iter().any(|finding| {
            finding.family() == Some(ForgeQueryRuntimeFacadeFamily::Inspect)

...
fn edit_batch_digest_is_deterministic_for_same_contracts() {
    ...
    let left = super::TopologyEditBatch::new(contracts.clone()).expect("non-empty edit batch");
    let right = super::TopologyEditBatch::new(contracts).expect("non-empty edit batch");
...
fn edit_batch_continuity_matrix_counts_naming_outcomes() {
    let batch = super::TopologyEditBatch::new(vec![
        TopologyEditContract::create_topology_entity(
            "m3.naming.vertex",
            TopologyEntityKind::Vertex,
        ),
...
fn continuity_matrix_exposes_overall_outcome_class() {
    let ambiguous =
        super::TopologyEditBatch::new(vec![TopologyEditContract::attach_boundary_membership(
    ...
    assert_eq!(
        ...
        Some(super::TopologyEditRejectionClass::NamingContinuityAmbiguous)
    );

    let rejected =
        super::TopologyEditBatch::new(vec![TopologyEditContract::retire_topology_entity(
    ...
    assert_eq!(
        ...
        Some(super::TopologyEditRejectionClass::NamingContinuityRejected)
    );
}

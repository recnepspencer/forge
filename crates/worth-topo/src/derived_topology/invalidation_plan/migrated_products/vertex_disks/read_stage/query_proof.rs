use super::super::VertexDiskMigrationError;
...
pub(super) fn query_report_digests_for_shared_vertex_views(
    views: &[TopologyHalfEdgeSharedVertexNeighborhoodView],
) -> Result<Vec<String>, VertexDiskMigrationError> {
    views
        .iter()
...
) -> Result<String, VertexDiskMigrationError> {
    require_query_shared_vertex_report(report)?;
    Ok(super::super::super::super::catalog::catalog_digest([
        "worth-topo:vertex-disk-query-report:v1".to_string(),
...
fn require_query_shared_vertex_report(
    report: &TopologyReadRequestReport,
) -> Result<(), VertexDiskMigrationError> {
    if report.request_family() != TopologyReadRequestFamily::HalfEdgeSharedVertexNeighborhood {
    ...
    let graph_access_proof = report
        .graph_access_proof()
        .ok_or(VertexDiskMigrationError::ReadStageQueryProofInvalid)?;

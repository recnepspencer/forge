...
use super::{WireViewReadSource, WireViewReadStageCounters};
...
impl WireViewReadStageReceipt {
    pub fn from_selected_plan_and_read_source(
        ...
        let native_query_read_receipt_digest = selected_row
            ...
            .ok_or(WireViewMigrationError::ReadStageReceiptMissingQueryReceipt)?
            .to_string();
        if !read_source
            .query_report_digests()
            .contains(&native_query_read_receipt_digest)
        {
            return Err(WireViewMigrationError::ReadStageQueryReceiptNotBoundToSource);
        }
        let selected_legality_receipt_digest = selected_row
            .legality_receipt_digest()
            .ok_or(WireViewMigrationError::ReadStageReceiptMissingLegalityReceipt)?
            .to_string();
        let selected_source_row_count = read_source.selected_rows().len();
        if selected_source_row_count == 0 {
            return Err(WireViewMigrationError::ReadStageTouchedClosureSelectedNoWireViewRows);
        }
        let touched_closure_wire_view_bound = touched_closure_wire_view_bound(selected_plan);
        if selected_source_row_count > touched_closure_wire_view_bound {
            return Err(WireViewMigrationError::ReadStageSelectedRowsExceedTouchedClosure);
        }

        let wire_view_selected_row_digest = selected_row.row_digest().to_string();
        ...
        let mut receipt_parts = vec![
            "worth-topo:wire-view-read-stage-receipt:v1".to_string(),
            ...
            format!("selected-row:{wire_view_selected_row_digest}"),
    ...
    pub fn wire_view_selected_row_digest(&self) -> &str {
        &self.wire_view_selected_row_digest
    }
...
fn touched_closure_wire_view_bound(selected_plan: &DerivedInvalidationSelectedPlan) -> usize {

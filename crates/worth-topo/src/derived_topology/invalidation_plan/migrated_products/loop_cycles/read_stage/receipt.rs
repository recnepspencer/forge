...
use super::{LoopCycleReadSource, LoopCycleReadStageCounters};
...
impl LoopCycleReadStageReceipt {
    pub fn from_selected_plan_and_read_source(
        ...
        let native_query_read_receipt_digest = selected_row
            ...
            .ok_or(LoopCycleMigrationError::ReadStageReceiptMissingQueryReceipt)?
            .to_string();
        let selected_legality_receipt_digest = selected_row
            .legality_receipt_digest()
            .ok_or(LoopCycleMigrationError::ReadStageReceiptMissingLegalityReceipt)?
            .to_string();
        let touched_closure_loop_cycle_bound = touched_closure_loop_cycle_bound(selected_plan);
        ...
        if selected_source_row_count > touched_closure_loop_cycle_bound {
            return Err(LoopCycleMigrationError::ReadStageSelectedRowsExceedTouchedClosure);
        }

        let loop_cycle_selected_row_digest = selected_row.row_digest().to_string();
        ...
        let mut receipt_parts = vec![
            "worth-topo:loop-cycle-read-stage-receipt:v1".to_string(),
            ...
            format!("selected-row:{loop_cycle_selected_row_digest}"),
...
fn touched_closure_loop_cycle_bound(selected_plan: &DerivedInvalidationSelectedPlan) -> usize {

...
pub struct LoopCycleOldAuthorityResidueRow {
    caller: String,
    owner: String,
    blocker: String,
...
impl LoopCycleOldAuthorityResidueRow {
    fn new(
        ...
        let row_digest = super::super::super::catalog::catalog_digest([
            "worth-topo:loop-cycle-old-authority-residue-row:v1".to_string(),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct LoopCycleOldAuthorityResidue {
    capped_direct_interpreter_count: usize,
    capped_rows: Vec<LoopCycleOldAuthorityResidueRow>,
    residue_digest: String,
}

impl LoopCycleOldAuthorityResidue {
    ...
    pub(crate) fn unknown_old_authority_for_tests() -> Self {
        Self::new(vec![LoopCycleOldAuthorityResidueRow::new(
            "derived_topology::loop_cycles::unknown_old_entry_point",
            "",
            "",
            "",
        )])
    ...
    fn new(capped_rows: Vec<LoopCycleOldAuthorityResidueRow>) -> Self {
        let capped_direct_interpreter_count = capped_rows.len();
        let mut parts = vec![
            "worth-topo:loop-cycle-old-authority-residue:v1".to_string(),
    ...
    pub fn capped_rows(&self) -> &[LoopCycleOldAuthorityResidueRow] {
        &self.capped_rows
    }
...
fn current_forbidden_source_rows() -> Vec<LoopCycleOldAuthorityResidueRow> {
    source_roots()
        .into_iter()
        .flat_map(scan_source_root_for_forbidden_loop_cycle_authority)
        .collect()
}

fn scan_source_root_for_forbidden_loop_cycle_authority(
    root: PathBuf,
) -> Vec<LoopCycleOldAuthorityResidueRow> {
    let mut rows = Vec::new();
    scan_directory(&root, &mut rows);
    rows
}

fn scan_directory(directory: &Path, rows: &mut Vec<LoopCycleOldAuthorityResidueRow>) {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(token) = forbidden_loop_cycle_authority_token(&source) {
            rows.push(LoopCycleOldAuthorityResidueRow::new(
                path.to_string_lossy().into_owned(),
                "loop-cycle migrated product source firewall",
                token,
                "delete direct loop-cycle authority and route through migrated read-stage",
            ));
        }
    }
}
...
pub(super) fn forbidden_loop_cycle_authority_token(source: &str) -> Option<&'static str> {
    [
        "derived_topology::loop_cycles",
        "derived_topology/loop_cycles",
        "LoopCycleBoundaryInterpretation",
    ]
    .into_iter()
...
fn is_excluded_source_path(path: &Path) -> bool {
    ...
    path.contains("/derived_topology/invalidation_plan/migrated_products/loop_cycles/")

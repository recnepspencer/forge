...
use super::{RadialRingReadSource, RadialRingReadStageCounters};
...
impl RadialRingReadStageReceipt {
    pub fn from_selected_plan_and_read_source(
        ...
        let native_query_read_receipt_digest = selected_row
            ...
            .ok_or(RadialRingMigrationError::ReadStageReceiptMissingQueryReceipt)?
            .to_string();
        let selected_legality_receipt_digest = selected_row
            .legality_receipt_digest()
            .ok_or(RadialRingMigrationError::ReadStageReceiptMissingLegalityReceipt)?
            .to_string();
        let touched_closure_radial_ring_bound = touched_closure_radial_ring_bound(selected_plan);
        ...
        if selected_source_row_count > touched_closure_radial_ring_bound {
            return Err(RadialRingMigrationError::ReadStageSelectedRowsExceedTouchedClosure);
        }

        let radial_ring_selected_row_digest = selected_row.row_digest().to_string();
        ...
        let mut receipt_parts = vec![
            "worth-topo:radial-ring-read-stage-receipt:v1".to_string(),
            ...
            format!("selected-row:{radial_ring_selected_row_digest}"),
...
fn touched_closure_radial_ring_bound(selected_plan: &DerivedInvalidationSelectedPlan) -> usize {

...
use super::{RadialRingMigrationError, RadialRingReadStageCounters, RadialRingReadStageReceipt};
...
pub struct RadialRingBoundarySourceRow {
...
impl RadialRingBoundarySourceRow {
    pub fn new(
        ...
        let row_digest = super::super::super::catalog::catalog_digest([
            "worth-topo:radial-ring-boundary-source-row:v1".to_string(),
    ...
    pub(crate) fn from_query_radial_neighborhood_view(
        view: &TopologyHalfEdgeRadialNeighborhoodView,
    ) -> Self {
...
pub struct RadialRingTouchedBoundaryRows {
...
#[cfg(test)]
impl RadialRingTouchedBoundaryRows {
    pub fn from_selected_rows(rows: Vec<RadialRingBoundarySourceRow>) -> Self {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RadialRingExecutionInput {
    selected_rows: Vec<RadialRingBoundarySourceRow>,
    available_source_row_count: usize,
    selected_plan_digest: String,
    radial_ring_selected_row_digest: String,
    source_rows_digest: String,
    read_stage_receipt_digest: String,
    touched_closure_radial_ring_bound: usize,
    read_stage_counters: RadialRingReadStageCounters,
    input_digest: String,
}

impl RadialRingExecutionInput {
    pub fn from_selected_plan_and_read_stage(
        selected_plan: &DerivedInvalidationSelectedPlan,
        read_stage_receipt: RadialRingReadStageReceipt,
    ) -> Result<Self, RadialRingMigrationError> {
        let radial_ring_selected_row_digest = selected_plan
            .selected_rows()
            .iter()
            .find(|row| row.family_identity() == DerivedTopologyProductFamilyIdentity::RadialRings)
            .map(|row| row.row_digest().to_string())
            .ok_or(RadialRingMigrationError::SelectedPlanMissingRadialRingRow)?;
        if selected_plan.selected_plan_digest() != read_stage_receipt.selected_plan_digest()
            ...
            || radial_ring_selected_row_digest
                != read_stage_receipt.radial_ring_selected_row_digest()
        {
            return Err(RadialRingMigrationError::ReadStageReceiptNotBoundToSelectedPlan);
        }
        ...
        let input_digest = super::super::super::catalog::catalog_digest([
            "worth-topo:radial-ring-execution-input:v2".to_string(),
            format!("selected-plan:{selected_plan_digest}"),
            format!("radial-ring-selected-row:{radial_ring_selected_row_digest}"),
        ...
        Ok(Self {
            ...
            touched_closure_radial_ring_bound: read_stage_receipt
    ...
    pub fn from_selected_plan(
        selected_plan: &DerivedInvalidationSelectedPlan,
        rows: RadialRingTouchedBoundaryRows,
    ) -> Result<Self, RadialRingMigrationError> {
        let radial_ring_selected_row_digest = selected_plan
            .selected_rows()
            .iter()
        ...
        let input_digest = super::super::super::catalog::catalog_digest([
            "worth-topo:radial-ring-execution-input:v1".to_string(),
            format!("selected-plan:{selected_plan_digest}"),
            format!("radial-ring-selected-row:{radial_ring_selected_row_digest}"),
            format!("source-rows:{source_rows_digest}"),
        ]);
        Ok(Self {
            ...
            radial_ring_selected_row_digest,
            source_rows_digest,
            read_stage_receipt_digest: "test-only-raw-radial-ring-input".to_string(),
            touched_closure_radial_ring_bound: selected_row_bound(selected_plan),
            read_stage_counters: RadialRingReadStageCounters::for_selected_rows(
                selected_row_count,
    ...
    pub fn radial_ring_selected_row_digest(&self) -> &str {
        &self.radial_ring_selected_row_digest
    }
    ...
    pub const fn read_stage_counters(&self) -> &RadialRingReadStageCounters {
...
fn source_rows_digest(
    ...
    let mut parts = vec![
        "worth-topo:radial-ring-touched-source-rows:v1".to_string(),

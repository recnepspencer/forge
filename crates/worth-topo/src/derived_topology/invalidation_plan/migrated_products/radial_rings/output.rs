...
use super::input::RadialRingExecutionInput;
use super::RadialRingReadStageCounters;

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RadialRingProductRow {
...
impl RadialRingProductRow {
    pub(crate) fn from_source_row(row: &super::RadialRingBoundarySourceRow) -> Self {
        let row_digest = super::super::super::catalog::catalog_digest([
            "worth-topo:radial-ring-product-row:v1".to_string(),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RadialRingDerivedProductOutput {
    rows: Vec<RadialRingProductRow>,
...
impl RadialRingDerivedProductOutput {
    pub(crate) fn from_execution_input(input: &RadialRingExecutionInput) -> Self {
        let rows = input
            .selected_rows()
            .iter()
            .map(RadialRingProductRow::from_source_row)
            .collect::<Vec<_>>();
        Self::from_rows(
            rows,
            input.touched_closure_radial_ring_bound(),
    ...
    pub(crate) fn from_rows(
        ...
        let mut parts = vec![
            "worth-topo:radial-ring-derived-product-output:v1".to_string(),
    ...
    pub fn rows(&self) -> &[RadialRingProductRow] {
        &self.rows
    }

use super::support::{
    close_radial_ring_slice_from_topology, selected_radial_ring_read_source,
    selected_radial_ring_topology_with_many_unrelated_shells,
    selected_radial_ring_topology_with_unrelated_shells, selected_radial_ring_touched_closure,
    selected_radial_rings_plan, source_row,
};
...
fn scale_pressure_counts_touched_rows_not_available_topology_rows() {
    ...
    let topology = selected_radial_ring_topology_with_many_unrelated_shells(10_000);
...
#[test]
fn fixture_topology_read_source_selects_only_touched_radial_root_rows() {
    let read_source = selected_radial_ring_read_source();
...
#[test]
fn fixture_topology_read_source_does_not_count_unrelated_radial_rings_as_available_work() {
    let plan = selected_radial_rings_plan("loop-touch");
    let touched_closure = selected_radial_ring_touched_closure("loop-touch");
    let topology = selected_radial_ring_topology_with_unrelated_shells();

    let read_source = super::super::RadialRingReadSource::select_from_touched_closure(
        &plan,
        &touched_closure,
        &topology,
    )

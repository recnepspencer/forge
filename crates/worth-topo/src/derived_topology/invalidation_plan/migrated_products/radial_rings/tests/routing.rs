use super::support::{
    ...
    selected_radial_ring_touched_closure_for_shell, selected_radial_rings_plan_for_shell,
};
...
fn same_family_declaration_routes_multiple_operator_touches_without_executor_edits() {
    ...
    let first_plan = selected_radial_rings_plan_for_shell("operator-a", 24);
    let second_plan = selected_radial_rings_plan_for_shell("operator-b", 99);
    let first_closure = selected_radial_ring_touched_closure_for_shell("operator-a", 24);
    let second_closure = selected_radial_ring_touched_closure_for_shell("operator-b", 99);
    let first = close_radial_ring_slice_from_topology(&first_plan, &first_closure, &topology);
    let second = close_radial_ring_slice_from_topology(&second_plan, &second_closure, &topology);

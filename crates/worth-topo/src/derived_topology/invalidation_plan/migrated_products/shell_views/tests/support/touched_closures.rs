...
pub(crate) fn selected_shell_view_touched_closure(
    semantic_family_key: &'static str,
) -> DerivedInvalidationTouchedClosure {
    selected_shell_view_touched_closure_for_shell(semantic_family_key, 24)
}

pub(crate) fn selected_shell_view_touched_closure_for_shell(
    semantic_family_key: &'static str,
    half_edge_slot: u64,
) -> DerivedInvalidationTouchedClosure {
    ...
    let proof = TopologyDeclaredTouchedGraphBasisProof::from_basis(semantic_family_key, basis)
        .expect("shell-view touch should lower to Query descriptor");

...
fn current_head_runtime_executes_connected_wire_split_program() {
    ...
    let mut workspace =
        topology_runtime(adapters, ".current-head.query-mutation.split-wire").expect("workspace");
    let surfaces =
        crate::projection::runtime_boundary::declared_query_surfaces::declare_topology_query_surfaces(
            &mut workspace,
        )
        .expect("declare surfaces");
    let wire_key = CreateKey::new(".current-head.query-mutation.split-wire.new_wire");
    let declaration = TopologySplitConnectedHalfEdgeSetToNewWireDeclaration::new(
        wire_key.as_str(),
        vec![
            TopologyWireSplitHalfEdgeMember::new(
                ".current-head.query-mutation.split-wire.new-wire-owns-half-edge-1",
                moved_half_edge_ids[0],
            ),
            TopologyWireSplitHalfEdgeMember::new(
                ".current-head.query-mutation.split-wire.new-wire-owns-half-edge-2",
...
fn current_head_runtime_denies_wire_split_when_moved_subset_is_disconnected() {
    ...
    let surfaces =
        ...
        .expect("declare surfaces");
    let wire_key = CreateKey::new(".current-head.query-mutation.split-wire-disconnected.new_wire");
    let declaration = TopologySplitConnectedHalfEdgeSetToNewWireDeclaration::new(
        wire_key.as_str(),
        vec![
            TopologyWireSplitHalfEdgeMember::new(
                ".current-head.query-mutation.split-wire-disconnected.new-wire-owns-half-edge-1",
                half_edge_ids[0],
            ),
            TopologyWireSplitHalfEdgeMember::new(
                ".current-head.query-mutation.split-wire-disconnected.new-wire-owns-half-edge-2",
                half_edge_ids[2],
            ),
        ],
    );

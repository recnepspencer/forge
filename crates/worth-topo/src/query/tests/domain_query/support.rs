...
use crate::query::{
    topology_runtime, TopologyQueryAssembly, TopologyQueryRowLookup, TopologyRuntimeAdapters,
};
...
pub(in crate::query::tests) fn seeded_sheet_disk_workspace(
    stem: &str,
) -> (
    ForgeQueryWorkspace,
...
pub(in crate::query::tests) fn default_query_mutation_evidence(
...
pub(in crate::query::tests) fn snapshot_basis_workspace(
...
pub(in crate::query::tests) struct QueryLookupRows {
    entity_rows: Vec<ForgeQueryEntity>,
    relation_rows: Vec<ForgeQueryEntity>,
}

impl QueryLookupRows {
    pub(in crate::query::tests) fn lookup(&self) -> TopologyQueryRowLookup<'_> {
        TopologyQueryRowLookup::new(&self.entity_rows, &self.relation_rows)
    }
}

pub(in crate::query::tests) fn current_lookup_rows(

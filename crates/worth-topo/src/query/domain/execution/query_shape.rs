...
pub(in crate::query::domain) const TOPOLOGY_ENTITY_ROOT: &str = "TopologyEntity";
...
pub(in crate::query::domain) fn identity_ordering(
) -> Result<OrderingSelector, TopologyDomainQueryError> {
    OrderingSelector::ascending(IDENTITY_ASPECT, IDENTITY_FIELD).map_err(|error| {
        TopologyDomainQueryError::canonical_lowering_resolution(format!("{error:?}"))
    })
}

pub(in crate::query::domain) fn identity_result_field(
) -> Result<AuthoredResultShapeField, TopologyDomainQueryError> {
    result_field(IDENTITY_ASPECT, IDENTITY_FIELD, IDENTITY_FIELD)
}

pub(in crate::query::domain) fn topology_kind_result_field(
) -> Result<AuthoredResultShapeField, TopologyDomainQueryError> {

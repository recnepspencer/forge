...
const QUERY_MOD_RS: &str = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/query/mod.rs"));
const QUERY_ASSEMBLY_MOD_RS: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/src/query/assembly/mod.rs"
));
...
fn topology_query_source_comments_do_not_keep_naming_purge_artifacts() {
    for source in [LIB_RS, FACADE_RS, QUERY_MOD_RS, QUERY_ASSEMBLY_MOD_RS] {

...
#[test]
fn current_head_runtime_executes_single_face_two_face_shell_split_workflow() {
    let mut runtime = build_worth_milestone_one_runtime().expect("worth runtime");
    let verified = seed_milestone_one_primitive(
        &mut runtime,
        "worth.current-head.query-edit.split-shell",
    ...
    let mut workspace =
        worth_topology_runtime(adapters, "worth.current-head.query-edit.split-shell")
            .expect("workspace");
    let assembly = WorthTopologyQueryAssembly::declare(&mut workspace).expect("declare assembly");
    let shell_key = WorthCreateKey::new("worth.current-head.query-edit.split-shell.new_shell");
    let batch = WorthTopologyEditBatch::new(vec![
        ...
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell.region-owns-shell",
            WorthShellOrWireMembershipKind::RegionOwnsShell,
            region,
            created_ref(shell_key.as_str()),
        ),
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell.shell-owns-face",
...
fn current_head_runtime_denies_shell_split_when_old_shell_owns_more_than_two_faces() {
    ...
    let verified = seed_milestone_one_primitive(
        &mut runtime,
        "worth.current-head.query-edit.split-shell-large",
    ...
    let adapters = WorthTopologyRuntimeAdapters::current_head(runtime);
    let mut workspace =
        worth_topology_runtime(adapters, "worth.current-head.query-edit.split-shell-large")
            .expect("workspace");
    let assembly = WorthTopologyQueryAssembly::declare(&mut workspace).expect("declare assembly");
    let shell_key =
        WorthCreateKey::new("worth.current-head.query-edit.split-shell-large.new_shell");
    let batch = WorthTopologyEditBatch::new(vec![
        WorthTopologyEditContract::create_topology_entity(
            shell_key.as_str(),
            WorthTopologyEntityKind::Shell,
        ...
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell-large.region-owns-shell",
            WorthShellOrWireMembershipKind::RegionOwnsShell,
            region,
            created_ref(shell_key.as_str()),
        ),
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell-large.shell-owns-face",
            WorthShellOrWireMembershipKind::ShellOwnsFace,
            created_ref(shell_key.as_str()),
            face_ids[0],
        ),
...
fn current_head_runtime_denies_shell_split_when_created_shell_keys_diverge() {
    ...
    let verified = seed_milestone_one_primitive(
        &mut runtime,
        "worth.current-head.query-edit.split-shell-diverged-key",
        &WorthMilestoneOnePrimitiveCase::SheetPatch { face_count: 2 },
    )
    .expect("seed topology");
    let (region, _shell, face_ids) =
    ...
    let region_shell_key =
        WorthCreateKey::new("worth.current-head.query-edit.split-shell-diverged-key.region");
    let face_shell_key =
        WorthCreateKey::new("worth.current-head.query-edit.split-shell-diverged-key.face");
    let batch = WorthTopologyEditBatch::new(vec![
        WorthTopologyEditContract::create_topology_entity(
        ...
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell-diverged-key.region-owns-shell",
            WorthShellOrWireMembershipKind::RegionOwnsShell,
            region,
            created_ref(region_shell_key.as_str()),
        ),
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell-diverged-key.shell-owns-face",
            WorthShellOrWireMembershipKind::ShellOwnsFace,
...
fn current_head_runtime_denies_shell_split_when_region_does_not_own_source_shell() {
    ...
    let verified = seed_milestone_one_primitive(
        &mut runtime,
        "worth.current-head.query-edit.split-shell-wrong-region",
        &WorthMilestoneOnePrimitiveCase::SheetPatch { face_count: 2 },
    )
    .expect("seed topology");
    ...
    let wrong_region = seed_minimal_topology(
        &mut runtime,
        "worth.current-head.query-edit.split-shell-wrong-region.other",
    )
    .expect("extra seed")
    .region;
    let adapters = WorthTopologyRuntimeAdapters::current_head(runtime);
    ...
    let assembly = WorthTopologyQueryAssembly::declare(&mut workspace).expect("declare assembly");
    let shell_key =
        WorthCreateKey::new("worth.current-head.query-edit.split-shell-wrong-region.new");
    let batch = WorthTopologyEditBatch::new(vec![
        WorthTopologyEditContract::create_topology_entity(
            shell_key.as_str(),
            WorthTopologyEntityKind::Shell,
        ...
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell-wrong-region.region-owns-shell",
            WorthShellOrWireMembershipKind::RegionOwnsShell,
            wrong_region,
            created_ref(shell_key.as_str()),
        ),
        WorthTopologyEditContract::attach_shell_or_wire_membership(
            "worth.current-head.query-edit.split-shell-wrong-region.shell-owns-face",
            WorthShellOrWireMembershipKind::ShellOwnsFace,
            created_ref(shell_key.as_str()),
            face_ids[0],
        ),

...
const DIRECT_VALIDATION_SURVIVOR: &str =
    "src/projection/planner_owned_routing/diagnostic_projection_input/source.rs";
...
pub(crate) fn certify_derived_validation_rehome_row(
    ...
    let diagnostic_surface =
        source_text("src/projection/planner_owned_routing/diagnostic_projection_input/source.rs")?;
    let report_types = source_text(
        "src/projection/planner_owned_routing/diagnostic_projection_input/report_types.rs",
    )?;
    ...
    ensure(report_types.contains("DerivedValidationExecutionReport"))?;
    ...
    closed_row(
        ...
        Some("src/projection/planner_owned_routing/diagnostic_projection_input/source.rs"),
        [
            "src/projection/planner_owned_routing/diagnostic_projection_input/source.rs",
            "src/projection/planner_owned_routing/diagnostic_projection_input/report_types.rs",
...
fn covered_validation_rehome_sources() -> Result<Vec<(String, String)>, TopologyCertificationError>
{
    ...
    sources.extend(collect_rs_sources_recursive(
        "src/projection/planner_owned_routing/diagnostic_projection_input",
    )?);
    sources.extend(collect_rs_sources_recursive(

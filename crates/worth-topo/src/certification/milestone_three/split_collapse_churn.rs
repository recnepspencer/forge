    ...
    F: FnMut() -> RelationalRuntime,
{
    ...
    Ok(MilestoneThreeHostileScenarioReport {
        ...
        detail: format!(
            "connected wire split and owner-retiring collapse churn both executed through query-native membership workflows with continuity `{continuity_outcome_class:?}`"
        ),
    })
}
...
fn wire_identity(
    materialized: &crate::materialization::MaterializedTopologyView,
    wire_id: EntityId,
    description: &str,

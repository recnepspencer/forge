...
use super::hostile_category_requirements::milestone_three_expected_primitive_family_labels;
...
use super::report::MilestoneThreeHostileSuiteReport;
use super::shared::{
...
pub(super) fn certify_milestone_three_primitive_family_closure_impl<F>(
...
pub(super) fn ensure_primitive_family_closure_rows(
...
pub(super) fn primitive_family_closure_labels(
    report: &MilestoneThreeHostileSuiteReport,
...
struct PrimitiveClosureExecution {
    ...
    F: FnMut() -> RelationalRuntime,
{
    ...
    let neighborhood = domain_query
        .local_rewire_neighborhood(&mut workspace, &moved_half_edge_identity, case.cycle_depth)
        .map_err(|error| TopologyCertificationError::Query(error.to_string()))?;
    let chosen_successor_identity = neighborhood
        .cycle_identities()
        .get(case.candidate_offset)
        .cloned()
        .ok_or_else(|| {
...
fn direct_primitive_closure_family_labels() -> &'static [&'static str] {
    &["WireClosed(n)", "SheetPatch(f)", "SolidShell(f)"]
}
...
fn primitive_family_closure_error(reason: &str) -> TopologyCertificationError {

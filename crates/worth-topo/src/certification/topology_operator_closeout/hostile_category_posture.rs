...
use super::edited_query_traversal::required_edited_query_traversal_views;
...
use super::primitive_family_closure::primitive_family_closure_labels;
use super::report::{
    MilestoneThreeHostileOutcomeClass, MilestoneThreeHostileScenario,
    MilestoneThreeHostileSuiteReport,
};

pub(super) fn build_hostile_certification_category_rows(
...
pub(super) fn ensure_hostile_certification_category_rows(
    report: &MilestoneThreeHostileSuiteReport,
...
fn operator_brutality_row(
    ...
    category_row(
        ...
        MilestoneThreeHostileCertificationStatus::Certified,
        report.family_coverage_rows.len(),
        report
            .family_coverage_rows
            .iter()
            .map(|row| format!("edit_family={:?}", row.family))
            .collect(),
        Vec::new(),
        replay_verified_count(report),
        report.failure_locality_rows.len(),
...
fn scale_depth_sustained_pressure_row(
    ...
    category_row(
        ...
        report
            ...
            .iter()
            .map(|row| format!("breadth_counter={}", row.scenario.as_str()))
            .collect(),
        vec![
            "missing_scale_sweep=high_cardinality_loops".to_string(),
            "missing_scale_sweep=high_face_count_shells".to_string(),
            "missing_scale_sweep=large_branch_local_histories".to_string(),
        ],
        replay_verified_count(report),
        report.failure_locality_rows.len(),
    )

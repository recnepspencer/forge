...
fn hostile_suite_report_gate_shape_is_complete() {
    ...
    assert_eq!(report.topology_mutation_digest_rows.len(), 5);
    assert_eq!(report.naming_mutation_continuity_matrix_rows.len(), 5);
    assert_eq!(report.naming_continuity_breadth_rows.len(), 5);
    assert_eq!(report.mutation_replay_parity_rows.len(), 5);
    ...
    assert_eq!(report.rejected_mutation_scope_report_rows.len(), 2);
    assert_eq!(report.determinism_rule_rows.len(), 13);
    assert_eq!(report.mutation_breadth_counter_rows.len(), 5);
    assert_eq!(report.mutation_fallout_breadth_rows.len(), 5);

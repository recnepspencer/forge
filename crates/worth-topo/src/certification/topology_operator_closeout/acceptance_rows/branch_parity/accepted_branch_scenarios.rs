    ...
    F: FnMut() -> RelationalRuntime,
{
    ...
    let mut branch = create_branch(&mut runtime, stem, "accepted.cancellation")?;
    ...
    let verified_one =
        apply_branch_declaration(&mut runtime, &mut branch, create_inner_loop.clone())?;
    ...
    let verified_two =
        apply_branch_declaration(&mut runtime, &mut branch, detach_inner_loop.clone())?;
    let retire_loop =
        TopologyRetireTopologyEntityDeclaration::new(loop_id, TopologyEntityKind::Loop);
    let verified_three = apply_branch_declaration(&mut runtime, &mut branch, retire_loop.clone())?;
    ...
    F: FnMut() -> RelationalRuntime,
{
    ...
    let mut branch = create_branch(&mut runtime, stem, "accepted.split_collapse")?;
    ...
    let verified_split = apply_branch_plan(&mut runtime, &mut branch, &split_plan)?;
    ...
    let verified_collapse = apply_branch_plan(&mut runtime, &mut branch, &collapse_plan)?;
    execution_from_verified_plans(&runtime, branch, vec![verified_split, verified_collapse])
}
    ...
    F: FnMut() -> RelationalRuntime,
{
    ...
    let mut branch = create_branch(&mut runtime, stem, "accepted.ambiguous_rewire")?;
    let plan =
        authority_expanded_rewire_plan(declaration, &format!("{stem}.branch_local.ambiguous"))?;
    let verified = apply_branch_plan(&mut runtime, &mut branch, &plan)?;
    execution_from_verified_plans(&runtime, branch, vec![verified])
}

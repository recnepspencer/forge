...
pub(in crate::certification::topology_operator_closeout) fn ensure_branch_local_edit_parity_rows(
...
fn ensure_accepted_branch_local_edit_parity_row(
    ...
    {
        ensure_accepted_branch_local_edit_parity_scenario(report, *scenario)?;
    }
    Ok(())
}

fn ensure_accepted_branch_local_edit_parity_scenario(
    report: &MilestoneThreeHostileSuiteReport,
    ...
    let accepted_verified = report.edit_branch_local_parity_rows.iter().any(|row| {
        row.scenario == Some(scenario)
            ...
            && row.edit_families == scenario_report.edit_families
            && edit_digest_shape_matches(
                &row.topology_edit_digest,
                &scenario_report.topology_edit_digest,
            )
            && row.naming_edit_continuity_matrix == scenario_report.naming_edit_continuity_matrix
    ...
    if accepted_verified {
        ...
        Err(closeout_requirement_error(&format!(
            "missing accepted branch-local topology edit parity row for {}",
            scenario.as_str()
        )))
    }
}
...
mod tests {
    ...
    use super::ensure_branch_local_edit_parity_rows;
    ...
    fn accepted_branch_local_gate_rejects_missing_authority_projection_marker() {
        ...
        assert!(
            ensure_branch_local_edit_parity_rows(&report).is_err(),

...
pub(crate) fn successor_candidate_with_retained_predecessor(
...
pub(crate) fn successor_relocation_declaration(
    neighborhood: &TopologyLocalRewireNeighborhoodView,
    new_successor_identity: &str,
) -> Result<TopologyRewireLoopSuccessorProgramDeclaration, String> {
...
) -> Result<&'a TopologyLoopNeighborEvidence, String> {
    neighborhood
        .cycle_half_edges()
        .iter()
...
fn entity_id_from_query_identity(
    identity: &str,
) -> Result<forge_relational::facade::identity::EntityId, String> {
...
fn relation_id_from_query_identity_label(
    identity: &str,
) -> Result<forge_relational::facade::identity::RelationId, String> {
...
fn query_identity_parts(identity: &str, expected_kind: &str) -> Result<[u64; 3], String> {
    ...
    if parts.next().is_some() {
        return Err(format!("query identity `{identity}` had too many fields"));
    }
...
fn parse_query_identity_part(
    ...
    part.ok_or_else(|| format!("query identity `{identity}` is missing {label}"))?
        .parse()
        .map_err(|_| format!("query identity `{identity}` has invalid {label}"))
}

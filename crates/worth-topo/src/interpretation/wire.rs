...
pub fn interpret_wires(view: &WorthTopologyView) -> Vec<WorthWireInterpretation> {
    ...
    view.wires
        .iter()
        .map(|wire| {
            let components = connected_components(wire.half_edge_ids.iter().copied().collect(), &half_edge_map);
            let degree_map = vertex_degree_map(wire.half_edge_ids.iter().copied().collect(), &half_edge_map);
                ...
                .filter(|candidate_id| {
                    half_edge_map
                        ...
                        .map(|candidate| {
                            ...
                            current_vertices.iter().any(|vertex_id| candidate_vertices.contains(vertex_id))
                        })
                        .unwrap_or(false)
                })
                .collect();

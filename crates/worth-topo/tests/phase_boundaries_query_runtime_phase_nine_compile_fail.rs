#[test]
fn query_runtime_phase_nine_compile_fail_boundaries_hold() {
    let workspace_temp = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("target")
        .join("trybuild-query-runtime-phase-nine");
    ...
    for golden in topology::facade::topology_query_runtime_phase_nine_golden_paths() {
        t.pass(golden.path());
    }
...
fn assert_forbidden_patterns_absent(path: &std::path::Path, relative_path: &str) {
    ...
    for pattern in topology::facade::PHASE_NINE_FORBIDDEN_SUBSTITUTION_PATTERNS {
        assert!(

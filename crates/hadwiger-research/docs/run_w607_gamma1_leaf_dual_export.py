...
import run_w607_gamma1_branch_tree_preflight as tree
import run_w607_v304_projected_parent_lift_diagnostic as parent


CRATE = parent.CRATE
TREE = CRATE / "docs" / "w607-gamma1-branch-tree-preflight.json"
EXCLUDE_CERT = CRATE / "docs" / "w607-v304-exclude-dual-cover-den1024.json"
OUT_PATH = CRATE / "docs" / "w607-gamma1-leaf-dual-export.json"
...
def main():
    ...
    for leaf_index, leaf in enumerate(leaves):
        included = tuple(sorted([BRANCH, *(v - 1 for v in leaf["included"])]))
        ...
        leaf_reports.append(
            {
                ...
                "included": [v + 1 for v in included if v != BRANCH],
                "base_included": BRANCH + 1,
    ...
    status = "FundGamma1LeafDualReplay" if len(successes) == len(leaf_reports) else "RetireGamma1LeafDualRounding"
    report = clean(
        {
            "schema": "forge.hadwiger.w607_gamma1_leaf_dual_export.v1",
            ...
            "branch_domain": "x304=1_residual",
            "base_included_vertex": BRANCH + 1,

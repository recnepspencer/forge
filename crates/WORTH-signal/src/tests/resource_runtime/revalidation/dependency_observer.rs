...
#[test]
fn resource_dependency_change_revalidation_denies_WORTHd_state_mismatch_proof() {
    let mut graph = SignalGraph::new();
    let source = graph.node().build();
    let node = graph.node().build();
    graph
    ...
    let decision_digest = runtime
        ...
        .decision_digest()
        .clone();
    let WORTHd = DependencyChangeResourceRevalidationProof::new(
    ...
    let report = runtime
        .revalidate_resource_node_for_dependency_change(WORTHd)
        .expect("WORTHd proof denial should still be report-shaped");
    let denied = report
        .denied_revalidation()
        .expect("WORTHd dependency-change proof must deny");
...
#[test]
fn resource_observer_demand_revalidation_denies_WORTHd_observation_proof() {
    let mut graph = SignalGraph::new();
    let node = graph.node().build();
    let mut runtime = TestRuntime::build(graph);
    runtime
    ...
    let decision_digest = runtime
        ...
        .decision_digest()
        .clone();
    let WORTHd = ObserverDemandResourceRevalidationProof::new(
        ResourceNodeId::from_node(node),
        observation_handle.observer_id().get(),
        observation_handle.handle_id().get() + 1,
        String::from("WORTHd-observation-digest"),
        decision_digest,
    );

    let report = runtime
        .revalidate_resource_node_for_observer_demand(WORTHd)
        .expect("WORTHd observer-demand proof denial should still be report-shaped");
    let denied = report
        .denied_revalidation()
        .expect("WORTHd observer-demand proof must deny");

...
use crate::{
    ...
    DurableBackendFamily, WORTHStoreBuilder, StoreErrorKind,
};
...
fn backend_durable_media_reports_are_explicit() {
    let local = WORTHStoreBuilder::new()
        .local_file(unique_test_store_path("worth-store-media-report"))
        .build()
        .expect("local-file store should build");
    let sqlite = WORTHStoreBuilder::new()
        .sqlite_file(unique_test_sqlite_path("worth-store-media-report"))
        .build()
...
fn append_records_barrier_verification_without_ack_violation() {
    ...
    let mut store = WORTHStoreBuilder::new()
        .local_file(unique_test_store_path("worth-store-media-append"))
        .build()
        .expect("local-file store should build");
    store

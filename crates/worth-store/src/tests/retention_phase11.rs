...
use crate::{
    ...
    WORTHStore, WORTHStoreBuilder, PinnedSnapshotPolicy, RetentionPolicyClass,
...
impl DurableLaneCase {
    ...
    fn build(&self) -> WORTHStore {
        match self {
            Self::LocalFile(path) => WORTHStoreBuilder::new()
                .local_file(path.clone())
                .build()
                .unwrap(),
            Self::Sqlite(path) => WORTHStoreBuilder::new()

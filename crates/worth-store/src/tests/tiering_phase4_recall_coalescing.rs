...
fn tiering_phase4_fixture() -> (WORTHStore, u64) {
...
fn tiering_phase4_sqlite_fixture(path: PathBuf) -> (WORTHStore, u64) {
    let mut runtime = runtime_with_demo_schema();
    create_entity(&mut runtime, "alpha");
    let envelope = latest_envelope(&runtime);
    let branch_id = envelope.branch_context.clone();
...
fn sqlite_in_flight_recall_state_resumes_after_reopen() {
    let path = unique_test_sqlite_path("worth-store-tiering-phase4-recall");
...
fn sqlite_completed_recall_state_fails_on_open() {
    let path = unique_test_sqlite_path("worth-store-tiering-phase4-completed-recall");
    let (store, snapshot_id) = tiering_phase4_sqlite_fixture(path.clone());
    drop(store);

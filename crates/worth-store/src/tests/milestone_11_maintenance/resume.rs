...
fn started_maintenance_can_resume_after_restart_in_both_durable_lanes() {
    let local_path = unique_test_store_path("worth-store-m11-maintenance-resume-local");
    ...
    let sqlite_path = unique_test_sqlite_path("worth-store-m11-maintenance-resume-sqlite");
...
fn resumed_started_work_preserves_escalated_plan_posture() {
    let path = unique_test_store_path("worth-store-m11-maintenance-resume-escalated-local");

use crate::{
    ...
    WORTHStore, WORTHStoreBuilder, MaxBatchItems, MaxCoveredCommits, MaxMaterializedBytes,
...
fn foreground_read_during_inflight_branch_move_exposes_transfer_observation() {
    let (mut store, branch_id, first_commit_id, _) =
        build_interleaving_store(WORTHStoreBuilder::new().in_memory());
    let basis = store
...
fn reopened_store_preserves_inflight_read_interleaving_observation() {
    let path = unique_test_sqlite_path("worth-store-tiering-phase5-interleaving");
    let (mut store, branch_id, first_commit_id, _) =
        build_interleaving_store(WORTHStoreBuilder::new().sqlite_file(path.clone()));
    let basis = store
        .read_stable_basis(stable_basis_request_for_store(
            &store,
            branch_id.clone(),

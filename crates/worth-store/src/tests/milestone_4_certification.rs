...
fn milestone_4_suite() -> CertificationSuite<String, String> {
    ...
    let path = unique_test_sqlite_path("worth-store-m4-snapshot-corruption");
    {
...
fn sqlite_missing_snapshot_image_fails_with_publication_gap_on_reopen() {
    let path = unique_test_sqlite_path("worth-store-m4-snapshot-gap");
    {
        let mut store = WORTHStoreBuilder::new()
            .sqlite_file(path.clone())
            .build()
...
fn sqlite_snapshot_version_mismatch_fails_typed_on_reopen() {
    let path = unique_test_sqlite_path("worth-store-m4-snapshot-version");
    {
        let mut store = WORTHStoreBuilder::new()
            .sqlite_file(path.clone())
            .build()

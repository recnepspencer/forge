...
use crate::{
    ...
    WORTHStore, WORTHStoreBuilder, PlacementBoundArtifactRef, PlacementExecutionOrigin,
...
fn tiering_phase3_fixture() -> (WORTHStore, BranchId, CommitId, u64, String) {
...
fn tiering_phase3_local_fixture(path: PathBuf) -> (WORTHStore, BranchId, CommitId, u64) {
    let mut runtime = runtime_with_demo_schema();
    create_entity(&mut runtime, "alpha");
    let envelope = latest_envelope(&runtime);
    let branch_id = envelope.branch_context.clone();
...
fn tiering_phase3_sqlite_fixture(path: PathBuf) -> (WORTHStore, BranchId, CommitId, u64, String) {
    let mut runtime = runtime_with_demo_schema();
    create_entity(&mut runtime, "alpha");
    let envelope = latest_envelope(&runtime);
    let branch_id = envelope.branch_context.clone();

use crate::{
    ...
    ComplexityStatus, ConservativeRetentionPolicy, DerivedFamilyRetentionPolicy, WORTHStore,
    WORTHStoreBuilder, RetentionPolicyClass, SingleEntityAspectScope,
};
...
) -> (WORTHStore, AspectLayoutReadRequest) {
...
fn milestone_10_certification_bundle_reports_clean_verified_loop() {
    let (mut store, request) = store_with_materialized_layout(WORTHStoreBuilder::new().in_memory());
...
fn milestone_10_certification_bundle_marks_uncut_compaction_after_restart() {
    let path = unique_test_store_path("worth-store-m10-uncut-compaction");
    let (mut store, _request) =
        store_with_materialized_layout(WORTHStoreBuilder::new().local_file(path.clone()));
    let control = store.export_authoritative_records();
...
fn milestone_10_certification_bundle_tracks_rebuild_debt_across_sqlite_restart() {
    let path = unique_test_sqlite_path("worth-store-m10-rebuild-debt");
    let (mut store, request) =
        store_with_materialized_layout(WORTHStoreBuilder::new().sqlite_file(path.clone()));
    let control = store.export_authoritative_records();

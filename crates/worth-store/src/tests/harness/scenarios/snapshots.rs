...
use crate::{WORTHStore, SnapshotCaptureRequest, SnapshotReadRequest, SnapshotRestoreRequest};
...
pub fn snapshot_restore_equivalence_run(lane: StoreLane) -> SnapshotParityRun {
    let mut store = build_store_for_lane(lane, "worth-store-harness-snapshot");

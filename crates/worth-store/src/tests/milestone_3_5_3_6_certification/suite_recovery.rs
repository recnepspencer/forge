...
fn milestone_3_6_suite() -> CertificationSuite<String, String> {
    ...
    let mut snapshot_store = WORTHStoreBuilder::new().in_memory().build().unwrap();
    ...
    let path = unique_test_store_path("worth-store-m36-quiescent-certification");
    ...
    durable
        ...
        .unwrap();
    drop(durable);
    let _first = WORTHStoreBuilder::new()
        .local_file(path.clone())
        .durable_mode(runtime_with_demo_schema())
        .build_pending()
        .unwrap()
        .recover()
        .unwrap();
    let second = WORTHStoreBuilder::new()
        .local_file(path)
        .durable_mode(runtime_with_demo_schema())
        .build_pending()
        .unwrap()
    ...
    let support_gap_local = {
        let path = unique_test_store_path("worth-store-m36-support-gap-local");
        let mut runtime = runtime_with_demo_schema();
    ...
    let support_gap_sqlite = {
        let path = unique_test_sqlite_path("worth-store-m36-support-gap-sqlite");
        let mut runtime = runtime_with_demo_schema();
        create_entity(&mut runtime, "alpha");
        let envelope = latest_envelope(&runtime);
        let mut store = WORTHStoreBuilder::new()

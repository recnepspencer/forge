use crate::data::graph::SignalGraph;

use super::super::execution::StageSlice;
use super::super::types::StageExecutor;
#[cfg(feature = "parallel")]
use super::super::types::{ParallelAdmissionReason, ParallelExecutionKind};

#[cfg(feature = "parallel")]
#[derive(Debug, Clone, Copy)]
pub(crate) struct StageParallelAdmission {
    pub(crate) use_parallel: bool,
    pub(crate) reason: ParallelAdmissionReason,
    pub(crate) kind: Option<ParallelExecutionKind>,
}

#[cfg(feature = "parallel")]
pub(crate) fn decide_stage_parallel_admission(
    graph: &SignalGraph,
    stage: &StageSlice<'_>,
    executor: StageExecutor,
) -> StageParallelAdmission {
    let Some(parallel_policy) = executor.parallel_policy() else {
        return StageParallelAdmission {
            use_parallel: false,
            reason: ParallelAdmissionReason::SerialExecutor,
            kind: None,
        };
    };
    if executor.is_full_parallel() {
        return StageParallelAdmission {
            use_parallel: false,
            reason: ParallelAdmissionReason::FullParallelUnsupportedByMutableEngine,
            kind: None,
        };
    }
    let stage_width = stage.tasks.len();
    if stage_width < parallel_policy.min_stage_width.get() {
        return StageParallelAdmission {
            use_parallel: false,
            reason: ParallelAdmissionReason::BelowMinStageWidth,
            kind: None,
        };
    }
    let installed_policy = graph.installed_runtime_policy();
    let effective_parallel_threshold = installed_policy.parallel_min_tasks();
    if stage_width < effective_parallel_threshold {
        return StageParallelAdmission {
            use_parallel: false,
            reason: ParallelAdmissionReason::BelowPolicyWorkThreshold,
            kind: None,
        };
    }
    let compute_pressure = stage
        .tasks
        .iter()
        .filter(|task| {
            !matches!(
                task.reason,
                super::super::types::TaskReason::MaybeStaleValidation
                    | super::super::types::TaskReason::MemoValidation
            )
        })
        .count();
    if compute_pressure.saturating_mul(2) < stage_width
        && stage_width < effective_parallel_threshold.saturating_mul(2)
    {
        return StageParallelAdmission {
            use_parallel: false,
            reason: ParallelAdmissionReason::ValidationHeavyStage,
            kind: None,
        };
    }
    if executor.is_full_parallel() && stage_width < installed_policy.full_parallel_min_tasks() {
        return StageParallelAdmission {
            use_parallel: false,
            reason: ParallelAdmissionReason::BelowFullParallelThreshold,
            kind: None,
        };
    }
    StageParallelAdmission {
        use_parallel: true,
        reason: match installed_policy.execution_objective() {
            worth_foundational::ExecutionObjectiveProfile::Throughput => {
                ParallelAdmissionReason::AdmittedThroughput
            }
            worth_foundational::ExecutionObjectiveProfile::Balanced => {
                ParallelAdmissionReason::AdmittedBalanced
            }
            worth_foundational::ExecutionObjectiveProfile::LatencyBounded => {
                ParallelAdmissionReason::AdmittedLatencyBounded
            }
        },
        kind: executor.parallel_kind(),
    }
}

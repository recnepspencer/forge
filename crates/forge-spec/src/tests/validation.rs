...
fn duplicate_single_cardinality_relation_is_rejected() {
    ...
    let he_a = draft
        .create_node(SpecNodeKind::HalfEdge, None, "he-a")
        .unwrap();
    let he_b = draft
        .create_node(SpecNodeKind::HalfEdge, None, "he-b")
        .unwrap();
    draft
        .add_relation(
...
fn missing_required_halfedge_relations_fail_commit_validation() {
    ...
    let mut draft = state.into_draft();
    let _ = draft
        .create_node(SpecNodeKind::HalfEdge, None, "dangling-he")
        .unwrap();
...
fn invalid_relation_kinds_are_rejected() {
    ...
    let vertex = draft
        .create_node(SpecNodeKind::Vertex, None, "vertex")
        .unwrap();
    let err = draft
...
fn ordered_face_inner_loop_ordinals_must_be_unique() {
    ...
    let outer = draft
        .create_node(SpecNodeKind::Loop, None, "outer")
        .unwrap();
    let inner_a = draft
        .create_node(SpecNodeKind::Loop, None, "inner-a")
        .unwrap();
    let inner_b = draft
        .create_node(SpecNodeKind::Loop, None, "inner-b")
        .unwrap();
    let he_outer = draft
        .create_node(SpecNodeKind::HalfEdge, None, "outer-he")
        .unwrap();
    let he_inner_a = draft
        .create_node(SpecNodeKind::HalfEdge, None, "inner-he-a")
        .unwrap();
    let he_inner_b = draft
        .create_node(SpecNodeKind::HalfEdge, None, "inner-he-b")
        .unwrap();
    ...
    for he in [he_outer, he_inner_a, he_inner_b] {
        draft
            .add_relation(RelationKind::HalfEdgeNext, he, he, 0, "next")
            .unwrap();
        draft
            .add_relation(RelationKind::HalfEdgeRadialNext, he, he, 0, "radial")
            .unwrap();
        draft
            .add_relation(RelationKind::HalfEdgeUsesEdge, he, edge, 0, "edge")
            .unwrap();
        draft
            .add_relation(RelationKind::HalfEdgeOriginVertex, he, vertex, 0, "origin")
            .unwrap();
        draft
            .add_relation(RelationKind::HalfEdgeBoundsFace, he, face, 0, "face")
            .unwrap();
    }

    draft
        .add_relation(RelationKind::FaceOuterLoop, face, outer, 0, "outer")
        .unwrap();
    draft
        .add_relation(
            RelationKind::LoopEntryHalfEdge,
    ...
    draft
        ...
        .unwrap();
    draft
        .add_relation(RelationKind::FaceInnerLoop, face, inner_a, 0, "inner-a")
        .unwrap();
    let err = draft

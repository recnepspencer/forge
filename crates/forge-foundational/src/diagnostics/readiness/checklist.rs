...
use super::vocabulary::{
    ...
    FoundationalDiagnosticForgeProofApi, FoundationalDiagnosticForgeProofForbiddenSurface,
    FoundationalDiagnosticForgeProofSurface, FoundationalDiagnosticMilestone6PhaseGate,
...
impl FoundationalDiagnosticProductionReadinessReport {
    ...
    fn has_evidence_for_each_forge_proof_api(&self) -> bool {
        self.forge_proof_api_evidence.len() == self.forge_proof_api_appendix.len()
            && self.forge_proof_api_evidence.iter().all(|evidence| {
                self.forge_proof_api_appendix.contains(&evidence.api())
                    && self
                        .forge_proof_api_evidence
                        .iter()

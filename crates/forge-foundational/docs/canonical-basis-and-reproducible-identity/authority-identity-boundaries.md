...
These types are available through the `forge_foundational` facade. They reuse
`forge_proof::AuthorityWitness` for admission instead of adding a second proof
system.
...
`forge-foundational` does not know runtime semantics. It does not know what a
...
- An external identity token is a host or runtime value before Forge admission.
...
Forge authority. The label can be logged or displayed, but it cannot be passed
...
- Use `forge-proof::AuthorityWitness` for admission. Do not create local proof
...
- Do not add runtime-specific marker names to `forge-foundational`.

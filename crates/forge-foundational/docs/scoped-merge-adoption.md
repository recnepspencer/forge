...
`forge-foundational` now provides the shared vocabulary for scoped merge and
...
This is vocabulary and proof shape, not merge execution. `forge-foundational`
...
3. Execute runtime-specific merge work outside `forge-foundational`.
...
`forge-proof::TransitionOutcome` remains the progression owner:
...
`forge-foundational` gives adopting crates the boundary artifacts they need
before execution:
...
After that, `forge-signal`, `forge-signal-wasm`, or another adopting runtime
...
- Diagnostics consume scoped merge artifacts through the existing diagnostic
  explanation surface.
- Production readiness uses `forge-proof` artifacts with a
...
compile-fail boundaries, residual debt, and `forge-proof` API evidence that
...
- imply that `forge-foundational` executes scoped merge or mutates branch graphs

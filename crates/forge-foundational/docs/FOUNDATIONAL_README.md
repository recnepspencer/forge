# Forge Foundational Orientation For AI Agents
...
1. What kind of problem is `forge-foundational` for?
2. When is `forge-foundational` the right crate, and when is a stronger crate
   such as `forge-query` or a domain runtime the right one instead?
...
Forge Foundational lives in this stack:

```text
application / downstream crate
-> owning runtime or domain crate
   (forge-query, forge-store, forge-signal, forge-relational, etc.)
-> forge-foundational
-> forge-proof
```
...
`forge-proof` owns proof-bearing progression law: witnesses, transitions,
stages, proof carriers, and checked progression topology.

`forge-foundational` owns shared boundary vocabulary: typed values, aspect
...
If `forge-query` already gives you typed projection-consumption facts,
...
Reach for `forge-foundational` when the object becomes one of these:
...
Reach for `forge-proof` when the real problem is progression law rather than
boundary vocabulary.
...
## What Forge Foundational Is Not

This crate is not:

- the public runtime facade for application reads and writes
- a replacement for `forge-query` projection consumption
...
| Need | Use |
| --- | --- |
| Typed facts from Query materialization | `forge-query` projection consumption |
| Query-owned authoritative or derived state | `forge-query` state, inspection, receipts, bindings |
| Domain-local runtime execution state | the owning runtime or domain crate |
| Shared trust-boundary identity | `forge-foundational` identity boundaries |
| Canonical basis, digest, export parity, mismatch classification | `forge-foundational` canonicalization |
| Shared profile or support posture | `forge-foundational` profiles |
| Shared summary/report/artifact/receipt vocabulary | `forge-foundational` boundary artifacts |
| Shared diagnostics or support reports | `forge-foundational` diagnostics |
| Shared provenance, lineage, or support-grade receipt meaning | `forge-foundational` boundary evidence |
| Shared performance claim or report vocabulary | `forge-foundational` performance |
| Proof-bearing progression law or checked phase transitions | `forge-proof` |
...
- if the semantics are now cross-crate boundary meaning, Foundational is
  probably right
- if the semantics are about proof-bearing progression, `forge-proof` is
  probably right
...
- `forge_foundational::canonicalization()`
- `forge_foundational::boundary_artifact_surface_inventory()`
- `forge_foundational::boundary_evidence()`
- `forge_foundational::foundational_branch_candidate()`
- `forge_foundational::foundational_merge(...)`
- `forge_foundational::canonicalization_api`
- `forge_foundational::profiles_api`
- `forge_foundational::boundary_evidence_api`
- `forge_foundational::performance_api`
- `forge_foundational::foundational_transition_milestone5_readiness_report()`
- the facade exports re-exported from `forge_foundational::*`
...
This family gives Forge one shared typed vocabulary for aspect keys, contracts,
...
- teaching app code to start from JSON when native aspect authoring is
  available
- flattening typed compatibility outcomes into one “parse failed” branch
...
That means AI consumers should not collapse “identity” into one bucket just
...
This family is much broader than “pick some profile enums.” It includes:
...
them as random knobs that can be collapsed into one vague “mode.”
...
This family gives Forge one shared language for:
...
The materialization subsystem is also much richer than “turn object into
output.” It makes these things explicit:
...
- delivery and availability are part of the contract, so “deferred,”
  “reconstructable,” and “unavailable” are first-class postures rather than
  prose notes
...
- current-basis strengthening reuses canonicalization and `forge-proof`; it is
...
This family gives Forge one shared language for diagnostic primitives,
...
This family is broader than “receipts and provenance.” It includes:
...
- category, locality, execution posture, descriptive role, and freshness are
  separate axes; they are not one catch-all “evidence status”
- provenance explains basis and freshness, but does not prove execution or
  continuity by itself
- receipts answer “what completed boundary truth do I have?” and still keep
...
- hide stale, reduced, replayed, or rebuilt posture behind generic “available”
  wording
...
This family gives Forge one shared way to talk about performance meaning across
...
This family gives Forge a shared language for transition-like work across
...
- typed non-success topology through `forge_proof::TransitionOutcome`
...
- it depends on `forge-proof` for progression topology instead of inventing a
  second proof kernel
...
- a substitute for `forge-proof` progression law
...
- do not replace `forge-query` typed fact receipts with Foundational types while
...
- do not move progression law into Foundational when `forge-proof` already owns
  it
...
- Start from `forge-query` when the semantics are Query-owned.
- Start from the domain runtime when the semantics are runtime-local.
- Start from `forge-proof` when the semantics are progression law.
- Start from `forge-foundational` when the semantics are shared boundary
  meaning.

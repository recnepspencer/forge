...
let value = vocabulary
    .struct_value()
    .with_field("title", forge_foundational::AspectValue::String("Ship it".into()))
    .with_field("label", forge_foundational::AspectValue::String("origin".into()))
    .with_field("done", forge_foundational::AspectValue::Bool(true))
    .finish()?;
```

What is authoritative here is the struct shape and its field declarations, not
the producer’s local map layout.

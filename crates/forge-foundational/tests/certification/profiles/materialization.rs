...
fn materialized_profile(
    materialized: forge_foundational::FoundationalProfileSet,
) -> MaterializedFoundationalProfileSet {
    let admitted = admit_same_profile(*materialized.admitted_or_self());

    match forge_foundational::materialize_admitted_foundational_profile(
        admitted,
        materialized,
        None,
...
trait MaterializedSeed {
    fn admitted_or_self(&self) -> &forge_foundational::FoundationalProfileSet;
}

impl MaterializedSeed for forge_foundational::FoundationalProfileSet {
    fn admitted_or_self(&self) -> &forge_foundational::FoundationalProfileSet {
        self
    }
}

...
fn transition_locator(id: u64) -> FoundationalTransitionLocator {
    FoundationalTransitionLocator::CommitParentage(FoundationalCommitParentageLocator::new(
        ...
        FoundationalCommitParentBasis::new(forge_foundational::EquivalenceBasisId::new(id + 10)),
    ))
}
...
fn grouped_boundary_evidence_surface_exposes_common_lower_and_stronger_lanes() {
    let _common_root: forge_foundational::BoundaryEvidenceFrontDoor =
        common_path::boundary_evidence();
    let _common_attachment: forge_foundational::FoundationalBoundaryEvidenceAttachmentFrontDoor =
        common_path::attachment();
    let _direct_attachment: forge_foundational::FoundationalBoundaryEvidenceAttachmentFrontDoor =
        attachment();
    assert_eq!(

...
fn accepts_boundary_evidence_readiness_proof(
    _: &forge_proof::Proof<
        FoundationalBoundaryEvidenceProductionReadinessCertified,
        forge_foundational::FoundationalBoundaryEvidenceProductionReadinessAuthority,
    >,
) {
}
...
fn production_readiness_artifact_carries_complete_machine_checkable_inventory() {
    ...
    assert_eq!(
        ...
        "forge-foundational.milestone-7"
    );
...
fn production_readiness_binds_surfaces_to_real_tests_and_exact_public_surface_inventory() {
    ...
    assert!(paths.contains("forge_foundational::boundary_evidence_api::common_path"));
    assert!(paths.contains("forge_foundational::boundary_evidence_api::stronger_lane::readmission"));
    assert!(paths.contains("forge_foundational::boundary_evidence_api::stronger_lane::readiness"));

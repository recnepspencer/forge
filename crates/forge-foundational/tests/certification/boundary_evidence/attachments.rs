...
fn transition_locator(id: u64) -> FoundationalTransitionLocator {
    FoundationalTransitionLocator::CommitParentage(FoundationalCommitParentageLocator::new(
        ...
        FoundationalCommitParentBasis::new(forge_foundational::EquivalenceBasisId::new(id + 100)),
    ))
}

fn provenance() -> forge_foundational::FoundationalBoundaryEvidenceProvenanceArtifact {
    boundary_evidence()
        .provenance()
...
fn receipt() -> forge_foundational::FoundationalBoundaryEvidenceExecutedReceiptArtifact {
    boundary_evidence()
        .receipt()
...
fn attested() -> forge_foundational::FoundationalBoundaryEvidenceAttestedLineageArtifact {
    boundary_evidence()
        .lineage()
...
fn support() -> forge_foundational::FoundationalBoundaryEvidencePublishedSupportArtifact {
    boundary_evidence()
        .support()
        .published_evidence()
...
fn materialized_mixed_bundle_from_common_path(
    locator_id: u64,
) -> forge_foundational::FoundationalMaterializedBoundaryEvidenceAttachmentBundle {
...
fn object_and_locator_continuity_attachments_stay_distinct() {
    ...
    assert_eq!(
        ...
        Some(
            forge_foundational::FoundationalBoundaryEvidenceContinuityAttachmentScope::ObjectLevel
        )
    );
    assert_eq!(
        locator_bundle.continuity_scope(),
        Some(
            forge_foundational::FoundationalBoundaryEvidenceContinuityAttachmentScope::LocatorLevel
        )
    );
}
...
fn diagnostic_bundle_targets_stay_descriptive_and_context_only() {
    ...
    assert_eq!(
        ...
        forge_foundational::FoundationalBoundaryEvidenceAttachmentTargetKind::DiagnosticBundle
    );
    assert_eq!(
...
fn canonical_basis_preparation_for_attachment_bundle_is_boundary_honest() {
    ...
    let basis =
        forge_foundational::prepare_boundary_evidence_attachment_bundle_for_canonical_basis(
            forge_foundational::CanonicalizationRuleVersion::new("m7.phase6.attachment")
                .expect("version"),
            &materialized,
        )
    ...
    assert!(basis.payload().entries().iter().any(
        |entry| entry.kind() == forge_foundational::CanonicalBasisEntryKind::BoundaryAttachment
    ));
}
...
fn stronger_lane_support_readmission_requires_support_attachment_and_boundary_bridge() {
    ...
    assert_eq!(
        denial,
        forge_foundational::FoundationalBoundaryEvidenceSupportReadmissionDenial::SupportAttachmentRequired
    );
...
fn common_lower_and_stronger_lanes_expose_the_same_phase6_surface() {
    ...
    let _front_door: forge_foundational::FoundationalBoundaryEvidenceAttachmentFrontDoor =
        attachment();
    let _common_front_door: forge_foundational::FoundationalBoundaryEvidenceAttachmentFrontDoor =
        common_path::attachment();
    let _authority =
        stronger_lane::readmission::foundational_boundary_evidence_attachment_readmission_authority(
        );
...
fn mixed_family_attachment_bundle_preserves_semantics_across_canonical_and_digest_participation() {
    ...
    let basis =
        forge_foundational::prepare_boundary_evidence_attachment_bundle_for_canonical_basis(
            forge_foundational::CanonicalizationRuleVersion::new("m7.phase6.mixed-bundle")
                .expect("version"),
            &materialized,
        )
        .expect_success("mixed attachment basis");
    let digest = forge_foundational::derive_boundary_evidence_attachment_bundle_digest(
        forge_foundational::CanonicalizationRuleVersion::new("m7.phase6.mixed-bundle")
            .expect("version"),
        &materialized,
...
fn mixed_family_attachment_bundle_is_canonical_across_independent_attachment_orderings() {
    ...
    let common_path_basis =
        forge_foundational::prepare_boundary_evidence_attachment_bundle_for_canonical_basis(
            forge_foundational::CanonicalizationRuleVersion::new("m7.phase6.ordering-hostility")
                .expect("version"),
    ...
    let common_path_digest = forge_foundational::derive_boundary_evidence_attachment_bundle_digest(
        forge_foundational::CanonicalizationRuleVersion::new("m7.phase6.ordering-hostility")
            .expect("version"),
        &common_path_materialized,
        CanonicalDigestAlgorithmId::test_stable_fixture(),
    ...
    let direct_path_digest = forge_foundational::derive_boundary_evidence_attachment_bundle_digest(
        forge_foundational::CanonicalizationRuleVersion::new("m7.phase6.ordering-hostility")
            .expect("version"),
        &direct_path_materialized,
        CanonicalDigestAlgorithmId::test_stable_fixture(),

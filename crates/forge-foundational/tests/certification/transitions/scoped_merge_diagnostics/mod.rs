...
fn denial_and_unavailable_explanations_attach_provenance_ready_scope_rows() {
    let denied = FoundationalScopedMergeDenialEvidence::new(
        ...
        forge_foundational::FoundationalDeniedScopeLocus::Node(selected_node("gear")),
    )
    .expect("denial");
    ...
    assert_eq!(
        ...
        forge_foundational::FoundationalDiagnosticOutcomeKind::Denied
    );
    assert!(row_codes(denied_bundle.rows()).contains(&"merge-scope.denied-origin"));
    assert_eq!(
        unavailable_bundle.outcome_kind(),
        forge_foundational::FoundationalDiagnosticOutcomeKind::Deferred
    );
...
fn materialize(
    input: forge_foundational::FoundationalDiagnosticExplanationInput,
    richness: DiagnosticRichnessProfile,
) -> forge_foundational::FoundationalDiagnosticExplanationBundle {
...
fn row_codes(rows: &[forge_foundational::FoundationalDiagnosticRow]) -> Vec<&str> {

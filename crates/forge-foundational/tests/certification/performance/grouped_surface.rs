...
fn grouped_performance_surface_exposes_certified_and_readiness_stronger_lanes() {
    ...
    assert!(stronger_lane_entries.iter().any(|entry| {
        entry.path() == "forge_foundational::performance_api::stronger_lane"
            && entry.teaches()
    ...
    assert!(stronger_lane_entries.iter().any(|entry| {
        entry.path() == "forge_foundational::performance_api::stronger_lane::certified"
            && entry.teaches()
    ...
    assert!(stronger_lane_entries.iter().any(|entry| {
        entry.path() == "forge_foundational::performance_api::stronger_lane::readiness"
            && entry.teaches()

...
fn accepts_performance_readiness_proof(
    _: &forge_proof::Proof<
        FoundationalPerformanceProductionReadinessCertified,
        forge_foundational::FoundationalPerformanceProductionReadinessAuthority,
    >,
) {
}
...
fn production_readiness_artifact_carries_complete_machine_checkable_inventory() {
    ...
    assert_eq!(
        ...
        "forge-foundational.milestone-8"
    );
...
fn production_readiness_binds_surfaces_to_real_tests_and_exact_public_surface_inventory() {
    ...
    assert!(paths.contains("forge_foundational::performance_api::common_path"));
    assert!(paths.contains("forge_foundational::performance_api::stronger_lane::certified"));
    assert!(paths.contains("forge_foundational::performance_api::stronger_lane::readiness"));

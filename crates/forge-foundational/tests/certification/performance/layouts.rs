...
fn layout_attachment_rejects_access_pattern_mismatch() {
    let claim = performance()
        .claim()
        .support_derived()
        .boundary(forge_foundational::FoundationalPerformanceBoundary::SupportAssembly)
        .evidence_strength(
            forge_foundational::FoundationalPerformanceEvidenceStrength::SupportDerivedPerformanceClaim,
        )
        .breadth_locality(
            forge_foundational::FoundationalPerformanceBreadthLocalityPosture::FamilyLocalBatch,
        )
        .access_pattern(FoundationalPerformanceAccessPatternPosture::TraversalLocal)
        .execution_temperature(
            forge_foundational::FoundationalPerformanceExecutionTemperature::SupportOnly,
        )
        .freshness_retention(
            forge_foundational::FoundationalPerformanceFreshnessRetentionPosture::StaleSupport,
        )
        .fallback_debt(forge_foundational::FoundationalPerformanceFallbackDebtPosture::Debt)

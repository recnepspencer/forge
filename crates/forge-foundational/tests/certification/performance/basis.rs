...
fn bundle_comparison_covers_full_claim_surface_not_just_boundary_and_freshness() {
    ...
    let right_claim = common
        ...
        .breadth_locality(
            forge_foundational::FoundationalPerformanceBreadthLocalityPosture::BasisLocalBatch,
        )
        .access_pattern(
            forge_foundational::FoundationalPerformanceAccessPatternPosture::TraversalLocal,
        )
        .execution_temperature(
            forge_foundational::FoundationalPerformanceExecutionTemperature::WarmPath,
        )
        .freshness_retention(
            forge_foundational::FoundationalPerformanceFreshnessRetentionPosture::ExactBasisCurrent,
        )
        .fallback_debt(forge_foundational::FoundationalPerformanceFallbackDebtPosture::Deferred)
        .include_work(FoundationalPerformanceWorkClass::AuthoritativeMutation)
        .exclude_work(FoundationalPerformanceWorkClass::SupportReportAssembly)
        .finish()
...
fn counter_backed_receipts_require_exact_rows_and_counter_backed_claims() {
    ...
    assert_eq!(
        mismatch,
        Err(
            forge_foundational::FoundationalCounterBackedPerformanceReceiptConstructionDenial::CounterValueMismatch
        )
    );
}

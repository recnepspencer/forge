...
fn delivery_and_source_seam_legality_remain_explicit_and_fail_closed() {
    assert_eq!(
        forge_foundational::evaluate_boundary_surface_disposition_legality(
            FoundationalBoundaryDeliveryClass::MustBeHot,
            FoundationalBoundaryAvailability::Deferred,
        ),
...
fn coordinated_bundle_materialization_preserves_member_categories_and_membership_rows() {
    ...
    assert_eq!(
        ...
        forge_foundational::FoundationalBoundaryArtifactCategory::Summary
    );
    assert_eq!(
        bundle_plan.report().expect("report").category(),
        forge_foundational::FoundationalBoundaryArtifactCategory::Report
    );
    assert_eq!(
        bundle_plan.receipt().expect("receipt").category(),
        forge_foundational::FoundationalBoundaryArtifactCategory::Receipt
    );
    assert_eq!(
        bundle_plan
    ...
    assert!(bundle_plan
        ...
        .any(|row| row.category()
            == Some(forge_foundational::FoundationalBoundaryArtifactCategory::Summary)));
    assert!(bundle_plan
        .membership_decision_rows()
        .iter()
        .any(|row| row.category()
            == Some(forge_foundational::FoundationalBoundaryArtifactCategory::Report)));
    assert!(bundle_plan
        .membership_decision_rows()
        .iter()
        .any(|row| row.category()
            == Some(forge_foundational::FoundationalBoundaryArtifactCategory::Receipt)));

    let bundle = bundle_plan.materialize().expect("bundle materialized");
    assert_eq!(
        bundle.primary().category(),
        forge_foundational::FoundationalBoundaryArtifactCategory::Artifact
    );
    assert_eq!(
        bundle.source(),
        FoundationalBoundaryMaterializationSource::CompatibilityLowered

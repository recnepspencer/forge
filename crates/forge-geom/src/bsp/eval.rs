    ...
    for plane in &centered_planes {
        ...
        if cell.vertices().is_empty() {
            return Err(KernelError::InvalidInput {
                message: "Plane intersection produced an empty cell".to_string(),
                context: None,
            });
        }
    }
...
///
/// Vertices with `signed_distance < -eps` are kept (inside).
/// Vertices with `signed_distance > eps` are removed (outside).
/// New vertices are created at plane-edge intersections.
pub fn clip_cell_by_plane(
    planes: &[Plane],
    cell: &ConvexCell,
    clip_plane_idx: usize,
    config: &BspConfig,
) -> Result<ConvexCell, KernelError> {
    ...
    for face in cell.faces() {
        ...
        for edge_idx in 0..vert_count {
            ...
            if vi_inside {
                let mapped = old_to_new[vi].ok_or_else(|| KernelError::InternalError {
                    message: "BSP clip: inside vertex missing from old_to_new map".to_string(),
                    context: None,
                })?;
...
fn create_bounding_box(bound: f64) -> Result<Vec<Plane>, KernelError> {
    Ok(vec![
...
fn build_initial_cube(planes: &[Plane], config: &BspConfig) -> Result<ConvexCell, KernelError> {

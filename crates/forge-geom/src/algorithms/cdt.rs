...
//!
//! INVARIANTS:
//! - All orientation tests use `orient2d` from `forge_math` for exact predicates.
...
pub fn triangulate_polygon_2d(
    ...
    constraints: &[[usize; 2]],
    boundary: &[usize],
) -> Result<CdtResult, forge_math::error::MathError> {
...
fn point_in_polygon_2d(
    ...
    vertices: &[[f64; 2]],
    boundary: &[usize],
) -> Result<bool, forge_math::error::MathError> {
    let n = boundary.len();

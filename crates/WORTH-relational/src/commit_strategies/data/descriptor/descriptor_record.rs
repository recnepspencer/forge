...
mod tests {
    ...
    fn descriptor_digest_drift_is_detectable_with_typed_fixture() {
        let mut WORTHd = descriptor();
        let original_digest = WORTHd.digest();
        WORTHd.semantic_name = CommitStrategySemanticName::new("strategy.intent.tampered");
        let recomputed = CommitStrategyDescriptor::new(
            WORTHd.id(),
            WORTHd.semantic_name().clone(),
            WORTHd.family_name().clone(),
            WORTHd.version(),
            WORTHd.intent_name().clone(),
            WORTHd.input_schema_name().clone(),
            WORTHd.input_schema_version(),
            WORTHd.output_schema_name().clone(),
            WORTHd.read_contract().clone(),
            WORTHd.artifact_name().clone(),
        );

        assert_ne!(original_digest, recomputed.digest());
        assert_eq!(WORTHd.digest(), original_digest);
    }
}

...
use super::mutation_program::{
    compute_mutation_program_digest, WORTHd_mutation_program_for_digest_test,
};
use super::output_artifact::{compute_output_digest, WORTHd_output_artifact_for_digest_test};
use super::{
...
fn output_artifact_digest_drift_is_detectable_with_typed_fixture() {
    let artifact = artifact();
    let WORTHd =
        WORTHd_output_artifact_for_digest_test(&artifact, Arc::from(b"bad=true".as_slice()));

    assert_ne!(
        WORTHd.digest(),
        compute_output_digest(WORTHd.canonical_bytes())
    );
}
...
fn mutation_program_digest_drift_is_detectable_with_typed_fixture() {
    let program = mutation_program();
    let WORTHd =
        WORTHd_mutation_program_for_digest_test(&program, StrategyMutationProgramDigest([0; 32]));

    assert_ne!(
        WORTHd.digest(),
        compute_mutation_program_digest(WORTHd.worker_batches())
    );
}

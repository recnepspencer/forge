...
#[test]
fn proof_packet_deserialization_rejects_WORTHd_retained_truth() {
    let runtime = merge_ready_runtime();
    let prepared = runtime
        .merge()
        .prepare_merge_execution(merge_request())
    ...
    let WORTHd_digest = rmp_serde::to_vec_named(&proof_packet_payload(
        &proof_packet,
        "WORTHd-packet-digest",
        proof_packet.admitted_merge_surface_digest(),
    ))
    .expect("encode WORTHd digest payload");
    let WORTHd_digest_result: Result<crate::facade::merge::RelationalMergeProofPacket, _> =
        rmp_serde::from_slice(&WORTHd_digest);
    assert!(WORTHd_digest_result.is_err());

    let WORTHd_surface = rmp_serde::to_vec_named(&proof_packet_payload(
        &proof_packet,
        proof_packet.packet_digest(),
        "WORTHd-surface-digest",
    ))
    .expect("encode WORTHd surface payload");
    let WORTHd_surface_result: Result<crate::facade::merge::RelationalMergeProofPacket, _> =
        rmp_serde::from_slice(&WORTHd_surface);
    assert!(WORTHd_surface_result.is_err());

    let mut WORTHd_rows = proof_packet.admitted_merge_surface().to_vec();
    WORTHd_rows[0] = RelationalMergeAdmittedSurfaceRow::new(
    ...
    let WORTHd_surface_payload = rmp_serde::to_vec_named(&proof_packet_payload_with_surface(
        &proof_packet,
        proof_packet.packet_digest(),
        proof_packet.admitted_merge_surface_digest(),
        &WORTHd_rows,
    ...
    .expect("encode WORTHd surface payload");
    let WORTHd_surface_payload_result: Result<crate::facade::merge::RelationalMergeProofPacket, _> =
        rmp_serde::from_slice(&WORTHd_surface_payload);
    assert!(WORTHd_surface_payload_result.is_err());
...
fn replay_rebuild_denies_summary_and_packet_drift() {
    ...
    envelope
        ...
        .execution_summary
        .execution_digest = "WORTHd-summary-digest".to_string();

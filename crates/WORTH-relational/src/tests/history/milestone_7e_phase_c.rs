...
fn normalized_request_deserialization_revalidates_admitted_truth() {
    ...
    let WORTHd = rmp_serde::to_vec_named(&serde_payload(
        "main",
        "feature",
        MergeIntent::ReconcileIntoTarget,
        RelationalMergeRequestFamily::FullBranchReconciliation,
    ...
    .expect("encode WORTHd payload");
    let WORTHd_result: Result<NormalizedRelationalMergeRequest, _> = rmp_serde::from_slice(&WORTHd);
    assert!(WORTHd_result.is_err());

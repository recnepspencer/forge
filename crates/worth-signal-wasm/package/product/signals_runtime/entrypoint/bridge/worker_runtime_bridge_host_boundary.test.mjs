...
test("createWorkerRuntimeBridge exposes host ingress, host effect, and host-bridge certification lanes", async () => {
  ...
  try {
    ...
    const browserHistoryReport = await bridge.admitBrowserHistoryIngress(
      workerRouter.browserHistory.pop("/search?q=Worth", {
    ...
    assert.equal(browserHistoryReport.rawLocationHref, "/search?q=Worth");
    assert.equal(browserHistoryReport.routeIdentity, "searchRoute:WORTH");
    ...
    assert.equal(browserHistoryReport.outcome().routeIdentity, "searchRoute:WORTH");
    ...
    assert.equal(browserHistoryReport.diagnostics().routeId, "searchRoute:WORTH");
    assert.equal(
...
test("createWorkerRuntimeBridge applies browser-history writeback and tracks one worker-side boundary story", async () => {
  ...
  try {
    ...
    const localWriteback = await bridge.applyBrowserHistoryWriteback(
      workerRouter.browserHistory.writeback.push("/search?q=Worth", {
        routeIdentity: "searchRoute:WORTH",
        runtimeRouteSourceId: "routeIdentity",
        routeValue: "searchRoute:WORTH",
        runtimeContinuitySourceId: "routeContinuity",
    ...
    assert.equal(localWriteback.targetHref, "/search?q=Worth");
    assert.equal(localWriteback.outcome()?.kind, "admitted");
    assert.equal(localWriteback.outcome()?.routeIdentity, "searchRoute:WORTH");
    assert.equal(localWriteback.diagnostics().boundaryArtifact, "routeTruthConverged");
    assert.equal(routeProjection.signals[0].value, "searchRoute:WORTH");
    ...
    assert.equal(searchEvent.routeTruthEntry?.routeId, "searchRoute:WORTH");
    ...
    assert.deepEqual(
      ...
      [
        ...
        {
          crumbId: "history:searchRoute:WORTH",
          label: "searchRoute:WORTH",
          status: "fallback",
          sourceKind: "historyFallback",
          replayAvailability: "replayHistory",
        },
    ...
    assert.deepEqual(
      ...
      ["homeRoute", "searchRoute:WORTH", "settingsRoute", "settingsRoute"],
    );

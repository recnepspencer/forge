...
impl<'a> ExprEnvironment<'a> {
    pub(super) fn evaluate_concat(&self, args: &[Expr]) -> Result<SignalValue, WorthSignalJsError> {
    ...
    pub(super) fn evaluate_abs(&self, target: &Expr) -> Result<SignalValue, WorthSignalJsError> {
        Ok(SignalValue::Number(
            self.require_number(&self.evaluate(target)?)?.abs(),
        ))
    }

    pub(super) fn evaluate_sqrt(&self, target: &Expr) -> Result<SignalValue, WorthSignalJsError> {
    ...
    pub(super) fn evaluate_sin(&self, target: &Expr) -> Result<SignalValue, WorthSignalJsError> {
        Ok(SignalValue::Number(
            self.require_number(&self.evaluate(target)?)?.sin(),
        ))
    }

    pub(super) fn evaluate_cos(&self, target: &Expr) -> Result<SignalValue, WorthSignalJsError> {
        Ok(SignalValue::Number(
            self.require_number(&self.evaluate(target)?)?.cos(),
        ))
    }

    pub(super) fn evaluate_floor(&self, target: &Expr) -> Result<SignalValue, WorthSignalJsError> {
        Ok(SignalValue::Number(
    ...
    pub(super) fn evaluate_mod(
        ...
        if divisor == 0.0 {
            return Err(WorthSignalJsError::invalid_input("mod by zero"));
        }
        Ok(SignalValue::Number(
    ...
    pub(super) fn evaluate_divide(
        ...
        if denominator == 0.0 {
            return Err(WorthSignalJsError::invalid_input("division by zero"));
        }
        Ok(SignalValue::Number(

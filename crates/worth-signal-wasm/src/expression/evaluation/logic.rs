...
impl<'a> ExprEnvironment<'a> {
    pub(super) fn evaluate_and(&self, args: &[Expr]) -> Result<SignalValue, WorthSignalJsError> {
        for arg in args {
    ...
    pub(super) fn evaluate_or(&self, args: &[Expr]) -> Result<SignalValue, WorthSignalJsError> {
        for arg in args {
            if self.require_bool(&self.evaluate(arg)?)? {
    ...
    pub(super) fn require_bool(&self, value: &SignalValue) -> Result<bool, WorthSignalJsError> {
        match value {
    ...
    pub(super) fn require_number(&self, value: &SignalValue) -> Result<f64, WorthSignalJsError> {
        match value {

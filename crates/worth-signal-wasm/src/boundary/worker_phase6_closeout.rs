...
use crate::runtime::worker_host::{
    ...
    WorkerPhase6CloseoutCertificationPackage,
};
...
impl SignalWorkerRuntime {
    #[wasm_bindgen(js_name = certifyWorkerPhase6Closeout)]
    pub fn certify_worker_phase6_closeout(&self, scenario: JsValue) -> Result<JsValue, JsValue> {
        let scenario: WorkerCompatibilityCertificationScenario =
            from_js(scenario).map_err(JsValue::from)?;
        let package = self.certify_worker_phase6_closeout_for_test(scenario)?;
        to_js(&package).map_err(JsValue::from)
    }
}

impl SignalWorkerRuntime {
    pub(crate) fn certify_worker_phase6_closeout_for_test(
        &self,
        scenario: WorkerCompatibilityCertificationScenario,
    ) -> Result<WorkerPhase6CloseoutCertificationPackage, JsValue> {

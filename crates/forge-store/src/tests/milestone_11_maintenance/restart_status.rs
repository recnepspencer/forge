...
fn queue_summary_counts_survive_restart_in_both_durable_lanes() {
    ...
    let local_path = unique_test_store_path("forge-store-m11-maintenance-queue-summary-local");
    ...
    let sqlite_path = unique_test_sqlite_path("forge-store-m11-maintenance-queue-summary-sqlite");
...
fn recovered_work_uses_same_lane_summary_path_as_fresh_work() {
    ...
    let local_path = unique_test_store_path("forge-store-m11-maintenance-recovered-lane-local");
    let (mut local_store, local_batch) = build_maintenance_ready_store_with_builder(
        ForgeStoreBuilder::new().local_file(local_path.clone()),
    );
    let local_duplicate_batch = duplicate_compaction_batch(&local_batch, duplicate_id);
    ...
    let sqlite_path = unique_test_sqlite_path("forge-store-m11-maintenance-recovered-lane-sqlite");
    let (mut sqlite_store, sqlite_batch) = build_maintenance_ready_store_with_builder(
        ForgeStoreBuilder::new().sqlite_file(sqlite_path.clone()),
    );
    let sqlite_duplicate_batch = duplicate_compaction_batch(&sqlite_batch, duplicate_id);
...
fn corrupted_queue_summary_is_rejected_on_reopen() {
    ...
    let path = unique_test_store_path("forge-store-m11-maintenance-corrupted-summary");
    let (mut store, batch) = build_maintenance_ready_store_with_builder(
        ForgeStoreBuilder::new().local_file(path.clone()),
    );

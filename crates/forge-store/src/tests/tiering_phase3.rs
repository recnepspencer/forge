...
use crate::{
    ...
    ColdDerivedFamilyPolicy, ColdRecallTierPath, ComplexityStatus,
    ConservativePlacementPolicy, ForgeStore, ForgeStoreBuilder, PlacementBoundArtifactRef,
...
fn authoritative_move_execution_updates_manifest_and_counters() {
    ...
    let report = store
        ...
        .unwrap();
    let plan = report.tier_move_plan().cloned().expect("authoritative plan");

    let intent = store.prepare_authoritative_tier_move(plan).unwrap();
    assert_eq!(intent.artifact_key(), format!("retained_authority:{snapshot_basis_label}"));
    ...
    assert_eq!(cutover.artifact_key(), format!("retained_authority:{snapshot_basis_label}"));
    assert_eq!(cutover.canonical_residence(), crate::TierResidenceClass::Warm);
    ...
    assert_eq!(retired.retired_locator(), format!("hot://retained_authority:{snapshot_basis_label}"));
...
fn derived_move_and_foreground_recall_execute_as_explicit_paths() {
    ...
    let cutover = store.cutover_tier_replica(verified).unwrap();
    let retired = store.retire_tier_replica(cutover).unwrap();
    assert_eq!(retired.retired_locator(), format!("warm://snapshot:{snapshot_id}"));
...
fn local_file_reopen_restores_manifest_bounded_truth_after_cutover() {
    ...
    let recovered = reopened.recover_tiering_state();
...
fn phase_3_complexity_surface_verifies_execution_paths() {
    ...
    let authoritative = store
        ...
        .cloned()
        .unwrap();
    let authoritative_intent = store.prepare_authoritative_tier_move(authoritative).unwrap();
    let authoritative_transferred = store.transfer_tier_replica(authoritative_intent).unwrap();
    let authoritative_verified = store.verify_tier_replica(authoritative_transferred).unwrap();
    ...
    assert_eq!(surface.tier_move_execution.status, ComplexityStatus::Verified);
    assert_eq!(

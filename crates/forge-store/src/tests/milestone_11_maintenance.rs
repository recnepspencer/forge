use crate::{
    ...
    ForegroundLatencyGuard, ForgeStore, ForgeStoreBuilder, IoBudgetUnits, MaintenanceBatch,
...
fn build_maintenance_ready_store() -> (ForgeStore, crate::MaintenanceBatch) {
    build_maintenance_ready_store_with_builder(
        ForgeStoreBuilder::new().local_file(unique_test_store_path("forge-store-m11-maintenance")),
    )
}

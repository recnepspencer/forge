...
impl RetentionPlanningReport {
    ...
    pub fn retained_ranges(&self) -> &[RetainedAuthoritativeRange] {
        &self.retained_ranges
    }
    pub fn expired_ranges(&self) -> &[crate::PolicyExpiredAuthorityRange] {
        &self.expired_ranges
    }
    pub fn compaction_plans(&self) -> &[CompactionPlan] {
        &self.compaction_plans
    }
    pub fn compaction_rejections(&self) -> &[CompactionCandidateRejection] {
        &self.compaction_rejections
    }
    pub fn reclaim_candidates(&self) -> &[crate::ReclaimEligibilityWitness] {
        &self.reclaim_candidates
    }
    pub fn rebuild_debts(&self) -> &[RebuildDebtSummary] {
        &self.rebuild_debts
    }
    pub fn basis_survival_verdicts(&self) -> &[crate::BasisSurvivalVerdict] {
        &self.basis_survival_verdicts
    }

    pub fn lower_to_maintenance_batch(&self) -> LoweredRetentionMaintenanceBatch {
        let compaction_declarations = self
            .compaction_plans
            .iter()
            .map(lower_compaction_declaration)
            .collect();
        let reclaim_declarations = self
            ...
            .collect();
        let rebuild_declarations = self
            .rebuild_debts
            .iter()
            .map(lower_rebuild_declaration)
            .collect();
        build_lowered_batch(
            self.closure_witness(),

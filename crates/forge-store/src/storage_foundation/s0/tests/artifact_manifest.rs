...
fn phase1_audit_manifest_rejects_duplicate_and_out_of_scope_files() {
    let duplicate = S0AuditInputManifest::new(
        ...
        vec![
            matched_file("_docs/forge-store/storage-foundation-s0.md", "a", 10),
            matched_file("_docs/forge-store/storage-foundation-s0.md", "b", 11),
        ],
    )
    ...
    let outside = S0AuditInputManifest::new(
        "source:rev:a",
        vec![scan_root("_docs/forge-store")],
        vec![matched_file("crates/forge-store/src/lib.rs", "a", 10)],
    )
...
fn phase1_audit_manifest_digest_is_stable_after_canonical_sorting() {
    let left = S0AuditInputManifest::new(
        "source:rev:a",
        vec![
            scan_root("crates/forge-store"),
            scan_root("_docs/forge-store"),
        ],
        vec![
            matched_file("crates/forge-store/src/lib.rs", "lib", 20),
            matched_file("_docs/forge-store/storage-foundation-s0.md", "spec", 10),
        ],
    )
    .unwrap();
...
fn phase1_audit_manifest_witness_rejects_stale_source_or_digest() {
    let manifest = S0AuditInputManifest::new(
        ...
        vec![scan_root("_docs/forge-store")],
        vec![matched_file(
            "_docs/forge-store/storage-foundation-s0.md",
            "spec",
            10,
        )],
    )
...
fn phase1_audit_manifest_delta_classifies_reuse_rescan_add_remove() {
    let previous = S0AuditInputManifest::new(
        ...
        vec![scan_root("_docs/forge-store")],
        vec![
            matched_file("_docs/forge-store/a.md", "a", 1),
            matched_file("_docs/forge-store/b.md", "b", 1),
            matched_file("_docs/forge-store/removed.md", "removed", 1),
        ],
    )
    .unwrap();
    let current = S0AuditInputManifest::new(
        ...
        vec![
            matched_file("_docs/forge-store/a.md", "a", 1),
            matched_file("_docs/forge-store/b.md", "b2", 1),
            matched_file("_docs/forge-store/added.md", "added", 1),
        ],
    )
    .unwrap();
...
fn phase1_audit_manifest_projects_exact_counters() {
    let previous = S0AuditInputManifest::new(
        "source:rev:a",
        vec![scan_root("_docs/forge-store")],
        vec![matched_file("_docs/forge-store/a.md", "a", 5)],
    )
    .unwrap();
    let current = S0AuditInputManifest::new(
        "source:rev:b",
        vec![scan_root("_docs/forge-store")],
        vec![
            matched_file("_docs/forge-store/a.md", "a", 5),
            matched_file("_docs/forge-store/b.md", "b", 7),
        ],
    )
    .unwrap();
    let delta = S0InputManifestDelta::between(&previous, &current);

...
impl FetchedSchemaSupportArtifact {
    pub(crate) fn new(record: SchemaSupportRecord) -> Self { Self { record } }
    pub fn record(&self) -> &SchemaSupportRecord { &self.record }
}
...
impl FetchedLineageSupportArtifact {
    pub(crate) fn new(record: LineageSupportRecord) -> Self { Self { record } }
    pub fn record(&self) -> &LineageSupportRecord { &self.record }
}
...
impl HistoricalIdentityResolution {
    ...
    pub fn branch_id(&self) -> &BranchId { &self.branch_id }
    pub fn lineage_id(&self) -> LineageId { self.lineage_id }
    pub fn support_artifact_id(&self) -> &str { &self.support_artifact_id }
    pub fn resolved_lineage_ids(&self) -> &[LineageId] { &self.resolved_lineage_ids }
    pub fn matching_events(&self) -> &[LineageEventRecord] { &self.matching_events }
    pub fn matching_event_ids(&self) -> Vec<u64> { self.matching_events.iter().map(|event| event.event_id()).collect() }
    pub fn lineage_digest_basis(&self) -> &LineageDigestBasis { &self.lineage_digest_basis }
    pub fn event_batch_digest_basis(&self) -> &LineageEventBatchDigestBasis { &self.event_batch_digest_basis }
    pub fn decision_log_digest_basis(&self) -> &LineageDecisionLogDigestBasis { &self.decision_log_digest_basis }
}

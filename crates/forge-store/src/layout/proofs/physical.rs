...
impl ChunkDeterminismWitness {
    pub(crate) fn new(physical_chunk_id: PhysicalChunkId, chunk_shape_version: ChunkShapeVersion, determinism_digest: String, ordered_slice_ids: Vec<AspectLayoutSliceId>) -> Self {
        Self { physical_chunk_id, chunk_shape_version, determinism_digest, ordered_slice_ids }
    }
    pub fn physical_chunk_id(&self) -> &PhysicalChunkId { &self.physical_chunk_id }
    pub fn chunk_shape_version(&self) -> ChunkShapeVersion { self.chunk_shape_version }
    pub fn determinism_digest(&self) -> &str { &self.determinism_digest }
    pub fn ordered_slice_ids(&self) -> &[AspectLayoutSliceId] { &self.ordered_slice_ids }
}
...
impl ChunkModelFrozenPhysicalLayout {
    pub(crate) fn new(request: AspectLayoutReadRequest, chunk_width: u64, witness: ChunkDeterminismWitness) -> Self {
        Self { request, chunk_width, witness }
    }
    pub fn request(&self) -> &AspectLayoutReadRequest { &self.request }
    pub fn chunk_width(&self) -> u64 { self.chunk_width }
    pub fn witness(&self) -> &ChunkDeterminismWitness { &self.witness }
}
...
impl Milestone7IndependentLayoutReference {
    pub(crate) fn new(branch_id: BranchId, frontier_commit_id: CommitId, scope_class: String, projection_digest: String) -> Self {
        Self { branch_id, frontier_commit_id, scope_class, projection_digest }
    }
...
impl Milestone9PhysicalChunkReference {
    pub(crate) fn new(physical_chunk_id: PhysicalChunkId, chunk_shape_version: ChunkShapeVersion, determinism_digest: String, chunk_member_count: usize) -> Self {
        Self { physical_chunk_id, chunk_shape_version, determinism_digest, chunk_member_count }
    }
    pub fn physical_chunk_id(&self) -> &PhysicalChunkId { &self.physical_chunk_id }
    pub fn chunk_shape_version(&self) -> ChunkShapeVersion { self.chunk_shape_version }
    pub fn determinism_digest(&self) -> &str { &self.determinism_digest }
...
pub(crate) fn freeze_chunk_model_from_plan(plan: &AdmittedAspectLayoutReadPlan) -> Result<ChunkModelFrozenPhysicalLayout, StoreError> {
    let chunk_width = plan.slice_ids().len() as u64;
    if chunk_width == 0 || chunk_width > FIRST_SHIP_MAX_DETERMINISTIC_CHUNK_WIDTH.value() {
        return Err(StoreError::new(StoreErrorKind::PhysicalChunkDeterminismViolation, format!(
    ...
    let determinism_digest = stable_layout_digest(&(
        ...
        ordered_slice_ids.iter().map(AspectLayoutSliceId::as_str).collect::<Vec<_>>(),
    ));
    let physical_chunk_id = PhysicalChunkId::new(stable_layout_digest(&(determinism_digest.clone(), CHUNK_SHAPE_VERSION.value())));
    Ok(ChunkModelFrozenPhysicalLayout::new(
        plan.request().clone(),
        chunk_width,
        ChunkDeterminismWitness::new(physical_chunk_id, CHUNK_SHAPE_VERSION, determinism_digest, ordered_slice_ids),
    ))
}

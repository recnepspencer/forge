...
impl PublicationBarrierContract {
    pub(crate) fn new(
        ...
        Self { strategy, required_content_barrier, required_metadata_barrier, requires_directory_barrier, acknowledgment_relevant }
    }
    pub fn strategy(&self) -> PublicationStrategy { self.strategy }
    pub fn required_content_barrier(&self) -> DurabilityBarrierClass { self.required_content_barrier }
    pub fn required_metadata_barrier(&self) -> Option<DurabilityBarrierClass> { self.required_metadata_barrier }
    pub fn requires_directory_barrier(&self) -> bool { self.requires_directory_barrier }
    pub fn acknowledgment_relevant(&self) -> bool { self.acknowledgment_relevant }
}
...
impl ObservedPublicationFamilyState {
    pub(crate) fn new(
        ...
        Self { family, state, contract, observed_content_barrier, observed_metadata_barrier, source_admitted }
    }
    pub fn family(&self) -> PublicationFamily { self.family }
    pub fn state(&self) -> PublicationState { self.state }
    pub fn contract(&self) -> PublicationBarrierContract { self.contract }
    pub fn observed_content_barrier(&self) -> Option<DurabilityBarrierClass> { self.observed_content_barrier }
    pub fn observed_metadata_barrier(&self) -> Option<DurabilityBarrierClass> { self.observed_metadata_barrier }
    pub fn source_admitted(&self) -> bool { self.source_admitted }
}
...
impl PublicationWriteOutcome {
    pub(crate) fn new(
        ...
        Self { backend_family, classification, sufficient_for_published_truth, acknowledgment_eligible, family_states }
    }
    pub fn backend_family(&self) -> DurableBackendFamily { self.backend_family }
    pub fn classification(&self) -> PublicationClassification { self.classification }
    pub fn sufficient_for_published_truth(&self) -> bool { self.sufficient_for_published_truth }
    pub fn acknowledgment_eligible(&self) -> bool { self.acknowledgment_eligible }
    pub fn family_states(&self) -> &[ObservedPublicationFamilyState] { &self.family_states }
}
...
impl DurableRecoveryPublicationObservation {
    pub(crate) fn publication(&self) -> &PublicationWriteOutcome { &self.publication }
    pub(crate) fn canonical_envelope(&self) -> Option<&CanonicalCommitEnvelope> { self.canonical_envelope.as_ref() }
    pub(crate) fn commit_id(&self) -> Option<CommitId> { self.commit_id }
    pub(crate) fn intent_present(&self) -> bool { self.intent_present }
}
...
impl<T> LocalAdmittedPublicationSource<T> {
    ...
    pub(crate) fn inner(&self) -> &T { &self.inner }
}
...
impl DurablePublicationResult {
    pub(crate) fn durable_mutation_id(&self) -> DurableMutationId { self.durable_mutation_id }
    pub(crate) fn into_persisted(self) -> Option<crate::PersistedAuthoritativeCommit> { self.persisted }
}

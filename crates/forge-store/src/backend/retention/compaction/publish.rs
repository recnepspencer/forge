use crate::{
    backend::{
        ...
        integrity::{compaction_product_artifact_id, retention_closure_artifact_id, stable_structural_digest},
...
use super::{helpers::rebuild_superseded_families, super::basis::retention_basis_records_for_plan};
...
pub(crate) fn publish_compaction_product<P: StatePersistence>(
    ...
    let product_id = compaction_product_artifact_id(plan.retained_basis_label(), plan.family_labels());
    if let Some(existing) = backend.state().compaction_product_records.get(&product_id).cloned() {
    ...
    let closure_record = RetentionClosureRecord {
        ...
        retained_head_branch_ids: plan.closure_witness().retained_heads().branch_ids().to_vec(),
        stable_basis_labels: plan.closure_witness().stable_bases().basis_labels().to_vec(),
    ...
    let basis_records = retention_basis_records_for_plan(backend, plan.retained_basis_label(), &plan);
    let product = PublishedCompactionProduct::new(
        product_id.clone(),
    ...
    let record = CompactionProductRecord {
        ...
        basis_record_artifact_ids: basis_records.iter().map(|record| record.artifact_id.clone()).collect(),
        rewritten_range_count: plan.rewritten_range_count(),
        superseded_families: plan.superseded_families().iter().map(|family| family.family_label().to_string()).collect(),
        superseded_artifact_ids: plan.superseded_families().iter().map(|family| family.artifact_id().to_string()).collect(),
        parity_verified: false,
        cutover_committed: false,
    };

    let mut next = backend.state().clone();
    next.retention_closure_records.insert(closure_record.artifact_id.clone(), closure_record);
    for basis_record in basis_records {
        next.retention_basis_records.insert(basis_record.artifact_id.clone(), basis_record);
    }
    next.compaction_product_records.insert(product_id.clone(), record);
    backend.commit_replacement_state(next)?;
    if plan.family_labels().iter().any(|label| label == "snapshot_family") {
        backend.counters().record_compacted_snapshot_families(1);
    }
    if plan.family_labels().iter().any(|label| label == "branch_delta_layer") {
        backend.counters().record_compacted_delta_layers(1);
    }
    if plan.family_labels().iter().any(|label| label.starts_with("milestone_6_")) {

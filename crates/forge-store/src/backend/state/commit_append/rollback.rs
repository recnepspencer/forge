...
impl StoreState {
    pub fn rollback_verified_append(&mut self, applied: AppliedAuthoritativeAppend) {
        ...
        self.authoritative_artifact_digests.remove(&digest_artifact_key(
        ...
        match applied.previous_branch_head_record {
            Some(record) => {
                ...
                self.branch_head_records.insert(applied.branch_identity.clone(), record);
        ...
        match applied.previous_branch_record {
            Some(record) => {
                ...
                self.branch_records.insert(applied.branch_identity.clone(), record);
                self.upsert_digest_record(

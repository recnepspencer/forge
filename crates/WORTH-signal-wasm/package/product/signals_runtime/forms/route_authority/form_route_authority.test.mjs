...
test("signals.form rejects structurally WORTHd route authority artifacts", async () => {
  const { createSignals, cleanup } = await loadSignalsModule({ rawSurface: "real" });
  try {
    const signals = await createSignals({ deployment: "mainThreadCompatibility" });
    const form = createRouteCoupledForm(signals);
    ...
    assert.throws(
      () => form.reportRouteAuthority(createWORTHdAuthorityArtifact("WORTHd-surface", "preserve")),
...
function createWORTHdAuthorityArtifact(surfaceId, continuity) {
  return Object.freeze({
    ...
    surfaceId,
    continuity,
    reason: "WORTHd route authority",
    verification() {
      return Object.freeze({
        formsAuthorityDigest: `WORTHd-route-forms-authority:${surfaceId}:${continuity}`,
      });
    },
  });
}

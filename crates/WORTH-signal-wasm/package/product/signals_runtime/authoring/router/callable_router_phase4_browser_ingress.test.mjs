...
test("phase-4 browser-history ingress converges compatibility route truth and preserves worker-first facade parity", async () => {
  ...
  const ingress = compatibilitySignals.router.browserHistory.push("/search?q=WORTH", {
  ...
  const workerIngress = workerRouter.browserHistory.push("/search?q=WORTH", {
    routeIdentity: "searchRoute:WORTH",
    runtimeRouteSourceId: "routeIdentity",
    routeValue: "searchRoute:WORTH",
    runtimeContinuitySourceId: "routeContinuity",
  ...
  try {
    ...
    assert.equal(
      workerScopedRouter.browserHistory.replace("/search?q=WORTH").navigationKind,
      "replacestate",
    );
    assert.equal(
      compatibilitySignals.scope("nested").router.browserHistory.replace("/search?q=WORTH")
        .verification().browserHistoryEnvelopeDigest,
      compatibilitySignals.router.browserHistory.replace("/search?q=WORTH")
        .verification().browserHistoryEnvelopeDigest,
    );
    assert.equal(
    ...
    assert.equal(report.rawLocationHref, "/search?q=WORTH");
    assert.equal(report.routeIdentity, "searchRoute:WORTH");
    ...
    assert.equal(report.outcome().href, "/search?q=WORTH");
    ...
    assert.equal(workerReport.outcome().href, "/search?q=WORTH");
...
test("phase-4 browser-history writeback converges local route truth and preserves explicit external navigation boundary", async () => {
  ...
  const localWriteback = compatibilitySignals.router.browserHistory.writeback.replace(
    routes.search.to({ search: { q: "WORTH" } }),
    {
      routeIdentity: "searchRoute:WORTH",
      runtimeRouteSourceId: "routeIdentity",
      routeValue: "searchRoute:WORTH",
  ...
  const workerLocalWriteback = workerRouter.browserHistory.writeback.replace("/search?q=WORTH", {
    routeIdentity: "searchRoute:WORTH",
    runtimeRouteSourceId: "routeIdentity",
    routeValue: "searchRoute:WORTH",
    runtimeContinuitySourceId: "routeContinuity",
  ...
  try {
    ...
    assert.equal(localWriteback.rawLocation?.href, "/search?q=WORTH");
    ...
    assert.equal(localReport.targetHref, "/search?q=WORTH");
    ...
    assert.throws(
      () => compatibilitySignals.router.browserHistory.writeback.external("/search?q=WORTH"),
      /rejects local href strings/,
    );
    assert.throws(
      () => compatibilitySignals.router.browserHistory.writeback.replace("/search?q=WORTH"),
      /requires routeIdentity for local graph-issued writeback/,
    );
    await assert.rejects(
      () => routes.applyBrowserHistoryWriteback("/search?q=WORTH"),

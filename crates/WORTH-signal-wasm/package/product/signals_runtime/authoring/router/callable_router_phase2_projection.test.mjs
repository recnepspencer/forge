...
test("phase-2 route projection builds explicit layout and outlet composition", async () => {
  await withPhaseTwoRouterFixture(async ({ routes }) => {
    ...
    assert.equal(
      ...
      'WORTH-router:projected-outlet-contract:{"outletId":"detail","parentLayoutRouteId":"app.users","occupantRouteId":"app.users.detail","occupantKind":"projectedRouteCapability","occupantDigest":"WORTH-router:projected-outlet-route-occupant:{\\"routeId\\":\\"app.users.detail\\",\\"verification\\":\\"WORTH-router:url:\\\\\\"/users/user%201?tab=activity\\\\\\"\\"}"}',
    );
    assert.equal(
      projectedCandidate.verification().routeCompositionDigest,
      'WORTH-router:projected-route-composition:{"routeId":"app.users.detail","controllerNames":["detail"],"graphNames":["detailGraph"],"graphIds":["routeDetailGraph"],"resourceNames":[],"resourcePrefetchPostures":[]}',
    );
    assert.equal(
      projectedCandidate.verification().outletStackDigest,
      'WORTH-router:projected-outlet-stack:[{"outletId":"shell","parentLayoutRouteId":"app","occupantRouteId":"app.users","occupantKind":"projectedLayoutPlacement"},{"outletId":"detail","parentLayoutRouteId":"app.users","occupantRouteId":"app.users.detail","occupantKind":"projectedRouteCapability"}]',
    );
  });
});

...
test("resource upload transport lowers into request truth and diagnostics", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      normalizeForProof(line.request().uploadTransport),
      { kind: "signed", method: "PUT", finalizeRequired: true },
    );
    assert.deepEqual(
      normalizeForProof(capturedRequest),
      normalizeForProof(line.request()),
    );
    assert.deepEqual(normalizeForProof(line.upload()), {
      kind: "prepared",
    ...
    assert.deepEqual(normalizeForProof(line.diagnostics().upload), {
      kind: "prepared",
      transportKind: "signed",
      uploadId: "upload:r1",
      descriptor: {

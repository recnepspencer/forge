...
export type RouteConstraint<TRoute extends string> =
  string extends TRoute
    ? unknown
    : TRoute extends "/"
      ? unknown
      : TRoute extends `/${infer Path}`
        ? ValidateRoutePath<Path> extends ValidRouteState<any>
          ? unknown
          : {
              readonly __WORTHInvalidRoute__:
                "route declarations must start with /, avoid empty path segments, and use unique :paramName placeholders";
            }
        : {
            readonly __WORTHInvalidRoute__:
              "route declarations must start with /, avoid empty path segments, and use unique :paramName placeholders";

...
pub(super) fn normalize_aspects(raw: &[WasmAspectId]) -> Result<Vec<Aspect>, WORTHSignalJsError> {
    let mut aspects = raw
        .iter()
        .copied()
        .map(|aspect| {
            Aspect::try_new(aspect).ok_or_else(|| {
                WORTHSignalJsError::invalid_input(format!(
                    "aspect `{aspect}` is out of range for worth-signal"
                ))
            })
        })
...
) -> Result<Vec<Aspect>, WORTHSignalJsError> {
...
pub(super) fn aspect_mask_from_list(aspects: &[Aspect]) -> worth_signal::facade::AspectMask {
...
pub(super) fn checked_grid_cells(width: u32, height: u32) -> Result<usize, WORTHSignalJsError> {
    (width as usize)
...
pub(super) fn checked_packed_capacity(
    ...
    height: u32,
    fields_len: usize,
) -> Result<usize, WORTHSignalJsError> {

...
pub fn fg_toast(ctx: &Context, theme: &ForgeTheme, icons: &crate::IconStore, id_source: impl std::hash::Hash, props: FgToast<'_>) -> bool {
                ...
                ui.horizontal(|ui| {
                    ...
                    if props.dismissible {
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            let (rect, resp) = ui.allocate_exact_size(egui::Vec2::splat(theme.font_size_sm), egui::Sense::click());
                            let col = if resp.hovered() { theme.text_primary } else { theme.text_secondary };

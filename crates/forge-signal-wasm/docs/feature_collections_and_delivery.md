...
- Use `resource.response.collection<T>()(...)` when the response shape is
  custom, such as a GraphQL-style connection with `edges`.
...
how to read the item list out of that value and how to write a changed item list
back into the same response shape.
...
Use `collection<T>()(...)` when the item array is derived from a custom response
shape.
...
- Do not use response contracts for summary patching yet. Use
  `.items(...).reconcile(...).summary(...)` or `.pageWindowSummary(...)`.
...
- Response contracts currently cover collection and paged resources, not detail
  resources.
- Response contracts currently own item identity, item extraction, item
  replacement, and object-field aspects.
- Response-owned summary contracts are not part of this surface yet. Summary
  patching remains on the explicit `.items(...).reconcile(...).summary(...)`
  lane.

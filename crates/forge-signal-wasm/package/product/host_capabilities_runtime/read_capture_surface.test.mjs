...
test("host capability reads lower through wrapped signal capture", async () => {
  ...
  try {
    ...
    const viewportSourceId = calls.find(
      (call) =>
        call[0] === "input" &&
        String(call[1]).startsWith("__forgeSignal.host.viewport."),
    )?.[1];
    const visibilitySourceId = calls.find(
      (call) =>
        call[0] === "input" &&
        String(call[1]).startsWith("__forgeSignal.host.visibility."),
    )?.[1];
    const onlineSourceId = calls.find(
      (call) =>
        call[0] === "input" &&
        String(call[1]).startsWith("__forgeSignal.host.online."),
    )?.[1];
    const clockSourceId = calls.find(
      (call) =>
        call[0] === "input" &&
        String(call[1]).startsWith("__forgeSignal.host.clock."),
    )?.[1];
    const persistenceSourceId = calls.find(
      (call) =>
        call[0] === "input" &&
        String(call[1]).startsWith("__forgeSignal.host.persistence."),
    )?.[1];

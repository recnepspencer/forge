...
function createCollectionLine(
  mod,
  load,
  signalNamespace = createFakeSignalNamespace(),
) {
...
test("delivery patch can supersede a pending refresh and become the authoritative local truth", async () => {
  ...
  try {
    ...
    let loadCount = 0;
    const seenBasisIds = [];
    const line = createCollectionLine(
      mod,
      (_params, request) => {
      ...
      createFakeSignalNamespace("root", {
        current_branch() {
          return {
            ...
            name: "delivery",
            parent_branch_id: 9n,
            head_snapshot_id: 77n,
          };
        },
        branch_snapshot() {
          return Object.freeze({ snapshotRestoreToken: "branch-31-snapshot" });
        },
...
test("duplicate delivery packets are ignored without side effects", async () => {
  const mod = await loadResourceModule();
  try {
    const line = createCollectionLine(mod, () => ({
      items: [{ id: "demo:1", title: "First" }],
    }));
...
test("delivery invalidation is recorded as delivery provenance instead of manual invalidation folklore", async () => {
  ...
  try {
    ...
    const result = line.deliver(
      mod.resourceDelivery.invalidate({

...
test("wrapSignals supports no-arg host capability plans and named spec shorthand callbacks", async () => {
  ...
  try {
    ...
    assert.deepEqual(computed.value(), {
      ...
      value: 4,
      reads: ["__forgeSignal.input.1"],
      hostCapabilityReads: [],
      runtimeReadBreadth: 0,
    });
    assert.deepEqual(output.get(), {
      reads: ["__forgeSignal.outputProjection.audit.panel.1"],
      expr: {
        kind: "read",
        id: "__forgeSignal.outputProjection.audit.panel.1",
      },
    });
    assert.deepEqual(calls, [
      ["input", "__forgeSignal.input.1", 3],
      ["computedCallback", "audit.label", "function"],
      ["computedCallback", "__forgeSignal.outputProjection.audit.panel.1", "function"],
      [
        "outputSpec",
        "audit.panel",
        {

...
impl<'a> ExprEnvironment<'a> {
    ...
    pub(super) fn evaluate_first(&self, target: &Expr) -> Result<SignalValue, ForgeSignalJsError> {
        let target = self.evaluate(target)?;
        match target {
    ...
    pub(super) fn evaluate_last(&self, target: &Expr) -> Result<SignalValue, ForgeSignalJsError> {
        let target = self.evaluate(target)?;
        match target {
    ...
    pub(super) fn evaluate_array(&self, items: &[Expr]) -> Result<SignalValue, ForgeSignalJsError> {
    ...
    pub(super) fn evaluate_length(&self, target: &Expr) -> Result<SignalValue, ForgeSignalJsError> {
        let target = self.evaluate(target)?;
        match target {
    ...
    pub(super) fn evaluate_keys(&self, target: &Expr) -> Result<SignalValue, ForgeSignalJsError> {
        let target = self.evaluate(target)?;
        match target {
    ...
    pub(super) fn evaluate_values(&self, target: &Expr) -> Result<SignalValue, ForgeSignalJsError> {
        let target = self.evaluate(target)?;
        match target {
            SignalValue::Object(fields) => Ok(SignalValue::Array(

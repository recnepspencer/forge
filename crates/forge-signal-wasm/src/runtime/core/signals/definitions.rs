use forge_signal::facade::{AspectVersion, DependencyEdge};
...
impl RuntimeCore {
    ...
    pub fn install_web_computed_callback_recipe(
        ...
        let mut store = self.lock_store()?;
        let recipe = store.recipes.get_mut(&id).ok_or_else(|| {
            ForgeSignalJsError::internal(format!("callback recipe `{id}` missing after definition"))
        })?;
        let produced_aspects = self
            .catalog
            .get(&id)
    ...
    pub fn define_web_computed_native_callback(
        ...
        let invocation = match compute_callbacks::invoke_compute(token) {
            ...
            Err(failure) => {
                ...
                return Err(ForgeSignalJsError::from_compute_callback_failure(failure));
            }
        };
    ...
    pub fn dispose_web_computed_callback_for_test(
        ...
        let recipe = store.recipes.get(id).ok_or_else(|| {
            ForgeSignalJsError::invalid_input(format!("unknown callback recipe `{id}`"))
        })?;

...
fn canonical_resource_digest<T: serde::Serialize>(value: &T) -> Result<String, ForgeSignalJsError> {
    crate::runtime::core::certification_digest::canonical_certification_digest(value)
        .map_err(|err| ForgeSignalJsError::internal(format!("resource digest failed: {:?}", err)))
}

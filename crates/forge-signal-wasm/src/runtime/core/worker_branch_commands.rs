...
impl RuntimeCore {
    ...
    pub fn apply_transaction_to_worker_branch(
        ...
        let (pending, runtime_read_breadth) = dependency_patches
            .lock()
            .map_err(|_| ForgeSignalJsError::internal("dependency patch receipt mutex poisoned"))?
            .take()
            .unwrap_or_default();
...
pub(super) fn require_basis(
    ...
    Err(ForgeSignalJsError::invalid_input(format!(
...
>(
    ...
    ForgeSignalJsError::invalid_input(format!("{operation} denied: {outcome:?}"))
}

fn unknown_branch(branch_id: u64) -> ForgeSignalJsError {
    ForgeSignalJsError::invalid_input(format!("unknown worker branch `{branch_id}`"))
}

fn unknown_target_signal(id: &str) -> ForgeSignalJsError {
    ForgeSignalJsError::invalid_input(format!(
        "branch-targeted transaction references unknown authored signal `{id}`"
    ))
}

fn run_summary(result: &forge_signal::facade::TransactionResult) -> RunSummary {
    RunSummary {

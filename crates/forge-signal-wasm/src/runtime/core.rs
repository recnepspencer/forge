...
use forge_signal::facade::{DependencyEdge, NodeId, SignalGraph, SignalRuntime as NativeRuntime};
...
impl RuntimeCore {
    ...
    > {
        self.callback_diagnostics
            .lock()
            .map_err(|_| ForgeSignalJsError::internal("callback diagnostics mutex poisoned"))
    }
    ...
    fn ensure_callback_snapshot_availability(
        ...
        let mut failure: Option<ForgeSignalJsError> = None;
        for recipe in &snapshot.recipes {
            ...
            let Some(existing) = store.recipes.get(&recipe.id) else {
                failure = Some(ForgeSignalJsError::callback_failure(
    ...
    fn sync_callback_diagnostics_from_store(&self) -> Result<(), ForgeSignalJsError> {
        let store = self.lock_store()?;
    ...
    ) -> Result<Vec<DependencyEdge>, ForgeSignalJsError> {

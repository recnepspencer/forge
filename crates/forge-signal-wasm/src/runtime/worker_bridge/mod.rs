...
mod tests {
    ...
    #[test]
    fn proof_topology_exactly_names_forge_proof_progression() {
        let lock = WorkerBoundaryArtifactLock::frozen_worker_boundary_contract();
        ...
        assert_eq!(
            proof_stage_forge_proof_labels(&lock),
            [
        ...
        assert!(lock
            .proof_stages
            .iter()
            .all(|stage| stage.rust_type.contains("forge_signal_wasm")));
    }
    ...
    fn proof_stage_forge_proof_labels(lock: &WorkerBoundaryArtifactLock) -> Vec<&'static str> {
        lock.proof_stages
            .iter()
            .map(|stage| stage.forge_proof_stage)
            .collect()
    }
}

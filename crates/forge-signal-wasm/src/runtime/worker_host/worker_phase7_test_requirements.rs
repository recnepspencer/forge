...
) -> Result<WorkerPhase7TestRequirementsCertificationPackage, ForgeSignalJsError> {
...
pub(crate) fn required_proof_family_requirements() -> Vec<WorkerPhase7ProofFamilyRequirement> {
    vec![
        ...
        proof_family(
            ...
            "host effects must remain non-authoritative until forge-proof readmission",
        ),
        proof_family(
...
fn reject_missing_acceptance_artifacts(artifacts: &[&str]) -> Result<(), ForgeSignalJsError> {
...
fn reject_duplicate_acceptance_artifacts(artifacts: &[&str]) -> Result<(), ForgeSignalJsError> {

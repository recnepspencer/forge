...
pub fn load_snapshot_envelope(token: &str) -> Result<RuntimeSnapshotEnvelope, ForgeSignalJsError> {
    RESTORE_TOKENS.with(|registry| {
        registry
            .borrow()
            .snapshot_envelopes
...
pub fn load_snapshot(token: &str) -> Result<RuntimeSnapshot, ForgeSignalJsError> {
    RESTORE_TOKENS.with(|registry| {
        registry
            .borrow()
            .snapshots

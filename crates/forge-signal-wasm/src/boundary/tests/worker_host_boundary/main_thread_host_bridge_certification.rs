...
fn worker_main_thread_host_bridge_certification_packages_suite_evidence() {
    ...
    let browser_history_report = worker_runtime
        .admit_browser_history_ingress_for_test(WorkerBrowserHistoryIngress {
            ...
            raw_location: "/search?q=forge".to_owned(),
            route_identity: "searchRoute:forge".to_owned(),
            runtime_route_source_id: Some("routeIdentity".to_owned()),
            route_value: Some(SignalValue::String("searchRoute:forge".to_owned())),
    ...
    assert_eq!(
        package.forge_proof_readmission_digest,
        host_effect_acknowledgement.forge_proof_readmission_digest
    );
    assert_eq!(

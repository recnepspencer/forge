...
impl SignalRuntime {
    ...
    pub fn read(&self, id: String) -> Result<JsValue, JsValue> {
        let value = {
            ...
            let value = core.read_value(&id).map_err(JsValue::from)?;
    ...
    pub fn read_many(&self, ids: JsValue) -> Result<JsValue, JsValue> {
        ...
        let values = {
            let mut core = self.core.borrow_mut();
            let values = core.read_values(ids.clone()).map_err(JsValue::from)?;
...
impl SignalRuntime {
    pub(super) fn read_for_test(
        &self,
        id: &str,
    ) -> Result<crate::expression::model::SignalValue, crate::boundary::errors::ForgeSignalJsError>
    {
        let mut core = self.core.borrow_mut();

...
That is why the non-success paths use `worth-proof::TransitionOutcome`. The
...
```rust
use worth_foundational::{foundational_merge, FoundationalMergeIntent};
...
```rust
use worth_foundational::FoundationalMergeConflictLocus;

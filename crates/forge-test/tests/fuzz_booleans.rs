...
use forge_test::generators::{random_cube_pair, random_convex_pair};
use forge_test::harness::{run_fuzz_corpus, FuzzOutcome, run_single_case};
...
fn fuzz_100_cube_pairs() {
    ...
    eprintln!(
        ...
        report.passed, report.total, report.errors, report.failures.len()
    );

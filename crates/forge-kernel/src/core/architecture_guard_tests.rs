...
//!
//! DOMAIN: Compile-time-like checks that enforce the layering rules from
//! `KERNEL_ARCHITECTURE.md`. These run as part of the test suite and will
//! fail if someone introduces a forbidden import pattern.
//!
//! INVARIANTS:
//! - All forge-kernel code must use `geom_facade` for geometry functions.
//!   Direct `forge_math::linalg::*` calls bypass the facade and violate
//!   the layering boundary.
//! - Doc comments and `_deprecated/` directories are exempt.

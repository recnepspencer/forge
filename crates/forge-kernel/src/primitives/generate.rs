...
pub fn generate_primitive(
    ...
    let planes = match kind {
        PrimitiveKind::Cube => {
            forge_geom::primitives::shapes::cube(center, size / 2.0)
        }
        PrimitiveKind::Tetrahedron => {
            forge_geom::primitives::shapes::tetrahedron(center, size)
        }
        PrimitiveKind::Dodecahedron => {
            forge_geom::primitives::shapes::dodecahedron(center, size)
        }
    };

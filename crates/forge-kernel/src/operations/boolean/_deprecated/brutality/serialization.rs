...
fn topology_state_json_roundtrip() {
    let result = run_boolean([0.0, 0.0, 0.0], 2.0, [1.0, 0.0, 0.0], 2.0, BooleanOp::Union);
    ...
    assert_eq!(
        deserialized.arena().vertex_count(),
        original_v,
        "Vertex count mismatch after roundtrip"
    );
    assert_eq!(
        deserialized.arena().half_edge_count(),
        original_e,
        "Halfedge count mismatch after roundtrip"
    );
    assert_eq!(
        deserialized.arena().face_count(),
        original_f,
        "Face count mismatch after roundtrip"
    );
}

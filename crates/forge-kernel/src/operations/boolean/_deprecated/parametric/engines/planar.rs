...
use crate::operations::boolean::_deprecated::parametric::traits::{
...
impl BooleanSplitter for PlanarSplitter {
    fn split(
        ...
        crate::operations::boolean::_deprecated::parametric::split::split_all_faces(
            target_topo,
            target_geom,
            tool_topo,
            tool_geom,
...
impl CoplanarResolver for EmberCoplanarResolver {
    fn resolve_coplanars(
        ...
        crate::operations::boolean::_deprecated::ember::classify::apply_ember_coplanar_overrides(
            target_classified,
            tool_classified,
            target_topo,
            target_geom,
...
impl BooleanAssembler for HalfEdgeAssembler {
    fn assemble(
        ...
        crate::operations::boolean::_deprecated::parametric::assemble::merge::assemble_result(
            target_arena,
            target_geom,
            target_faces,
            target_prov,
...
impl BooleanPostprocessor for StandardPostprocessor {
    fn postprocess(
            ...
            == Some("1")
        {
            ...
            let (new_state, _) =
                crate::operations::boolean::_deprecated::parametric::postprocess::merge_coplanar_faces(
                    state, ctx,
                )?;
            new_state
        };
        ...
        let (new_state, _) =
            crate::operations::boolean::_deprecated::parametric::postprocess::remove_redundant_vertices(
                state, ctx,
            )?;
        let state = new_state;
        if std::env::var("FORGE_DEBUG_VALIDATE_PHASES").ok().as_deref() == Some("1") {

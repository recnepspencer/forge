...
fn all_inner_faces_classify_inside_outer() {
    for center in [[0.0, 0.0, 0.0], [0.1, 0.2, 0.3], [1.5, -0.5, 0.7]] {
        ...
        let inside_count = count_faces_inside(&topo_inner, &geom_inner, &topo_outer, &geom_outer);
...
fn count_faces_inside(
    ...
    source_topo
        ...
        .filter(|(fid, _fdata)| {
            ...
            let result =
                classify_point_in_solid(target_topo.arena(), &vertex_lookup, None, &centroid, &tol)
                    .unwrap();

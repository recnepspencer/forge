...
fn classify_overlapping_cubes_has_inside_faces() {
    ...
    assert!(
        inside_count > 0,
        "Expected some faces to be classified as Inside"
    );
}
...
fn intersection_of_concentric_cubes() {
    ...
    assert_eq!(
        ...
        0,
        "Outer faces should all be outside inner"
    );
    assert_eq!(
        bool_result.tool_faces_kept(),
        6,
        "Inner faces should all be inside outer"
    );
}
...
fn subtraction_of_concentric_cubes() {
    ...
    assert_eq!(
        bool_result.target_faces_kept(),
        6,
        "Outer cube kept as-is (zero-split containment)"
    );
    assert_eq!(
        bool_result.tool_faces_kept(),
        6,
        "Inner faces reversed to form hole (zero-split containment)"
    );
}
...
fn debug_split_counts_concentric() {
    ...
    eprintln!(
        "Before split: target faces={}, tool faces={}",
        topo_a.arena().face_count(),
        topo_b.arena().face_count()
    );
    ...
    eprintln!(
        "After split: target faces={}, tool faces={}",
    ...
    let target_inside = target_classified
        .iter()
        .filter(|f| f.classification() == FaceClassification::Inside)
        .count();
    let target_outside = target_classified
        .iter()
        .filter(|f| f.classification() == FaceClassification::Outside)
        .count();
    let target_boundary = target_classified
        .iter()
        .filter(|f| f.classification() == FaceClassification::OnBoundary)
        .count();
    let tool_inside = tool_classified
        .iter()
        .filter(|f| f.classification() == FaceClassification::Inside)
        .count();
    let tool_outside = tool_classified
        .iter()
        .filter(|f| f.classification() == FaceClassification::Outside)
        .count();
    let tool_boundary = tool_classified
        .iter()
        .filter(|f| f.classification() == FaceClassification::OnBoundary)
        .count();
    ...
    eprintln!(
        ...
        target_inside, target_outside, target_boundary
    );
    eprintln!(
        "Tool: inside={}, outside={}, boundary={}",
        tool_inside, tool_outside, tool_boundary
    );

    for (fid, _) in t_topo.arena().iter_faces() {
        let verts: Vec<_> = forge_topo::traverse::FaceEdgeIterator::new(t_topo.arena(), fid)
            .unwrap()
            .flat_map(|res| res.ok())
            .map(|he_id| {

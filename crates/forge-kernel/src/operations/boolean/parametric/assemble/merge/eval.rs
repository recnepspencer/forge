...
use crate::operations::boolean::parametric::traits::BooleanEngine;
use crate::operations::boolean::parametric::engines::planar::{planar_engine, planar_engine_parametric};
...
fn execute_boolean_pipeline(
    ...
    let _coincidence_graph = crate::shared_ops::coincidence::build_face_coincidence_prepass(
...
fn try_zero_split_early_return(
    ...
    let (result_topo, mut result_geom, result_brep) = ctx.scope("postprocess", |ctx| {
        let (rt_state, _) = crate::operations::boolean::parametric::postprocess::merge_coplanar_faces(
            crate::core::KernelState::new(result_topo, result_geom, result_brep), ctx
        )?;
        let (rt_state, _) = crate::operations::boolean::parametric::postprocess::remove_redundant_vertices(rt_state, ctx)?;

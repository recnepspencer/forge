...
fn fresh_cube_intersection() {
    ...
    let e = (arena.half_edge_count() / 2) as isize;
    let f = arena.face_count() as isize;
    assert_eq!(v - e + f, 2, "Intersection Euler violation: V={v} E={e} F={f}");
}
...
fn chained_union_union() {
    ...
    match execute_boolean_logged(input) {
        Ok(r) => {
            ...
            let e = (arena.half_edge_count() / 2) as isize;
            let f = arena.face_count() as isize;
            assert_eq!(v - e + f, 2, "Chained Union→Union Euler violation: V={v} E={e} F={f}");
        }
...
fn vertex_provenance_audit() {
    ...
    let mut unique_planes: Vec<(forge_geom::Plane, usize)> = Vec::new();
    ...
    eprintln!("  {} faces, {} vertices, {} unique planes", 
        arena.face_count(), arena.vertex_count(), unique_planes.len());
    
    for (fid, pidx) in &face_planes {
        let plane = geom_ab.get_face_plane(*fid).unwrap();
        let n = plane.normal();
        eprintln!("  F#{} -> plane {} normal=[{:.2}, {:.2}, {:.2}]",
            fid.index(), pidx, n[0], n[1], n[2]);
    }
    ...
    for (vid, _) in arena.iter_vertices() {
        ...
        if plane_set.len() >= 3 {
            ...
            eprintln!("  V#{}: [{:.2}, {:.2}, {:.2}] -> {} planes (faces: {}) ✓", 
                vid.index(), pos[0], pos[1], pos[2], plane_set.len(), faces_seen.join(","));
        } else {
            vertices_without.push(vid);
            eprintln!("  V#{}: [{:.2}, {:.2}, {:.2}] -> {} planes (faces: {}) ✗ NO PROVENANCE", 
                vid.index(), pos[0], pos[1], pos[2], plane_set.len(), faces_seen.join(","));
        }
    }
    
    eprintln!("  {}/{} vertices have provenance", vertices_with_provenance, arena.vertex_count());
...
fn split_classify_audit() {
    ...
    let split_result = split_all_faces(
        topo_ab, geom_ab, topo_c, geom_c, &mut ctx,
    ...
    eprintln!("  Target (A∪B) after split: V={} E={} F={}", 
        st.arena().vertex_count(),
        st.arena().half_edge_count() / 2,
        st.arena().face_count());
    eprintln!("  Tool (C) after split: V={} E={} F={}",
    ...
    for (fid, _) in st.arena().iter_faces() {
        if let Some(plane) = sg.get_face_plane(fid) {
            let n = plane.normal();
            eprintln!("  F#{}: n=[{:.2},{:.2},{:.2}] d={:.2}",
                fid.index(), n[0], n[1], n[2], plane.offset());
        }
    }
    ...
    let tc = classify_faces(
        st.arena(), &sg, tt.arena(), &tg, FaceOrigin::Target, &mut ctx,
    ).expect("classify target");
    let tlc = classify_faces(
        tt.arena(), &tg, st.arena(), &sg, FaceOrigin::Tool, &mut ctx,
    ...
    eprintln!("--- Selected target faces for Intersection: {} ---", sel_t.len());
    for fid in &sel_t {
        eprintln!("  F#{}", fid.index());
    }
    eprintln!("--- Selected tool faces for Intersection: {} ---", sel_tl.len());
    for fid in &sel_tl {
        eprintln!("  F#{}", fid.index());
    }
...
fn fresh_cube_intersection_same_dims() {
    ...
    match execute_boolean_logged(input) {
        Ok(r) => {
            ...
            assert_eq!(v - e + f, 2, "Fresh cube ∩ C Euler violation: V={v} E={e} F={f}");
        }
...
fn boolean_result_intersection() {
    ...
    eprintln!("  HE: {}/{} active/slots", arena.half_edge_count(), arena.half_edge_slot_count());
    eprintln!("  V:  {}/{} active/slots", arena.vertex_count(), arena.vertex_slot_count());
    eprintln!("  F:  {}/{} active/slots", arena.face_count(), arena.face_slot_count());

    // Print vertex positions
    for (vid, _vdata) in arena.iter_vertices() {
        if let Some(pos) = geom_ab.get_vertex_position(vid) {
            eprintln!("  V#{}: [{:.4}, {:.4}, {:.4}]", vid.index(), pos[0], pos[1], pos[2]);
        }
    }
    ...
    for (fid, _) in arena.iter_faces() {
        if let Some(plane) = geom_ab.get_face_plane(fid) {
            let n = plane.normal();
            eprintln!("  F#{}: normal=[{:.4}, {:.4}, {:.4}] offset={:.4}",
                fid.index(), n[0], n[1], n[2], plane.offset());
        }
    }
    ...
    match execute_boolean_logged(input) {
        Ok(r) => {
            ...
            assert_eq!(v - e + f, 2, "Boolean result ∩ C Euler violation: V={v} E={e} F={f}");
        }
...
fn chained_booleans_preserve_euler() {
    ...
    for (fid, fdata) in arena.iter_faces() {
        ...
        if arena.get_loop(loop_id).is_err() {
            eprintln!("  STALE Face F#{}(gen{}) -> Loop L#{}(gen{}): BAD",
                fid.index(), fid.generation(), loop_id.index(), loop_id.generation());
            stale_count += 1;
        }
    }
    ...
    for (lid, ldata) in arena.iter_loops() {
        ...
        if arena.get_half_edge(he).is_err() {
            eprintln!("  STALE Loop L#{}(gen{}) -> HE#{}(gen{}): BAD",
                lid.index(), lid.generation(), he.index(), he.generation());
            stale_count += 1;
        }
    }
    ...
    for (vid, vdata) in arena.iter_vertices() {
        ...
        if arena.get_half_edge(out).is_err() {
            eprintln!("  STALE Vertex V#{}(gen{}) -> HE#{}(gen{}): BAD",
                vid.index(), vid.generation(), out.index(), out.generation());
            stale_count += 1;
        }
    }

    assert_eq!(stale_count, 0, "First boolean result contains {} stale handles!", stale_count);

...
fn cube_far_from_origin() {
    let cfg = test_config();
    assert_valid_solid(
        make_cube([1e8, -1e8, 5e7], 2.0, &cfg).unwrap().get_value(),
        "cube_far",
    );
}
...
fn cube_large_scale() {
    let cfg = test_config();
    assert_valid_solid(
        make_cube([0.0; 3], 1e6, &cfg).unwrap().get_value(),
        "cube_large",
    );
}
...
fn cube_non_origin_center() {
    let cfg = test_config();
    assert_valid_solid(
        make_cube([3.7, -2.1, 8.9], 1.5, &cfg).unwrap().get_value(),
        "cube_offset",
    );
}
...
fn tetrahedron_far_from_origin() {
    let cfg = test_config();
    assert_valid_solid(
        make_tetrahedron([1e6, 1e6, 1e6], 1.0, &cfg)
            .unwrap()
            .get_value(),
        "tet_far",
    );
...
fn dodecahedron_scaled_large() {
    let cfg = test_config();
    assert_valid_solid(
        make_dodecahedron([0.0; 3], 100.0, &cfg)
            .unwrap()
            .get_value(),
        "dodec_large",
    );

...
fn compute_face_aabb(arena: &TopologyArena, geom: &GeometryStore, face: FaceId) -> Option<FaceAabb> {
    ...
    if found_any { Some(FaceAabb { min, max }) } else { None }
}
...
fn normals_anti_parallel(a: &forge_geom::Plane, b: &forge_geom::Plane) -> bool {
...
fn find_coplanar_pairs_exact(
    ...
    let tool_data: Vec<_> = tool_topo.arena().iter_faces()
        .filter_map(|(fid, _)| {
...
mod tests {
    ...
    fn detect_shared_face_between_touching_cubes() {
        ...
        let (excl_a, excl_b) = find_coplanar_pairs_exact(
            &topo_a, &geom_a, &topo_b, &geom_b,
        );

        assert!(excl_a.len() >= 1, "Target should have at least 1 excluded face (shared x=1 face)");
        assert!(excl_b.len() >= 1, "Tool should have at least 1 excluded face (shared x=1 face)");
    ...
    fn no_shared_faces_for_disjoint_cubes() {
        ...
        assert_eq!(excl_a.len(), 0, "Disjoint cubes should have no overlapping coplanars");
        assert_eq!(excl_b.len(), 0, "Disjoint cubes should have no overlapping coplanars");
    }
    ...
    fn exclusion_is_symmetric() {
        ...
        let (excl_ab_a, excl_ab_b) = find_coplanar_pairs_exact(
            &topo_a, &geom_a, &topo_b, &geom_b,
        );
        let (excl_ba_b, excl_ba_a) = find_coplanar_pairs_exact(
            &topo_b, &geom_b, &topo_a, &geom_a,
        );

        assert_eq!(excl_ab_a.len(), excl_ba_a.len(), "Symmetry: target exclusion count");
        assert_eq!(excl_ab_b.len(), excl_ba_b.len(), "Symmetry: tool exclusion count");
    }
}

...
pub fn merge_coplanar_faces(
    ...
    loop {
        let (new_topo, merged_count) = run_merge_pass(&current_topo, geom, &config)?;
        if merged_count == 0 {
            break;
        }
...
pub fn remove_redundant_vertices(
    ...
    loop {
        let (new_topo, count) = run_vertex_cleanup_pass(&current_topo, geom, &config)?;
        if count == 0 {
            break;
        }
        current_topo = new_topo;

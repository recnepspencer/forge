...
// use crate::boolean::eval::quantize_position; // Removed

fn is_same_point(a: [f64; 3], b: [f64; 3]) -> bool {
    let dx = a[0] - b[0];
    let dy = a[1] - b[1];
    let dz = a[2] - b[2];
...
fn remove_zero_length_edges(
    ...
    for (he_id, he_data) in draft.arena().iter_half_edges() {
        ...
        let origin_pos = geom.get_vertex_position(origin);
        let dest_pos = geom.get_vertex_position(dest);

        if let (Some(p1), Some(p2)) = (origin_pos, dest_pos) {
    ...
    for he_id in zero_edges {
        ...
        draft.arena_mut().get_half_edge_mut(prev_id)?.next = next_id;
        ...
        if let Ok(twin_data) = draft.arena().get_half_edge(twin_id) {
            ...
            if twin_prev != twin_id && twin_next != twin_id {
                draft.arena_mut().get_half_edge_mut(twin_prev)?.next = twin_next;
            }
        }
        ...
        if let Some(loop_id) = outer_loop_id {
            draft.arena_mut().get_loop_mut(loop_id)?.half_edge = next_id;
        }
        ...
        if let Some(vid) = outgoing_update {
            draft.arena_mut().get_vertex_mut(vid)?.outgoing = next_id;
        }
...
fn remove_degenerate_faces(
    ...
    for face_id in &faces {
        let edges = match face_edges(draft.arena(), *face_id) {
            Ok(e) => e,
            Err(_) => continue,
        };

        // Simply checking for duplicate positions loop
        let mut seen_points: Vec<[f64; 3]> = Vec::new();
        for he_id in &edges {
            if let Ok(he_data) = draft.arena().get_half_edge(*he_id) {
                if let Some(pos) = geom.get_vertex_position(he_data.origin) {
                    // check if pos is in seen_points
                    let mut found = false;
                    for seen in &seen_points {
                        if is_same_point(*pos, *seen) {
                            found = true;
                            break;
                        }
                    }
                    if !found {
                        seen_points.push(*pos);
                    }
                }
            }
        }

        if seen_points.len() < 3 {
            ...
            if let Ok(he_data) = draft.arena().get_half_edge(*he_id) {
                ...
                if let Ok(_twin_data) = draft.arena().get_half_edge(twin_id) {
                    ...
                    if let (Ok(prev_t), Ok(next_t)) = (prev_of_twin, next_of_twin_result) {
                        if prev_t != twin_id {
                            let _ = draft.arena_mut().get_half_edge_mut(prev_t).map(|d| d.next = next_t);
                        }
                    }
                }
                let _ = draft.arena_mut().remove_half_edge(*he_id);

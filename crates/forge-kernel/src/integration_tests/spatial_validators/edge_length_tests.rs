...
fn zero_length_detected() {
    ...
    let result = validate_zero_length_edges(
        arena,
        &|v| {
            if v == v0 || v == v1 { Some([5.0, 5.0, 5.0]) } else { None }
        },
        &tol,
    );
    assert!(result.is_err());
    match result.unwrap_err() {
        KernelError::TopologyViolation { err: TopologyError::ZeroLengthEdge { computed_length, .. }, .. } => {
...
fn sub_tolerance_length_detected() {
    ...
    let (_h0, v0, v1) = build_edge(&mut draft);
    let arena = draft.arena();
    let tol = MockToleranceProvider { default_tolerance: 1e-4 };

    let result = validate_zero_length_edges(
        arena,
        &|v| {
            if v == v0 { Some([0.0, 0.0, 0.0]) }
            else if v == v1 { Some([1e-5, 0.0, 0.0]) }
            else { None }
        },
        &tol,
    );
    assert!(result.is_err());
    match result.unwrap_err() {
        KernelError::TopologyViolation { err: TopologyError::ZeroLengthEdge { computed_length, threshold, .. }, .. } => {
...
fn self_loop_skipped() {
    ...
    let result = validate_zero_length_edges(
        arena,
        &|v| { if v == v0 { Some([1.0, 2.0, 3.0]) } else { None } },
        &tol,
    );
...
fn multiple_edges_one_zero() {
    ...
    let result = validate_zero_length_edges(
        ...
        &|v| {
            ...
            else if v == u0 { Some([5.0, 5.0, 5.0]) }
            else if v == u1 { Some([5.0, 5.0, 5.0]) }
            else { None }
        },
        &tol,
    );

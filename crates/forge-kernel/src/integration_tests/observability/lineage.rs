...
use crate::integration_tests::harness::shapes::{
    collect_face_loop, first_halfedge_of_face, tetrahedron, unit_cube,
};
...
fn test_make_block_lineage_covers_all_entities() {
    ...
    assert!(
        !events.is_empty(),
        "Lineage events must be non-empty after build"
    );
...
fn test_lineage_event_log_all_creations() {
    ...
    for (i, event) in events.iter().enumerate() {
        assert!(
            ...
            "Event {} should be EntityCreated, got: {:?}",
            i,
            event
        );
    }
...
fn test_lineage_root_contains_operation_attribution() {
    ...
    let lineage = store
        .get_lineage(&face_ref)
        .expect("face should have lineage");
    ...
    let (vtx_id, _) = arena
        .iter_vertices()
        .next()
        .expect("cube should have vertices");
    let vtx_ref = EntityRef::new(EntityKind::Vertex, vtx_id.index(), vtx_id.generation());
    let vtx_lineage = store
        .get_lineage(&vtx_ref)
        .expect("vertex should have lineage");
...
fn test_euler_operator_produces_lineage() {
    ...
    let parent_hash = draft
        .lineage_store()
        .get_lineage(&parent_he_ref)
        .expect("Parent halfedge must have lineage")
        .get_ancestry_hash();
    ...
    let vtx_ref = EntityRef::new(
        EntityKind::Vertex,
        se.new_vertex.index(),
        se.new_vertex.generation(),
    );
    let vtx_lineage = draft
        .lineage_store()
        .get_lineage(&vtx_ref)
    ...
    let parent_face_hash = draft
        .lineage_store()
        .get_lineage(&parent_face_ref)
        .expect("Parent face must have lineage")
        .get_ancestry_hash();
    ...
    let new_face_ref = EntityRef::new(
        EntityKind::Face,
        mef.new_face.index(),
        mef.new_face.generation(),
    );
    let mef_lineage = draft
        .lineage_store()
        .get_lineage(&new_face_ref)
...
fn test_lineage_integrity_survives_multi_step_chain() {
    ...
    for eref in store.tracked_entities() {
        ...
        assert!(
            hashes.insert(hash),
            "Duplicate ancestry hash {:?} on entity {:?}",
            hash,
            eref
        );
    ...
    for event in events {
        if let LineageEvent::EntityCreated { lineage, .. } = event {
            assert!(
                ...
                "Entity created with no origin features: event {:?}",
                event
            );
            for &fid in lineage.get_origin_features() {
                assert_ne!(
                    fid, FEATURE_ID_UNSET,
                    "Entity created with FEATURE_ID_UNSET: event {:?}",
                    event
                );
            }
        }

...
fn split_then_rejoin_is_identity() {
    ...
    let face_valence = collect_face_loop(
        draft.arena(),
        first_halfedge_of_face(draft.arena(), jf.surviving_face).unwrap(),
    )
    .unwrap()
    .len();
...
fn join_two_adjacent_cube_faces() {
    ...
    let jf = draft
        .execute(JoinFaces { edge: start_he })
        .unwrap()
        .into_value();
...
fn split_mef_then_join_neighbor() {
    ...
    assert!(
        neighbor_he.is_some(),
        "New face must have a neighbor other than original"
    );
    let neighbor_he = neighbor_he.unwrap();

    let _jf = draft
        .execute(JoinFaces { edge: neighbor_he })
        .unwrap()
        .into_value();
    ...
    assert_eq!(
        ...
        6,
        "JoinFaces removes 1 face (back to 6)"
    );

...
fn kfmrh_then_mekl_bridges_hole() {
    ...
    let inner_loops_before = draft.arena().get_face(target_face).unwrap().inner_loops().len();
    ...
    assert_eq!(draft.arena().face_count(), face_count_before - 1, "KFMRH removes 1 face");
    let inner_loops_after = draft.arena().get_face(target_face).unwrap().inner_loops().len();
    ...
    let inner_loops_after_mekl = draft.arena().get_face(target_face).unwrap().inner_loops().len();
    ...
    assert_eq!(
        merged_valence, outer_valence + inner_valence + 2,
...
fn mekl_rejects_cross_face_halfedges() {
    ...
    assert!(result.is_err(), "MEKL should reject halfedges on different faces");
}
...
fn mekl_keml_roundtrip() {
    ...
    assert_eq!(
        draft.arena().get_face(target_face).unwrap().inner_loops().len(), 1,

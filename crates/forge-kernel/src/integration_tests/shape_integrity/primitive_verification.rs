...
fn cube_topology_and_volume() {
    let env = shapes::unit_cube()
        .expect("unit cube should succeed")
        .into_value();
    verify(&env)
        .named("cube")
        .faces(6)
...
fn tetrahedron_topology() {
    let env = shapes::tetrahedron()
        .expect("tetrahedron should succeed")
        .into_value();
    verify(&env)
        .named("tetrahedron")
        .faces(4)
...
fn dodecahedron_topology() {
    let env = shapes::dodecahedron([0.0; 3], 1.0)
        .expect("dodecahedron should succeed")
        .into_value();
    verify(&env)
        .named("dodecahedron")
        .faces(12)
...
fn triangular_prism_topology() {
    let env = shapes::prism([0.0; 3], 3, 1.0, 2.0)
        .expect("prism should succeed")
        .into_value();
    verify(&env)
        .named("prism_3")
        .faces(5)
...
fn hexagonal_prism_topology() {
    let env = shapes::prism([0.0; 3], 6, 1.0, 2.0)
        .expect("prism should succeed")
        .into_value();
    verify(&env)
        .named("prism_6")
...
fn block_non_uniform_topology_and_volume() {
    let env = shapes::block([0.0; 3], [1.0, 2.0, 3.0])
...
fn wedge_topology_and_volume() {
    let env = shapes::wedge([0.0; 3], [1.0, 1.0, 1.0])
        .expect("wedge should succeed")
        .into_value();
    verify(&env)
        .named("wedge")
        .faces(6)
...
fn triangular_pyramid_topology() {
    let env = shapes::pyramid([0.0; 3], 3, 1.0, 2.0)
        .expect("pyramid should succeed")
        .into_value();
    verify(&env).named("pyramid_3").shells(1).bodies(1).pass();
}
...
fn quad_pyramid_topology_and_volume() {
    let env = shapes::pyramid([0.0; 3], 4, 1.0, 2.0)
        .expect("pyramid should succeed")
        .into_value();
    verify(&env)
        .named("pyramid_4")
...
fn large_cube_scale_invariant() {
    let env = shapes::cube([0.0; 3], 100.0)
        .expect("large cube should succeed")
        .into_value();
    verify(&env)
        .named("large_cube")
        .faces(6)
...
fn offset_cube_position_invariant() {
    let env = shapes::cube([500.0, 500.0, 500.0], 1.0)
        .expect("offset cube should succeed")
        .into_value();
    verify(&env)
        .named("offset_cube")
        .faces(6)
...
fn zero_size_cube_rejected() {
    ...
    assert!(
        result.is_err(),
        "Zero-size cube should be rejected by validator"
    );
}
...
fn negative_size_cube_rejected() {
    ...
    assert!(
        result.is_err(),
        "Negative-size cube should be rejected by validator"
    );
}
...
fn nan_center_cube_rejected() {
    ...
    assert!(
        result.is_err(),
        "NaN center should be rejected by validator"
    );
}
...
fn inf_center_cube_rejected() {
    ...
    assert!(
        result.is_err(),
        "Infinity center should be rejected by validator"
    );
}
...
fn zero_extent_block_rejected() {
    ...
    assert!(
        result.is_err(),
        "Zero half-extent should be rejected by validator"
    );
}
...
fn prism_two_sides_rejected() {
    ...
    assert!(
        result.is_err(),
        "Prism with 2 sides should be rejected by validator"
    );
}
...
fn prism_zero_radius_rejected() {
    ...
    assert!(
        result.is_err(),
        "Zero-radius prism should be rejected by validator"
    );
}
...
fn prism_zero_height_rejected() {
    ...
    assert!(
        result.is_err(),
        "Zero-height prism should be rejected by validator"
    );
}

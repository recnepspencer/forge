...
use crate::integration_tests::harness::comparison::{
    analytical_reference, assert_geo_eq, assert_geo_eq_3d, exact_planar, volume_invariance,
    ComparisonPolicy,
};
use crate::integration_tests::harness::oracles::{centroid_of, volume_of};
...
fn volume_scales_cubically() {
    let small = shapes::cube([0.0; 3], 2.0)
        .expect("cube(2) failed")
        .into_value();
    let big = shapes::cube([0.0; 3], 4.0)
        .expect("cube(4) failed")
        .into_value();
    ...
    assert_geo_eq(
        ...
        8.0,
        &policy,
        "cube volume should scale cubically (4/2)^3 = 8",
    );
}

#[test]
fn prism_volume_linear_in_height() {
    let short = shapes::prism([0.0; 3], 6, 3.0, 1.0)
        .expect("prism(h=1) failed")
        .into_value();
    let tall = shapes::prism([0.0; 3], 6, 3.0, 2.0)
        .expect("prism(h=2) failed")
        .into_value();
    ...
    assert_geo_eq(
        ...
        2.0,
        &policy,
        "prism volume should be linear in height",
    );
}

#[test]
fn volume_translation_invariant() {
    let at_origin = shapes::cube([0.0; 3], 3.0)
        .expect("cube origin failed")
        .into_value();
    let far_away = shapes::cube([1e6, 1e6, 1e6], 3.0)
        .expect("cube far failed")
        .into_value();
    ...
    assert_geo_eq(
        ...
        vol_far,
        &policy,
        "volume must be translation-invariant",
    );
}

#[test]
fn block_volume_matches_analytical() {
    let env = shapes::block([0.0; 3], [5.0, 5.0, 5.0])
        .expect("block failed")
        .into_value();
    ...
    assert_geo_eq(
        ...
        1000.0,
        &policy,
        "block [5,5,5] volume should be 10*10*10=1000",
    );
}
...
fn volume_of_complex_solid_is_positive_and_consistent() {
    ...
    let env_origin = shapes::dodecahedron([0.0; 3], 2.0)
        .expect("dodecahedron origin failed")
        .into_value();
    let env_offset = shapes::dodecahedron([15.0, -20.0, 50.0], 2.0)
        .expect("dodecahedron offset failed")
        .into_value();
    ...
    assert!(
        vol_origin > 10.0,
        "Dodecahedron volume must be significantly positive"
    );
    ...
    assert_geo_eq(
        ...
        vol_offset,
        &policy,
        "Volume of complex solid must be invariant under translation",
    );
}
...
fn centroid_of_offset_cube() {
    let env = shapes::cube([3.0, 5.0, 7.0], 2.0)
        .expect("cube failed")
        .into_value();
    let center = centroid_of(&env).expect("centroid_of failed");
    let policy = analytical_reference();
...
fn centroid_of_block_at_origin() {
    let env = shapes::block([0.0; 3], [2.0, 3.0, 4.0])
        .expect("block failed")
        .into_value();
    let center = centroid_of(&env).expect("centroid_of failed");
    let policy = exact_planar();
...
fn centroid_of_symmetric_tetrahedron_near_center() {
    ...
    assert_geo_eq_3d(
        ...
        expected,
        &policy,
        "tetrahedron centroid must match exact average of vertices",
    );
}
...
fn volume_is_positive_for_all_primitives() {
    let shapes: Vec<(&str, _)> = vec![
        ...
        (
            "dodecahedron",
            shapes::dodecahedron([0.0; 3], 2.0).expect("dodec"),
        ),
        (
            "hex_prism",
            shapes::prism([0.0; 3], 6, 3.0, 4.0).expect("prism"),
        ),
        (
            "pyramid",
            shapes::pyramid([0.0; 3], 4, 3.0, 4.0).expect("pyramid"),
        ),
        (
            "wedge",
            shapes::wedge([0.0; 3], [2.0, 3.0, 4.0]).expect("wedge"),
        ),
    ];

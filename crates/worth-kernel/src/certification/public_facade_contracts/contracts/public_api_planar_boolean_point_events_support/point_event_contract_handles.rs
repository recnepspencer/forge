    ...
    ($name:ident, $domain:expr, $world:expr, $domain_ty:ty, $world_ty:ty) => {
        pub(crate) fn $name(
            ...
            ForgeQueryApplicationFacade::runtime_backed_default()
                ...
                .validate()
                .expect("validated point-event contract domain")
                .admit()
                .expect("admitted point-event contract domain")
        }
    };
}
...
> {
    ForgeQueryApplicationFacade::runtime_backed_default()
        ...
        .with_operating_context(PlanarPredicateAuthorityQueryWorld::new("point-event"))
        .validate()
        .expect("validated point-event predicate domain")
        .admit()
        .expect("admitted point-event predicate domain")
}

...
fn run_with_large_stack(body: impl FnOnce() + Send + 'static) {
    std::thread::Builder::new()
        .name("workload-catalog-nmt-construction".to_string())
        .stack_size(16 * 1024 * 1024)
        .spawn(body)
        .expect("NMT construction catalog contract thread should spawn")
        .join()
        .expect("NMT construction catalog contract thread should finish");
}

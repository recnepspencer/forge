...
pub(crate) fn assert_foreign_split_authorities_are_rejected(
    ...
    assert_foreign_spatial_lookup_product_is_rejected(
        replay_subject,
        replay_report,
        completed_split_handoff,
        &foreign_completed_split_handoff,
...
fn assert_foreign_decision_log_is_rejected(
    ...
    let (authority, lookup) = spatial_touch_authority_and_lookup(
        completed_split_handoff,
...
fn assert_foreign_spatial_touch_authority_is_rejected(
    ...
    let (foreign_authority, foreign_lookup) = spatial_touch_authority_and_lookup(
        foreign_completed_split_handoff,
...
fn assert_foreign_spatial_lookup_product_is_rejected(
    replay_subject: &EdgeSplitReplayParitySubject,
    replay_report: &PlanarBooleanEdgeSplitReplayParityReport,
    completed_split_handoff: &CompletedBooleanSplitHandoff,
    foreign_completed_split_handoff: &CompletedBooleanSplitHandoff,
) {
    let (authority, _) = spatial_touch_authority_and_lookup(
        completed_split_handoff,
        replay_subject.original_ledger.receipt(),
    );
    let (_, foreign_lookup) = spatial_touch_authority_and_lookup(
        foreign_completed_split_handoff,
        foreign_completed_split_handoff.split_ledger_receipt(),
    );
    let denial = PlanarBooleanDownstreamSplitConsumption::admit(
    ...
    .expect_err("foreign spatial lookup product must not certify downstream split consumption");
    assert_eq!(
        denial.kind(),
        PlanarBooleanDownstreamSplitConsumptionDenialKind::ForeignSpatialLookupProduct
    );
}

fn spatial_touch_authority_and_lookup(
    ...
    let lookup = authority
        ...
        .expect("completed split handoff should admit spatial evidence lookup");
    (authority, lookup)
}

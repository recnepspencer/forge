...
use workload_evidence_support::{
    assert_split_handoff_rejects_foreign_event_ledger_lookup_packet,
...
#[test]
fn boolean_split_handoff_rejects_foreign_event_ledger_lookup_packets() {
    reduced_pair_support::run_with_large_stack(|| {
        let subject =
            MetabossEventExtractionSubject::certify("phase11 foreign split lookup packet denial");
        assert_split_handoff_rejects_foreign_event_ledger_lookup_packet(&subject);
    });
}

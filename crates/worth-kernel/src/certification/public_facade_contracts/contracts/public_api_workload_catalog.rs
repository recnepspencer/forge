...
mod tests {
    ...
    #[path = "boolean_anti_theatre.rs"]
    mod boolean_anti_theatre;
    #[path = "boolean_operand_pairs.rs"]
    mod boolean_operand_pairs;
    ...
    fn workload_catalog_recipes_emit_complete_evidence_ledgers() {
        run_with_large_stack(|| {
            for recipe in admitted_catalog_recipes() {
                ...
                assert_catalog_query_receipt_is_digest(
                    built.declaration().query_declaration_digest(),
                );
    ...
    fn coplanar_overlap_storm_topology_breadth_is_explicit_and_receipt_backed() {
        run_with_large_stack(|| {
            ...
            let smaller_storm = catalog_breadth(
                WorkloadCatalog::coplanar_overlap_storm().with_topology_breadth(
    ...
    fn run_with_large_stack(body: impl FnOnce() + Send + 'static) {
        std::thread::Builder::new()
            .name("workload-catalog-contract".to_string())
            .stack_size(16 * 1024 * 1024)
            .spawn(body)
            .expect("workload catalog contract thread should spawn")
            .join()
            .expect("workload catalog contract thread should finish");
    }
}

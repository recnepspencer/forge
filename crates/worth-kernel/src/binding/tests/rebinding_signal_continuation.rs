...
#[test]
fn rebinding_signal_compatibility_and_continuation_are_real_runtime_lanes() {
    ...
    match prepared_checked.outcome() {
        ...
        ForgeQueryPreparedContinuationOutcome::Denied(_) => panic!("continuation: denied"),
        ForgeQueryPreparedContinuationOutcome::Failed(_) => panic!("continuation: failed"),
    };
    ...
    let prepared = match handle.prepare_continuation_from_target(
        ...
        ForgeQueryPreparedContinuationOutcome::Denied(_) => panic!("owned continuation: denied"),
        ForgeQueryPreparedContinuationOutcome::Failed(_) => panic!("owned continuation: failed"),
    };

    let execution_checked = handle.execute_prepared_continuation_checked(prepared);

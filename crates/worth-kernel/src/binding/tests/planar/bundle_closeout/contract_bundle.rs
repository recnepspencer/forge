...
#[test]
fn kernel_consumes_planar_contract_bundle_without_boolean_or_imprint_synthesis() {
    let readiness = readiness_receipt();
    assert!(readiness.is_ready_for_m7());
    assert_eq!(readiness.boolean_result(), None);
    assert_eq!(readiness.imprint_action(), None);
    assert_eq!(readiness.basis().topology_basis_identity(), TOPOLOGY);
    assert_eq!(
        readiness.basis().movement_rotation_posture_identity(),
        MOVEMENT
    );
    assert_eq!(readiness.basis().family_rows().len(), 11);
    assert!(!readiness.fact_digest().is_empty());
}
...
pub(crate) fn readiness_receipt() -> PlanarContractBundleValidationReceipt {
    ...
    let bundle = PlanarBooleanReadinessBundle::builder()
        ...
        .topology_contract(super::topology_fixture::kernel_topology_contract_receipt())
        .precision(precision)
        .local_frame(frame)

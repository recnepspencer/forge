...
pub enum PlannerOwnedRoutingQueryGapKind {
    MissingArtifact,
    UnsupportedPublicPath,
    MissingBoundaryExposure,
    MissingIdentitySemantics,
}
...
impl PlannerOwnedRoutingDisplacedLane {
    ...
    pub const fn path(self) -> &'static str {
        match self {
            ...
            Self::TopoDiagnosticSurfaces => "crates/worth-topo/src/projection/diagnostic_surfaces/",
            Self::TopoQueryBackedConsumerCutover => {
...
impl PlannerOwnedRoutingReplacementLane {
    ...
    pub const fn path(self) -> &'static str {
        match self {
            ...
            Self::TopoQueryBackedReadFamily => {
                "crates/worth-topo/src/projection/planner_owned_routing/query_backed_read_family/"
            }
            Self::TopoInvalidationRoute => {
                "crates/worth-topo/src/projection/planner_owned_routing/invalidation_route/"
            }
            Self::TopoDiagnosticProjectionInput => {
                "crates/worth-topo/src/projection/planner_owned_routing/diagnostic_projection_input/"
            }
            Self::SpatialEvidenceLookupRoute => {
                "crates/worth-spatial/src/workload_platform/planner_owned_routing/evidence_lookup_route/"
            }
            Self::SpatialPublicCloseoutRoute => {

...
use crate::workload_composition::{trace_note, trace_scope, WorthWorkload, WorthWorkloadParts};
...
pub(crate) fn build_catalog_workload(
    ...
    trace_scope("build_catalog_workload", || {
        let topology = trace_scope("catalog_workload_topology_seed", || {
            build_topology_seed(
                recipe,
                declaration,
                topology_breadth,
        ...
        let bound_geometry = trace_scope("catalog_workload_bind_geometry", || {
            bind_seed_geometry(&topology, declaration, planar_loop_boundary_profile)
        })?;
        let bound_geometry_for_catalog = bound_geometry.clone();
        let geometry_receipts = bound_geometry.receipts().clone();
        let surface_support = trace_scope("catalog_workload_surface_support", || {
            certify_surface_support(bound_geometry, declaration)
        })?;
        let support_receipts = surface_support.receipts().clone();
        let projected = trace_scope("catalog_workload_projection", || {
            project_supported_geometry(surface_support.clone(), declaration)
        })?;
        let projection_receipts = projected.receipts().clone();
        let transformed = trace_scope("catalog_workload_transform", || {
        ...
        let retained_replay = trace_scope("catalog_workload_retained_replay", || {
        ...
        let diagnostics = trace_scope("catalog_workload_diagnostics", || {
            DiagnosticWorkload::for_retained_replay(retained_replay.stage_receipt())
        ...
        let response = trace_scope("catalog_workload_response", || {
            ResponseWorkload::for_diagnostics(&diagnostics)
        ...
        let evidence_ledger = trace_scope("catalog_workload_evidence_ledger", || {
            complete_catalog_evidence_ledger(CatalogEvidenceParts {
                topology: &topology,
        ...
        let workload = trace_scope("catalog_workload_compose", || {
...
fn bind_seed_geometry(
    ...
    let planar_faces = trace_scope("bind_seed_geometry_planar_faces", || {
        PlanarFaceCarrierSet::for_seed_faces(topology)
    });
    let planar_edges = trace_scope("bind_seed_geometry_planar_edges", || {
        PlanarEdgeCarrierSet::for_seed_edges(topology)
    });
    let planar_loops = trace_scope("bind_seed_geometry_planar_loops", || {
        PlanarLoopCarrierSet::for_seed_loops_with_profile(topology, planar_loop_boundary_profile)
    });
    trace_note(format!(
        "bind geometry counts faces={} edges={} loops={}",
        planar_faces.faces().len(),
        planar_edges.edges().len(),
        planar_loops.loops().len()
    ));
    trace_scope("bind_seed_geometry_admit", || {
        GeometryBindingWorkload::for_topology_seed(topology)
            .declared(format!("bind catalog geometry for {declaration}"))
            .with_planar_faces(planar_faces)
            .with_planar_edges(planar_edges)
            .with_planar_loops(planar_loops)
            .admit()
            .map_err(WorkloadCatalogError::from)
    })
}
...
fn certify_surface_support(
    ...
    trace_scope("surface_support_certify_plane", || {
...
fn project_supported_geometry(
    ...
    trace_scope("project_supported_geometry_to_certified_plane", || {
...
fn transform_projected_geometry(
    ...
    trace_scope("transform_projected_geometry_sequence", || {
...
fn replay_transformed_geometry(
    ...
    match retained_replay_recipe {
        RetainedReplayRecipe::StageReceiptOnly => {
            trace_scope("retained_replay_stage_only_admit", || {
        ...
        RetainedReplayRecipe::RetainedCancellationChain => {
            let captured = trace_scope("retained_replay_capture_canonical_chain", || {
                canonical_retained_cancellation_chain_capture(
                    "workload-catalog-retained-cancellation-chain",
                )
            })?;
            let admitted_capture = trace_scope("retained_replay_admit_capture", || {
                admit_retained_replay_capture(captured)
            });
            let replayed = trace_scope("retained_replay_replay_workload", || {
                ReplayWorkload::for_transformed_workload(transformed)
                    .declared(format!("catalog retained replay for {declaration}"))
                    .with_admitted_retained_replay_capture(admitted_capture)
                    .replay()
            })?;
...
> {
    let rows = trace_scope("catalog_evidence_rows_from_receipts", || {
        vec![
    ...
    trace_note(format!("catalog evidence row count={}", rows.len()));
    let ledger = trace_scope("catalog_evidence_ledger_index_rows", || {
        WorkloadEvidenceLedger::from_rows(rows)
    })?;
    trace_scope("catalog_evidence_ledger_certify_complete", || {
        ledger
            .certify_complete()
            .map_err(WorkloadCatalogError::from)
    })

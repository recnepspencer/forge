...
use crate::workload_composition::public_closeout::{
    build_architecture_alignment_report, WorthTouchedGraphConflictMilestoneFifteenSeed,
...
use crate::workload_composition::{
    WorthTouchedGraphConflictAdmittedPublicProofInput, WorthTouchedGraphConflictProofChain,
...
{
    let assembly_input = WorthTouchedGraphConflictPublicProofAssemblyInput::new(
        input,
        cutover,
        selected_route_packet,
        admitted_public_proof_input,
    )?;
    let residue_chain = assembly_input.residue_chain();
    ...
    let proof_chain =
        WorthTouchedGraphConflictProofChain::from_selected_route_packet(assembly_input.selected_route_packet());
    if proof_chain.selected_conflict_plan_digests().is_empty()
        ...
        || proof_chain.evidence_lookup_public_closeout_digest().is_empty()
        || proof_chain

...
pub(crate) fn row_digest(
    ...
    Ok(digest_owned_parts(&[
        ...
        row.current_head_lane().lane_digest().to_string(),
        row.branch_local_lane().lane_digest().to_string(),
        row.replay_lane().lane_digest().to_string(),
    ...
    for lane in PrimitiveConstructionCorpusAuthoringOrderLane::all() {
        ...
        rows.push((
            ...
            normalized_matrix_digest(
                lane_rows
                    .iter()
                    .map(|row| (row.scenario_id().to_string(), row_parity_digest(row))),
            ),
        ));
    }
    Ok(rows)
...
fn row_parity_digest(row: &PrimitiveConstructionCorpusReplaySiegeRow) -> String {
    let runtime_truth = row.runtime_truth();
    digest_owned_parts(&[
        row.scenario_id().to_string(),
        row.family().as_str().to_string(),
        ...
        runtime_truth_kind(runtime_truth).to_string(),
        row.current_head_lane().lane_digest().to_string(),
        row.branch_local_lane().execution_gap().code().to_string(),
        row.replay_lane().replay_gap().code().to_string(),
        admitted_runtime_truth(runtime_truth)
            .map(PrimitiveConstructionAdmittedRuntimeTruth::birth_truth_digest)
            .unwrap_or_default()
            .to_string(),

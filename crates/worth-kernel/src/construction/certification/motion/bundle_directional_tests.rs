...
use crate::facade::diagnostics::motion::PrimitiveConstructionMotionRuntimeSurfaceStatus;
...
#[test]
fn motion_report_bundle_supports_directional_reorient_anchor_paths() {
    let mut shape_u_workspace = workspace("worth-kernel.motion-bundle.directional.shape-u");
    let shape_u_bundle = super::prepare_primitive_construction_reorient_motion_report_bundle(
        &mut shape_u_workspace,
        shape_u_intent(),
    )
    .expect("shape-u reorient bundle");
    let mut shape_v_workspace = workspace("worth-kernel.motion-bundle.directional.shape-v");
    let shape_v_bundle = super::prepare_primitive_construction_reorient_motion_report_bundle(
        &mut shape_v_workspace,
        shape_v_intent(),
    )
    .expect("shape-v reorient bundle");
    let mut frame_workspace = workspace("worth-kernel.motion-bundle.directional.frame-axis");
    let frame_axis_bundle = super::prepare_primitive_construction_reorient_motion_report_bundle(
        &mut frame_workspace,
        frame_axis_intent(),
    )
    .expect("frame-axis reorient bundle");
    let mut direct_shape_u_workspace =
        workspace("worth-kernel.motion-bundle.directional.shape-u.direct");
    ...
    let mut direct_shape_v_workspace =
        workspace("worth-kernel.motion-bundle.directional.shape-v.direct");
    ...
    let mut direct_frame_workspace =
        workspace("worth-kernel.motion-bundle.directional.frame-axis.direct");

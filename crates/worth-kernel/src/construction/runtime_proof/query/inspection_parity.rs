...
impl PrimitiveConstructionQueryInspectionParityReport {
    fn new(
        ...
        let fact_report = prepared.evidence().topology_fact_report();
        let realization_truth =
            PrimitiveConstructionRuntimeRealizationTruth::from_artifact(artifact);
        let parity_verified =
            fact_report.required_query_families() == [ForgeQueryRuntimeFacadeFamily::Inspect]
                && artifact.inspection_surface() == fact_report.inspection_surface()
                && artifact.topology_fact_digest() == fact_report.report_digest()
                && fact_report.read_surface()
                    == TopologyConstructionCertificationReadSurface::ProjectionConsumptionFromInspectionReceipt
                && fact_report.provenance()
        ...
        let report_digest = digest_owned_parts(&[
            ...
            fact_report
                ...
                .collect::<Vec<_>>()
                .join("|"),
            fact_report.read_surface().as_str().to_string(),
            artifact.inspection_surface().as_str().to_string(),
            fact_report.provenance().as_str().to_string(),
        ...
        Self {
            ...
            required_query_families: fact_report.required_query_families().to_vec(),
            read_surface: fact_report.read_surface(),
            inspection_surface: artifact.inspection_surface(),
            fact_provenance: fact_report.provenance(),
...
> {
    ...
    let prepared = prepare_primitive_construction_result(intent)
        .map_err(PrimitiveConstructionQueryInspectionParityError::Result)?;

...
mod tests {
    ...
    fn export_record_serializes_json() {
        let text = export_record(&Example { name: "WORTH" }, ExportFormat::JsonCompact).unwrap();
        assert!(text.contains("WORTH"));
    }
    ...
    fn record_archive_collects_named_exports() {
        ...
        archive
            ...
            .unwrap();
        archive.insert_text_asset("report", "reports/report.txt", "text/plain", "WORTH");
        ...
        assert!(archive.records.get("example").unwrap().contains("WORTH"));

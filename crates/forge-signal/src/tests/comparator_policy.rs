...
fn exact_comparator_detects_any_change() {
    ...
    let mut compute = |_id: NodeId, _g: &SignalGraph| Ok(AspectVersion::new(0, 1));
...
fn tolerance_comparator_skips_small_version_delta() {
    ...
    let mut compute_a_v10 = |_id: NodeId, _g: &SignalGraph| Ok(AspectVersion::new(0, 10));
    let mut compute_a_v12 = |_id: NodeId, _g: &SignalGraph| Ok(AspectVersion::new(0, 12));
    let mut compute_b = |_id: NodeId, _g: &SignalGraph| Ok(AspectVersion::new(0, 100));
    ...
    let mut compute_c = |_id: NodeId, _g: &SignalGraph| Ok(AspectVersion::new(0, 1_000));

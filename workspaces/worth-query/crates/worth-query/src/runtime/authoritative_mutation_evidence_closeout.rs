...
use super::{
    WorthQueryAuthoritativeMutationEvidenceSupport, WorthQueryMutationSurfaceReport,
    WorthQueryRuntimeBackendPosture, WorthQueryRuntimePublicApiNamingContract,
    WorthQueryRuntimePublicSupportMatrix,
};

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorthQueryAuthoritativeMutationEvidenceCloseout {
...
impl WorthQueryAuthoritativeMutationEvidenceCloseout {
    pub fn derive(
        backend_posture: WorthQueryRuntimeBackendPosture,
        ...
        query_support: &WorthQueryAuthoritativeMutationEvidenceSupport,
        ...
        let safe_to_build_now = vec![
            ...
            "installed application invariant execution belongs to provider-session progression; generic graph composition exposes no caller-authored invariant callback or no-op completion lane".to_string(),
            "contributed domain-capability invariant denials expose attempted-shape summaries with declared collections, declared symbols, capability families, and lifecycle families without granting graph-execution authority".to_string(),
        ...
        let migration_guidance = vec![
            ...
            "use `workspace.compose_graph(...)` when one logical mutation needs symbolic resolution, verified preconditions, or lineage as part of the ordinary receipt story; installed application operations reach invariant authority only through provider-session progression".to_string(),
            ...
            "delete local graph-program rejection reconstruction once `admission_trace()` and contributed-capability `domain_invariant_summary()` cover the non-executable denied-path explanation contract".to_string(),
        ...
        let required_verification_commands = vec![
            ...
            "cargo fmt -p worth-runtime-bridge".to_string(),
            "cargo check -p worth-runtime-bridge --tests".to_string(),
            "cargo test -p worth-runtime-bridge".to_string(),
            "cargo test --manifest-path crates/worth-runtime-bridge/Cargo.toml --test phase_boundaries_compile_fail".to_string(),
            "git diff --check".to_string(),
        ];
        let mut parts = vec![
            "worth_query_authoritative_mutation_evidence_closeout_v1".to_string(),

...
use crate::runtime::{WorthQueryGraphReadPolicyTenantAuthorityRequest, WorthQuerySessionLabel};
...
pub fn policy_tenant_authority_request(
    ...
    let branch = BranchAccessGrant::synthetic_granted("branch-a", &policy);
    let schema = phase_fourteen_schema(tenant_label);
    WorthQueryGraphReadPolicyTenantAuthorityRequest::current_read(
        canonical.query(),
        policy,
        tenant,
        branch,
...
pub fn session_label(name: &str) -> WorthQuerySessionLabel {
    WorthQuerySessionLabel::scoped_strs("graph-read-access-phase-fourteen", [name])

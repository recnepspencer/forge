...
pub(super) fn collection_registration(
    ...
    WorthQueryGraphObligationRegistration::schema_contract_validator(
        WorthQueryGraphObligationRuleIdentity::new("test.preview-branch-obligation", label, "v1")
            .unwrap(),
...
pub(super) fn touch_bearing_intent(
    ...
    WorthQueryTouchBearingIntentDeclaration::new(plain_intent(name), descriptor)
}

pub(super) fn touch_bearing_intent_declaration(name: &str) -> WorthQueryIntentDeclaration {
    touch_bearing_intent(name, task_touch_descriptor(name)).into_declaration()
}

pub(super) fn plain_intent(name: &str) -> WorthQueryIntentDeclaration {
...
pub(super) fn task_touch_descriptor(id: &str) -> WorthQueryGraphTouchDescriptor {
    WorthQueryGraphTouchDescriptor::from_mutation_command_batch(&[task_insert_command(id)])

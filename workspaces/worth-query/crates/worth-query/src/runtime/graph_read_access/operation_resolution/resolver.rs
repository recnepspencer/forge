...
pub(crate) fn resolve_graph_read_operations_for_read_graph(
    ...
    if let Some(declaration) = read_graph.domain_graph_operations().first() {
        let installed_authority = read_graph.installed_operation_authority(declaration);
        let declared_relation_names = declared_operation_relation_names(declaration);
        if let Some(denial) =
            registry.matching_unsupported_declared_operation(declaration, installed_authority)
        {
        ...
        if let Some(registration) =
            registry.matching_declared_operation(declaration, installed_authority)
        {

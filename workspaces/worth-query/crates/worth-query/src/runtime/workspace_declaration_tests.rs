use super::{
    ...
    WorthQueryLiveViewBuilder, WorthQueryWorkspaceLiveViewDeclaration,
};
...
fn traversal_relation_declarations_reject_zero_depth() {
    let error = WorthQueryLiveViewBuilder::surface("runtime.zero-depth")
...
fn traversal_relation_declarations_reject_duplicates() {
    let error = WorthQueryLiveViewBuilder::surface("runtime.duplicate-relation")
        .from("WorthTopologyEntity")
...
fn traversal_relation_declarations_lower_into_request_and_schema_view() {
    let declaration = WorthQueryLiveViewBuilder::surface("runtime.traversal-lowered")
        .from("WorthTopologyEntity")
...
fn direct_live_view_declaration_rejects_traversal_schema_mismatch() {
    ...
    let error = WorthQueryWorkspaceLiveViewDeclaration::try_from_request(request, schema_view)

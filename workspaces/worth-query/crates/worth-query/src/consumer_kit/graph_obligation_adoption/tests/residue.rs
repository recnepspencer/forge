...
fn residue_manifest_rejects_growth_over_cap() {
    let error = WorthQueryGraphObligationResidueManifest::capped([
...
fn residue_manifest_certifies_candidate_against_previous_without_growth() {
    let previous = WorthQueryGraphObligationResidueManifest::capped([residue_row(1)]).unwrap();
    let candidate = WorthQueryGraphObligationResidueManifest::capped([residue_row(0)]).unwrap();
    let certification =
        WorthQueryGraphObligationResidueManifest::certify_candidate_against_previous(
            &previous, &candidate,
        )
        .unwrap();
...
fn residue_manifest_rejects_candidate_growth_or_contract_drift() {
    let previous = WorthQueryGraphObligationResidueManifest::capped([residue_row(1)]).unwrap();
    let grown =
        WorthQueryGraphObligationResidueManifest::capped([residue_row_with_cap(2, 2)]).unwrap();
    let growth_error =
        WorthQueryGraphObligationResidueManifest::certify_candidate_against_previous(
            &previous, &grown,
        )
    ...
    let changed_contract = WorthQueryGraphObligationResidueManifest::capped([
        WorthQueryGraphObligationResidueRow::explicit(
            "manual selector comments",
            "worth-query",
            "phase-14",
    ...
    .unwrap();
    let drift_error = WorthQueryGraphObligationResidueManifest::certify_candidate_against_previous(
        &previous,
        &changed_contract,
    )
    .unwrap_err();

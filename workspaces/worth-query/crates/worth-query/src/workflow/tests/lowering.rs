...
use worth_runtime_bridge::facade::{BridgeRequestKind, BridgeWritebackRequestMode};
...
fn runtime_writeback_lowering_emits_bridge_declaration_and_causality() {
    ...
    assert_eq!(
        ...
        Some(worth_runtime_bridge::facade::BridgeWritebackFamilyKind::ProjectedStateDiff)
    );
    assert_eq!(
        lowered.bridge_declaration().effect_class(),
        worth_runtime_bridge::facade::BridgeWritebackEffectClass::ProjectedStateDiff
    );
    assert_eq!(
        lowered.bridge_declaration().strategy_class(),
        Some(
            worth_runtime_bridge::facade::BridgeWritebackStrategyClass::ProjectedStateDiffReconciliation,
        )
    );
    assert_eq!(
        lowered.bridge_declaration().idempotence_class(),
        worth_runtime_bridge::facade::BridgeWritebackIdempotenceClass::RequireSemanticNoopSuppression
    );
}

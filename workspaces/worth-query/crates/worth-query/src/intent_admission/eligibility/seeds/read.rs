...
#[derive(Clone, Debug, PartialEq)]
pub struct WorthQueryLiveReadIntentSeed {
...
impl WorthQueryReadExecutionIntentSeed {
    pub fn current_runtime(read_family: WorthQueryReadFamily) -> Self {
        let request_label = format!("read.family.{}", read_family.family_name());
        let request_input_digest = hash_parts(&[
            "worth_query_read_execution_intent_seed_v1".to_string(),
...
impl WorthQueryLiveReadIntentSeed {
    pub fn from_live_view<T>(live_view: &WorthQueryLiveView<T>) -> Self {
        Self::from_installation(live_view.subscription_installation())
    }

    pub fn from_installation(installation: &WorthQueryRuntimeLiveSubscriptionInstallation) -> Self {
        ...
        let request_input_digest = hash_parts(&[
            "worth_query_live_read_intent_seed_v1".to_string(),

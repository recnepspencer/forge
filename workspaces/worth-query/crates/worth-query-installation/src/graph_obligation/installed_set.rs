...
///
/// Public consumers may inspect this set but cannot construct one:
///
/// ```compile_fail
...
impl WorthQueryInstalledGraphObligationSet {
    ...
    fn build(
        ...
        let installation = installation_evidence(&rows, canonical_work);
        Ok(Self {
            identity,
    ...
    ) -> &[WorthQueryInstalledGraphObligation] {
        self.selection_index.select(self.rows.as_slice(), kind)
    }

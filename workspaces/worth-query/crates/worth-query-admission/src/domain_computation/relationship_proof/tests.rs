...
) -> super::super::policy_basis::AdmittedPolicyTenantContext {
...
fn bounded_ancestor_descriptor_requires_nonzero_bound() {
    ...
    assert!(matches!(
        error,
        worth_query_declaration::facade::authoring::AuthoringError::UnsupportedTraversalDepth {
            depth: 0
        }
    ));
}

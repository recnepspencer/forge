...
impl RelationshipProofError {
    ...
    fn denied(failure_class: RelationshipProofFailureClass, message: &'static str) -> Self {
        let mut counters = RelationshipProofCounters::default();
        counters.deny();
        Self::new(failure_class, message, counters)
    }

    pub fn query_shape_mismatch(message: &'static str) -> Self {
        Self::denied(RelationshipProofFailureClass::QueryShapeMismatch, message)
    }

    pub fn unbounded_recursive_walk(message: &'static str) -> Self {
        Self::denied(

...
pub struct WorthQueryApplicationPreviewResult<Query, QueryResult> {
...
{
    pub fn execute_application_query_preview<
        Query,
        Parameters,
        QueryResult,
        Principal,
    ...
    >(
        ...
        plan: WorthQueryAdmittedApplicationQueryPlan<
            ...
            Scope,
        >,
    ) -> Result<WorthQueryApplicationPreviewResult<Query, QueryResult>, WorthQueryBoundedLaneDenial>
    where
        QueryResult: WorthQueryApplicationProjection<Schema, Query>,
    {
        let result = execute_bounded_lane(self, plan, WorthQueryApplicationQueryLane::Preview)?;
        let (rows, receipt) = result.into_parts();
        Ok(WorthQueryApplicationPreviewResult {
            rows,
            receipt,
            _query: PhantomData,
        })
...
impl<Query, QueryResult> WorthQueryApplicationPreviewResult<Query, QueryResult> {

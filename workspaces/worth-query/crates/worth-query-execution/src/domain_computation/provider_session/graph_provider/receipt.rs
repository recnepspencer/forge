...
use super::{
    WorthQueryExecutionGraphReadProduct, WorthQueryGraphCommitCall, WorthQueryGraphProviderCall,
    WorthQueryGraphProviderCallKind, WorthQueryGraphReceiptAdmissionDenial,
};
...
pub struct WorthQueryGraphProviderReceipt {
    ...
    projection: Option<Arc<WorthQueryExecutionGraphReadProduct>>,
}
...
impl WorthQueryGraphProviderReceipt {
    ...
    pub(super) fn admit_graph_call(
        ...
        let evidence_identity = graph_evidence_identity(call, projection.as_deref());
...
fn graph_evidence_identity(
    call: &WorthQueryGraphProviderCall,
    projection: Option<&WorthQueryExecutionGraphReadProduct>,
) -> Arc<str> {
    Arc::from(hash_parts(&[
        ...
        format!(
            "projection:{}",
            projection
                .map(WorthQueryExecutionGraphReadProduct::result_digest)

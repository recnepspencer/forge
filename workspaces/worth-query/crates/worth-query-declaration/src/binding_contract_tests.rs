use crate::authoring::{
    AspectFieldSelector, AuthoredResultShapeField, RawAuthoredQuery, RawAuthoredResultShape,
    RootEntityKey,
};
use crate::binding::resolve_bindings;
use crate::binding::{
    derive_binding_requirements, BindingFailureClass, BindingResolutionError, BoundBinding,
    BoundBindings, IdentityBindingDescriptor, NonIdentityBindingMetadata, QueryBindingDescriptor,
    QueryBindingSlot, QueryBindingSubject,
};
use crate::canonicalization::pipeline::QueryCanonicalizer;
use crate::canonicalization::{
    canonicalize_request, CanonicalizationFailureClass, QueryCanonicalizationError,
};
use crate::diagnostics::NormalizationEvent;
...
fn equivalent_binding_order_does_not_change_query_digest() {
    ...
    let request_a = crate::authoring::GuidedAuthoringPath::pair_collection_with_bindings(
        query.clone(),
        shape.clone(),
        bindings_a,
    )
    .unwrap();
    let request_b = crate::authoring::GuidedAuthoringPath::pair_collection_with_bindings(
        query, shape, bindings_b,
    )
    .unwrap();

    let bundle_a = canonicalize_request(request_a).unwrap();
    let bundle_b = canonicalize_request(request_b).unwrap();
    ...
    assert!(bundle_a
        ...
        .events()
        .iter()
        .any(|event| matches!(event, NormalizationEvent::NonIdentityBindingIgnored { .. })));
...
fn forbidden_binding_metadata_is_rejected_at_binding_boundary() {
    ...
    assert!(matches!(
        error,
        crate::binding::BindingError::ForbiddenMetadataKey { .. }
    ));
}
...
fn unsupported_binding_metadata_is_rejected_at_binding_boundary() {
    ...
    assert!(matches!(
        error,
        crate::binding::BindingError::UnsupportedMetadataKey { .. }
    ));
}
...
fn duplicate_binding_descriptor_same_subject_deduplicates() {
    ...
    let request =
        crate::authoring::GuidedAuthoringPath::pair_detail_with_bindings(query, shape, bindings)
            .unwrap();
    ...
    assert!(bundle.report().events().iter().any(|event| matches!(
        event,
        NormalizationEvent::IdentityBindingCollapsedDuplicate { .. }
    )));
}
...
fn validated_bundle_derives_binding_requirements() {
    ...
    let validated = validate_canonical_bundle(canonical, identity_schema()).unwrap();

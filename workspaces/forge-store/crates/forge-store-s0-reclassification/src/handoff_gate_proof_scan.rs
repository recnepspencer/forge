...
use crate::S0HandoffGateProofEvidenceDenial;

pub(crate) struct S0HandoffGateProofSurfaceCounts {
...
pub(crate) fn scan_current_s0_handoff_gate_proof_surfaces(
    ...
    Ok(S0HandoffGateProofSurfaceCounts {
...
) -> Result<usize, S0HandoffGateProofEvidenceDenial> {
...
fn public_facade_surface_count(root: &Path) -> Result<usize, S0HandoffGateProofEvidenceDenial> {
    let facade = read_workspace_file(root, "workspaces/forge-store/crates/forge-store/src/lib.rs")?;
...
fn native_harness_surface_count(root: &Path) -> Result<usize, S0HandoffGateProofEvidenceDenial> {
    let source = read_workspace_file(
        root,
        "workspaces/forge-store/crates/forge-store-test-support/src/native_aspect_fixture_authoring.rs",
    )?;
...
fn collect_current_source_files(
    root: &Path,
) -> Result<Vec<PathBuf>, S0HandoffGateProofEvidenceDenial> {
    let mut files = Vec::new();
    collect_source_files(&root.join("crates/forge-store"), &mut files)?;
    collect_source_files(&root.join("workspaces/forge-store"), &mut files)?;
    files.sort();
    Ok(files)
}
...
fn is_terminal_projection_dependency_line(path: &str, line: &str) -> bool {
    path == "workspaces/forge-store/crates/forge-store-aspect-native/Cargo.toml"
        && line.contains(&["serde", "json"].join("_"))
}
...
fn facade_module_body<'a>(
...
) -> Result<&'a str, S0HandoffGateProofEvidenceDenial> {
    ...
    Err(S0HandoffGateProofEvidenceDenial::MissingPublicFacadeProof)
}
...
fn read_workspace_file(
    root: &Path,
    relative: &str,
) -> Result<String, S0HandoffGateProofEvidenceDenial> {
...
fn repository_root() -> Result<PathBuf, S0HandoffGateProofEvidenceDenial> {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(4)
        .map(Path::to_path_buf)
        .ok_or_else(|| {
            S0HandoffGateProofEvidenceDenial::SourceReadFailed("repository root".to_string())
        })
}
...
fn source_read_failed(error: impl ToString) -> S0HandoffGateProofEvidenceDenial {
    S0HandoffGateProofEvidenceDenial::SourceReadFailed(error.to_string())
}

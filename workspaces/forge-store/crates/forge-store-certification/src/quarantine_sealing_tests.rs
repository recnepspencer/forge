...
fn handoff_posture_stays_distinct_from_repair_or_recovery_claims() {
    ...
    let record = PhysicalQuarantineAuthority::seal(
        QuarantineSealRequest::from_executed_finding(finding)
            .with_handoff_posture(QuarantineHandoffPosture::S10RepairOwnerRequired),
    )
    .unwrap();
...
fn seal_ambiguous_page_damage(label: &str) -> forge_store_physical_integrity::QuarantineRecord {

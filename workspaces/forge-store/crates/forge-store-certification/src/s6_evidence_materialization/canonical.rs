...
use forge_foundational::CanonicalBasisConstructionDenial;
...
fn rule_version() -> CanonicalizationRuleVersion {
    CanonicalizationRuleVersion::new("forge-store.s6.certification-evidence.v1")
        .expect("static S6 evidence canonicalization rule version is valid")
}

const fn domain() -> CanonicalBasisDomain {
    CanonicalBasisDomain::Future("forge-store.s6.certification-evidence")
}

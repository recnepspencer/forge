...
use forge_foundational::{CanonicalBasisConstructionDenial, CanonicalDigestId};
...
fn rule_version() -> CanonicalizationRuleVersion {
    CanonicalizationRuleVersion::new("forge-store.s5.executed-isolation.v1")
        .expect("static S5 evidence canonicalization rule version is valid")
}

const fn domain() -> CanonicalBasisDomain {
    CanonicalBasisDomain::Future("forge-store.s5.executed-isolation")
}

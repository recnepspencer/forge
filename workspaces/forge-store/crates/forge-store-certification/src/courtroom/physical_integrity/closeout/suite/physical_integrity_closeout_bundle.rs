use crate::{
    ...
    IntegrityCompositionEvidence, IntegrityHarnessExecutionEvidence,
...
pub fn close_physical_integrity_from_executed_evidence(
    ...
    line_cap_composition: IntegrityCompositionEvidence,
    ...
    Ok(vec![
        PhysicalIntegrityCloseoutSuiteEvidence::corruption_localization(
            physical_integrity_suite_harness(
                ...
                IntegrityHarnessExecutionEvidence::corruption_localization(&localized_boundaries),
            )?,
            localized_boundaries,
        ),
        PhysicalIntegrityCloseoutSuiteEvidence::boundary_denial(
            physical_integrity_suite_harness(
                ...
                IntegrityHarnessExecutionEvidence::boundary_denial(&denied_boundaries),
            )?,
            denied_boundaries,
        ),
        ...
        PhysicalIntegrityCloseoutSuiteEvidence::line_cap_composition(
            physical_integrity_suite_harness(
                ...
                IntegrityHarnessExecutionEvidence::line_cap_composition(&line_cap_composition),
            )?,
            line_cap_composition,
        ),
    ])
...
) -> Result<crate::IntegrityHarnessTranscriptEvidence, PhysicalIntegrityCloseoutDenial> {
...
fn require_physical_integrity_readiness_scope(
    ...
    if readiness.payload().claims_later_sequence_semantics() {
        Err(PhysicalIntegrityCloseoutDenial::IntegrityReadinessClaimsRecoverySequence)
    } else {
        Ok(())
    }
}
...
fn require_recovery_handoff(
    ...
    if !payload.proves_no_raw_bytes_crossed() || !handoff.proves_no_raw_bytes_crossed() {
        return Err(PhysicalIntegrityCloseoutDenial::RecoveryHandoffContainsRawBytes);
    }
    if payload.claims_recovery() || handoff.claims_recovery() {
        return Err(PhysicalIntegrityCloseoutDenial::RecoveryHandoffClaimsRecoveryAuthority);
    }
    if payload.page_frames().is_empty()
        || payload.wal_frames().is_empty()
        || payload.checkpoint_records().is_empty()
    {
        return Err(PhysicalIntegrityCloseoutDenial::MissingRecoveryHandoffPayload);
    }
    Ok(())
}
...
fn require_recovery_handoff_suite_evidence(
    ...
    if evidence.matches_readiness(&request.recovery_handoff) {
        ...
        Err(PhysicalIntegrityCloseoutDenial::RecoveryHandoffEvidenceMismatch)
    }
}

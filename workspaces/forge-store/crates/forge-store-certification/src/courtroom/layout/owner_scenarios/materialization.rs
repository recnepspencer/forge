...
fn execute_btree_publication(ledger: &mut LayoutOwnerObservationLedger) {
    ...
    let family = admit_family(DurableArtifactFamilyId::PhysicalPage, &security);
    let catalog = admitted_layout_bootstrap_catalog();
    let publication = forge_store_test_support::harness::physical_isolation::publication::
...
fn execute_imported_blob(ledger: &mut LayoutOwnerObservationLedger) {
    ...
    let admitted = forge_store_blob_chunks::certification_test_authority::
        execute_readmitted_blob_import_for_store(
            "layout.owner-matrix.imported-blob",
            "store.physical.default_instance",
        );
    let foreign = forge_store_blob_chunks::certification_test_authority::
        execute_readmitted_blob_import_for_store(
...
fn execute_restore(ledger: &mut LayoutOwnerObservationLedger) {
    let catalog = admitted_layout_bootstrap_catalog();
    let reopened =
        forge_store_test_support::reopened_recovery_artifact_fixture("layout-owner-restore");
    let observations =
        forge_store_operations::certification_test_authority::execute_restore_owner_scenarios(
            &catalog, &reopened,
        );
...
fn tenant_page_security() -> forge_store_security::StoreAdmittedSecurityScope {
    security_scope(
...
fn tenant_blob_security() -> forge_store_security::StoreAdmittedSecurityScope {
    security_scope(
        SecurityScopeFixtureAuthority::Current,
...
fn lsm_family_cases() -> [forge_store_layout_indexes::AdmittedPhysicalArtifactFamily; 3] {

...
fn recovery_entry_rejects_wrong_or_unbounded_recovery_envelopes_before_admission() {
    ...
    let denial = forge_store_recovery_physics::RecoveryMemoryEnvelope::from_admitted(compaction)
        .unwrap_err();
    ...
    let unbounded = admission
        .admit(
            forge_store_buffer_pool::BackgroundEnvelopeRequest::recovery_planning()
                .resident_frames(1)
                .resident_bytes(128)
...
fn quarantine_summary_preserves_its_damage_case_across_mixed_recovery_blockers() {
    ...
    let damage_map = recovery_blocking_wal_damage_map()
        .with_unresolved_authority_damage(
            forge_store_recovery_physics::RecoveryBlockedByIntegrityDamage::unresolved_authority_damage(
    ...
    assert_eq!(
        ...
        forge_store_contracts::CorruptionHandoffDamageCase::ChecksumMismatch
    );
    assert_eq!(
        handoff.repair_capability(),
        forge_store_recovery_physics::RecoveryCorruptionRepairCapability::ClassifyGenerationPosture
    );
}

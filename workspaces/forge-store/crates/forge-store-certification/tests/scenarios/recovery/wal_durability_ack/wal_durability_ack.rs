#[path = "ack_assertions.rs"]
mod ack_assertions;
#[path = "adversarial_ack_denial_paths.rs"]
mod adversarial_ack_denial_paths;
#[path = "append_path_basis.rs"]
mod append_path_basis;
#[path = "certified_barrier_observations.rs"]
mod certified_barrier_observations;
#[path = "crash_assertions.rs"]
mod crash_assertions;
#[path = "durable_profile_paths.rs"]
...
fn profile_authority_denies_barriers_outside_its_exact_required_set() {
    ...
    assert_eq!(
        denial.kind(),
        forge_store_physical_backend::BackendDurabilityBarrierDenialKind::BarrierNotRequiredByProfile
    );
    assert_eq!(
        denial.profile_id(),
        forge_store_physical_backend::BackendDurabilityProfileId::PosixFileFsyncDirFsync
    );
    assert_eq!(
        denial.barrier(),
        WalDurabilityBarrier::WindowsFlushFileBuffers
...
fn barrier_receipts_cannot_complete_a_different_append_scope() {
    ...
    assert_eq!(
        denial.profile_id(),
        Some(forge_store_physical_backend::BackendDurabilityProfileId::PosixFileFsyncDirFsync)
    );

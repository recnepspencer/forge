...
fn sharp_mode_certifies_and_fuzzy_attempts_deny_explicitly() {
    ...
    let fuzzy = forge_store_recovery_physics::FuzzyCheckpointCertificationModeDenial::missing_begin_end_records();
    assert_eq!(
        fuzzy.kind(),
        forge_store_recovery_physics::FuzzyCheckpointCertificationModeDenialKind::MissingBeginEndRecords
    );
    assert_eq!(
...
fn assert_denial(
    result: Result<CheckpointManifest, forge_store_recovery_physics::CheckpointValidationDenial>,

...
fn compile_fail_fixtures() -> [CompileFailFixture; 7] {
    [
        CompileFailFixture {
        ...
        CompileFailFixture {
            name: "adversarial_inputs_cannot_form_replay_bundle.rs",
            expected_stderr: &[
                "S8LayoutExecutedScenarioWitness",
        ...
        CompileFailFixture {
            name: "closeout_suite_certificate_cannot_form_s9_handoff.rs",
            expected_stderr: &[
                "S8LayoutHandoffReadiness",
                "S8LayoutCloseoutSuiteCertificate",
            ],
        },
        CompileFailFixture {
            name: "static_grammar_cannot_use_certify_handoff_api.rs",
            expected_stderr: &["certify_s8_layout_handoff", "unresolved import"],
        },
        CompileFailFixture {
            name: "generic_machine_summary_cannot_form_s9_handoff.rs",
            expected_stderr: &["S8LayoutHandoffReadiness", "S9_REQUIRED_LAYOUT_MACHINES"],
        },
        CompileFailFixture {
            name: "machine_inventory_cannot_be_minted.rs",
            expected_stderr: &["_seal", "private"],
        },
    ]
}
...
fn fixture_path(manifest_dir: &std::path::Path, fixture_name: &str) -> std::path::PathBuf {
    manifest_dir
        .join("tests")
        .join("ui")
        .join("s8_layout_access_path_harness")
        .join(fixture_name)
}

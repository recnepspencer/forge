...
fn compile_fail_fixtures() -> Vec<CompileFailFixture> {
    vec![
        ...
        CompileFailFixture {
            name: "copied_digest_cannot_be_transcript_identity.rs",
            expected_stderr: &["PhysicalSimulationTranscriptIdentity", "[u8; 32]"],
        },
        CompileFailFixture {
            name: "terminal_json_cannot_satisfy_replay_bundle.rs",
            expected_stderr: &["SimulationReplayBundle", "Value"],
        },
        CompileFailFixture {
            name: "loose_log_cannot_satisfy_replay_bundle.rs",
            expected_stderr: &["SimulationReplayBundle", "String"],
        },
        CompileFailFixture {
            name: "terminal_json_cannot_satisfy_evidence_bundle.rs",
            expected_stderr: &["PhysicalCertificationEvidenceBundle", "Value"],
        },
        CompileFailFixture {
            name: "same_run_comparison_cannot_satisfy_evidence_bundle.rs",
            expected_stderr: &["PhysicalCertificationEvidenceBundle", "SameRunComparison"],
        },
        CompileFailFixture {
        ...
        CompileFailFixture {
            name: "foundational_bundle_cannot_satisfy_store_evidence.rs",
            expected_stderr: &[
...
fn compile_fail_case_dir(fixture_name: &str) -> std::path::PathBuf {
    std::env::temp_dir()
        .join("forge-store-s45-transcript-evidence-ui")

...
use crate::{
    ...
    AdmittedKeyScopeLayoutRule, SecurityCustodyLookupAccessShape, StoreAdmittedSecurityScope,
    StoreKeyScope, StoreLayoutAccessSecurityBoundaryWitness, StoreSecurityAuthoritySource,
    StoreSecurityScopeAdmissionCounterSnapshot, StoreSecurityScopeDenial,
};

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct KeyScopeLayoutReport {
    rule: AdmittedKeyScopeLayoutRule,
...
impl KeyScopeLayoutReport {
    pub const fn rule(&self) -> AdmittedKeyScopeLayoutRule {
        self.rule
    }
...
impl StoreAdmittedSecurityScope {
    pub fn admit_key_scope_layout(
        &self,
        rule: &AdmittedKeyScopeLayoutRule,
    ) -> KeyScopeLayoutReport {
        KeyScopeLayoutReport {
            rule: *rule,

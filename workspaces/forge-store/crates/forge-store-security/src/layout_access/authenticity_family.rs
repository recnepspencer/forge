...
use crate::{
    ...
    AdmittedAuthenticityLayoutRule, SecurityCustodyLookupAccessShape, StoreAdmittedSecurityScope,
    StoreAuthenticityRequirement, StoreLayoutAccessSecurityBoundaryWitness,
    StoreSecurityAuthoritySource, StoreSecurityScopeAdmissionCounterSnapshot,
    StoreSecurityScopeDenial,
};

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AuthenticityLayoutReport {
    rule: AdmittedAuthenticityLayoutRule,
...
impl AuthenticityLayoutReport {
    pub const fn rule(&self) -> AdmittedAuthenticityLayoutRule {
        self.rule
    }
    ...
    pub const fn authenticity_requirement(&self) -> StoreAuthenticityRequirement {
        self.security_boundary.authenticity_requirement()
    }
...
impl StoreAdmittedSecurityScope {
    pub fn admit_authenticity_layout(
        &self,
        rule: &AdmittedAuthenticityLayoutRule,
    ) -> AuthenticityLayoutReport {
        AuthenticityLayoutReport {
            rule: *rule,

use super::envelope_evidence::BoundedInspectionEnvelopeEvidence;
use super::payload::{
    S4IntegrityHandoffCounters, S4IntegrityHandoffPayload, SealedS4IntegrityHandoffPayloadParts,
};
use crate::{
...
impl S4IntegrityHandoffPayloadDeclaration {
    ...
    pub fn seal(self) -> Result<S4IntegrityHandoffPayload, S4IntegrityHandoffDenial> {
        ...
        let sealed_parts = SealedS4IntegrityHandoffPayloadParts {
            root_manifest,
            segment_manifest,
            ...
            checksum_basis,
            counters: S4IntegrityHandoffCounters::new(
        ...
        Ok(S4IntegrityHandoffPayload::from_sealed_parts(sealed_parts))
    }
}
...
fn classify_handoff_counters(
    declaration: &S4IntegrityHandoffPayloadDeclaration,
) -> Option<S4IntegrityHandoffCounterClassification> {
    let pre_decode = declaration.inspection_envelope.as_ref()?.counters();
    Some(S4IntegrityHandoffCounterClassification {
...
struct S4IntegrityHandoffCounterClassification {

...
```compile_fail
use forge_store_readiness::ProtectedIntegrityViewCapability;
use forge_store_recovery_physics::{RecoveryEntryAdmission, RecoveryMemoryEnvelope};
...
```compile_fail
use forge_store_readiness::IntegrityInspectionLifetimeLaw;
use forge_store_recovery_physics::{S4IntegrityHandoffAdmission, S4IntegrityHandoffPayload};
...
use forge_store_recovery_physics::DurableAckReceipt;
...
use forge_store_recovery_physics::WalAppendDurabilityScope;
...
```compile_fail
use forge_store_recovery_physics::WalPrefixFrameObservation;
```
...
```compile_fail
use forge_store_recovery_physics::RedoRecordGrammar;
...
```compile_fail
use forge_store_recovery_physics::FreshRuntimeRecoveryDriver;
...
```compile_fail
use forge_store_recovery_physics::{S4LoweredCrashHarnessEvidence, S4RecoveryCrashSeam};

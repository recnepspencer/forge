...
fn add_reference_epoch(
    ...
    match super::footprint_ranges::latch_domain(reference) {
        forge_store_physical_format::PhysicalCellReuseDomain::Segment => Ok(builder.with_segment(
            root.admit_segment_publication_epoch(reference.current_generation())?
                .epoch(),
        )),
        forge_store_physical_format::PhysicalCellReuseDomain::ExtentAllocation
        | forge_store_physical_format::PhysicalCellReuseDomain::FreeSpaceReuse => Ok(builder
            .with_extent(
                root.admit_extent_publication_epoch(reference.current_generation())?
                    .epoch(),
            )),
        forge_store_physical_format::PhysicalCellReuseDomain::Page
        | forge_store_physical_format::PhysicalCellReuseDomain::SlotAllocation => Ok(builder
            .with_page(
                root.admit_page_publication_epoch(reference.current_generation())?
                    .epoch(),
            )),
        forge_store_physical_format::PhysicalCellReuseDomain::RootPublication => Ok(builder),
    }
}

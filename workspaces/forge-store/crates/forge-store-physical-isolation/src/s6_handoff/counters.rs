use super::S6IoQosIsolationReadinessDenial;
...
pub enum S6ExecutedIsolationCounterKind {
    Outcome,
    Wait,
    Retry,
    LatchCounterRow,
...
impl PhysicalIsolationCounterSnapshot {
    ...
    pub(crate) const fn from_store_executed_counts(
        ...
        if latch_counter_rows == 0 {
            return Err(S6IoQosIsolationReadinessDenial::MissingLatchCounters);
        }
        if reclaim_counter_rows == 0 {
            return Err(S6IoQosIsolationReadinessDenial::MissingReclaimCounters);
        }
        if protected_byte_footprint == 0 {
            return Err(S6IoQosIsolationReadinessDenial::MissingProtectedByteFootprint);
        }
        Ok(Self {
            outcome_count,
            wait_count,

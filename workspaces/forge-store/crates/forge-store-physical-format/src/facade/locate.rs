...
use crate::{
    ...
    PhysicalReference, PhysicalReferenceAuthority, PhysicalReferenceKind,
    PlatformPhysicalFacadeCounterSnapshot, PlatformPhysicalFacadeDenial,
    PlatformPhysicalFacadeDenialKind, PlatformPhysicalFramedRecord, PlatformPhysicalLocateReport,
};

pub(crate) fn classify_and_locate_record<'a>(
    ...
    match classify_reference_kind(reference) {
        ReferenceLocateClass::PageSlot => {
            locate_page_slot(storage, page_records, references, counters, reference)
        }
        ReferenceLocateClass::ExtentBacked => {
            locate_extent(storage, extent_records, references, counters, reference)
        }
        ReferenceLocateClass::Unsupported => Err(assemble_missing_record_denial()),
    }
}

enum ReferenceLocateClass {
    PageSlot,
    ExtentBacked,
    Unsupported,
}

fn classify_reference_kind(reference: PhysicalReference) -> ReferenceLocateClass {
    match reference.kind() {
        PhysicalReferenceKind::PageSlot => ReferenceLocateClass::PageSlot,
        PhysicalReferenceKind::ExtentBacked => ReferenceLocateClass::ExtentBacked,
        _ => ReferenceLocateClass::Unsupported,
    }
}

fn assemble_missing_record_denial() -> PlatformPhysicalFacadeDenial {

...
const TEST_PLAN_BINDING: S8PreExecutionPlanBinding =
    S8PreExecutionPlanBinding::new(7, 11, 13, 17, 0);
...
fn baseline_btree_execution_directs_lookup_by_separator_branch() {
    ...
    assert_eq!(
        witness
            .execute_separator_directed_lookup(
                TEST_PLAN_BINDING,
                crate::PhysicalRecordSlot::from_raw(11).unwrap()
            )
            .branch(),
    ...
    assert_eq!(
        witness
            .execute_separator_directed_lookup(
                TEST_PLAN_BINDING,
                crate::PhysicalRecordSlot::from_raw(12).unwrap()
            )
            .branch(),
...
fn baseline_btree_execution_emits_exact_strategy_lane_classification() {
    ...
    assert_eq!(
        witness
            .execute_separator_directed_range_lookup(
                TEST_PLAN_BINDING,
                crate::PhysicalRecordSlot::from_raw(11).unwrap()
            )
            .exact_counters()
    ...
    assert_eq!(
        witness
            .execute_separator_directed_prefix_lookup(
                TEST_PLAN_BINDING,
                crate::PhysicalRecordSlot::from_raw(11).unwrap()
            )
            .shape(),
    ...
    assert_eq!(
        witness
            .execute_replay_recovery(TEST_PLAN_BINDING)
            .exact_counters()
            .maintenance_reads(),
        1
    );

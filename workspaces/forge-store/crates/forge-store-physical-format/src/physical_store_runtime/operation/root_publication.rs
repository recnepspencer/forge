...
use super::{PhysicalStoreRuntime, PlatformPhysicalRootPublicationReport};

impl PhysicalStoreRuntime {
    pub fn publish_physical_root(
        &mut self,
    ) -> Result<PlatformPhysicalRootPublicationReport, PhysicalStoreRuntimeDenial> {
...
) -> Result<EncodedRootPublication, PhysicalStoreRuntimeDenial> {
...
fn advance_root_generation(
    next_root_generation: &mut u64,
) -> Result<PhysicalGeneration, PhysicalStoreRuntimeDenial> {
    let generation = PhysicalGeneration::from_raw(*next_root_generation).map_err(|_| {
        PhysicalStoreRuntimeDenial::new(PhysicalStoreRuntimeDenialKind::MissingPhysicalRoot)
    })?;
...
fn manifested_segments(storage: &PhysicalStoreRuntimeStorage) -> Vec<crate::SegmentGenerationCell> {

...
fn canonical_heavy_fixture_identity_binds_phase23_basis() {
    ...
    assert_eq!(
        ...
        forge_store_physical_backend::HeavyFixtureBackendProfile::StoreOwnedLocalDisk
    );
}
...
fn hostile_phase23_lanes_are_typed_denials() {
    ...
    assert_eq!(
        ...
        forge_store_blob_chunks::HeavyBlobQualificationDenial::LogicalSizeOnlyProofNotCanonical
    );
    assert_eq!(
        deny_whole_object_expected_buffer(),
        forge_store_blob_chunks::HeavyBlobQualificationDenial::WholeObjectExpectedBufferNotCanonical
    );
    assert_eq!(
        deny_hidden_temporary_sidecar(),
        forge_store_blob_chunks::HeavyBlobQualificationDenial::HiddenTemporarySidecarNotCanonical
    );
    assert_eq!(
        deny_ambient_chaos_corpus_as_canonical(),
        forge_store_blob_chunks::HeavyBlobQualificationDenial::AmbientCorpusNotCanonical
    );
}

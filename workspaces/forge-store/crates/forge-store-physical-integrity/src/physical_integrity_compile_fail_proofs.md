...
```compile_fail
use forge_store_physical_integrity::IntegrityEntryWitness;
...
```compile_fail
use forge_store_physical_integrity::IntegrityEntryRequest;
use std::path::Path;
...
```compile_fail
use forge_store_physical_integrity::IntegrityEntryAdmission;
use forge_store_readiness::S3PhysicalIntegrityReadinessPayload;
...
```compile_fail
use forge_store_physical_integrity::ChecksumAlgorithmDeclaration;
...
use forge_store_physical_integrity::ChecksumAlgorithmId;
...
```compile_fail
use forge_store_physical_integrity::{LogicalDecodeGate, ProtectedPhysicalByteView};
...
```compile_fail
use forge_store_physical_integrity::{DeclaredPhysicalChecksum, LogicalDecodeGate};
...
```compile_fail
use forge_store_physical_integrity::LogicalDecodeGate;
...
```compile_fail
use forge_store_physical_integrity::IntegrityCheckedFrame;
...
```compile_fail
use forge_store_physical_integrity::PhysicalScopeAdmission;
...
```compile_fail
use forge_store_physical_integrity::ScopedPhysicalValidatorInput;
...
```compile_fail
use forge_store_physical_integrity::PhysicalContainerIntegrity;
...
```compile_fail
use forge_store_physical_integrity::RebuildableDerivedDamage;
...
```compile_fail
use forge_store_physical_integrity::{DerivedRebuildInput, ManifestIntegrityReport};
...
```compile_fail
use forge_store_physical_integrity::{IndexPageIntegrityReport, IntegrityCheckedPage};
...
```compile_fail
use forge_store_physical_integrity::ChunkIntegrityStreamingWindow;
...
use forge_store_physical_integrity::GenerationIntegrityReport;
...
```compile_fail
use forge_store_physical_integrity::ExecutedQuarantineFinding;
...
```compile_fail
use forge_store_physical_integrity::QuarantineReceipt;
...
```compile_fail
use forge_store_physical_integrity::FoundationalQuarantineReceiptBasis;
...
```compile_fail
use forge_store_physical_integrity::QuarantineSealRequest;
...
```compile_fail
use forge_store_physical_integrity::{DerivedRebuildInput, QuarantineSealRequest};
...
```compile_fail
use forge_store_physical_integrity::{IntegrityCheckedPage, PhysicalIntegrityEvidenceBundle};
...
```compile_fail
use forge_store_physical_integrity::{IntegrityProofProgressionReport, QuarantineRecord};

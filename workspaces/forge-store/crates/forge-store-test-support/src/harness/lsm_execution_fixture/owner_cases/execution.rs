...
fn observe_replay(observations: &mut Vec<LsmExecutionOwnerCaseObservation>) {
    let world = execution_world::world(43, [41, 42, 43]);
    let source = forge_store_layout_indexes::lsm_strategy()
...
fn prepare(world: &LsmExecutionWorld) -> forge_store_layout_indexes::PreparedLsmCompaction {
    lsm_compaction_runtime()
        .execute(world.demand())
        .into_result()
        .unwrap()
...
fn interlocked(world: &LsmExecutionWorld) -> forge_store_layout_indexes::InterlockedLsmCompaction {
    lsm_physical_compaction_runtime()

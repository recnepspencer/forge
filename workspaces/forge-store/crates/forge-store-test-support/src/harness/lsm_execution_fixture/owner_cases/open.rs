...
use forge_store_wal::{AdmittedWalArtifactStore, BlobWalRecordKind};
...
fn durable_record_binding_mismatch() -> LsmMembershipOwnerCaseObservation {
    ...
    bytes[b"forge-store:wal-lsm-membership:v1 ".len()] ^= 1;
...
fn persisted_membership_artifact_invalid() -> LsmMembershipOwnerCaseObservation {
    let world = world::complete_membership();
    let body = b"forge-store:wal-lsm-membership:v1 0";
...
fn open(anchor: &forge_store_wal::AdmittedWalAppendReceipt) -> LsmMembershipOwnerCaseObservation {
    let current_scope =
        forge_store_security::admitted_store_wal_checkpoint_security_scope_for_layout_partition_test();

...
fn persisted_lsm_membership_cannot_be_readmitted_under_another_store_family() {
    ...
    let (foreign_family, _) = crate::strategy::tests_support::admit_strategy_scope(
        forge_store_contracts::DurableArtifactFamilyId::PublicationWalIntent,
        forge_store_security::StoreKeyScope::WalCheckpointEnvelope,
        forge_store_security::StoreTenantScope::StoreInternal,
        forge_store_security::StoreAuthenticityRequirement::required(
            forge_store_security::StoreAuthenticityRequirementClass::AuthenticatedWalRecord,
        ),

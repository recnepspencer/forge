...
pub(crate) fn publication_plan(
    ...
    operation_digest: &str,
    generation: u64,
) -> forge_store_physical_isolation::CopyOnWritePublicationPlan {
    let inputs = publication_inputs(authority, operation_digest, generation);
    forge_store_test_support::harness::physical_isolation::publication::admitted_copy_on_write_plan(
        &inputs,
    )
}
...
pub(crate) fn rollback_publication_plan(
    ...
    let store = forge_store_physical_format::PhysicalStoreIdentity::from_aspect_identity(
        authority.identity().clone(),
    );
    let migration = publication_inputs(authority, "layout-migration-publication", 1_901);
    let rollback = forge_store_test_support::harness::physical_isolation::publication::successor_publication_inputs_for_store(
        &migration,
        &store,
        operation_digest,
        generation,
...
fn current_publication_source(
    authority: &StoreCurrentAuthorityWitness,
) -> forge_store_physical_isolation::PublicationRootCandidate {
...
fn publication_inputs(
    ...
    operation_digest: &str,
    generation: u64,
) -> forge_store_test_support::harness::physical_isolation::publication::PublicationInputs {
    let store = forge_store_physical_format::PhysicalStoreIdentity::from_aspect_identity(
        authority.identity().clone(),
    );
    forge_store_test_support::harness::physical_isolation::publication::publication_inputs_for_store(
        &store,
        operation_digest,
        generation,
    )

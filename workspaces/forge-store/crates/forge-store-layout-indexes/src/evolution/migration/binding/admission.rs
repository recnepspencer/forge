...
use forge_store_compatibility::RollingWindowCompatibilityReceipt;
...
pub enum LayoutBindingSourceIdentity {
    Bootstrap {
        root: forge_store_physical_format::PhysicalRootReference,
        owner: forge_store_physical_format::PhysicalGenerationOwner,
        format: forge_store_physical_format::PhysicalFormatVersion,
        physical_source: forge_store_physical_format::PhysicalReference,
    },
    Publication(forge_store_physical_format::PhysicalReference),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::evolution::migration) struct LayoutBindingFingerprint {
    family: forge_store_contracts::DurableArtifactFamilyId,
...
impl LayoutBindingWitness {
    ...
    pub const fn bootstrap_source(
        &self,
    ) -> Option<forge_store_physical_format::PhysicalRootReference> {
        match self.source {
    ...
    pub const fn publication_source(
        &self,
    ) -> Option<forge_store_physical_format::PhysicalReference> {
        match self.source {
    ...
    pub(in crate::evolution::migration) fn accepts_publication_source(
        &self,
        actual: forge_store_physical_format::RootPublicationValidationWitness,
    ) -> bool {
        match self.source {
...
pub struct LayoutBindingRequest {
    ...
    physical_source: forge_store_physical_isolation::PublicationRootCandidate,

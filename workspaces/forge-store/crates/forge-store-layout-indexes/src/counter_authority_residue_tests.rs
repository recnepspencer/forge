...
fn lsm_counter_observations_are_read_only_and_owner_issued() {
    ...
    let replay_operation = source("src/strategy/lsm/replay/operation.rs");
    let replay_evidence = source("src/strategy/lsm/replay/evidence.rs");
    let publication = source("src/strategy/lsm/compaction/publication_evidence.rs");
    let compaction = source("src/strategy/lsm/compaction/evidence.rs");
    ...
    assert!(lookup.contains("BaselineLsmCounterObservation::lookup(comparisons)"));
    assert!(!lookup.contains("tombstone_blocks_older: bool,\n        counters:"));
    assert!(!replay_operation.contains("BaselineLsmCounterObservation"));
    assert!(replay_evidence.contains("BaselineLsmCounterObservation::replay("));
    assert!(publication.contains("BaselineLsmCounterObservation::manifest_publication("));
...
fn degraded_counter_observation_comes_from_the_physical_owner_receipt() {
    ...
    let physical =
        std::fs::read_to_string(std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join(
            "../forge-store-physical-format/src/physical_store_runtime/observation/receipt.rs",
        ))
        .unwrap();
...
fn lsm_replay_execution_is_issued_inside_the_execution_owner() {
    let execution = source("src/strategy/lsm/replay/evidence.rs").replace('\r', "");
    let module = source("src/strategy/lsm/replay/mod.rs");
    ...
    assert!(module.contains("mod evidence;"));
    assert!(module.contains("mod operation;"));

...
fn registry_admits_supported_requests_to_stable_owner_snapshots() {
    ...
    assert_eq!(first.admitted_strategy().family(), S8LayoutStrategyFamily::BaselineBTreeRange);
    assert_eq!(first.granted_capability(), S8LayoutStrategyCapability::PointLookup);
}

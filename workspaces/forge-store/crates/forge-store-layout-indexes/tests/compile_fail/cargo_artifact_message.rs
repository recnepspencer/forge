...
mod tests {
    ...
    fn parses_library_artifact_with_windows_path() {
        let message = parse(
            r#"{"reason":"compiler-artifact","target":{"kind":["lib"],"name":"forge-store-layout-indexes"},"filenames":["C:\\target\\libforge_store_layout_indexes.rlib"]}"#,
        )
        .expect("compiler artifact parses");

        assert_eq!(message.target_name, "forge-store-layout-indexes");
        assert_eq!(
            message.filenames[0].to_string_lossy(),
            r"C:\target\libforge_store_layout_indexes.rlib"
        );
    }

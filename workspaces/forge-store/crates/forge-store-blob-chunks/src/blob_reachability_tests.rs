...
use crate::blob_reachability_hold_test_support::root_candidate_resume_checkpoint;
use crate::{
...
fn backup_requires_s10_handoff_and_export_uses_s7_readiness() {
    ...
    assert!(matches!(
        BlobReachabilityProtectedHold::from_export_readiness(
            &backup,
            crate::blob_reachability_edges::BlobReachabilityAuthorityKey::from_published(
                &published
            )
        ),
        Err(BlobReachabilityDenial::InvalidProtectedHold { .. })
    ...
    let export_hold = BlobReachabilityProtectedHold::from_export_readiness(
        &export,
        crate::blob_reachability_edges::BlobReachabilityAuthorityKey::from_published(&published),
    )
...
fn published_with_sequence_and_bytes(
    ...
    let sequence = crate::blob_chunk_test_support::admitted_sequence_for_scope(scope, bytes);
    let (published, _) =

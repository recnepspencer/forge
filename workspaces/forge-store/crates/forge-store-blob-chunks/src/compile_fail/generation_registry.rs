...
//!
//! let _forged = BlobGeneration::published(7);
//! ```
//! Blob generation registry authority cannot be synthesized from raw fields:
//! ```compile_fail
//! use forge_store_blob_chunks::BlobGenerationRegistryAuthority;
//!
//! let _forged = BlobGenerationRegistryAuthority {
//!     current_authority: todo!(),
//! };
//! ```
//! Blob generation registry entries cannot be synthesized from copied facts:
//! ```compile_fail
//! use forge_store_blob_chunks::BlobGenerationRegistryEntry;
//!
//! let _forged = BlobGenerationRegistryEntry {
//!     object_id: todo!(),
//!     generation: todo!(),
...
//! ```compile_fail
//! use forge_store_blob_chunks::BlobGenerationObservation;
//!
//! let _forged = BlobGenerationObservation {
//!     object_id: todo!(),
//!     generation: todo!(),
//!     chunk_tree_root: todo!(),
//!     logical_content_digest: todo!(),
...
//! ```compile_fail
//! use forge_store_blob_chunks::{BlobGenerationObservation, LifecycleReceipt};
//!
...
//! ```compile_fail
//! use forge_store_blob_chunks::{BlobObjectClassificationAdmission, LifecycleReceipt};
//!
//! let receipt: LifecycleReceipt = todo!();
...
//! ```compile_fail
//! use forge_store_blob_chunks::{BlobGenerationRegistryEntry, ChunkTreeRoot};
//!

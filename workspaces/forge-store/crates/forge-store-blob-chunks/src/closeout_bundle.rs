...
impl S7ExecutedLifecycleEvidenceBundle {
    ...
    pub const fn lifecycle_declaration(&self) -> &BlobLifecycleDeclaration {
        &self.lifecycle_declaration
    }
    pub const fn lifecycle_counters(&self) -> BlobLifecycleCounterSnapshot {
        self.lifecycle_counters
    }
    pub const fn reachability(&self) -> &BlobChunkReachabilityProofSet {
        &self.reachability
    }
    pub const fn admitted_placement(&self) -> &AdmittedBlobPlacement {
        &self.admitted_placement
    }
    pub const fn root_publication(&self) -> &BlobChunkRootPublication {
        &self.root_publication
    }
    pub const fn export_object_id(&self) -> &BlobObjectId {
        &self.export_object_id
    }
    pub const fn export_generation(&self) -> BlobGeneration {
        self.export_generation
    }
    pub const fn export_chunk_tree_root(&self) -> &ChunkTreeRoot {
        &self.export_chunk_tree_root
    }
    pub const fn export_security_metadata(&self) -> BlobChunkSecurityMetadataWitness {
        self.export_security_metadata
    }
    pub const fn export_digest_evidence(&self) -> &BlobExportDigestEvidence {
        &self.export_digest_evidence
    }
    pub fn export_offline_declarations(&self) -> &[BlobExportOfflineChunkDeclaration] {
        &self.export_offline_declarations
    }
    pub const fn export_counters(&self) -> BlobExportBundleCounters {
    ...
    pub fn export_logical_digest_matches_lifecycle(&self) -> bool {
        self.export_digest_evidence.logical_content_digest()
    ...
    pub fn reachability_matches_lifecycle_identity(&self) -> bool {
        self.reachability
            ...
            && self.reachability.security_metadata()
                == self.lifecycle_declaration.security_metadata()
    }

    pub fn placement_matches_reachability(&self) -> bool {
        self.admitted_placement
            .matches_reachability(&self.reachability)
    }

    pub fn placement_matches_lifecycle_scope(&self) -> bool {
        self.admitted_placement.security_metadata()
            == self.lifecycle_declaration.security_metadata()
            && self.admitted_placement.stored_digest()
                == self.lifecycle_declaration.stored_chunk_digest()
    }
}

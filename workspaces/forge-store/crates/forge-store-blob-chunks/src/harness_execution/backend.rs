...
pub(super) fn current_authority(identity_key: &str, value: &str) -> StoreCurrentAuthorityWitness {
    ...
    let physical = StorePhysicalBoundaryWitness::from_physical_authority(
        forge_store_contracts::StorePhysicalAuthorityWitness::for_aspect_native_boundary(
            forge_store_contracts::ROADMAP_2_ASPECT_NATIVE_GATE_SCOPE,
        )
...
pub(super) fn stable_digest(raw: &str) -> forge_store_contracts::StableDigest {
    forge_store_contracts::StableDigest::new(raw).expect("stable digest")
}

fn expect_success<S, D, De, St, R, F>(
    outcome: forge_proof::TransitionOutcome<S, D, De, St, R, F>,
    message: &str,
) -> S
where
    S: core::fmt::Debug,

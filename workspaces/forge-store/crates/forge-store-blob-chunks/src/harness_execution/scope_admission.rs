use forge_store_security::{
    ...
    StoreAuthenticityRequirementClass, StoreCustodyPosture, StoreKeyScope,
...
use crate::{
    BlobChunkSecurityScope, BlobChunkSequenceAdmission, BlobChunkSize,
...
pub(super) fn blob_scope(case: &str, scope_class: BlobHarnessSecurityScopeClass) -> BlobChunkSecurityScope {
    let tenant_scope = match scope_class {
        BlobHarnessSecurityScopeClass::ScopePreserving => StoreTenantScope::TenantPhysicalBoundary,
        BlobHarnessSecurityScopeClass::CrossScopeDenied => StoreTenantScope::MultiTenantPhysicalBoundary,
    };
    ...
    let expectation = StoreSecurityScopeAdmissionExpectation::new(
        ...
        tenant_scope,
        StoreAuthenticityRequirement::required(StoreAuthenticityRequirementClass::AuthenticatedBlobChunk),
...
pub(super) fn integrity_proof_for_scope(
    ...
    let rule = BlobChunkingRuleAdmission::fixed_size(BlobChunkSize::from_bytes(bytes.len() as u64).expect("chunk bytes"))
        .expect("rule");

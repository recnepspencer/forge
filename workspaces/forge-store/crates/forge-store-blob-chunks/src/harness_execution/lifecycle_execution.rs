...
pub(super) fn execute_lifecycle(
    ...
    let declaration = BlobLifecycleDeclaration::new(
        ...
        AuthenticatedFrameDigest::from_declared_digest(stable_digest(&format!("sha256:{case}.frame"))),
    ...
    let placement = admit_placement(case, &reachability, crate::BlobHarnessPlacementClass::StoreLocal);
    let store_authority =
        BlobLifecycleStoreAuthority::from_current_store_authority(current_authority(case, "blob-harness-lifecycle"));
    ...
    let ready = BlobLifecycleAdmission::start(declaration)
        ...
        .success("placement")
        .ready_for_execution(BlobLifecycleReadinessAuthority::from_admitted_placement(placement.clone()))
        .success("readiness");
    ...
    ExecutedBlobLane { lifecycle, reachability, placement }
}

...
pub struct BlobExportedChunkBytes<'a> {
    leaf: BlobChunkProofLeaf,
    bytes: BlobChunkByteWindow<'a>,
}

impl<'a> BlobExportedChunkBytes<'a> {
    pub(crate) fn collect_from_leaf(
        leaf: &BlobChunkProofLeaf,
        bytes: BlobChunkByteWindow<'a>,
    ) -> Result<Self, BlobExportBundleDenial> {
...
const fn export_chunk_rule_version() -> &'static str {

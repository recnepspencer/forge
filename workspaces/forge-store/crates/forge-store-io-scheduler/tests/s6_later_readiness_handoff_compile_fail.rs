...
"#,
        ...
        toml_path(&crates_root.join("forge-store-operations")),
        toml_path(&crates_root.join("forge-store-tiering")),
        toml_path(&crates_root.join("forge-store-certification")),
    )
}
...
fn compile_fail_target_dir(case_dir: &std::path::Path) -> std::path::PathBuf {
    std::env::temp_dir()
        .join("forge_s6_later_handoff_cf")

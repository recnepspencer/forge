...
pub(crate) fn prove_background_pacing_current(
    ...
    let outcome = match freshness {
        BackgroundPacingFreshness::Current => forge_proof::TransitionOutcome::<
            BackgroundPacingReady,
            BackgroundPacingDenial,
            BackgroundPacingDeferredProof,
            BackgroundPacingStale,
        ...
        BackgroundPacingFreshness::Deferred => {
            forge_proof::TransitionOutcome::deferred(BackgroundPacingDeferredProof { class })
        }
        BackgroundPacingFreshness::Denied => forge_proof::TransitionOutcome::denied(
            BackgroundPacingDenial::PacingProgressionDenied(class),
        ),
        BackgroundPacingFreshness::Stale => {
            forge_proof::TransitionOutcome::stale(BackgroundPacingStale { class })
        }
        BackgroundPacingFreshness::RebindRequired => {
            forge_proof::TransitionOutcome::rebind_required(BackgroundPacingRebindRequired {
                class,
            })
        }
        BackgroundPacingFreshness::Failed => forge_proof::TransitionOutcome::denied(

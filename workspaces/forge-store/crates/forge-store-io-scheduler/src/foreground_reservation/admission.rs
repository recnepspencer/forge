...
pub fn admit_foreground_reservation(
    ...
    match prove_foreground_reservation_progression(&request).into_raw() {
        ...
        forge_proof::TransitionOutcome::Denied(denial) => return denied(&request, denial),
        forge_proof::TransitionOutcome::Deferred(deferred) => match deferred {},
        forge_proof::TransitionOutcome::Stale(_) => return stale_rebind_required(&request),
        forge_proof::TransitionOutcome::RebindRequired(_) => {
            return stale_rebind_required(&request)
        }
        forge_proof::TransitionOutcome::Failed(failed) => match failed {},
    }

...
impl UiIntentPayload for PlatformPulseActionPayload {
    const SCHEMA: UiIntentSchema = UiIntentSchema::stable("platform.pulse.action.payload", 2);
    const FIELDS: UiIntentPayloadFieldSet =
        UiIntentPayloadFieldSet::new(&[PLATFORM_PULSE_ACTION_REVISION_FIELD.descriptor()]);

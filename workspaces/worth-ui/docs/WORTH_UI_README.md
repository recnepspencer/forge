...
runtime-owned UI composition layer for Forge/Worth applications.
...
Forge Query owns domain/runtime truth, query declarations, touched graph
...
| Query-backed state | Forge Query via UI query binding |
...
```mermaid
sequenceDiagram
    participant Query as Forge Query
...
```text
worth-ui-query-binding + forge-query projection consumption
```
...
4. Forge Query orientation and facade

...
use read_fonts::{tables::sbix::Strike, types::GlyphId, TableProvider};
...
fn validate_sbix_table(
    ...
    validate_header(sbix)?;
    let mut png_graphics = 0_u32;
    let mut covered = BTreeSet::new();
    for strike in sbix.strikes().iter() {
        let strike = strike.map_err(|_| malformed())?;
        let (strike_pngs, strike_covered) = validate_strike(&strike, glyph_count)?;
        png_graphics = png_graphics
            .checked_add(strike_pngs)
            .ok_or_else(malformed)?;
        covered.extend(strike_covered);
    }
    if png_graphics == 0 {
        Err(malformed())
    } else {
...
fn validate_strike(
    ...
    let (png_graphics, graphics) = collect_graphics(strike, glyph_count)?;
    validate_duplicates(&graphics)?;
    Ok((png_graphics, graphics.keys().copied().collect()))
}

fn collect_graphics(
    strike: &Strike<'_>,
    glyph_count: u16,
) -> Result<(u32, BTreeMap<u16, SbixGraphic>), UiFontCollectionAdmissionDenial> {
    ...
    Ok((png_graphics, graphics))
}

fn validate_duplicates(
    graphics: &BTreeMap<u16, SbixGraphic>,
) -> Result<(), UiFontCollectionAdmissionDenial> {
    if graphics.values().any(|graphic| match graphic {
        SbixGraphic::Duplicate(target) => graphics.get(target) != Some(&SbixGraphic::Png),
        SbixGraphic::Png => false,
    }) {
        Err(malformed())
    } else {
        Ok(())

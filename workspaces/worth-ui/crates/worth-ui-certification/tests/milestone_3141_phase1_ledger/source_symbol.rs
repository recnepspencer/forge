...
pub(super) fn validate(source: &Path, symbol: &str) -> Result<(), String> {
    ...
    validate_text(&source_text, symbol, &source.display().to_string())
}

pub(super) fn validate_text(source_text: &str, symbol: &str, identity: &str) -> Result<(), String> {
    syn::parse_str::<syn::Ident>(symbol).map_err(|_| format!("invalid Rust symbol {symbol}"))?;
    let syntax = syn::parse_file(source_text)
        .map_err(|error| format!("cannot parse {identity}: {error}"))?;

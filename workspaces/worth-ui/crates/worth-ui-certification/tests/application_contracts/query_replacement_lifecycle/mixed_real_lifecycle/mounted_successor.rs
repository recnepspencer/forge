...
use worth_ui::facade::app::{
    UiMountedAllocationMeasurementRequest, WorthUiMountedApplicationReplacementOutcome,
    WorthUiMountedPreviewDisposition, WorthUiMountedPreviewOutcome,
    WorthUiMountedReplacementPreparationOutcome, WorthUiVisibleRange,
};
...
fn real_file_mount_measure_preview_and_watcher_edit_publish_one_mounted_successor() {
    ...
    assert_eq!(mounted.predecessor(), Some(ordinary.frame()));
    ...
    assert_translated_cross_lane_frame(&recorder, 2, mounted.frame());
...
fn assert_all_execution_lanes(frame: &worth_ui::facade::mounted::UiPreparedMountedFrame) {
    for expected in [

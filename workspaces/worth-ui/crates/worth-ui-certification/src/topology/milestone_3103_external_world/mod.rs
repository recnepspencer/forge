...
const LEDGER: &str = "_docs/worth-ui/milestone-3.10.3-phase-3-proof-ledger.csv";

pub fn audit_milestone_3103_phase3_external_world(
...
fn audit_ledger(text: &str) -> Result<(), String> {
    ...
    if rows.len() != 12 {
        return Err(format!(
            "Phase 3 proof ledger should contain twelve guarantees; found {}",
            rows.len()
        ));
    }
    for (index, row) in rows.iter().enumerate() {
        let expected_id = format!("P3-{:02}", index + 1);
        if !row.starts_with(&format!("{expected_id},")) {
            return Err(format!(
                "Phase 3 proof ledger row {} should be `{expected_id}`",
                index + 1
            ));
        }
        if !row.contains(",\"PROVED\",\"") {
            return Err(format!(
                "Phase 3 proof ledger `{expected_id}` should be PROVED"
            ));
        }
    }
    Ok(())

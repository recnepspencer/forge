...
const LEGACY_CRATE_EXPECTATIONS: &[LegacyCrateExpectation] = &[
    LegacyCrateExpectation {
        package_name: "forge-ui-components",
        member_path: "crates/forge-ui-components",
        disposition: LegacyDisposition::Owner,
    },
    LegacyCrateExpectation {
        package_name: "forge-ui-theme",
        member_path: "crates/forge-ui-theme",
        disposition: LegacyDisposition::Owner,
    },
    LegacyCrateExpectation {
        package_name: "forge-ui-adapters",
        member_path: "crates/forge-ui-adapters",
        disposition: LegacyDisposition::Removed,
    },
    LegacyCrateExpectation {
        package_name: "forge-ui-state",
        member_path: "crates/forge-ui-state",
        disposition: LegacyDisposition::Removed,
    },
    LegacyCrateExpectation {
        package_name: "forge-ui-types",
        member_path: "crates/forge-ui-types",
        disposition: LegacyDisposition::Removed,
    },
];
...
pub fn audit_no_parallel_legacy_authority(workspace_root: &Path) -> Vec<String> {
    ...
    let owner_expectations = [
        (
            "forge-ui-components",
            workspace_root.join("crates/forge-ui-components/Cargo.toml"),
        ),
        (
            "forge-ui-theme",
    ...
    for (crate_name, manifest_path) in owner_expectations {
        ...
        for forbidden_legacy_dep in ["forge-ui-adapters", "forge-ui-state", "forge-ui-types"] {
    ...
    let component_root_source =
        read_source(&workspace_root.join("crates/forge-ui-components/src/lib.rs"));
    ...
    if !component_forbidden_symbols.is_empty() {
        violations.push(format!(
            "forge-ui-components crate root reintroduces legacy authority symbols: {}",
            component_forbidden_symbols.join(", ")
        ));
    }

    let theme_root_source = read_source(&workspace_root.join("crates/forge-ui-theme/src/lib.rs"));
    ...
    if !theme_forbidden_symbols.is_empty() {
        violations.push(format!(
            "forge-ui-theme crate root reintroduces legacy authority symbols: {}",
...
pub fn audit_legacy_shim_honesty(workspace_root: &Path) -> Vec<String> {
    ...
    for retired_root in [
        workspace_root.join("crates/forge-ui-adapters"),
        workspace_root.join("crates/forge-ui-state"),
        workspace_root.join("crates/forge-ui-types"),
    ] {
...
pub fn audit_legacy_public_surface_narrowing(workspace_root: &Path) -> Vec<String> {
    ...
    for retired_member in [
        "crates/forge-ui-adapters",
        "crates/forge-ui-state",
        "crates/forge-ui-types",
    ] {
    ...
    let component_dependencies =
        crate_dependency_keys(&workspace_root.join("crates/forge-ui-components/Cargo.toml"));
    if component_dependencies.contains("forge-ui-types") {
        violations.push(
            "forge-ui-components still widens the old forge-ui-types surface instead of owning its visual API directly"
                .to_string(),
        );
    }

    let theme_dependencies =
        crate_dependency_keys(&workspace_root.join("crates/forge-ui-theme/Cargo.toml"));
    if theme_dependencies.contains("forge-ui-types") {
        violations.push(
            "forge-ui-theme still widens the old forge-ui-types surface instead of owning theme truth directly"
                .to_string(),
        );
    }

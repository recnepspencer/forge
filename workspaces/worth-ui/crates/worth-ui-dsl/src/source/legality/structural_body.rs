            ...
            match self.peek_identifier().as_deref() {
                ...
                Some("sizing") => {
                    ...
                    if sizing_contract_id_text.replace(sizing_id.clone()).is_some() {
                        return Err(WorthUiStructuralParseFailure {
                            code: WorthUiStructuralLanguageDiagnosticCode::DuplicateRegionSizingDeclaration,
                ...
                Some("state") => {
                    ...
                    if state_slot_id_text.replace(state_id.clone()).is_some() {
                        return Err(WorthUiStructuralParseFailure {
                            code: WorthUiStructuralLanguageDiagnosticCode::DuplicateRegionStateDeclaration,
        ...
        while !self.peek_semicolon() {
            match self.peek_identifier().as_deref() {
                Some("placement") => {
                    ...
                    {
                        return Err(WorthUiStructuralParseFailure {
                            code: WorthUiStructuralLanguageDiagnosticCode::DuplicateMountPlacementDeclaration,
                ...
                Some("state") => {
                    ...
                    if state_slot_id_text.replace(state_id.clone()).is_some() {
                        return Err(WorthUiStructuralParseFailure {
                            code: WorthUiStructuralLanguageDiagnosticCode::DuplicateMountStateDeclaration,
                            authored_text: state_id,
                            structural_locus: format!("{locus}/mount:{surface_id_text}"),
                        });
                    }

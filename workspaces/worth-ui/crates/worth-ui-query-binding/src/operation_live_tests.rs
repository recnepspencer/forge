use crate::{
    WorthUiCollectionAllocationEffect, WorthUiCollectionChangeKind, WorthUiCollectionGraphEffect,
    WorthUiCollectionMeasurementEffect, WorthUiOperationLiveCloseOutcome,
    WorthUiOperationLiveRefreshDenial, WorthUiOperationLiveRefreshError,
    WorthUiOperationLiveRefreshOutcome, WorthUiQueryBindingSuccessionDenial,
};
...
fn assert_incremental_update(consequence: &crate::WorthUiCollectionChangeConsequence) {
    ...
    // Query materializes the one changed native fact while planning the exact
    // patch. WUI borrows that applied fact without scanning or reconstructing it.
    assert_eq!(consequence.query_work().native_facts_materialized(), 1);

...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum WorthUiQueryResolutionMode {
    RuntimeDirect,
    StoreDirect,
}
    ...
    pub fn resolution_mode(&self) -> WorthUiQueryResolutionMode {
        match self.resolution_report.resolution_mode() {
            worth_query::facade::foundation::BasisResolutionMode::RuntimeDirect => {
                WorthUiQueryResolutionMode::RuntimeDirect
            }
            worth_query::facade::foundation::BasisResolutionMode::StoreDirect => {
                WorthUiQueryResolutionMode::StoreDirect
            }
        }
    }

    pub fn basis_digest_for_diagnostics(&self) -> &str {
        self.resolution_report.basis_digest().as_str()
    }

    pub fn canonical_basis_digest(&self) -> worth_foundational::CanonicalDerivedDigest {

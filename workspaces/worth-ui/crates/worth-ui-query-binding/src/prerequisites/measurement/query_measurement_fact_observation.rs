...
pub struct WorthUiQueryMeasurementRefinementCounters {
    declared_measurement_family_count: usize,
...
impl WorthUiQueryMeasurementFactObservation {
    ...
    > {
        ...
        let declared_measurement_family_count = authority
            .contract()
            .fact_families()
            .iter()
            .filter(|family| {
                family.kind().as_str() == "display_field"
                    || family.kind().as_str() == "derived_field"
            })
            .count();
        let projected_measurement_fact_count = authority.facts().display_fields().len()
        ...
        Ok((
            ...
            WorthUiQueryMeasurementRefinementCounters {
                declared_measurement_family_count,
...
impl WorthUiQueryMeasurementRefinementCounters {
    pub fn declared_measurement_family_count(self) -> usize {
        self.declared_measurement_family_count
    }

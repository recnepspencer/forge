...
pub(crate) struct WorthUiPlanRegionExecutable {
    ...
    spatial_meaning:
        Option<Rc<crate::runtime::planning::execution_plan_input::WorthUiSpatialPlanMeaning>>,
    realtime_meaning:
...
impl WorthUiPlanRegionExecutable {
    pub(super) fn lower(
        ...
        let child_range_count = (!child_targets.is_empty())
            .then(|| {
                crate::runtime::execution::handle_allocation::WorthUiHandleCapacity::child_range(
                    child_targets.len(),
                )
            })
            .transpose()?;
    ...
    pub(super) fn semantic_digest(&self, seed: u64) -> u64 {
        ...
        digest = fold(
            ...
            self.spatial_meaning.as_deref().map_or(
                0,
                crate::runtime::planning::execution_plan_input::WorthUiSpatialPlanMeaning::semantic_digest,
            ),
        );
        digest = fold(
            digest,

...
fn admit_member_intrinsic_contribution_witness(
    ...
    let mut query_extent_bits = None;
    ...
    for input in measurement_basis.evidence_inputs() {
        if let Some(evidence) = input.as_child_intrinsic_measurement() {
            ...
            if let Some(receipt) = evidence.query_projection_fact() {
                query_extent_bits = receipt
                    .observations()
                    .first()
                    .map(|observation| observation.extent().to_bits());
            }
    ...
    if (query_extent_bits.is_some() || saw_host_intrinsic)
    ...
    match (query_extent_bits, host_intrinsic) {
        (None, None) => Ok(None),
        (Some(primary_extent_bits), None) => {
        ...
        (None, Some(host)) => Ok(Some(UiConstraintChildIntrinsicContribution::new(
            contributor_graph_node_identity,
            axis_scope,
            host.primary_extent_bits,
            host.cross_extent_bits,
        ...
        (Some(primary_extent_bits), Some(host)) => {
            if host.primary_extent_bits != primary_extent_bits
...
fn merge_host_intrinsic(
    ...
    Ok(Some(match existing {
        None => admitted,
        Some(existing)
            if existing.primary_extent_bits == admitted.primary_extent_bits
                && existing.cross_extent_bits == admitted.cross_extent_bits
...
fn admitted_host_intrinsic(
    ...
    let (primary_extent_bits, cross_extent_bits) =
...
fn extents_for_axis_scope(
    ...
    axis_scope: UiConstraintAxisScope,
    primary_axis: UiLayoutOperatorPrimaryAxis,
) -> Result<(u32, Option<u32>), UiConstraintPropagationDenialReason> {
    ...
    Ok(match axis_scope {
        UiConstraintAxisScope::Primary => (primary.to_bits(), None),
        ...
        UiConstraintAxisScope::Both => {
            let cross = cross
                .ok_or(UiConstraintPropagationDenialReason::MissingRequiredIntrinsicContribution)?;
            (primary.to_bits(), Some(cross.to_bits()))
        }
    })
}

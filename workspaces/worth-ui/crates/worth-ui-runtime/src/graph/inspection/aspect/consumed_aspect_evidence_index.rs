...
pub(crate) struct UiConsumedAspectEvidenceIndex {
...
impl UiConsumedAspectEvidenceIndex {
    pub(crate) fn rebuild(
        ...
        let by_canonical_label: BTreeMap<Box<str>, UiAspectEvidenceNeighborhood> = graph_snapshot
            ...
            .consumed_aspects()
            .iter()
            .map(|(aspect, consumers)| {
                (
                    aspect.canonical_label().into(),
                    neighborhood_for_consumers(
                        graph_snapshot,
...
fn neighborhood_for_consumers(
    ...
    for consumer in consumers {
        match consumer.kind() {
            UiGraphAspectConsumerKind::GraphNode(graph_node_identity) => {

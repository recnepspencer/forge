...
pub enum UiGraphContainmentClaim {
    RootPage,
    PageSet { page_set_name: Box<str> },
    Region { region_name: Box<str> },
    Mosaic { mosaic_name: Box<str> },
    LocalComposition { local_composition_name: Box<str> },
    Control { control_name: Box<str> },
    DiagnosticSurface { diagnostic_surface_name: Box<str> },
}

...
mod operator_digest;
use operator_digest::{containment_kind_digest, operator_family_digest, operator_kind_digest};
...
impl UiLayoutOperatorPlanningContract {
    pub(crate) fn new(mut input: UiLayoutOperatorPlanningContractInput) -> Self {
        input
            .evidence_requirements
            .sort_unstable_by_key(evidence_requirement_rank);
        let semantics = UiLayoutOperatorPlanningSemantics::for_operator_kind(
            input.operator_kind,
            input.measurement_mode,
            input.basis_source,
            input.ownership_posture,
        );
        let identity = planning_contract_identity(&input, &semantics);
...
fn planning_contract_identity(
    input: &UiLayoutOperatorPlanningContractInput,
    semantics: &UiLayoutOperatorPlanningSemantics,
) -> UiLayoutOperatorContractIdentity {
    let basis = stable_text_digest("worth-ui.layout-operator-planning-contract")
        ^ operator_kind_digest(input.operator_kind).rotate_left(7)
        ^ operator_family_digest(input.operator_family).rotate_left(11)
        ^ containment_kind_digest(input.containment_kind).rotate_left(13)
        ^ mosaic_sizing_contract_id_digest(input.mosaic_sizing_contract_id.as_ref())
            .rotate_left(15)
        ^ slot_participation_kind_digest(input.slot_participation_kind).rotate_left(17)
        ^ ordering_guarantee_digest(input.ordering_guarantee).rotate_left(19)
        ^ repetition_posture_digest(input.repetition_posture).rotate_left(21)
        ^ (input.neighborhood_class as u64).rotate_left(23)
        ^ (input.membership_rule as u64).rotate_left(27)
        ^ measurement_mode_digest(input.measurement_mode).rotate_left(29)
        ^ constraint_modifier_digest(input.constraint_modifier).rotate_left(31)
        ^ basis_source_digest(input.basis_source).rotate_left(37)
        ^ ownership_posture_digest(input.ownership_posture).rotate_left(41)
        ^ semantics.identity_digest().rotate_left(43);
    UiLayoutOperatorContractIdentity::new(input.evidence_requirements.iter().fold(
        basis,
        |digest, requirement| {
            digest.rotate_left(11) ^ evidence_requirement_digest(*requirement).rotate_left(47)
        },
    ))
}
...
fn measurement_mode_digest(mode: Option<UiDeclaredMeasurementMode>) -> u64 {
    match mode {
        ...
        Some(UiDeclaredMeasurementMode::ViewportInset {
        ...
        Some(UiDeclaredMeasurementMode::ViewportRegion {
            horizontal,
            vertical,
        }) => stable_text_digest(&format!(
            "worth-ui.operator-contract.mode.viewport-region:{}:{}",
            horizontal.digest_basis(),
            vertical.digest_basis(),
        )),

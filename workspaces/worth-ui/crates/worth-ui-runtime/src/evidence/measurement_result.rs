use worth_ui_host_contract::{
    ...
    UiHostObservationValue, UiMeasurementRequestIdentity, UiNativeControlIntrinsicSizeObservation,
...
impl UiMeasurementValue {
    ...
    pub(crate) fn from_host_observation_value(value: UiHostObservationValue) -> Self {
        match value {
            UiHostObservationValue::TextIntrinsicSize(value) => Self::TextIntrinsicSize(value),
            UiHostObservationValue::TextBaselineMetrics(value) => Self::TextBaselineMetrics(value),
            UiHostObservationValue::FontMetrics(value) => Self::FontMetrics(value),
            UiHostObservationValue::NativeControlIntrinsicSize(value) => {
                Self::NativeControlIntrinsicSize(value)
            }
            UiHostObservationValue::ViewportExtent(value) => Self::ViewportExtent(value),
            UiHostObservationValue::DpiScaleFactor(value) => Self::DpiScaleFactor(value),
            UiHostObservationValue::PortalAnchorRect(value) => Self::PortalAnchorRect(value),
            UiHostObservationValue::ScrollContainerViewport(value) => {
                Self::ScrollContainerViewport(value)
            }
        }
    }
}
...
impl UiMeasurementResult {
    pub(crate) fn from_host_observation(
        ...
        let value = UiMeasurementValue::from_host_observation_value(observation.value().clone());

...
use worth_query::facade::read::{
    current, declare, project_facts, WorthQueryProjectionDeclaration, WorthQueryProjectionOutcome,
};
...
pub(crate) fn entity_identity_projection_context(
    ...
    projection_consumption_with_world(lane_label, project_facts().entity_identities())
}
...
fn projection_consumption(
    ...
    let (prerequisites, attempt, _) = projection_consumption_with_world(lane_label, declaration);

...
use crate::obligations::inspection::{
    ...
    UiObligationEvidenceRecord, UiObligationLegalityReasonEvidence,
};
...
fn admission_record(
    ...
    UiObligationEvidenceRecord::new(
        ...
        UiObligationEvidenceAuthoritySource::AdmissionReport,
        authority_digest,
        target.graph_node_identity().digest(),
        touch_identity_digest,
        None,
        UiObligationEvidenceDecision::Admission,
        None,
        None,
        legality_reason.and_then(denial_posture_from_legality_reason),
        Box::new([]),
        prerequisite_sources_from_target(target).into_boxed_slice(),
        query_prerequisite_evidence_from_target(target).into_boxed_slice(),
        None,
        legality_reason,
    )
}

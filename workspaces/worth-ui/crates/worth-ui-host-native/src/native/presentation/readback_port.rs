...
pub(super) trait UiNativeReadbackPort {
    ...
    ) -> Result<UiNativeReadbackObservation, UiNativeReadbackFailure>;
}

#[derive(Debug)]
pub(super) enum UiNativeReadbackFailure {
    Poll,
    Completion,
    Mapping,
    MalformedBytes,
...
pub(super) struct UiNativeReadbackObservation {
    pixels: [[u8; 4]; 2],
    crossing_count: u8,
}

impl UiNativeReadbackObservation {
    pub(super) const fn into_parts(self) -> ([[u8; 4]; 2], u8) {
        (self.pixels, self.crossing_count)
    }
}
...
impl UiNativeReadbackPort for UiWgpuNativeReadbackPort {
    fn read_two_pixels(
        ...
        let first = bytes
            ...
            .ok_or(UiNativeReadbackFailure::MalformedBytes)?;
        let second = bytes
            .get(256..260)
            .and_then(|pixel| pixel.try_into().ok())
            .ok_or(UiNativeReadbackFailure::MalformedBytes)?;
        drop(bytes);
        buffer.unmap();
        Ok(UiNativeReadbackObservation {
            pixels: [first, second],
            crossing_count: 1,
        })
    }
...
fn map_bytes(
    ...
    buffer: &wgpu::Buffer,
    submission: &wgpu::SubmissionIndex,
) -> Result<wgpu::BufferView, UiNativeReadbackFailure> {
    ...
    device
        ...
        .map_err(|_| UiNativeReadbackFailure::Poll)?;
    receiver
        .recv_timeout(std::time::Duration::from_millis(50))
        .map_err(|_| UiNativeReadbackFailure::Completion)?
        .map_err(|_| UiNativeReadbackFailure::Mapping)?;
...
mod tests {
    ...
    fn wgpu_readback_port_returns_the_mapped_nonuniform_source_bytes() {
        super::prove_nonuniform_readback_port();
    }
}

#[cfg(test)]
pub(crate) fn prove_nonuniform_readback_port() {
    ...
    let observation = UiWgpuNativeReadbackPort::read_two_pixels(&device, &readback, &submission)
        .expect("exact production readback port succeeds");
    let (pixels, crossing_count) = observation.into_parts();
    assert_eq!(pixels, [[13, 29, 71, 255], [199, 5, 151, 17]]);
    assert_eq!(crossing_count, 1);
}

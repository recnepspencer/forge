...
pub(super) fn run(
    ...
    let mut timings = timing::C6SiegeTimings::new();

    let started = Instant::now();
    let mutants = crate::mutation_campaign::load_physical_work_evidence(mutant_report, workspace)?;
    timings.record(timing::SiegePhase::MutationEvidence, started.elapsed());

    let started = Instant::now();
    let world = world::C6SiegeWorld::create(target_root)?;
    timings.record(timing::SiegePhase::World, started.elapsed());

    let binaries = binary_binding::BuiltCourtroomExecutables::build(workspace)?;
    timings.record(
        timing::SiegePhase::BinaryBuild,
        binaries.cargo_build_elapsed(),
    );
    let binding = binaries.binding_timings();
    timings.record(
        timing::SiegePhase::SourceInventory,
        binding.source_inventory(),
    );
    timings.record(
        timing::SiegePhase::PrebuildSourceBinding,
    ...
    timings.record(timing::SiegePhase::RunProvenance, started.elapsed());
    let started = Instant::now();
    let evidence = oracle::verify(&world, &binaries, observations, mutants, rerun)?;
    timings.record(timing::SiegePhase::OracleVerification, started.elapsed());
    timings.record(
        timing::SiegePhase::CampaignBeforeReport,

...
fn expected_contract(
    ...
    match id {
        ...
        "C8-P1-LEDGER-01" => Ok(("The living ledger contains every Phase 1 semantic guarantee exactly once with causal evidence and closed finding history", "C8 closure-ledger gate", "phase_one_ledger_is_complete_resolved_and_bound_to_current_source plus causal ledger mutants", "historical review capture tokens are retained as non-reproducible non-proof; the current closure review is exactly source-manifested")),
        "C8-P1-LEDGER-02" => Ok(("Every proved Phase 1 row is bound to its own causally relevant current source closure", "C8 closure-ledger source identity", "per_guarantee_source_closures_are_causal over role-specific artifacts producers consumers observer children trybuild and gates", "historical review capture tokens are not source proof")),

...
pub(crate) struct RuntimeRootObservationConnectorRequest {
    isolated_store_root: PathBuf,
    external_report_path: PathBuf,
    wire: RootWireIdentity,
}

impl RuntimeRootObservationConnectorRequest {
    pub(crate) fn new(
        ...
        wire.require_binding(RootWireRole::Recovery, scenario.identity(), expected_run)?;
        Ok(Self {
            isolated_store_root: row.root().to_path_buf(),
            external_report_path: scenario.reports().runtime().to_path_buf(),
            wire,
        })
    }

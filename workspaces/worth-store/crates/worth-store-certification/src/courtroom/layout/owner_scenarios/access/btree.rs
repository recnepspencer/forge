...
fn replay_source(
    ...
    expected_store: worth_store_physical_format::PhysicalStoreIdentity,
...
pub(super) fn execute_lookup(ledger: &mut LayoutOwnerObservationLedger) -> AccessScenarioEvidence {
    ...
    for (source, slot) in [
            ...
            )
        {
            performance = outcome
                .counter_receipt()
                .map(worth_store_layout_indexes::LayoutAccessPerformanceReceipt::from_btree_lookup);
...
fn page_request_with_source<'a>(
    ...
    security: &'a worth_store_security::StoreAdmittedSecurityScope,
    slot: PhysicalRecordSlot,
    source: worth_store_layout_indexes::BaselineBTreeReadSource,
) -> PageLookupRequest<'a> {

...
) -> worth_store_wal::BlobWalReplayRebuildWitness {
    let worth_store_layout_indexes::LayoutMaterializationSourceKind::LsmReplacement(identity) =
...
pub(super) fn wal_source_with_next_identity(
    ...
    let identity =
        worth_store_wal::BlobWalRecordIdentity::new(identity.sequence() + 1, identity.kind())
            .expect("the hostile WAL identity remains structurally valid");
    wal_source(identity, security)
}

fn wal_source(
    identity: worth_store_wal::BlobWalRecordIdentity,
    security: &worth_store_security::StoreAdmittedSecurityScope,
) -> worth_store_wal::BlobWalReplayRebuildWitness {
    let record = worth_store_wal::BlobWalRecordEnvelope::new(
        identity,
        worth_store_wal::DurablePublicationDeclaration::wal_frame(
            worth_store_wal::WalFrameDurablePublicationScope::new(
                1,
                1,
                10,
                20,
    ...
    worth_store_wal::BlobWalReplayRebuildWitness::admit(
        worth_store_wal::WalSecurityMetadataEnvelope::wal_record(record, metadata),
    )
}

use worth_store_layout_indexes::integrity::{layout_corruption, offline_readmission};
...
pub(super) fn record(ledger: &mut LayoutOwnerObservationLedger) {
    ...
    let reopened =
        worth_store_test_support::reopened_recovery_artifact_fixture("integrity-offline-matrix");
    let requirement = || {
        layout_corruption()
    ...
    let other_reopened =
        worth_store_test_support::reopened_recovery_artifact_fixture("integrity-offline-other");

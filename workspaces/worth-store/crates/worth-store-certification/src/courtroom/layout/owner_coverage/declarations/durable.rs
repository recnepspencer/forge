...
pub(super) fn register(declarations: &mut LayoutOwnerCaseDeclarations) {
    ...
    for (family, operation) in [
        ...
        declarations.insert(
            family,
            worth_store_lsm_authority::lsm_membership_owner_case_inventory()
    ...
    for (family, operation) in [
        ...
        declarations.insert(
            family,
            worth_store_layout_indexes::lsm_execution_owner_case_inventory()
                .filter(move |case| case.id().operation() == operation)
                .map(|case| case.id().disposition().as_str()),
        );
    }
    declarations.insert(
        ...
        worth_store_physical_isolation::compaction_owner_case_inventory()

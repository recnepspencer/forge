...
pub(super) use record_matching::PhaseRecordArtifacts;
use record_matching::{
    matching_record_fingerprints, operation_record_identity, operation_with, record_set_identity,
};
...
pub(super) fn repair_execution_identity(
    ...
    Ok(operation_record_identity(
        ...
        b"s10-phase-12",
        |record| {
            repair_effect_started(record)
                || repair_receipt(record)
                || repair_disposition(record)
                || publication_prepared(record)
                || publication_pending(record)
                || publication_disposition(record)
                || fence_released(record)

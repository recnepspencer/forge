...
fn source_map_covers_scanned_store_workspace_family_surfaces() {
    ...
    let scope_roots = [
        ...
        "workspaces/worth-store/crates/worth-store-readiness/src",
    ...
    assert!(scanned.iter().any(|family| {
        family.family_name() == "DerivedIndexAuthorityEvidence"
            && family.source_path()
                == "workspaces/worth-store/crates/worth-store-physical-integrity/src/index_pages/index_page_integrity_request.rs"
    }));
    assert!(scanned.iter().any(|family| {
        family.family_name() == "ExecutedIntegrityBoundaryDenialEvidence"
            && family.source_path()
                == "workspaces/worth-store/crates/worth-store-certification/src/courtroom/physical_integrity/closeout/suite/physical_integrity_closeout_proof.rs"
    }));
    assert!(scanned.iter().any(|family| {
        family.family_name() == "PhysicalRecoverySource"
            && family.source_path()
                == "workspaces/worth-store/crates/worth-store-recovery-physics/src/source_precedence/physical_source.rs"
    }));
    assert!(scanned.iter().any(|family| {
...
fn unclassified_store_evidence_family_fails_with_classifying_subsystem() {
    let denial = certify_store_canonical_basis_source_rows(&[
        StoreCanonicalBasisInventoryRow::unclassified(
            ...
            "workspaces/worth-store/crates/worth-store-recovery-physics/src/new_receipt.rs",
    ...
    assert_eq!(
        ...
        StoreCanonicalBasisInventoryDenial::UnclassifiedEvidenceFamily {
            ...
            classifying_subsystem: "worth-store-recovery-physics",
        }
    );
}

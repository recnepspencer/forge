...
use worth_store_buffer_pool::ResidentFrameDenialKind;
...
fn clean_shutdown_evidence_rejects_unproven_denial_rows() {
    ...
    let report = table.dirty_shutdown_closeout();
    let evidence = DirtyPublicationEvidenceReport::from_shutdown(report).unwrap();
    let fake_denial = worth_store_buffer_pool::ResidentFrameDenial::from_shortcut_attempt(
        worth_store_buffer_pool::ResidentFrameShortcutAttempt::BackendPrivateResidue,
    );

...
fn publication_only_compaction_observation_cannot_satisfy_counter_contracts() {
    ...
    assert_eq!(
        ...
        CounterMismatchEvidence::PositiveCounterNotPositive {
            kind: worth_store_physical_certification::CounterContractKind::BlockedReclaimAttempts,
            actual: 0,
        }
    );
}
...
fn assert_oracle_denied<O: PhysicalProofOracle>(
    ...
    assert!(matches!(
        denial,
        worth_store_physical_certification::OracleDenial::CompactionInterlockObservationDenied {
            oracle
        } if oracle == expected
    ));
}

...
fn evidence_materializes_foundational_packaging_without_authority_promotion() {
    ...
    assert_eq!(
        ...
        worth_foundational::FoundationalBoundaryMaterializationSource::NativeAuthority
    );
    assert_eq!(
        worth_store_physical_certification::reject_foundational_materialization_as_store_authority(
        )
        .unwrap_err(),
        worth_store_physical_certification::PhysicalEvidenceBundleDenial::FoundationalMaterializationIsNotStoreAuthority
    );
}
...
fn boundary_bridged_foundational_evidence_requires_explicit_store_readmission() {
    ...
    let readmitted =
        worth_store_physical_certification::readmit_foundational_physical_evidence_after_boundary(
            bridged,
        );
...
fn storage_no_fault_control(artifact_id: u64) -> PhysicalFaultEvent {
    PhysicalFaultEvent::no_fault_control(
        ...
        PhysicalArtifactFaultLocus::root_pointer(
            ...
            worth_store_physical_certification::ExpectedFaultLocalization::ProductionDriverBoundary,
        ),
    )
    .unwrap()
}

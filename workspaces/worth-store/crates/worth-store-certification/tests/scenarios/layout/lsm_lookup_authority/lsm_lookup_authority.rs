...
use worth_store_lsm_authority::{LsmMembershipDenial, LsmMembershipReplayPosture};
use worth_store_security::admitted_store_wal_checkpoint_security_scope_for_layout_partition_test;
use worth_store_security::admitted_tenant_wal_checkpoint_security_scope_for_layout_partition_test;
...
use worth_store_wal::{AdmittedWalAppendReceipt, AdmittedWalArtifactStore, StoreWalRecordIdentity};
...
fn ordinary_runtime_selects_and_executes_persisted_lsm_membership() {
    ...
    assert_eq!(
        newest
            ...
            .source()
            .kind(),
        worth_store_layout_indexes::LayoutMaterializationSourceKind::LsmReplacement(
...
fn missing_activation_cannot_retire_durable_membership() {
    ...
    assert!(
        worth_store_lsm_authority::select_lsm_compaction_membership(&reopened, fixture.key())
            .into_result()
            .is_ok()
    );
    assert_eq!(
        worth_store_lsm_authority::lookup_published_lsm_membership(&reopened, fixture.key())
            .into_result(),
...
fn every_torn_activation_prefix_fails_closed_or_leaves_old_membership_active() {
    ...
    for cut in 1..fixture.activation_bytes().len() {
        ...
        match reopen(fixture.anchor()) {
            Ok(reopened) => {
                assert!(worth_store_lsm_authority::select_lsm_compaction_membership(
                    &reopened,
                    fixture.key()
                )
                .into_result()
                .is_ok());
                assert_eq!(
                    worth_store_lsm_authority::lookup_published_lsm_membership(
                        &reopened,
                        fixture.key(),
                    )
                    .into_result(),
...
fn malformed_replay_membership_is_rejected_by_its_owning_boundary() {
    ...
    assert!(matrix
        .permutation_denials()
        .iter()
        .all(|denial| *denial == worth_store_lsm_authority::LsmReplaySourceDenial::MembershipSequenceNotStrictlyIncreasing));
    assert_eq!(
        matrix.duplicate_sequence_denial(),
        worth_store_lsm_authority::LsmReplaySourceDenial::MembershipSequenceNotStrictlyIncreasing
    );
    assert!(matrix
...
fn execute(
    ...
    security: &worth_store_security::StoreCurrentSecurityScopeWitnessSet,
    source: worth_store_layout_indexes::BaselineLsmLookupSource,
    probe_sequence: u64,
) -> worth_store_layout_indexes::BaselineLsmLookupExecution {
    layout_read_runtime()

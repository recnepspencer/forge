...
fn physical_isolation_scenario_families_lower_replay_cover_and_emit_evidence_through_simulation_harness_pipeline(
    ...
    for lane in worth_store_certification::physical_isolation_lanes() {
        ...
        let mutation = worth_store_certification::PhysicalIsolationMutationEvidence::from_replay(
            plan.scenario_family(),
            &replay,
        );
        let matrix = worth_store_certification::physical_isolation_coverage_matrix(
            lane.scenario(),
            &plan,
            &replay,
            &mutation,
...
fn physical_isolation_family_lowering_is_readiness_gated_and_rejects_missing_required_simulation_harness_surfaces(
    ...
    let lane = worth_store_certification::physical_isolation_lanes()
        .into_iter()
        .find(|lane| {
            lane.scenario().definition().family()
...
fn readiness_shape_probe_remains_non_claiming_and_not_physical_isolation_closeout_authority() {
    let probe = physical_scenario("store.physical.s5.readiness-shape.non-claim")
        ...
        .schedule(worth_store_test_support::physical_isolation_boundary_yieldpoint())
...
fn assert_family_mutation_topology(
    family: PhysicalSimulationScenarioFamily,
    mutation: &worth_store_certification::PhysicalIsolationMutationEvidence,
    ...
    let mutation_result_row = matrix
        ...
        .unwrap();
    for required in worth_store_certification::physical_isolation_required_mutation_rows(family) {
        assert!(
...
fn row_for_matrix_contains_physical_isolation_mutation(
    ...
    mutation: worth_store_physical_certification::PhysicalIsolationMutationKind,
) -> bool {
    row.has_dimension(
        &worth_store_physical_certification::CoverageRowDimension::PhysicalIsolationMutation(
            mutation,
        ),
    )
}

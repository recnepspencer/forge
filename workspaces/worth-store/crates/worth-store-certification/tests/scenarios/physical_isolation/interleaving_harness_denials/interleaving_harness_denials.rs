...
fn physical_isolation_interleaving_oracle_rejects_wrong_family_observation_topology() {
    for lane in worth_store_certification::physical_isolation_lanes() {
        ...
        let trace =
            worth_store_certification::observe_physical_isolation_trace(&plan, &schedule, fixtures)
                .unwrap();
...
fn physical_isolation_observer_requires_scheduled_mutation_rows_for_mutation_bound_families() {
    for lane in worth_store_certification::physical_isolation_lanes() {
        let plan =
            lower_physical_simulation_plan(lane.scenario().clone(), complete_context()).unwrap();
        let schedule = schedule(&plan);
        let result = worth_store_certification::observe_physical_isolation_trace(
            &plan,
            &schedule,
            ...
            == PhysicalSimulationScenarioFamily::PhysicalIsolationFutureChunkStability
        {
            ...
            let mutation =
                worth_store_certification::PhysicalIsolationMutationEvidence::try_from_replay(
            ...
            assert_eq!(
                ...
                worth_store_certification::physical_isolation_required_mutation_rows(
                    PhysicalSimulationScenarioFamily::PhysicalIsolationFutureChunkStability,
                )
            );
        } else {
...
fn future_chunk_trace_polluted_with_compaction_mutations(
    ...
    let compaction_lane = worth_store_certification::physical_isolation_lanes()
        .into_iter()
        .find(|lane| {

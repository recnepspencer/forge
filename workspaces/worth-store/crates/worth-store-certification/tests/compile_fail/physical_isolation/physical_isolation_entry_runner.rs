...
fn compile_fail_fixtures() -> Vec<CompileFailFixture> {
    vec![
        ...
        CompileFailFixture {
            name: "readiness_alone_cannot_register_lane.rs",
            expected_stderr: &["PhysicalIsolationHarnessReadinessReceipt"],
        },
        CompileFailFixture {
            name: "copied_rows_cannot_register_lane.rs",
            expected_stderr: &["PhysicalIsolationHarnessReadinessReceipt"],
        },
        CompileFailFixture {
...
fn compile_fail_case_dir(fixture_name: &str) -> std::path::PathBuf {
    std::env::temp_dir()
        .join("worth-store-s5-physical-isolation-entry-ui")

...
fn compile_fail_fixtures() -> Vec<CompileFailFixture> {
    vec![
        CompileFailFixture {
            name: "handoff_receipt_cannot_be_struct_literal.rs",
            expected_stderr: &["PhysicalIsolationHarnessReadinessReceipt", "private"],
        },
        CompileFailFixture {
            name: "physical_isolation_cannot_accept_generic_runner.rs",
            expected_stderr: &["accept_store_owned_s5_harness_readiness", "no"],
        },
        CompileFailFixture {
            name: "generic_runner_cannot_implement_readiness_contract.rs",
            expected_stderr: &["PhysicalIsolationHarnessReadinessContract", "not found"],
        },
        CompileFailFixture {
            name: "future_slot_cannot_be_readiness.rs",
            expected_stderr: &[
...
fn compile_fail_case_dir(fixture_name: &str) -> std::path::PathBuf {
    std::env::temp_dir()
        .join("worth-store-s45-s5-readiness-ui")
...
fn fixture_manifest(repo_root: &std::path::Path) -> String {
    format!(
        ...
        manifest_path(&repo_root.join("workspaces/worth-store/crates/worth-store-physical-certification")),
        manifest_path(&repo_root.join("workspaces/worth-store/crates/worth-store-physical-isolation")),
        manifest_path(&repo_root.join("workspaces/worth-store/crates/worth-store-readiness")),
    )
}

...
fn run_compile_fail_case(case_dir: &std::path::Path) -> std::process::Output {
    ...
    std::process::Command::new(rustc)
        ...
        .arg(format!(
            ...
            cargo_artifacts::compiled_extern(TEST_TARGET, "worth_store_physical_certification")
                .display()
        ))
        .arg("--extern")
        .arg(format!(
            "worth_proof={}",
            cargo_artifacts::compiled_extern(TEST_TARGET, "worth_proof").display()
        ))
        .arg("--extern")
        .arg(format!(
            "worth_store_readiness={}",
            cargo_artifacts::compiled_extern(TEST_TARGET, "worth_store_readiness").display()
        ))
        .arg("-o")
...
fn compile_fail_case_dir(fixture_name: &str) -> std::path::PathBuf {
    std::env::temp_dir()
        .join("worth-store-s45-shortcut-ui")

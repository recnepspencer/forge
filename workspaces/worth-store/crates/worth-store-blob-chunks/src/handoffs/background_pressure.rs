...
pub const fn blob_ingest_background_pressure_shape(bytes: u64) -> BackgroundPressureDeclaration {
    BackgroundPressureDeclaration::blob_ingest_pressure(bytes)
}

pub const fn blob_migration_background_pressure_shape(bytes: u64) -> BackgroundPressureDeclaration {
...
pub const fn blob_background_pressure_kind(
    ...
    match declaration.kind() {
        BackgroundPressureKind::CompactionRewrite => Some(BlobBackgroundPressureKind::Compaction),
        BackgroundPressureKind::BlobIngestPressure => Some(BlobBackgroundPressureKind::Ingest),

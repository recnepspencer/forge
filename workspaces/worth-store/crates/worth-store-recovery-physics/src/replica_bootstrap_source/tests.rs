...
fn bounded_real_media_resolution_issues_an_exact_recovery_owned_cut() {
    ...
    let cut = RecoveryPhysicsBootstrapSourceOwner::resolve(world.request(), world.open_media(), 3)
...
fn same_length_source_substitution_after_open_cannot_resolve() {
    ...
    let denial = RecoveryPhysicsBootstrapSourceOwner::resolve(world.request(), media, 2)
        .expect_err("changed bytes must not become a resolved cut");
    assert!(matches!(
        denial,
        BootstrapSourceResolutionDenial::ArtifactDigestMismatch
            | BootstrapSourceResolutionDenial::Media(
...
#[test]
fn a_digest_complete_set_without_blob_reachability_is_not_a_bootstrap_source() {
    let world = BootstrapSourceWorld::create();
    let artifacts = world
        .artifacts
        .iter()
    ...
    assert!(matches!(
        denial,
        BootstrapSourceResolutionDenial::MissingRequiredFamily(BootstrapSourceArtifactFamily::Blob)
    ));
}

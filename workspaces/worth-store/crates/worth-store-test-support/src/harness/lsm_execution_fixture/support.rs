...
#[cfg(test)]
pub(super) use worth_store_layout_indexes::access_planning;
#[cfg(test)]
pub(super) use worth_store_layout_indexes::declarations::layout_declarations;
#[cfg(test)]
pub(super) use worth_store_layout_indexes::LsmStrategy;
#[cfg(test)]
pub(super) use worth_store_lsm_authority::LsmMembershipRecord;
#[cfg(test)]
pub(super) use worth_store_wal::{BlobWalRecordEnvelope, DurablePublicationDeclaration};
...
impl ExecutedLsmCompactionFixture {
    ...
    pub const fn replay_source(&self) -> &worth_store_lsm_authority::AdmittedLsmReplaySource {
        &self.replay_source
    }
}
...
pub fn lsm_membership_replacement_crash_fixture() -> LsmMembershipReplacementCrashFixture {
    ...
    let publication = worth_store_lsm_authority::admit_lsm_membership_replacement(
        &selected, activation, checkpoint,
    )
    .unwrap();
    worth_store_lsm_authority::replace_lsm_membership(&mut session, &selected, &publication)
        .into_result()
        .unwrap();
...
fn published_lsm_manifest_materializes_its_frontier_without_claiming_live_exactness() {
    ...
    let declaration = declarations
        .declaration(worth_store_contracts::DurableArtifactFamilyId::PublicationWalIntent)
    ...
    assert_eq!(
        ...
        worth_store_layout_indexes::IndexMaintenanceMode::AsynchronousLagged
    );

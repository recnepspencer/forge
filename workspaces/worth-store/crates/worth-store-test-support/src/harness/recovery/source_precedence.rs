...
pub fn checkpoint_base(
    ...
    checkpoint_base_for_root(start, end, redo, order, root_record_reference())
}

pub fn checkpoint_base_for_root(
    start: u64,
    end: u64,
    redo: u64,
    order: u64,
    ...
    let validation = validate(manifest_for_root(start, end, redo, root));
...
pub fn wal_tail_for_checkpoint(
...
pub fn wal_only_tail(start: u64, end: u64, order: u64) -> WalTailRedoSource {
...
pub fn wal_only_tail_denial_from_torn_frame(start: u64, end: u64) -> WalOnlyTailProofDenial {
...
) -> worth_store_physical_integrity::PhysicalIntegrityEvidenceBundle {
...
pub fn wal_tail_quarantine_record() -> worth_store_physical_integrity::QuarantineRecord {
    wal_only_tail_fixture::wal_tail_quarantine_record(wal_range(30, 40))
}

pub fn compaction_visible_product_evidence(
...
pub fn admitted_compaction_cutover_for_generation(
    generation_value: u64,
) -> (
...
pub fn compaction_generation_mismatch_denial() -> CompactionVisibleProductEvidenceDenial {
...
pub fn compaction_cutover_basis_mismatch_denial() -> CompactionVisibleProductEvidenceDenial {
...
pub fn compaction_durability_artifact_mismatch_denial() -> CompactionVisibleProductEvidenceDenial {
    let generation = CompactionGenerationIdentity::new(7);
    let (_, cutover_receipt) = checkpoint_base(10, 20, 19, 1);
    let cutover = AdmittedCompactionCutoverRecord::from_checkpoint_cutover_receipt(
        generation,
...
pub fn compaction_durability_range_mismatch_denial() -> CompactionVisibleProductEvidenceDenial {
    let generation = CompactionGenerationIdentity::new(7);
    let (_, cutover_receipt) = checkpoint_base(10, 20, 19, 1);
    let cutover = AdmittedCompactionCutoverRecord::from_checkpoint_cutover_receipt(
        generation,
...
pub fn manifest(start: u64, end: u64, redo: u64) -> CheckpointManifest {
    manifest_for_root(start, end, redo, root_record_reference())
}

pub fn manifest_for_root(
    start: u64,
    end: u64,
    redo: u64,
    root: PhysicalReference,
    ...
    CheckpointManifest::sharp(
        CheckpointRootPosture::root_present(root),
        frontier(redo),
...
) -> worth_store_recovery_physics::LocatedCheckpointCandidate {

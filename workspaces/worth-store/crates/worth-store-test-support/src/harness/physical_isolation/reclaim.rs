...
pub struct ReclaimFixture {
    pub root: worth_store_physical_isolation::CurrentPhysicalRoot,
    pub lease: ProtectedReferenceLease,
    pub barrier: PhysicalReadReachabilityBarrier,
    pub compact: CompactProtectedReferenceSet,
    pub candidates: ReclaimCandidateSet,
    pub released: ReleasedOldReachability,
}
...
impl ReclaimFixture {
    ...
    pub fn with_operation_digest_authority(generation: u64, digest: &str) -> Self {
        let authority = physical_authority_from_operation_digest_closeout(digest);
        Self::from_authority(generation, &authority)
    }

    pub fn lease_for(&self, kind: HazardLeaseKind) -> ProtectedReferenceLease {
    ...
    fn from_authority(
        ...
        let compact = worth_store_physical_isolation::CompactProtectedReferenceSet::from_reference_set_with_scratch(
            references.clone(),
            ReadPlanAdmissionScratchArena::for_protected_reference_capacity(1),
        )
        .unwrap();
...
pub fn eligibility_after_releases(worlds: [ReclaimFixture; 2]) -> Vec<bool> {
    let mut table =
...
pub fn assert_reclaim_eligible_from_live_table(
...
pub fn assert_reclaim_blocked_by_live_hazard(
    world: &ReclaimFixture,
    table: &HazardLeaseTable,
    expected_live_entries: u64,
) {

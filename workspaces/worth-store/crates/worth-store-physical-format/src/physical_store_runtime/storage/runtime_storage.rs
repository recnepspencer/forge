...
use crate::{
    ...
    PhysicalReferenceAuthority, PhysicalSegmentId, PhysicalStoreRuntimeDenial,
    PhysicalStoreRuntimeDenialKind, RootPublicationCell, SlotGenerationCell,
};
use std::collections::BTreeMap;

#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub(crate) struct PhysicalStoreRuntimeStorage {
...
impl PhysicalStoreRuntimeStorage {
    ...
    ) -> Result<&StoredPageBytes, PhysicalStoreRuntimeDenial> {
    ...
    pub(crate) fn extent_for_reference(
        &self,
        reference: PhysicalReference,
    ) -> Result<&StoredExtentBytes, PhysicalStoreRuntimeDenial> {
        let segment_id = reference.segment_id().ok_or_else(missing_record)?;

use crate::{
    ExtentGenerationCell, InMemoryPhysicalFormatModel, InMemoryPhysicalFormatModelDenialKind,
    InMemoryPhysicalFormatModelRequest, PersistedPhysicalLayout, PhysicalExtentId,
    PhysicalGeneration, PhysicalGenerationAuthority, PhysicalRecordSlot, PhysicalReferenceKind,
    PhysicalSegmentId, PlatformPhysicalAppendRequest, PlatformPhysicalLayoutAccessRequest,
    SlotGenerationCell,
};
...
#[test]
fn model_append_publish_scan_restore_and_locate_stays_in_memory() {
    ...
    let append = model
        ...
        .expect("append through model");

    assert_eq!(append.reference().kind(), PhysicalReferenceKind::PageSlot);

    let mut page_layout = model.page_access();
    let located = page_layout
        .locate_record(append.reference())
        .expect("locate through model");
    assert_eq!(located.record_view().payload().as_bytes(), b"small");
    let mut page_layout = model.page_access();
    let read = page_layout
        .read_record(append.reference())
        .expect("read through model");
    assert_eq!(read.record_view().payload().as_bytes(), b"small");
    let mut frame_layout = model.frame_access();
    let framed = frame_layout
        .read_frame(append.reference())
        .expect("frame-admitted read through model");
    assert_eq!(framed.frame_view().payload().as_bytes(), b"small");
    assert_eq!(model.counters().locates(), 1);
    assert_eq!(model.counters().reads(), 2);

    let published = model
        .publish_physical_root()
        .expect("clean root publication");
    let scan = model.scan_physical_layout().expect("verifier scan");
    assert!(scan
        .platform_evidence()
        .satisfies_in_memory_observation_contract());
    assert_eq!(model.counters().appends(), 1);
    assert_eq!(model.counters().root_publications(), 1);
    assert_eq!(model.counters().scans(), 1);

    let mut restored = InMemoryPhysicalFormatModel::restore(
        readiness(),
        InMemoryPhysicalFormatModelRequest::physical_format_canonical(),
        published.replay_artifact(),
    )
    .expect("restore through verifier");
    let mut restoreed_page_layout = restored.page_access();
    let restoreed_locate = restoreed_page_layout
        .locate_record(append.reference())
        .expect("restore locate by physical reference");
    assert_eq!(
        restoreed_locate.record_view().payload().as_bytes(),
        b"small"
    );
}
...
fn hidden_broad_scan_is_rejected_before_physical_traversal_with_owner_receipt() {
    let mut model = start_model();
    let denial =
        model.reject_hidden_broad_scan(PlatformPhysicalLayoutAccessRequest::hidden_broad_scan());
...
fn physical_owner_admits_only_operation_compatible_resource_grants() {
    ...
    assert!(model
        ...
        .is_err());
    assert!(model.admit_degraded_exact_scan(0, maintenance).is_err());
    assert!(model.admit_degraded_exact_scan(33, maintenance).is_err());
    assert!(model.admit_degraded_exact_scan(8, maintenance).is_ok());
}
...
fn physical_admission_cannot_spend_unused_envelope_capacity() {
    ...
    assert!(model.admit_degraded_exact_scan(1, receipt).is_ok());
    assert!(model.admit_degraded_exact_scan(8, receipt).is_err());
}

#[test]
fn restore_rejects_ambiguous_root_candidates_without_guessing() {
    let mut model = start_model();
    model
        .append_physical_record(PlatformPhysicalAppendRequest::page_slot(
            slot_cell(),
    ...
    let denial = InMemoryPhysicalFormatModel::restore(
        readiness(),
        InMemoryPhysicalFormatModelRequest::physical_format_canonical(),
        crate::InMemoryPhysicalFormatReplayArtifact::from_persisted_layout(
            InMemoryPhysicalFormatModelRequest::physical_format_canonical()
                .headers()
                .clone(),
            builder.build(),
    ...
    .expect_err("ambiguous persisted root candidates deny restore");
...
#[test]
fn extent_records_route_through_model_as_peer_physical_placement() {
    let mut model = start_model();
    ...
    let mut extent_layout = model.extent_access();
...
fn forbidden_shortcuts_are_typed_and_counted() {
    ...
    assert_eq!(model.counters().appends(), 0);
    assert_eq!(model.counters().reads(), 0);
    assert_eq!(model.counters().locates(), 0);
    assert_eq!(model.counters().root_publications(), 0);
    assert_eq!(model.counters().scans(), 0);
    assert_eq!(model.counters().full_store_materialization_rejections(), 1);
    assert_eq!(model.counters().backend_residue_guess_rejections(), 1);
}

fn start_model() -> InMemoryPhysicalFormatModel {
    InMemoryPhysicalFormatModel::start_empty_model(
        readiness(),
        InMemoryPhysicalFormatModelRequest::physical_format_canonical(),
    )
    .expect("open physical store model")
}

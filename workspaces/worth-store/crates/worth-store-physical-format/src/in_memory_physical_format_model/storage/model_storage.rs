use super::{
    ...
    reference_index::{build_reference_index, StoredReferenceIndex},
    segment_occupancy::{build_segment_occupancy, StoredSegmentOccupancy},
};
use crate::{
    ExtentGenerationCell, InMemoryPhysicalFormatModelDenial, InMemoryPhysicalFormatModelDenialKind,
    ManifestTraversalReport, PersistedExtentBytes, PersistedPageBytes, PersistedPhysicalLayout,
    PhysicalGenerationAuthority, PhysicalReference, PhysicalReferenceAuthority, PhysicalSegmentId,
    RootPublicationCell, SlotGenerationCell,
};
use std::collections::BTreeMap;

#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub(crate) struct InMemoryPhysicalFormatModelStorage {
...
impl InMemoryPhysicalFormatModelStorage {
    ...
    ) -> Result<&StoredPageBytes, InMemoryPhysicalFormatModelDenial> {
    ...
    pub(crate) fn extent_for_reference(
        &self,
        reference: PhysicalReference,
    ) -> Result<&StoredExtentBytes, InMemoryPhysicalFormatModelDenial> {
        let segment_id = reference.segment_id().ok_or_else(missing_record)?;
    ...
    pub(crate) fn model_discovered_references(&self) -> Vec<PhysicalReference> {
    ...
    fn model_segment_count(&self) -> u32 {
        self.segment_occupancy.len() as u32
    }

    fn model_allocation_class_count(&self) -> u32 {

pub(crate) fn assert_cutover_dag_semantics(explanation: &crate::CanonicalOwnerPlanDagExplanation) {
    use crate::{OwnerPlanEffect, OwnerPlanExecutionStage, StoreOwnerKind};

    for (owner, effect, stage, irreversible) in [
        (
        ...
        (
            ...
            OwnerPlanEffect::PublishNonCurrentRoot,
            OwnerPlanExecutionStage::Cutover,
            true,
        ),
        (
            StoreOwnerKind::Authority,
            OwnerPlanEffect::ReadmitCurrentAuthority,
    ...
    let first = explanation
        ...
        .expect("publication is the first irreversible cutover effect");
    assert_eq!(
        explanation
            .nodes()
            .iter()
            .find(|node| node.identity() == first)
            .expect("irreversible identity belongs to cutover DAG")
            .owner(),
        StoreOwnerKind::PhysicalIsolation
    );
}

impl crate::physical_runtime::instance::PhysicalStoreWorkRuntime {
    ...
    > {
        ...
        let (first_dispatched, first_outcome, first_recovery) = first_execution.into_parts();
        let (second_dispatched, second_outcome, second_recovery) = second_execution.into_parts();
        let recovery = first_recovery.join(second_recovery);

        let first = crate::physical_runtime::PhysicalWorkSettlement::settle(
            crate::physical_runtime::PhysicalExecutorDispatch::new(
                first_dispatched,
                second_outcome,
                recovery,
            ),
        ...
        let (first, first_revocation, first_activity) = first.into_parts();
        ...
        let (second, second_revocation, second_activity) = second.into_parts();

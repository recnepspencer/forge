...
use super::arguments::BoundedResidencyInvocation;
...
use configuration::BoundedResidencyConfiguration;

pub(super) fn validate_configuration(path: &Path) -> Result<(), String> {
    BoundedResidencyConfiguration::read(path).map(|_| ())
}

pub(super) fn run(invocation: BoundedResidencyInvocation) -> Result<(), String> {
...
fn run_inheritance_siege(
    ...
    let profile = super::exact_write::profile()?;
    ...
    let identity = protocol::PhysicalWorkCourtroomWorldIdentity::new(
    ...
    let dirty = writeback_pressure::prove(&serving, gate)?;
    let world = protocol::BoundedResidencyWorldEvidence {
        identity,

...
mod tests {
    ...
    fn parses_library_artifact_with_windows_path() {
        let message = parse(
            r#"{"reason":"compiler-artifact","target":{"kind":["lib"],"name":"worth-store-layout-indexes"},"filenames":["C:\\target\\libworth_store_layout_indexes.rlib"]}"#,
        )
        .expect("compiler artifact parses");

        assert_eq!(message.target_name, "worth-store-layout-indexes");
        assert_eq!(
            message.filenames[0].to_string_lossy(),
            r"C:\target\libworth_store_layout_indexes.rlib"
        );
    }

    #[test]
    fn ignores_non_artifact_and_binary_messages() {
        assert!(parse(r#"{"reason":"build-finished","success":true}"#).is_none());
        assert!(parse(
            r#"{"reason":"compiler-artifact","target":{"kind":["bin"],"name":"runner"},"filenames":["runner.exe"]}"#
        )
        .is_none());
    }
}

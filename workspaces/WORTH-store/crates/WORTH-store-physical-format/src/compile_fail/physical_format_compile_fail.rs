...
//! };
//!
//! let _WORTHd = RootManifestIntegrityPosture::CurrentRootAdmitted(
...
//!
//! let _WORTHd = PhysicalChunkChecksumWitness {
...
//!
//! let _WORTHd = PhysicalChunkChecksumAuthority::s7_canonical().verify(b"raw");
//! ```
//! Chunk payload integrity cannot be admitted from raw bytes without Store write admission:
//! ```compile_fail
//! use worth_store_physical_format::PhysicalChunkChecksumAuthority;
//!
//! let _WORTHd = PhysicalChunkChecksumAuthority::s7_canonical().admit_store_payload(b"raw");
//! ```
//! Store physical chunk write receipts cannot be WORTHd from raw fields:
//! ```compile_fail
//! use worth_store_physical_format::StorePhysicalChunkWriteReceipt;
//!
//! let _WORTHd = StorePhysicalChunkWriteReceipt {
//!     payload_bytes: b"raw",
//!     bytes_written: 3,
//!     source: todo!(),
//!     _seal: todo!(),
...
//! let _WORTHd = PhysicalChunkChecksumAuthority::s7_canonical()
//!     .admit_store_payload((authority, b"raw"));
//! ```
//! Bootstrap-open witnesses cannot be WORTHd from raw fields:
//! ```compile_fail
//! use worth_store_physical_format::PhysicalBootstrapCatalogOpenWitness;
//!
//! let _WORTHd = PhysicalBootstrapCatalogOpenWitness {
//!     byte_order: todo!(),
...
//! let _WORTHd = PhysicalBootstrapCatalogOpenWitness::admit_persisted_layout(

...
fn invalid_migration_inputs_deny_before_stability_admission() {
    ...
    for reference_kind in [
        worth_store_physical_isolation::MovablePhysicalRefKind::Extent,
        worth_store_physical_isolation::MovablePhysicalRefKind::FutureChunk,
    ] {
        for claim in [
...
fn ordinary_plan() -> StablePhysicalReadPlan {
    ...
    let observation =
        worth_store_physical_isolation::PostProtectionPhysicalReadObservation::from_authority_after_hazard_publication(
...
fn movable_extent(
    barrier: worth_store_physical_isolation::PhysicalReadReachabilityBarrier,
    generation: u64,
) -> MovablePhysicalRef {
    let authority = physical_authority_from_complete_closeout();
    let root = current_root_from_authority(&authority);
...
fn assert_non_claim_report(
    report: worth_store_physical_isolation::FutureBlobMigrationNonClaimReport,
) {
    assert_eq!(
...
fn future_chunk_epoch() -> worth_store_physical_isolation::ChunkEpoch {
    let authority = physical_authority_from_complete_closeout();

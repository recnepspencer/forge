#[test]
fn forbidden_shortcut_authority_cannot_be_WORTHd_at_compile_time() {
...
fn run_compile_fail_case(case_dir: &std::path::Path) -> std::process::Output {
    ...
    std::process::Command::new(rustc)
        ...
        .arg(format!(
            ...
            latest_rlib(&deps_dir, "libworth_store_physical_certification").display()
        ))
        .arg("--extern")
        .arg(format!(
            "worth_proof={}",
            latest_rlib(&deps_dir, "libworth_proof").display()
        ))
        .arg("--extern")
        .arg(format!(
            "worth_store_readiness={}",
            latest_rlib(&deps_dir, "libworth_store_readiness").display()
        ))
        .arg("--extern")
        .arg(format!(
            "serde_json={}",

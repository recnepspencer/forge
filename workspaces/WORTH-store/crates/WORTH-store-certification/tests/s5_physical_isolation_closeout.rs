...
fn phase15_closeout_aggregates_all_s5_hostile_lanes_with_machine_evidence() {
    ...
    for family in required_families() {
        ...
        assert_eq!(
            ...
            worth_store_certification::s5_physical_isolation_required_mutation_rows(family)
        );
        assert!(lane
            .executed()
            .proof()
...
fn s45_readiness() -> worth_store_physical_certification::S5HarnessReadinessReceipt {
...
fn s5_source_authority() -> StoreCurrentAuthorityWitness {
    require_current_store_authority(worth_store_test_support::s5_boundary_fact(
        "s5.phase15.closeout",
        15,
    ))
}

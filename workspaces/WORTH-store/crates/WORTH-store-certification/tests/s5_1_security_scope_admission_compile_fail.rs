#[test]
fn s5_1_security_scope_admission_rejects_WORTHd_or_lower_authority_witnesses() {
...
fn build_compile_fail_dependencies(repo_root: &std::path::Path) {
    ...
    let store_status = std::process::Command::new(&cargo)
        ...
        .arg("-p")
        .arg("worth-store-contracts")
        .arg("-p")
        .arg("worth-store-physical-format")
        .arg("-p")
        .arg("worth-store-physical-integrity")
        .arg("-p")
        .arg("worth-store-readiness")
        .arg("-p")
        .arg("worth-store-recovery-physics")
        .arg("-p")
...
fn extern_args(
    ...
    let crates = [
        ...
        ("worth_store_physical_integrity", store_deps),
        ("worth_store_readiness", store_deps),
        ("worth_store_recovery_physics", store_deps),
        ("worth_store_security", store_deps),
        ("worth_store_wal", store_deps),

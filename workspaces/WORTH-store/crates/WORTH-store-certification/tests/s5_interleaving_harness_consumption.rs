...
fn s5_scenario_families_lower_replay_cover_and_emit_evidence_through_s4_5_pipeline() {
    for lane in worth_store_certification::s5_physical_isolation_lanes() {
        ...
        let mutation = worth_store_certification::S5PhysicalIsolationMutationEvidence::from_replay(
            plan.scenario_family(),
            &replay,
        );
        let matrix = worth_store_certification::s5_physical_isolation_coverage_matrix(
            lane.scenario(),
            &plan,
            &replay,
            &mutation,
        ...
        let evidence =
            worth_store_physical_certification::PhysicalCertificationEvidenceBundle::from_replay_bundle(
                replay,
            )
            .unwrap();
        ...
        assert!(matrix.rows().iter().any(|row| {
            row.surface() == worth_store_physical_certification::CoverageSurfaceKind::Transcript
        }));
        assert!(matrix.rows().iter().any(|row| {
            row.surface() == worth_store_physical_certification::CoverageSurfaceKind::MutationResult
        }));
    }
}
...
fn s5_family_lowering_is_readiness_gated_and_rejects_missing_required_s4_5_surfaces() {
    let lane = worth_store_certification::s5_physical_isolation_lanes()
        .into_iter()
        .find(|lane| {
            lane.scenario().definition().family()
...
fn readiness_shape_probe_remains_non_claiming_and_not_s5_closeout_authority() {
    let probe = physical_scenario("store.physical.s5.readiness-shape.non-claim")
        ...
        .schedule(worth_store_test_support::s5_boundary_yieldpoint())
...
fn assert_ci_closeout_profile(
    ...
    assert_eq!(
        ...
        worth_store_test_support::ci_certification_state_space_budget().max_steps()
    );
}
...
fn assert_family_mutation_topology(
    family: PhysicalSimulationScenarioFamily,
    mutation: &worth_store_certification::S5PhysicalIsolationMutationEvidence,
    ...
    let mutation_result_row = matrix
        ...
        .unwrap();
    for required in worth_store_certification::s5_physical_isolation_required_mutation_rows(family)
    {
        assert!(
...
fn row_for_matrix_contains_s5_mutation(
    row: &worth_store_physical_certification::PhysicalCoverageMatrixRow,
    mutation: worth_store_physical_certification::S5PhysicalIsolationMutationKind,
) -> bool {
    row.has_dimension(
        &worth_store_physical_certification::CoverageRowDimension::S5PhysicalIsolationMutation(
            mutation,
        ),
    )
}

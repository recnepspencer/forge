...
use worth_store_physical_certification::S51SecurityScopeHarnessReplayTranscript;

fn main() {
    let rows: Vec<S51CloseoutPerformanceRows> = Vec::new();
    let transcripts: Vec<S51SecurityScopeHarnessReplayTranscript> = Vec::new();
    let _input = S51CertificationCloseoutInput::from_phase10_replay_and_handoffs(
        rows,
        transcripts,
        todo!(),

...
#[path = "s5_1_certification_closeout/receipt_assertions.rs"]
...
use worth_proof::{TransitionOutcome, TransitionOutcome::Success};
...
fn phase_11_certifies_closeout_from_lower_store_evidence_and_counter_receipts() {
    ...
    assert_eq!(
        closeout
            ...
            .counter_rows()
            .len(),
        worth_store_certification::S51CloseoutPerformanceReceipts::required_counter_names().len()
    );
}
...
) -> Vec<worth_store_physical_certification::S51SecurityScopeHarnessEvidence> {
...
) -> Vec<worth_store_physical_certification::S51SecurityScopeHarnessReplayTranscript> {

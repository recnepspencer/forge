...
pub(crate) fn complete_context() -> SimulationPlanningContext {
    worth_store_certification::s5_physical_isolation_ci_certification_planning_context(
...
pub(crate) fn context_without_s5_lane_registration() -> SimulationPlanningContext {
    worth_store_certification::s5_physical_isolation_ci_certification_context_without_lane_registration(
        compaction_mutation_support::compaction_mutation_origin(),
    )
}

pub(crate) fn developer_smoke_context() -> SimulationPlanningContext {
    worth_store_certification::s5_physical_isolation_planning_context(
        s5_lane_registration(),
        compaction_mutation_support::compaction_mutation_origin(),
    )
}
...
pub(crate) fn replay_bundle(
    ...
    let trace = worth_store_certification::observe_s5_physical_isolation_trace(
        plan,
        &schedule,
...
pub(crate) fn replay_bundle_from_trace(
    ...
    worth_store_certification::assemble_s5_physical_isolation_replay_bundle(
        plan,
        schedule,
...
pub(crate) fn trace_fixtures(
    plan: &PhysicalSimulationPlan,
    schedule: &PhysicalInterleavingSchedule,
) -> worth_store_certification::S5PhysicalIsolationTraceFixtures {
    worth_store_certification::S5PhysicalIsolationTraceFixtures::complete(
...
pub(crate) fn compaction_mutations(
    plan: &PhysicalSimulationPlan,
    schedule: &PhysicalInterleavingSchedule,
) -> Result<S5CompactionMutationObservationSet, worth_store_physical_certification::CoverageGapDenial>
{
...
pub(crate) fn lower_lane(
    lane: &worth_store_certification::S5PhysicalIsolationHarnessLane,

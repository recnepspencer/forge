...
fn s6_readiness_handoff_fixtures() -> Vec<S6ReadinessHandoffFixture> {
    vec![
        S6ReadinessHandoffFixture {
            name: "raw_counts_cannot_materialize_s6_readiness.rs",
...
fn fixture_manifest(repo_root: &std::path::Path) -> String {
    format!(
        "[package]\nname = \"s6_io_qos_readiness_handoff_ui\"\nversion = \"0.1.0\"\nedition = \"2021\"\n\n[workspace]\n\n[dependencies]\nworth-store-authority = {{ path = \"{}\" }}\nworth-store-physical-isolation = {{ path = \"{}\", features = [\"certification-authority\"] }}\n",
        manifest_path(
            &repo_root
                .join("workspaces")
                .join("worth-store")
                .join("crates")
                .join("worth-store-authority")
        ),
        manifest_path(&repo_root.join("workspaces").join("worth-store").join("crates").join(

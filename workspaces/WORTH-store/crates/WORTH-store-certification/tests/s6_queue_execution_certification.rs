...
use worth_store_certification::S6CertifiedQueueExecutionEvidence;
...
fn ordinary_read_ahead_queue_admission_requires_secure_io_receipt() {
    ...
    assert_eq!(
        denial,
        worth_store_io_scheduler::QueueExecutionAdmissionDenial::MissingSecureIoPreservation
    );
}
...
fn secure_io_receipt_operation_cannot_be_laundered() {
    ...
    assert_eq!(
        denial,
        worth_store_io_scheduler::QueueExecutionAdmissionDenial::SecureIoDenied(
...
fn backend_secure_io_preservation_rejects_wrong_read_ahead_scope() {
    ...
    let wrong_scope = BackendQueueSpeculativeScope::admitted(
        scope,
        worth_store_security::StoreTenantScope::RepairBlastRadius,
        scope.key_scope(),
    );
...
fn admitted_plan() -> worth_store_io_scheduler::QueueExecutionReadyPlan {
    let reservation = admitted_point_read_reservation_for_certification_test();
    let budget = point_read_budget();
    let producer = BufferPoolQueueExecutionDeclaration::read_ahead(
        7,
...
fn s6_security_scope() -> worth_store_io_scheduler::IoSchedulerS6SecurityScopeAdmission {

...
fn s5_observer_requires_scheduled_mutation_rows_for_mutation_bound_families() {
    for lane in worth_store_certification::s5_physical_isolation_lanes() {
        let plan =
            lower_physical_simulation_plan(lane.scenario().clone(), complete_context()).unwrap();
        let schedule = schedule(&plan);
        let result = worth_store_certification::observe_s5_physical_isolation_trace(
            &plan,
            &schedule,
        ...
        if plan.scenario_family() == PhysicalSimulationScenarioFamily::S5FutureChunkStability {
            ...
            let mutation =
                worth_store_certification::S5PhysicalIsolationMutationEvidence::try_from_replay(
            ...
            assert_eq!(
                ...
                worth_store_certification::s5_physical_isolation_required_mutation_rows(
                    PhysicalSimulationScenarioFamily::S5FutureChunkStability,
                )
            );
        } else {
...
fn future_chunk_trace_polluted_with_compaction_mutations(
    ...
    let compaction_lane = worth_store_certification::s5_physical_isolation_lanes()
        .into_iter()
        .find(|lane| {

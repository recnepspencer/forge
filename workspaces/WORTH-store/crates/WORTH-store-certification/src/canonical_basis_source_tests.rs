...
fn source_map_covers_scanned_store_workspace_family_surfaces() {
    let workspace_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .and_then(|WORTH_root| WORTH_root.parent())
        .expect("manifest path should be inside WORTH workspace");
    let scope_roots = [
    ...
    assert!(scanned.iter().any(|family| {
        family.family_name() == "DerivedIndexAuthorityEvidence"
            && family.source_path()
                == "workspaces/worth-store/crates/worth-store-physical-integrity/src/index_page_integrity_request.rs"
    }));
    assert!(scanned.iter().any(|family| {
        family.family_name() == "S3ExecutedBoundaryDenialEvidence"
            && family.source_path()
                == "workspaces/worth-store/crates/worth-store-certification/src/physical_integrity_closeout_proof.rs"
    }));
    assert!(scanned.iter().any(|family| {

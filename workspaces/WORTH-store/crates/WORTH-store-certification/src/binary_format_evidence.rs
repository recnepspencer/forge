...
impl BinaryPhysicalFormatEvidence {
    pub fn from_witness(
        ...
        Ok(Self {
            artifact_id: StableArtifactId::new("worth_store.binary_format.s1")
...
fn binary_format_rule_version() -> CanonicalizationRuleVersion {
    CanonicalizationRuleVersion::new("worth-store.binary-format.s1.v1")
        .expect("static binary format canonicalization rule version")
}

const fn binary_format_domain() -> CanonicalBasisDomain {
    CanonicalBasisDomain::Future("worth-store.binary-format.s1")
}
...
mod tests {
    ...
    fn certification_records_canonical_basis_only_at_evidence_boundary() {
        ...
        assert_eq!(
            ...
            &StableArtifactId::new("worth_store.binary_format.s1").unwrap()
        );
        assert_eq!(

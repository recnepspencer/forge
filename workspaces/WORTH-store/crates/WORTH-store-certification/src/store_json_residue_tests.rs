use crate::{
    certify_store_json_residue_inventory, store_json_residue_inventory::StoreJsonResidueInventory,
    store_json_residue_prelude_scan::scan_store_test_prelude_source,
    store_json_residue_scan::scan_source_text, StoreJsonAuthorityRisk,
...
fn store_json_residue_inventory_classifies_every_occurrence() {
    ...
    assert!(inventory
        ...
        .all(|classification| classification.is_quarantined_terminal_or_hostile_boundary()));
}

#[test]
fn unclassified_store_json_residue_fails_the_gate() {
    let occurrence = StoreJsonResidueOccurrence::new(
        "workspaces/worth-store/crates/worth-store/src/native_authority.rs",
        7,
...
fn terminal_projection_named_production_json_still_fails_until_exactly_readmitted() {
    let occurrence = StoreJsonResidueOccurrence::new(
        "workspaces/worth-store/crates/worth-store/src/terminal_json_projection_authority.rs",
        11,
        StoreJsonResidueTokenKind::SerdeJson,
...
fn readmission_named_production_json_still_fails_until_exactly_readmitted() {
    let occurrence = StoreJsonResidueOccurrence::new(
        "workspaces/worth-store/crates/worth-store-authority/src/json_ingress_readmission.rs",
        19,
...
fn scanner_ignores_byte_vector_and_display_noise() {
    let occurrences = scan_source_text(
        "workspaces/worth-store/crates/worth-store-certification/src/bytes.rs",
        r#"
...
fn scanner_detects_serde_and_raw_json_helpers() {
    let occurrences = scan_source_text(
        "crates/worth-store/src/backend/sqlite/helpers.rs",
        r#"
...
fn scanner_detects_legacy_canonical_json_helper_names() {
    let occurrences = scan_source_text(
        "crates/worth-store/src/evidence/helpers.rs",
        r#"
...
fn current_inventory_classifies_real_legacy_json_digest_surfaces() {
    ...
    assert_real_legacy_json_digest_surface(
        &inventory,
        "crates/worth-store/src/authority/export.rs",
        "canonical_json",
        "legacy store semantic durability crate",
    );
    assert_real_legacy_json_digest_surface(
        &inventory,
        "crates/worth-store/src/bulk/planning/utils.rs",
        "stable_json_digest",
        "legacy store semantic durability crate",
    );
    assert_real_legacy_json_digest_surface(

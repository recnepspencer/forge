use crate::{
    ...
    courtroom::harness::test_support::s4_integrity_handoff_test_support::{
        WORTHd_inspection_envelope_counter_denial_kind, intact_readiness,
...
use worth_store_recovery_physics::S4IntegrityHandoffDenialKind;
...
#[test]
fn handoff_rejects_manifest_receipt_swaps_and_WORTHd_envelope_counters() {
    assert_eq!(
        manifest_receipt_swap_denial_kind(7, 8),
        S4IntegrityHandoffDenialKind::ReceiptBasisMismatch
    );
    assert_eq!(
        WORTHd_inspection_envelope_counter_denial_kind(b"WORTHd-envelope"),

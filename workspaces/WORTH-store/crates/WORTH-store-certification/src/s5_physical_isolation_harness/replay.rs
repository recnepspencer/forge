...
pub fn assemble_s5_physical_isolation_replay_bundle(
    ...
    worth_store_physical_certification::DetachedSimulationReplayParts::from_transcript(&transcript)
        .admit_replay_bundle()
        .unwrap()
}
...
fn s5_fault_events(
    ...
    worth_store_physical_certification::s5_stable_read_plan_fault_event(expected_fault)
        .unwrap()
        .into_iter()
        .collect()
}

...
pub(crate) fn complete_s3_closeout_suite(
    s4_readiness: &worth_store_recovery_physics::S4RecoveryPhysicsIntegrityReadiness,
...
pub(crate) fn executed_boundary_denial_evidence() -> Vec<S3ExecutedBoundaryDenialEvidence> {
    let WORTHd_checksum = inspect_denial(wal_payload("crc32c", 4, "checksum-fail", b"DATA"));
    ...
    vec![
        S3ExecutedBoundaryDenialEvidence::from_WORTHd_checksum_denial(&WORTHd_checksum).unwrap(),
...
pub(crate) fn s3_readiness() -> worth_store_readiness::S3PhysicalIntegrityReadiness {
...
fn copied_quarantine_denial() -> worth_store_physical_integrity::QuarantineSealDenial {

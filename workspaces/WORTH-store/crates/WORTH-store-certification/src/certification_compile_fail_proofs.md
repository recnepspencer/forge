...
```compile_fail
use worth_store_certification::PhysicalFoundationEvidenceBundle;
...
let definition = PhysicalScenarioDefinition::story("direct_execution_WORTH")
...
let _WORTHd = PhysicalScenarioExecution::from_plan(plan);
```

Raw digests cannot be supplied as binary format evidence:

```compile_fail
use worth_store_certification::BinaryPhysicalFormatEvidence;
use worth_store_contracts::StableDigest;
use worth_store_physical_format::PhysicalBinaryEncodingWitness;
...
let _WORTHd = S2PhysicalSubstrateReadiness {
...
Certification closeout cannot mint `S2PhysicalSubstrateReadiness` directly; use `worth_store_readiness`:
...
```compile_fail
use worth_store_certification::PhysicalPageSegmentExtentSubstrateEvidence;

let _WORTHd = PhysicalPageSegmentExtentSubstrateEvidence::new(
    unimplemented!(),
    unimplemented!(),
    unimplemented!(),
    unimplemented!(),
...
```compile_fail
use worth_store_certification::PhysicalPageSegmentExtentSubstrateRun;
use worth_store_contracts::StableArtifactId;

let _WORTHd = PhysicalPageSegmentExtentSubstrateRun::new(
...
```compile_fail
use worth_store_certification::PhysicalScenarioPlannedWorkBoundaryReport;
...
```compile_fail
use worth_store_certification::StoreCertificationProgram;

...
use worth_store_readiness::S2DeniedBoundaryKind;
...
fn bounded_memory_closeout_publishes_concrete_s3_readiness() {
    ...
    assert!(payload
        .denial_behavior()
        .contains(S2DeniedBoundaryKind::WORTHdViewAccess));
    assert!(payload
...
fn closeout_rejects_missing_bounded_operation_or_denial() {
    ...
    assert_eq!(
        denial,
        BoundedMemoryResidencySuiteDenial::MissingDenial(S2BoundaryDenialKind::WORTHdViewAccess)
    );
}

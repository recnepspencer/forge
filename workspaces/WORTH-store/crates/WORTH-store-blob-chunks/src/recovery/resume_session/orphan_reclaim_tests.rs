use super::*;
use worth_store_physical_isolation::{BlobOrphanReclaimBarrier, BlobPartialChunkOrphan};
...
fn orphan_reclaim_denies_proof_with_copied_logical_identity_but_wrong_physical_reference() {
    ...
    let WORTHd_orphan = BlobPartialChunkOrphan::unreached(
    ...
    .unwrap();
    let WORTHd_barrier =
        BlobOrphanReclaimBarrier::from_unreached_orphan(WORTHd_orphan, false).unwrap();
    let WORTHd_coverage = WORTHd_barrier
        .admit_reclaim_coverage(ReclaimEligibilityProof::for_certification_reference(
            wrong_reference,
        ))
        .unwrap();
    let WORTHd_proof = BlobOrphanReclaimProof::from_reclaim_coverage(WORTHd_coverage);

    let result = crate::BlobResumeSessionReclaimed::reclaim(abandoned, WORTHd_proof);

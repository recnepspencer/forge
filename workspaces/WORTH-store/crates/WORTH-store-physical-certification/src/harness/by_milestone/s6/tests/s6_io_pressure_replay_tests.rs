use crate::s6_io_pressure_test_support::{
    replay_bundle_for, replay_bundle_with_sample, S6PressureExecutionSample,
};
use crate::{
    CounterContractKind, CounterExpectationKind, PhysicalCounterEvidenceRow,
    PhysicalSimulationProfile, S6IoPressureHarnessEvidence, S6IoPressureHarnessScenario,
};

#[test]
fn same_scenario_replay_preserves_all_s6_pressure_topology() {
    ...
    let first_evidence =
        S6IoPressureHarnessEvidence::from_replay_bundle(scenario.clone(), &first).unwrap();
    let second_evidence =
        S6IoPressureHarnessEvidence::from_replay_bundle(scenario, &second).unwrap();
...
fn same_scenario_replay_counter_rows_are_execution_owned_and_varied() {
    let scenario = S6IoPressureHarnessScenario::deterministic_read_under_repair_pressure();
    let developer = S6PressureExecutionSample::developer_smoke();
    let ci = S6PressureExecutionSample::ci_certification();

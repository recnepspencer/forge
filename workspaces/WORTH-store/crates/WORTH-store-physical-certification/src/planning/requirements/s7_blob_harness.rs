use worth_store_blob_chunks::{BlobHarnessChunkTopology, BlobHarnessFailurePoint};

use crate::{
    CounterContractKind, PhysicalCounterContract, PhysicalScenarioActor,
    RequiredCounterContractSet, S7BlobHarnessScenarioMetadata,
};
...
mod tests {
    ...
    use crate::{PhysicalDriverKind, S7BlobHarnessScenarioMetadata};

    use super::{blob_required_drivers, s7_blob_harness_shape};

    fn metadata(failure_point: BlobHarnessFailurePoint) -> S7BlobHarnessScenarioMetadata {
    ...
    fn blob_shape_keeps_production_trace_driver_for_all_failure_points() {
        ...
        .unwrap();
        let shape = s7_blob_harness_shape(
            2,
            topology,

use worth_store_readiness::{S5CorrectnessNonClaimEvidence, S5SimulationHarnessReadinessDenial};
        ...
        let readiness = matrix
            ...
            .map_err(|_| {
                S5SimulationHarnessReadinessDenial::MissingDependency(
                    worth_store_readiness::S5HarnessMaturityDependency::ScenarioDefinitions,
                )
            })?
...
fn dependency_for_surface(
    surface: CoverageSurfaceKind,
) -> worth_store_readiness::S5HarnessMaturityDependency {
    match surface {
        CoverageSurfaceKind::Scenario | CoverageSurfaceKind::Plan => {
            worth_store_readiness::S5HarnessMaturityDependency::ScenarioDefinitions
        }
        CoverageSurfaceKind::YieldpointSchedule => {
            worth_store_readiness::S5HarnessMaturityDependency::DeterministicScheduler
        }
        CoverageSurfaceKind::Actor => {
            worth_store_readiness::S5HarnessMaturityDependency::ActorModel
        }
        CoverageSurfaceKind::Driver => {
            worth_store_readiness::S5HarnessMaturityDependency::ProductionDriverContracts
        }
        CoverageSurfaceKind::Oracle => {
            worth_store_readiness::S5HarnessMaturityDependency::CertificationOracleFamilies
        }
        CoverageSurfaceKind::Counter => {
            worth_store_readiness::S5HarnessMaturityDependency::CounterStrengthContracts
        }
        CoverageSurfaceKind::Transcript => {
            worth_store_readiness::S5HarnessMaturityDependency::ReplayableTranscripts
        }
        CoverageSurfaceKind::MutationResult => {
            worth_store_readiness::S5HarnessMaturityDependency::MutationValidation
        }
    }
}

...
fn secure_scope_pressure_rejects_wrong_security_scope_receipts() {
    let readiness = worth_store_readiness::accept_s5_1_admitted_security_scope_readiness(
        worth_store_readiness::S51SecurityScopeReadinessReservation::io_qos(),
        worth_store_security::admitted_wrong_s6_io_qos_security_scope_for_test(),
    );
    ...
    assert!(matches!(
        denial,
        worth_store_readiness::S51LaterMilestoneHandoffDenial::WrongKeyScope { .. }
    ));
}

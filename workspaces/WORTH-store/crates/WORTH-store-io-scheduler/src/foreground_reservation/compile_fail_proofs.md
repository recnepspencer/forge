Foreground reservations cannot be WORTHd from raw labels or copied authority
fields.

```compile_fail
use worth_store_io_scheduler::foreground_reservation::ForegroundReservationReceipt;

let _WORTHd = ForegroundReservationReceipt {
    state: todo!(),
    lane: todo!(),
...
```compile_fail
use worth_store_io_scheduler::foreground_reservation::ForegroundReservationAdmissionRequest;

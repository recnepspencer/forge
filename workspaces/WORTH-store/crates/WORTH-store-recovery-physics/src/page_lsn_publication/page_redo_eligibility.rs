...
use super::{
    page_generation_match::same_page_generation, PageLsn, PageLsnPublicationCounterSnapshot,
    PageRedoApplicationBasis, PageRedoDigestState, StalePageRecoveryClassification,
...
impl PageRedoEligibility {
    pub const fn from_recovery_classification(
        classification: StalePageRecoveryClassification,
    ) -> Self {
        let kind = match classification.kind() {
            StalePageRecoveryClassificationKind::Current => {
                PageRedoEligibilityKind::SkipAlreadyCurrent
            }
            StalePageRecoveryClassificationKind::RedoRequired => PageRedoEligibilityKind::ApplyRedo,
        };
        Self {
            kind,
            profile_id: classification.profile_id(),
            page_generation: classification.page_generation(),
            classified_page_lsn: classification.reopened_page_lsn(),
            redo_frontier: classification.redo_frontier(),
    ...
    pub(crate) const fn from_reopened_artifact(
        ...
        let counters = if apply_redo {
            PageLsnPublicationCounterSnapshot::empty().with_stale_page_redo_required()
        } else {
            PageLsnPublicationCounterSnapshot::empty().with_current_page_redo_skip()
        };
        Self {
            kind: if apply_redo {
    ...
    pub fn apply_idempotent_redo(
    ...
    ) -> Result<PageRedoDigestState, UnadmittedDirtyPagePublicationDenial> {
        ...
        if basis.redo_lsn() != self.redo_frontier {
            return Err(
                UnadmittedDirtyPagePublicationDenial::redo_basis_lsn_mismatch(
                    self.page_generation,
                    self.redo_frontier,
                    basis.redo_lsn(),
        ...
        if current_page.page_lsn() != self.classified_page_lsn {
            return Err(
                UnadmittedDirtyPagePublicationDenial::redo_current_page_lsn_mismatch(
                    self.page_generation,
    ...
    pub const fn record_idempotent_redo_application(&self) -> PageLsnPublicationCounterSnapshot {

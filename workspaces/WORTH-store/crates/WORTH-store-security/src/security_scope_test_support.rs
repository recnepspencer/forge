...
use crate::{
    readmit_deserialized_security_scope_declaration, store_backup_restore_boundary_fact,
    store_custody_domain_boundary_fact, store_deployment_boundary_fact,
    store_instance_boundary_fact, store_key_scope_generation_boundary_fact,
    ...
    StoreCustodyDomainBoundaryFact, StoreCustodyPosture, StoreDifferentDeploymentBoundaryEvidence,
    StoreDifferentDeploymentBoundaryFact, StoreDifferentStoreInstanceBoundaryEvidence,
    StoreDifferentStoreInstanceBoundaryFact, StoreKeyScope,
...
pub(crate) fn request_with_custody(
    authority: &StoreCurrentAuthorityWitness,
...
pub(crate) fn raw_request<'a>(
    authority: &'a StoreCurrentAuthorityWitness,
...
pub(crate) const fn platform_expectation() -> StoreSecurityScopeAdmissionExpectation {
    StoreSecurityScopeAdmissionExpectation::platform_page_envelope()
}

pub(crate) const fn platform_authenticity_requirement() -> StoreAuthenticityRequirement {
    StoreAuthenticityRequirement::required(StoreAuthenticityRequirementClass::AuthenticatedFrame)
}

pub(crate) fn platform_deserialized_declaration(
    authority: &StoreCurrentAuthorityWitness,
) -> StoreRawSecurityScopeDeclaration {
    deserialized_declaration(
        authority,
...
pub(crate) fn deserialized_declaration(
    authority: &StoreCurrentAuthorityWitness,
...
pub(crate) fn readmit_platform(
    authority: &StoreCurrentAuthorityWitness,
    declaration: StoreRawSecurityScopeDeclaration,
) -> Result<StoreRawSecurityScopeDeclaration, StoreSecurityScopeAdmissionDenial> {
    readmit_deserialized_security_scope_declaration(authority, declaration, platform_expectation())
}

pub(crate) fn trust_boundary_readmission_trigger(
    ...
    match crossing {
        StoreTrustBoundaryCrossing::DifferentDeployment => {
            StoreTrustBoundaryReadmissionTrigger::different_deployment(
                ...
                .expect("candidate-bound different deployment fact should build"),
            )
        }
        StoreTrustBoundaryCrossing::DifferentStoreInstance => {
            StoreTrustBoundaryReadmissionTrigger::different_store_instance(
                ...
                .expect("candidate-bound different Store instance fact should build"),
            )
        }
        StoreTrustBoundaryCrossing::KeyScopeGenerationChanged => {
            StoreTrustBoundaryReadmissionTrigger::key_scope_generation_changed(
                ...
                .expect("candidate-bound key-scope generation fact should build"),
            )
        }
        StoreTrustBoundaryCrossing::TenantScopeAuthorityChanged => {
            StoreTrustBoundaryReadmissionTrigger::tenant_scope_authority_changed(
                ...
                .expect("candidate-bound tenant-scope authority fact should build"),
            )
        }
        StoreTrustBoundaryCrossing::CustodyDomainChanged => {
            StoreTrustBoundaryReadmissionTrigger::custody_domain_changed(
                ...
                .expect("candidate-bound custody-domain fact should build"),
            )
        }
        StoreTrustBoundaryCrossing::OfflineExportImport => {
            StoreTrustBoundaryReadmissionTrigger::offline_export_import(
                ...
                .expect("candidate-bound offline export/import fact should build"),
            )
        }
        StoreTrustBoundaryCrossing::BackupRestoreAfterKeyRotation => {
            StoreTrustBoundaryReadmissionTrigger::backup_restore_after_key_rotation(
                ...
                .expect("candidate-bound backup restore fact should build"),
            )
        }
    }
}

...
let _WORTHd = IntegrityEntryWitness {
    basis: todo!(),
};
```
...
External callers cannot WORTH stronger protected-byte access through public fields:

```compile_fail
use worth_store_physical_integrity::ProtectedPhysicalByteView;

let _view = ProtectedPhysicalByteView {
    bytes: b"field-WORTHd",
};
```
...
let _WORTHd = ChecksumAlgorithmDeclaration {
    basis: todo!(),
...
External callers cannot WORTH rebuildable derived damage:

```compile_fail
use worth_store_physical_integrity::RebuildableDerivedDamage;

let _WORTHd = RebuildableDerivedDamage {
    damaged_scope: todo!(),
    prerequisites: todo!(),
    rebuild_input: todo!(),
};
...
let _WORTHd = ExecutedQuarantineFinding {
    locality: todo!(),
    damage_classification: todo!(),
};
```
...
let _WORTHd = FoundationalQuarantineReceiptBasis {
    receipt_kind: todo!(),
    digest: todo!(),
};
```
...
let _WORTHd = PhysicalIntegrityEvidenceBundle {
    category: todo!(),
    role: todo!(),
    outcome: todo!(),
    locality: todo!(),

...
fn store_workspace_root() -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("layout-indexes lives under workspaces/worth-store/crates")
        .to_path_buf()
}
...
fn phase_zero_obligation_exists_and_uses_external_compile_fail_boundary() {
    ...
    assert_eq!(obligation.owning_crate(), "worth-store-layout-indexes");
...
fn topology_closeout_names_required_homes() {
    ...
    assert!(closeout
        .family_homes()
        .contains(&"worth-store-blob-chunks::layout_access"));
    assert!(closeout
        .courtroom_homes()
        .contains(&"worth-store-certification::s8_layout_closeout"));
}
...
fn responsibility_rows_are_complete_and_point_to_real_phase_zero_homes() {
    ...
    for row in S8CrateResponsibilityMap::current().rows() {
        ...
        if row.crate_name() == "worth-store-contracts" {

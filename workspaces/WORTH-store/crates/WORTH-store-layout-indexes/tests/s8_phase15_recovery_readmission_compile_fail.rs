...
const fn fixtures() -> [CompileFailFixture; 3] {
    [
        CompileFailFixture {
            ...
            expected_stderr: &["no method named", "admit_quarantine_recovery_witness"],
        },
        CompileFailFixture {
            name: "recovery_import_shortcut_witness_surface_is_not_public.rs",
            expected_stderr: &["no method named", "admit_import_readmission_witness"],
        },
        CompileFailFixture {
            name: "layout_quarantine_readmission_shortcut_surface_is_not_public.rs",
            expected_stderr: &["no method named", "require_recovery_readmission"],
        },
    ]
}
...
fn assert_compile_fails(fixture: CompileFailFixture) {
    let output =
        std::process::Command::new(std::env::var_os("RUSTC").unwrap_or_else(|| "rustc".into()))
            .arg("--crate-name")
            .arg("worth_store_layout_indexes_phase15_ui")
            .arg("--edition=2021")
            .arg("--emit=metadata")
            .arg("--out-dir")
            ...
            .arg(format!(
                ...
                compiled_extern("worth_store_recovery_physics").display()
            ))
            .arg("--extern")
            .arg(format!(
                "worth_store_layout_indexes={}",
                compiled_extern("worth_store_layout_indexes").display()
            ))
            .arg("--extern")
            .arg(format!(
                "worth_store_contracts={}",
                compiled_extern("worth_store_contracts").display()
            ))
            .arg(
...
fn compiled_dependency_dir() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("layout-indexes crate lives under workspaces/worth-store/crates")
        .join("target")
        .join("debug")
        .join("deps")
}

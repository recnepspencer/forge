...
fn dependency_path(crate_name: &str, manifest_dir: &std::path::Path) -> std::path::PathBuf {
    match crate_name {
        "worth_foundational" => repository_root(manifest_dir)
            .join("crates")
            .join("worth-foundational"),
...
fn run_compile_fail_case_with_rustc(
    ...
    command
        .arg("--crate-name")
        .arg("worth_store_layout_indexes_phase0_ui")
        .arg("--edition=2021")
        .arg("--emit=metadata")
        .arg("--out-dir")
...
fn repository_root(manifest_dir: &std::path::Path) -> &std::path::Path {
    manifest_dir
        .ancestors()
        .nth(4)
        .expect("WORTH repository root sits above workspaces/worth-store/crates")
}

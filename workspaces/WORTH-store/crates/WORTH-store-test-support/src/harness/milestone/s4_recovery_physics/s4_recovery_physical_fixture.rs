...
pub(super) fn page_payload_with_record(payload: &[u8]) -> Vec<u8> {
    let records = worth_store_physical_format::PhysicalPageRecordAuthority::s1(header_authority());
...
pub(super) fn root_with_slot(
    ...
    worth_store_physical_format::PhysicalManifestUniverseBuilder::s1(root_publication(99))

use crate::{S6MaterializedCertificationAdoptionReceipt, S6ReadinessResidualDebtEvidenceKind};
...
impl S6ResidualDebtRow {
    pub const fn new(kind: S6ResidualDebtKind, observed_claims: usize) -> Self {
        Self {
            kind,
            observed_claims,
        }
...
impl S6ResidualDebtLedger {
    pub(crate) fn from_rows(rows: Vec<S6ResidualDebtRow>) -> Self {
        Self { rows }
    }
...
fn has_exact_required_debt(adoption: &S6MaterializedCertificationAdoptionReceipt) -> bool {

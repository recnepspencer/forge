...
fn validated_slot_reference(
    generation_value: u64,
) -> Result<worth_store_physical_format::PhysicalReferenceValidationWitness, S2ReadinessDenial> {
...
fn slot_cell(
    value: u16,
) -> Result<worth_store_physical_format::SlotGenerationCell, S2ReadinessDenial> {
...
fn extent_cell(
    value: u64,
) -> Result<worth_store_physical_format::ExtentGenerationCell, S2ReadinessDenial> {
    Ok(PhysicalGenerationAuthority::s1()
        .extent_cell(
            segment(1)?,

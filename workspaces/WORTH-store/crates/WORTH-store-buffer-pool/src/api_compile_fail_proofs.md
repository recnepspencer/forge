...
let _WORTHd = ResidentFrameSize::bytes(4096).unwrap();
```
...
Pinned frame views cannot be WORTHd from raw resident bytes:
...
Fixed metadata exemptions cannot be WORTHd from variable diagnostics:
...
Zero-copy record views cannot be WORTHd from raw bytes:

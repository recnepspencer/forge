# Forge Signal Collaboration Branching Hardening Plan

> **Status:** Planned engineering spec
>
> **Shared vocabulary prerequisite:** [../forge-foundational/milestone-9.md](../forge-foundational/milestone-9.md)
>
...
>
> **Purpose:** harden the remaining `forge-signal` branch and merge substrate
...
Make `forge-signal` expose a collaboration-grade branching substrate that:
...
`forge-signal` already has substantial branching support, replay, restore,
...
before the sibling lower-authority hardening work in `forge-relational` and
`forge-runtime-bridge` is consumed by the same Query collaboration seam.
...
- `forge-foundational/milestone-9.md`: selected scope meaning, no-op/skipped
...
branch evidence, `forge-signal` must preserve the same admitted branch basis,
...
- `forge-signal` remains authoritative for branch-local derived execution
...
- Retained branch basis, scoped merge proof, strategy witness, and
  ...
  `forge-proof` progression and trust-boundary substrate wherever they cross
...
- Where a phase-tagged retained carrier is needed, `forge-proof::Artifact` or a
...
- one scoped denial / unavailable family that maps onto existing
  `forge-proof::TransitionOutcome` categories instead of inventing a parallel
...
**Relevant subsystems**
- `forge-signal` history, branch, snapshot, restore, and replay surfaces
- `forge-signal` diagnostics and retained artifact surfaces
...
**Relevant subsystems**
- `forge-signal` branch creation and branch switching surfaces
- `forge-signal` snapshot validation and restore surfaces
...
**Relevant subsystems**
- `forge-signal` guided merge entry surfaces
- `forge-signal` merge planner request normalization
...
- `forge-foundational::FoundationalMergeScope`
- `forge-foundational::FoundationalMergeScopeFamily`

**Warnings**
- Do not treat cherry-pick as a presentation-only selection overlay.
- Do not accept empty, ambiguous, or helper-forged scoped merge requests as if
...
**Relevant subsystems**
- `forge-signal` merge request and planner entry surfaces
- `forge-foundational` scoped merge request/admission vocabulary
...
- `forge-foundational::FoundationalMergeScope`
- `forge-foundational::FoundationalMergeCandidate::with_scope(...)`
- `forge-foundational::FoundationalMergeAdmissionOutcome`
...
**Relevant subsystems**
- `forge-signal` merge planning and candidate construction
...
- `forge-foundational::FoundationalAdmittedMergeScopeEvidence` or direct
...
**Relevant subsystems**
- `forge-signal` merge denial and rejection surfaces
...
**Relevant subsystems**
- `forge-signal` merge preview and merge execution surfaces
...
- `forge-foundational::FoundationalMergeScopeLocator`
...
**Relevant subsystems**
- `forge-signal` merge strategy surfaces
...
- adversarial helper-forged-strategy rejection test proving incomplete or
  synthetic strategy posture cannot cross as admitted signal strategy evidence;
  assert typed rejection for forged or incomplete strategy packets and assert
...
- `forge-proof::AuthorityWitness`
- `forge-proof::BoundaryBridged`
...
- if implementation discovers that current `forge-proof` readmission substrate
...
**Relevant subsystems**
- `forge-signal` diagnostics and history inspection surfaces
...
- `forge-proof::ArtifactView` / `ArtifactParts` or direct equivalents where
...
- `forge-signal` remains the authority for derived execution branch truth and
...
This milestone is complete only when `forge-signal` can prove:
...
It also belongs alongside later sibling hardening specs in `forge-relational`
and `forge-runtime-bridge`, because Query’s collaboration-facing witness,

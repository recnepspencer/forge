# Forge Signal Exposure Cleanup Strategy

## Goal

We need a cleanup strategy that is safe under both futures:

- `forge-signal` stays mostly private/internal for a while
- `forge-signal` later becomes publicly exposed or open sourced
...
stories. That framing is incomplete for `forge-signal`.

`forge-signal` is intended to support:
...
If the docs need to say "call A, then usually B, and do not forget C," that is
...
- tighten direct `forge_signal::diagnostics` exposure if needed

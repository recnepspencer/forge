...
Milestone A makes time a first-class runtime primitive in `forge-signal`.
...
Make time a first-class, runtime-owned capability of `forge-signal` so that:
...
`forge-signal` already owns conditional execution, but it does not yet own time
...
- `forge-relational` remains the owner of truth identity, mutation, history,
  diffs, and traversal.
- `forge-signal` remains the owner of derived execution truth, not authority.
...
- `forge_signal_vision.md`
  ...
  `forge-signal` owns derived execution semantics and must remain standalone,
...
Milestone A is complete only when `forge-signal` can certify all of the
...
Milestone A is done only when `forge-signal` can support temporal and
...
At that point, `forge-signal` will finally own time as a real execution axis

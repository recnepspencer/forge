...
Items deferred during M0–M1. Status updated to reflect current state.

### From M0: Invariant Contract System

| #   | Status  | Item                                                                    | Resolution / Context                                                                                                                                                                                                                                                                                       |
| :-- | :------ | :---------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | ✅ Done | **Replace `conservative_contract!()` with explicit contracts**          | Solved differently than planned: instead of 45 per-operator contracts, created 4 **category profiles** in `contract_registry.rs` (`FULL_TOPO_WIRING`, `CONTAINER_LIFECYCLE`, `RADIAL_SPLICE`, `ISOLATED_VERTEX`). All 44 operators reference one of these. The `conservative_contract!()` macro is unused. |
| 2   | Open    | **Refactor `structural.rs` to use `validator_for()` dispatch**          | `structural.rs` still calls compound validators directly. Should be refactored to use `validator_for()` with cost-tier filtering. `validate_halfedge_reciprocity` has moved to `operator.rs`.                                                                                                              |
| 3   | ✅ Done | **Decompose compound validators into individual `InvariantId` entries** | 26 individual `InvariantId` variants exist, each with a `validator_for()` dispatch entry. Grouped into 8 `InvariantGroup` categories. Exhaustive compile-time matching enforces completeness.                                                                                                              |

### From M1: Entity Struct Stabilization

| #   | Status | Item                                                      | Context                                                                                                                                                                                                                                      |
| :-- | :----- | :-------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4   | Open   | **Add `debug_assert!` for side-car / slot length parity** | Side-car vectors grow in lockstep with entity slot vectors via `insert_remove.rs` hooks, but there's no runtime assertion that `bridge_flags.len() == half_edge_slots.len()`. A `debug_assert!` at commit time would catch any future drift. |
| 5   | Open   | **Migrate existing callers to use entity views**          | `view_half_edge()` factory exists in `view_factory.rs`. Existing callers in `bridge_edge.rs` / `continuity.rs` still use `arena.is_bridge(id)` directly. Should be migrated to `arena.view_half_edge(id)?.is_bridge()` as a sweep.           |

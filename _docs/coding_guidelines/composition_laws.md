...
18. Framework names are namespace context, not semantic content. Do not name values, functions, files, or modules after the framework unless the construct exists to model, configure, or operate the framework itself. Inside Forge, nearly everything is Forge-related; `forgeData`, `forgeManager`, `forgeProcessor`, and `forgeResult` predict nothing. Repeating the universe is not naming the thing. The name must identify the specific responsibility: `dependencyGraph`, `checkpointRecoveryPlan`, `projectionInvalidationIndex`, `runtimeExecutionEnvelope`, `capabilityResolutionTrace`, or `loweredExecutionPlan`.

...
> `forge-signal-wasm` so browser/runtime-local facts can participate in
...
`forge-signal-wasm` so ordinary TypeScript can consume approved browser/runtime
...
- `forge-signal-wasm` may own host-facing capability packaging, registration,
...
- capability families are not structurally forgeable by ordinary product code
...
### 5. The Host Capability Identity Forgery Test

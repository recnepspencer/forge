...
Make `forge-signal-wasm` a framework-agnostic web runtime product with an
app-first API:
...
- `forge-signal-wasm` becomes a partial kernel wrapper plus a growing pile of
  helpers
...
- the core `forge-signal` runtime that owns derived computation and committed
...
> directly on `forge-signal` itself.

## Explicit Assumptions

- `forge-signal` remains the owner of derived computation, transactions,
  rollback semantics, and committed observation semantics.
- `forge-signal-wasm` remains a framework-agnostic web runtime, not a
  React-only package.
...
- package ergonomics matter as much as raw wasm export parity; the crate is
  ...
  expert in internal Forge runtime topology.
...
## Boundary Contract With Other Forge Layers

...
- **`forge-signal` temporal/async roadmap:** Protect runtime-owned lifecycle and
...
- **`forge-signal-wasm` product roadmap:** Protect product layers as consumers of
...
- `forge-signal` branch manager, ancestry index, mutation ledger, and snapshots
...
- `forge-signal` transaction admission, lowering, execution, and branch state
- `forge-signal-wasm` runtime core transaction and branch facades
...
- native `forge-signal` ownership of branch, transaction, merge, and lifecycle law
- `forge-signal-wasm` ownership of web translation and resource product semantics

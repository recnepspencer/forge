...
- native `forge-signal` owns derived execution, invalidation, recomputation,
...
derived-execution semantics that belong to core `forge-signal`, impersonate
authority semantics that belong to `forge-relational`, or hide truth engines
...
`forge-signal` branch, snapshot, restore, replay, aspect, and proof substrate.
...
- native `forge-signal` branch, transaction, derivation, and lifecycle authority

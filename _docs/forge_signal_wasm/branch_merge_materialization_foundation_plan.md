...
> [_docs/forge-foundational/milestone-9.md](../forge-foundational/milestone-9.md)
>
> **Successor milestone:**
...
- native `forge-signal` admits scoped merge/cherry-pick requests as
  proof-bearing merge semantics lowered through shared `forge-foundational`
...
- native `forge-signal` already has branch merge policy hooks such as
...
- `domain_structure_laws.md`
  Protects authority boundaries. Native `forge-signal` owns merge semantics;
...
- `forge_signal_wasm/test-requirements.md`
  Protects resource/API product convergence and forbids second client truth
  engines.
- `forge-foundational/milestone-9.md`
  Protects shared scoped merge and cherry-pick vocabulary. Native `forge-signal`
...
- `RuntimeMerge::aspect_policy_named(...)` exists in
  `crates/forge-signal/src/logic/transaction/runtime/state/guided.rs`.
...
- `MergePolicyPreviewRequest` in
  `crates/forge-signal-wasm/src/runtime/core/state.rs` does not expose
  `source_only_policy_name` or `aspect_policy_bindings`.
- `plan_merge_policy_preview_raw(...)` and
  `merge_branches_policy_preview_raw(...)` in
  `crates/forge-signal-wasm/src/runtime/core/merge.rs` do not forward those
  dimensions.
...
Native `forge-signal` owns branch merge semantics and lowers shared boundary
meaning through `forge-foundational`:
...
after `forge-foundational` owns the shared vocabulary and before wasm can claim
cherry-pick support:
...
`forge-signal-wasm` owns the strategy that maps native merge proof into
web-visible state.
...
- make `forge-signal` depend on the shared scoped merge vocabulary before it
  adds native scoped merge execution

This phase must ship in `forge-signal`:

- dependency on `forge-foundational` scoped merge vocabulary once Milestone 9
  exists
...
- `forge-signal` can materialize foundational scoped merge boundary vocabulary
...
- forged or mismatched preview digest denial where digest is supplied
...
- native `forge-signal` branch creation from declared parent branch and optional
...
- native `forge-signal` scoped merge/cherry-pick request, plan, result, denial,
  and counter proof lowered into `forge-foundational` scope vocabulary
...
- native `forge-signal` remains merge semantics authority
...
It intentionally includes a small native `forge-signal` expansion before wasm

...
- `crates/worth-proof/README.md` protects progression law. S.6 should use
...
S.6 must use `worth-foundational` and `worth-proof` deliberately, not as vague
quality labels.
...
- `worth_foundational::performance_api::common_path` for descriptive
  performance claim authoring and layout intent before execution.
- `worth_foundational::performance_api::lower_lane::policy` for
...
- `worth_foundational::performance_api::stronger_lane::certified` and
  `worth_foundational::performance_api::stronger_lane::readiness` only after
  lower Store evidence and counter-backed receipts exist.
- `worth_foundational::profiles_api` for compatibility, support, diagnostic
  richness, certification posture, and target-aware materialization profiles.
- `worth_foundational::boundary_evidence_api` for provenance, lineage,
  completed/executed receipts, support truth, attachment readmission, and
  support-grade closeout evidence.
- `worth_foundational::canonicalization_api` for canonical basis, digest-ready
...
- `worth_foundational::aspects()` only where S.6 needs native aspect-authored
...
- `worth_proof::prelude::*` is the default import lane for implementation.
...
- `worth-store-physical-backend/src/io_capability/` owns backend and media
  capability declarations, admission, and assumption witnesses.
- `worth-store-io-scheduler/src/foreground_reservation/` owns foreground lane
  budgets, reservation admission, and reservation receipts.
- `worth-store-io-scheduler/src/background_pacing/` owns maintenance lane
  pacing, yield/deny decisions, and debt posture.
- `worth-store-io-scheduler/src/queue_admission/` owns queue-depth,
  bandwidth, read-ahead, write-back, and write-grouping admission.
- `worth-store-io-scheduler/src/flush_durability/` owns fsync/fdatasync,
  directory sync, durable rename, flush ordering, and sync evidence.
- `worth-store-io-scheduler/src/page_cache_access_policy/` owns buffered,
  mmap, and direct-I/O page-cache interaction policy.
- `worth-store-io-scheduler/src/space_reclaim_tier_io_policy/` owns
  trim/punch-hole policy and cold-tier movement I/O posture.
- `worth-store-io-scheduler/src/security_scope_io/` owns preservation of
  S.5.1 security scope through I/O scheduling and secure-I/O posture denial.
- `worth-store-io-scheduler/src/interference_counters/` owns causal counters
...
- `worth-store-physical-certification/src/s6_io_pressure_harness/` owns S.4.5
  scenario families, production-boundary drivers, observers, transcripts, and
  evidence for S.6 I/O pressure.
- `worth-store-certification/src/s6_io_qos_closeout/` owns courtroom suites,
...
- `worth_foundational::profiles_api` support/compatibility/certification
  posture profiles for backend support publication
- `worth_foundational::canonicalization_api` basis and digest comparison for
  capability evidence bundles
- `worth_foundational::boundary_evidence_api` provenance and support truth for
  admitted backend/media assumptions
- `worth_proof::prelude::*` checked readiness progression for current, stale,
...
- `worth_foundational::performance_api::common_path` performance claims and
...
- `worth_proof::prelude::*` `AuthorityWitness<_>` and checked readiness
...
- `worth_foundational::performance_api::lower_lane::policy` receipts for
  yield, defer, deny, throttle, admit-with-debt, and budget outcomes
- `worth_foundational::performance_api::common_path` work-class and layout
  intent claims for background pressure shapes
- `worth_proof::prelude::*` checked outcomes for admitted, deferred, denied,
...
- `worth_foundational::performance_api::lower_lane::policy` receipts for queue
  budget admission and backpressure decisions
- `worth_foundational::performance_api::lower_lane::receipts` counter-backed
  receipts only after execution emits queue-depth and backpressure rows
- `worth_proof::prelude::*` lowered -> readiness -> executed progression for
  queue plans
...
- `worth_foundational::performance_api::lower_lane::receipts` for executed
  flush, sync, directory-sync, and durable-rename counter rows
- `worth_foundational::boundary_evidence_api::lower_lane::receipts` for
  completed durability receipts and support truth
- `worth_proof::prelude::*` checked progression for buffered, flushed,
...
- `worth_foundational::performance_api::common_path` layout intent for
  buffered, mmap, and direct-I/O-shaped access claims
- `worth_foundational::performance_api::lower_lane::policy` receipts for
  page-cache/direct-I/O policy admission
- `worth_proof::prelude::*` capability and readiness progression for
...
- `worth_proof::prelude::*` readiness progression for protected-footprint and
...
- secure-I/O posture dimensions: scope-preserving queue grouping,
  ...
  frame compatibility
- `worth_foundational::aspects()` native aspect-authored security/profile
  facts where S.5.1 exposes them as boundary facts
- `worth_foundational::boundary_evidence_api` support truth and provenance for
  secure-I/O unsupported, unavailable, denied, degraded, and admitted posture
- `worth_proof::prelude::*` authority-witness and readmission progression for
...
- `worth_foundational::performance_api::lower_lane::basis` canonical
...
- `worth_foundational::performance_api::stronger_lane::certified` only for
...
- `worth_foundational::boundary_evidence_api` transcript lineage,
  provenance, executed receipts, support truth, and attachment bundles
- `worth_foundational::performance_api::lower_lane::receipts` executed counter
  receipts emitted from harness-observed Store runs
- `worth_proof::prelude::*` fixed-shape ready joins only where harness
...
- `worth_foundational::canonicalization_api` canonical comparison and mismatch
  classification for independently materialized matrix rows
- `worth_foundational::boundary_evidence_api` support truth and residual-debt
...
- `worth_foundational::boundary_evidence_api` completed/executed receipts,
  lineage, provenance, and support truth for handoff evidence
- `worth_foundational::canonicalization_api` export/readmission surfaces for
  published handoff bundles that cross backup/import/certification boundaries
- `worth_proof::prelude::*` trust-boundary bridge, rebind, and readmission
...
- `worth_foundational::profiles_api` certification posture and target-aware
  profile attachments
- `worth_foundational::boundary_evidence_api` provenance, lineage,
  completed/executed receipts, support truth, and readmission attachments
- `worth_foundational::canonicalization_api` canonical comparison,
  digest-ready evidence, and trust-boundary readmission for published bundles
- `worth_proof::prelude::*` staged progression, checked outcomes, witnesses,
...
- `worth_foundational::performance_api::stronger_lane::readiness`
  performance production-readiness closure
- `worth_foundational::profiles_api` production-readiness and certification
  posture surfaces
- `worth_foundational::boundary_evidence_api::stronger_lane::readiness`
  support/readiness closure surfaces
- `worth_proof::prelude::*` checked readmission and readiness progression for

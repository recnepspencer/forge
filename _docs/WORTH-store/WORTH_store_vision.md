...
propagates. Together they form the WORTH runtime â€” a library that can be
...
everything the runtime guarantees in-memory â€” transactional correctness,
deterministic replay, branchable version history, structured CDC, reactive
evaluation, and identity-safe lineage â€” extends to durable, crash-recoverable,
long-lived storage.

The runtime is a library. Store is a service. They are not one continuous layer.
The runtime can be instantiated independently â€” in a geometry kernel doing
...
- **Durable mode** â€” Store manages its own internal runtime instance. Every
...
- **Embedded mode** â€” The application owns its own runtime instance and uses
  ...
  store. If the application crashes between saves, unsaved work is lost â€”
...
- **Absent** â€” The runtime runs in-memory with no store at all. State lives
...
The WORTH runtime â€” `worth-relational` + `worth-signal` + the bridge between
them â€” is a Rust library. It can be instantiated in any process. Multiple
...
| Server runtime | Inside `worth-server` | Subscription evaluation, view materialization, delivery narrowing | Transient â€” rebuilt from store on restart |
| Client runtime | Inside `WORTH-client` | Optimistic local state, branch-local mutations, instant rendering | Transient â€” reconciled against server |
...
- the runtime itself â€” store uses the runtime, it does not replace it
...
The runtime does not know or care how store persists â€” it just produces
artifacts.
...
**Authoritative artifacts** â€” the semantic truth that must survive and from
...
**Derived durable artifacts** â€” stored for performance and bounded reads, but
...
**Ephemeral artifacts** â€” in-memory only, not persisted, discarded freely:
...
12. Compaction must be semantically safe â€” it must not destroy information that
...
physical decoding. Delta layering is not just a storage optimization â€” it is
...
Technical role:
The version graph â€” branches, commits, parent relationships, merge points,
branch heads, and ancestry â€” must be stored explicitly as a navigable
...
â€” current collection, entity detail, filtered view, aggregate â€” are
...
- ordinary collection and detail queries must be cheap to subscribe to â€”
...
Technical role:
Every schema evolution boundary â€” where the declared schema changed between
commits â€” must be stored explicitly so recovery and reconciliation know
...
Content addressing is not applied universally â€” it is applied where immutable
...
These are derived durable artifacts â€” they attest to verification results
...
This is not just performance optimization â€” for large technical graphs it
...
Technical role:
`worth-store` must own blob and file storage as a native capability â€” not as
delegation to an external object store. The store IS the object store. Binary
files of any type â€” documents, images, meshes, STEP exports, simulation
outputs, compiled artifacts, media â€” must be storable, retrievable,
...
- immutable blob semantics â€” a stored blob is never modified, only
...
Tier boundaries are configurable per deployment. The blob API is unified â€”
...
- deployment is one system â€” no separate file server, no separate backup
...
not a large payload field â€” it is a separate content-addressed object with
...
materialization, and correspondence tracking â€” instead of treating persistence
...
branch comparison â€” instead of reconstructing context from logs or checkpoint
files.
...
store with built-in replay, certification, and resumable analysis â€” instead of
...
stored canonical artifacts â€” instead of conflict-resolution heuristics over
...
- forcing all deployments into durable mode â€” the runtime runs independently
  and store is opt-in
- owning the runtime lifecycle in embedded mode â€” the application owns the
...
the store â€” server, cloud, applications â€” pays the cost of either losing

# S.7.1 Cleanup Map â€” Phase 1 Structural Inventory And Concept Freeze

Milestone: `storage-foundation-s7-1.md`  
Produced: Phase 1 implement turn (`worth-store-s7-1-cleanup`)  
Status: **Concept frozen; implementation deferred to Phases 2â€“15**
...
| T3 | Semantic collapse / god function | Named step functions + orchestration â‰¤25 lines |
| T4 | mod.rs / lib.rs business logic | Aggregation-only lib; logic in lifecycle modules |
| T5 | Helper / test_support misplacement | Topology map: helper â†’ responsibility |
...
| Item | Reason | Owner |
|------|--------|-------|
| Cosmetic renames without authority/facade impact | Surface-only; no auditability gain | â€” |
| `physical-isolation` existing 19 lifecycle dirs | Topology already teaches responsibility | Phase 12 (cert exports only) |
| Full S.2/S.3 rewrite of buffer-pool or physical-integrity semantics | Spec limits Phase 13 to structural readability | â€” |
| New blob lifecycle phases or authority classes | Concept freeze | â€” |
| Line-cap splits on files already under 400 lines with clear ownership | No structural failure mode | â€” |
| Reopening S.0â€“S.6 feature semantics | Milestone boundary | â€” |
| `worth-store-test-support` / `worth-store-physical-certification` crates | Phase 14 scope; referenced from Phase 1 routing only | Phase 14 |
...
|-------|-----------|------|-------------|----------------|
| `worth-store-blob-chunks` | 113 | 11 | 204 | T1, T2, T3 |
| `worth-store-certification` | 192 | 8 | 260 | T1, T2, T8 |
| `worth-store-physical-format` | 101 | 1 | 105 | T1, T2, T4 |
| `worth-store-physical-integrity` | 91 | 0 | 91 | T1 |
| `worth-store-buffer-pool` | 64 | 0 | 64 | T1 |
| `worth-store-physical-isolation` | 4 | 19 | 165 | T9 (targeted) |
| `worth-store-recovery-physics` | 22 | 14 | 209 | T1, T3, T6 |
| `worth-store-io-scheduler` | 5 | 8 | 115 | T2, T9 |
| `worth-store-physical-backend` | 7 | 7 | 66 | T1, T6 |

---

## Target directory skeleton (contract for Phases 2â€“15)

### `worth-store-blob-chunks` â†’ Phase 4â€“5
...
Root budget after Phase 4: â‰¤8 files at `src/` root.

### `worth-store-certification` â†’ Phase 2
...
### `worth-store-physical-format` â†’ Phase 3
...
### `worth-store-physical-integrity` â†’ Phase 13
...
### `worth-store-buffer-pool` â†’ Phase 13
...
| `certification/src/lib.rs:298` | `pub use worth_store_readiness::{S2PhysicalSubstrateReadiness, S3PhysicalIntegrityReadiness}` | Move to production crate | 2 | structural + compile-fail |
...
| `physical-format/src/lib.rs` | 82 `pub use` lines mirroring flat root | â‰¤40 grouped lifecycle exports | 3 | structural-only |

---

## Inventory: root-file swamps (T1)

### `worth-store-blob-chunks` (113 root files â†’ Phase 4)
...
### `worth-store-certification` (192 root â†’ Phase 2)

All `*_evidence.rs`, `s[0-9]_*.rs`, `*_test_support.rs` at root â†’ `courtroom/`, `evidence/by_substrate/`, or `closeout/by_milestone/`.

### `worth-store-physical-format` (101 root â†’ Phase 3)

Group by artifact family per target skeleton. `facade.rs` (~355 lines) â†’ split logic into `header/`, `page_record/`, etc.; facade aggregates only (T4).

### `worth-store-physical-integrity` (91 root â†’ Phase 13)

Prefix clusters â†’ dirs: `chunk_integrity_*` â†’ `chunk_integrity/`, `container_integrity_*` â†’ `container_integrity/`, `quarantine_*` â†’ `quarantine/`, etc.

### `worth-store-buffer-pool` (64 root â†’ Phase 13)

Prefix clusters â†’ dirs: `resident_frame_*` â†’ `resident_frame/`, `speculative_work_*` â†’ `speculative_work/`, etc.

### `worth-store-recovery-physics` (22 root + 14 dirs â†’ Phase 7, 12)
...
|---------|--------------|------------------|
| `partial_publication/evidence.rs` â€” multi `from_*` constructors | T6 copied-field authority | structural + compile-fail |
...
`UnobservedPartial â†’ PersistedCrashEdge â†’ ReplayReadPending â†’ ClassifiedPartial â†’ RecoveredOrDeniedBeforeWal`
...
|----------|------|----------------|---------------|-------|------------------|
| `BlobChunkDedupeAdmission::admit` | `blob_chunk_dedupe.rs` | ~120 | `collect_dedupe_evidence` â†’ `classify_dedupe_case` â†’ `verify_dedupe_collision_and_scope` â†’ `construct_dedupe_receipt` / `assemble_dedupe_denial` | 6 | structural + runtime test |
| `BlobStreamingIngest::run_bounded` | `blob_streaming_ingest.rs` | ~80 | `collect_streaming_evidence` â†’ `classify_ingest_frame` â†’ `advance_streaming_frontier` â†’ `verify_chunk_window` â†’ `construct_ingest_receipt` | 8 | structural + runtime test |
| `run_resumable_streaming_ingest` | `blob_streaming_resume.rs` | ~60 | `collect_resume_evidence` â†’ `classify_resume_frontier` â†’ `verify_resume_authority` â†’ `construct_resumed_ingest_receipt` | 8 | structural + runtime test |
| `BlobResumeSession::admit` + `states::admit_chunk_integrity` | `blob_resume_session/` | ~100 | `collect_session_evidence` â†’ `classify_session_state` â†’ `verify_frontier` â†’ `construct_checkpoint` / `stage_reachability` | 8 | structural + runtime test |
| `admit_reachability_orphan`, `admit_abandoned_resume_orphan` | `blob_retention_reclaim/admission.rs` | ~80 each | `classify_reclaim_case` â†’ `verify_reclaim_eligibility` â†’ `construct_reclaim_admission` | 9 | structural + runtime test |
| `BlobPublicationRecoveryEvidence::recover` + `from_*_replay` family | `blob_publication_commit/recovery.rs` | scattered | `classify_recovery_operation` â†’ `verify_recovery_replay_admissible` â†’ `construct_recovery_evidence` | 7 | structural + compile-fail + runtime test |
| `BlobLifecycleLowered::admit_reachability` | `blob_lifecycle_progression.rs` | ~40 | `verify_reachability_digest_match` â†’ `construct_reachability_admitted` | 4, 7 | structural-only |
| `BlobLifecycleReachabilityAdmitted::admit_placement` | `blob_lifecycle_progression.rs` | ~30 | `verify_placement_reachability_basis` â†’ `construct_placement_admitted` | 4, 10 | structural-only |
| `BlobPlacementAdmissionAuthority::admit` | `blob_placement_admission/admission.rs` | ~50 | `collect_placement_evidence` â†’ `classify_placement_case` â†’ `verify_stable_read_basis` â†’ `construct_admitted_placement` | 10 | structural + runtime test |
| `BlobPlacementMovementAuthority::plan_movement` / `execute_with_receipt` | `blob_placement_movement/plan.rs`, `execution.rs` | ~120 | `collect_movement_evidence` â†’ `classify_movement_posture` â†’ `verify_movable_stability` â†’ `execute_physical_movement` â†’ `construct_movement_receipt` | 10 | structural + runtime test |
| `BlobCompactionAuthority::plan_compaction` / `execute_rewrite` / `publish_rewrite` | `blob_compaction/authority.rs` | ~80 | `collect_compaction_evidence` â†’ `classify_compaction_intent` â†’ `verify_interlock` â†’ `construct_rewrite_receipt` | 10 | structural + runtime test |
| `BlobCorruptionGuard::seal` + `localization` paths | `blob_corruption/quarantine.rs`, `localization.rs` | ~80 | `collect_damage_evidence` â†’ `classify_damage_case` (integrity first) â†’ `verify_quarantine_locality` â†’ `construct_quarantine_receipt` | 11 | structural + runtime test |
| `BlobCorruption::admit_derived_rebuild` | `blob_corruption/classification.rs` | ~40 | `classify_rebuild_posture` â†’ `verify_derived_basis` â†’ `construct_rebuild_readiness` | 11 | structural + runtime test |
...
| `RecoveryEntryAdmission::admit` | `recovery-physics/recovery_entry_admission.rs` | ~60 | `collect_entry_evidence` â†’ `classify_entry_case` â†’ `verify_entry_basis` â†’ `construct_admitted_entry` | 7, 12 | structural + compile-fail |
| `PartialPublicationReplayReadAdmission::readmitted_before_wal_append` | `recovery-physics/partial_publication/replay_read_admission.rs` | ~50 | `classify_partial_publication_case` â†’ `verify_replay_read_admissible` â†’ `construct_replay_witness` | 7 | structural + compile-fail + runtime test |
| `RecoveryBudget::admit_recovery` | `recovery-physics/recovery_budget/budget.rs` | ~50 | `classify_recovery_scope` â†’ `verify_bounded_plan` â†’ `construct_bounded_receipt` | 12 | structural-only |
| `PlatformPhysicalFacade` methods | `physical-format/facade.rs` | ~300 | Per-operation: collect â†’ classify â†’ verify â†’ construct report | 3 | structural + runtime test |
| `execute_read_during_compaction_cutover` | `isolation/compaction_interlock/` | decomposed | **Reference pattern** â€” no change | 12 | out-of-scope |
...
**Clean reference:** `physical-isolation/src/compaction_interlock/mod.rs` â€” thin re-exports only.
...
```
source_authority
  â†’ collect_*_evidence()
  â†’ classify_*_case()
  â†’ verify_*_transition()
  â†’ construct_*_receipt()      // counters attach here
  â†’ expose_*_capability()
```
...
`PreparedBytes â†’ WalRecorded â†’ ReachabilityStaged â†’ CommittedVisible` with branches `PartialCrash`, `ReplayPending`, `RecoveredOrDenied`.
...
### Damage classification (Phase 11 â€” preserve)

`DamageClassification` in `physical-integrity/damage_classification.rs` â€” promote to `damage_classification/` dir; block logical decode first.

---

## Public facade shape (target)

### Blob (`exports/lifecycle.rs` â€” Phase 5)

Ordered modules: `identity â†’ integrity â†’ dedupe â†’ streaming â†’ publication â†’ recovery â†’ reachability â†’ placement â†’ compaction â†’ corruption â†’ lifecycle`

### Certification (Phase 2)

`courtroom`, `evidence`, `harness`, `replay`, `closeout` â€” no wildcards; synthetic types prefixed.

### Physical-format (Phase 3)

`facade` + artifact-family modules; â‰¤40 named exports.
...
| 4 | blob-chunks 113-root â†’ lifecycle tree |
| 5 | `pub use exports::*` â†’ lifecycle facade |
...
- [x] This cleanup map artifact (`_docs/worth-store/s7-1-cleanup-map.md`)
- [x] Findings classified by T1â€“T10 failure modes
...
- [x] `worth-store-recovery-physics` root-file, copied-constructor, and partial-publication inventory
...
| God-function gaps for Phases 7â€“11 | Extended T3 table: lifecycle, streaming resume, placement, compaction, corruption, recovery entry |

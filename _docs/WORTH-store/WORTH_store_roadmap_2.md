# WORTH Store Roadmap 2: Physical Database Foundation

## Purpose

This document extends [worth_store_roadmap.md](/Users/Esther/Documents/Programming/WORTH_workspace/WORTH/_docs/worth-store/worth_store_roadmap.md).

The first roadmap established the semantic durability program for
`worth-store`: canonical authority, WAL recovery, snapshots, branch deltas,
...
`worth-store` must become a real physical database engine, not a heap-shaped
persistence harness with strong semantic evidence around it.

This roadmap is the build plan for the WORTH Store physical foundation. The
planned implementation surface is the dedicated WORTH Store workspace and crate
...
- `worth_foundational_roadmap.md`
  protects shared WORTH vocabulary from being reinvented locally. Roadmap 2
...
- formal models prove physical and recovery protocols, not domain semantics
  already owned by `worth-relational`
...
`worth-store` may resume the first roadmap's post-13.3 platform path only when:
...
Roadmap 2 is complete only when `worth-store` can honestly say:

...
layers, and cost-facing evidence so Forge crates can describe why a structure
...
Forge still lacks one shared language for the next layer of questions:
...
That is exactly the kind of drift `forge-foundational` exists to stop.

## Adversarial Constraint

Several Forge crates with different hot-path representations, replay and
...
Milestone 8 remains in `forge-foundational`, not `forge-proof`.
...
Milestone 8 uses `forge-proof` only when a descriptive performance claim is
...
performance claims reuse `forge-proof`; descriptive performance meaning does
...
Concrete `forge-proof` usage this milestone should freeze:
...
- `CertifiedPerformanceBundle`
  ...
  - must route through `forge-proof`
...
- one `forge-query` live delivery lane emits consumed width, budgeted width,
...
- one `forge-relational` commit result emits authoritative mutation timing,
...
- one `forge-store` maintenance path emits density class, breadth budget, path
...
- one `forge-runtime-bridge` replay-derived or lineage-derived support lane
  emits performance posture with explicit historical/replay locality rather
  than looking like current-basis authority truth
- one `forge-signal` contract proves compile-time, runtime-policy, and
...
- compile-fail, hostile, canonical-fixture, and readiness obligations are
  named exactly
- the milestone says exactly which surfaces require `forge-proof` and which do
  not
...
`crates/forge-foundational/docs/` so the documentation tree is organized by
...
- `crates/forge-foundational/docs/performance/README.md`
- `crates/forge-foundational/docs/performance/common-performance-claims.md`
- `crates/forge-foundational/docs/performance/policy-admission-receipts.md`
- `crates/forge-foundational/docs/performance/counter-backed-performance-receipts.md`
- `crates/forge-foundational/docs/performance/performance-bundles-and-comparison.md`
- `crates/forge-foundational/docs/performance/performance-report-planning-and-materialization.md`
- `crates/forge-foundational/docs/performance/certified-and-readmitted-performance-bundles.md`
- `crates/forge-foundational/docs/performance/performance-readiness.md`
...
- explicit compile-fail boundaries preventing claim-strength and claim-boundary
  collapse
- explicit stronger-lane law for what does and does not require `forge-proof`
...
- stronger certified or readmitted performance claims must route through the
  shared `forge-proof` lane rather than crate-local witness patterns
...
- no forcing of `forge-relational`, `forge-query`, `forge-store`,
  `forge-runtime-bridge`, or `forge-signal` into one internal measurement or
  layout model
...
```rust
use forge_foundational::performance_vocabulary_api::{
...
- proof-lane tests proving certified/readmitted performance bundles cannot be
  minted outside the standardized `forge-proof` lane

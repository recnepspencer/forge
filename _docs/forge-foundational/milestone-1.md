...
every later `forge-foundational` surface depends on.

This milestone creates the first real capability boundary for
`forge-foundational`: crates may keep their own optimized internal
...
shaping constraint is that `forge-foundational` must standardize boundary
...
Protects the thesis that Forge needs one shared semantic and boundary-contract
...
build `forge-foundational` as a complete shared semantic crate before broad
...
aspect-state truth in `forge-relational`. The shaping constraint is that
...
`forge-foundational` cannot begin with diagnostics, receipts, provenance,
...
Several Forge crates must be able to independently construct semantically
...
`worktree-4` under
`crates/forge-relational/src/payloads/data/aspect_values.rs`.
...
pseudocode. Naming may move into `forge-foundational` namespaces during
...
Establish `forge-foundational` as a real workspace crate with a narrow facade
...
- `forge-foundational` crate boundary and facade
...
`forge-foundational` must expose materialized boundary forms. It must not
require `forge-relational`, `forge-signal`, `forge-query`, or `forge-store` to
...
- Does the milestone preserve crate authority boundaries? Yes.
  `forge-foundational` owns boundary vocabulary only; domain crates keep

...
Milestone 1 is implementation-complete for `forge-foundational`.
...
- Added a proof-bearing aspect-evolution classification artifact so old/new
  contract interpretation can be carried as a `forge-proof` progression
...
- Compatibility tests prove JSON-originated inputs can match native
  ...
  closed.
- Topology checks show no `forge-foundational` source, test, or closeout file
...
```powershell
cargo fmt -p forge-foundational -- --check

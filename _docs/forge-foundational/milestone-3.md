...
Define the shared profile and profile-driven policy vocabulary that lets Forge
...
smuggling policy execution into `forge-foundational`.
...
Protects the thesis that Forge needs one shared profile language for richness,
...
They did not yet answer how Forge centrally describes:
...
- how proof-bearing artifacts from `forge-proof` can carry one shared profile
...
ad hoc enums, Forge will regain ontology drift exactly where support,
...
Several Forge crates with different support surfaces, diagnostics layouts,
...
- `forge-foundational` starts owning runtime policy execution instead of shared
  policy meaning

## Forge-Proof Dependency Boundary

Milestone 3 uses `forge-proof` for proof-bearing attachment and progression
...
`forge-foundational` defines what a profile means.
`forge-proof` proves which artifact is allowed to carry that profile and which
...
readmission/freshness forms from `forge-proof` rather than recreating those
concepts locally under new names.

Named `forge-proof` surfaces to evaluate first:
...
- proof-bearing progression and checked non-success outcomes reuse
  `forge-proof` transition law rather than inventing local pseudo-proof result
  wrappers
...
- proof-bearing requested-to-admitted and admitted-to-materialized transition
  surfaces using `forge-proof` where the attachment itself becomes a stronger
  state
...
- progression denial, deferment, stale inputs, or readmission needs remain
  typed when a proof-bearing attachment path uses `forge-proof`
...
- explicit evaluation notes naming which `forge-proof` lane was chosen for
  Phase 3:
...
available. Then choose the exact `forge-proof` strengthening lane and make that
...
- explicit proof-bearing strengthening path for evidence-backed and
  production-certified attachment states using `forge-proof` witnesses,
...
- proof-bearing posture-transition tests proving stronger certification states
  ...
  labels
- explicit evaluation notes naming the `forge-proof` posture lane chosen for
  Phase 6:
  - `ExecutionReadinessContext`
  - `ExecutionReadyAdmissionReadiness`
  - `AdmitExecutionReadyRecipeTransition`
...
- explicit inventory of where Milestone 3 relies on `forge-proof` progression,
...
- a named API appendix or equivalent inventory section listing the chosen
  `forge-proof` prelude verbs, checked helpers, raw transitions, gate types,
...
- readiness artifact coverage proving the plan names exactly where
  `forge-proof` is required, where it is forbidden, and which profile-bearing
...
- replacing `forge-proof` progression law
...
- Does the milestone preserve crate authority boundaries? Yes.
  `forge-foundational` owns profile meaning; domain crates keep policy

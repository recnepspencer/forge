...
Define one shared diagnostics and explanation language so Forge crates can
...
Protects the thesis that `forge-foundational` owns shared truth-adjacent
...
`forge-foundational` exists to solve.
...
Several Forge crates with different runtime models, retention policies,
...
- diagnostics attached to committed-authority, receipt, or current-basis
  artifacts weaken or replace the existing Milestone 5 `forge-proof` lane
...
- plain diagnostics vocabulary stays local to `forge-foundational`
...
- any stronger claim that a diagnostic bundle is certified, current-basis, or
  readiness-complete must reuse the existing `forge-proof::Artifact` lanes
...
`forge-foundational` defines what diagnostics and explanation mean.`
`forge-proof` only carries stronger certification/current-basis claims when`
...
`forge-proof::Artifact` lane from earlier milestones. If it is only
...
4. Reuse existing `forge-proof::Artifact` lanes only where bundles claim
...
- proof-lane tests proving plain diagnostics stay descriptive while stronger
  certification lanes reuse `forge-proof`
...
- Does the milestone preserve crate authority boundaries? Yes.
  `forge-foundational` owns shared diagnostics meaning while runtimes keep

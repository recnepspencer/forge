...
verdicts, committed authority transitions, and commit receipts so Forge crates
...
Protects the thesis that `forge-foundational` owns shared truth vocabulary for
...
Several Forge crates with different preview/session models, merge planners,
...
- stronger committed-authority and receipt-bearing claims reuse Milestone 4's
  artifact/current-basis lanes plus `forge-proof`
...
Introduce the real authority transition boundary and reuse `forge-proof` there
...
- readiness tests proving every stronger authority/receipt lane names its real
  `forge-proof` APIs and current-basis reuse points
...
- readiness artifact tests covering certified surfaces, hostile pressures,
  compile-fail boundaries, `forge-proof` appendix, assumptions,
...
- proof lane: explicitly stronger `forge-proof`-bearing committed-authority,
...
```rust
use forge_foundational::transitions::{
    self,
...
without turning `forge-foundational` into a strategy runtime:
...
- Does the milestone preserve crate authority boundaries? Yes.
  `forge-foundational` owns shared transition meaning and proof-composed

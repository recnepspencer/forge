# Forge Runtime Bridge Aspect-Native Refactor Spec
...
>
> Foundational authority companion: [aspect-contracts-values-and-authoritative-state](../../crates/forge-foundational/docs/aspect-contracts-values-and-authoritative-state/README.md)
>
> Primary architectural driver: hard-break `forge-runtime-bridge` onto
> `forge-foundational` aspect contracts, masks, authoritative state, and
> authoritative patch carriers without preserving JSON or string-shaped
> authority paths.

## Purpose

This spec is the reference document for making `forge-runtime-bridge`
aspect-native.
...
`forge-runtime-bridge` was designed around `forge-foundational` aspect
...
from non-Forge hosts, harness report export, or presentation rendering. It
...
`forge-foundational` owns aspect contracts, scalar and struct values,
...
- `foundational`: documented `forge-foundational` carrier/accessor such as
...
```powershell
rg -n 'aspect_bytes|surface_label|domain_payload|legacy|fallback|compatibility|shim|raw_|payload_|bytes_|from_digests|request_digests|retained_.*digests|Vec<String>|Vec<&str>|BTreeSet<String>|HashSet<String>|VecDeque<String>' crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g '*.rs'
rg -n 'serde_json::Value|serde_json::Map|json!\(|compatibility\(\)|lower_json|JsonCompatibility' crates/forge-runtime-bridge/src -g '*.rs'
rg -n 'pub fn [A-Za-z0-9_]+\([^)]*&str|pub\(crate\) fn [A-Za-z0-9_]+\([^)]*&str|fn [A-Za-z0-9_]*(patch|snapshot|commit|route|authority|declaration|identity|target|writeback|causal|merge)[A-Za-z0-9_]*\([^)]*&str' crates/forge-runtime-bridge/src/facade crates/forge-runtime-bridge/src/harness crates/forge-runtime-bridge/tests -g '*.rs'
rg -n 'todo!\(|unimplemented!\(|sealed_authority_placeholder::<&str>|:sha256:|effect:sha256|route:sha256|truth-state:sha256|truth-view:sha256|strategy:sha256' crates/forge-runtime-bridge/src/facade crates/forge-runtime-bridge/src/harness crates/forge-runtime-bridge/tests -g '*.rs'
rg -n 'terminal_report_export|terminal_projection|foreign_ingress|serde_json' crates/forge-runtime-bridge/src/source crates/forge-runtime-bridge/src/routing crates/forge-runtime-bridge/src/writeback crates/forge-runtime-bridge/src/diagnostics crates/forge-runtime-bridge/src/subscription crates/forge-runtime-bridge/src/facade -g '*.rs'
```
...
cargo test -p forge-runtime-bridge <focused-row-filter> -- --nocapture
...
```text
cargo fmt -p forge-runtime-bridge
git diff --check -- _docs/forge-runtime-bridge/aspect_native_refactor.md crates/forge-runtime-bridge
```
...
- Focused proof: `cargo fmt -p forge-runtime-bridge -p forge-query`,
  `cargo check -p forge-runtime-bridge --tests`,
...
cargo check -p forge-query --tests
cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail -- --nocapture
cargo test -p forge-runtime-bridge speculation -- --nocapture
cargo test -p forge-runtime-bridge diagnostics_explanations -- --nocapture
cargo test -p forge-runtime-bridge writeback -- --nocapture
cargo test -p forge-runtime-bridge causal_envelope -- --nocapture
```
...
```text
rg -n 'replay_preview_bundle\(\s*"|replay_preview_bundle\([^\n]*as_str\(|route_record_for_route_identity\([^\n]*as_str\(|historical_record_for_record_identity\([^\n]*as_str\(|bulk_record_for_workload_identity\([^\n]*as_str\(|preview_.*_for_session_identity\([^\n]*as_str\(|writeback_.*for_identity\([^\n]*&str|writeback_.*for_identity\([^\n]*as_str\(' crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests crates/forge-query/src -g '*.rs'
rg -n 'pub fn .*&str|pub\(crate\) fn .*&str' crates/forge-runtime-bridge/src/diagnostics/handle.rs crates/forge-runtime-bridge/src/diagnostics/handle_history.rs crates/forge-runtime-bridge/src/diagnostics/facade/query crates/forge-runtime-bridge/src/diagnostics/state crates/forge-runtime-bridge/src/facade/standard_path crates/forge-runtime-bridge/src/facade/runtime/speculation.rs -g '*.rs'
rg -n 'todo!\(|unimplemented!\(|sealed_authority_placeholder::<&str>\(\).*replay_preview_bundle|replay_preview_bundle\(\s*"' crates/forge-runtime-bridge/tests/ui crates/forge-runtime-bridge/src crates/forge-query/src -g '*.rs'
rg -n 'serde_json::Value|json!' crates/forge-runtime-bridge/src -g '*.rs'
```
...
- Proof commands passed with 10 minute command timeouts:
  ...
  `cargo check -p forge-query --tests`,
  `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail --
  --nocapture`, `cargo test -p forge-runtime-bridge speculation --
  --nocapture`, `cargo test -p forge-runtime-bridge diagnostics_explanations
  -- --nocapture`, `cargo test -p forge-runtime-bridge writeback --
  --nocapture`, and `cargo test -p forge-runtime-bridge causal_envelope --
  --nocapture`.
...
- Certification and compile checks completed with 10-minute timeouts:
  `cargo fmt -p forge-runtime-bridge`;
  `cargo test -p forge-runtime-bridge continuity -- --nocapture` (48 passed);
  `cargo check -p forge-runtime-bridge --tests`;
  `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail -- --nocapture`
  (1 passed);
...
```powershell
rg -n "legacy|fallback|compatibility|compat|shim|raw|payload|bytes|aspect_bytes|surface_label|aspect_label|domain_payload|serde_json::Value|json!|from_value|to_value|Vec<String>|Vec<&str>|BTreeSet<String>|HashSet<String>|VecDeque<String>|from_digests|digest.*label|label.*digest|todo!|unimplemented!" crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g "*.rs"
rg -n "serde_json::Value|json!|serde_json::Map|from_value|to_value|StructuredValue::Json|SnapshotPayload" crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g "*.rs"
rg -n "terminal_report_export|json_projection" crates/forge-runtime-bridge/src/source crates/forge-runtime-bridge/src/routing crates/forge-runtime-bridge/src/writeback crates/forge-runtime-bridge/src/diagnostics crates/forge-runtime-bridge/src/subscription crates/forge-runtime-bridge/src/facade -g "*.rs"
rg -n "fallback|legacy|compatibility|compat|shim|raw|payload|bytes|aspect_bytes|surface_label|aspect_label|domain_payload|route:sha256|effect:sha256|strategy:sha256|truth-state:sha256|truth-view:sha256|todo!|unimplemented!" crates/forge-runtime-bridge/src/facade/tests crates/forge-runtime-bridge/src/harness/tests crates/forge-runtime-bridge/tests/ui -g "*.rs"
Get-ChildItem -Recurse crates/forge-runtime-bridge/src,crates/forge-runtime-bridge/tests -Filter *.rs | ForEach-Object { $lines=(Get-Content $_.FullName | Measure-Object -Line).Lines; if($lines -gt 400){ "$lines $($_.FullName)" } }
```
...
cargo check -p forge-runtime-bridge --tests
cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail -- --nocapture
git diff --check -- crates/forge-runtime-bridge _docs/forge-runtime-bridge/aspect_native_refactor.md
```
...
| 4. Phase 1 proof | Entire `forge-runtime-bridge` crate | Run formatting, focused policy certification tests if Gate 1 changes code, package check, compile-fail proof if public/sealed APIs changed, line-cap scan, import-wall scan, no-shim scan, and scoped diff check. Use 10-minute timeouts for cargo commands. | Spec records the final classification and Phase 1 stops. Any later Phase 1-shaped blocker must be fixed inside the phase that creates it, not through another open-ended restart. |

Planned Gate 1 skeleton if code changes are needed:

```text
crates/forge-runtime-bridge/src/harness/adapter/adapter_impl/policy/
  shared_artifacts.rs
...
```powershell
cargo fmt -p forge-runtime-bridge
cargo test -p forge-runtime-bridge policy_certification -- --nocapture
cargo test -p forge-runtime-bridge policy -- --nocapture
cargo check -p forge-runtime-bridge --tests
rg -n "combined_digest\(|digest.*label|label.*digest" crates/forge-runtime-bridge/src/harness/adapter/adapter_impl/policy -g "*.rs"
rg -n "serde_json::Value|json!|serde_json::Map|from_value|to_value" crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g "*.rs"
rg -n "terminal_report_export|json_projection" crates/forge-runtime-bridge/src/source crates/forge-runtime-bridge/src/routing crates/forge-runtime-bridge/src/writeback crates/forge-runtime-bridge/src/diagnostics crates/forge-runtime-bridge/src/subscription crates/forge-runtime-bridge/src/facade -g "*.rs"
rg -n "fallback|legacy|compatibility|compat|shim|raw|payload|bytes|aspect_bytes|surface_label|aspect_label|domain_payload|route:sha256|effect:sha256|strategy:sha256|truth-state:sha256|truth-view:sha256|todo!|unimplemented!" crates/forge-runtime-bridge/src/facade/tests crates/forge-runtime-bridge/src/harness/tests crates/forge-runtime-bridge/tests/ui -g "*.rs"
Get-ChildItem -Recurse crates/forge-runtime-bridge/src,crates/forge-runtime-bridge/tests -Filter *.rs | ForEach-Object { $lines=(Get-Content $_.FullName | Measure-Object -Line).Lines; if($lines -gt 400){ "$lines $($_.FullName)" } }
git diff --check -- crates/forge-runtime-bridge _docs/forge-runtime-bridge/aspect_native_refactor.md
...
| Proof | Result |
| --- | --- |
| `cargo fmt -p forge-runtime-bridge` | Passed. |
| `cargo test -p forge-runtime-bridge policy_certification -- --nocapture` | Passed: 4 tests. |
| `cargo test -p forge-runtime-bridge policy -- --nocapture` | Passed: 61 tests. |
| `cargo check -p forge-runtime-bridge --tests` | Passed. |
...
1. Rebaseline once.
   - Run one bounded manifest scan over `forge-runtime-bridge` production,
...
| `cargo fmt -p forge-runtime-bridge` | Passed. |
| `cargo check -p forge-runtime-bridge --tests` | Passed. |
| `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail -- --nocapture` | Passed. |
...
Get-ChildItem -Path crates/forge-runtime-bridge/src,crates/forge-runtime-bridge/tests -Recurse -Filter *.rs | ForEach-Object { $c=(Get-Content $_.FullName).Length; if($c -gt 400){ '{0}:{1}' -f $c,$_.FullName } }
```
...
| 5. Closeout proof | Complete | Entire `forge-runtime-bridge` crate. | Run final format/check/focused tests, line-cap scan, import-wall scan, no-shim scan, and write the final classification table. | Final proof has no blocker rows: package check and compile-fail tests pass, runtime-bridge source line-cap scan is clean, import wall is clean, and remaining broad no-shim hits are classified as foundational/canonical/terminal/presentation mechanics. |

Gate 2 exact residue command:

```powershell
rg -n "Vec<String>|Vec<&str>|BTreeSet<String>|HashSet<String>|VecDeque<String>|&\[String\]|request_digests|equivalence_members|record_digests|from_digests|synthetic_|retained_.*digests" crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g "*.rs"
```
...
- Verification evidence: `cargo fmt -p forge-runtime-bridge`,
  `cargo check -p forge-runtime-bridge --tests`, `cargo test -p
  forge-runtime-bridge --test phase_boundaries_compile_fail -- --nocapture`,
...
- Start files:
  - `crates/forge-runtime-bridge/src/subscription/certification/source_artifact_index.rs`
  - `crates/forge-runtime-bridge/src/subscription/certification/source_artifact_index/**`
...
- Verified with `cargo fmt -p forge-runtime-bridge`,
  warning-clean `cargo check -p forge-runtime-bridge --tests`,
  `cargo test -p forge-runtime-bridge subscription_certification --
  --nocapture`, `cargo test -p forge-runtime-bridge
  subscription::certification -- --nocapture`, `cargo test -p
  forge-runtime-bridge source -- --nocapture`, and `cargo test -p
  forge-runtime-bridge --test phase_boundaries_compile_fail -- --nocapture`.
...
- Start files:
  - `crates/forge-runtime-bridge/src/harness/adapter/**/terminal_report_export/**`
  - `crates/forge-runtime-bridge/src/harness/tests/**/terminal_report_export/**`
...
- Start files:
  - `crates/forge-runtime-bridge/src/facade/tests/**`
  - `crates/forge-runtime-bridge/src/harness/tests/**`
  - `crates/forge-runtime-bridge/tests/ui/**`
- Required action:
...
```powershell
rg -n "Vec<String>|Vec<&str>|BTreeSet<String>|HashSet<String>|VecDeque<String>|&\[String\]|request_digests|equivalence_members|record_digests|from_digests|synthetic_" crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g "*.rs"
rg -n "serde_json::Value|json!|serde_json::Map|from_value|to_value" crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g "*.rs"
rg -n "fallback|legacy|compatibility|compat|shim|raw|payload|bytes|aspect_bytes|surface_label|aspect_label|domain_payload|route:sha256|effect:sha256|strategy:sha256|truth-state:sha256|truth-view:sha256|todo!|unimplemented!" crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests -g "*.rs"
```
...
rg -n "Vec<String>|Vec<&str>|BTreeSet<String>|HashSet<String>|VecDeque<String>|&\[String\]|request_digests|equivalence_members|record_digests|read_packet_aspect_value_texts|ranked_materials_by_damage" `
  crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests `
  -g "*.rs"
...
- `foundational-accessor`: `payload()` or `bytes()` calls on
  `forge-foundational` proof/value carriers, not bridge-local payload or byte
  authority.
...
```text
crates/forge-runtime-bridge/src/subscription/certification/source_artifact_index.rs
crates/forge-runtime-bridge/src/source/record_digests.rs
crates/forge-runtime-bridge/src/source/records.rs
crates/forge-runtime-bridge/src/facade/tests/causal_envelope/retained_mapping_support.rs
```
...
- Verified with `cargo fmt -p forge-runtime-bridge`,
  `cargo test -p forge-runtime-bridge causal_envelope -- --nocapture`,
  `cargo test -p forge-runtime-bridge subscription_certification --
  --nocapture`, `cargo check -p forge-runtime-bridge --tests`, the
...
```text
crates/forge-runtime-bridge/src/harness/adapter/terminal_report_export/json_projection.rs
crates/forge-runtime-bridge/src/harness/adapter/adapter_impl/**/terminal_report_export/*json_projection.rs
crates/forge-runtime-bridge/src/harness/tests/pricing_support/terminal_report_export/*.rs
crates/forge-runtime-bridge/src/harness/tests/pricing_showcase_support/terminal_report_export/*.rs
```
...
rg -n "terminal_report_export|json_projection" `
  crates/forge-runtime-bridge/src/source `
  crates/forge-runtime-bridge/src/routing `
  crates/forge-runtime-bridge/src/writeback `
  crates/forge-runtime-bridge/src/replay `
  crates/forge-runtime-bridge/src/diagnostics `
  crates/forge-runtime-bridge/src/subscription `
  crates/forge-runtime-bridge/src/facade `
  -g "*.rs"
```
...
- Renamed the capture helpers to terminal snapshot capture value vocabulary and
  aliased the external `forge_harness::SnapshotPayload` type at the boundary so
...
crates/forge-runtime-bridge/tests/ui/<sealed native proof fixture>.rs
```
...
```powershell
$over = Get-ChildItem crates/forge-runtime-bridge/src,crates/forge-runtime-bridge/tests -Recurse -Filter *.rs | ForEach-Object { $count = (Get-Content $_.FullName | Measure-Object -Line).Lines; if ($count -gt 400) { "$count $($_.FullName)" } }
...
git diff --check -- _docs/forge-runtime-bridge/aspect_native_refactor.md crates/forge-runtime-bridge/src crates/forge-runtime-bridge/tests
```
...
rg -n "Vec<String>|Vec<&str>|BTreeSet<String>|HashSet<String>|VecDeque<String>|&\[String\]|read_packet_aspect_value_texts|ranked_materials_by_damage\[|ranked_materials_by_damage\.first\(" `
  crates/forge-runtime-bridge/src/harness/tests/pricing_support.rs `
  crates/forge-runtime-bridge/src/harness/tests/pricing_support `
  crates/forge-runtime-bridge/src/harness/tests/pricing_shock `
  crates/forge-runtime-bridge/src/harness/tests/pricing_showcase_support `
  crates/forge-runtime-bridge/src/mapping/freezing/tests.rs `
  crates/forge-runtime-bridge/src/harness/tests/support.rs `
  -g "*.rs"
```
...
- Execution order:
  1. Run the no-shim residue scan over `crates/forge-runtime-bridge/src` and
     `crates/forge-runtime-bridge/tests`.
...
- `canonical_basis.rs` and `snapshot/read_result.rs` retain direct
  ...
  `forge-foundational` used to reach canonical basis sequence and validated
  aspect value views.
...
- crate-wide line-cap scan for `crates/forge-runtime-bridge/src` and
  `crates/forge-runtime-bridge/tests`
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo check -p forge-runtime-bridge --tests`
- `cargo test -p forge-runtime-bridge harness -- --nocapture`
- `cargo test -p forge-runtime-bridge snapshot -- --nocapture`
- `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail --
  --nocapture`
...
- Phase 1 line-cap enforcement drift was closed while entering Phase 2: all
  stale `forge-runtime-bridge` entries were removed from the workspace Rust
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge source -- --nocapture`
- `cargo test -p forge-runtime-bridge snapshot -- --nocapture`
- `cargo check -p forge-runtime-bridge --tests`
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo check -p forge-runtime-bridge --tests`
- `cargo test -p forge-runtime-bridge mapping::freezing -- --nocapture`
- `cargo test -p forge-runtime-bridge input::envelope -- --nocapture`
- `cargo test -p forge-runtime-bridge routing::eligibility -- --nocapture`
- `cargo test -p forge-runtime-bridge routing::planning::plan -- --nocapture`
- `cargo test -p forge-runtime-bridge routing::surfaces -- --nocapture`
- `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail -- --nocapture`
...
- Patch and routing error coordinates now retain typed
  ...
  compile-fail proof uses a sealed native target placeholder rather than forged
  target-basis strings.

Phase 4 lowered invalidation target proof, recorded 2026-06-02:

- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge routing -- --nocapture` (37 passed)
- `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail --
  --nocapture` (1 passed)
- `cargo check -p forge-runtime-bridge --tests`
...
- `cargo test -p forge-runtime-bridge mapping::freezing -- --nocapture`
  (11 passed)
- `cargo test -p forge-runtime-bridge routing::canonicalization --
  --nocapture` (1 passed)
- `cargo test -p forge-runtime-bridge bulk_workload -- --nocapture`
  (11 passed)
- `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail --
  --nocapture` (1 passed)
...
- `cargo test -p forge-runtime-bridge routing::canonicalization --
  --nocapture`
- `cargo test -p forge-runtime-bridge routing::planning::plan --
  --nocapture`
- `cargo check -p forge-runtime-bridge --tests`
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge packet_reduction -- --nocapture`
  (11 passed)
- `cargo test -p forge-runtime-bridge bulk_workload -- --nocapture`
  (10 passed)
- `cargo check -p forge-runtime-bridge --tests`
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge routing::canonicalization -- --nocapture`
  (3 passed)
- `cargo test -p forge-runtime-bridge routing::lowering -- --nocapture`
  (1 passed)
- `cargo check -p forge-runtime-bridge --tests`
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge routing::lowering::targets --
  --nocapture` (1 passed)
- `cargo test -p forge-runtime-bridge fine_grained_precision -- --nocapture`
  (3 passed)
- `cargo check -p forge-runtime-bridge --tests`
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge subscription::declaration --
  --nocapture` (17 passed)
- `cargo test -p forge-runtime-bridge routing::lowering -- --nocapture`
  (2 passed)
- `cargo test -p forge-runtime-bridge routing::canonicalization --
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge snapshot -- --nocapture` (50 passed)
- `cargo test -p forge-runtime-bridge routing::canonicalization --
  --nocapture` (4 passed)
- `cargo test -p forge-runtime-bridge routing::planning::plan -- --nocapture`
  (1 passed)
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge continuity -- --nocapture` (49 passed)
- `cargo test -p forge-runtime-bridge routing::planning::plan -- --nocapture`
  (1 passed)
- `cargo check -p forge-runtime-bridge --tests`
...
- `cargo fmt -p forge-runtime-bridge`
- `cargo test -p forge-runtime-bridge delivery::snapshot -- --nocapture`
  (9 passed)
- `cargo test -p forge-runtime-bridge snapshot -- --nocapture` (50 passed)
- `cargo check -p forge-runtime-bridge --tests`
- `cargo test -p forge-runtime-bridge --test phase_boundaries_compile_fail --
...
- Patch construction and missing-route registration denials now retain native
  ...
  target coordinates and no longer teaches forged target-basis string input.
- Targeted residue scan found no old decomposed coordinate constructor, no
  cached `target_canonical_basis_projection`, and no `forged-target-basis`
...
- Verification: `cargo fmt -p forge-runtime-bridge`;
  `cargo test -p forge-runtime-bridge routing::planning::plan -- --nocapture`
  (1 passed); `cargo test -p forge-runtime-bridge explanations -- --nocapture`
  (6 passed); `cargo check -p forge-runtime-bridge --tests`; duplicate route
...
- Tests prove grouped truth rejects ambiguous whole-aspect grouping bindings,
  ...
  aspect-key text.
- Verification: `cargo fmt -p forge-runtime-bridge`;
  `cargo test -p forge-runtime-bridge source -- --nocapture`; and
  `cargo check -p forge-runtime-bridge --tests` all passed with 10 minute
...
- The public `GroupedProjectionSource` trait hard-break is complete:
  ...
  the direct runtime-bridge, forge-query, and forge-relational implementors now
  ...
  or read-value shims.
- Verification: `cargo fmt -p forge-runtime-bridge -p forge-query -p
  forge-relational`; `cargo test -p forge-runtime-bridge source --
  --nocapture`; `cargo check -p forge-runtime-bridge --tests`; `cargo check -p
  forge-query --tests`; and `cargo check -p forge-relational --tests` all
...
- Query-facing grouped planning/live projection surfaces now retain native
  ...
  `DeclarativeLiveViewShape`, `ForgeQueryLiveViewBuilder`,
...
- Verification: `cargo test -p forge-query projection_consumption --
  --nocapture`, `cargo check -p forge-query --tests`, and
  `cargo test -p forge-runtime-bridge source -- --nocapture` all passed with
...
- Causal-envelope evidence references no longer accept raw reference identity
  ...
  facade causal-envelope/writeback mapping tests and forge-query causal
...
- Forge-query batch mutation inspection no longer asks runtime-bridge for
...
- For source changes that alter production bridge semantics, run focused
  `forge-runtime-bridge` checks with a 10 minute timeout per command.
- When a source change touches shared query or relational bridge surfaces, also
  run the relevant focused `forge-query` and `forge-relational` checks with a

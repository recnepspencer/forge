...
Establish the zero-cost core of `forge-proof`: phase-typed artifacts,
...
Everything later in `forge-proof` depends on whether the core artifact model is
...
- `forge-proof` remains a static-first proof substrate, not a dynamic runtime
  registry or graph engine.
- `forge-foundational` will later own diagnostics, lineage, provenance,
...
- `domain_laws.md`
  ...
  decomposition. `forge-proof` must own progression law only; it must not start
...
> Several Forge subsystems with different payload shapes, proof sets, trust
...
Forge repeatedly carries across phases.
...
- no drift of `forge-proof` into diagnostics, provenance, digests, or runtime
  semantics
...
Milestone 1 is complete only when `forge-proof` satisfies the named milestone
suite:

...
>
> **Adjacent milestone:** [milestone-7.md](/C:/Users/shepworth/Documents/programming/forge/_docs/forge-proof/milestone-7.md)
>
> **Primary architectural driver:** make the shipped `forge-proof` surface read
...
Harden the public `forge-proof` API surface so common Forge authoring becomes
...
that real Forge crates reach for it first instead of falling back to bespoke
...
It is that a normal Forge engineer or AI can open a domain module and
understand:
...
- `MENTALITY.md`
  ...
  long-term Forge authoring pressure, not from cosmetic builder enthusiasm.
- `arch_laws.md`
...
- `forge_proof_vision.md`
  The main protection here is ownership: `forge-proof` remains the compile-time
...
- `forge_signal_wasm/api_surface_dx_plan.md`
...
> Real Forge authoring across `relational`, `query`, `signal`, and
> `runtime-bridge` must be able to use a pleasant `forge-proof` lane that is
...
- a substitute for `forge-foundational`
...
- an AI should be able to stay inside the blessed pleasant lane for the
  overwhelming majority of ordinary Forge authoring without needing to study
...
- grouped reads must not become a back door for `forge-foundational`-style
...
- the ownership split between `forge-proof`, `forge-foundational`, and domain
  crates
...
This milestone is complete only when `forge-proof` can prove:
...
- make the pleasant lane the default recommendation for real Forge authoring

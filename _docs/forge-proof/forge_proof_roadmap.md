# Forge Proof Future Roadmap

## Purpose

This document defines the future work for `forge-proof`.
...
stale-honest, and execution-boundary-honest as the rest of the Forge stack
needs it to be.
...
> A Forge subsystem with branch-sensitive proof validity, authority-gated
...
- allows callers to forge a stronger proof-bearing form through ordinary
  construction
...
- each milestone must preserve the ownership split:
  `forge-proof` owns proof-bearing progression law, `forge-foundational` owns
...
`forge-proof` is not intended to become a second runtime. It is intended to
become the shared static substrate beneath several runtimes and planners.

That means:

- ordinary Forge crate authors should be able to express most proof-bearing
  lifecycle progression through `forge-proof` instead of bespoke typestate
  scaffolding
- `forge-proof` may expose zero-cost markers, traits, witnesses, and transition
...
- if a proof-bearing progression pattern is repeated across Forge, it needs an
  explicit roadmap home here
- if the lower crate remains authoritative for the semantics, the roadmap must
  say so explicitly instead of implying `forge-proof` became a second domain
  engine

## Early Cross-Feature Proof Gates

The hardest failures in `forge-proof` live at feature intersections rather than
...
The core substrate must support repeated Forge proof-bearing progression
...
No caller outside the proving boundary may be able to forge a stronger
...
- compile-fail coverage for forged minting attempts and constructor bypass
...
- hostile tests proving symbolic/unresolved recipe stages cannot be forged into
...
Forge must not be able to accidentally consume a proof outside the basis under
...
- proof that receipts and descriptive execution artifacts remain outside
  `forge-proof` while post-execution proof state can still be modeled
...
subset of Forge flows that split one proof-bearing artifact into several
...
machinery in Forge by closing hostile substrate certification and reference
migration lanes.
...
richness on representative real Forge flows.

### Must Ship

- hostile compile-fail suites for illegal progression, forged minting, and
  stale-proof misuse
...
- reference migrations from at least one proof-heavy family in
  `forge-signal`, one in `forge-relational`, and one in `forge-query` or
  `forge-store`
- explicit migration guidance for when to use `forge-proof` versus
  `forge-foundational`
...
- no expansion of `forge-proof` into diagnostics, lineage, or storage ownership

### Proof Obligations

- hostile compile-fail suites for illegal progression, stale misuse, forged
...
- predictable proof-bearing API shape across Forge

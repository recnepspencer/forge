# Forge Query Runtime API Next Batch Closeout

## Status

The Forge Query runtime API next batch is closed as of 2026-04-27 for the
...
- `forge_query_vision.md`: the closed surface advances the promise that one
  ...
  runtimes.
- `forge_query_roadmap.md`: the batch preserves the roadmap rule: declare query
...
- `ForgeQueryRuntime` is the ordinary runtime facade.
- `ForgeQueryRuntimeBackend`, `ForgeQueryRuntimeBackendParts`, and
  `ForgeQueryBridgeBackedRuntimeBackend` form the runtime backend seam.
- `ForgeQueryRuntimeSupportProfile` and facade-family support metadata expose
...
- runtime patch draining emits `QueryDeliveryBatch` /
  `ForgeQueryRuntimeDeliveryBatch` evidence instead of making app code consume
...
- `ForgeQueryDerivedViewHandle` and maintained derived views support live and
...
- `cargo fmt -p forge-query -p forge-ui`
...
for ordinary `forge-query` consumers.

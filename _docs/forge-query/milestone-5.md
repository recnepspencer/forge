...
>
> **Prior milestone:** [milestone-4.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-query/milestone-4.md)
>
> **Prior closeout:** [milestone-4-closeout.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-query/milestone-4-closeout.md)
>
...
then the vision breaks. `forge-query` would have one truth for one-shot reads
...
- `forge_query_roadmap.md`: Milestone 5 is the first proof that query meaning
...
- live-maintained results are query-shaped and result-family-shaped; consumers
  ...
  child updates
- `forge-signal` owns scheduling and dependency invalidation mechanics;
  `forge-query` owns how a query lowers into signal-facing relevance contracts
  and query-shaped patches
- the runtime bridge owns patch-to-invalidation routing and change summaries;
  `forge-query` must consume bridge-owned change descriptors rather than reach
...
- compile-fail or privacy hardening proving that live artifacts cannot be
  forged or constructed from raw change records externally
...
Milestone 5 is complete only when `forge-query` can prove:
...
and forces `forge-query` to rediscover semantics through broad rereads.
...
The milestone preserves authority boundaries because `forge-query` owns live
promotion and query-shaped patch meaning, `forge-signal` still owns scheduling,

...
admission decision lattice in `forge-query`.
...
- phase-seven crate documentation installed into the categorized
  `crates/forge-query/docs` tree with compile-checked examples and certification
  linkage
...
- categorized crate documentation under `crates/forge-query/docs`
...
- `cargo fmt --all`
- `cargo test -p forge-query public_doc_wording -- --nocapture`
- `cargo test -p forge-query certification -- --nocapture`
- `cargo test -p forge-query --test phase_boundaries_intent_admission_compile_fail -- --nocapture`

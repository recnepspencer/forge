# Milestone 9.6 Closeout Plan — Product Boundary Debt Closure
...
boundaries — with **no surviving same-class folklore** in forge-query or the
primary downstream consumer (`worth-kernel` construction):

| Boundary | Ordinary-path contract |
|----------|------------------------|
| Evidence identity | Sealed `ForgeQueryEvidenceIdentity::compose`; no format-string / pipe-join digest assembly on covered surfaces and feeders |
| Stop-class matching | `ForgeQueryRuntimeError::stop_class()`; no message grep control flow on covered denial paths |
| Session label identity | `ForgeQuerySessionLabel` intake on preview/branch; typed collision posture |

**Honest closure requires all of:**

1. Every covered surface and same-class upstream/adjacent feeder uses typed boundaries (spec Closure Gate §1–2).
2. Support/profile derives `Closed` only from proof — not from a curated scan alone (§3).
3. Sibling boundaries architecturally consistent — no mixed model (§4).
4. Exclusions named with owner milestone; no hiding same-class debt (§5).
5. Acceptance evidence in spec §682–710 — especially full `cargo test -p forge-query --lib` green and worth-kernel consumer migration.

---

## 2. Current state (2026-06-16 QA)

### Green (forge-query certification lanes)

| Gate | Result |
|------|--------|
| `cargo test -p forge-query session_label --lib` | **21/21** (fixed: basis_admission tag `basis_evidence`) |
| `cargo test -p forge-query evidence_identity --lib` | 19/19 |
| `cargo test -p forge-query stop_class --lib` | 22/22 |
| `cargo test -p forge-query identity_boundary --lib` | 24/24 |

### Red

| Gate | Result |
|------|--------|
| `cargo test -p forge-query --lib` | **2277 pass / 47 fail** |
...
### WS-1 — Lib test regression burn-down (47 failures)

**Goal:** `cargo test -p forge-query --lib` green.

Fix in dependency order (each slice lands with focused test run before next):

| Slice | Tests | Root cause | Primary touch |
|-------|-------|------------|---------------|
| **WS-1a** | 9 | Assembly builders missing `snapshot_identity` adapter → `MissingSnapshotIdentityAdapter` | `runtime/tests/assembly/builder.rs`, support_profile + invariant_registration tests |
...
| **WS-1e** | 8 | Mutation continuity / naming / binding / batch — digest or entity identity drift | `runtime/tests/mutation/{continuity,existing_binding,naming,verified_update_existing}/` |
...
**Gate:** `cargo test -p forge-query --lib`

**Done when:** zero failures; no `@ignore` or suite narrowing.

---

### WS-2 — Primary consumer migration (worth-kernel)
...
**Done when:** branch/preview basis digest tests use Query-owned identity; at least one consumer-shaped test proves stop-class matching without string ops on covered denial paths (mirrors forge-query Phase 4 bar).

---

### WS-3 — Inventory and closure-honesty hardening

**Goal:** Support report `Closed` means what the spec says.

1. **Expand scan coverage** — add to `EXACT_ZERO_FORMAT_DIGEST_PATHS` or a new feeder scan:
...
2. **Review exclusions** — `EXCLUDED_FOLKLORE_PATHS` (`projection_consumption/`, `workflow/`, `domain_capabilities/`):
   - For each: document as **named deferral** with owner milestone, **or** migrate and remove exclusion.
   - Same-class debt in excluded dirs cannot block 9.6 close if spec §5 applies — but must be explicitly defended in closeout note.

3. **Tie closure posture to certification** — extend `ForgeQueryIdentityBoundaryClosure::derived()` or add closeout gate test:
...
4. **String-matching scan** — widen beyond single file `runtime/tests/stop_class/consumer_support/routing.rs` to worth-kernel construction tests once migrated.
...
### WS-4 — Hostile re-QA and doc closeout

**Goal:** Replace stale 2026-06-10 closeout note with evidence-backed closure.

1. Re-run hostile QA prompt (spec Phase 7 + Closure Gate) — code inspection, not tests alone.
2. Update [milestone-9.6.md](./milestone-9.6.md):
   - Status → `Closed` only after WS-1–WS-3 gates pass
   - Closeout note with date, branch, gate command outputs
3. Update [forge_query_roadmap.md](./forge_query_roadmap.md) Milestone 9.6 entry if posture changed during re-open.
4. Append evidence paths under `_docs/forge-query/goal_mode_*` or new `milestone-9.6-closeout-evidence.md` (gate logs, failure count before/after).
...
**Critical path:** WS-1 → WS-3 → WS-4. WS-2 can run in parallel with WS-1c–D but **must complete before WS-4** (spec requires consumer boundary closure).

**Do not:** fold bridge-truth Phase 10 items (worth-spatial, compile-fail matrix expansion) into this plan — track separately.
...
```text
cargo test -p forge-query application::support::identity_boundary_inventory::tests --lib
cargo test -p forge-query public_doc_coverage::tests::identity_boundary_docs --lib
```
...
| worth-kernel scope creep (100+ `digest_owned_parts`) | WS-2 scopes to **construction basis/stop-class paths** from spec; broader digest migration → named follow-on (9.7/9.8 prep) |
...
#### 2026-06-16 — WS-3/WS-4 closeout (query-repair)

...
Without Milestone 9.3, `forge-query` could still fail in subtle but dangerous
ways:
...
- `forge_query_vision.md`: Query promises one typed read model promotable to
...
- `forge_query_roadmap.md`: Milestone 9.3 must prove automatic subscription
...
- `forge-query` owns query-family support reporting, subscription diagnostic
...
- `forge-query` does not own bridge subscription protocol semantics, signal
...
- `forge-runtime-bridge` remains authoritative for bridge declaration family,
  basis request posture, preview residue classes, and bridge-facing lifecycle
  semantics beneath admitted query lowering.
- `forge-signal` remains authoritative for admitted observation strategy,
  invalidation, scheduling, and maintenance execution beneath the active query
  lane.
- `forge-relational` remains authoritative for query meaning, policy/tenant
...
Milestone 9.3 must extend `crates/forge-query/src/subscription/` through
...
- `crates/forge-query/src/harness/milestone_nine_three_certification/`
  - `mod.rs`
  - `builders.rs`
  - `rows.rs`
  - `tests.rs`
...
- Runtime-backed support reporting, diagnostic bundling, bridge parity
  explanation, and family certification are not blocked on `forge-store`.
...
- 9.3 assumes bridge family and slice semantics remain owned by
  `forge-runtime-bridge`; Query only certifies parity against those surfaces.
...
Milestone 9.3 is complete only when `forge-query` can prove:

# Forge Query Aspect API Finalization Plan
...
> code when `forge-relational`, `forge-store`, and the runtime bridge are
...
Finalize the public Forge Query mutation API around aspect-native authoring,
...
Forge Query now has a stabilized runtime-backed facade for live views,
...
- `domain_laws.md`: Forge Query must stay domain-neutral. The plan can be
...
- `forge_query_vision.md`: Query is the daily-driver query framework where
  developer experience lives or dies. Public writes must speak the same aspect
  vocabulary as the rest of the framework.
- `forge_query_roadmap.md`: the guiding rule remains `declare query intent
...
against Forge Query using aspect-native writes now, and that same code must
...
The exact surface names may adapt to local Forge Query naming style, but the
...
- no domain semantics inside Forge Query
...
- Forge Query remains the public facade, not the truth/storage authority
- `forge-relational`, `forge-store`, and the runtime bridge remain free to
...
- removing JSON-backed truth storage from `forge-relational`
- redesigning `forge-store` around native aspect persistence
...
- embedding geometry, workflow, table, or DSL semantics into Forge Query
...
`forge-relational` / `forge-store` / bridge rewrite begins.

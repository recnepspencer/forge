...
> **Status:** Phase 2B **closed** (2026-06-15) — evidence/feeder root breaks and
> repair are complete for `forge-query` + `forge-runtime-bridge`. Downstream
> crates (worth-topo, forge-server, etc.) are explicitly out of Phase 2B scope
...
### Forge-Foundational Substrate

Use `forge-foundational` as the standard generic boundary vocabulary. It must
...
3  -> forge-relational source-truth admission
4  -> forge-runtime-bridge authority carriers
5  -> forge-query intake, storage, adapters, and harnesses
6  -> forge-query receipts, write surfaces, and shared read pins
...
5. **Downstream and harness consumers** — worth-topo, worth-kernel, forge-server,
...
- [x] Every lower-authority category has an intended `forge-foundational`
  substrate type.
...
inventory to `_docs/forge-query/bridge_truth_identity_exposure_report.md`. Each
...
- `forge-runtime-bridge`: bridge identity constructor/accessor and
  evidence-category failures.
- `forge-query`: receipt/intake/storage/feeders reject projection, digest,
...
- `ForgeQueryEvidenceIdentityEncoder::field_identity(impl AsRef<str>)` removed.
- `ForgeQueryEvidenceIdentityEncoder::field_identity_sequence(...)` removed.
- `ForgeQueryEvidenceIdentityEncoder::field_bridge_identity(...)` removed.
- `ForgeQueryEvidenceIdentityEncoder::optional_identity(...)` removed.
- `ForgeQueryEvidenceIdentity::as_str()` made crate-private.
- `ForgeQueryEvidenceIdentity::compose(...)` made crate-private.
- `ForgeQueryEvidenceIdentity::bridge_evidence_identity()` made crate-private.
- `ForgeQueryEvidenceIdentity::bridge_external_identity_evidence()` made
  crate-private.
...
- `ForgeQueryEvidenceIdentity` no longer implements `Display`.
- `ForgeQueryEvidenceIdentity` no longer implements `AsRef<str>`.
...
| `E0277` missing `AsRef<str>` bound | 3 | Evidence identity string coercion | Generic string-like API attempted to accept `ForgeQueryEvidenceIdentity`. |
...
- `forge-relational/src/identity_authority/` must classify every source-truth
...
- `forge-signal` does not currently use the same
  `FoundationalAuthorityIdentity` category scaffold as relational/bridge/query.
  Its protection comes from `forge_proof::Artifact` authority bases, typed
...
- `forge-relational` is typed and sealed for the upstream bridge-presentation
...
- `forge-signal` is quarantined as a signal proof/domain-token lane for the
  ...
  authority APIs.
- `forge-runtime-bridge` membrane identity categories are compiler-gated for
...
- Verification artifacts:
  ...
  `_docs/forge-query/upstream_foundation_gate_signal_trybuild.txt`, and
  `_docs/forge-query/upstream_foundation_gate_signal_lib.txt`.
- Phase 2A does not add a `forge-signal` `FoundationalAuthorityIdentity`
  ...
  Forge identity-family boundaries. Today the public boundary is already
...
- Letting `ForgeQueryEvidenceIdentity` become authority merely because it
...
composition substrate** and mandatory `forge-foundational` category admission so
...
|------|-------|----------|--------------|
| 0 | `forge-foundational` admission / projection / digest derive | substrate | Types exist; not **required** at Query roots |
...
| 3 | **Query evidence composition** (`ForgeQueryEvidenceIdentity` encoder) | **B** | **Not broken — primary parent** |
| 4 | Query truth IDs (`ForgeQueryCommitIdentity`, snapshot, entity) | C | Partial — external-label + round-trip open |
...
`forge-query`, you broke a child — move up a tier.
...
| `ForgeQueryEvidenceIdentityEncoder::field_identity(AsRef<str>)` globally | **Parent** — expect 50–150+ error files |
| `ForgeQueryEvidenceIdentity::as_str` / `AsRef<str>` globally | **Parent** |
...
`cargo check -p forge-query` red across subscription, causal, intent,
...
Extend `crates/forge-query/src/identity_authority/phase_one_root_break_targets.rs`
...
|-----|----------------------|
| `ForgeQueryEvidenceIdentityEncoder::field_identity` | Remove public `impl AsRef<str>` ingress; identity slots accept category types only |
| `ForgeQueryEvidenceIdentityEncoder::field_identity_sequence` | Same |
| `ForgeQueryEvidenceIdentityEncoder::optional_identity` / `optional_evidence_identity` on `&str` | Same |
| `ForgeQueryEvidenceIdentityEncoder::field_evidence_identity` | Must not delegate to string ingress; require `&ForgeQueryEvidenceIdentity` or admitted authority handle without `AsRef<str>` collapse |
| `ForgeQueryEvidenceIdentityEncoder::field_bridge_identity` | Must not flatten to `terminal_projection_for_reporting()` at compose time |
| `ForgeQueryEvidenceIdentity::compose` | Witness-gate or restrict to owner admission modules |
| `ForgeQueryEvidenceIdentity::as_str` | Terminal projection module only |
| `impl AsRef<str> for ForgeQueryEvidenceIdentity` | Remove |
| `ForgeQueryEvidenceIdentity::bridge_evidence_identity` | Remove or owner-gate — no rebuild from digest token text |
| `ForgeQueryEvidenceIdentity::bridge_external_identity_evidence` | External-token path only with explicit admission |

**Tier 4 — Query truth IDs (Frontier C)**

| API | Required restriction |
|-----|----------------------|
| `ForgeQueryCommitIdentity::from_external_authority_label` | Remove; external labels → `QueryExternalIdentityToken` → witness admission |
| `ForgeQuerySnapshotIdentity::from_external_authority_label` | Same |
| `ForgeQueryEntityIdentity::authored_command` | Same |
| `ForgeQuery*Identity::preview(ForgeQueryEvidenceIdentity)` | Require admitted evidence / owner witness |
| `ForgeQuery*Identity::evidence_identity` | Must not feed compose/routing/admission; terminal export or digest evidence only |
| `impl Display for ForgeQuery*Identity` | Remove or opaque debug only |
| `ForgeQueryEntityIdentity: Ord/Hash` via `as_str()` | Compare typed authority, not projection text |
...
2. Create all `tests/ui/query_identity_authority/*.rs` fixtures and capture
   `.stderr` expectations once `forge-query` compiles far enough for trybuild
...
```powershell
cargo fmt --all
rg -n "ForgeQueryEvidenceIdentityEncoder::field_identity|ForgeQueryEvidenceIdentity::as_str|hash_parts|BridgeIdentityEvidence::from_external_authority" crates/forge-query/src/identity_authority/phase_one_root_break_targets.rs
rg --files crates/forge-query/tests/ui/query_identity_authority
rg -n "forge_query_identity_phase_one_compile_fail_targets" crates/forge-query/tests/phase_boundaries_query_identity_authority_compile_fail.rs
cargo test -p forge-query --test phase_boundaries_query_identity_authority_compile_fail --no-run
```

The `cargo test --no-run` command may fail while `forge-query` is red from 2B
...
1. Privatize or remove `ForgeQueryEvidenceIdentityEncoder::field_identity` and
   string-based `optional_identity` / `field_identity_sequence`.
2. Split replacement APIs, e.g.:
   - `field_authority_identity` → `&ForgeQueryEvidenceIdentity` or
...
3. Remove public `ForgeQueryEvidenceIdentity::as_str` and `impl AsRef<str>`.
4. Run `cargo check -p forge-query 2>&1` → record **full** ledger in
...
- [x] `cargo check -p forge-query --lib` is red at the Query evidence/feeder
  frontier with QA-expanded digest converter discovery.
- [x] `cargo check --workspace` is red at the same `forge-query` frontier.
- [x] Raw transcripts are captured in
  `_docs/forge-query/frontier2_query_root_break_cargo_check.txt` and
...
- [x] After 2B-5, `cargo check -p forge-query --lib` remains intentionally red
...
1. Run `cargo check -p forge-query 2>&1` (or `--workspace` when unblocked).
2. Record **unique error file count**. Parent break: expect **≥ 50 files** in
   `forge-query` for Frontier B. If &lt; 20, the break was too shallow.
...
- [x] Frontier B break landed; `cargo check -p forge-query` was red for category
...
- [x] Frontier C–E breaks landed (C: external-token truth ID admission; D/E:
  bridge + digest folklore roots cut and repaired in `forge-query`).
- [x] No public `field_identity(AsRef<str>)`, no public
  `ForgeQueryEvidenceIdentity: AsRef<str>`, no dual `String` + typed authority
  storage pattern in new code.
- [x] Repair slices 2B-5+ green `forge-query --lib` and `--lib --tests` under
...
section in _docs/forge-query/milestone-9.6-bridge-truth-identity-lowering.md.
...
3. Frontier B: break ForgeQueryEvidenceIdentity encoder string ingress and
   public as_str/AsRef<str>. Do NOT fix call sites in the same commit.
4. cargo check -p forge-query; write 2B-* rows to bridge_truth_identity_exposure_report.md.
...
downstream crates; marking Phase 2B done with <50 error files in forge-query.
```
...
**Phase 4** — `forge-runtime-bridge/src/identity.rs`, facade exports; bridge
...
adapters under `forge-query`; Query intake must reject projection/digest/external
...
- `ForgeQueryEvidenceIdentity` is evidence unless explicitly admitted by the
...
| Phase 9 downstream and harness cleanup | 9 | relational IDs, bridge truth carriers, Query typed mutation/read receipts | typed lower identity -> bridge/Query typed receipt -> harness/downstream adapter -> terminal report/compat projection | Phase 9 open: forge-server adapters, hadwiger-research, and forge-ui still contain string-authority consumers. Query harness/certification and worth-topo certification rows are fixed or verified stale; `worth-kernel` and `forge-kernel` had no current blocker in this sweep. |
...
| Foundational evidence | `ForgeQueryDeclarationFoundationalEvidenceInput` | admitted progression -> foundational evidence | `handle_identity_digest()` | Fixed | Foundational evidence world admission now compares the subject's retained `ForgeQueryEvidenceIdentity` handle against the expected admitted world-basis handle identity when the subject carries typed world basis; digest strings remain only for denial detail/reporting compatibility. Gates: `cargo test -p forge-query application::declaration_evidence --lib` (`12 passed`), `cargo check -p forge-query` |
...
| Active lane admission | activation/admission/query/bridge/signal identities | `ActiveSubscriptionLaneAdmission` | digest-looking accessors | Fixed | Admission stores activation/admission/query/bridge/basis/checkpoint/signal values as `ForgeQueryEvidenceIdentity` fields and exposes digest-looking methods only as reporting projections; typed identity accessors remain available for authority composition. Gate: `cargo test -p forge-query subscription::tests::active::active_lifecycle --lib` (`12 passed`) |
| Active lane registry | active lane + attachment | registry keys/handles | stringified keys | Fixed | Registry indices use `ActiveSubscriptionLaneDigest` and `SubscriptionConsumerAttachmentDigest` typed wrappers backed by `ForgeQueryEvidenceIdentity`, not raw display strings; handles retain typed basis/checkpoint identities. Gate: `cargo test -p forge-query subscription::tests::active::active_lifecycle --lib` (`12 passed`) |
...
| Query rows / deltas | topology row identity | `ForgeQueryEntityIdentity` plus explicit `identity.id` projection | endpoint/reporting labels | Phase 8 fixed | Typed row authority remains `ForgeQueryEntityIdentity`; endpoint labels are explicit terminal projections and Query read materialization no longer indexes by evidence digest |
...
| Relation endpoint lowering | relation endpoints | typed `ForgeQueryEntityIdentity` / relational parts | endpoint label projection | Phase 8 fixed | Existing graph relation endpoints require relational record parts and refuse evidence-digest fallback |
...
| Milestone-eight harness | bridge harness labels | patch/head/snapshot/branch fixtures | typed relational/bridge fixtures plus terminal labels | Phase 9 fixed | Covered by detailed fixed harness rows and focused forge-query gates recorded in the row 1051 closeout evidence |
...
| Worth-topo derived chain | topology derived-chain certification | inspection/write receipt assertions | typed commit identity comparison | Phase 9 fixed | Derived-chain inspection compares typed `ForgeQueryCommitIdentity` handles; remaining surface names are derived-view labels. Gate: `cargo test -p worth-topo certification::projection_closeout::tests::derived_chain --lib` (`2 passed`). |
| Hadwiger research | test write authority | typed `ForgeQueryMutationReceipt` constructor | terminal Hadwiger report strings | Phase 9 fixed | Research graph invariant fixtures now use typed Query commit/snapshot/entity identities and typed lower-runtime envelope/source identity accessors; gate `cargo test -p hadwiger-research --test research_graph_invariants` passed (`10 passed`). |
| Forge UI todo truth | UI truth state | task truth/snapshot routing | explicit snapshot projection label | Phase 9 fixed | Todo truth now routes mutations through typed `ForgeQueryEntityIdentity`, exposes board state as `TodoSnapshotProjection`, and treats the combined snapshot label as terminal UI diagnostics. Gate: `cargo check -p forge-ui` passed. |
| Forge-server compat request | HTTP JSON request | compat request parsing | `entity_identity` JSON strings | Phase 9 fixed | Compat update/delete request parsing admits entity identity only from canonical relational bridge record text via `RelationalBridgeRecordIdentityParts::from_bridge_entity_identity(...)` before constructing `ForgeQueryEntityIdentity`; raw HTTP labels remain external compatibility input and cannot satisfy authority. Gates: `cargo check -p forge-server`, `cargo test -p forge-server --test compat_http_phase_three` (`8 passed`) |
| Forge-server mutation result | direct/compat mutation response | result digest | `receipt.commit_identity()` | Phase 9 fixed | Single-mutation response result digests now project from typed commit evidence identity (`commit_evidence_identity().as_str()`), while inspection assertions compare typed commit/snapshot handles. Gates: `cargo test -p forge-server --test forge_native_facade_entry` (`62 passed`), `cargo test -p forge-server --test compat_http_phase_three` (`8 passed`) |
| Forge-server query execution | handoff workspace | compatibility precondition | `workspace().snapshot_token()` | Phase 9 fixed | Compat mutation precondition observes the workspace typed snapshot identity and projects it only as the terminal compatibility validator label; direct read/state paths use explicit reporting accessors instead of the removed snapshot-token trait seam. Gate: `cargo check -p forge-server` |
...
6. Phase 2A + 2B hard gates still enforced — no restored public string constructors,
   ...
   `ForgeQueryEvidenceIdentity: AsRef<str>`.
...
3. Phase 2A: truth-routing root breaks (closed). Phase 2B: install evidence/feeder
   root breaks per Phase 2B slices; run `cargo check -p forge-query` (then
...
| 9 | hadwiger-research | `tests/research_graph_invariants.rs`, `src/research_graph_invariants/requests.rs`, `src/agent_advisory/operations.rs` | Test write authority builds `ForgeQueryMutationReceipt` with `commit_identity: commit_identity.to_string()` and `snapshot_token: format!("{commit_identity}:snapshot")`; downstream Hadwiger code still called removed digest accessors on lower-runtime boundary envelopes and grouped compositions. | Fixed | Research graph invariant fixtures now construct `ForgeQueryMutationReceipt` through typed `ForgeQueryCommitIdentity`, `ForgeQuerySnapshotIdentity`, and `ForgeQueryEntityIdentity` values derived from deterministic relational fixture parts; lower-runtime boundary source and envelope reporting strings project from typed identity accessors only at Hadwiger report edges; grouped contribution report strings use typed composition identity. Gate: `cargo test -p hadwiger-research --test research_graph_invariants` (`10 passed`). |
| 9 | forge-ui | `src/todo/truth.rs`, `src/showcase.rs` | Todo truth state stores `snapshot_token: String`, synthesizes workspace snapshot text from child snapshot tokens, and routes mutations through string entity identities. | Fixed | Todo truth now routes update/delete by typed `ForgeQueryEntityIdentity`, exposes board snapshot state as explicit `TodoSnapshotProjection`, and projects the combined snapshot label only for UI diagnostics. Gate: `cargo check -p forge-ui` passed. |
| 9 | forge-server | `surfaces/compat_http/mutation_execution/schema.rs` | Compat HTTP mutation requests parse `entity_identity` and `resolved_entity_identity` as JSON strings and build canonical request digests from formatted identity text. | Fixed | Compat request JSON remains an external-token boundary, but update/delete authority now requires canonical relational bridge record text admitted through `RelationalBridgeRecordIdentityParts::from_bridge_entity_identity(...)` before building `ForgeQueryEntityIdentity`; non-canonical raw labels deny before authority construction. Gate: `cargo test -p forge-server --test compat_http_phase_three` (`8 passed`) |
| 9 | forge-server | `forge_native/direct/mutation.rs`, `surfaces/compat_http/mutation_execution/response.rs` | Server mutation result digests use `receipt.commit_identity()` as a string result digest for single mutations. | Fixed | Single-mutation responses project `commit_evidence_identity().as_str()` as terminal result digest; typed commit authority remains available only through typed receipt/inspection accessors. Gates: `cargo test -p forge-server --test forge_native_facade_entry` (`62 passed`), `cargo test -p forge-server --test compat_http_phase_three` (`8 passed`) |
...
| Artifact | When | Path |
|----------|------|------|
| Compiler failure ledger / exposure report | Phase 2 | `_docs/forge-query/bridge_truth_identity_exposure_report.md` — first `cargo check --workspace` failure inventory after root authority breaks; classify attempted category, required category, owner phase, and closure route |
| Compile-fail gates | Phase 2+ | `forge-runtime-bridge/tests/ui/`, `forge-query/tests/ui/`, downstream/harness UI fixtures for forbidden substitutions |

...
- [ ] All matrix rows with `Fix Phase = 7` are `Fixed`, including feeder bundles
  572–577 (not path rows alone)
- [ ] Local gates clean for Phase 7 surfaces (at minimum):
...
>
> **Phase 7 QA status:** `NOT CLEARED` — feeder bundles 572–577 remain `Open`;
> re-run Phase 7 QA gate after fixes.
>
> **Phase 8 milestone status:** `Blocked on Phase 7 QA` — path rows may show
> `Fixed` where worth-topo code landed early; sequencing treats Phase 8 as
> incomplete until Phase 7 QA is `CLEARED`.
>

# Forge Query Todo Showcase Spec
...
>
> **Test requirements reference:** [test-requirements.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-query/test-requirements.md)
>
> **Most relevant capability specs:**
> - [milestone-5.2.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-query/milestone-5.2.md)
> - [milestone-5.5.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-query/milestone-5.5.md)
> - [milestone-8.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-query/milestone-8.md)
>
> **Primary architectural driver:** prove that `forge-query` can act as the
...
single coherent UI, that `forge-query` can provide live views, grouped board
...
The Forge Query vision makes a very strong product claim:

- `forge-relational` defines truth
- `forge-query` is how consumers ask for it
...
`forge-query` is not a read helper. It is the app surface.
...
- `arch_laws.md`: the key protection is facade and authority discipline. The
  showcase must make `forge-query` the daily-driver facade while leaving truth,
...
- `forge_query_vision.md`: the single most important thing it protects is that
...
- `forge_query_roadmap.md`: the strongest shaping constraint is that `forge-
...
direct consequence of existing Forge Query surfaces, with the application layer
providing mostly schema, seed data, layout, and a small amount of presentation
logic.

## Product Decision Lock

- this is a `forge-query` showcase first and a todo app second
...
- if a capability is not admitted cleanly through `forge-query`, the showcase
...
That story is understandable to any audience and exposes several Forge Query
...
- drag-and-drop if it materially increases code volume or obscures the query
  story
- any new lower-layer capability that would be better solved inside a Forge
...
- `forge-query` remains the app-facing facade
...
- the app's most impressive moments come from `forge-query` capability reuse,
...
- full support for every Forge Query feature family
...
- the app is downstream of already-specified Forge Query capability families
...
- "impressive" UI that secretly bypasses Forge Query
...
This spec belongs under `_docs/forge-query/` because it is a product-facing
demonstration of Forge Query's application surface, not a generic UI exercise.
...
- feature work that would require a new Forge Query roadmap milestone to be
  honest

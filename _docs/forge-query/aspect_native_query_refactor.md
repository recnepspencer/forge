# Forge Query Aspect-Native Query Refactor
...
> [aspect-contracts-values-and-authoritative-state](../../crates/forge-foundational/docs/aspect-contracts-values-and-authoritative-state/README.md)
>
> Primary architectural driver: hard-break `forge-query` internals onto
> `forge-foundational` contracts, locators, field paths, values, masks,
...
- `crates/forge-query/src/projection_consumption/extraction/row_like.rs` now feeds native aspect values into consumed field facts, but broader materialization rows still retain JSON compatibility surfaces.
...
A clean scan for `serde_json::Value`, `aspect_path`, or `ForgeQueryAspectValue`
...
- accepts `ForgeQueryAspectValue` as path-plus-JSON authority;
...
`crates/forge-query/src` is a blocker for closeout of the phase that touched
...
`ForgeQueryWriteCommand::UpdateAspect { aspect_path, value }`,
path-bearing delete variants, and `ForgeQueryAspectValue` are legacy
...
- residue scan output for `serde_json::Value`, `json!`, `aspect_path`,
  ...
  `ForgeQueryAspectValue` is triaged by zone as discovery evidence only;
...
- Use `cargo check -p forge-query` or narrower `cargo check` commands as the
...
- When a source change touches bridge-backed execution or relational authority
  boundaries, also run relevant focused `forge-runtime-bridge` or
  `forge-relational` checks only after the Query slice is coherent enough that
...
2. run `cargo check -p forge-query` only when compiler feedback will guide the
...
authority. That slice should target `ForgeQueryWriteCommand`,
`ForgeQueryAspectValue`, existing-truth probe facts, and the directly coupled
...
- `ForgeQueryAspectValue` now retains parsed native desired-aspect state and
  exposes JSON only through terminal projection helpers.
- `ForgeQueryAspectTouch` retains parsed native delete/touch targets instead
  of raw touched aspect path strings.
- `ForgeQueryAspectValue::new_clear` and
  `ForgeQueryAspectMutationBuilder::clear` now require
  `ForgeQueryAspectTouch`, so clear-authority cannot be constructed from raw
  aspect path strings inside the proof-bearing mutation builder.
- `ForgeQuerySymbolicAspectReference::same_batch_entity_identity`,
  `ForgeQueryAspectMutationBuilder::symbolic_entity_identity`, and graph
  relation symbolic identity authoring now require `ForgeQueryAspectTouch`, so
  symbolic reference authority cannot be promoted from raw aspect path strings.
- `ForgeQueryAspectValue::new_set_native` and crate-local symbolic
  evidence-identity set construction now require `ForgeQueryAspectTouch`, so
  native/evidence set-aspect proof construction reuses admitted targets instead
  of re-parsing terminal path text.
- `ForgeQueryAspectValue::new`, `ForgeQueryAspectValue::new_set`,
  `ForgeQueryAspectMutationBuilder::aspect`, and graph relation set-aspect
  authoring now require `ForgeQueryAspectTouch`, so proof-bearing set-aspect
...
- Set-aspect construction now requires foundational `AspectValue` instead of
  ...
  `ForgeQueryProgramValue` into native scalar values, and the old desired-aspect
...
- Graph touch descriptor rows, descriptor constructors, obligation selectors,
  and lookup-key derivation now retain `ForgeQueryAspectMutationOperation` and
  `ForgeQueryAspectTouch` values for declared/touched aspect evidence instead
  of accepting or parsing `set:path` and touched-path strings.
- `ForgeQueryDeleteMutationBuilder::touch` and `touches` now require
  `ForgeQueryAspectTouch`, so delete touch authority cannot be constructed
  from raw aspect path strings inside the proof-bearing builder.
- `ForgeQueryWriteCommand::UpdateAspect` no longer carries
...
- backend `write` and `write_batch` now require
  `ForgeQueryBackendAdmissibleMutation`, so a raw authored
  `ForgeQueryWriteCommand` cannot satisfy backend execution directly.
- existing-truth probe requests now retain `ForgeQueryAspectTouch` values and
...
- existing-truth probes now return `ForgeQueryExistingTruthProbeField` with a
...
- retained upstream inputs now accept computed rows as
  `ForgeQueryRetainedMaterializedRow` carriers at construction; terminal JSON
...
- `ForgeQueryEffectDelivery` now stores `ForgeQueryEffectPayload`, a native
  payload carrier with private fields, instead of retaining
  `serde_json::Value` as delivery meaning.
- effect routing constructs native `ForgeQueryEffectPayload` values for
...
- `ForgeQueryEffectPayload` is exported through the runtime facade so public
...
- `ForgeQueryMutationDelta` now stores admitted `ForgeQueryAspectTouch` values
  instead of retaining raw touched aspect path strings.
- `ForgeQueryLivePatch` now has the same native touch carrier shape and exposes
...
- memory workspace insert/update receipts preserve parsed touch proof from
  `ForgeQueryAspectValue` through `ForgeQueryAspectTouch::from_parsed_target`
...
- `ForgeQueryDerivedMaterializationResult` now exposes crate-local retained
...
- `ForgeQueryAspectMutationOperation` now stores `ForgeQueryAspectTouch`
...
- `ForgeQuerySymbolicAspectReference`,
  `ForgeQuerySymbolicAspectResolutionEvidence`, and graph-composition
...
- `ForgeQueryVerifiedAssumptionSet` now stores asserted aspects as
  `ForgeQueryAspectTouch` values instead of raw asserted aspect path strings.
- `ForgeQueryVerificationReadSetBreadth` computes distinct asserted aspect
...
- existing-truth assertion verification now preserves the parsed targets from
  `ForgeQueryAspectValue` when building verified assumption evidence.

### Phase 5 effect delivery touch slice

- `ForgeQueryEffectTrigger` and `ForgeQueryEffectExpression` now retain
  `ForgeQueryAspectTouch` declarations for trigger, input, and output aspects
  instead of storing raw aspect path strings.
- public effect trigger and expression constructors, plus the workspace effect
  builder trigger/condition methods, now require `ForgeQueryAspectTouch`
  values so weaker authoring strings cannot satisfy effect declaration
  authority.
- `ForgeQueryEffectDelivery` now stores changed trigger aspects as
  `ForgeQueryAspectTouch` instead of raw aspect path strings.
- `ForgeQueryEffectPayload` stores native changed-aspect touches and exposes
...
- `ForgeQueryDerivedPatch` now stores produced/changed aspects as
  `ForgeQueryAspectTouch` instead of raw aspect path strings.
- public `ForgeQueryDerivedPatch::incremental` and
  `whole_refresh_materialized` constructors now require
  `ForgeQueryAspectTouch` values, so maintainers cannot construct patch
...
- `ForgeQueryRetainedRefreshContext` now stores touched aspects as
  `ForgeQueryAspectTouch` instead of raw touched aspect path strings.
...
- `ForgeQueryPreviewExecutionEvidence` now stores preview-routed aspect changes
  as `ForgeQueryAspectTouch` values instead of raw aspect path strings.
...
- `ForgeQueryBatchWriteReceipt` now stores batch touched aspects as
  `ForgeQueryAspectTouch` instead of raw touched aspect path strings.
- batch receipt aggregate derivation deduplicates admitted touches from write
  receipt deltas, so callers cannot forge batch touched-aspect evidence by
...
- graph touch descriptor rows now store touched aspects as
  `ForgeQueryAspectTouch` values instead of raw touched aspect path strings.
- graph read touch shapes now require `ForgeQueryAspectTouch` values at
...
- declared mutation graph touch descriptors and graph obligation aspect
  selectors now admit touched aspect paths through `ForgeQueryAspectTouch`
...
- `ForgeQueryDerivedView` now stores dependency and produced aspects as
  `ForgeQueryAspectTouch` values instead of raw string vectors.
- `ForgeQueryDerivedView::new` and `ForgeQueryDerivedView::produces` now
  require `ForgeQueryAspectTouch` values, so callers cannot construct a
...
- `ForgeQueryAspectValueTemplate` now stores its aspect identity as
  `ForgeQueryAspectTouch` instead of retaining a raw aspect path string.
- `ForgeQueryWriteCommandTemplate::UpdateAspect` now stores an admitted
  `ForgeQueryAspectTouch`; binding projects path text only when constructing
  the already-native `ForgeQueryAspectValue`.
- `ForgeQueryAspectValueTemplate::new` and
  `ForgeQueryTypedPort::with_required_aspect` now require
  `ForgeQueryAspectTouch`, so program command template and typed port
...
- `ForgeQueryEntity` now stores external projection path values as native
  `AspectValue` and derives JSON only through terminal row projection helpers.
- `ForgeQueryEntity` terminal JSON row projection helpers are now cfg-test
...
- `ForgeQueryRetainedMaterializedRow` is now the retained proof carrier for
...
- `ForgeQueryDerivedMaterializationResult` and
  `ForgeQueryDerivedViewMaterialization` now store retained materialized rows
...
- `ForgeQueryDerivedPatch` no longer stores raw `serde_json::Value` as its
  payload field.
- derived patch constructors now require `ForgeQueryDerivedPatchPayload`, so
  callers must supply an explicit native payload proof instead of passing raw
  JSON into the stronger patch proof state.
- `ForgeQueryDerivedPatchPayload` now stores private native payload variants:
...
- derived materialization execution now constructs
  `ForgeQueryDerivedMaterializationResult` from retained materialized rows
...
- shared-read published artifact generation also binds retained
  ...
  consumers.
- `ForgeQueryDerivedMaterializationResult` production construction no longer
...
- `ForgeQueryRetainedMaterializedRow` now has a native
  `from_scalar_values` constructor over foundational `AspectValue` maps, with
  private storage preserved.
- `ForgeQueryDerivedViewMaterialization` exposes native row publication
...
- `ForgeQueryMutationMetadata` no longer stores metadata entries as
  `serde_json::Value`.
- metadata values are retained as `ForgeQueryMutationMetadataValue`, a private
...
- `ForgeQueryBackendAdmissibleMutation` no longer retains a whole
  `ForgeQueryWriteCommand` behind the admitted proof wrapper.
...
- The old command reconstruction/read path is gone from the proof state:
  ...
  call the crate-local admission constructor, and cannot forge the private
...
- Write-authority execution receipt routing was split into its own adapter
  ...
  Rust line cap.
- Verification for this slice: `cargo fmt -p forge-query`,
  `cargo check -p forge-query --tests`, normal `cargo test -p forge-query
...
- Bridge writeback authority no longer lowers admitted Query mutations into a
  synthetic `forge.query.writeback` scalar marker.
- Added a crate-private `ForgeQueryBridgeWritebackEffectIntent` proof that
  accepts only `ForgeQueryBackendAdmissibleMutation` and lowers it into a
...
- program operation inputs, outputs, and value-expression literals now retain
  `ForgeQueryProgramValue` instead of storing raw `serde_json::Value`.
- `ForgeQueryProgramValue` now retains a private native value tree rather than
...
- value-expression evaluation assembles object and array results from retained
  ...
  them.
- public `ForgeQueryValueExpr::literal`, `ForgeQueryOperationInput::new`, and
  `ForgeQueryOperationOutput::new` require `ForgeQueryProgramValue`, so public
  program authoring cannot promote raw JSON into the retained program-value
  proof state.
- runtime live-read program outputs now build `ForgeQueryProgramValue` rows
...
- `ForgeQueryProgramTrace` records bound input names from the retained program
  value map rather than from a raw JSON input map.
- `ForgeQueryPortType::TerminalJson` remains a terminal program-port compatibility
...
- directly coupled cfg-test program fixtures now construct
  `ForgeQueryProgramValue` for operation inputs and value-expression literals
...
- `tests/aspect_native_query_compile_fail.rs` now mechanically proves that raw
  ...
  payload fields cannot be forged through public struct literals.
...
- `ForgeQueryIntentDeclaration` now retains `ForgeQueryIntentInput` instead
  of storing raw `serde_json::Value` directly.
- `ForgeQueryIntentInput` now retains a private native input tree rather than
  a named `serde_json::Value` wrapper.
...
- directly coupled production/certification fixtures now build object-shaped
  `ForgeQueryIntentInput` values instead of `json!` payloads.
...
- `cargo check -p forge-query` mechanically proves production code no longer
...
- `ForgeQueryAspectValue` and `ForgeQueryDesiredAspectValue` terminal JSON
...
- `ForgeQueryExistingTruthProbeField::terminal_json_projection_string` is now
...
- Intent-admission effect certification fixtures now declare live views and
  effects with `ForgeQueryNativeRow` handle markers instead of
  `serde_json::Value`.
...
- Runtime API stabilization transcript fixtures and aspect API finalization
  certification rows now use `ForgeQueryNativeRow` for live, computed, and
  effect handles instead of `serde_json::Value`.
- Consumer-kit in-memory backend behavior tests now use `ForgeQueryNativeRow`
  live-view markers instead of `serde_json::Value`, so even src-hosted test
  surfaces demonstrate the native read handle path.
- `cargo check -p forge-query` plus the handle-marker scan mechanically prove
...
- Dead cfg-test terminal JSON mutators on
  `ForgeQueryDerivedViewMaterialization` were removed; retained-row native
...
- The remaining cfg-test
  `ForgeQueryDerivedMaterializationResult::new(Vec<serde_json::Value>, ...)`,
...
- Runtime-local retained materialization fixtures now construct
  `ForgeQueryRetainedMaterializedRow` values explicitly from
  `ForgeQueryRetainedFieldPath -> AspectValue` maps before creating a derived
...
- Derived artifact binding tests now use `ForgeQueryNativeRow` handle markers
  and decode into typed row structs instead of using `serde_json::Value`
  handles and JSON row assertions.
- Focused verification covered `cargo check -p forge-query`,
  `cargo test -p forge-query retained_scalar --lib`,
  `cargo test -p forge-query retained_live --lib`, and
  `cargo test -p forge-query decode_row --lib`; the deleted constructor scan
  for the old JSON-row materialization names returned no matches.

### Phase 3 retained upstream input ingress deletion slice

- `ForgeQueryRetainedUpstreamInputs::terminal_json_test_inputs` was removed;
...
- The retained materialized row compatibility admission function
  `ForgeQueryRetainedMaterializedRow::from_terminal_json_row` was deleted
...
- The supporting JSON-to-`AspectValue` scalar converter was removed, leaving
  ...
  `ForgeQueryRetainedFieldPath -> AspectValue` path and terminal JSON only as
...
- `ForgeQueryRetainedUpstreamInputs::terminal_json_computed_rows_projection`
...
- Focused verification covered `cargo check -p forge-query`,
  `cargo test -p forge-query retained_upstreams_decode_single_computed_rows_through_query_runtime_floor --lib`,
  `cargo test -p forge-query upstream --lib`, and
  `cargo test -p forge-query --test aspect_native_query_compile_fail`.

### Runtime test native handle marker slice

- Src-hosted runtime tests now use `ForgeQueryNativeRow` for live, maintained
...
- The handle-marker residue scans for `ForgeQueryLiveView<Value>`,
  `ForgeQueryDerivedViewHandle<Value>`, `ForgeQueryEffectHandle<Value>`,
  ...
  return no matches across `crates/forge-query/src` and
  `crates/forge-query/tests`.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo test -p forge-query --lib`, and `cargo test -p
...
- The aspect-native compile-fail suite now proves public callers cannot call
  terminal JSON projection accessors on `ForgeQueryDerivedPatchPayload`,
  `ForgeQueryDerivedPatch`, `ForgeQueryEffectPayload`, or
...
- Verification covered `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail` to create the new expected diagnostics,
  followed by normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail` and `cargo check -p forge-query`.

### Mutation delta native touch constructor fence slice

- `ForgeQueryMutationDelta::from_touched_aspects` is now the public mutation
  delta constructor; the former string-path `ForgeQueryMutationDelta::new`
...
- Runtime write receipts, computed patch conversion, certification fixtures,
  ...
  test authorities now build deltas from retained `ForgeQueryAspectTouch`
  values instead of round-tripping through dotted path strings.
- `ForgeQueryWriteCommand::declared_aspect_touches()` exposes the retained
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo test -p forge-query --lib`, `TRYBUILD=overwrite cargo
  test -p forge-query --test aspect_native_query_compile_fail`, normal `cargo
  test -p forge-query --test aspect_native_query_compile_fail`, and a residue
  scan showing the only `ForgeQueryMutationDelta::new` reference is the
  compile-fail fixture.

### Mutation delta terminal projection naming slice

- The neutral `ForgeQueryMutationDelta::aspect_paths()` alias was removed so
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo test -p forge-query --lib --no-run`,
  `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail`, normal `cargo test -p forge-query --test
...
- The neutral `ForgeQueryWriteCommand::declared_aspect_paths()` method and its
...
- The neutral `ForgeQueryLivePatch::aspect_paths()` alias was removed so live
...
- The neutral `ForgeQueryDerivedPatch::aspect_paths()` alias was removed so
...
- The neutral `ForgeQueryEffectDelivery::aspect_paths()` alias was removed so
...
- The neutral `ForgeQueryExistingTruthProbeRequest::aspect_paths()` alias was
...
- The neutral `ForgeQueryPreviewExecutionEvidence::aspect_paths()` alias was
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  only `ForgeQueryWriteCommand::touched_aspect_paths()` call is the compile-fail
  fixture.

### Retained refresh context touched path alias fence slice

- The neutral `ForgeQueryRetainedRefreshContext::touched_aspect_paths()` method
...
- The neutral `ForgeQueryVerifiedAssumptionSet::asserted_aspect_paths()` method
...
- The neutral `ForgeQueryBatchWriteReceipt::touched_aspect_paths()` method was
...
- The neutral
  `ForgeQueryBatchWriteReceiptInspection::touched_aspect_paths()` and
  `ForgeQueryBatchWriteComponentInspection::touched_aspect_paths()` methods
...
- The internal neutral
  `ForgeQueryGraphTouchDescriptorRow::touched_aspect_paths()` alias was removed
...
- No public compile-fail fixture was added because the row alias is crate-local
  ...
  internal use.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, and a focused residue scan confirming the removed neutral
...
- The neutral `ForgeQueryEffectPayload::input_aspects()`,
  `ForgeQueryEffectPayload::output_aspects()`, and
  `ForgeQueryEffectPayload::changed_aspects()` string accessors were removed so
...
- `ForgeQueryEffectPayload` now mirrors the effect declaration vocabulary:
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail`, normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, and a focused residue scan showing the
...
- The neutral `ForgeQueryDerivedView::dependency_aspects()` and
  `ForgeQueryDerivedView::produced_aspects()` string accessors were removed so
...
- Preview routing and derived maintainer test helpers now keep
  `ForgeQueryAspectTouch` values alive instead of projecting strings and
  re-admitting them.
- The neutral `ForgeQueryComputedInspectionEvidence::dependency_aspects()` and
  `ForgeQueryComputedInspectionEvidence::produced_aspects()` inspection aliases
...
- `ForgeQueryEffectInspectionEvidence` now stores trigger aspects as
  `ForgeQueryAspectTouch` values instead of terminal strings.
- The neutral `ForgeQueryEffectInspectionEvidence::trigger_aspects()` accessor
...
- `ForgeQueryEffectInspectionEvidence` now stores condition input/output
  aspects as `ForgeQueryAspectTouch` values instead of terminal strings.
...
- Denial, probe, and symbolic-resolution surfaces no longer expose dotted
  ...
  `ForgeQueryExistingTruthAssertionDenial::asserted_aspect_path()`,
  `ForgeQueryExistingTruthProbeDenial::probed_aspect_path()`,
  `ForgeQueryExistingTruthProbeField::aspect_path()`,
  `ForgeQuerySymbolicAspectResolutionEvidence::aspect_path()`, and
  `ForgeQueryGraphCompositionResolutionEntry::aspect_path()` were removed.
...
- The native/proof side remains intact: symbolic aspect resolution and graph
  composition resolution entries still expose `ForgeQueryAspectTouch` through
...
- Graph-read admitted proof rows and field-authority rows no longer expose
  ...
  aliases.
- `ForgeQueryGraphReadPredicateFieldAuthority`,
  `ForgeQueryGraphReadOrderingFieldAuthority`,
  ...
  `ForgeQueryAdmittedBooleanPredicateLeaf`, and
  `ForgeQueryBooleanPredicateSelectivityRow` now reserve authority access for
...
- `ForgeQueryRetainedScalarFieldFact` keeps native authority available through
  `field_path()` and exposes dotted reporting text only through
  `terminal_field_key_projection()`.
- `ForgeQueryRetainedScalarAlignmentFact` keeps native left/right retained
...
- `ForgeQueryAspectValue` no longer exposes its parsed mutation target through
...
- The aspect-native compile-fail suite now proves public callers cannot recover
  a dotted aspect path from `ForgeQueryAspectValue` through the removed neutral
  alias.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo test -p forge-query --lib --no-run`,
  `TRYBUILD=overwrite cargo test -p forge-query --test
...
- `ForgeQueryAspectMutationOperation` no longer exposes its native touch
  through the neutral `aspect_path()` alias.
- `ForgeQuerySymbolicAspectReference` now exposes `aspect_touch()` publicly for
...
- Graph obligation selector matching compares required touches through native
  `ForgeQueryAspectTouch` authority instead of re-checking declared operation
  coverage through dotted strings.
- The aspect-native compile-fail suite now proves public callers cannot recover
  a dotted aspect path from `ForgeQueryAspectMutationOperation` or
  `ForgeQuerySymbolicAspectReference` through the removed neutral aliases.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail`, normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, `cargo test -p forge-query --lib
...
- `ForgeQueryEntity::aspect_value` now requires a foundational `AspectKey`
  instead of accepting a raw string lookup.
- `ForgeQueryEntity::aspect_values` now iterates native `AspectKey` values
...
- `ForgeQueryEntity::external_scalar_value` now requires a foundational
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail`, normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, and `cargo test -p forge-query --lib
  --no-run`.
...
- `ProjectionFactFieldPath` now exposes its validated `CanonicalFieldPath`,
  ...
  `ForgeQueryEntity::external_scalar_value` path directly instead of reparsing
  dotted text.
- The aspect-native compile-fail suite now proves public callers cannot call
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail`, normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, and `cargo test -p forge-query
  --lib --no-run`.
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail`, and normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, and `cargo test -p forge-query
  --lib --no-run`.
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`,
  focused `cargo test -p forge-query live --lib`, focused `cargo test -p
  forge-query view_shape_live --lib`, line-count checks noting `live/mod.rs`
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, `cargo test -p
  forge-query --lib --no-run`, and focused scans showing the removed
...
- `ForgeQueryRuntime::read_derived(...)` was removed so runtime callers cannot
...
- Runtime callers now use `read_derived_result(...)` and must either propagate
  `ForgeQueryRuntimeError` or make the expectation explicit at test/assertion
  sites.
- The retained materialization carrier remains unchanged:
  `read_derived_result(...)` returns `ForgeQueryDerivedMaterializationResult`,
...
- `ForgeQueryRuntime::drain_derived_patches(...)` and
  `ForgeQueryWorkspace::observe_computed(...)` now require
  `&ForgeQueryDerivedViewHandle<_>` instead of raw derived view-name text.
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite cargo
  test -p forge-query --test aspect_native_query_compile_fail`, normal `cargo
  test -p forge-query --test aspect_native_query_compile_fail`, focused `cargo
  test -p forge-query computed --lib`, line-count checks, and focused scans
...
- `ForgeQueryWorkspace::materialize(...)` was removed so workspace callers
...
- The retained materialization carrier remains unchanged:
  `materialize_result(...)` returns `ForgeQueryDerivedMaterializationResult`
...
- `ForgeQueryExistingTruthProbe::field(&str)` was removed so public and runtime
  callers cannot reparse raw dotted aspect paths when reading verified existing
  truth.
- Probe field lookup now flows through
  `field_for_touch(&ForgeQueryAspectTouch)`, carrying the already-parsed native
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused runtime tests
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query policy_live`, and focused scans showing the removed
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  test -p forge-query --test aspect_native_query_compile_fail`, normal `cargo
  test -p forge-query --test aspect_native_query_compile_fail`, focused `cargo
  test -p forge-query policy_delivery`, line-count checks, and focused scans
...
- `ForgeQueryRetainedScalarFactSet::field_value(&str)` was removed so public
...
- Retained scalar fact-set lookups now flow through
  `field_value_at(&ForgeQueryRetainedFieldPath)`, keeping the admitted retained
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query retained_scalar_fact_set_reads_nested_fields`, file line-count
...
- `ForgeQueryDerivedMaterializationBundle::includes_view_name(&str)` and
  `ForgeQueryLiveArtifactBundle::includes_view_name(&str)` were removed so
...
- Derived and live artifact bundle containment now flows through
  `includes_target(...)` with `ForgeQueryDerivedMaterializationTarget` or
  `ForgeQueryLiveArtifactTarget`, preserving the admitted target carrier
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, line-count checks, and
...
- `ForgeQueryGraphTouchDescriptor::touches_aspect_path(&str)` was removed so
...
- Descriptor containment now flows through
  `touches_aspect(&ForgeQueryAspectTouch)`, comparing retained touched aspects
...
- Graph obligation touch selector matching now passes its retained
  `ForgeQueryAspectTouch` directly into descriptor containment instead of
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query graph_touch_descriptor --lib`, focused `cargo test -p
  forge-query touch_descriptor --lib`, line-count checks, and focused scans
...
- `ForgeQueryGraphTouchDescriptor::touches_collection(&str)` is no longer a
  public descriptor lookup surface.
- Public descriptor collection containment now flows through
  `touches_target_collection(&ForgeQueryMutationTargetCollectionIdentity)`, so
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query touch_descriptor --lib`, focused `cargo test -p forge-query
...
- Added `ForgeQueryMutationMetadataKey` as the admitted key token for mutation
  metadata lookup.
- `ForgeQueryMutationMetadata::get(...)` now requires
  `&ForgeQueryMutationMetadataKey` instead of `&str`, so public receipt,
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  --lib`, focused `cargo test -p forge-query delete_with --lib`, focused
...
- `ForgeQueryMutationMetadata::entries()` no longer exposes
  `&BTreeMap<String, ForgeQueryMutationMetadataValue>` as a public raw storage
  map.
- Public metadata iteration now yields
  `(ForgeQueryMutationMetadataKey, &ForgeQueryMutationMetadataValue)` so
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  `cargo test -p forge-query delete_with --lib`, focused `cargo test -p
  forge-query
  refresh_fallback_maintainer_receives_retained_mutation_metadata --lib`,
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`,
  focused `cargo test -p forge-query declarative --lib`, focused `cargo test
  -p forge-query read_composition --lib`, line-count checks noting
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`,
  focused `cargo test -p forge-query declarative --lib`, line-count checks
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`,
  focused `cargo test -p forge-query writeback --lib`, focused `cargo test
  -p forge-query declarative --lib`, line-count checks noting
  `declarative_live.rs` is already allowlisted, and focused scans confirming
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`,
  focused `cargo test -p forge-query authoring --lib`, focused `cargo test -p
  forge-query read_composition --lib`, line-count checks for the touched
...
- `ForgeQueryDerivedMaterializationBundle::materialization_by_name(...)`,
  `ForgeQueryDerivedArtifactBinding::materialization_by_name(...)`,
  `ForgeQueryLiveArtifactBundle::read_by_name(...)`, and
  `ForgeQueryLiveArtifactBinding::read_by_name(...)` are no longer public
  facade shortcuts.
- Public callers must use typed derived handles or live views through
  `materialization(&ForgeQueryDerivedViewHandle<_>)` and
  `read(&ForgeQueryLiveView<_>)`; raw by-name lookup remains crate-private for
...
- `ForgeQueryRetainedUpstreamInputs::live_rows(&str)` and
  `ForgeQueryRetainedUpstreamInputs::retained_computed_rows(&str)` are no
  longer public maintainer-facing shortcuts.
- Public maintainer access now flows through typed handles with
  `live_rows_for(&ForgeQueryLiveView<_>)` and
  `retained_computed_rows_for(&ForgeQueryDerivedViewHandle<_>)`, so external
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query retained_upstream --lib`, focused `cargo test -p forge-query
...
- `ForgeQueryWorkspace::read_live_by_name(...)` was removed so public callers
  cannot execute live reads from arbitrary raw view-name text.
- `ForgeQueryWorkspace::state_live_by_name(...)`,
...
- Public workspace live access now flows through typed live-view handles:
  `read_live_result(&ForgeQueryLiveView<_>)`,
  `state_live(&ForgeQueryLiveView<_>)`,
  `subscription_basis_digest(&ForgeQueryLiveView<_>)`, and
  `inspect_live(&ForgeQueryLiveView<_>)`.
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query read_obligation_dispatch --lib`, focused `cargo test -p
  forge-query in_memory_test_runtime_executes_public_insert_and_live_read
...
- `ForgeQueryDerivedArtifactBinding::consume_scalar_fields_by_name(...)` and
...
- Public retained scalar extraction and alignment now require typed derived
  view handles through `consume_scalar_fields(&ForgeQueryDerivedViewHandle<_>,
  ...)` and `verify_scalar_alignment(&ForgeQueryDerivedViewHandle<_>,
  &ForgeQueryDerivedViewHandle<_>, ...)`.
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query retained_scalar_fact_set --lib`, focused `cargo test -p
  forge-query retained_scalar_alignment --lib`, line-count checks, and focused
...
- `ForgeQueryAspectTouch` no longer exposes the neutral public
  `aspect_path()` alias.
...
- The aspect-native compile-fail suite now proves facade callers cannot use the
  removed `ForgeQueryAspectTouch::aspect_path()` alias.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query graph_touch_descriptor --lib`, focused `cargo
  test -p forge-query aspect_crud --lib`, line-count checks, and focused scans
  showing `aspect_path()` survives only in compile-fail fixtures.

### Graph touch selector native constructor naming fence slice

- `ForgeQueryGraphTouchSelector::aspect_path(...)` was renamed to
  `aspect_touch(...)` because the constructor takes an admitted
  `ForgeQueryAspectTouch`, not raw dotted aspect-path authority.
...
- Graph obligation support-matrix, lookup-selection, registration selector,
  ...
  constructor name.
- The aspect-native compile-fail suite now proves facade callers cannot use the
  removed `ForgeQueryGraphTouchSelector::aspect_path(...)` constructor.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query selector_matching --lib`, focused `cargo test -p
  forge-query graph_touch_descriptor --lib`, focused `cargo test -p
  forge-query lookup_selection --lib`, line-count checks, and focused scans
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query validation --lib`, focused `cargo test -p
  forge-query schema_view --lib`, line-count checks, and focused scans showing
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query traversal --lib`, focused `cargo test -p
  forge-query read_composition --lib`, and focused scans showing the old
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query validation --lib`, focused `cargo test -p
  forge-query read_composition --lib`, line-count checks, and focused scans
  showing the old validated traversal relation alias survives only in the
  compile-fail fixture.

### Graph-read domain operation relation proof slice

- `ForgeQueryAdmittedGraphReadDomainOperationReference` now stores a
  `RelationName` proof carrier instead of raw relation-name `String` text.
- `ForgeQueryGraphReadOperationRegistration` and admitted domain registered
...
- The aspect-native compile-fail suite now proves facade callers cannot use
  the removed `ForgeQueryAdmittedGraphReadRelation::relation()` string alias.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query --tests`, `TRYBUILD=overwrite cargo test -p forge-query --test
  aspect_native_query_compile_fail`, normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query --test
  graph_read_access_phase_one`, line-count checks, and focused scans.

### Aspect value constructor and JSON-number validation slice

- `ForgeQueryAspectValue::new_set(...)` and `new_clear(...)` are now
...
- The aspect-native compile-fail suite now proves facade callers cannot call the
  direct `ForgeQueryAspectValue::new_set(...)` constructor while the older
...
- A mechanical `serde_json` dependency-demotion probe moved `serde_json` out of
  normal forge-query dependencies during `cargo check -p forge-query`. That red
  state exposed two in-scope native-value validations and two remaining
  consumer-kit support document contracts.
- `ForgeQueryProgramValue::decimal_text(...)` and
  `ForgeQueryIntentInput::decimal_text(...)` now validate JSON-number text with
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite
  cargo test -p forge-query --test aspect_native_query_compile_fail`, normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, and the
...
- `ForgeQueryRetainedMaterializedRow::from_scalar_values(...)` is now
...
- Public maintainer/test authoring now uses
  `ForgeQueryRetainedMaterializedRowBuilder`, which admits native
  `ForgeQueryRetainedFieldPath` plus foundational `AspectValue` pairs and then
  consumes the builder through `try_build()`.
- A deliberate `cargo check -p forge-query --tests` red probe showed the only
...
- `ProjectMaterializedFacts` and
  ...
  removed.
- The deliberate red probe first broke production `cargo check -p forge-query`
  on seven certification/report fixture call sites. After migrating those to
  terminal request names, production went green; `cargo check -p forge-query
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  test -p forge-query --test aspect_native_query_compile_fail`, normal `cargo
  test -p forge-query --test aspect_native_query_compile_fail`, focused `cargo
  test -p forge-query projection_consumption --lib`, line-count checks for the
...
- `cargo check -p forge-query --tests` then exposed saved-query, view-shape,
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  test -p forge-query --test aspect_native_query_compile_fail`, normal `cargo
  test -p forge-query --test aspect_native_query_compile_fail`, focused `cargo
  test -p forge-query view_shape --lib`, line-count checks, and focused scans
...
- `ForgeQueryDerivedMaterializationTarget::new(...)` and
  `ForgeQueryLiveArtifactTarget::new(...)` are no longer public facade
...
- Public construction now flows through the typed routes:
  `From<&ForgeQueryDerivedViewHandle<_>>` for derived materialization targets
  and `From<&ForgeQueryLiveView<_>>` for live artifact targets.
...
- The deliberate red probe kept production `cargo check -p forge-query` green
  while `cargo check -p forge-query --tests` exposed the retained-live support
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query retained_live --lib`, line-count checks, and focused scans
...
- `ForgeQueryAspectValue` no longer exposes test-visible
  ...
  projection helper.
- `ForgeQueryExistingTruthProbeField` no longer exposes
...
- The deliberate red probe first kept production `cargo check -p forge-query`
  green, then `cargo check -p forge-query --tests` exposed the bridge support
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query probe_existing --lib`, focused `cargo test -p forge-query
  bridge_backed_verification_execution --lib`, focused `cargo test -p
  forge-query verified_existing --lib`, focused `cargo test -p forge-query
...
- `ForgeQueryProgramValue`, `ForgeQueryOperationInput`, and
  `ForgeQueryOperationOutput` no longer expose crate-test-only terminal JSON
...
- Program values now expose small native read-only inspection routes:
  ...
  JSON indexing.
- The deliberate red probe with `cargo check -p forge-query --tests` exposed
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query compiled_typed_program --lib`, focused `cargo test -p
  forge-query sandboxed_preview_run_operation --lib`, line-count allowlist
...
- Added native fail-closed retained-row accessors:
  `ForgeQueryDerivedMaterializationResult::single_retained_row()` and
  `ForgeQueryRetainedUpstreamInputs::single_retained_computed_row(...)`.
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query retained_upstreams --lib`, focused `cargo test -p forge-query
  derived_materialization_bundle --lib`, focused `cargo test -p forge-query
  row_pair_uses_bound --lib`, focused `cargo test -p forge-query
  row_triple_uses_bound --lib`, focused `cargo test -p forge-query
  materialize_intent_common_path_helper --lib`, focused `cargo test -p
  forge-query batch_write_retained_artifact --lib`, line-count checks, and
...
- `ForgeQueryEffectPayload` and `ForgeQueryEffectDelivery` no longer expose
  crate-test-only `terminal_json_payload_projection()` helpers. Runtime effect
  delivery remains native through `ForgeQueryEffectPayload`,
  `ForgeQueryAspectTouch`, and explicit terminal aspect projections for
  reporting.
- `ForgeQueryEntity` no longer exposes crate-test-only
...
- Runtime tests now inspect entity rows through native helpers over
  ...
  semantics.
- The deliberate `cargo check -p forge-query --tests` red probe exposed the
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query aspect_crud --lib`, focused `cargo test -p forge-query
  graph_composition --lib`, focused `cargo test -p forge-query
  sandboxed_preview_run_operation --lib`, focused `cargo test -p forge-query
  preview_promotion --lib`, focused `cargo test -p forge-query basis_context
  --lib`, focused `cargo test -p forge-query symbolic_reference --lib`, and
...
- The deliberate `cargo check -p forge-query --tests` red probe produced 74
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test
  -p forge-query computed --lib`, focused `cargo test -p forge-query effect
  --lib`, focused `cargo test -p forge-query effect_delivery --lib`, focused
  `cargo test -p forge-query preview_promotion --lib`, line-count checks, and
...
- Stateful bridge writes now apply admitted `ForgeQueryAspectValue` values into
...
- The public bridge runtime support used by integration tests now follows the
  ...
  into `ForgeQueryEntity`, and public existing-truth seed/probe/assertion state
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test
  -p forge-query bridge_backed_verification_execution --lib`, focused `cargo
  test -p forge-query probe_existing --lib`, focused `cargo test -p
  forge-query live --lib`, focused `cargo test -p forge-query --test
  public_bridge_runtime_bootstrap`, focused `cargo test -p forge-query --test
  graph_composition_public_bridge`, focused `cargo test -p forge-query --test
...
- The adapter seed API now requires `AspectValue` directly, and mismatch
  ...
  response.
- The deliberate `cargo check -p forge-query --tests` red probe exposed 16
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query bridge_backed_verification_execution --lib`, focused `cargo test
  -p forge-query graph_composition_edge_split --lib`, focused `cargo test -p
  forge-query graph_composition_verified_existing --lib`, broad filtered
  `cargo test -p forge-query graph_composition --lib`, line-count checks, and
...
- Computed runtime tests now assert retained materialization and derived patch
  ...
  vocabulary aligned with the `ForgeQueryRetainedMaterializedRow` +
  `ForgeQueryRetainedFieldPath` + `AspectValue` proof carriers.
- The deliberate red probe with `cargo check -p forge-query --tests` exposed
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test
  -p forge-query computed --lib`, line-count allowlist confirmation for the
...
- The parity assertion now uses typed `MutationReceiptSummary` and
  ...
  delta touch shape is retained as `Vec<ForgeQueryAspectTouch>` instead of
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test
  -p forge-query phase_nine --lib`, line-count checks, and focused scans
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test
  -p forge-query support_snapshot --lib`, focused `cargo test -p forge-query
...
- `ForgeQueryMutationDelta` no longer exposes
  `terminal_aspect_paths_projection()`. Public receipt consumers must keep
  delta evidence as native `ForgeQueryAspectTouch` values through
...
- Production/reporting call sites that truly need terminal text now project
  locally from `&[ForgeQueryAspectTouch]`: equivalence reports, write-receipt
...
- Runtime tests and test maintainers now compare or propagate native touches
  ...
  re-admitting with `ForgeQueryAspectTouch::new(...)`; receipt assertions use
  `test_aspect_touches(...)` slices.
- Added an aspect-native trybuild fixture proving facade callers cannot call
  the removed `ForgeQueryMutationDelta::terminal_aspect_paths_projection()`
  method.
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite`
  and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
...
- `ForgeQueryDerivedPatch` no longer exposes
  `terminal_aspect_paths_projection()`. Public computed-patch consumers must
  keep changed/produced aspect evidence as native `ForgeQueryAspectTouch`
  values through `aspect_touches()`.
- The deliberate root break left production `cargo check -p forge-query`
  green, proving core computed execution was already retaining native touches.
  `cargo check -p forge-query --tests` then exposed the remaining five
...
- Added an aspect-native trybuild fixture proving facade callers cannot call
  the removed `ForgeQueryDerivedPatch::terminal_aspect_paths_projection()`
...
- `ForgeQueryEffectDelivery` no longer exposes
...
- Effect inspection identity construction likewise projects terminal path text
  ...
  native `ForgeQueryAspectTouch` slices with `test_aspect_touches(...)`.
- Added an aspect-native trybuild fixture proving facade callers cannot call
  the removed `ForgeQueryEffectDelivery::terminal_aspect_paths_projection()`
...
- `ForgeQueryPreviewExecutionEvidence` no longer exposes
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  --lib`, focused `cargo test -p forge-query
...
- `ForgeQueryExistingTruthProbeRequest` no longer exposes
  `terminal_aspect_paths_projection()`. Public existing-truth probe consumers
  must retain requested aspects as native `ForgeQueryAspectTouch` values
...
- Added an aspect-native trybuild fixture proving facade callers cannot call
  the removed
  `ForgeQueryExistingTruthProbeRequest::terminal_aspect_paths_projection()`
  method.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite`
  and normal `cargo test -p forge-query --test
...
- `ForgeQueryLivePatch` no longer exposes
...
- Production `cargo check -p forge-query` stayed green after the break, showing
  ...
  boundaries.
- Added an aspect-native trybuild fixture proving facade callers cannot call
  the removed `ForgeQueryLivePatch::terminal_aspect_paths_projection()`
  method.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite`
  and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query live
...
- Deleted the unused
  `ForgeQueryGraphReadTouchShape::terminal_aspect_paths_projection()` helper.
  Graph read touch shape evidence already exposes native `ForgeQueryAspectTouch`
...
- This was an internal dead-helper cleanup rather than a facade boundary, so no
  ...
  remains under `crates/forge-query/src`, apart from local `pub(super)` terminal
  projection helpers scoped to effect/preview boundary modules.
- Verification covered `cargo check -p forge-query`, `cargo check -p
  forge-query --tests`, line-count checks, and focused scans for remaining
...
- Removed public terminal touched/asserted aspect-path projection helpers from
  ...
  expose native `ForgeQueryAspectTouch` values through `touched_aspects()`,
...
- The deliberate production red probe with `cargo check -p forge-query`
...
- The deliberate test red probe with `cargo check -p forge-query --tests`
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused batch,
  ...
  terminal projection methods remain under `crates/forge-query/src`.
...
- Removed public terminal dependency/produced aspect-list projections from
  `ForgeQueryComputedInspectionEvidence` and `ForgeQueryDerivedView`, public
  terminal input/output/changed projections from `ForgeQueryEffectPayload` and
  `ForgeQueryEffectExpression`, public trigger aspect projection from
  `ForgeQueryEffectTrigger`, and the dead graph touch descriptor row terminal
  declared-operation projection. These surfaces already expose native
  `ForgeQueryAspectTouch` or `ForgeQueryAspectMutationOperation` accessors;
...
- The deliberate test red probe exposed five computed/intent assertions that
  ...
  now compare native `ForgeQueryAspectTouch` slices with
  `test_aspect_touches(...)`.
- Added aspect-native trybuild fixtures proving facade callers cannot call the
...
- Removed public terminal trigger, condition-input, and condition-output
  aspect-list projections from `ForgeQueryEffectInspectionEvidence`. Effect
  inspection evidence now exposes native `ForgeQueryAspectTouch` slices through
...
- Production `cargo check -p forge-query` stayed green after the break,
...
- Removed public terminal field-key projection helpers from retained scalar
  ...
  native `ForgeQueryRetainedFieldPath` carriers, so facade callers must keep
...
- Removed public `terminal_aspect_path_projection()` helpers from
  `ForgeQuerySymbolicAspectResolutionEvidence` and
  `ForgeQueryGraphCompositionResolutionEntry`. Both surfaces already expose
  native `ForgeQueryAspectTouch` carriers, so facade callers must retain the
...
- Removed public `terminal_aspect_path_projection()` from
  `ForgeQueryAspectMutationOperation`. Mutation operations already expose their
  native `ForgeQueryAspectTouch`, so facade callers must keep the operation
...
- Removed public `terminal_aspect_path_projection()` from
  `ForgeQueryAspectValue`. Aspect values already expose native
  `ForgeQueryAspectTouch` through `aspect_touch()`, so callers must keep the
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  survives only in the compile-fail fixture for `ForgeQueryAspectValue`.

### Symbolic aspect reference terminal projection removal slice

- Removed public `terminal_aspect_path_projection()` from
  `ForgeQuerySymbolicAspectReference`. Symbolic aspect references already
  expose their native `ForgeQueryAspectTouch`, so callers must retain the
...
- Added native `aspect_touch()` access to `ForgeQueryExistingTruthProbeField`
...
- Removed public terminal aspect-path projection helpers from
  `ForgeQueryExistingTruthAssertionDenial` and
  `ForgeQueryExistingTruthProbeDenial`. Denials still retain typed internal
...
- The deliberate `cargo check -p forge-query --tests` red exposed seven
  ...
  now compare `ForgeQueryAspectTouch` values.
...
- Replaced terminal aspect/field storage on
  `ForgeQueryBooleanExpressionAdmissionError` and
  `ForgeQueryGraphReadSchemaReferenceAdmissionError` with native `AspectKey`
...
- Removed public `ForgeQueryRetainedFieldPath::from_authoring_path(...)` and
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  policy_delivery --lib`, focused `cargo test -p forge-query milestone_nine
...
- Removed public `ForgeQueryAspectTouch::terminal_aspect_path_projection()`.
...
- Added an aspect-native trybuild fixture proving facade callers cannot call
  the removed `ForgeQueryAspectTouch::terminal_aspect_path_projection()`.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite`
  and normal `cargo test -p forge-query --test
...
- Removed public terminal relation projection helpers from
  ...
  `ForgeQueryAdmittedGraphReadDomainOperationReference`, and
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  traversal --lib`, focused `cargo test -p forge-query --test
  graph_read_access_phase_one`, focused `cargo test -p forge-query
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  -p forge-query projection_consumption --lib`, line-count checks, and focused
...
- The deliberate compiler break kept `cargo check -p forge-query` green and
  used `cargo check -p forge-query --tests` to expose 24 test/certification
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  test -p forge-query --test phase_boundaries_compile_fail`, focused `cargo
  test -p forge-query view_shape --lib`, focused `cargo test -p forge-query
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  validation --lib`, focused `cargo test -p forge-query collection --lib`,
...
- Removed the crate-wide `ForgeQueryAspectTouch::terminal_projection_for_boundary()`
...
- Added native `ForgeQueryAspectTouch::matches_or_contains(...)` proof-state
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, `TRYBUILD=overwrite`
  and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query effect
  --lib`, focused `cargo test -p forge-query computed --lib`, focused `cargo
  test -p forge-query graph_composition --lib`, line-count checks, and a
...
- `ForgeQueryMemoryWorkspace::field_patch_from_aspect_values(...)` no longer
  projects an admitted `ForgeQueryAspectValue` back to terminal text in order to
  rediscover declared aspect authority. It now carries the value's
  `ForgeQueryAspectTouch` into declaration matching and relational field-patch
  construction.
- Declared memory-workspace aspects match requested mutations through native
  `ForgeQueryAspectTouch` equality/containment over foundational aspect keys and
...
- Added a memory-workspace regression proving that a declaration
  ...
  evidence.
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, `cargo check -p forge-query --tests`, focused `cargo test -p
  forge-query memory_workspace --lib`, focused `cargo test -p forge-query
  probe_existing --lib`, focused `cargo test -p forge-query verified_existing
...
- Changed `ForgeQueryExistingTruthAssertionDenial::new(...)` so an
  aspect-specific denial must receive `Option<ForgeQueryAspectTouch>` instead
...
- Changed `ForgeQueryExistingTruthProbeDenial::new(...)` the same way, so
...
- The deliberate `cargo check -p forge-query --tests` red exposed public bridge
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  forge-query`, deliberate red and final green `cargo check -p forge-query
  --tests`, `TRYBUILD=overwrite` and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  probe_existing --lib`, focused `cargo test -p forge-query verify_existing
  --lib`, focused `cargo test -p forge-query stop_class --lib`, focused `cargo
  test -p forge-query --test public_bridge_runtime_bootstrap`, line-count
...
- Changed graph obligation touch lookup keys so aspect selectors use
  `ForgeQueryAspectTouch` and declared aspect operations use
  `ForgeQueryAspectMutationOperation` instead of terminal dotted-string lookup
  keys.
- Removed public `ForgeQueryGraphTouchSelector::selector_value()` and
...
- Added an internal `ForgeQueryGraphTouchSelectorClass` proof-state classifier
...
- Verification covered `cargo fmt -p forge-query`, `cargo check -p
  ...
  and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  graph_obligation --lib`, focused `cargo test -p forge-query
...
- Removed public `ForgeQueryDerivedPatchPayload::from_scalar_value(...)` and
...
- Deliberately removed `ForgeQueryRetainedFieldPath::from_terminal_ingress(...)`
  before migrating callers. The red `cargo check -p forge-query --tests` pass
...
- Retained binding extraction now converts a requested `ProjectionFactFieldPath`
  directly into `ForgeQueryRetainedFieldPath` through its canonical field path,
...
- Added an aspect-native trybuild fixture proving facade callers cannot call
  ...
  `ForgeQueryRetainedFieldPath::from_terminal_ingress(...)` was crate-local
...
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, final green `cargo check -p
  forge-query`, `TRYBUILD=overwrite` and normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  projection_consumption --lib`, focused `cargo test -p forge-query
...
- The deliberate `cargo check -p forge-query --tests` red exposed authorized
...
- Added an aspect-native trybuild fixture proving facade callers cannot mint
  ...
  constructors.
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, green `cargo check -p
  forge-query`, `TRYBUILD=overwrite` and normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  authorized_projection --lib`, focused `cargo test -p forge-query
  policy_live --lib`, focused `cargo test -p forge-query
  projection_consumption --lib`, line-count checks, and focused scans proving
...
- The deliberate red `cargo check -p forge-query --tests` exposed policy-live
...
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, `TRYBUILD=overwrite` and normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query policy_live --lib`, focused `cargo test -p
  forge-query milestone_nine_certification --lib`, line-count checks, and
...
- Renamed public `ForgeQueryPortType::TerminalJson` to
  `ForgeQueryPortType::ProgramValue`, keeping program port language tied to the
...
- The first deliberate red exposed remaining `DeclarativeWritebackValue::String`
  ...
  produced a re-export red (`ForgeQueryIntentInput` still being exported through
  `declaration`) before the final green pass.
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, green `cargo check -p
  forge-query`, `TRYBUILD=overwrite` and normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  declarative --lib`, focused `cargo test -p forge-query program --lib`,
...
- `ForgeQueryAspect` now stores its external projection path as a
  `CanonicalFieldPath` at construction time instead of retaining terminal
  dotted text as the internal authority source.
- Added a crate-local native constructor for `ForgeQueryAspect` so internal
...
- `ForgeQueryTestBackendSchema` now stores external projection paths as
  ...
  `ForgeQueryAspect` without a second terminal ingress.
...
- Added aspect-native trybuild fixtures proving facade callers cannot treat
  `ForgeQueryAspect::external_projection_path()` or
  `ForgeQueryTestBackendSchema::aspects()` projection paths as `&str`.
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, `TRYBUILD=overwrite` and normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test
  -p forge-query memory_workspace --lib`, focused `cargo test -p forge-query
...
- This is a Law 41 storage-authority hardening slice. It intentionally keeps
  public string authoring at `ForgeQueryAspect::new(...)`, but once accepted,
...
- Added `ForgeQueryParsedAspectTarget::from_native_parts(...)` and
  `ForgeQueryAspectTouch::from_native_parts(...)` so native callers can build
...
- Runtime read obligation dispatch now lowers declarative projection,
  predicate, and ordering fields into native `ForgeQueryAspectTouch` values
...
- Graph touch descriptor inventory now stores and counts declared/touched
  aspect sets as `ForgeQueryAspectTouch` and
  `ForgeQueryAspectMutationOperation` instead of retaining
  `BTreeSet<String>` path/operation projections.
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, focused `cargo test -p forge-query
  read_obligation_dispatch --lib`, focused `cargo test -p forge-query preview
  --lib`, focused `cargo test -p forge-query graph_touch_descriptor --lib`,
...
- Preview session routing now keeps `live_affected` and `computed_affected`
  maps as `BTreeMap<String, Vec<ForgeQueryAspectTouch>>` instead of projecting
...
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, focused `cargo test -p forge-query live --lib`,
  focused `cargo test -p forge-query preview --lib`, attempted focused
  `cargo test -p forge-query live_subscription --lib` (0 matching tests), line
...
- Computed inspection identity helpers now accept
  `&[ForgeQueryAspectTouch]` for dependency aspects, produced aspects, and
...
- Effect inspection identity now maps trigger, condition input/output, and
  pending delivery touch slices directly from native `ForgeQueryAspectTouch`
...
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, focused `cargo test -p forge-query computed --lib`,
  focused `cargo test -p forge-query effect --lib`, line-count checks, and
...
- Write, batch, and intent receipts now retain affected live and derived view
  evidence as `ForgeQueryLiveArtifactTarget` and
  `ForgeQueryDerivedMaterializationTarget` values instead of raw
  `Vec<String>` storage.
- Backend/source adapter `affected_live_view_ids(...)` remains a terminal
  ...
  through the runtime-only `ForgeQueryLiveArtifactTarget::from_view_name(...)`
...
- Verification covered a deliberate red then green `cargo check -p
  forge-query --tests`, `TRYBUILD=overwrite` and normal `cargo test -p
  forge-query --test aspect_native_query_compile_fail`, focused live mutation
...
- Changed public `ForgeQueryLiveViewBuilder::select(...)` and
...
- Workspace live-view build still emits terminal field text for delivered
  ...
  request authority.
- The deliberate red `cargo check -p forge-query --tests` exposed 226 builder
...
- Verification covered `cargo fmt -p forge-query`, deliberate red then green
  `cargo check -p forge-query --tests`, green `cargo check -p forge-query`,
  `TRYBUILD=overwrite` and normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  workspace_declaration --lib`, focused `cargo test -p forge-query --test
...
- Exported `AspectName` and `FieldName` through the facade beside
  `RelationName`, and updated `forge_query_schema!` to mint the native name
  carriers at macro expansion before constructing schema evidence.
- The deliberate red `cargo check -p forge-query --tests` exposed 129 schema
...
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  ...
  forge-query`, `TRYBUILD=overwrite` and normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  schema_view --lib`, focused `cargo test -p forge-query workspace_declaration
...
- Removed the crate-local `visibility_for_parts(...)` lookup path. Authorized
  ...
  authority.
- The deliberate red `cargo check -p forge-query --tests` exposed 32 remaining
...
- Write receipt committed-truth identity now derives committed delta
  descriptors from `ForgeQueryAspectTouch::native_digest_part()` instead of
...
- Batch write receipt identity now encodes touched aspects from native
  `ForgeQueryAspectTouch` values through a scoped evidence helper. The helper
...
- Unified batch-write inspection digest inputs now carry
  `&[ForgeQueryAspectTouch]` for touched aspects instead of
...
- Verification covered `cargo fmt -p forge-query`, red and green
  `cargo check -p forge-query --tests`, green `cargo check -p forge-query`,
  focused `cargo test -p forge-query batch_write --lib`, focused `cargo test
  -p forge-query aspect_api_finalization --lib`, line-count checks, and a
...
- Changed the crate-local `ForgeQueryWorkspace::probe_existing(...)` shortcut
  so it accepts `ForgeQueryAspectTouch` values directly instead of admitting an
  arbitrary iterator of terminal aspect-path strings and minting proof inside
  the shortcut.
- `ForgeQueryExistingTruthProbeRequest` already retained native touches; its
  request digest now encodes `ForgeQueryAspectTouch::native_digest_part()`
...
- Intent-admission routing certification no longer reprojects a native probe
  ...
  shortcut.
- The deliberate red `cargo check -p forge-query --tests` exposed the remaining
...
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, green `cargo check -p
  forge-query`, focused `cargo test -p forge-query probe_existing --lib`,
  focused `cargo test -p forge-query intent_admission --lib`, touched-file
...
- Narrowed `ForgeQueryAspectTouch::aspect_path_text_for_boundary()` from
...
- The deliberate red `cargo check -p forge-query --tests` exposed seven
...
- Evidence/reporting consumers now encode `ForgeQueryAspectTouch` with
...
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, green `cargo check -p
  forge-query`, focused `cargo test -p forge-query memory_workspace --lib`,
  focused `cargo test -p forge-query consumer_kit::test_backend --lib`,
  focused `cargo test -p forge-query intent_admission --lib`, touched-file
...
- Narrowed `ForgeQueryAspectTouch::aspect_path_text_for_boundary()` again from
...
- The deliberate red `cargo check -p forge-query --tests` exposed 25
...
- Evidence, inspection, graph-composition, verified-assumption, and metadata
  consumers now use `ForgeQueryAspectTouch::native_digest_part()` instead of
...
- Stateful bridge test support stopped using terminal aspect text as its
  ...
  `CanonicalFieldPath` directly from `ForgeQueryAspectTouch` native aspect and
  field-path parts.
- Graph-composition resolution assertion helpers now compare expected fixture
  aspect labels by first admitting them into `ForgeQueryAspectTouch` and then
  comparing native digest material, preserving readable test fixtures without
  preserving terminal strings as the proof value.
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, green `cargo check -p
  forge-query`, focused `cargo test -p forge-query computed --lib`, focused
  `cargo test -p forge-query effect --lib`, focused `cargo test -p forge-query
  batch_write --lib`, focused `cargo test -p forge-query graph_composition
...
- Graph-obligation selector identity now encodes `ForgeQueryAspectTouch`
...
- The graph-touch descriptor compile-fail fixture was updated so it continues
  ...
  field.
- Verification covered `cargo fmt -p forge-query`, deliberate red and final
  green `cargo check -p forge-query --tests`, focused `cargo test -p
  forge-query graph_obligation --lib`, focused `cargo test -p forge-query
  graph_composition --lib`, focused `cargo test -p forge-query
  read_obligation_dispatch --lib`, `cargo test -p forge-query --test
...
- Mutation declared-aspect aggregate evidence now writes `aspect_touch` fields
  from `ForgeQueryAspectTouch::native_digest_part()` instead of retaining
...
- The internal denied-aspect helper was renamed from
  `ForgeQueryDeniedAspectPath` / `denied_aspect_path` to
  `ForgeQueryDeniedAspectTouch` / `denied_aspect_touch`.
...
- Most importantly, deleting the last production consumers exposed that
  `ForgeQueryAspectTouch::aspect_path_text_for_boundary()` and
  `ForgeQueryParsedAspectTarget::authoring_path_projection()` were no longer
  needed. Both methods were removed, and `ForgeQueryParsedAspectTarget` no
...
- The aspect-native compile-fail suite was updated for the private-field
  ...
  unavailable.
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, focused `cargo test -p forge-query probe_existing
  --lib`, focused `cargo test -p forge-query existing_truth --lib`, focused
  `cargo test -p forge-query mixed_authority --lib`, focused `cargo test -p
  forge-query graph_obligation --lib`, `cargo test -p forge-query --test
...
- `ForgeQueryVerifiedAssumptionSet` continues to retain
  `ForgeQueryAspectTouch` values for asserted aspects, and its digest evidence
...
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, focused `cargo test -p forge-query graph_composition
  --lib`, focused `cargo test -p forge-query existing_truth --lib`, focused
...
- Broke the next broad root deliberately: `ForgeQueryDeclarationAspectContract`,
...
- The deliberate `cargo check -p forge-query --tests` red exposed 66
...
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, green plain `cargo check -p forge-query`, focused
  `cargo test -p forge-query declaration_aspect --lib`, focused `cargo test -p
  forge-query declaration_publication --lib`, focused `cargo test -p
  forge-query declaration_route_plan --lib`, focused `cargo test -p
  forge-query declaration_receipt --lib`, focused `cargo test -p forge-query
  declaration_signal_compatibility --lib`, focused `cargo test -p forge-query
  declaration_legality --lib`, focused `cargo test -p forge-query
  declaration_progression --lib`, focused `cargo test -p forge-query
  declaration_entry_orchestration --lib`, focused `cargo test -p forge-query
  declaration_entry_seam --lib`, focused `cargo test -p forge-query
  declaration_relational_routing --lib`, focused `cargo test -p forge-query
...
- Closed the retained-scalar compatibility hole left by the terminal ingress
  quarantine slice: `ForgeQueryDerivedArtifactBinding::consume_scalar_fields(...)`
  now requires `ForgeQueryRetainedFieldPath` values instead of raw field-path
  strings.
- `ForgeQueryDerivedArtifactBinding::verify_scalar_alignment(...)` now requires
  pairs of `ForgeQueryRetainedFieldPath` values, so alignment cannot be requested
...
- Deliberate red `cargo check -p forge-query --tests` exposed retained scalar
...
- Verification: `cargo fmt -p forge-query`, trybuild overwrite plus normal
  ...
  forge-query`, touched-file line counts, and the earlier deliberate red
  `cargo check -p forge-query --tests` pass captured in
  `target/aspect-native-retained-scalar-red.log`.

## Query live-read execution installation proof fence

- Removed the crate-local `ForgeQueryRuntime::execute_live_read_by_name(...)`
  execution shortcut. Direct live reads now execute through
  `execute_live_read_for_installation(...)`, consuming
  `ForgeQueryRuntimeLiveSubscriptionInstallation` proof state instead of
  reconstituting read authority from a terminal view-name string.
- `ForgeQueryRuntime::read_live_result(...)` and canonical live-read receipt
  tests now flow through the live view handle's subscription installation.
- Live artifact targets constructed from `&ForgeQueryLiveView<_>` now retain
...
- Deliberate red `cargo check -p forge-query --tests` exposed the remaining
...
- The existing artifact-target compile-fail fixture now proves
  `ForgeQueryLiveArtifactTarget::new("...")` is absent rather than merely
  private, strengthening the raw-name construction fence.
- Verification: `cargo fmt -p forge-query`, deliberate red then green
  `cargo check -p forge-query --tests`, trybuild overwrite plus normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query live_receipt --lib`, focused `cargo test -p
  forge-query live_artifact --lib`, focused `cargo test -p forge-query
...
- `ForgeQueryAspect` now stores a native `ForgeQueryAspectTouch` plus
...
- `ForgeQueryAspect::new(...)` now requires those native carriers directly.
...
- Kept terminal authoring-string parsing behind the explicitly named
  crate-local `ForgeQueryAspectTouch::from_authoring_path(...)` boundary for
...
- The deliberate red `cargo check -p forge-query --tests` exposed the expected
...
- Added aspect-native trybuild coverage proving facade callers cannot call
  `ForgeQueryAspectTouch::new("title.value")`, while existing compile-fail
...
- Verification: `cargo fmt -p forge-query`, deliberate red then green
  ...
  focused residue scan proving `ForgeQueryAspectTouch::new` survives only in
...
- Added public read-only `source_field_key()` accessors so downstream code can
  ...
  accessor pairs.
- The deliberate `cargo check -p forge-query --tests` red exposed 59 real
...
- Closed the retained-upstream proof hole where
  `ForgeQueryRetainedUpstreamInputs::new(...)` publicly minted retained input
...
- Public retained-upstream access now requires either a typed live/derived view
  ...
  upstream declared by the `ForgeQueryDerivedView`.
...
- Deliberate red `cargo check -p forge-query --tests` exposed retained-upstream
...
- Removed public terminal aspect-field projection helpers from
  `ForgeQueryDeclarationAspectPublication` and
  `ForgeQueryGroupedAspectParticipationSummary`. Public consumers now keep the
...
- Verification: `cargo fmt -p forge-query`, deliberate red then green
  ...
  trybuild overwrite plus normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  declaration_route_plan --lib`, focused `cargo test -p forge-query grouped
...
- Removed the free retained-row builder surface entirely and narrowed
  `ForgeQueryDerivedPatchPayload::from_retained_row(s)` plus
  `ForgeQueryDerivedViewMaterialization::{replace_retained_rows,push_retained_row}`
...
- Added scoped maintainer transitions:
  ...
  `ForgeQueryDerivedPatchPayload::from_retained_scalar_values`. External
...
- The deliberate red `cargo check -p forge-query --tests` exposed the public
...
- Converted the memory-workspace entity row authority from a sidecar
  ...
  values. `ForgeQueryAspect::external_projection_path()` remains the declared
  schema/import mapping, but materialized `ForgeQueryEntity` rows no longer
...
- The deliberate red `cargo check -p forge-query --tests` exposed real
...
- Verification: `cargo fmt -p forge-query`, green `cargo check -p
  forge-query --tests`, green `cargo check -p forge-query`, normal `cargo test
  -p forge-query --test aspect_native_query_compile_fail`, focused
...
- Strengthened `ForgeQueryBackendAdmissibleMutation` from a thin unwrap-capable
...
- Changed `ForgeQueryRuntimeWriteAuthorityAdapter::write`,
  `write_batch`, bridge mutation authority construction, and write-authority
  execution receipt construction to accept `ForgeQueryBackendAdmissibleMutation`
  instead of raw authored commands.
- The deliberate red `cargo check -p forge-query --tests` exposed exactly the
...
- Added aspect-native compile-fail coverage proving facade callers cannot call
  ...
  and cannot construct `ForgeQueryBackendAdmissibleMutation` by struct literal.
- Verification: `cargo fmt -p forge-query`, deliberate red then green
  `cargo check -p forge-query --tests`, normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, focused write-authority drift and
  signal-invalidation receipt tests, `public_bridge_runtime_bootstrap`,
...
- Marked every `ForgeQueryWriteCommand` struct variant as `#[non_exhaustive]`.
  ...
  with enum variant literals such as `ForgeQueryWriteCommand::Delete { ... }`.
- This preserves `ForgeQueryWriteCommand` as an observable lower-level command
...
- Verification covered `cargo fmt -p forge-query`, green `cargo check -p
  ...
  forge-query --test aspect_native_query_compile_fail`, focused `cargo test -p
  forge-query mutation::aspect_crud --lib`, focused `cargo test -p
  forge-query runtime_write --lib`, and touched-file line counts. The regular
...
- Changed the public
  ...
  contract so adapters return only `Result<(), ForgeQueryExistingTruthAssertionDenial>`.
  External verification adapters can now accept or deny native aspect values,
  but they no longer mint `ForgeQueryVerifiedExistingTruthAssertion`.
- Moved the verified assertion transition into
  `ForgeQueryBridgeBackedRuntimeBackend`. After the adapter accepts the binding
...
- Verification covered deliberate red then green `cargo check -p forge-query
  --tests`, `cargo fmt -p forge-query`, normal `cargo test -p forge-query
  --test aspect_native_query_compile_fail`, focused `cargo test -p forge-query
  verify_existing --lib`, focused `cargo test -p forge-query probe_existing
...
- Removed public raw-name declared-upstream lookup from
  `ForgeQueryRetainedUpstreamInputs`. Maintainers can no longer ask for
...
- Added handle-shaped public lookups:
  ...
  specific upstream must carry the `ForgeQueryLiveView` or
  `ForgeQueryDerivedViewHandle` it declared against.
...
- Verification covered deliberate red then green `cargo check -p forge-query
  ...
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query retained_upstreams --lib`, focused
  `cargo test -p forge-query derived_materialization_bundle --lib`, and scans
...
- Added typed target iteration/accessors:
  `ForgeQueryDerivedMaterializationBundle::targets()`,
  `ForgeQueryLiveArtifactBundle::targets()`,
  `ForgeQueryDerivedArtifactBinding::targets()`, and
  `ForgeQueryLiveArtifactBinding::targets()`. Authority-bearing consumers now
  carry `ForgeQueryDerivedMaterializationTarget` or
  `ForgeQueryLiveArtifactTarget` proof carriers.
...
- Removed raw-name binding lookup from `ForgeQueryLiveArtifactBinding` and
  `ForgeQueryDerivedArtifactBinding`. The compiler exposed a deeper retained
  scalar consumer after the derived binding lookup was removed; scalar fact
  extraction and scalar alignment now thread
  `ForgeQueryDerivedMaterializationTarget` internally instead of hopping
...
- Verification covered deliberate red then green `cargo check -p forge-query
  --tests`, `cargo fmt -p forge-query`, trybuild overwrite plus normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  `cargo test -p forge-query derived_materialization_bundle --lib`, focused
  `cargo test -p forge-query post_write_artifact --lib`, focused
  `cargo test -p forge-query retained_scalar --lib`, focused
  `cargo test -p forge-query projection_consumption --lib`, final green
  `cargo check -p forge-query --tests`, and touched-file line counts. The only
...
- Removed public collection string authority from
  `ForgeQueryWriteCommand` and `ForgeQueryBackendAdmissibleMutation`.
...
- Added `declared_collection_identity()` returning
  `ForgeQueryMutationTargetCollectionIdentity`. Admission seeds, mutation
...
- Verification covered deliberate red then green `cargo check -p forge-query
  ...
  `cargo test -p forge-query --test public_bridge_runtime_bootstrap`, scans
...
- Converted retained scalar fact-set and scalar-alignment proof objects from
  retained view-name strings to `ForgeQueryDerivedMaterializationTarget`
  carriers. `ForgeQueryRetainedScalarFactSet` now exposes `target()`, and
  `ForgeQueryRetainedScalarAlignment` exposes `left_target()` and
  `right_target()`.
- Narrowed `ForgeQueryDerivedMaterializationTarget::view_name()` and
  `ForgeQueryLiveArtifactTarget::view_name()` to crate-internal visibility.
...
- Verification covered green `cargo check -p forge-query --tests`,
  `cargo fmt -p forge-query`, trybuild overwrite plus normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`,
  focused `cargo test -p forge-query retained_scalar --lib`, final green
  `cargo check -p forge-query --tests`, removed-alias scans, and touched-file
  line counts.
...
- Converted `ForgeQueryWriteReceipt` declared/target collection state from
  `Option<String>` storage and neutral public string accessors to
  `ForgeQueryMutationTargetCollectionIdentity` proof carriers.
...
- Verification covered deliberate red then green `cargo check -p forge-query
  --tests`, `cargo fmt -p forge-query`, trybuild overwrite plus normal
  `cargo test -p forge-query --test aspect_native_query_compile_fail`,
  focused `cargo test -p forge-query mutation::aspect_crud --lib`, focused
  `cargo test -p forge-query mutation::aspect_preview --lib`,
  `cargo test -p forge-query --test public_submission_lane_replacements`,
  final green `cargo check -p forge-query --tests`, removed-alias scans, and
  touched-file line counts. Focused `cargo test -p forge-query
...
- Added backend receipt coverage:
  ...
  proves a `ForgeQueryWriteCommand::Delete` admitted mutation builds bridge
  authority with an authoritative-patch canonical basis containing no set/clear
  operations.
- Verification covered `cargo fmt -p forge-query`, focused
  `cargo test -p forge-query bridge_writeback_effect_intent --lib`, focused
  `cargo test -p forge-query mutation::delete --lib`, final green
  `cargo check -p forge-query --tests`, a scan proving the removed empty-patch
...
- Removed the public neutral `target_collection()` string alias from
  `ForgeQueryExistingTruthTargetBinding`. The binding now exposes
...
- Migrated backend mutation authority, backend-admissible declared-collection
  ...
  to either carry `ForgeQueryMutationTargetCollectionIdentity` directly or make
...
- Verification covered `cargo fmt -p forge-query`, green
  ...
  `cargo test -p forge-query --test aspect_native_query_compile_fail`, focused
  green `cargo test -p forge-query existing_truth --lib`, removed-alias scans,
  and touched-file line counts.
...
- Deliberately broke broad roots before fixing consumers, to expose real
  downstream reliance through `cargo check -p forge-query --tests` instead of
...
- Removed neutral command/backend-admissible native bag aliases:
  ...
  `ForgeQueryWriteCommand` and `ForgeQueryBackendAdmissibleMutation`. The
...
- Verification covered `cargo fmt -p forge-query`, deliberate red
  `cargo check -p forge-query --tests` passes at 12/70/143 reported errors,
  green final `cargo check -p forge-query --tests`, trybuild overwrite plus
  green normal `cargo test -p forge-query --test
  aspect_native_query_compile_fail`, focused green
  `cargo test -p forge-query mutation::aspect_crud --lib`, green
  `cargo test -p forge-query --test public_bridge_runtime_bootstrap`, removed

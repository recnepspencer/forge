...
>
> **Next milestone:** [Milestone 9.3.5](./forge_query_roadmap.md#milestone-935-intent-admission-decision-lattice-and-decision-trace)
...
- `forge_query_vision.md`: aspect projection is explicit, materialization is
...
- `forge_query_roadmap.md`: 9.3.4 sits after basis lifecycle and effect
...
- `forge-query` owns projection-consumption declarations, eligibility, contract
  binding, typed consumed fact sets, public receipts/envelopes, support
  metadata, DX helpers, and certification.
- `forge-relational` remains authoritative for truth identities, relation
  topology, snapshot meaning, commit history, and branch/head semantics.
- `forge-runtime-bridge` remains authoritative for preview/writeback lifecycle,
  bridge-owned route/effect evidence, and any lower-runtime writeback artifacts
  referenced by consumed facts.
- `forge-signal` may contribute downstream invalidation or explanation evidence,
  but it must not become the canonical source of consumed projection facts.
- `forge-store` remains deferred for persisted consumed-fact receipts, durable
...
`forge-query` is the owning crate for Milestone 9.3.4. It must add:

- a new `projection_consumption` subdomain under `crates/forge-query/src`
- `ProjectionConsumptionDeclaration`,
...
`forge-query` must also tighten existing public surfaces so 9.3.4 does not rely
on raw payload bags as its final public contract:

- `ForgeQueryDerivedViewMaterialization` may remain an internal/source-facing
...
- any new common-path projection-consumption APIs must return the 9.3.4 proof
  chain, not raw `Vec<Value>`, raw `Vec<String>`, or ad hoc `ForgeQueryEntity`
  parsing helpers

### `forge-runtime-bridge`: required changes

`forge-runtime-bridge` must remain the owner of bridge truth-view
...
- Query must consume existing bridge writeback authority artifacts already
  reachable through `ForgeQueryWriteReceipt` provenance/evidence instead of
...
`forge-relational` must remain the owner of authoritative row identity, field
...
- add one explicit facade-level grouped/topology authority artifact or accessor
  in `forge-relational` rather than rebuilding that authority in Query
...
`forge-signal` is not an owner of consumed projection fact authority in 9.3.4.

Required signal boundary for this milestone:

- no new source-of-truth or consumed-fact APIs move into `forge-signal`
...
- raw `ForgeQueryDerivedViewMaterialization` rows as a public source family
...
- `forge-query`: add declaration authoring and source-family selection APIs in
  the new `projection_consumption/declaration` subdomain and export them
  through the facade
- `forge-runtime-bridge`: no new declaration surface; Query declarations that
  target bridge-backed sources must reference existing bridge source artifacts
  only
- `forge-relational`: no new declaration surface; relational meaning stays
  behind existing authoritative row-set/grouped-projection artifacts
- `forge-signal`: no changes
...
- `forge-query`: implement eligibility, warning-bearing admission, denial,
  deferred, and
  source-mismatch proof families in `projection_consumption/eligibility`
- `forge-runtime-bridge`: no new eligibility logic; bridge support is consumed
  through existing source/writeback artifacts and their current facade reach
- `forge-relational`: no new eligibility logic; relational authority is
...
- `forge-query`: implement contract binding in
  ...
  public surfaces
- `forge-runtime-bridge`: if Query cannot bind one bridge-backed source family
...
- `forge-relational`: if Query cannot bind the first grouped/topology hostile
...
- `forge-query`: implement fact extraction adapters in
  ...
  - admitted relational row-set/grouped sources that are already facade
    reachable
- `forge-runtime-bridge`: no Query-shaped fact extraction types; only existing
  bridge source artifacts or one narrowly added accessor if Phase 3 proved a
  public reachability gap
- `forge-relational`: no Query-shaped fact extraction types; only existing
...
- `forge-query`: implement receipt/envelope shaping, DX helpers, target
  transcripts, and inspection support in `projection_consumption/receipts` and
  `projection_consumption/dx`
- `forge-runtime-bridge`: no new receipt surface; Query receipts may reference
  bridge source/writeback identities but do not replace bridge receipts
- `forge-relational`: no new Query receipt surface; relational stays
...
- `forge-query`: implement inventories, support matrices, proof-shape audits,
  public-boundary audits, certification bundles, compile-fail fixtures, and
  hostile topology/Worth certification
- `forge-runtime-bridge`: no certification ownership change; bridge remains a
  source/oracle input to Query certification where applicable
- `forge-relational`: no certification ownership change; relational remains an
...
- `forge-runtime-bridge`, `forge-relational`, and `forge-signal` do not become
...
Runtime-backed projection consumption is not blocked on `forge-store`.

...
If `forge-query` does not own those distinctions explicitly:
...
- `forge_query_vision.md`: branch-scoped reads, time-travel, diff, lineage, and
...
- `forge_query_roadmap.md`: Milestone 5.4 exists specifically to prove
...
- historical evaluation authority remains in lower runtimes; `forge-query`
...
- compile-fail or privacy hardening proving correspondence and historical-path
  proof artifacts cannot be forged externally
...
Milestone 5.4 is complete only when `forge-query` can prove:
...
`forge-query` owns the typed query-facing descriptors, metadata, denials,

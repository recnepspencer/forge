...
Make Forge Store reject damaged physical bytes before they can become ambiguous
semantic failures.
...
- `forge-proof` progression vocabulary should be used where S.3 entry,
...
- `forge-proof` sealing patterns should be used where they prevent forged
...
- `forge-relational` owns semantic truth, transaction meaning, branch meaning,
  and logical decode semantics.
- `forge-foundational` standardizes boundary evidence meaning; it does not own
  page, frame, chunk, WAL, or manifest integrity authority.
- `forge-proof` standardizes proof-bearing progression law; it does not own

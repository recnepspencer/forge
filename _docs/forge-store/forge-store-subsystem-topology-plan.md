# Forge Store Subsystem Topology Plan

## Purpose

Give the entire Forge Store workspace one permanent, navigable domain topology
before performing broad mechanical moves.

This plan and `forge-store-topology-inventory.csv` are paired artifacts:
...
Forge Store is organized into seven dependency bands. These bands express
direction of knowledge; they are not ceremonial layers.

### Band 0: Durable Vocabulary

`forge-store-contracts` contains only dependency-neutral durable identities,
...
- `forge-store-budgets`
- `forge-store-claim-boundaries`
- `forge-store-modes`
- `forge-store-operations-vocabulary`
- `forge-store-security`

Physical encodings of their values belong in `forge-store-physical-format`.
...
- `forge-store-schema-lineage`
- `forge-store-reclaim-policy`
...
- `forge-store-subscription-support`
...
- `forge-store-readiness` owns only real process/startup admission that has a
  runtime consumer. Milestone completion and certification closeout do not
  belong here.
- `forge-store-extensions` owns extension declaration, admission, registry, and
  compatibility boundaries.
- `forge-store-analysis` owns durable analysis-lane contracts and execution.
- `forge-store` is the thin product facade and composition root.

### Band 6: Courtroom And Engineering Evidence

- `forge-store-certification` owns verdicts and acceptance programs.
- `forge-store-physical-certification` owns reusable adversarial simulation and
  physical harness machinery.
- `forge-store-formal-models` owns executable abstract models.
- `forge-store-test-support` owns inputs, fixtures, drivers, and assertions.
...
| --- | --- | --- |
| `forge-store` | keep thin | Public product facade and composition root. |
| `forge-store-analysis` | freeze until built | Durable analysis lanes, basis admission, and analysis artifact lifecycle. |
| `forge-store-aspect-native` | reorganize | Aspect-native authority, canonical basis, ingress readmission, terminal projection, and receipts. |
| `forge-store-authority` | reorganize | Current authority, source admission, retention, projection, and readmission. |
| `forge-store-blob-chunks` | preserve domains, remove scaffolding | Blob identity, chunking, lifecycle, publication, reachability, retention, streaming, export/import, and compaction. |
| `forge-store-branch-deltas` | grow by domain | Delta identity, layering, base sharing, reads, compaction, and publication. |
| `forge-store-bulk` | freeze until built | Bulk planning, admission, execution, checkpointing, and publication. |
| `forge-store-budgets` | reorganize | Resource units, envelopes, pre-execution admission, counters, and denials. |
| `forge-store-buffer-pool` | reorganize root | Residency, pinning, record access, dirty pages, eviction, allocation, and bounded background work. |
| `forge-store-certification` | major reorganization | Courtroom programs, evidence projections, scenarios, oracles, and verdicts by permanent subsystem. |
| `forge-store-claim-boundaries` | reorganize | Claim declarations, forbidden claims, classification, promotion, and accounting. |
| `forge-store-compatibility` | grow by domain | Version compatibility, migration bridges, legacy readmission, and disposition. |
| `forge-store-contracts` | shrink and reorganize | Lowest stable shared vocabulary only. |
| `forge-store-extensions` | preserve and sharpen | Extension declaration, admission, registry, posture, and compatibility. |
| `forge-store-formal-models` | freeze until built | Executable models and refinement checks. |
| `forge-store-io-scheduler` | reorganize | Resource accounting, foreground reservation, background pacing, queues, and backend dispatch. |
| `forge-store-layout-indexes` | major reorganization | Catalog, keyspace, strategies, materialization, access, maintenance, evolution, and integrity adaptation. |
| `forge-store-live-query` | grow by domain | Durable query basis, resume position, invalidation input, and observation. |
| `forge-store-maintenance` | reorganize | Maintenance planning, admission, scheduling, execution, and publication. |
| `forge-store-modes` | preserve | Durable, embedded, and absent lifecycle contracts. |
| `forge-store-offline-verifier` | reorganize | Offline discovery, bounded scanning, verification, findings, and reports. |
| `forge-store-operations` | reorganize | Backup/export/import, repair planning, quarantine coordination, and operator-facing execution outcomes. |
| `forge-store-operations-vocabulary` | shrink | Dependency-neutral operation declarations shared across owners. |
| `forge-store-physical-backend` | reorganize | Backend capabilities, access policy, durability ordering, placement observation, and physical execution. |
| `forge-store-physical-certification` | preserve engine, remove milestone topology | Actors, scenarios, schedules, faults, drivers, observations, oracles, coverage, and transcripts. |
| `forge-store-physical-format` | reorganize | Binary identity, framing, records, manifests, references, security encoding, and decode admission. |
| `forge-store-physical-integrity` | major root reorganization | Pre-decode admission, checksums, container/page/chunk/WAL integrity, quarantine, and scrub. |
| `forge-store-physical-isolation` | preserve domains, remove later-owner knowledge | Epochs, latches, hazards, read stability, compaction, publication, reclaim, and checkpoint interlocks. |
| `forge-store-readiness` | shrink aggressively | Real runtime composition/startup admission only. |
| `forge-store-reclaim-policy` | reorganize | Candidate classification, reachability, holds, admission, permits, and accounting. |
| `forge-store-recovery-physics` | reorganize | Entry admission, source precedence, replay, checkpoint cutover, publication recovery, and readmission. |
| `forge-store-replication` | freeze until built | Artifact publication, peer admission, transfer, verification, cursoring, and convergence evidence. |
| `forge-store-retention` | grow by domain | Retention declarations, holds, anchors, evaluation, and expiry decisions. |
| `forge-store-s0-reclassification` | retire after migration | Move permanent claim behavior to its owner; move closeout evidence to certification. |
| `forge-store-schema-lineage` | freeze until built | Durable schema boundaries, lineage records, lookup, and compatibility inputs. |
| `forge-store-security` | major root reorganization | Scope, authenticity, custody/readmission, trust boundaries, and durable security declarations. |
| `forge-store-snapshots` | grow by domain | Snapshot identity, materialization, publication, lookup, and restore inputs. |
| `forge-store-subscription-support` | freeze until built | Durable cursors, resume contracts, retention interaction, and compatibility. |
| `forge-store-test-support` | reorganize | Cross-crate worlds, fixtures, deterministic drivers, faults, and assertions only. |
| `forge-store-tiering` | reorganize | Tier posture, placement, transition admission, IO readiness, and observation. |
| `forge-store-wal` | reorganize | Record vocabulary, append, flush/durability, publication, topology, and recovery reads. |
...
`forge-store-aspect-native`, `forge-store-authority`, and
`forge-store-security` must lose physical-format imports. Their physical byte
...
forge-store-physical-backend/src/
  capability/
  access_policy/
  durability/
  placement/
...
forge-store-physical-integrity/src/
  admission/
    entry/
    physical_scope/
    pre_decode/
...
forge-store-physical-isolation/src/
  epoch/
  latch/
  hazard/
  generation/
...
```text
forge-store-recovery-physics/src/
  entry/
    declaration/
    admission/
  source_precedence/
...
forge-store-offline-verifier/src/
  discovery/
  scan/
  verification/
  findings/
...
```text
forge-store-claim-boundaries/src/
  declaration/
  classification/
  forbidden_claims/
  promotion/
...
forge-store-operations-vocabulary/src/
  backup/
  repair/
  replication/
  maintenance/
...
forge-store-reclaim-policy/src/
  candidate/
  reachability/
  holds/
  admission/
...
```text
forge-store-schema-lineage/src/
  schema_boundary/
  lineage_record/
  lookup/
...
forge-store-subscription-support/src/
  cursor/
  resume/
  retention_interlock/
  compatibility/
...
forge-store-physical-certification/src/
  actors/
  scenarios/
  schedules/
  faults/
...
`forge-store-readiness` is retained only for real runtime composition admission.
...
- every current non-ignored Forge Store file has exactly one CSV row;

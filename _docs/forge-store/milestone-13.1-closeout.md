...
and typed resume classification inside `forge-store`. It intentionally does not
...
| Durable publish/fetch | `ForgeStore::publish_subscription_support`, `ForgeStore::fetch_subscription_support` | persistence, local-file/sqlite reopen, direct lookup counter tests |
...
| Resume classification | `ForgeStore::classify_subscription_support_resume` | exact, degraded, drift, cross-family, rebuild, session-loss, and certification tests |
...
cargo test -p forge-store subscription_support_certification --lib
cargo test -p forge-store subscription_support --lib
...
- full `forge-store` suite: 522 tests passed plus compile-fail and doctest phases

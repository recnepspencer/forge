...
S.5.1 must use `forge-foundational` and `forge-proof` deliberately, not as
...
**Use `forge-foundational` for shared boundary meaning**

- use `forge_foundational::aspects()` for native aspect authoring, validation,
  authoritative aspect state admission, masks, and patches where security-scope
  facts cross an aspect-native Store boundary.
- use `forge_foundational::compatibility()` only for explicit JSON/readmission
  tests or terminal projection bridge debt; native S.5.1 authoring must not go
  through JSON.
- use `forge_foundational::canonicalization_api::{common_path, lower_lane,
...
- use `forge_foundational::boundary_evidence_api::{common_path, lower_lane,
...
- use `forge_foundational::profiles_api::{common_path, lower_lane,
...
- use `forge_foundational::performance_api::{common_path, lower_lane,
...
**Use `forge-proof` for progression law**

- import through `forge_proof::prelude::*` by default and use raw surfaces only
...
- `forge-store-security/src/scope_vocabulary.rs` owns key scope, key version,
  tenant scope, authenticity class, and custody posture terms.
- `forge-store-security/src/scope_admission.rs` owns sealed admission into
  current security-scope witnesses.
- `forge-store-security/src/authenticity_posture.rs` owns authenticity result
  categories separate from physical integrity.
- `forge-store-security/src/security_scope_counters.rs` owns exact admission,
  denial, drift, stale-key, unsupported, and authenticity counters.
- `forge-store-physical-format/src/security_metadata.rs` or a narrower
  existing physical-format module owns page/frame/manifest metadata carriage.
- `forge-store-wal/src/security_metadata.rs` owns WAL/checkpoint metadata
  carriage if WAL topology is separate enough to require its own module.
- `forge-store-blob-chunks/src/security_scope_readiness.rs` owns S.7 blob
  readiness seeds.
- `forge-store-operations` or `forge-store-offline-verifier` owns backup,
  export, and repair readiness seeds where those artifacts already live.
- `forge-store-readiness/src/s6_security_scope_readiness.rs` owns the S.6
  handoff that I/O QoS must consume.
- `forge-store-certification/src/s5_1_security_scope_harness/` owns courtroom
...
- `forge_foundational::aspects()` for native aspect contracts, values,
  validation, authoritative state admission, projection/mutation/diagnostic
  masks, and patch vocabulary.
- `forge_foundational::canonicalization_api::lower_lane::basis` and
  `forge_foundational::canonicalization_api::stronger_lane` for canonical basis
  readiness and readmission vocabulary.
- `forge_proof::prelude::*` for witness-authorized progression surfaces.
...
- Use `forge-proof` for legal progression and checked outcome topology; use
...
- `forge_proof::prelude::{recipe, AuthorityWitness, CapabilityWitness,
  ProofOutcome, ProofOutcomeKind}`
- `forge_proof::raw::{AssumptionBasis, TransitionOutcome}` when explicit
  freshness or transition nouns are clearer than the pleasant lane.
- `forge_foundational::boundary_evidence_api::lower_lane` for executed,
  completed, provenance, and support-truth receipt vocabulary after Store
  admission succeeds.
- `forge_foundational::canonicalization_api::lower_lane::basis` for reproducible
...
- `forge_foundational::aspects().authoritative_state()` for boundary state
  snapshots when security metadata is exposed as aspect-native evidence.
- `forge_foundational::canonicalization_api::common_path` and
...
- `forge_foundational::profiles_api::lower_lane::attachment` when attaching
...
- `forge_foundational::profiles_api::lower_lane::attachment` for diagnostic,
  compatibility, certification, or support posture on propagated metadata.
- `forge_foundational::boundary_evidence_api::lower_lane` for drift and
...
- `forge_proof::prelude::ProofOutcomeKind` or a locally named Store outcome
...
- `forge_foundational::boundary_evidence_api::lower_lane` for executed receipt,
  support-truth, degraded/unavailable posture, and closeout evidence.
- `forge_foundational::profiles_api::lower_lane::progression` for requested,
...
- `forge_foundational::canonicalization_api::lower_lane::comparison` for
  cross-scope dedupe equivalence/mismatch classification.
- `forge_foundational::boundary_evidence_api::common_path` and `lower_lane`
  for provenance, executed receipt, support-truth, degraded recovery, and
  readmission attachments on chunk readiness evidence.
- `forge_foundational::profiles_api::lower_lane::attachment` for retention,
  compatibility, certification, and support posture on blob/chunk artifacts.
- `forge_proof::prelude::{CanonicalVec, UniqueVec, non_empty, pair,
...
- `forge_foundational::profiles_api::lower_lane::attachment` for custody,
...
- `forge_foundational::boundary_evidence_api::lower_lane` for support-truth,
  degraded recovery, quarantine, and repair-read closeout evidence.
- `forge_foundational::profiles_api::lower_lane::attachment` for repair
...
- `forge_proof::prelude::{proof_flow, recipe, gate_ready, ready_now,
  join_ready, compose_ready}` for staged progression and ready-join handoff
  construction.
- `forge_foundational::boundary_evidence_api::lower_lane` for published
...
- S.5.1 readiness handoffs from Phase 9
- `forge_foundational::performance_api::common_path` for performance claim
  authoring and layout intent.
- `forge_foundational::performance_api::lower_lane::policy` for pre-execution
...
- `forge_foundational::performance_api::stronger_lane` for certified or
...
- External identity systems may provide admission evidence, but Store does not
  ...
  authority.
- `forge-proof` provides progression law and checked outcomes; Store lower

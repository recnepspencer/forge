...
Make Forge Store recover acknowledged durable physical effects exactly once in
...
- `forge-relational` owns semantic truth and transaction meaning.
- `forge-foundational` owns shared boundary evidence vocabulary, not recovery
  authority.
- `forge-proof` owns progression law, not WAL semantics.

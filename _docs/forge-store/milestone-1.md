...
Define one canonical durable artifact model for Forge Store and make it the
...
It is the milestone that decides whether `forge-store` becomes:

- a real durability authority layer for the Forge runtime, or
...
- `forge-runtime-bridge/milestone-1.md`
...
- Milestone 1 ships with one production-grade embedded backend baseline and one
  backend abstraction owned by `forge-store`
...
- the trait belongs to `forge-store`
...
impl ForgeStoreBuilder {
    ...
    pub fn build(self) -> Result<ForgeStore, StoreBuildError>;
}

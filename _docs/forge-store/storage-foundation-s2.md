...
Make Forge Store operate through a bounded physical buffer-pool regime where
...
- `forge_store_roadmap.md`
  ...
  replace `forge-relational` ownership of in-memory truth.
...
`workspaces/forge-store/crates/forge-store-buffer-pool/src/`
...
- `forge-foundational` performance and profile vocabulary
...
- `forge-store-budgets`
- `forge-store-certification`
- `forge-foundational` performance vocabulary
...
- `forge-store-physical-format`
- `forge-store-certification`
- `forge-foundational` layout vocabulary
...
- Shortcut rejection: a test that bypasses scenario-plan lowering, fabricates
  ...
  `forge-store-test-support` fails certification.
...
- Adversarial denial: over-budget page residency, pin leaks, dirty overflow,
  whole-store materialization, whole-object streaming, and forged view access
...
- persisted large-store fixtures and allocation sentinels owned by
  `forge-store-test-support`, with all proof meaning retained by
  `forge-store-certification`
...
- S.1 physical byte authority remains the only ordinary source for physical page
  and frame access.
- `forge-relational` continues to own semantic runtime memory, transaction
...
- `test_support_cannot_own_buffer_pool_certification_meaning`
  proves `forge-store-test-support` can supply mechanics but cannot own

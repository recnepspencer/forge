# Forge Store Milestone 12 Closeout Implementation Plan
...
- Compatibility catalog, manifest digest identities, declared edge registry,
  ...
  `crates/forge-store/src/compatibility/`.
- The certification runner in
  `crates/forge-store/src/compatibility/certification_runner.rs` is real and
...
- write `_docs/forge-store/milestone-12-closeout.md`
- update `_docs/forge-store/milestone-12.md` from draft to closed status with a
  closeout link
- update `_docs/forge-store/forge_store_roadmap.md` to mark Milestone 12 closed
...
cargo test -p forge-store compatibility_facade --lib
...
real publication behavior, and `_docs/forge-store/milestone-12-closeout.md`

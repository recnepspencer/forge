...
> **Status:** Closed via [milestone-2-closeout.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-store/milestone-2-closeout.md)
>
...
>
> **Prerequisite milestone:** [milestone-1.md](/Users/Esther/Documents/Programming/forge_workspace/forge/_docs/forge-store/milestone-1.md)
>
...
- Forge runtime can be hosted by store without store gaining semantic authority
...
- absent mode may still use shared libraries or helper types from the Forge
...
- `forge_relational_vision.md`
  The most important thing it protects is that `forge-relational` owns truth
...
- operating mode describes runtime lifecycle ownership and durability contract,
  ...
  hydrated
- Forge runtime semantics are always the truth engine in every admitted mode
- `forge-store` only sometimes owns the live runtime instance; when it does,
...
Durable mode is the deployment shape where `forge-store` owns a live internal
Forge runtime instance and owns the durable acknowledgment boundary around that
runtime.
...
Embedded mode is the deployment shape where the application owns the live Forge
runtime instance and `forge-store` acts as a persistence service for artifacts
...
Absent mode is the deployment shape where Forge runtime runs without
`forge-store` at all.
...
- absent mode must not be constructed through a required `forge-store` facade
  or builder
...
- the runtime must be constructible without `ForgeStoreFacade`
...
- runtime semantics remain owned by Forge runtime, not by store mode wrappers
...
- The most dangerous naive design here is one `ForgeStore` object plus a `mode`

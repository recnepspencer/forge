...
- `perf_laws.md` protects visible cost and memory honesty. S.7 must publish
  ...
  sneaks in.
- `forge_store_roadmap_2.md` protects the physical database foundation.
...
S.7 must use `forge-foundational` only where Store is crossing a shared
...
- `forge_foundational::aspects()` for aspect-native declarations at import,
  export, evidence, and support boundaries.
- `forge_foundational::compatibility().json()` only for hostile/readmission
  tests that prove JSON cannot become authority without native lowering.
- `forge_foundational::canonicalization()` plus
...
- `forge_foundational::boundary_evidence()` plus
  ...
  evidence.
- `forge_foundational::performance_api::{common_path, lower_lane,
...
- `forge_foundational::profiles()` and `forge_foundational::profiles_api` for
  profile, support posture, compatibility posture, materialization posture, and
  proof-bearing artifact profile attachment.

Required Proof surfaces:

- `use forge_proof::prelude::*;` for the ordinary S.7 progression lane.
...
- `forge_proof::raw::*` only when the pleasant lane would hide a real
...
- `forge_proof::prelude::*` with `recipe(...)`, `.resolve_with(...)`,
  `.lower_with(...)`, `.ready_with(...)`, `.execute()`, `ProofOutcomeKind`,
  `AuthorityWitness<_>`, and `CapabilityWitness<_>` for the phase progression
  skeleton
- `forge_foundational::boundary_evidence()` and
  `forge_foundational::boundary_evidence_api::lower_lane::receipts` for the
...
- Keep Store-specific lifecycle authority in `forge-store-blob-chunks`; use
  `forge-foundational` and `forge-proof` only for shared vocabulary and proof
  reporting.
...
- `forge_foundational::aspects()` for aspect-native security-scope evidence
...
- `forge_foundational::canonicalization_api::lower_lane::basis` for
  canonical security-scope basis entries used in evidence bundles
- `forge_proof::prelude::{AuthorityWitness, ProofOutcomeKind}` for
...
- `forge_foundational::canonicalization_api::lower_lane::{basis, comparison,
  digest}`
...
- `forge_proof::prelude::{pair, non_empty, join_ready}` for fixed-shape
...
- `prepare_canonical_basis_sequence(...)`,
  `prepare_canonical_basis_bundle(...)`, `derive_canonical_digest(...)`, and
  `compare_canonical_basis(...)`
- `forge_proof::prelude::{non_empty, join_ready, compose_ready}` for ordered
...
- `forge_proof::prelude::{recipe, ProofOutcomeKind, AuthorityWitness}`
...
- A semantic object reference from `forge-relational` may point at Store blob
...
- Decide whether initial `BlobObjectId` allocation lives in
  `forge-store-blob-chunks` directly or behind a narrower authority module.
...
- `forge_proof::prelude::{proof_flow, ProofOutcomeKind, join_ready}`
...
- `forge_foundational::profiles()` and
  `forge_foundational::profiles_api::{common_path, lower_lane,
...
- `forge_proof::prelude::{proof_flow, ProofOutcomeKind}` for scenario plan ->
...
- `forge_foundational::performance_api::lower_lane::{policy, receipts,
  reports}`
...
- `forge_proof::prelude::{ready_now, gate_ready, ProofOutcomeKind}` for
...
- `forge_proof::prelude::{join_ready, ProofOutcomeKind}` for chunk-tree plus
...
- `forge_foundational::boundary_evidence_api::lower_lane::{receipts, support}`
- `forge_proof::prelude::{ProofOutcomeKind, join_ready}`
...
- `forge_proof::prelude::{recipe, proof_flow, ProofOutcomeKind}`
- `.bridge_trust_boundary()`, `.rebind_with(...)`, and `.readmit_with(...)`
...
- `forge_proof::prelude::{pair, join_ready, compose_ready, ProofOutcomeKind}`
...
- `FoundationalBoundaryEvidenceAttachmentBundle`,
  `FoundationalBoundaryEvidenceExecutedReceiptArtifact`
- `forge_proof::prelude::{non_empty, ProofOutcomeKind}` for non-empty
...
- `forge_foundational::performance_api::lower_lane::receipts` for reclaim and
...
- `forge_foundational::performance_api::common_path` for layout intent claims
- `FoundationalLayoutIntentClaim`, `FoundationalPerformanceLayoutIntent`,
  ...
  `FoundationalCounterBackedPerformanceReceipt`
- `forge_proof::prelude::{recipe, ready_now, ProofOutcomeKind}` for placement
...
- `FoundationalCounterBackedPerformanceReceipt`
- `forge_proof::prelude::{join_ready, ProofOutcomeKind}` for lifecycle plus
  placement readiness
...
- `forge_foundational::performance_api::lower_lane::receipts`
- `forge_proof::prelude::{recipe, join_ready, ProofOutcomeKind}`
...
- `forge_foundational::aspects()` for native export declarations
- `forge_foundational::canonicalization_api::lower_lane::{export, digest}`
- `prepare_canonical_export_bundle(...)`,
  `CanonicalExportReadyArtifact`, and `CanonicalExportReadinessProofs`
- `forge_proof::prelude::{recipe, ProofOutcomeKind}`
...
- `forge_foundational::compatibility().json()` for hostile JSON readmission
  tests
...
- `forge_proof::prelude::{recipe, ProofOutcomeKind}` plus
  `.bridge_trust_boundary()`, `.rebind_with(...)`, and `.readmit_with(...)`
...
- `FoundationalBoundaryEvidenceAttachmentTarget`,
  ...
  `FoundationalBoundaryEvidenceExecutedReceiptArtifact`
- `forge_foundational::canonicalization_api::lower_lane::basis` for capsule
  canonical basis preparation
- `forge_proof::prelude::{non_empty, join_ready, compose_ready,
...
- Decide whether capsule readiness lives in `forge-store-blob-chunks` directly
  or in a narrow handoff module consumed by `forge-store-replication`.
...
- `forge_foundational::boundary_evidence_api::lower_lane::{attachments,
  receipts, support}`
- `forge_foundational::performance_api::lower_lane::{basis, receipts, reports}`
- `forge_proof::prelude::{proof_flow, ProofOutcomeKind, join_ready}` for
...
- `forge_foundational::profiles_api::lower_lane::{identity, materialization,
  certification}`
...
- `FoundationalCertifiedPerformanceBundle`,
  ...
  `FoundationalCounterBackedPerformanceReceipt`
- `forge_proof::prelude::{ready_now, ProofOutcomeKind}` for heavy-profile
...
- `forge_proof::prelude::{proof_flow, ProofOutcomeKind}` and
  `forge_proof::raw::*` only for closeout proof stages whose raw topology must
  stay visible
...
- Keep `forge-store-certification` as materializer and verifier; lower Store

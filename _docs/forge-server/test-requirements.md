# Forge Server Test Requirements
...
`forge-server` cannot be considered shipped merely because:

- an HTTP route returns a body
- a WebSocket sends frames
- a lease survives one reconnect
- a Forge-native client can fetch a view
...
- direct Forge-native product ergonomics without endpoint-glue reimplementation
...
> misuse of Forge-native facade surfaces, the server must preserve canonical
...
- prove that Forge-native facade ergonomics do not hide capability posture,
...
Broad high-level equality is specifically insufficient for `forge-server`.
...
Prove that every facade, Forge-native, and compatibility surface crosses the
...
### 2. Forge-Native No-Glue Equivalence Test

Purpose

Prove that a Forge-native application can consume server-managed Query meaning
...
- express the same ordinary product flow through:
  - direct Forge-native facade consumption
...
Forge-native ergonomics reduce glue without reducing semantic honesty.
...
meaning model and does not quietly diverge from the Forge-native facade.
...
Together, these tests prove that `forge-server` is:

- Query-first rather than route-local
- ergonomically direct for Forge-native apps rather than endpoint-glue-driven
...
No `forge-server` milestone should be considered closed until its named

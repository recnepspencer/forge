# Forge Server Future Roadmap

## Purpose

This document defines the future work for `forge-server`.
...
- `composition_laws.md`: request APIs, Forge-native facade surfaces, sync
...
- `perf_laws.md`: no cheap-looking surface may conceal broad scans, replay
  ...
  meaning.
- `forge_server_vision.md`: the server owns network delivery operations, not
...
`forge-server` must survive the hostile case where a naive server would:

- rebuild Query meaning as route-local endpoint folklore
- ship a Forge-native app surface that still requires handwritten API glue,
...
- `forge-relational` already owns authoritative truth, CDC, history, branches,
  and commit serialization
- `forge-runtime-bridge` Milestone 17 is closed for temporal/async basis,
...
- each milestone must consume Query, Bridge, Relational, and Store contracts
  ...
  network edge
- each milestone must make Forge-native ergonomics better, not worse
- each milestone must treat regulated-industry posture as a first-class
  ...
  in scope
- each milestone must keep request/response APIs, Forge-native facade
...
1. Forge-native direct-consumption surfaces
2. compatibility request/response APIs for non-Forge and explicit external
   integration consumers

That means the roadmap does not treat handwritten HTTP endpoints as the center
of gravity anymore. Forge-native facade work may come before or alongside
...
Create one typed server runtime front door so every future Forge-native facade,
...
- `forge-server` remains a facade over typed subsystem boundaries rather than a
  handler bag
...
- a typed Forge-native server facade for ordinary Query-backed product work
...
- typed client-consumable denial and support posture when a Forge-native path
  requests an unsupported capability
- enough direct-consumption shape that a Forge-native app can avoid large
  families of handwritten endpoints for ordinary live product work

### Must Preserve

- the Forge-native facade does not become a second Query runtime
...
- the same product meaning expressed through direct Forge-native consumption
...
- compatibility APIs stay visibly secondary to the Forge-native facade for
  ordinary Forge product development
...
`forge-server` is roadmap-complete only when:

- the Forge-native direct-consumption server surface is shipped

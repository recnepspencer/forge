...
**Status:** Open — Phases 1 and 2 proved; Phase 3 is next. The unrelated
repository-wide Rust line-cap baseline remains red and is recorded below.
**Historical policy:** Milestones 9.9, 9.10, and 9.15 and completed Milestone
9.16 rows retain their recorded status. This ledger adds stronger guarantees;
it does not reopen, revoke, or rewrite historical status or closure.
...
|---|---:|---|---|---|
| C1 | 1 | Every installed query and operation owns one sealed graph-obligation set with exhaustive kind, owner, selection, graph-read requirement, resource, effect, and terminal-evidence meaning. | **PROVED** | Installation authority tests, exhaustive owner matrix, schema/generation substitution denials, entry/byte budgets, and no public constructor. |
| C2 | 1 | Relational owns graph truth and exact mechanics, Runtime Bridge owns installed correspondence/lowering, Signal owns policy evidence, and Query owns only legal composition and progression. | **PROVED** | Owner-conformance tests, foreign-evidence hostility, dependency checks, and no recreation of lower decisions in Query. |
| C3 | 2 | There is one installed selection path and selection carries no execution authority. | **PROVED** | Public construction denial, selector mutation sensitivity, unsupported-owner denial, and zero selection-only execution success. |
| C4 | 2 | Every read and mutation uses one requirement, inventory, structural-cost, budget, capacity-reservation, and sealed-plan admission chain. | **PROVED** | Real Bank read/mutation transcripts, non-authority inspection, forged input/review denial, and exact admission counters. |
| C5 | 2 | One managed provider session begins before every decision-bearing graph read and binds runtime, generation, query/operation, principal/context, typed branch, branch-qualified basis, provider, run, and reservation. | **PROVED** | Session-before-observation trace, cross-affinity denials for every dimension, equal-version cross-branch substitution denial, and all facts carrying the exact branch and session identities. |
...
| C12 | 5 | Destination packages own the actual authority; no destination package imports or wraps the monolith and no compatibility path survives. | **OPEN** | Boundary checks, Cargo dependency inspection, visibility checks, and zero obsolete residue. |
...
| C14 | 5 | Public documentation and examples describe and compile against the single real facade and no removed consumer-kit or direct-planning workflow. | **OPEN** | AI README and named feature-doc audit plus executable examples. |
| C15 | 5 | Milestone 9.16 can resume at Runtime Phase 7.3 without a parallel selector, graph-read planner, authorization lane, or receipt model. | **OPEN** | Phase 7.2 consumer evidence through the new path, 7.3 prerequisite audit, and explicit handoff record. |
| C.L | all | The ledger covers every authority, lifecycle, owner, public boundary, deletion target, and performance claim without a Cartesian test matrix. | **OPEN** | Skeptical ledger audit mapping each proof to a plausible defect and independent oracle. |
...
| CQ4 | Critical | Actual graph-obligation authority remains in the monolith rather than destination packages. | **OPEN** | Move actual authority by phase, migrate consumers, then delete the complete monolith tree. |
...
| CQ7 | Critical | Application-query admission does not consume one canonical installed obligation selection before graph-read planning. | **CLOSED** | Application-query admission consumes the installed Phase 1 obligation set through the single Phase 2 selection, requirement, review, reservation, plan, and session spine. |
| CQ8 | Critical | Application-query and authorization observations can occur before the Milestone 9.15 provider session that later consumes their evidence. | **CLOSED** | The managed provider session opens before every decision-bearing observation, and all retained facts are exact-session and typed-branch bound. |
...
| CQ11 | High | Historical consumer-kit/reference-consumer proof can demonstrate local authority construction rather than real public-facade adoption. | **OPEN** | Replace it with hostile external-consumer evidence; remove the obsolete documentation and support surface. |
...
| Surface | Required disposition | Status |
|---|---|---|
| `worth-query/src/runtime/mutation/graph_composition/obligation/` | Move actual authority to destination packages, migrate every consumer, then delete the entire tree. | **OPEN** |
| Selection-backed obligation executors | Delete; actual owner evidence is required. | **OPEN** |
| Manual invariant-pack callbacks and no-op default composition | Delete; installed provider-session invariant execution is the only path. | **OPEN** |
| Public proof-like graph-read planning constructors and executable review | Seal or replace with non-authority inspection. | **OPEN** |
| Monolith admitted graph-read plan and executor | Delete after destination cutover. | **OPEN** |
| Application-query graph-read plan authority and raw Relational execution | Replace with the canonical sealed plan and session-owned port. | **OPEN** |
| Pre-session authorization observation | Delete; all observation begins within the managed session. | **OPEN** |
| Consumer Kit local graph-obligation registration/execution authority | Delete or reduce to non-authoritative public-facade inspection if a real audience remains. | **OPEN** |
| Compatibility exports, aliases, wrappers, and monolith dependencies | Delete; the product is unreleased and has one ordinary path. | **OPEN** |
...
7. one residue inventory that names every obsolete authority and public route.
...
critical finding is `CLOSED`, every deletion row is complete, the hostile
external consumer demonstrates exactly one obligation and graph-read planning
path, and the public docs plus permanent enforcement agree with the production
topology. Closure adds a handoff to Milestone 9.16 Runtime Phase 7.3; it changes
no historical milestone status.

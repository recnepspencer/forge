...
| R7.10 | 7.4 | Delegation is a Query-owned proof-carrying transition in which every child grant is equal to or narrower than its exact current parent across scope, purpose, disclosure, validity, workflow, and downstream delegation posture. | **OPEN** | A lawful multi-link chain plus one-axis action, resource, relation, field, purpose, amount, workflow, validity-start, validity-end, and downstream-posture widening attacks derived through production admission. Q7.55 closes alternate-grant policy-path borrowing. |
...
| R7.23 | 7.4 | Delegated authority retains the exact ordered grant, grantor, grantee, parent, schema, runtime, generation, session, and graph-basis lineage; a copied or alternate-valid chain cannot replace it during re-admission. | **OPEN** | Initial and retained copied-parent, alternate-parent, foreign-runtime, stale-generation, fresh-session, and provenance-substitution hostility with private construction and non-promotion compiler proof. Q7.55 proves every grant-bearing installed path is anchored to the exact selected grant. |
| R7.24 | 7.4 | Every parent grant's active status, freshly sampled trusted-time validity, workflow, resource relationship, grantor/grantee relationship, and installed policy decision remain part of the retained currentness evidence consumed by the canonical operation, governed-query/session, provider-comparison, idempotency, and commit progression boundaries. | **OPEN** | Parent revocation, expiry across a fresh session, workflow drift, resource drift, grantor/grantee drift, alternate-parent substitution, and policy-path drift after lawful admission deny before consumer or effect authority. Phase 7.7 R7.14 remains the sole complete one-shot/continuation/history/preview/live parity cutover. |
| R7.25 | 7.4 | Delegation depth is explicitly installed and identity-bearing; each grant has at most one parent; cycles, excessive depth, and width deny before authority; warm work scales with the exact chain and touched adjacency rather than total grant population. | **OPEN** | Installed-depth identity twins, depth/width/cycle attacks, exact per-link counter slope, unrelated-population equality, and exact-zero warm canonical/SHA/text work. Q7.54 and Q7.56 close hidden anchor scans and unreported selection work. |
| R7.26 | 7.4 | Exact delegation lineage remains private non-reusable Query evidence. Any later portable provenance attachment is downstream descriptive material and cannot construct, replace, or re-admit a grant, lineage, access context, or operation authority. | **OPEN** | Facade/compiler/residue evidence proving private lineage construction, move-only access, no authority constructor from provenance, and no reverse Foundational/Relational/Runtime-Bridge dependency. |
...
| Q7.47 | High | Delegation depth is neither an installed bound nor part of capability identity, and the current limit field is only stored. A deep or cyclic chain has no predeclared work ceiling, while copied zero-limit children are never rejected. | **OPEN** | Install and canonicalize an explicit depth bound, require strict downstream reduction, and deny depth, cycle, and width before access authority. |
| Q7.48 | High | The provenance declaration is currently only an identity-bearing marker. Execution retains no exact ordered chain or affinity, so a descriptive provenance label cannot distinguish the observed parent lineage and no compiler boundary prevents future promotion. | **OPEN** | Private exact lineage must be derived only from current Relational evidence and carried inside the move-only capability fact; public/compiler residue must prove descriptive provenance cannot re-enter authority. |
| Q7.49 | High | The capability decision container can retain only one Relational observation plus one Bridge decision. Adding parent checks as side observations would either drop their freshness, inflate the installed exact fact family, or rely on a cross-file convention that they remain coupled. | **OPEN** | Replace the leaf-only payload with one typed composite capability fact whose currentness, provider conversion, counters, and Bridge-retention checks structurally include the full ordered chain. |
...
| Q7.56 | High | The first Q7.55 repair performs a non-authoritative grant-selection observation before the exact anchored decision, but `authorization.relational_counters()` reports only retained decision evidence. A green locality test could therefore launder selection scans out of the claimed warm work. | **CLOSED** | Exact-grant selection returns its structural counters, Query carries them as preparatory work without currentness authority, and complete admission work has the exact zero/one/two-link slope plus equality across 256 unrelated grants. |
...
### Phase 7.4 restart audit and implementation checkpoint
...
comparator deletion. The complete execution and declaration suites, strict
declaration and execution lint, Relational authorization suite, Worth UI
workspace check, boundary check, and generated-context check pass. R7.10 and
R7.23-R7.26 remain `OPEN` until the final Phase 7.4 certification and residue
review. This checkpoint does not advance 7.5.

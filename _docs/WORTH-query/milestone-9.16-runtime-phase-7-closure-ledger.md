...
**Status:** Open — Phase 7.1 re-proved; Phase 7.2 implementation in progress
...
| R7.2 | 7.1 | Action, resource, relation, field, purpose, amount, cardinality, workflow stage, validity, active grant status, delegation, provenance, and constrained context are explicit identity-bearing dimensions; grant workflow is aligned to an explicit resource-side current field. | **PROVED** | Schema closure requires the typed active-status field/value and both grant/resource workflow fields, enforces their owners and matching value types, and independent status/resource-workflow twins change structured and digest identity. Bank binds `CapabilityGrantStatus::Active`, `CapabilityWorkflowStageField`, and `EstateWorkflowStageField` explicitly. |
...
| R7.4 | 7.1 | Capability meaning is prepared once through Foundational canonical basis and typed sequence digest, while Query installation supplies the stronger authority binding. | **PROVED** | Active status and grant/resource workflow bindings enter the same Foundational structured basis, typed digest slot, and Query authority seal; no parallel identity grammar was added. |
| R7.5 | 7.1 | Capability installation has bounded contract count and canonical bytes; warm installed-capability lookup performs zero basis preparation, hashing, or digest text materialization. | **PROVED** | Exact entry/byte ceilings and all 17 Bank contracts remain inside budget after currentness expansion; the existing 4,096 typed-lookup proof remains exact-zero for basis, digest, and digest-text work. |
...
| R7.7 | 7.2 | Exact authorization decision facts and read sets survive compare-and-commit and every query lane re-admission. | **OPEN** | Relevant drift denies; unrelated drift remains lawful; lane consumers agree. |

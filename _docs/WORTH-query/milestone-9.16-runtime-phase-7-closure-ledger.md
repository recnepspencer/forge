...
| Q7.118 | Critical | Query publication lowers six successful lifecycle transitions, but no production surface publishes the complete required authorization, disclosure, elevation, and review outcome taxonomy. Missing-capability, explicit-policy-denial, purpose, conflict, and separation meaning still collapses into generic or aliased denial; field omission and review required are successful governed outcomes rather than denials; and Foundational denied closeout previously contradicted itself by reporting both `Executed` and `did_execute() == false`. | **OPEN — GATE D** | Retain exact Signal rule outcomes through Runtime Bridge so Query can mint a closed, non-authoritative typed explanation cause at the original decision boundary. Add total publication lowering for missing capability, explicit policy denial, explicit safe scope mismatch, purpose mismatch, conflict, separation of duty, field omission, elevation required/denied/expired, and review required, preserving the original Query outcome and a cross-family/runtime non-aliasing outcome identity without string parsing, graph reconstruction, alternate-scope discovery, protected-value disclosure, or authority promotion. Prove generic Query mismatch/elevation-required outcomes, structured product-reachable ordinary/disclosure/emergency Bank transcripts, caller-inexpressible Bank mismatch dimensions, denied no-effect posture, profile narrowing/widening, and focused non-promotion. |
...
| Q7.122 | Critical | Gate D's original ten-family taxonomy omitted installed explicit policy denial, and the first implementation relabeled `ExplicitDenyRuleMatched` as `MissingCapability`. A caller with exact authority that is explicitly denied would therefore receive a false missing-authority explanation while the retained ordered cause proved otherwise. | **OPEN — GATE D / Q7.118** | Expand the closed taxonomy to eleven families and preserve explicit policy denial as its own exact Query explanation cause and Foundational diagnostic code. Prove absence and explicit-deny twins at Query's real installed-composition boundary plus ordered multi-cause precedence without semantic aliasing; Bank installs no product explicit-deny rule and must not invent one for evidence. |
...
Q7.118 and Q7.122 remain open. Closure still requires a deterministic public
Bank elevation-expired transcript without wall-clock sleeping; public-publisher
profile narrowing/widening and complete category, denial-class, provenance,
receipt, string-independence, and non-promotion evidence; and real
missing-versus-explicit-deny publication twins at the installed Query decision
boundary. The exhaustive eleven-family diagnostic table alone is not accepted
as a substitute for those public causal proofs.

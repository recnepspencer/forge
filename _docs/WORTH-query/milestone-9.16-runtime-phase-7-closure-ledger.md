...
| R7.12 | 7.6 | Emergency elevation is a linear installed typestate with a bounded upper authority and mandatory distinct-actor review. | **OPEN** | Exact active-use admission and the first request transition are implemented. Request authorization consumes one ordinary capability access, the indexed lifecycle owner, the exact authorizing grant, Query-owned time, invariant projection, exact provider effects, and a move-only requested receipt. Approval, close/revoke/expiry, mandatory-review completion, delivery cutoffs, Bank request consumption, and the complete hostile lifecycle remain required. |
...
| Q7.68 | Critical | The first lifecycle-index checkpoint queried the exact lifecycle role during ordinary operation progression but discarded a successful lookup. Once an application supplied a matching lifecycle input and operation program, the ordinary authorization and compare-and-commit APIs could have progressed the transition without its dedicated typestate. | **CLOSED** | Ordinary operation progression now denies every indexed lifecycle operation as `ElevationTransitionRequired`, and ordinary compare-and-commit independently denies any request-bound program. Only `authorize_elevation_request`, exact request-program materialization, and `compare_and_commit_elevation_request` form the request path. Two causal bypass tests exercise both cutoffs. |
| Q7.69 | Critical | The initial lifecycle model stored `active`, `review-required`, and `reviewed` beside elevation state while also storing mandatory-review `required`/`completed`, creating competing posture authorities; it also accepted an arbitrary caller duration with no installed maximum. | **CLOSED** | Persisted elevation state is now only requested/approved/expired/revoked; active is derived from approved plus Query-owned time, and review required/completed lives only on the review entity. The installed, canonical maximum duration is nonzero, exactly representable on its timeline, and enforced before request progression. Declaration identity/closure tests and zero/overlong runtime request tests prove the correction. Later approval/review transitions must preserve this model and will reopen the row if they introduce another posture owner. |

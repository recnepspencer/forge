...
| R7.12 | 7.6 | Emergency elevation is a linear installed typestate with a bounded upper authority and mandatory distinct-actor review. | **OPEN** | Exact request, approval, approved active use, Query-derived revoked/expired close, and mandatory distinct-actor review transitions are implemented through indexed lifecycle ownership, ordinary installed capability composition, exact invariant facts and effects, the ordinary provider boundary, and move-only receipts. Delivery cutoffs, Bank consumption, forbidden-disbursement evidence, and the complete consumer-hostile lifecycle remain required. |
...
| Q7.70 | Critical | The first request binder resolved the proposed upper-bound grant only as an entity of the installed grant kind. A requester could therefore persist an elevation linked to a different, revoked, future, or unrelated grant even though another exact grant supplied the request authority. | **CLOSED** | The request binder now requires the proposed grant entity to equal the exact grant retained by the ordinary request-capability decision. A hostile world with two same-kind grants proves that the non-authorizing grant is rejected before lifecycle operation authority or effects exist. |
...
| Q7.73 | Critical | Bank lifecycle capability composition currently requires output actor relationships such as approver and reviewer as preconditions even though the lifecycle transitions are responsible for creating those relationships. That makes the consumer transition circular or impossible. | **OPEN** | The generic approval contract distinguishes input requester evidence from the approver relation it writes. The Bank Phase 7.6 consumer and Phase 7.7 cutover must migrate the corresponding request/approval/review composition and prove each transition from its true pre-state. |
...
| Q7.77 | High | The first terminal fixture draft accepted the mandatory-review identity again in close and review inputs even though the move-only receipt already owned it. That proxy could disagree with the receipt and created an unused competing selector. | **CLOSED** | Close and review inputs select only the exact elevation required by installed actor composition. Query derives the review identity exclusively from the consumed approved or mandatory-review receipt, while invariant projection must resolve and reobserve that exact review before either specialized program can materialize. |

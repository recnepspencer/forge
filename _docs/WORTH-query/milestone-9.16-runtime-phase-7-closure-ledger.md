...
| R7.12 | 7.6 | Emergency elevation is a linear installed typestate with a bounded upper authority and mandatory distinct-actor review. | **OPEN** | Exact request, approval, approved active use, Query-derived revoked/expired close, and mandatory distinct-actor review transitions are implemented through indexed lifecycle ownership, ordinary installed capability composition, exact invariant facts and effects, the ordinary provider boundary, and move-only receipts. Delivery cutoffs, forbidden-disbursement evidence, and the complete generic consumer-hostile lifecycle remain required. The real Bank consumer belongs exclusively to Phase 7.7. |
...
| Q7.80 | High | The generic lane suites prove ordinary capability revocation and current authorization over pinned data, but no production-backed query consumes approved-elevation authority across continuation, historical, preview, and live delivery. Terminal lifecycle tests could therefore remain green if elevated authorization were dropped or pinned before a consumer result. | **OPEN** | Requires one elevation-governed installed query and adversarial current-truth cutoffs after cursor, historical plan, preview plan, and live lease admission, including Query-time expiry and a queued live cause. |

...
| R7.24 | 7.4 | Every parent grant's active status, trusted-time validity, workflow, resource relationship, grantor/grantee relationship, and installed policy decision remain part of the retained currentness evidence consumed by operation, query, continuation, history, preview, live, idempotency, and commit progression. | **OPEN** | Parent revocation, expiry, workflow drift, resource drift, relationship drift, and policy-path drift after a lawful admission deny before consumer or effect authority. |
...
| Q7.46 | High | `ApplicationCapabilityScope::is_within` is a public descriptive policy engine unused by production execution, while Bank owns a second comparator with different delegation strictness. These surfaces can disagree with the installed Query transition and become competing narrowing authorities. | **OPEN** | Migrate their independent one-axis evidence to production-derived admission, retain only an explicitly non-authoritative test oracle where needed, and remove the ordinary public comparator after consumer-residue proof. |

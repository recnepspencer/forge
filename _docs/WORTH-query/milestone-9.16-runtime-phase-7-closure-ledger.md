...
| R7.7 | 7.2 | Exact authorization decision facts and read sets can be retained and revalidated at a later snapshot without rebuilding policy meaning, substituting an equivalent grant/path, or granting reusable authority. | **DEFECT — Q7.50** | Principal and policy freshness are typed separately, provider fact cardinality is installed and exact, and equivalent grant and composition-path replacement deny. Phase 7.4 source audit found that transfer to a fresh graph-work session reuses the retained trusted-time sample instead of resampling it. Q7.50 must close before this row can return to `PROVED`. Full lane consumption remains R7.14. |
...
| Q7.46 | High | `ApplicationCapabilityScope::is_within` is a descriptive Query policy engine unused by production execution and able to disagree with the installed Query transition. | **OPEN** | Migrate its useful one-axis evidence to production-derived admission and delete the displaced declaration comparator plus its tests after consumer-residue proof. Bank's later consumer cutover is tracked separately by Q7.52 and R7.13. |
...
| Q7.50 | Critical | Capability transfer to a fresh graph-work session selects `retained_time_sample()` instead of sampling the Query-owned clock. A grant or parent that expires between access admission and governed-query admission can therefore remain current under the earlier timestamp. | **OPEN** | Remove the retained-time fallback, require a fresh trusted sample at every privileged re-admission, and prove an exact parent expiry between access and governed-query sessions denies before result or effect authority. Reopen R7.7 and R7.24 while this finding is open. |
...
| Q7.53 | Medium | Moving Query's descriptive comparator behind `cfg(test)` left two dead public-shaped accessors. Strict `cargo clippy -p worth-query-declaration --all-targets -- -D warnings` is red, and a full parallel test policy engine remains after production admission became authoritative. | **OPEN** | Delete the displaced comparator and its tests after the production one-axis family covers the relevant dimensions; rerun strict declaration Clippy and residue search. |
...
The restart QA found three additional defects or ledger omissions:
...
R7.10 and R7.23-R7.26 remain `OPEN`. Closure still requires amount and related
relationship narrowing, fresh-session expiry, parent grantor and policy-path
drift, exact root/one-link/two-link work slope, unrelated-population equality,
final comparator deletion and residue, and the focused plus constitutional
verification named by those rows. This checkpoint does not advance 7.5.

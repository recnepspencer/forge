...
**Status:** Open — Phase 7.1 validity basis reopened; Phase 7.2 implementation in progress
...
| R7.2 | 7.1 | Action, resource, relation, field, purpose, amount, cardinality, workflow stage, validity timeline, active grant status, delegation, provenance, and constrained context are explicit identity-bearing dimensions; grant workflow is aligned to an explicit resource-side current field. | **OPEN** | Re-prove schema closure, canonical identity, and Bank installation after adding the installed validity timeline. |
...
| R7.4 | 7.1 | Capability meaning is prepared once through Foundational canonical basis and typed sequence digest, while Query installation supplies the stronger authority binding. | **OPEN** | Re-prove structured and digest identity after the validity timeline enters the canonical basis and authority seal. |
| R7.5 | 7.1 | Capability installation has bounded contract count and canonical bytes; warm installed-capability lookup performs zero basis preparation, hashing, or digest text materialization. | **OPEN** | Re-run exact entry/byte ceilings and warm lookup evidence after the canonical currentness expansion. |
...
| R7.16 | 7.2 | Relational owns current graph observation, Signal owns installed boolean composition, Runtime Bridge owns correspondence, and Query alone combines those proofs into attempt authority; no layer silently recreates another layer's decision. | **OPEN** | Neutral Relational evidence, nested Signal rule proof, Bridge correspondence hostility, Query residue review, and compiler-inaccessible authority construction. |
| R7.17 | 7.2 | Capability plans compile at the cold application-runtime publication seam; warm admission performs typed lookup plus touched-graph work and cannot recompile or rehash installed meaning as unrelated graph population grows. | **OPEN** | Cold compilation counters, exact-zero warm canonical/digest/text counters, and unrelated-population scale probes. |
...
| Q7.12 | Critical | Relational currently emits its own flat allow/deny decision, Signal and Runtime Bridge understand only flat paths, and application-query admission drops the resulting evidence. The installed capability AND-of-OR grammar therefore has no single proof-carrying execution authority. | **OPEN** | Make Relational observation neutral, install nested Signal/Bridge composition, retain the evidence in Query authority, and remove Query-local or discarded-decision paths. |

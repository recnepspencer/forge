...
Store integration authority
  ...
  resolution_session_record/     durable decisions and workflow checkpoints
...
still decides whether the resolved plan can prepare. Store persists session
records. Query exposes participant and operator workflows. A persisted role or
...
model. Earn `JoinedClose` only after the replacement Store runtime durably
recovers sessions, decisions, and pins without promoting persisted authority.

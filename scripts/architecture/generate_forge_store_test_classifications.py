#!/usr/bin/env python3
"""Classify unresolved Forge Store integration tests by permanent semantics."""
...
OUT_DIR = ROOT / "_docs/forge-store/topology-classifications"
FIELDS = (
...
def domain_for(value: str, crate: str) -> str:
    lowered = value.lower()
    if crate == "forge-store-layout-indexes" or phase_boundary(lowered) or any(token in lowered for token in ("s8", "layout", "btree", "lsm", "index")):
        return "layout"
...
def classify(row: dict[str, str], content_hash: str, batch: str) -> dict[str, str]:
    ...
    if crate == "forge-store-io-scheduler":
        action = "delete"
        target_dir = ""
        name = ""
    ...
    elif crate == "forge-store-test-support":
        target_crate = "forge-store-physical-certification"
    ...
    elif crate == "forge-store-certification" and stale_certification_test(normalized):
        action = "delete"
        target_dir = ""
        name = ""
    ...
    target_path = ""
    if action == "move":
        target_path = f"workspaces/forge-store/crates/{target_crate}/{target_dir}/{name}"
    return {
...
def selections() -> list[tuple[str, Path, list[dict[str, str]]]]:
    ...
    groups = (
        ("014-certification-tests", "014-certification-tests.csv", lambda r: r["crate"] == "forge-store-certification"),
        ("015-layout-indexes-tests", "015-layout-indexes-tests.csv", lambda r: r["crate"] == "forge-store-layout-indexes"),
        ("016-remaining-tests-and-support", "016-remaining-tests-and-support.csv", lambda r: r["crate"] not in {"forge-store-certification", "forge-store-layout-indexes"}),
    )

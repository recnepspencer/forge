#!/usr/bin/env python3
"""Generate the reviewable Forge Store topology migration inventory."""
...
WORKSPACE_PREFIX = PurePosixPath("workspaces/forge-store")
DEFAULT_OUTPUT = REPO_ROOT / "_docs/forge-store/forge-store-topology-inventory.csv"
CLASSIFICATION_DIR = REPO_ROOT / "_docs/forge-store/topology-classifications"
...
def locate_crate(relative: PurePosixPath) -> tuple[str, PurePosixPath]:
    ...
    if len(parts) >= 4 and parts[:3] == ("workspaces", "forge-store", "crates"):
        return parts[3], PurePosixPath(*parts[4:])
    return "forge-store-workspace", PurePosixPath(*parts[2:])
...
def target_for(crate: str, crate_relative: PurePosixPath, role: str) -> tuple[str, str | None]:
    ...
    if crate == "forge-store-layout-indexes" and subsystem in LAYOUT_INDEX_TARGETS:
    ...
    if crate == "forge-store-certification" and subsystem == "crate_root" and filename not in {"lib.rs", "public_api.rs", "internal_modules.rs"}:
...
def classify(path: Path) -> dict[str, str | int]:
    ...
    if milestone_named:
        ...
        action = "move"
        candidate = "move"
        confidence = "high" if crate != "forge-store-certification" else "medium"
        review = "false"
    ...
    if action == "move" and target_directory:
        target_path = f"workspaces/forge-store/crates/{crate}/src/{target_directory}/{crate_relative.name}"

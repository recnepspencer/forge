#!/usr/bin/env python3
"""Generate the reviewed Worth Store test-support production topology batch."""
...
OUTPUT = ROOT / "_docs/worth-store/topology-classifications/013-test-support-production.csv"
BATCH = "013-test-support-production"
CRATE = "worth-store-test-support"
PREFIX = f"workspaces/worth-store/crates/{CRATE}/src/"
FIELDS = (
...
def target(relative: str) -> tuple[str, str, str, str, str]:
    ...
    if relative.startswith(security_prefix):
        ...
        return "move", "worth-store-physical-certification", "drivers/security", "scope_execution.rs", "Move executable security simulation from test support to the physical harness."
    ...
    if relative.startswith(layout_prefix):
        if name == "executed_runtime.rs":
            return "move", "worth-store-physical-certification", "drivers/layout", "access_runtime.rs", "Move production-facade execution into the physical certification harness."
...
def render() -> str:
    ...
    for row in rows:
        ...
        target_path = f"workspaces/worth-store/crates/{crate}/src/{directory}/{name}" if action == "move" else ""
        classified.append({

#!/usr/bin/env python3
"""Generate the reviewed Forge Store certification production batch."""
...
OUTPUT = ROOT / "_docs/forge-store/topology-classifications/011-certification-production.csv"
BATCH = "011-certification-production"
PREFIX = "workspaces/forge-store/crates/forge-store-certification/src/"
...
def load_rows() -> list[dict[str, str]]:
    with INVENTORY.open(newline="", encoding="utf-8") as source:
        return [
            ...
            if row["crate"] == "forge-store-certification"
...
def harness_target(relative: str, domain: str) -> tuple[str, str, str] | None:
    ...
    if lowered.startswith("courtroom/harness/test_support/"):
        ...
        return "forge-store-physical-certification", target_dir, clean_name(path.name)
    if lowered.startswith("s4_recovery_harness/certification/"):
        target_dir = "courtroom/recovery/harness"
        return "forge-store-certification", target_dir, clean_name(path.name)
    ...
    if lowered == "s4_recovery_harness_exports.rs":
        return "forge-store-physical-certification", "scenarios/recovery", "facade.rs"
...
def classify(row: dict[str, str], hashes: dict[str, str]) -> dict[str, str]:
    ...
    action = "move"
    target_crate = "forge-store-certification"
    ...
    if action in {"move", "keep"}:
        target_path = f"workspaces/forge-store/crates/{target_crate}/src/{target_dir}/{name}"
        if target_dir == "src":
            target_path = f"workspaces/forge-store/crates/{target_crate}/src/{name}"

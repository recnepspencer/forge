...
OUTPUT = ROOT / "_docs/worth-store/topology-classifications/012-physical-certification-production.csv"
BATCH = "012-physical-certification-production"
CRATE = "worth-store-physical-certification"
...
def row_target(relative: str) -> tuple[str, str, str, str, str]:
    ...
    if lowered.startswith("s7_closeout/"):
        if name in {"denial.rs", "mod.rs"}:
            return "merge", "worth-store-certification", "courtroom/blobs/closeout", name, "Merge duplicate closeout vocabulary into the courtroom-owned blob closeout family."
        return "move", "worth-store-certification", "courtroom/blobs/closeout", name, "Move closeout judgment from the reusable harness into the certification courtroom."
...
def render() -> str:
    ...
    for row in rows:
        ...
        if action in {"move", "keep", "merge"}:
            target = f"workspaces/worth-store/crates/{crate}/src/{directory}/{name}"

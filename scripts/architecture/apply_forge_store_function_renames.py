#!/usr/bin/env python3
"""Apply the reviewed Forge Store function rename inventory."""
...
INVENTORY = REPO_ROOT / "_docs/forge-store/forge-store-function-rename-inventory.csv"
...
def apply_overloaded_renames(files: dict[Path, str]) -> None:
    isolation = CRATES_ROOT / "forge-store-io-scheduler/src/admission/isolation.rs"
    ...
    complexity = CRATES_ROOT / "forge-store-physical-format/src/binary_format/operation_complexity.rs"
    ...
    replay = CRATES_ROOT / "forge-store-physical-format/src/facade/replay_artifact.rs"
    ...
    reopen = CRATES_ROOT / "forge-store-physical-format/src/facade/reopen.rs"
    text = files[reopen]
...
def main() -> None:
    ...
    readiness = CRATES_ROOT / "forge-store-readiness/src/physical_integrity/readiness.rs"

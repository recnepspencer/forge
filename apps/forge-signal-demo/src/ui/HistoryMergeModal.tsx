  ...
  const executeMerge = async () => {
    ...
    try {
      ...
      if (choices.some((choice) => choice === "target")) {
        onUnavailable("Forge Forms captured mixed conflict choices, but this runtime only exposes source-resolution or reject-shared-state merge execution.");
        setBusy(false);
        return;
      }
    ...
    <div className="history-merge-layer" role="presentation">
      ...
      <div className="history-merge-modal" role="dialog" aria-modal="true" aria-label="Merge branches">
        <div className="history-merge-header">
          ...
          <h3>{phase === "pick" ? "Choose two branches" : "Resolve conflicts with Forge Forms"}</h3>
        </div>

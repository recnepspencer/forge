...
function HistoryWorkbench({ runtime }: { runtime: GearRuntime }) {
  ...
  const controls = (closeButton: boolean) => (
    <>
      <p className="history-kicker">Forge runtime state</p>
...
export function HistorySection({ onNavigate }: HistorySectionProps) {
  ...
  useEffect(() => {
    ...
    createSignals({ deployment: "mainThreadCompatibility" })
      .then((signals) => {
        if (disposed) return;
        const forgeSignals = signals as unknown as SignalsFacade;
        setRuntime({
          signals: forgeSignals,
          history: forgeSignals.history(),
          inputs: {
            innerRadius: forgeSignals.input(initialGearParams.innerRadius, { debugName: "landing.gear.innerRadius" }),
            outerRadius: forgeSignals.input(initialGearParams.outerRadius, { debugName: "landing.gear.outerRadius" }),
            thickness: forgeSignals.input(initialGearParams.thickness, { debugName: "landing.gear.thickness" }),
            teeth: forgeSignals.input(initialGearParams.teeth, { debugName: "landing.gear.teeth" }),
          },
        });
      })
    ...
    <div className="xai-section-band accent-history history-section">
      <div className="xai-section-heading">
        ...
        <p>
          ...
          snapshots. This gear is just Three.js; the timeline is Forge
          <code> history()</code>.
        </p>
      </div>

...
function ForgePanel({ baseMs, highlightId, onClaim, onController }: PanelProps) {
  const store = useMemo(() => createPoServer(), []);
  ...
  useEffect(() => {
    if (!forge || initRef.current) return;
    initRef.current = true;
    forge.line.refresh();
    void forge.line.awaitSettlement().then(() => {
  ...
  const controller = useMemo<PanelController>(() => ({
    addLine: (poLine) => {
      ...
      forge.line.patch(forge.linesFamily.patch.insert({
        itemId: draft.id,
        placement: "append",
        nextItem: draft,
      }));
      ...
      void execution.settled().then((result: any) => {
        if (result?.resultKind === "rejected") {
          ...
          forge.line.patch(forge.linesFamily.patch.delete({ itemId: draft.id }));
          pushLedger(
          ...
          pushLedger(
            ...
            "delivered item patch consumed canonical server truth — other pending rows preserved",
            {
              ...
              lastEffect: summarizeEffectEnvelope(forge.line.diagnostics().lastEffect),
            },
          );
        }
        pulse();
  ...
  const exportLedger = useCallback(() => {
    ...
    try {
      lifecycle = forge.line.history().lifecycle.map((entry: any) => ({
        event: entry.event,
...
export function ResourcesSection({ onNavigate }: ResourcesSectionProps) {
  ...
  const settleBoth = useCallback((lineId: string, accepted: boolean) => {
    ...
    forgeController.current?.settle(lineId, accepted);
  }, []);
  ...
  const startFirstLine = useCallback(() => {
    ...
    forgeController.current?.addLine(LINE_RISKY);
  ...
  const startSecondLine = useCallback(() => {
    ...
    forgeController.current?.addLine(LINE_SAFE);
    setPhase("overlap");
    schedule(() => {
  ...
  const resetBoth = useCallback(() => {
    ...
    void forgeController.current?.reset();
    ...
    <div className="accent-resources po-section">
      ...
      <div className="po-grid">
        ...
        <ForgePanel
          ...
          highlightId={highlightId}
          onClaim={handleClaim}
          onController={(controller) => { forgeController.current = controller; }}
        />
      </div>

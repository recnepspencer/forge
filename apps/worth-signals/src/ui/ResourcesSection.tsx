...
import { ResourcesForgePanel } from "./ResourcesForgePanel";
...
export function ResourcesSection({ onNavigate }: ResourcesSectionProps) {
  ...
  const playCollision = useCallback(() => {
    ...
    void forgeController.current?.addLine(LINE_RISKY);
    setPhase("arming");
    schedule(() => {
  ...
  const runTenRequests = useCallback(async () => {
    ...
    for (const request of scenario.requests) {
      ...
      admissions.push(
        ...
        forgeController.current?.addLine(request.line, options),
      );
    }
    ...
    <div className="accent-resources po-section">
      ...
      <div className="signals-docs-row">
        <button onClick={() => onNavigate("#/docs/resources/effects/concurrency-and-dependencies")} type="button">
          Read concurrency and dependencies <span aria-hidden="true">→</span>
        </button>
      </div>
    </div>
  );

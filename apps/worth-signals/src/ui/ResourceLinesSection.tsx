...
import { createReactSignalsStore, useResourceLine } from "forge-signal-wasm/react";
...
function buildForge(signals: SignalsRuntime, server: StorefrontServer): StorefrontForge {
  const api = (signals as unknown as {
    api: (options: Record<string, unknown>) => {
      url: (path: string) => {
        detail: (declaration: Record<string, unknown>) => StorefrontForge["productFamily"];
        response: (contract: unknown) => {
          list: (declaration: Record<string, unknown>) => StorefrontForge["catalogFamily"];
        };
      };
    };
  }).api({
  ...
  const catalogLine = (catalogFamily as { line: (params: Record<string, never>) => StorefrontForge["catalogLine"] }).line({});
...
function StorefrontWorkbench({ forge }: { forge: StorefrontForge }): React.ReactElement {
  const reactStore = React.useMemo(() => createReactSignalsStore(forge.signals), [forge]);
  ...
  React.useEffect(() => {
    ...
    if (forge.catalogLine.value() === null) {
      forge.catalogLine.refresh();
      schedulePulse();
    }
  }, [forge, schedulePulse]);
  ...
  const deliverServerPush = React.useCallback(
    (packet: ServerPushPacket, targetId: string) => {
      const targetLine = forge.productFamily.line({ productId: targetId });
      const send = (basisId: string | null): { kind?: string; expectedBasisId?: string | null } =>
        targetLine.deliver(
          forge.productFamily.delivery.field({
  ...
  React.useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      const packet = forge.server.pushChange(FOCUS_PRODUCT_ID, "price");
    ...
    <>
      <section className="rl-catalog-strip" aria-label="Catalog line">
        ...
        <span className="rl-catalog-meta">
          {forge.catalogLine.history().lifecycle.length} recorded events · its own recorder, its own truth
        </span>
      </section>
        ...
        <article className={`rl-product-card${ambush ? " is-alarmed" : ""}`}>
          ...
          {revealed ? (
            <div className="rl-action-row">
              ...
              <button
                onClick={() => {
                  const packet = forge.server.pushChange(selectedId, Math.random() > 0.5 ? "price" : "inventory");
...
export function ResourceLinesSection({ onNavigate }: ResourceLinesSectionProps): React.ReactElement {
  const [forge, setForge] = React.useState<StorefrontForge | null>(null);
  ...
  React.useEffect(() => {
    ...
    createSignals({ deployment: "mainThreadCompatibility" })
      .then((signals) => {
        if (!active) return;
        setForge(buildForge(signals, createStorefrontServer()));
      })
      .catch((error: unknown) => {
  ...
  return (
    <div className="accent-resources rl-section">
      ...
      {!forge && !bootError ? <div className="signals-runtime-message">Connecting to the Worth runtime…</div> : null}
      {forge ? <StorefrontWorkbench forge={forge} /> : null}

...
export function ResourcesForgePanel({
  baseMs,
  highlightId,
  onAgreement,
  onController,
  ...
  const bumpDag = useCallback(() => {
    ...
    for (const entry of admittedEffects.current) {
      ...
      const summary = forge.line.effects().get(entry.effectId) as EffectSummaryLike | null;
  ...
  useEffect(() => {
    if (!forge) return;
    void forge.line.awaitSettlement().then(() => bumpDag());
  }, [forge, bumpDag]);

  const lineSignal = useMemo(() => (forge ? forge.line.signal() : null), [forge]);
  const watchedValue = useSignal<readonly PoLine[] | null>(signals as unknown, lineSignal);
  const lines = forge ? ((forge.line.value() as readonly PoLine[] | null) ?? watchedValue ?? null) : null;
  const loading = !forge || (!lines && !bootError);
  ...
  const selectedReceipt = useMemo(() => {
    if (!forge || !selectedEffectId) return null;
    return createRuntimeReceipt(
      forge.line.effects(),
      selectedEffectId,
      settlementSnapshot.get(selectedEffectId) ?? null,
    );
  }, [forge, selectedEffectId, settlementSnapshot]);
  ...
  const persistEffect = useCallback(async (draft: PoLine, effectId: string) => {
    ...
    try {
      ...
      const settlement = await forge.line.effects().confirm(effectId, {
        responseId: `demo:${draft.id}:confirmed`,
        serverPatch: forge.linesFamily.patch.insert({
          itemId: saved.id,
          placement: "append",
          nextItem: saved,
        }),
      ...
      const terminal = forge.line.effects().get(effectId)?.terminal;
      ...
      const settlement = await forge.line.effects().reject(effectId, {
  ...
  }, [bumpDag, cancelRetiredDependents, forge, store]);
  ...
  const controller = useMemo<PanelController>(() => ({
    addLine: (poLine, options = {}) => {
      ...
      const admission = (async () => {
        ...
        const insert = forge.linesFamily.patch.insert({
          itemId: draft.id,
          placement: "append",
          nextItem: draft,
        });
        const patch = parentId
          ...
          : insert;
        const result = await forge.line.patch(patch, {
    ...
    reset: async () => {
      if (forge) {
        const openEffects = [...forge.line.effects().open()].reverse();
        for (const effect of openEffects) {
          if (forge.line.effects().get(effect.effectId)?.lifecycle === "Retired") continue;
          await forge.line.effects().reject(effect.effectId, {
      ...
      if (!forge) return;
      forge.line.invalidate();
      forge.line.refresh();
      const settlement = await forge.line.awaitSettlement({ timeoutMs: 5_000 });
      if (settlement.resultKind !== "fulfilled") {
        throw new TypeError(`Worth reset reload ended as ${settlement.resultKind}`);
      }
      const value = forge.line.value() as readonly PoLine[] | null;
  ...
  }), [bumpDag, forge, persistEffect, store]);
  ...
  const convergence = useMemo<ConvergenceFacts | null>(() => {
    if (phase !== "batchSettled" || !forge) return null;
    const counters = forge.line.effects().counters() as { openEffectCount?: number };
    return {
  ...
  }, [agreement?.kind, dagLanes, forge, phase]);
  ...
  const exportReceipts = useCallback(() => {
    ...
    const artifact = {
      ...
      projection: forge.line.effects().projection(),
      counters: forge.line.effects().counters(),
      visibleValue: forge.line.value(),
    };
